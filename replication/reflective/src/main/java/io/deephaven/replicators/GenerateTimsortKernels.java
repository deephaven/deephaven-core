...
public class GenerateTimsortKernels {
        ...
        KeyColumn singleKey() {
            if (keys.size() != 1) {
                throw new UnsupportedOperationException(
                        "multi-column key kernels are not implemented yet; add tuple comparison and per-column "
                                + "temporaries to the emitters");
            }
            return keys.get(0);
        }
    ...
    private static void generate(final KernelSpec spec) throws IOException {
        final TypeSpec type = new KernelEmitter(spec).emit();
        final JavaFile.Builder fileBuilder = JavaFile.builder(PACKAGE, type).indent("    ");
        ...
        System.out.println("Generating " + spec.className() + " to " + SOURCE_ROOT);
