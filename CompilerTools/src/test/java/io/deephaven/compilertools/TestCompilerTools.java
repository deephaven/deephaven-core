...
public class TestCompilerTools {
    ...
    // Two nearly-identical classes, so we can get an idea of how long it takes to compile one of
    // them
    static {
        final StringBuilder testClassCode1 =
            new StringBuilder("        public class $CLASSNAME$ {");
        ...
        for (int i = 0; i < NUM_METHODS; i++) {
            testClassCode1.append("            public static void testMethod").append(i)
                .append(" (String [] args) {");
    ...
    public void testParallelCompile() throws Throwable {
        ...
        // Use a unique value added to the class name to guarantee unique classes, in case
        // workspaces aren't cleared out correctly
        final long startTimeOffset = System.currentTimeMillis();

        // Get a baseline estimate of compilation time, ignoring the first run as it's typically
        // much longer
        ...
        for (long i = 0; i < NUM_COMPILE_TESTS; i++) {
            ...
            System.out.println(printMillis(endTimeTest) + ": compile test of " + testClassName
                + " took " + compileTestMillis + " millis");
            if (i > 0) {
        ...
        System.out.println("Average compile time millis: " + averageCompileTime + ", delay will be "
            + waitStartMillis + " millis");

        final String className = "TestClass" + startTimeOffset + NUM_COMPILE_TESTS;
        System.out.println(
            printMillis(System.currentTimeMillis()) + ": starting test with class " + className);
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            ...
            threads[i] = new Thread(() -> {
                try {
                    final long delay =
                        fi == 0 ? 0 : fi * WAIT_BETWEEN_THREAD_START_MILLIS + waitStartMillis;
                    final long startTime = System.currentTimeMillis();
                    compile(fi == 0, className);
                    final long endTime = System.currentTimeMillis();
                    System.out.println(printMillis(endTime) + ": thread " + fi
                        + " completed with specified delay=" + delay
        ...
        synchronized (raisedThrowables) {
            System.out.println(raisedThrowables.size() +
                (raisedThrowables.size() == 1 ? " exception was raised"
    ...
    private void compile(boolean printDetails, final String className) throws Exception {
        ...
        if (printDetails) {
            ...
            System.out.println(printMillis(endMillis) + ": Thread 0 ending compile: ("
                + (endMillis - startMillis) + ") millis elapsed");
        }
    }
    ...
    public void testSimpleCompile() throws Exception {
        ...
        final Class<?> clazz1 = CompilerTools.compile("Test", program1Text, "com.deephaven.test",
            codeLog, Collections.emptyMap());
    ...
    public void testCollidingCompile() throws Exception {
        ...
        for (int i = 0; i < 100; ++i) {
            Thread t = new Thread(() -> {
                StringBuilder codeLog = new StringBuilder();
                try {
                    final Class<?> clazz1 = CompilerTools.compile("Test", program1Text,
                        "com.deephaven.test", codeLog, Collections.emptyMap());
                    final Method m1 = clazz1.getMethod("main", String[].class);
                    Object[] args1 = new Object[] {new String[] {"hello", "there"}};
                    m1.invoke(null, args1);
                } catch (Exception e) {
