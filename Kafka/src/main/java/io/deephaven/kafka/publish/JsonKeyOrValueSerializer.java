...
public class JsonKeyOrValueSerializer implements KeyOrValueSerializer {
    ...
    private <T> void putNullable(final ObjectNode node, final String field, final T value,
            final BiConsumer<String, T> putter) {
        if (value == null) {
            if (outputNulls) {
                node.putNull(field);
            }
    ...
    protected void makeFieldProcessor(final String fieldName, final String columnName) {
        ...
        if (byte.class.equals(src.getType())) {
            ...
            fieldProcessors.put(fieldName,
                    new JSONFieldProcessor(fieldName, (node, field, idx, isRemoval) -> putNullable(node, field,
                            isRemoval ? src.getPrevBoolean(idx) : src.getBoolean(idx), node::put)));
        } else if (BigDecimal.class.equals(src.getType())) {
            fieldProcessors.put(fieldName,
                    new JSONFieldProcessor(fieldName, (node, field, idx, isRemoval) -> putNullable(node, field,
                            isRemoval ? (BigDecimal) src.getPrev(idx) : (BigDecimal) src.get(idx), node::put)));
        } else if (BigInteger.class.equals(src.getType())) {
            fieldProcessors.put(fieldName,
                    new JSONFieldProcessor(fieldName, (node, field, idx, isRemoval) -> putNullable(node, field,
                            isRemoval ? (BigInteger) src.getPrev(idx) : (BigInteger) src.get(idx), node::put)));
        } else {
    ...
    private JSONFieldProcessor makeTimestampProcessor(final String fieldName) {
        return new JSONFieldProcessor(fieldName,
                (node, field, idx, isRemoval) -> node.put(field, String.valueOf(DBDateTime.now().getNanos())));
    }
