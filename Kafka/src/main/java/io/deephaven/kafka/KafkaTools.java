...
public class KafkaTools {
        ...
        try (final CloseableHttpClient client = HttpClients.custom().build()) {
            ...
            if (statusCode != HttpStatus.SC_OK) {
                throw new UncheckedDeephavenException(
                    "Got status code " + statusCode + " requesting " + request);
            }
        ...
        switch (fieldType) {
            ...
            case UNION:
                ...
                if (unionType1 == Schema.Type.NULL) {
                    pushColumnTypesFromAvroField(
                        columnsOut, mappedOut, prefix, field, fieldName, fieldSchema, mappedName,
                        unionType0, fieldNameToColumnName);
                    return;
                } else if (unionType0 == Schema.Type.NULL) {
                    pushColumnTypesFromAvroField(
                        columnsOut, mappedOut, prefix, field, fieldName, fieldSchema, mappedName,
                        unionType1, fieldNameToColumnName);
                    return;
                }
                throw new UnsupportedOperationException("Union " + fieldName
                    + " not supported; only unions with NULL are supported at this time.");
            case RECORD:
            ...
            default:
                throw new UnsupportedOperationException(
                    "Type " + fieldType + " not supported for field " + fieldName);
        }
        if (mappedOut != null) {
    ...
    public static abstract class KeyOrValueSpec {
        ...
        public static final class Avro extends KeyOrValueSpec {
            ...
            private Avro(final Schema schema,
                final Function<String, String> fieldNameToColumnName) {
                this.schema = schema;
        ...
        /**
         * The names for the key or value columns can be provided in the properties as
         * "key.column.name" or "value.column.name", and otherwise default to "key" or "value". The
         * types for key or value are either specified in the properties as "key.type" or
         * "value.type", or deduced from the serializer classes for key or value in the provided
         * Properties object.
         */
    ...
    public enum TableType {
        /**
         * <p>
         * Consume all partitions into a single interleaved stream table, which will present only
         * newly-available rows to downstream operations and visualizations.
         * <p>
         * See {@link Table#STREAM_TABLE_ATTRIBUTE} for a detailed explanation of stream table
         * semantics, and {@link io.deephaven.db.v2.StreamTableTools} for related tooling.
         */
        Stream(false, false),
        /**
         ...
         * <p>
         * The resulting per-partition tables are aggregated into a single {@link TableMap} keyed by
         * {@link Integer} partition, which is then presented as a {@link Table} proxy via
         * {@link TransformableTableMap#asTable(boolean, boolean, boolean) asTable} with
         * {@code strictKeys=true}, {@code allowCoalesce=true}, and {@code sanityCheckJoins=true}.
         * <p>
         * See {@link TransformableTableMap#asTableMap()} to explicitly work with the underlying
         * {@link TableMap} and {@link TransformableTableMap#asTable(boolean, boolean, boolean)} for
         * alternative proxy options.
         */
        StreamMap(false, true),
        /**
         * <p>
         * As in {@link #Append}, but each partition is mapped to a distinct in-memory append-only
         * table.
         * <p>
         * The resulting per-partition tables are aggregated into a single {@link TableMap} keyed by
         * {@link Integer} partition, which is then presented as a {@link Table} proxy via
    ...
    /**
     * The types for key or value are either specified in the properties as "key.type" or
     * "value.type", or deduced from the serializer classes for key or value in the provided
     * Properties object.
     */
    @SuppressWarnings("unused")
     ...
     *
     * @param kafkaConsumerProperties Properties to configure this table and also to be passed to
     *        create the KafkaConsumer
     * @param topic Kafka topic name
     * @param partitionFilter A predicate returning true for the partitions to consume
     * @param partitionToInitialOffset A function specifying the desired initial offset for each
     *        partition consumed
     ...
     * @return The result table containing Kafka stream data formatted according to
     *         {@code resultType}
     */
    @SuppressWarnings("unused")
    public static Table consumeToTable(
        ...
        final KeyOrValueIngestData keyIngestData =
            getIngestData(KeyOrValue.KEY, kafkaConsumerProperties, columnDefinitions,
                nextColumnIndexMut, keySpec);
        final KeyOrValueIngestData valueIngestData =
            getIngestData(KeyOrValue.VALUE, kafkaConsumerProperties, columnDefinitions,
                nextColumnIndexMut, valueSpec);

        final TableDefinition tableDefinition = new TableDefinition(columnDefinitions);

        final StreamTableMap streamTableMap =
            resultType.isMap ? new StreamTableMap(tableDefinition) : null;
        ...
        final Supplier<Pair<StreamToTableAdapter, ConsumerRecordToStreamPublisherAdapter>> adapterFactory =
            () -> {
                ...
                streamPublisher.setChunkFactory(
                    () -> streamToTableAdapter.makeChunksForDefinition(CHUNK_SIZE),
                ...
                return new Pair<>(
                    ...
                    KafkaStreamPublisher.make(
                        ...
                        keyIngestData == null ? Function.identity()
                            : keyIngestData.toObjectChunkMapper,
                        valueIngestData == null ? Function.identity()
                            : valueIngestData.toObjectChunkMapper));
            };

        final UnaryOperator<Table> tableConversion =
            resultType.isAppend ? StreamTableTools::streamToAppendOnlyTable
        ...
        if (resultType.isMap) {
            ...
            partitionToConsumer = (final int partition) -> {
                ...
                final Table partitionTable =
                    tableConversion.apply(partitionAdapterPair.getFirst().table());
                streamTableMap.enqueueUpdate(
                    () -> Assert.eqNull(streamTableMap.put(partition, partitionTable),
                        "streamTableMap.put(partition, partitionTable)"));
                return new SimpleKafkaStreamConsumer(partitionAdapterPair.getSecond(),
            ...
            partitionToConsumer = (final int partition) -> new SimpleKafkaStreamConsumer(
                singleAdapterPair.getSecond(), singleAdapterPair.getFirst());
        }
    ...
    private static void setDeserIfNotSet(final Properties prop, final KeyOrValue keyOrValue,
        final String value) {
    ...
    private static KeyOrValueIngestData getIngestData(
        ...
        switch (keyOrValueSpec.dataFormat()) {
            case AVRO:
                setDeserIfNotSet(kafkaConsumerProperties, keyOrValue,
                ...
                if (avroSpec.schema != null) {
                    ...
                    final String schemaServiceUrl =
                        kafkaConsumerProperties.getProperty(SCHEMA_SERVER_PROPERTY);
                    schema = getAvroSchema(schemaServiceUrl, avroSpec.schemaName,
                        avroSpec.schemaVersion);
                }
                avroSchemaToColumnDefinitions(
                    columnDefinitions, data.fieldNameToColumnName, schema,
                    avroSpec.fieldNameToColumnName);
                data.extra = schema;
                break;
            case JSON:
                ...
                if (jsonSpec.fieldNameToColumnName != null) {
                    for (final Map.Entry<String, String> entry : jsonSpec.fieldNameToColumnName
                        .entrySet()) {
            ...
            case SIMPLE:
                ...
                if (simpleSpec.dataType == null) {
                    colDef = getKeyOrValueCol(keyOrValue, kafkaConsumerProperties,
                        simpleSpec.columnName, false);
                } else {
                    colDef = ColumnDefinition.fromGenericType(simpleSpec.columnName,
                        simpleSpec.dataType);
                }
                final String propKey = (keyOrValue == KeyOrValue.KEY)
                    ? ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG
                ...
                if (!kafkaConsumerProperties.containsKey(propKey)) {
                    ...
                    if (dataType == short.class) {
                        ...
                        throw new UncheckedDeephavenException(
                            "Deserializer for " + keyOrValue
                                + " not set in kafka consumer properties " +
            ...
            default:
                throw new IllegalStateException(
                    "Unhandled spec type:" + keyOrValueSpec.dataFormat());
        }
        return data;
    }
    ...
    private static ColumnDefinition<?> getKeyOrValueCol(
        ...
        if (columnNameArg != null) {
            ...
            if (columnName == null || columnName.equals("")) {
                ...
                throw new IllegalArgumentException(
                    "Property for " + nameDefault + " can't be empty.");
            }
        } else {
    ...
    private static ColumnDefinition<? extends Serializable> columnDefinitionFromDeserializer(
        ...
        throw new IllegalArgumentException(
            "Deserializer type " + deserializer + " for " + deserializerProperty
                + " not supported.");
    }
    ...
    public static final IntToLongFunction ALL_PARTITIONS_DONT_SEEK =
        KafkaIngester.ALL_PARTITIONS_DONT_SEEK;
    @SuppressWarnings("unused")
    public static final IntToLongFunction ALL_PARTITIONS_SEEK_TO_END =
        KafkaIngester.ALL_PARTITIONS_SEEK_TO_END;
    @SuppressWarnings("unused")
    ...
    public static IntToLongFunction partitionToOffsetFromParallelArrays(
        ...
        final TIntLongHashMap map =
            new TIntLongHashMap(partitions.length, 0.5f, 0, KafkaIngester.DONT_SEEK);
