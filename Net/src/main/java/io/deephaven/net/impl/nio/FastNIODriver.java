...
public final class FastNIODriver implements Runnable {
    ...
    public static Scheduler[] createSchedulers(String name, String property, Logger log, Configuration config) {
        ...
        for (int i = 0; i < numSchedulers; ++i) {
            schedulers[i] = createDrivers(name + "-" + i, log, threadsPerScheduler, threadsPerScheduler, timeoutsOrSpins, spinsUntilPark, false, doTimingStats, doSpinSelect).getScheduler();
        }
        return schedulers;
    }

    public static FastNIODriver createDrivers(String name, Logger log, int initialThreads, int maxThreads, long workTimeout, int spinsUntilPark, boolean crashOnMax) {
        return createDrivers(name, log, initialThreads, maxThreads, workTimeout, spinsUntilPark, crashOnMax, true, false);
    }

    public static FastNIODriver createDrivers(String name, Logger log, int initialThreads, int maxThreads, long workTimeout, int spinsUntilPark, boolean crashOnMax, boolean doTimingStats, boolean doSpinSelect) {
        ...
        log.info().append(name).append(": Starting FastNIODriver Scheduler: threads: ").append(initialThreads)
                ...
                .endl();
        try {
            final Scheduler scheduler = new YASchedulerImpl(name, NioUtil.reduceSelectorGarbage(Selector.open()), log, doTimingStats, doSpinSelect);
            ...
            for (int i = 0; i < initialThreads; ++i) {
                threads[i] = createNewThread(name, scheduler, mutex, shutdown, workTimeout, created, destroyed, available, maxThreads, crashOnMax);
            }
            for (int i = 0; i < initialThreads; ++i) {
                threads[i].start();
            }
    ...
    private static InternalThread createNewThread(final String name, final Scheduler scheduler, final UnfairMutex mutex, final AtomicBoolean shutdown, final long workTimeout, final AtomicInteger created, final AtomicInteger destroyed, final AtomicInteger available, final int maxThreads, final boolean crashOnMax) {
        InternalThread t = new InternalThread(new FastNIODriver(name, scheduler, mutex, shutdown, workTimeout, created, destroyed, available, maxThreads, crashOnMax));
        t.setDaemon(true);
    ...
    private FastNIODriver(final String name, final Scheduler scheduler, final UnfairMutex mutex, final AtomicBoolean shutdown, final long workTimeout, final AtomicInteger created, final AtomicInteger destroyed, final AtomicInteger available, final int maxThreads, final boolean crashOnMax) {
            ...
            public void call() {
                if (!alreadyHandedOff) {
                    if (shouldCreate()) {
                        ...
                        createNewThread(name, scheduler, mutex, shutdown, workTimeout, created, destroyed, available, maxThreads, crashOnMax).start();
                    }
                    mutex.unlock();
        ...
        if (available.get() == 0) {
            ...
            if (created.get() == maxThreads) {
                if (crashOnMax) {
                    ...
                    CommBase.signalFatalError("FastNIODriver: exceeded maximum thread pool limit: "+summary(), new Throwable());
                }
                return false;
            }
            return true;
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                available.getAndDecrement();
                 do { scheduler.work(workTimeout, mutexUnlockHandoff); } while ( mutex.getOwner() == me );
                available.getAndIncrement();

            } catch (Throwable x) {
                throwable = x;
                shutdown.set(true);
                scheduler.installJob(new TimedJob() { public void timedOut() { }  }, 0); // wake us up yo
                mutexUnlockHandoff.call(); // we aren't sure whether the scheduler.work has already called the handoff or not yet, so go ahead and call it (it won't double release it)
        ...
        if (throwable == null) {
            ...
            log.fatal(throwable).append("Thread ").append(me.getName()).append(" is terminating on a fatal exception: ").append(summary()).endl();
        }

        if (throwable != null) CommBase.signalFatalError("Unhandled throwable from FastNIODriver scheduler", throwable);
    }
    ...
    public boolean shutdown(long maxWait) {
        shutdown.set(true);
        scheduler.installJob(new TimedJob() { public void timedOut() { }  }, 0);
