...
public class FastDoubleParserFromByteArray {
    ...
    /**
     * Special value in {@link #CHAR_TO_HEX_MAP} for characters that are neither a hex digit nor a decimal point
     * character..
     */
    ...
    private static NumberFormatException newNumberFormatException(byte[] str, int off, int len) {
        if (len > 1024) {
            ...
            return new NumberFormatException(
                    "For input string: \"" + new String(str, off, len, StandardCharsets.ISO_8859_1) + "\"");
        }
    }
     ...
     *
     * @param str the string to be parsed, a byte array with characters in ISO-8859-1, ASCII or UTF-8 encoding
    ...
    /**
     * Returns a Double object holding the double value represented by the argument string {@code str}.
     * <p>
     * This method can be used as a drop in for method {@link Double#valueOf(String)}. (Assuming that the API of this
     * method has not changed since Java SE 16).
     * <p>
     * Leading and trailing whitespace characters in {@code str} are ignored. Whitespace is removed as if by the
     * {@link String#trim()} method; that is, characters in the range [U+0000,U+0020].
     * <p>
     ...
     */
    private static double parseRestOfDecimalFloatLiteral(byte[] str, int endIndex, int index, boolean isNegative,
            boolean hasLeadingZero, int off) {
        // Parse digits
        // ------------
        ...
        double result = FastDoubleMath.decFloatLiteralToDouble(index, isNegative, digits, exponent, virtualIndexOfPoint,
                exp_number, isDigitsTruncated, skipCountInTruncatedDigits);
    ...
    private static double parseRestOfHexFloatingPointLiteral(
        ...
        double d = FastDoubleMath.hexFloatLiteralToDouble(index, isNegative, digits, exponent, virtualIndexOfPoint,
                exp_number, isDigitsTruncated, skipCountInTruncatedDigits);
