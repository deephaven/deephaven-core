/*
 * @(#)FastDoubleMath.java Copyright © 2021. Werner Randelshofer, Switzerland. MIT License.
 */
 ...
 * <p>
 * The code contains enhancements from Daniel Lemire's fast_float_parser, so that it can parse double Strings with very
 * long sequences of numbers
 * <p>
 * References:
 * <dl>
 * <dt>Daniel Lemire, fast_double_parser, 4x faster than strtod. Apache License 2.0 or Boost Software License.</dt>
 * <dd><a href="https://github.com/lemire/fast_double_parser">github.com</a></dd>
 *
 * <dt>Daniel Lemire, fast_float number parsing library: 4x faster than strtod. Apache License 2.0.</dt>
 * <dd><a href="https://github.com/fastfloat/fast_float">github.com</a></dd>
 *
 * <dt>Daniel Lemire, Number Parsing at a Gigabyte per Second, Software: Practice and Experience 51 (8), 2021.
...
class FastDoubleMath {
    /**
     * The smallest non-zero float (binary64) is 2^−1074. We take as input numbers of the form w x 10^q where w < 2^64.
     * We have that {@literal w * 10^-343 < 2^(64-344) 5^-343 < 2^-1076}.
     * <p>
     * However, we have that {@literal (2^64-1) * 10^-342 = (2^64-1) * 2^-342 * 5^-342 > 2^−1074}. Thus it is possible
     * for a number of the form w * 10^-342 where w is a 64-bit value to be a non-zero floating-point number.
     * <p>
     * ********
     * <p>
     * If we are solely interested in the *normal* numbers then the smallest value is 2^-1022. We can generate a value
     * larger than 2^-1022 with expressions of the form w * 10^-326. Thus we need to pick FASTFLOAT_SMALLEST_POWER >=
     * -326.
     * <p>
     * ********
     * <p>
     * Any number of form w * 10^309 where w>= 1 is going to be infinite in binary64 so we never need to worry about
     * powers of 5 greater than 308.
     */
     ...
     * effectively we are trying to match w * 2^q * 5^q to m * 2^p. Thus the powers of two are not a concern since they
     * can be represented exactly using the binary notation, only the powers of five affect the binary significand.
     * <p>
     * <p>
     * The mantissas of powers of ten from -308 to 308, extended out to sixty four bits. The array contains the powers
     * of ten approximated as a 64-bit mantissa. It goes from 10^FASTFLOAT_SMALLEST_POWER to 10^FASTFLOAT_LARGEST_POWER
     * (inclusively). The mantissa is truncated, and never rounded up. Uses about 5KB.
     */
    ...
    static double decFloatLiteralToDouble(int index, boolean isNegative, long digits, int exponent,
            int virtualIndexOfPoint, long exp_number, boolean isDigitsTruncated, int skipCountInTruncatedDigits) {
        if (digits == 0) {
            return isNegative ? -0.0 : 0.0;
        }
        final double outDouble;
        if (isDigitsTruncated) {
            final long exponentOfTruncatedDigits =
                    virtualIndexOfPoint - index + skipCountInTruncatedDigits + exp_number;
            ...
            if (FASTFLOAT_DEC_SMALLEST_POWER <= exponentOfTruncatedDigits
                ...
                double withoutRounding =
                        tryDecToDoubleWithFastAlgorithm(isNegative, digits, (int) exponentOfTruncatedDigits);
                double roundedUp =
                        tryDecToDoubleWithFastAlgorithm(isNegative, digits + 1, (int) exponentOfTruncatedDigits);
    ...
    static double hexFloatLiteralToDouble(int index, boolean isNegative, long digits, long exponent,
            int virtualIndexOfPoint, long exp_number, boolean isDigitsTruncated, int skipCountInTruncatedDigits) {
        if (digits == 0) {
            return isNegative ? -0.0 : 0.0;
        }
    ...
    /**
     * Attempts to compute {@literal digits * 10^(power)} exactly; and if "negative" is true, negate the result.
     * <p>
     * This function will only work in some cases, when it does not work it returns null. This should work *most of the
     * time* (like 99% of the time). We assume that power is in the [FASTFLOAT_SMALLEST_POWER, FASTFLOAT_LARGEST_POWER]
    ...
    /**
     * Attempts to compute {@literal digits * 2^(power)} exactly; and if "negative" is true, negate the result.
     * <p>
