...
public class ArrowVarCharVectorTest {
    private static final List<Long> expectedRows = Arrays.asList(0L, 1L, 2L, 4L, 8L, 9L);

    private static final String[] expectedValues = new String[] {"my", "vector", null, "it", "is", "success"};
    ...
    private static QueryTable loadTable() {
        ...
        final File dataFile = new File(ArrowVarCharVectorTest.class.getResource("/var_char_vector.arrow").getFile());
    ...
    public void testReadArrowFile() {
        ...
        final ColumnSource<String> cs = (ColumnSource<String>)table.getColumnSources().stream().findFirst().get();
    ...
    public void testFillChunk() {
        ...
        try (final ChunkSource.FillContext fillContext = cs.makeFillContext(table.intSize());
            final WritableObjectChunk<String, Values> chunk = WritableObjectChunk.makeWritableChunk(table.intSize())) {
