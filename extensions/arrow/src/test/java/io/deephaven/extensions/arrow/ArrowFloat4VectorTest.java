...
public class ArrowFloat4VectorTest {
    private static final List<Long> expectedRows = Arrays.asList(0L, 1L, 2L, 4L, 8L, 9L);

    private static final float[] expectedValues = new float[] {78.786F, 2.9F, QueryConstants.NULL_FLOAT, 13.2F, 0F, 21.01F};
    ...
    private static QueryTable loadTable() {
        ...
        final File dataFile = new File(ArrowFloat4VectorTest.class.getResource("/float4_vector.arrow").getFile());
    ...
    public void testReadArrowFile() {
        ...
        final ColumnSource<Float> cs = (ColumnSource<Float>)table.getColumnSources().stream().findFirst().get();

        ArrowWrapperTools.Shareable.resetNumBlocksLoaded();
        final MutableInt pos = new MutableInt();
        table.getRowSet().forAllRowKeys(rowKey -> Assert.assertEquals(expectedValues[pos.getAndIncrement()], cs.getFloat(rowKey), 1e-5));
    ...
    public void testFillChunk() {
        ...
        try (final ChunkSource.FillContext fillContext = cs.makeFillContext(table.intSize());
            final WritableFloatChunk<Values> chunk = WritableFloatChunk.makeWritableChunk(table.intSize())) {
