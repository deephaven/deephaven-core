...
public class ArrowIntVectorTest {
    private static final List<Long> expectedRows = Arrays.asList(0L, 1L, 2L, 4L, 8L, 9L);

    private static final int[] expectedValues = new int[] {76428765, 9480636, QueryConstants.NULL_INT, 164366688, -1575247918, -23126};
    ...
    public void testReadArrowFile() {
        ...
        final ColumnSource<Integer> cs = (ColumnSource<Integer>)table.getColumnSources().stream().findFirst().get();

        ArrowWrapperTools.Shareable.resetNumBlocksLoaded();
        final MutableInt pos = new MutableInt();
        table.getRowSet().forAllRowKeys(rowKey -> Assert.assertEquals(expectedValues[pos.getAndIncrement()], cs.getInt(rowKey)));
    ...
    public void testFillChunk() {
        ...
        try (final ChunkSource.FillContext fillContext = cs.makeFillContext(table.intSize());
            final WritableIntChunk<Values> chunk = WritableIntChunk.makeWritableChunk(table.intSize())) {
