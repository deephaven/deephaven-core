...
public class ArrowFixedSizeBinaryVectorTest {
    private static final List<Long> expectedRows = Arrays.asList(0L, 1L, 2L, 4L, 8L, 9L);

    private static final byte[][] expectedValues = new byte[][] {"hi".getBytes(), "it".getBytes(), null, "is".getBytes(), "me".getBytes(), "!!".getBytes()};
