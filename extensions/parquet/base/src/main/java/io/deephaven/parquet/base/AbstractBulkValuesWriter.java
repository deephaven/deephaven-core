...
public abstract class AbstractBulkValuesWriter<BUFFER_TYPE, VALUE_TYPE> extends ValuesWriter
    ...
    public int writeBulkVector(@NotNull final BUFFER_TYPE bulkValues,
        ...
        final IntBuffer nullsOffsets = writeBulkFilterNulls(bulkValues, nullValue, nonNullValueCount).nullOffsets;
        return applyDlAndRl(vectorSizes, rlEncoder, dlEncoder, nullsOffsets);
    }

    /**
     * <p>Compute the definition levels and repetition levels.  These are how Parquet encodes data that has a complex
     * structure.  In this case, the most complex structure we encode are Deephaven Vectors.</p><br/>
     *
     * <p>Definition and Repetition levels are used to define how the complex structure is encoded into multiple
     * columns.  Repetition levels define the number of common prefix elements an item shares with it's immediate
     * predecessor, where the value 0 represents the root node of a single instance of a structure.  The best way
     * to think about this is to understand how it is read back - see
     * <a href="https://storage.googleapis.com/pub-tools-public-publication-data/pdf/36632.pdf">Dremel Paper</a>
     * pgs 3 and 4.</p><br/>
     *
     * <p>Basically, take the data structure being encoded and draw a Finite State Machine.  If a repetition level less
     * than the depth of that element is read, it means that the value belongs to a different instance of the parent
     * item, and processing should advance to the next field.  If the value is allowed to be repeated, and the RL is
     * equal to the current field depth then keep reading values from the same field.</p><br/>
     *
     * <p>In our case,  we are storing Vectors, which are only this</p>
     * <pre>
     * message Vector {
     ...
     * </pre>
     *
     * <p>So we can have a repetition level of 0 or 1, where 0 indicates the first element of a new vector
     * and 1 indicates another entry in the current vector.</p>
     *
     * <p>Definition levels are used to encode how many preceeding optional fields are defined at a given point.  For
     * Vectors, as defined above,  this gives us three different possible values for the definition value of a single
     * item:
     * <ul>
     *     0 - The array itself at this position is a null
     *     1 - The array exists, but is empty
     *     2 - The element at this position in the array is null
     *     3 - The value in the array exists and is non null
     * </ul>
     * </p>
     *
