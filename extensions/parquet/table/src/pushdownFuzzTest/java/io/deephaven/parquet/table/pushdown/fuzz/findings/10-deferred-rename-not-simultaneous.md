...
**Jira:** [DH-23611](https://deephaven.atlassian.net/browse/DH-23611)
...
- **Some name must be reassigned before a *later* column reads it.** This is the trigger as DH-23611
  states it: "a destination name that equals the source name of a rename appearing later in
  definition order". A collision with an *earlier* column is harmless — that column read the name
  before it was reassigned — so the check requires `cj > ci`. Without the hazard at all, `view` is
  already correct, and behaviour is untouched for every ordinary rename.
