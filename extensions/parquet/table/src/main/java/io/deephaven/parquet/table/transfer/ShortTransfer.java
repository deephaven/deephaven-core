...
public class ShortTransfer implements TransferObject<IntBuffer> {

    private final ColumnSource<?> columnSource;
    private final ChunkSource.GetContext context;
    private final IntBuffer buffer;
    private ShortChunk<? extends Values> chunk;
    private short minValue = QueryConstants.NULL_SHORT;
    private short maxValue = QueryConstants.NULL_SHORT;
    ...
    final public void fetchData(@NotNull final RowSequence rs) {
        chunk = columnSource.getChunk(context, rs).asShortChunk();
    }
    ...
    private void copyAllFromChunkToBuffer() {
        for (int chunkIdx = 0; chunkIdx < chunk.size(); ++chunkIdx) {
            short value = chunk.get(chunkIdx);
            if (value != QueryConstants.NULL_SHORT) {
                if (minValue == QueryConstants.NULL_SHORT) {
