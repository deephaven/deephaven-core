...
public final class ParquetColumnRegionChar<ATTR extends Any> extends ParquetColumnRegionBase<ATTR>
    ...
    public PushdownResult performPushdownAction(
        ...
        if (ctx.matchFilter() != null) {
            try (final RowSet matches = CharRegionBinarySearchKernel.binsearchMatchFilter(
        ...
        if (ctx.rangeFilter() instanceof CharRangeFilter) {
            try (final RowSet matches = CharRegionBinarySearchKernel.binsearchRangeFilter(
                    this, firstKey, lastKey, sortColumn, (CharRangeFilter) ctx.rangeFilter());
