 ...
 * {@link #maybeOverlaps(ShortRangeFilter, Statistics)} serves range filters, which need no preparation.
 * <p>
 ...
 * <ul>
 * <li>A <b>stored sentinel</b> -- a value equal to {@code NULL_SHORT} -- is this class's business. To Parquet it is an
...
final class ShortPushdownHandler {
    ...
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final WhereFilter filter) {
        if (filter instanceof ShortRangeFilter) {
            return maybeCreateEvaluator((ShortRangeFilter) filter);
        }
    ...
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final ShortRangeFilter shortRangeFilter) {
        ...
        final boolean unboundedBelow = dhLower == QueryConstants.NULL_SHORT;
        ...
        return statistics -> {
            ...
            if (unboundedBelow) {
                if (mutableMin.get() <= QueryConstants.NULL_SHORT
                        && QueryConstants.NULL_SHORT <= mutableMax.get()) {
