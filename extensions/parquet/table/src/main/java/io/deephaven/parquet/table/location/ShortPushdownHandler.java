...
/**
 * Applies a {@link ShortRangeFilter} or a short-typed {@link MatchFilter} to one row group's {@code min}/{@code max}
 ...
 * a value equal to {@code NULL_SHORT} -- is this class's business. To Parquet it is an ordinary value, sitting inside
...
final class ShortPushdownHandler {
    ...
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final WhereFilter filter) {
        if (filter instanceof final ShortRangeFilter shortRangeFilter) {
            return maybeCreateEvaluator(shortRangeFilter);
        }
     ...
     */
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final ShortRangeFilter shortRangeFilter) {
        final short dhLower = shortRangeFilter.getLower();
        final short dhUpper = shortRangeFilter.getUpper();
        final boolean lowerInclusive = shortRangeFilter.isLowerInclusive();
        final boolean upperInclusive = shortRangeFilter.isUpperInclusive();
        ...
        return statistics -> {
            final short[] minMax = decodeMinMax(statistics);
     ...
     */
    @Nullable
    private static short[] decodeMinMax(@NotNull final Statistics<?> statistics) {
        final short[] minMax = new short[2];
        if (!MinMaxFromStatistics.getMinMaxForShorts(statistics, v -> minMax[0] = v, v -> minMax[1] = v)) {
            return null;
        }
        return minMax;
    }
