...
/**
 * Applies a {@link IntRangeFilter} or a int-typed {@link MatchFilter} to one row group's {@code min}/{@code max}
...
final class IntPushdownHandler {
    ...
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final WhereFilter filter) {
        if (filter instanceof final IntRangeFilter intRangeFilter) {
            return maybeCreateEvaluator(intRangeFilter);
        }
     ...
     */
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final IntRangeFilter intRangeFilter) {
        final int dhLower = intRangeFilter.getLower();
        final int dhUpper = intRangeFilter.getUpper();
        final boolean lowerInclusive = intRangeFilter.isLowerInclusive();
        final boolean upperInclusive = intRangeFilter.isUpperInclusive();
        ...
        return statistics -> {
            final int[] minMax = decodeMinMax(statistics);
     ...
     */
    @Nullable
    private static int[] decodeMinMax(@NotNull final Statistics<?> statistics) {
        final int[] minMax = new int[2];
        if (!MinMaxFromStatistics.getMinMaxForInts(statistics, v -> minMax[0] = v, v -> minMax[1] = v)) {
            return null;
        }
        return minMax;
    }
