...
final class IntPushdownHandler {
    ...
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final WhereFilter filter) {
        if (filter instanceof IntRangeFilter) {
            return maybeCreateEvaluator((IntRangeFilter) filter);
        }
    ...
    static StatisticsEvaluator maybeCreateEvaluator(@NotNull final IntRangeFilter intRangeFilter) {
        ...
        final boolean unboundedBelow = dhLower == QueryConstants.NULL_INT;
        ...
        return statistics -> {
            ...
            if (unboundedBelow) {
                if (mutableMin.get() <= QueryConstants.NULL_INT
                        && QueryConstants.NULL_INT <= mutableMax.get()) {
