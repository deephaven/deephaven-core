...
public final class ParquetColumnRegionFloat<ATTR extends Any> extends ParquetColumnRegionBase<ATTR>
    ...
    public PushdownResult performPushdownAction(
        ...
        if (ctx.matchFilter() != null) {
            try (final RowSet matches = FloatRegionBinarySearchKernel.binsearchMatchFilter(
        ...
        if (ctx.rangeFilter() instanceof FloatRangeFilter) {
            try (final RowSet matches = FloatRegionBinarySearchKernel.binsearchRangeFilter(
                    this, firstKey, lastKey, sortColumn, (FloatRangeFilter) ctx.rangeFilter());
