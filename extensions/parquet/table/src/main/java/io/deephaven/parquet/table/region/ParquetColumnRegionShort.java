...
public final class ParquetColumnRegionShort<ATTR extends Any> extends ParquetColumnRegionBase<ATTR>
    ...
    public PushdownResult performPushdownAction(
        ...
        if (ctx.matchFilter() != null) {
            try (final RowSet matches = ShortRegionBinarySearchKernel.binsearchMatchFilter(
        ...
        if (ctx.rangeFilter() instanceof ShortRangeFilter) {
            try (final RowSet matches = ShortRegionBinarySearchKernel.binsearchRangeFilter(
                    this, firstKey, lastKey, sortColumn, (ShortRangeFilter) ctx.rangeFilter());
