...
public class PublishToKafka<K, V> extends LivenessArtifact {
     ...
     * {@link PeriodicUpdateGraph#DEFAULT PeriodicUpdateGraph} by holding either its
     * {@link PeriodicUpdateGraph#exclusiveLock() exclusive lock} or its {@link PeriodicUpdateGraph#sharedLock() shared
     * lock}. The publisher will install a listener in order to produce new records as updates become available. Callers
     * must be sure to maintain a reference to the publisher and ensure that it remains
