...
class PyIcebergTestUtils {
    static final Table EXPECTED_DATA;

    static {
        // sorting on multiple columns compiles a sort kernel, which requires an ExecutionContext with a
        // QueryCompiler; this class is initialized from tests that do not otherwise open one
        try (final SafeCloseable ignored = TestExecutionContext.createForUnitTests().open()) {
            EXPECTED_DATA = TableTools.newTable(
