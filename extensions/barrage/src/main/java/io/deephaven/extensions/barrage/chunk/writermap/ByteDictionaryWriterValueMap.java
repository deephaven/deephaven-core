...
final class ByteDictionaryWriterValueMap implements DictionaryWriterValueMap {

    private final Byte2IntOpenHashMap valueToIndex = new Byte2IntOpenHashMap();
    private final ByteArrayList values = new ByteArrayList();

    ByteDictionaryWriterValueMap() {
        valueToIndex.defaultReturnValue(-1);
    }

    private int getOrAdd(final byte value) {
        ...
        if (subset == null) {
            ...
            if (useDeephavenNulls) {
                ...
                for (int i = 0; i < size; ++i) {
                    ...
                    out.set(outPos++, v == QueryConstants.NULL_BYTE ? QueryConstants.NULL_INT : getOrAdd(v));
                }
            }
        } else {
            if (useDeephavenNulls) {
    ...
    public WritableChunk<Values> buildChunk(final int fromInclusive, final int toExclusive) {
        ...
        final WritableByteChunk<Values> out = WritableByteChunk.makeWritableChunk(numValues);
