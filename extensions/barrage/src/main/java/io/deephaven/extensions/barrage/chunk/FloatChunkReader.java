...
public class FloatChunkReader extends BaseChunkReader<WritableFloatChunk<Values>> {
    ...
    public FloatChunkReader(BarrageOptions options) {
    ...
    private static void useDeephavenNulls(
        ...
        if (is instanceof BarrageProtoUtil.BarrageDataInputStream) {
            // Read the payload in bounded windows, decoding each float from the little-endian bytes as we go, so a
            ...
            for (int ei = 0; ei < numElements;) {
                ...
                final byte[] payload = lis.readRawBytes(n * Float.BYTES);
                for (int jj = 0; jj < n; ++jj) {
                    chunk.set(offset + ei + jj, Float.intBitsToFloat((int) LITTLE_ENDIAN_INT.get(payload, jj * Float.BYTES)));
                }
                ei += n;
            }
        } else {
