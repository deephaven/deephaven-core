...
public class ByteChunkWriter<SOURCE_CHUNK_TYPE extends Chunk<Values>> extends BaseChunkWriter<SOURCE_CHUNK_TYPE> {
    ...
    public static ChunkWriter<ObjectChunk<Byte, Values>> makeBoxed(
            @NotNull final ChunkWriter<ByteChunk<Values>> innerWriter) {
        return new ChunkWriter<>() {
            @Override
            public Context makeContext(@NotNull final ObjectChunk<Byte, Values> chunk, final long rowOffset) {
