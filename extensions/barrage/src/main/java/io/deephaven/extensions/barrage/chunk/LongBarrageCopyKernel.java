...
public class LongBarrageCopyKernel {
     ...
     * then never name.
     */
    private static WritableLongChunk<Values>[][] asTypedChunks(final WritableChunk<Values>[][] chunks) {
        // noinspection unchecked
        final WritableLongChunk<Values>[][] typed = new WritableLongChunk[chunks.length][];
        for (int di = 0; di < chunks.length; ++di) {
            if (chunks[di] == null) {
                continue;
            }
            // noinspection unchecked
            typed[di] = new WritableLongChunk[chunks[di].length];
            for (int ci = 0; ci < chunks[di].length; ++ci) {
                typed[di][ci] = chunks[di][ci].asWritableLongChunk();
            }
        }
        return typed;
    }
    ...
    private static class LongBarrageCopyKernelImpl implements BarrageCopyKernel {
        ...
        public void copy(
            ...
            final WritableLongChunk<Values>[][] typedAddChunks = asTypedChunks(addChunks);
            final WritableLongChunk<Values>[][] typedModChunks = asTypedChunks(modChunks);
            ...
            for (int ri = 0; ri < runs.count; ++ri) {
                ...
                final WritableLongChunk<Values>[] originChunks =
