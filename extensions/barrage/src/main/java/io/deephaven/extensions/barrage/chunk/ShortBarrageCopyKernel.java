...
public class ShortBarrageCopyKernel {
     ...
     * then never name.
     */
    private static WritableShortChunk<Values>[][] asTypedChunks(final WritableChunk<Values>[][] chunks) {
        // noinspection unchecked
        final WritableShortChunk<Values>[][] typed = new WritableShortChunk[chunks.length][];
        for (int di = 0; di < chunks.length; ++di) {
            if (chunks[di] == null) {
                continue;
            }
            // noinspection unchecked
            typed[di] = new WritableShortChunk[chunks[di].length];
            for (int ci = 0; ci < chunks[di].length; ++ci) {
                typed[di][ci] = chunks[di][ci].asWritableShortChunk();
            }
        }
        return typed;
    }
    ...
    private static class ShortBarrageCopyKernelImpl implements BarrageCopyKernel {
        ...
        public void copy(
            ...
            final WritableShortChunk<Values>[][] typedAddChunks = asTypedChunks(addChunks);
            final WritableShortChunk<Values>[][] typedModChunks = asTypedChunks(modChunks);
            ...
            for (int ri = 0; ri < runs.count; ++ri) {
                ...
                final WritableShortChunk<Values>[] originChunks =
