...
public class DoubleBarrageCopyKernel {
     ...
     * then never name.
     */
    private static WritableDoubleChunk<Values>[][] asTypedChunks(final WritableChunk<Values>[][] chunks) {
        // noinspection unchecked
        final WritableDoubleChunk<Values>[][] typed = new WritableDoubleChunk[chunks.length][];
        for (int di = 0; di < chunks.length; ++di) {
            if (chunks[di] == null) {
                continue;
            }
            // noinspection unchecked
            typed[di] = new WritableDoubleChunk[chunks[di].length];
            for (int ci = 0; ci < chunks[di].length; ++ci) {
                typed[di][ci] = chunks[di][ci].asWritableDoubleChunk();
            }
        }
        return typed;
    }
    ...
    private static class DoubleBarrageCopyKernelImpl implements BarrageCopyKernel {
        ...
        public void copy(
            ...
            final WritableDoubleChunk<Values>[][] typedAddChunks = asTypedChunks(addChunks);
            final WritableDoubleChunk<Values>[][] typedModChunks = asTypedChunks(modChunks);
