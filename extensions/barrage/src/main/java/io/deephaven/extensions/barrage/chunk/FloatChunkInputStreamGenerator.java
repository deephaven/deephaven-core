...
public class FloatChunkInputStreamGenerator extends BaseChunkInputStreamGenerator<FloatChunk<Values>> {
    ...
    public static <T> FloatChunkInputStreamGenerator convertWithTransform(
        ...
        for (int i = 0; i < inChunk.size(); ++i) {
            ...
            outChunk.set(i, value == null ? QueryConstants.NULL_FLOAT : transform.applyAsFloat(value));
        }
        if (inChunk instanceof PoolableChunk) {
            ((PoolableChunk) inChunk).close();
        }
    ...
    static WritableChunk<Values> extractChunkFromInputStream(
        ...
        return extractChunkFromInputStreamWithConversion(
                elementSize, options, FloatConversion.IDENTITY, fieldNodeIter, bufferInfoIter, is, outChunk, outOffset,
                totalRows);
    }
    ...
    static <T> WritableObjectChunk<T, Values> extractChunkFromInputStreamWithTransform(
        ...
        try (final WritableFloatChunk<Values> inner = extractChunkFromInputStream(
                elementSize, options, fieldNodeIter, bufferInfoIter, is, null, 0, 0).asWritableFloatChunk()) {
            ...
            for (int ii = 0; ii < inner.size(); ++ii) {
                ...
                chunk.set(outOffset + ii, value == NULL_FLOAT ? null : transform.apply(value));
            }
