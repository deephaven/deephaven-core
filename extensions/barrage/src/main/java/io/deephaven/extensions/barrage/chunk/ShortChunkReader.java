...
public class ShortChunkReader extends BaseChunkReader<WritableShortChunk<Values>> {
    ...
    // Reads a little-endian short from a byte[] at a byte offset in a single (possibly unaligned) load.
    ...
    private static void useDeephavenNulls(
        ...
        if (is instanceof BarrageProtoUtil.BarrageDataInputStream) {
            // Read the payload in bounded windows, decoding each short from the little-endian bytes as we go, so a
            ...
            for (int ei = 0; ei < numElements;) {
                ...
                final byte[] payload = lis.readRawBytes(n * Short.BYTES);
                for (int jj = 0; jj < n; ++jj) {
                    chunk.set(offset + ei + jj, (short) LITTLE_ENDIAN_SHORT.get(payload, jj * Short.BYTES));
                }
                ei += n;
            }
        } else {
