...
public class IntBarrageCopyKernel {
    ...
    private static void copyByRuns(
            ...
            final IntBarrageCopyKernelContext context) {
        // hoisted out of the loops
        final int deltaChunkSize = context.deltaChunkSize;
        final WritableIntChunk<Values>[][] addChunks = context.addChunks;
        final WritableIntChunk<Values>[][] modChunks = context.modChunks;
    ...
    private static void copyByElements(
        ...
        for (int mi = 0; mi < dest.length; ++mi) {
            ...
            final WritableIntChunk<Values> destChunk = dest[mi];
    ...
    private static void copy(
        ...
        if (runs.elementMapping != null
            ...
            runs.convertRunsToElementMapping(intContext.deltaChunkSize);
            copyByElements(runs.elementMapping, dest, intContext);
        } else {
            copyByRuns(runs, dest, intContext);
        }
    }
