...
public class CharBarrageCopyKernel {
     ...
     * then never name.
     */
    private static WritableCharChunk<Values>[][] asTypedChunks(final WritableChunk<Values>[][] chunks) {
        // noinspection unchecked
        final WritableCharChunk<Values>[][] typed = new WritableCharChunk[chunks.length][];
        for (int di = 0; di < chunks.length; ++di) {
            if (chunks[di] == null) {
                continue;
            }
            // noinspection unchecked
            typed[di] = new WritableCharChunk[chunks[di].length];
            for (int ci = 0; ci < chunks[di].length; ++ci) {
                typed[di][ci] = chunks[di][ci].asWritableCharChunk();
            }
        }
        return typed;
    }
    ...
    private static class CharBarrageCopyKernelImpl implements BarrageCopyKernel {
        ...
        public void copy(
            ...
            final WritableCharChunk<Values>[][] typedAddChunks = asTypedChunks(addChunks);
            final WritableCharChunk<Values>[][] typedModChunks = asTypedChunks(modChunks);
            ...
            for (int ri = 0; ri < runs.count; ++ri) {
                ...
                final WritableCharChunk<Values>[] originChunks =
