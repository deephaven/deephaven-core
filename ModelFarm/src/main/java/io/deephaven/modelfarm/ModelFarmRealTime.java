 ...
 * the unique identifier is queued for execution. Once the security reaches the top of the execution
 * queue, the most recent data for the unique identifier is used for execution.
 * <p>
 * This is useful for real-time processing, where executing unique identifiers as fast as possible
 * is desired.
 * <p>
 * The execution priority is determined by how long it has been since the data changed. The
 * prioritizer can be used to bump a unique identifier to a higher execution priority.
 *
...
public class ModelFarmRealTime<KEYTYPE, DATATYPE, ROWDATAMANAGERTYPE extends RowDataManager<KEYTYPE, DATATYPE>>
    ...
    private static final boolean LOG_PERF = Configuration.getInstance()
        .getBooleanWithDefault("ModelFarm.logModelFarmRealTimePerformance", false);

    private final ModelFarmBase.GetDataLockType GETDATA_LOCK_TYPE =
        ModelFarmBase.GetDataLockType.valueOf(Configuration.getInstance()
            .getStringWithDefault("ModelFarm.ModelFarmRealTime.getDataLockType", "LTM_READ_LOCK"));
    private final KeyedPriorityBlockingQueue<KEYTYPE> execQueue =
        new KeyedPriorityBlockingQueue<>();
     ...
     */
    public ModelFarmRealTime(final int nThreads, final Model<DATATYPE> model,
        ...
        log.warn().append("ModelFarmRealTime lock type: ").append(GETDATA_LOCK_TYPE.toString())
            .endl();
    ...
    protected void execute() throws InterruptedException {
        ...
        final AtomicBoolean isEvaling =
            isEvalingMap.computeIfAbsent(key, k -> new AtomicBoolean(false));

        // If another thread is already evaling this key, then reenque the key and move on to the
        // next key.
        if (!isEvaling.compareAndSet(false, true)) {
            ...
            if (LOG_PERF) {
                log.warn().append("ModelFarmRealTime.execute PERFORMANCE: isExec=false all=")
                    .append((t2 - t0) / 1000)
                    .append(" take=").append((t1 - t0) / 1000).append(" enqueue=")
                    .append((t2 - t1) / 1000).endl();
            }
        ...
        try {
            ...
            if (LOG_PERF) {
                log.warn().append("ModelFarmRealTime.execute PERFORMANCE: isExec=true all=")
                    .append((t4 - t0) / 1000)
                    .append(" take=").append((t1 - t0) / 1000).append(" get=")
                    .append((t2 - t1) / 1000)
                    .append(" prio=").append((t3 - t2) / 1000).append(" exec=" + (t4 - t3) / 1000)
                    .endl();
            }
        } finally {
            if (!isEvaling.compareAndSet(true, false)) {
                // noinspection ThrowFromFinallyBlock -- once a thread sets isEvaling to true for a
                // key, no other thread should set it to false.
                throw new IllegalStateException(
                    "isEvaling is false but should be true for key " + key);
            }
        }
    }
