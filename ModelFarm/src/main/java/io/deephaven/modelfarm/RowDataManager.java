...
public interface RowDataManager<KEYTYPE, DATATYPE> {
    ...
    /**
     * Gets the previous unique identifier value for a row. One column of each table is designated
     * as a unique identifier for data rows.
     * <p>
     * This function should only be called during an update loop or while holding the LTM lock.
     *
     * @param index table row index.
     ...
     * <p>
     * This method should be called while the LTM lock is held. This can occur either during the
     * update loop or the LTM lock can be acquired outside the update loop. If the LTM lock is not
     * held, the loaded data can be inconsistent or corrupt.
     *
