 ...
 *
 * When we search left or right from an intermediate node like whitespace, dot, comma or EOF, we
 * will create a new CompletionRequest at the new index.
 *
 * This currently uses absolute cursor positions, but we want this to be a line/column position
 * instead, so we can completely remove consideration of absolute cursors.
 *
 * This mistake was made by trying to base V2 on V1 semantics which are not really relevant when
 * considering Monaco, LSP and Javacc which all use line/column semantics.
 *
 * Absolute cursor positions are unfortunately deeply entwined in {@link ChunkerCompleter}, so we
 * are leaving it in place for now.
 *
 * Note that this class also maintains a map of loaded table definitions, so that repeated
 * completions will not pay to load the same table definition more than once.
 */
public class CompletionRequest {
    ...
    public TableDefinition getTableDefinition(final ChunkerCompleter completer,
        final ParsedDocument doc, VariableProvider variables, String name) {
        // Each request maintains a local cache of looked-up table definitions, to avoid going to
        // the VariableHandler unless needed
        ...
        if (localDefs.containsKey(name)) {
            // if there wasn't a table definition in the script session once, it won't magically
            // appear again later.
            // This might seem a little excessive, but in python at least, it is non-"free" to check
            // if binding a variable exists.
            return localDefs.get(name);
        }
        TableDefinition result = variables.getTableDefinition(name);
        if (result == null) {
            // If the result was null, we can try to search for an assign statement that is
            // initialized w/ something we _can_ grok.
            final List<ChunkerAssign> assignment = completer.findAssignment(doc, this, name);
            if (!assignment.isEmpty()) {
                // ok! there was an assignment to our table variable that occurred before user's
                // cursor (this is,
                // of course, bad when user creates some random functions that are defined in any
                // order).
        ...
        for (Node argument : invoke.getArguments()) {
            if (argument instanceof ChunkerInvoke) {
                ...
                if (colArgs.isEmpty()) {
                    ...
                    // in this case, there is no valid table definition, so we'll just skip this
                    // column.
                    // In the future, we may want to consider user-defined zero-arg functions that
                ...
                switch (colMethod) {
                    ...
                    case "floatCol":
                        ...
                        break;
                    case "col":
                        // We _could_ technically try to guess from the col() varargs what the type
                        // is, but, not worth it atm.
    ...
    private String toStringLiteral(final Node node) {
        ...
        } // TODO: if it's a variable name, try to trace it back to a static assignment of a string,
          // or a binding variable.
        return null;
    }
