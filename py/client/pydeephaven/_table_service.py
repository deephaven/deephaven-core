...
class TableService:
    ...
    def batch(self, ops: list[TableOp]) -> Table:
        ...
        try:
            response = self.session.wrap_bidi_rpc(
                self._grpc_table_stub.Batch, table_pb2.BatchTableRequest(ops=batch_ops)
            )
                ...
                if exported.result_id.WhichOneof("ref") == "ticket":
                    exported_tables.append(
                        Table(
                            self.session,
                            ticket=ExportTicket(
                                ticket_bytes=exported.result_id.ticket.ticket
                            ),
    ...
    def grpc_table_op(
        self, table: Optional[Table], op: TableOp, table_class: type = Table
        ...
        try:
            ...
            response = self.session.wrap_rpc(
                ...
                op.make_grpc_request(
                    result_id=export_ticket.pb_ticket, source_id=table_reference
                ),
            )
            if response.success:
                return table_class(
                    self.session,
                    ticket=ExportTicket(response.result_id.ticket.ticket),
