    ...
    def build_batch(self) -> list[Any]:
        ...
        for table_op in self.table_ops[1:-1]:
            ...
            self.grpc_table_ops.append(
                table_op.make_grpc_request_for_batch(
                    result_id=result_id, source_id=self._curr_source
                )
            )
            self._curr_source = table_pb2.TableReference(
                batch_offset=len(self.grpc_table_ops) - 1
            )

        # the last op in the batch needs a result_id to reference the result
        export_ticket = self.session.make_export_ticket()
        self.grpc_table_ops.append(
            self.table_ops[-1].make_grpc_request_for_batch(
                result_id=export_ticket.pb_ticket, source_id=self._curr_source
            )
        )
