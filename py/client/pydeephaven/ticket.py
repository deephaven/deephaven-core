...
class SharedTicket(Ticket):
    ...
    def __init__(self, ticket_bytes: bytes):
        ...
        if not ticket_bytes:
            ...
            raise DHError(
                f"SharedTicket: ticket {ticket_bytes!r} is not a shared ticket"
            )
...
class ExportTicket(Ticket):
    ...
    def __init__(self, ticket_bytes: bytes):
        ...
        if not ticket_bytes:
            ...
            raise DHError(
                f"ExportTicket: ticket {ticket_bytes!r} is not an export ticket"
            )
...
class ScopeTicket(Ticket):
    ...
    def __init__(self, ticket_bytes: bytes):
        ...
        if not ticket_bytes:
            ...
            raise DHError(f"ScopeTicket: ticket {ticket_bytes!r} is not a scope ticket")
...
class ApplicationTicket(Ticket):
    ...
    def __init__(self, ticket_bytes: bytes):
        ...
        if not ticket_bytes:
            ...
            raise DHError(
                f"ApplicationTicket: ticket {ticket_bytes!r} is not an application ticket"
            )
        elif len(ticket_bytes.split(b"/")) != 4:
            raise DHError(
                f"ApplicationTicket: ticket {ticket_bytes!r} is not in the correct format"
            )
...
def _ticket_from_proto(ticket: ticket_pb2.Ticket) -> Ticket:
    ...
    if ticket_bytes.startswith(b"h"):
        ...
        raise DHError(f"Unknown ticket type: {ticket_bytes!r}")
...
class ServerObject:
    ...
    type: str
    """The type of the object. May be empty, indicating that the instance cannot be connected to or otherwise directly
    used from the client."""

    ticket: Ticket
    """The ticket that points to the object on the server."""

    def __init__(self, type: str, ticket: Ticket):
        self.type = type
        self.ticket = ticket
...
def _server_object_from_proto(typed_ticket: ticket_pb2.TypedTicket) -> ServerObject:
    ...
    return ServerObject(
        type=typed_ticket.type, ticket=_ticket_from_proto(typed_ticket.ticket)
    )
