...
class MultiJoinTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.static_tableA = self.session.import_table(pa_table).select(
            ["a", "b", "c1=c", "d1=d", "e1=e"]
        )
        self.static_tableB = self.static_tableA.update(
            ["c2=c1+1", "d2=d1+2", "e2=e1+3"]
        ).drop_columns(["c1", "d1", "e1"])
        self.ticking_tableA = (
            self.session.time_table("PT00:00:00.001")
            .update(
                [
                    "a = i",
                    "b = i*i % 13",
        ...
        self.ticking_tableB = self.ticking_tableA.update(
            ["c2=c1+1", "d2=d1+2", "e2=e1+3"]
        ).drop_columns(["c1", "d1", "e1"])
    ...
    def test_static_simple(self):
        ...
        mj_table = multi_join(
            input=[self.static_tableA, self.static_tableB], on=["a", "b"]
        )
    ...
    def test_ticking_simple(self):
        # Test with multiple input tables
        mj_table = multi_join(
            input=[self.ticking_tableA, self.ticking_tableB], on=["a", "b"]
        )
    ...
    def test_static(self):
        ...
        mj_input = [
            MultiJoinInput(
                table=self.static_tableA, on=["key1=a", "key2=b"], joins=["c1", "e1"]
            ),
            MultiJoinInput(
                table=self.static_tableB, on=["key1=a", "key2=b"], joins=["d2"]
            ),
        ]
        ...
        mj_table = multi_join(
            MultiJoinInput(
                table=self.static_tableA, on=["key1=a", "key2=b"], joins="c1"
            )
        )
    ...
    def test_ticking(self):
        ...
        mj_input = [
            MultiJoinInput(
                table=self.ticking_tableA, on=["key1=a", "key2=b"], joins=["c1", "e1"]
            ),
            MultiJoinInput(
                table=self.ticking_tableB, on=["key1=a", "key2=b"], joins=["d2"]
            ),
        ]
        mj_table = multi_join(mj_input)
        ...
        # Test with a single input
        mj_table = multi_join(
            input=MultiJoinInput(
                table=self.ticking_tableA, on=["key1=a", "key2=b"], joins="c1"
            )
        )
    ...
    def test_errors(self):
        ...
        with self.assertRaises(DHError) as cm:
            multi_join(mj_input, on=["key1=a", "key2=b"])
        self.assertIn("on parameter is not permitted", str(cm.exception))

        session = Session()
        t = (
            session.time_table("PT00:00:00.001")
            .update(
                [
                    "a = i",
                    "b = i*i % 13",
