...
class QueryTestCase(BaseTestCase):
    ...
    def test_snapshot_when(self):
        ...
        query = self.session.query(source_table).snapshot_when(
    ...
    def test_agg_by(self):
        ...
        query = (
            self.session.query(table)
            .agg_by(aggs=[agg.avg(cols=["a"])], by=["b"])
            .update(formulas=["Col1 = i + 1"])
            .tail(5)
            .update(formulas=["Col2=i*i"])
        )
    ...
    def test_where_in(self):
        ...
        with self.subTest("where_not_in"):
            ...
            self.assertGreaterEqual(
                test_table.size - unique_table.size, result_table.size
            )
