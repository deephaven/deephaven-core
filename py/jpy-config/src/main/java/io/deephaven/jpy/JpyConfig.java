...
/**
 * This class encapsulates the configuration data and invocation of
 * {/@link PyLibInitializer#initPyLib(String, String, String)},
 * {/@link PyLib#setProgramName(String)},
 * {/@link PyLib#setPythonHome(String)},
 * and {/@link PyLib#startPython(int, String...)}.
 *
 * <p>Note:
 * <p>We *don't* want JpyConfig to have an explicit dependency on jpy anymore - that way we can still
 ...
 * have fake @links to it though, as it gives useful context for developers.
 * In a better world, the jpy project itself would be better configure-able (ie, not static), and
 * this type of external configuration class wouldn't be necessary.
 */
final public class JpyConfig {
  /**
   * We can't reference the values in {/@link Diag} directly - that would cause {/@link org.jpy.PyLib}
   * to prematurely initialize.
   *
   * We could:
   * 1) Refactor {/@link Diag} so as not to initialize {/@link PyLib}
   * 2) Use compile-time code generation against {/@link Diag}
   * 3) Test to make sure {/@link Flag} and {/@link Diag} are in-sync
   *
