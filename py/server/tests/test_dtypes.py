...
class DTypesTestCase(BaseTestCase):
    ...
    def test_j_type(self):
        ...
        self.assertEqual(
            dtypes.ZonedDateTime.j_type, jpy.get_type("java.time.ZonedDateTime")
        )
        ...
        self.assertEqual(
            dtypes.instant_array.j_type, jpy.get_type("[Ljava.time.Instant;")
        )
        self.assertEqual(
            dtypes.zdt_array.j_type, jpy.get_type("[Ljava.time.ZonedDateTime;")
        )
    ...
    def test_instant_array(self):
        ...
        j_array = dtypes.array(Instant, np_array)
        self.assertTrue(
            all(x == to_j_instant(int(y)) for x, y in zip(j_array, np_array))
        )
