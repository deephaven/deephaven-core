...
class TableTestCase(BaseTestCase):
    ...
    def test_generated_table_timed_refresh(self):
        def table_generator_function():
            return empty_table(1).update(
                "Timestamp = io.deephaven.base.clock.Clock.system().currentTimeMillis()"
            )

        with update_graph.exclusive_lock(self.test_update_graph):
            result_table = function_generated_table(
                table_generator_function, refresh_interval_ms=2000
            )
            self.assertEqual(result_table.size, 1)
            first_row_key = get_row_key(0, result_table)
            initial_time = result_table.j_table.getColumnSource("Timestamp").get(
                first_row_key
            )
            ...
            first_row_key = get_row_key(0, result_table)
            later_time = result_table.j_table.getColumnSource("Timestamp").get(
                first_row_key
            )
    ...
    def test_generated_table_1trigger(self):
        ...
        result_table = function_generated_table(
            table_generator_function, source_tables=input_table_lastby
        )
    ...
    def test_generated_table_2triggers(self):
        ...
        def table_generator_function():
            with liveness_scope():
                ...
                my_str = (
                    None
                    if t1.size == 0
                    else t1.j_table.getColumnSource("MyStr").get(get_row_key(0, t1))
                )
                my_int = (
                    None
                    if t2.size == 0
                    else t2.j_table.getColumnSource("MyInt").getInt(get_row_key(0, t2))
                )
        ...
        result_int = result_table.j_table.getColumnSource("ResultInt").get(
            first_row_key
        )
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            ...
            append_only_input_table2.add(
                new_table([int_col(name="MyInt", data=[12345])])
            )
    ...
    def test_generated_table_kwargs(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            result_table = function_generated_table(
                ...
                kwargs={
                    "nrows": 5,
                    "query_string": "Timestamp = io.deephaven.base.clock.Clock.system().currentTimeMillis()",
                },
            )
            self.assertEqual(result_table.size, 5)
            first_row_key = get_row_key(0, result_table)
    ...
    def test_generated_table_wrong_kwargs(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            ...
            self.assertIn(
                "table_generator_function() got an unexpected keyword argument 'nrow'",
                str(cm.exception),
            )
    ...
    def test_generated_table_too_few_args(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            with self.assertRaises(Exception) as cm:
                result_table = function_generated_table(
                    table_generator_function, refresh_interval_ms=2000, args=(1,)
                )
            self.assertIn(
                "table_generator_function() missing 1 required positional argument: 'query_string'",
                str(cm.exception),
            )
    ...
    def test_generated_table_too_few_kwargs(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            ...
            self.assertIn(
                "table_generator_function() missing 1 required positional argument: 'nrows'",
                str(cm.exception),
            )
    ...
    def test_generated_table_too_few_args_and_kwargs(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            ...
            self.assertIn(
                "table_generator_function() missing 2 required positional arguments: 'arg2' and 'arg4'",
                str(cm.exception),
            )
    ...
    def test_generated_table_too_many_args(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            ...
            self.assertIn(
                "table_generator_function() takes 2 positional arguments but 3 were given",
                str(cm.exception),
            )
    ...
    def test_generated_table_too_many_kwargs(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            ...
            self.assertIn(
                "table_generator_function() got an unexpected keyword argument 'non_argument'",
                str(cm.exception),
            )
    ...
    def test_generated_table_too_many_args_and_kwargs(self):
        ...
        with update_graph.exclusive_lock(self.test_update_graph):
            ...
            self.assertIn(
                "table_generator_function() got multiple values for argument 'nrows'",
                str(cm.exception),
            )
