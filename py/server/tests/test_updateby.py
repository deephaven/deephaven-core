...
from deephaven import empty_table, read_csv, time_table, update_graph
...
class UpdateByTestCase(BaseTestCase):
    def setUp(self):
        super().setUp()
        self.static_table = read_csv("tests/data/test_table.csv").update(
            "Timestamp=now()"
        )
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.em_op_ctrl = OperationControl(
            on_null=BadDataBehavior.THROW,
        ...
        cls.rolling_ops = [
            ...
            rolling_sum_tick(
                cols=["rsum_a = a", "rsum_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_sum_time(
                ts_col="Timestamp",
                cols=["rsum_b = b", "rsum_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_sum_time(
                ts_col="Timestamp",
                cols=["rsum_b = b", "rsum_e = e"],
                rev_time=10_000_000_000,
            ...
            rolling_group_tick(
                cols=["rgroup_a = a", "rgroup_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_group_time(
                ts_col="Timestamp",
                cols=["rgroup_b = b", "rgroup_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_group_time(
                ts_col="Timestamp",
            ...
            rolling_avg_tick(
                cols=["ravg_a = a", "ravg_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_avg_time(
                ts_col="Timestamp",
                cols=["ravg_b = b", "ravg_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_avg_time(
                ts_col="Timestamp",
            ...
            rolling_min_tick(
                cols=["rmin_a = a", "rmin_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_min_time(
                ts_col="Timestamp",
                cols=["rmin_b = b", "rmin_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_min_time(
                ts_col="Timestamp",
            ...
            rolling_max_tick(
                cols=["rmax_a = a", "rmax_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_max_time(
                ts_col="Timestamp",
                cols=["rmax_b = b", "rmax_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_max_time(
                ts_col="Timestamp",
            ...
            rolling_prod_tick(
                cols=["rprod_a = a", "rprod_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_prod_time(
                ts_col="Timestamp",
                cols=["rprod_b = b", "rprod_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_prod_time(
                ts_col="Timestamp",
            ...
            rolling_count_tick(
                cols=["rcount_a = a", "rcount_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_count_time(
                ts_col="Timestamp",
                cols=["rcount_b = b", "rcount_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_count_time(
                ts_col="Timestamp",
            ...
            rolling_std_tick(
                cols=["rstd_a = a", "rstd_d = d"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_std_time(
                ts_col="Timestamp",
                cols=["rstd_b = b", "rstd_e = e"],
                rev_time="PT00:00:10",
            ),
            rolling_std_time(
                ts_col="Timestamp",
            ...
            rolling_wavg_tick(
                wcol="b", cols=["rwavg_a = a", "rwavg_d = d"], rev_ticks=10
            ),
            rolling_wavg_tick(
                wcol="b",
                cols=["rwavg_a = a", "rwavg_d = d"],
                rev_ticks=10,
                fwd_ticks=10,
            ),
            rolling_wavg_time(
        ...
        cls.rolling_ops_one_output = [
            ...
            rolling_formula_tick(
                formula="formula_ad=avg(a) + avg(b)", rev_ticks=10, fwd_ticks=10
            ),
            rolling_formula_time(
                formula="formula_be=sum(b) + sum(e)",
                ts_col="Timestamp",
                rev_time="PT00:00:10",
            ),
            rolling_formula_time(
                formula="formula_be=avg(b) + avg(e)",
                ts_col="Timestamp",
                rev_time=10_000_000_000,
                fwd_time=-10_000_000_00,
            ),
            rolling_formula_time(
                formula="formula_be=sum(b) + sum(b)",
                ts_col="Timestamp",
                rev_time="PT30S",
                fwd_time="-PT00:00:20",
            ),
            ...
            rolling_count_where_tick(
                col="count_2", filters=["a > 0", "a <= 50"], rev_ticks=10, fwd_ticks=10
            ),
            rolling_count_where_time(
                col="count_3",
                filters="a > 50",
    ...
    def test_em_proxy(self):
        pt_proxies = [
            self.static_table.partition_by("b").proxy(),
        ...
        for op in self.em_ops:
            with self.subTest(op):
                for pt_proxy in pt_proxies:
                    ...
                    for ct, rct in zip(
                        pt_proxy.target.constituent_tables,
                        rt_proxy.target.constituent_tables,
                    ):
    ...
    def test_simple_ops_proxy(self):
        pt_proxies = [
            self.static_table.partition_by("c").proxy(),
        ...
        for op in self.simple_ops:
            with self.subTest(op):
                for pt_proxy in pt_proxies:
                    ...
                    self.assertEqual(
                        len(rt_proxy.target.constituent_table_columns),
    ...
    def test_rolling_count_where_output(self):
        ...
        count_aggs = [
            ...
            rolling_count_where_tick(
                col="count2", filters="a >= 1 && b == 0", rev_ticks=2
            ),
        ]
        result_table = test_table.update_by(ops=count_aggs)
        self.assertEqual(result_table.size, 4)
