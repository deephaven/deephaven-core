...
class ParquetTestCase(BaseTestCase):
    ...
    def test_crd(self):
        ...
        with self.subTest(msg="write_tables(Table[], destinations, table_definition"):
            batch_write(
                [table, table], [file_location, file_location2], table_definition
            )
            self.assertTrue(os.path.exists(file_location))
    ...
    def test_crd_with_instructions(self):
        ...
        with self.subTest(
            msg="write_tables(Table[], destinations, table_definition, col_instructions"
        ):
            batch_write(
                [table, table],
    ...
    def test_int96_timestamps(self):
        ...
        pyarrow.parquet.write_table(
            table, "data_from_pa.parquet", use_deprecated_int96_timestamps=True
        )
        from_disk_int96 = read(
            "data_from_pa.parquet", file_layout=ParquetFileLayout.SINGLE_FILE
        )
        ...
        from_disk_int64 = read(
            "data_from_dh.parquet", file_layout=ParquetFileLayout.SINGLE_FILE
        )
    ...
    def round_trip_with_compression(
        self, compression_codec_name, dh_table, vector_columns=False
    ):
        ...
        if pandas.__version__.split(".")[0] == "1":
            ...
            dataframe = pandas.read_parquet(
                "data_from_dh.parquet", dtype_backend="numpy_nullable"
            )
        ...
        dataframe.to_parquet(
            ...
            compression=None
            if compression_codec_name == "UNCOMPRESSED"
            else "LZ4"
            if compression_codec_name == "LZ4_RAW" or compression_codec_name == "LZ4RAW"
            else compression_codec_name,
        )
        result_table = read(
            "data_from_pandas.parquet", file_layout=ParquetFileLayout.SINGLE_FILE
        )
        self.assert_table_equals(dh_table, result_table)
    ...
    def test_writing_lists_via_pyarrow(self):
        ...
        pa_table = pyarrow.table(
            {"numList": [[2, 2, 4]], "stringList": [["Flamingo", "Parrot", "Dog"]]}
        )
    ...
    def test_dictionary_encoding(self):
        dh_table = empty_table(10).update(
            formulas=[
        ...
        self.assertTrue(
            (metadata.row_group(0).column(0).path_in_schema == "shortStringColumn")
            & ("RLE_DICTIONARY" in str(metadata.row_group(0).column(0).encodings))
        )
        self.assertTrue(
            (metadata.row_group(0).column(1).path_in_schema == "longStringColumn")
            & ("RLE_DICTIONARY" not in str(metadata.row_group(0).column(2).encodings))
        )
        self.assertTrue(
            (metadata.row_group(0).column(2).path_in_schema == "someIntColumn")
            & ("RLE_DICTIONARY" not in str(metadata.row_group(0).column(2).encodings))
        )
    ...
    def test_dates_and_time(self):
        dh_table = empty_table(10000).update(
            formulas=[
        ...
        self.assertTrue(
            df_from_disk[["nullDateColumn", "nullTimeColumn"]].isnull().values.all()
        )
        self.assertTrue(
            df_from_pandas[["nullDateColumn", "nullTimeColumn"]].isnull().values.all()
        )
        ...
        self.assert_table_equals(
            dh_table.select(["someDateColumn", "someTimeColumn"]),
    ...
    def test_time_with_different_units(self):
        ...
        dh_table = empty_table(20000).update(
            formulas=[
                "someTimeColumn = i % 10 == 0 ? null : java.time.LocalTime.of(i%24, i%60, (i+10)%60)"
            ]
        )
        ...
        schema_nsec = table.schema.set(
            0, pyarrow.field("someTimeColumn", pyarrow.time64("ns"))
        )
        time_test_helper(table, schema_nsec, "data_from_pq_nsec.parquet")

        schema_usec = table.schema.set(
            0, pyarrow.field("someTimeColumn", pyarrow.time64("us"))
        )
        time_test_helper(table, schema_usec, "data_from_pq_usec.parquet")

        schema_msec = table.schema.set(
            0, pyarrow.field("someTimeColumn", pyarrow.time32("ms"))
        )
    ...
    def test_timestamp_with_different_units(self):
        ...
        df_from_disk["time_ms"] = (
            df_from_disk["time_ms"]
            .dt.tz_localize(None)
            .astype("datetime64[ms]")
            .dt.tz_localize("UTC")
        )
        df_from_disk["time_us"] = (
            df_from_disk["time_us"]
            .dt.tz_localize(None)
    ...
    def test_read_with_table_definition_no_layout(self):
        ...
        write(table, single_parquet)
        from_disk = read(
            single_parquet, table_definition={"x": dtypes.int32, "y": dtypes.double}
        )
    ...
    def test_read_parquet_from_s3(self):
        ...
        with self.assertRaises(Exception):
            read(
                "s3://dh-s3-parquet-test1/multiColFile.parquet",
                special_instructions=s3_instructions,
            ).select()
    ...
    def test_write_partitioned_data(self):
        ...
        def verify_table_from_disk(table):
            ...
            self.assert_table_equals(
                table.select().sort(["X", "Y"]), source.sort(["X", "Y"])
            )
        ...
        shutil.rmtree(root_dir)
        write_partitioned(
            partitioned_table, destination_dir=root_dir, base_name=base_name
        )
        verify_table_from_disk(read(root_dir))
        verify_file_names()
        ...
        self.verify_index_files(
            os.path.join(root_dir, "X=Aa/.dh_metadata/indexes/Y,Number")
        )
        ...
        shutil.rmtree(root_dir)
        write_partitioned(
            source, table_definition=table_definition, destination_dir=root_dir
        )
        verify_table_from_disk(read(root_dir))
    ...
    def test_write_with_index_columns(self):
        first_table = empty_table(10).update(
            formulas=["x=i", "y=(double)(i/10.0)", "z=(double)(i*i)"]
        )
        ...
        second_table = empty_table(10).update(
            formulas=["x=i*5", "y=(double)(i/5.0)", "z=(double)(i*i*i)"]
        )
        batch_write(
    ...
    def test_unsigned_ints(self):
        ...
        pyarrow.parquet.write_table(
            pyarrow.Table.from_pandas(df), "data_from_pyarrow.parquet"
        )
        schema_from_disk = pyarrow.parquet.read_metadata(
            "data_from_pyarrow.parquet"
        ).schema.to_arrow_schema()
        self.assertTrue(schema_from_disk.field("uint8Col").type.equals(pyarrow.uint8()))
        self.assertTrue(
            schema_from_disk.field("uint16Col").type.equals(pyarrow.uint16())
        )
        self.assertTrue(
            schema_from_disk.field("uint32Col").type.equals(pyarrow.uint32())
        )
    ...
    def test_unsigned_byte_cast(self):
        ...
        table_from_disk = read(
            "data_from_pyarrow.parquet", table_definition={"uint8Col": dtypes.char}
        )
        ...
        # UByte -> Short
        table_from_disk = read(
            "data_from_pyarrow.parquet", table_definition={"uint8Col": dtypes.short}
        )
        ...
        # UByte -> Int
        table_from_disk = read(
            "data_from_pyarrow.parquet", table_definition={"uint8Col": dtypes.int32}
        )
    ...
    def test_v2_pages(self):
        def test_v2_pages_helper(dh_table):
            ...
            pyarrow.parquet.write_table(
                pa_table, "data_from_pq_v2.parquet", data_page_version="2.0"
            )
