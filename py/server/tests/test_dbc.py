...
class DbcTestCase(BaseTestCase):
    ...
    def test_read_odbc(self):
        ...
        with turbodbc.connect(connection_string=connection_string) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT t_ts, t_id, t_instrument, t_exchange, t_price, t_size FROM CRYPTO_TRADES"
                )
    ...
    def test_read_adbc(self):
        ...
        with adbc_driver_postgresql.dbapi.connect(uri) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT t_ts, t_exchange, t_price, t_size FROM CRYPTO_TRADES LIMIT 10"
                )
    ...
    @unittest.skipIf(
        ...
        reason="connectorx not available on Linux/Arm64"
