...
class TestBackend(TableDataServiceBackend):
    def __init__(
        self,
        gen_pa_table: Generator[pa.Table, None, None],
        pt_schema: pa.Schema,
        ...
        self.size_sub_failure_cb_called_cond: threading.Condition = (
            threading.Condition()
        )
        self.num_table_location_subs: int = 0
        self.location_cb: Optional[
            Callable[[TableLocationKeyImpl, Optional[pa.Table]], None]
        ] = None
    ...
    def column_values(
        ...
        if table_key.key == "test":
            values_cb(
                self.partitions[table_location_key]
                .select([col])
    ...
    def _th_new_partitions(
        ...
        while (
            not self.sub_new_partition_cancelled and self.subscriptions_enabled_for_test
        ):
            try:
                with exec_ctx:
                    pa_table = next(self.gen_pa_table)
    ...
    def subscribe_to_table_locations(
        ...
        th = threading.Thread(
            target=self._th_new_partitions,
            args=(table_key, exec_ctx, location_cb, failure_cb),
        )
        th.start()
    ...
    def _th_partition_size_changes(
        ...
        while (
            self.subscriptions_enabled_for_test
            and self.partitions_size_subscriptions[table_location_key]
        ):
    ...
    def subscribe_to_table_location_size(
        ...
        if (
            table_location_key in self.partitions_size_subscriptions
            and self.partitions_size_subscriptions[table_location_key]
        ):
        ...
        th = threading.Thread(
            target=self._th_partition_size_changes,
            args=(table_key, table_location_key, size_cb, failure_cb),
        )
        th.start()
...
class TableDataServiceTestCase(BaseTestCase):
    ...
    def test_make_partitioned_table_without_partition_schema(self):
        backend = TestBackend(self.gen_pa_table(), pt_schema=self.pa_table.schema)
        data_service = TableDataService(backend)
        partitioned_table = data_service.make_partitioned_table(
            TableKeyImpl("test"), refreshing=False
        )
    ...
    def test_make_static_table_with_partition_schema(self):
        pc_schema = pa.schema(
            [
                pa.field(name="Ticker", type=pa.string()),
                pa.field(name="Exchange", type=pa.string()),
            ]
        )
        backend = TestBackend(
            self.gen_pa_table(), pt_schema=self.pa_table.schema, pc_schema=pc_schema
        )
        data_service = TableDataService(backend)
        table = data_service.make_table(TableKeyImpl("test"), refreshing=False)
        self.assertIsNotNone(table)
    ...
    def test_live_table_subscribes_tlk_only_once(self):
        ...
        data_service = TableDataService(backend)
        table = data_service.make_table(
            TableKeyImpl("test"), refreshing=True
        ).coalesce()
        partitioned_table = (
            data_service.make_partitioned_table(TableKeyImpl("test"), refreshing=True)
            .merge()
            .coalesce()
        )
        self.assertIsNotNone(table)
    ...
    def test_partition_size_sub_failure(self):
        ...
        with backend.size_sub_failure_cb_called_cond:
            if not backend.size_sub_failure_cb_called_cond.wait_for(
                lambda: backend.is_size_sub_failure_cb_called, timeout=5
            ):
