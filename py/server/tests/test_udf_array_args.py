...
def test_udf(x {th}, y {th}) -> bool:
    ...
    return isinstance(x, int) and isinstance(y, j_array_type)
            ...
            with self.subTest(
                "with null cells"
            ):  # no auto DH null to np.nan conversion
...
def test_udf(x, y: np.ndarray[{_J_TYPE_NP_DTYPE_MAP[j_dtype]}]) -> np.bool_:
    ...
    def test_np_object_array(self):
        with self.subTest("PyObject"):
            ...
            def test_udf(
                x, y: np.ndarray[C]
            ) -> bool:  # not conversion supported typehint
                import jpy
        ...
        with self.subTest("JObject"):
            ...
            t = empty_table(10).update("X = i % 3").update("Y= fn(X + 1)")
...
def test_udf(x, y: np.ndarray[{_J_TYPE_NP_DTYPE_MAP[j_dtype]}]) -> bool:
    ...
    return True
                ...
                if j_dtype not in {"float", "double"}:
                    ...
                    self.assertRegex(
                        str(cm.exception),
                        "failed to convert DH nulls to np.nan .* type_promotion is False",
                    )
                else:
                    res = tbl.update("Z = test_udf(X, Y)")
                    self.assertEqual(10, res.to_string().count("true"))
        ...
        with self.subTest("boolean"):
            ...
            def test_udf(
                p1: np.ndarray[np.bool_], p2=None, tp: bool = True
            ) -> np.bool_:
            ...
            t = (
                empty_table(100)
                .update(["X = i % 10", "Y = i % 3 == 0? true : null"])
                .group_by("X")
            )
            rest = t.update(["X1 = test_udf(Y)"])
            self.assertEqual(17, rest.to_string().count("true"))
