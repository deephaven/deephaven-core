...
def start_jvm_for_tests(jvm_props: Optional[dict[str, str]] = None):
    jvm.preload_jvm_dll()
    import jpy
    ...
    if not jpy.has_jvm():
        ...
        jvm.init_jvm(
            ...
            jvm_classpath=_expand_wildcards_in_list(
        ...
        no_op_thread_factory = jpy.get_type(
            "io.deephaven.util.thread.ThreadInitializationFactory"
        ).NO_OP
        _JOperationInitializationThreadPool = jpy.get_type(
            "io.deephaven.engine.table.impl.OperationInitializationThreadPool"
        )
        _j_operation_initializer = _JOperationInitializationThreadPool(
            no_op_thread_factory
        )

        _JPeriodicUpdateGraph = jpy.get_type(
            "io.deephaven.engine.updategraph.impl.PeriodicUpdateGraph"
        )
        _j_test_update_graph = (
            _JPeriodicUpdateGraph.newBuilder(
                _JPeriodicUpdateGraph.DEFAULT_UPDATE_GRAPH_NAME
            )
        ...
        py_dh_session = _JPythonScriptSession(
            _j_test_update_graph,
...
def _expand_wildcards_in_item(element):
    ...
    try:
        base = re.search(r"(.*)/\*(.jar)?$", element.replace("\\", "/")).group(1)
        # expand base
        return glob(f"{base}/*.jar")
