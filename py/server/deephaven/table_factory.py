...
_JOptional = jpy.get_type("java.util.Optional")
...
_JFunctionGeneratedTableSpec = jpy.get_type(
...
def function_generated_table(
    ...
    'source_tables' ticks or (b) after refresh_interval_ms have elapsed. At most one of 'refresh_interval_ms' or
    'source_tables' may be set. When neither is set there is no refresh trigger: the 'table_generator' runs exactly
    once, and the result is a static table.
    ...
    Args:
        ...
        refresh_interval_ms (Optional[int]): Interval (in milliseconds) at which the 'table_generator' function is
            rerun. A non-positive interval is treated the same as no trigger at all, producing a static result.
    ...
    def table_generator_function():
        ...
        # A None result means "no new table"; the previously produced result is retained for the next cycle.
        if result is None:
            return _JOptional.empty()

        if isinstance(result, Table) is False:
            raise DHError("table_generator did not return a Table")

        return _JOptional.of(result.j_table)

    # Treat the table_generator_function as a Java lambda (i.e., wrap it in a Java Supplier<Optional<Table>>):
    ...
    try:
        builder = _JFunctionGeneratedTableSpec.builder()
        builder.retainingLastTableSupplier(table_generator_j_function)
        ...
        if source_tables is not None:
            ...
            builder.refreshInterval(_JDuration.ofMillis(refresh_interval_ms))
        spec = builder.build()
        ...
        raise DHError(e, "failed to create a function-generated table.") from e
