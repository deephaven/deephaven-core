...
try:
    ...
    warnings.warn(
        message="import turbodbc failed, please install turbodbc with arrow support, the ODBC driver manager,"
        "and the target database driver first.",
        stacklevel=2,
    )
    turbodbc = None
...
def read_cursor(cursor: "turbodbc.cursor.Cursor") -> Table:
    ...
    if turbodbc is None:
        raise DHError(
            message="turbodbc is not installed, please install it, the ODBC driver manager, and the target "
