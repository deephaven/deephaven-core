...
class TableDataService(JObjectWrapper):
    ...
    def __init__(
        self,
        backend: TableDataServiceBackend,
        *,
        chunk_reader_factory: Optional[jpy.JType] = None,
        stream_reader_options: Optional[jpy.JType] = None,
    ...
    def make_table(self, table_key: TableKey, *, refreshing: bool) -> Table:
        ...
        try:
            ...
            raise DHError(
                e, message=f"failed to make a table for the key {table_key}"
            ) from e

    def make_partitioned_table(
        self, table_key: TableKey, *, refreshing: bool
    ) -> PartitionedTable:
        ...
        try:
            return PartitionedTable(
                self._j_tbl_service.makePartitionedTable(j_table_key, refreshing)
            )
        except Exception as e:
            raise DHError(
                e, message=f"failed to make a partitioned table for the key {table_key}"
            ) from e

    def _table_schema(
        self, table_key: TableKey, schema_cb: jpy.JType, failure_cb: jpy.JType
    ) -> None:
        ...
        def schema_cb_proxy(
            dt_schema: pa.Schema, pc_schema: Optional[pa.Schema] = None
        ):
            ...
            schema_cb.accept(
                jpy.array("java.nio.ByteBuffer", [j_pc_schema_bb, j_dt_schema_bb])
            )
    ...
    def _table_locations(
        self,
        table_key: TableKey,
        location_cb: jpy.JType,
        success_cb: jpy.JType,
        failure_cb: jpy.JType,
    ) -> None:
        ...
        def location_cb_proxy(
            ...
            if pt_table is None or pt_table.to_batches() is None:
                ...
                if pt_table.num_rows != 1:
                    raise ValueError(
                        "The number of rows in the pyarrow table for partitioning values must be 1"
                    )
                bb_list = [
        ...
        self._backend.table_locations(
            table_key, location_cb_proxy, success_cb_proxy, failure_cb_proxy
        )

    def _subscribe_to_table_locations(
        self,
        table_key: TableKey,
        location_cb: jpy.JType,
        success_cb: jpy.JType,
        ...
        return self._backend.subscribe_to_table_locations(
            table_key, location_cb_proxy, success_cb_proxy, failure_cb_proxy
        )

    def _table_location_size(
        self,
        table_key: TableKey,
        table_location_key: TableLocationKey,
        size_cb: jpy.JType,
        ...
        self._backend.table_location_size(
            table_key, table_location_key, size_cb_proxy, failure_cb_proxy
        )

    def _subscribe_to_table_location_size(
        self,
        table_key: TableKey,
        table_location_key: TableLocationKey,
        size_cb: jpy.JType,
        ...
        return self._backend.subscribe_to_table_location_size(
            table_key,
            table_location_key,
            size_cb_proxy,
            success_cb_proxy,
    ...
    def _column_values(
        ...
        def values_cb_proxy(pt_table: pa.Table):
            if len(pt_table) < min_rows or len(pt_table) > max_rows:
                raise ValueError(
                    "The number of rows in the pyarrow table for column values must be in the range of "
