...
class TickingBasicTestCase(unittest.TestCase):
    def test_ticking_basic_time_table(self):
        ...
        table = session.time_table(period=fifth_second_in_nanos).update(
            formulas=["Col1 = i"]
        )
        ...
        listener_handle = dh.listen(
            table, lambda update: update_table_added(update.added("Col1"))
        )
        ...
        while True:
            ...
            self.assertTrue(
                now_seconds - start_seconds < timeout_seconds
            )  # eventually fail
