    ...
    def handle_update(self, update):
        ...
        for i in range(10):
            ...
            if i != 5:
                ...
                expected_local_date_data[key].append(
                    dt.datetime(2001, 3, 1) + dt.timedelta(days=i)
                )
                expected_local_time_data[key].append(
        ...
        expected_doubles = pa.array(
            expected_double_data.values(), pa.list_(pa.float64())
        )
        expected_bools = pa.array(expected_bool_data.values(), pa.list_(pa.bool_()))
        expected_strings = pa.array(
            expected_string_data.values(), pa.list_(pa.string())
        )
        expected_date_times = pa.array(
            expected_date_time_data.values(), pa.list_(pa.timestamp("ns", tz="UTC"))
        )
        expected_local_dates = pa.array(
            expected_local_date_data.values(), pa.list_(pa.date64())
        )
        expected_local_times = pa.array(
            expected_local_time_data.values(), pa.list_(pa.time64("ns"))
        )
