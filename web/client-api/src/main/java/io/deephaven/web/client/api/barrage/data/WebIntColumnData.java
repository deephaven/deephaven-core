...
public class WebIntColumnData extends WebColumnData {
    ...
    public void applyUpdate(
        ...
        IntChunk<Values> intChunk = dataIter.hasNext() ? dataIter.next().asIntChunk() : null;
        while (destOffset < newLength) {
            if (nextRemove != null && nextRemove.getFirst() == retainSourceOffset) {
                ...
                for (long ii = 0; ii < size; ++ii) {
                    while (intChunk != null && chunkSourceOffset == intChunk.size()) {
                        intChunk = dataIter.hasNext() ? dataIter.next().asIntChunk() : null;
                        chunkSourceOffset = 0;
                    }
                    assert intChunk != null;
                    int value = intChunk.get(chunkSourceOffset++);
                    tmpStorage.setAt(destOffset++, value == QueryConstants.NULL_INT ? null : Js.asAny(value));
                }
