...
/**
 * Simple wrapper to emulate RangeSet/Index in JS, with the caveat that LongWrappers may make poor
 * keys in plain JS.
 */
public class JsRangeSet {
    ...
    public static JsRangeSet ofSortedRanges(JsRangeSet[] ranges) {
        Range[] rangeArray = Arrays.stream(ranges)
            .flatMap(r -> StreamSupport.stream(
                Spliterators.spliterator(r.range.rangeIterator(), Long.MAX_VALUE, 0), false))
            .toArray(Range[]::new);
    ...
    public JsIterator<LongWrapper> iterator() {
        return new JsIterator<>(
            StreamSupport
                .longStream(Spliterators.spliterator(range.indexIterator(), Long.MAX_VALUE, 0),
                    false)
