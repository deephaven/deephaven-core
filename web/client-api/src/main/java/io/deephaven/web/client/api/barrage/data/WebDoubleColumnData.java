...
public class WebDoubleColumnData extends WebColumnData {
    ...
    public void applyUpdate(
        ...
        DoubleChunk<Values> doubleChunk = dataIter.hasNext() ? dataIter.next().asDoubleChunk() : null;
        while (destOffset < newLength) {
            if (nextRemove != null && nextRemove.getFirst() == retainSourceOffset) {
                ...
                for (long ii = 0; ii < size; ++ii) {
                    while (doubleChunk != null && chunkSourceOffset == doubleChunk.size()) {
                        doubleChunk = dataIter.hasNext() ? dataIter.next().asDoubleChunk() : null;
                        chunkSourceOffset = 0;
                    }
                    assert doubleChunk != null;
                    double value = doubleChunk.get(chunkSourceOffset++);
                    tmpStorage.setAt(destOffset++, value == QueryConstants.NULL_DOUBLE ? null : Js.asAny(value));
                }
