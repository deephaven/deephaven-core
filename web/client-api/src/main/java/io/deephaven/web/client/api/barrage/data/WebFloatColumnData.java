...
public class WebFloatColumnData extends WebColumnData {
    ...
    public void applyUpdate(
        ...
        FloatChunk<Values> floatChunk = dataIter.hasNext() ? dataIter.next().asFloatChunk() : null;
        while (destOffset < newLength) {
            if (nextRemove != null && nextRemove.getFirst() == retainSourceOffset) {
                ...
                for (long ii = 0; ii < size; ++ii) {
                    while (floatChunk != null && chunkSourceOffset == floatChunk.size()) {
                        floatChunk = dataIter.hasNext() ? dataIter.next().asFloatChunk() : null;
                        chunkSourceOffset = 0;
                    }
                    assert floatChunk != null;
                    float value = floatChunk.get(chunkSourceOffset++);
                    tmpStorage.setAt(destOffset++, value == QueryConstants.NULL_FLOAT ? null : Js.asAny(value));
                }
