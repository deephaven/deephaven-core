...
/**
 * Represents the subscriptions that are needed to get data for the series in a figure. The equals
 * and hashcode method are designed to only match the features which can change while still
 * maintaining the current subscription - the actual "subscribed" series can change as long as they
 * don't affect these other parameters.
 */
public final class FigureSubscription {
    /**
     * Minimum multiple that the table size needs to be decreased by in order to trigger
     * downsampling. For example, if there were 10k rows in the table, and downsampling might only
     * reduce this to 6k, if the factor is 2 we would not downsample. If there had been 15k rows,
     * then with a factor of 2 we would downsample.
     */
    ...
    // The series that are currently watching this table - note that this can change, so is not
    // included
    // in the hashcode/equals. What cannot change is the set of columns that are required by these
    // series,
    ...
    public FigureSubscription(JsFigure figure, final JsTable originalTable,
        final JsFigure.AxisRange downsampleAxisRange,
        final JsFigure.DownsampleParams downsampleParams, final Set<JsSeries> series) {
        this.figure = figure;
        ...
        this.requiredColumns = Collections
            .unmodifiableSet(includedSeries.stream().flatMap(s -> Arrays.stream(s.getSources()))
                .map(source -> source.getDescriptor().getColumnName()).collect(Collectors.toSet()));
    }

    /**
     * Ensures that all of these series are present in this subscription, and returns the ones which
     * were added, if any, so they can get a notification of their current data.
     *
     * Replacement series must match this subscription already - the only facet that will be checked
     * is that the columns match.
     */
    public Set<JsSeries> replaceSeries(final Set<JsSeries> replacements) {
        ...
        // For each of the series in copy, if this subscription is downsampled we need to notify of
        // this fact.
        if (downsampleAxisRange != null) {
        ...
        if (firstEventFired) {
            // Next, if any data has loaded, regardless of downsample state, we need to fire an
            // update event for those series
            ...
            // Finally, if data was loaded and also the subscription is downsampled, we need to
            // notify that the downsample
            // is complete
            if (downsampleAxisRange != null) {
    ...
    /*
     * Equality of two FigureSubscriptions is based on: o identity of the original table o equality
     * of the oneclick filter o equality of the downsampleAxisRange o equality of the
     * downsampleParams EXCEPT the series array (since we permit that to change) o equality of the
     * required columns found in all includedSeries As such, the hashCode should also be based on
     * these.
     */
    ...
    public int hashCode() {
        ...
        result = 31 * result + originalTable.hashCode();// JsTable has no hashCode, so each instance
                                                        // should have its own value (until
                                                        // collisions happen)
    ...
    public boolean equals(Object o) {
        ...
        if ((other.downsampleParams == null && downsampleParams != null
            || other.downsampleParams != null && downsampleParams == null) // one is null and the
                                                                           // other isnt
            || (other.downsampleParams != null && // now we know both are non-null, check if their
                                                  // two fields that we care about match
                (other.downsampleParams.getPixelCount() != downsampleParams.getPixelCount()
                    || Arrays.equals(other.downsampleParams.getyCols(),
    ...
    public void subscribe() {
        ...
        subscription = originalTable.copy(false).then(table -> {
            ...
            if (downsampleAxisRange == null) {
                ...
                if (table.getSize() > DownsampleOptions.MAX_SERIES_SIZE) {
                    if (includedSeries.stream().map(JsSeries::getDownsampleOptions)
                        ...
                        figure.downsampleNeeded("Disable downsampling to retrieve all items",
                            includedSeries, table.getSize());
                        ...
                        figure.downsampleNeeded("Too many items to disable downsampling",
                            includedSeries, table.getSize());
                        // noinspection unchecked
            ...
            } else if (table.getSize() < 2 * (1 + downsampleParams.getyCols().length)
                * downsampleParams.getPixelCount() * MIN_DOWNSAMPLE_FACTOR) {
                // Each "pixel" requires at most 2 rows per column in the table (max and min), so we
                // check if the pixel count times that is sufficiently
                // greater than the pixel size. The MIN_DOWNSAMPLE_FACTOR field is used to define
                ...
                figure.fireEvent(JsFigure.EVENT_DOWNSAMPLESTARTED, init);
                Promise<JsTable> downsampled = tablePromise.then(t -> t
                    .downsample(zoomRange, downsampleParams.getPixelCount(),
                        downsampleAxisRange.getxCol(), downsampleParams.getyCols())
                    .then(resultTable -> Promise.resolve(resultTable), err -> {
                        ...
                        if (table.getSize() > DownsampleOptions.MAX_SERIES_SIZE) {
                            if (includedSeries.stream().map(JsSeries::getDownsampleOptions)
                                .noneMatch(options -> options == DownsampleOptions.DISABLE)) {
                                // stop, we can't downsample, we haven't been told to disable, and
                                // there are too many items to fetch them outright
                                // noinspection unchecked
    ...
    private Promise<TableSubscription> subscribe(final Promise<JsTable> tablePromise) {
        ...
        return tablePromise.then(table -> {
            ...
            // TODO, technically we can probably unsubscribe to the table at this point, since we're
            // listening to the subscription itself

            this.currentData = new ChartData(table);
            sub.addEventListener(TableSubscription.EVENT_UPDATED, e -> {
                // refire with specifics for the columns that we're watching here, after updating
                // data arrays
                ...
                CustomEventInit event = CustomEventInit.create();
                event.setDetail(new DataUpdateEvent(includedSeries.toArray(new JsSeries[0]),
                    currentData, subscriptionUpdateData));
                figure.fireEvent(JsFigure.EVENT_UPDATED, event);
