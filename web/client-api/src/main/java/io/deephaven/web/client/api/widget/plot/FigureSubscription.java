 ...
 * are designed to only match the features which can change while still maintaining the current subscription - the actual
 * "subscribed" series can change as long as they don't affect these other parameters.
 */
public final class FigureSubscription {
    /**
     * Minimum multiple that the table size needs to be decreased by in order to trigger downsampling. For example,
     * if there were 10k rows in the table, and downsampling might only reduce this to 6k, if the factor is 2 we
     * would not downsample. If there had been 15k rows, then with a factor of 2 we would downsample.
     */
    ...
    public FigureSubscription(JsFigure figure, final JsTable originalTable, final JsFigure.AxisRange downsampleAxisRange, final JsFigure.DownsampleParams downsampleParams, final Set<JsSeries> series) {
        this.figure = figure;
    ...
    public Set<JsSeries> replaceSeries(final Set<JsSeries> replacements) {
        ...
        if (firstEventFired) {
            // Next, if any data has loaded, regardless of downsample state, we need to fire an update event for those series
            ...
            // Finally, if data was loaded and also the subscription is downsampled, we need to notify that the downsample
            // is complete
            if (downsampleAxisRange != null) {
    ...
    /*
    Equality of two FigureSubscriptions is based on:
     o  identity of the original table
     o  equality of the oneclick filter
     o  equality of the downsampleAxisRange
     o  equality of the downsampleParams EXCEPT the series array (since we permit that to change)
     o  equality of the required columns found in all includedSeries
    As such, the hashCode should also be based on these.
     */

    @Override
    public int hashCode() {
        int result = figure.hashCode();
        result = 31 * result + originalTable.hashCode();//JsTable has no hashCode, so each instance should have its own value (until collisions happen)
        result = 31 * result + (downsampleAxisRange == null ? 0 : downsampleAxisRange.hashCode());
        result = 31 * result + (downsampleParams == null ? 0 : downsampleParams.getPixelCount() * 31 + Arrays.hashCode(downsampleParams.getyCols()));
    ...
    public boolean equals(Object o) {
        ...
        if ((other.downsampleParams == null && downsampleParams != null || other.downsampleParams != null && downsampleParams == null) //one is null and the other isnt
                || (other.downsampleParams != null && //now we know both are non-null, check if their two fields that we care about match
                (other.downsampleParams.getPixelCount() != downsampleParams.getPixelCount() || Arrays.equals(other.downsampleParams.getyCols(), downsampleParams.getyCols())))
        ) {
            return false;
        }
            ...
            if (downsampleAxisRange == null) {
                ...
                if (table.getSize() > DownsampleOptions.MAX_SERIES_SIZE) {
                    if (includedSeries.stream().map(JsSeries::getDownsampleOptions).noneMatch(options -> options == DownsampleOptions.DISABLE)) {
                        // stop, we can't downsample, we haven't been told to disable, and there are too many items  to fetch them outright
                ...
                // Each "pixel" requires at most 2 rows per column in the table (max and min), so we check if the pixel count times that is sufficiently
    ...
    private Promise<TableSubscription> subscribe(final Promise<JsTable> tablePromise) {
        ...
        return tablePromise.then(table -> {
            ...
            // TODO, technically we can probably unsubscribe to the table at this point, since we're listening to the subscription itself
