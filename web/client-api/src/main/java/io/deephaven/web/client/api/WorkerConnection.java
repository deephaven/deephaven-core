...
public class WorkerConnection {
    ...
    public Promise<JsTable> newTable(String[] columnNames, String[] types, String[][] data, String userTimeZone,
        ...
        return newState(failHandler, (c, cts, metadata) -> {
            ...
            ResponseStreamWrapper<PutResult> doPutResponseStream =
                    ResponseStreamWrapper.of(browserFlightServiceClient.openDoPut(schemaMessage, metadata()));
    ...
    private void flush() {
        ...
        for (ClientTableState state : statesToFlush) {
            if (state.hasNoSubscriptions()) {
                ...
                // new BidirectionStreamEmul(flightServiceClient::openDoExchange, flightServiceClient::nextDoExchange,
                // subscriptionReq, BarrageMessageType.BarrageSubscriptionRequest, reqOffset);

                ResponseStreamWrapper<FlightData> stream =
                        ResponseStreamWrapper.of(browserFlightServiceClient.openDoExchange(request, metadata));
