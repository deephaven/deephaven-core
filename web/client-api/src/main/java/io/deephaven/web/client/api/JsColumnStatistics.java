...
public class JsColumnStatistics {
    ...
    public enum StatType {
        ...
        COUNT("COUNT", "long"), SIZE("SIZE", "long"), UNIQUE_VALUES("UNIQUE VALUES", "long"), SUM(
            "SUM", null), SUM_ABS("SUM (ABS)", null), AVG("AVG", "double"), AVG_ABS("AVG (ABS)",
                "double"), MIN("MIN",
    ...
    public JsColumnStatistics(ColumnStatistics statistics) {
        ...
        if (statistics.getType() == ColumnStatistics.ColumnType.NUMERIC) {
            ...
            if (statistics.getCount() > 0) {
                statisticsMap.set(StatType.AVG.getDisplayName(),
                    statistics.getSum() / (double) statistics.getCount());
            ...
            statisticsMap.set(StatType.COUNT.getDisplayName(), (double) statistics.getCount());
            if (statistics.getCount() > 0) {
                statisticsMap.set(StatType.MIN.getDisplayName(),
                    new DateWrapper(statistics.getMinDateTime()));
                statisticsMap.set(StatType.MAX.getDisplayName(),
                    new DateWrapper(statistics.getMaxDateTime()));
            }
        } else {
            statisticsMap.set(StatType.COUNT.getDisplayName(), (double) statistics.getCount());
            if (statistics.getType() == ColumnStatistics.ColumnType.COMPARABLE) {
                statisticsMap.set(StatType.UNIQUE_VALUES.getDisplayName(),
                    (double) statistics.getNumUnique());
            }
        }
        ...
        if (statistics.getType() == ColumnStatistics.ColumnType.COMPARABLE) {
            ...
            assert keys.length == values.length
                : "Table Statistics Unique Value Count does not have the same" +
                    "number of keys and values.  Keys = " + keys.length + ", Values = "
                    + values.length;
