...
public class JsColumnStatistics {
    ...
    static {
        Arrays.stream(StatType.values()).forEach(type -> STAT_TYPE_MAP.put(type.getDisplayName(), type.getFormatType()));
    }
    ...
    public JsColumnStatistics(ColumnStatistics statistics) {
        ...
        if (statistics.getType() == ColumnStatistics.ColumnType.NUMERIC) {
            ...
            if (statistics.getCount() > 0) {
                ...
                statisticsMap.set(StatType.AVG_ABS.getDisplayName(), statistics.getAbsSum() / (double) statistics.getCount());
