...
public class WebShortColumnData extends WebColumnData {
    ...
    public void applyUpdate(
        ...
        ShortChunk<Values> shortChunk = dataIter.hasNext() ? dataIter.next().asShortChunk() : null;
        while (destOffset < newLength) {
            if (nextRemove != null && nextRemove.getFirst() == retainSourceOffset) {
                ...
                for (long ii = 0; ii < size; ++ii) {
                    while (shortChunk != null && chunkSourceOffset == shortChunk.size()) {
                        shortChunk = dataIter.hasNext() ? dataIter.next().asShortChunk() : null;
                        chunkSourceOffset = 0;
                    }
                    assert shortChunk != null;
                    short value = shortChunk.get(chunkSourceOffset++);
                    tmpStorage.setAt(destOffset++, value == QueryConstants.NULL_SHORT ? null : Js.asAny(value));
                }
