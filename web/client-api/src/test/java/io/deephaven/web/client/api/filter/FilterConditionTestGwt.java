...
public class FilterConditionTestGwt extends GWTTestCase {
    ...
    public void testCreateSimpleFilters() {
        ...
        assertEquals("ColumnName == (ignore case) 1", c.filter().eqIgnoreCase(FilterValue.ofNumber(1)).toString());
        assertEquals("ColumnName != (ignore case) 1", c.filter().notEqIgnoreCase(FilterValue.ofNumber(1)).toString());
        ...
        assertEquals("ColumnName >= 1", c.filter().greaterThanOrEqualTo(FilterValue.ofNumber(1)).toString());
        assertEquals("ColumnName <= 1", c.filter().lessThanOrEqualTo(FilterValue.ofNumber(1)).toString());

        assertEquals("ColumnName in 1", c.filter().in(arr(FilterValue.ofNumber(1))).toString());
        assertEquals("ColumnName not in 1", c.filter().notIn(arr(FilterValue.ofNumber(1))).toString());
        assertEquals("ColumnName icase in 1", c.filter().inIgnoreCase(arr(FilterValue.ofNumber(1))).toString());
        assertEquals("ColumnName icase not in 1", c.filter().notInIgnoreCase(arr(FilterValue.ofNumber(1))).toString());
