...
public class FilterConditionTestGwt extends GWTTestCase {
    ...
    private Column getColumn() {
        return new Column(0, 0, -1, -1, "int", "ColumnName", false, -1, null, false);
    }
    ...
    public void testCreateSimpleFilters() {
        ...
        assertEquals("ColumnName >= 1",
                c.filter().greaterThanOrEqualTo(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(1))).toString());
        assertEquals("ColumnName <= 1",
                c.filter().lessThanOrEqualTo(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(1))).toString());
        ...
        assertEquals("ColumnName icase in 1",
                c.filter().inIgnoreCase(arr(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(1)))).toString());
        assertEquals("ColumnName icase not in 1",
                c.filter().notInIgnoreCase(arr(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(1)))).toString());
        ...
        assertEquals("foo5(1)",
                FilterCondition.invoke("foo5", FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(1))).toString());
    ...
    public void testCreateCombinedFilters() {
        ...
        assertEquals("(ColumnName == 1 && ColumnName != 2)",
                c.filter().eq(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(1)))
                        .and(c.filter().notEq(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(2)))).toString());

        // individual OR
        assertEquals("(ColumnName == 1 || ColumnName != 2)",
                c.filter().eq(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(1)))
                        .or(c.filter().notEq(FilterValue.ofNumber(FilterValue.OfNumberUnionParam.of(2)))).toString());
