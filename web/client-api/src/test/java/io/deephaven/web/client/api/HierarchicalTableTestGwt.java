...
public class HierarchicalTableTestGwt extends AbstractAsyncGwtTestCase {
    ...
    private final TableSourceBuilder tables = new TableSourceBuilder()
            ...
            .script("ticking_rollup", "time_table('PT0.1s').update(['Y=Math.sin(i/3)', 'X=i%3']).format_columns(['Y=Y>0 ? GREEN : RED']).rollup(aggs=[agg.first('Y')],by=['X'],include_constituents=True)")
            ;
        ...
        connect(tables)
                ...
                .then(treeTable -> {
                    ...
                    return waitForEventWhere(treeTable, JsTreeTable.EVENT_UPDATED,
                            (CustomEvent<JsTreeTable.TreeViewportData> d) -> d.detail
                                    .getTreeSize() == 1
                                    && d.detail.getRows().getAtAsAny(0).<JsTreeTable.TreeViewportData.TreeRow>cast()
                                            .hasChildren(),
                            10001)
                            .then(JsTreeTable::getViewportData)
                            .then(data -> {
                                assertEquals(1.0, data.getTreeSize());
                                JsTreeTable.TreeViewportData.TreeRow row1 = data.getRows().getAt(0);
                            ...
                            .then(JsTreeTable::getViewportData)
                            .then(data -> {
                                JsTreeTable.TreeViewportData.TreeRow row2 = data.getRows().getAt(1);
                ...
                .then(rollup -> {
                    ...
                    return waitForEventWhere(rollup, JsTreeTable.EVENT_UPDATED,
                            ...
                            .then(data -> {
                                ...
                                // Wait for the expand to occur and table to show all 10 rows
                                return waitForEventWhere(rollup, JsTreeTable.EVENT_UPDATED,
                                        (CustomEvent<JsTreeTable.TreeViewportData> d) -> {
                                            return d.detail
                                                    .getTreeSize() > 4;
                                        },
                                        14008);
                            ...
                            .then(JsTreeTable::getViewportData)
                            .then(data -> {
                                JsTreeTable.TreeViewportData.TreeRow row3 = data.getRows().getAt(2);
