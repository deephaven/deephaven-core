 ...
 *
 * This is sort of the opposite of {@link ConvertInvalidInExpressions}, which converts EQs to INs when the types
...
public class ConvertEqToIn extends ReplacingVisitor {
    ...
    private FilterDescriptor handle(final FilterDescriptor descriptor, final FilterDescriptor.FilterOperation in) {
        // if one is a reference and one is a literal, we can process it and ensure the reference is first
        final FilterDescriptor first = descriptor.getChildren()[0];
        final FilterDescriptor second = descriptor.getChildren()[1];
        if (first.getOperation() == FilterDescriptor.FilterOperation.REFERENCE && second.getOperation() == FilterDescriptor.FilterOperation.LITERAL) {
            return replaceWithIn(in, first, second);
        } else if (first.getOperation() == FilterDescriptor.FilterOperation.LITERAL && second.getOperation() == FilterDescriptor.FilterOperation.REFERENCE) {
    ...
    private FilterDescriptor replaceWithIn(final FilterDescriptor.FilterOperation operation, final FilterDescriptor reference, final FilterDescriptor literal) {
        return new FilterDescriptor(operation, null, null, new FilterDescriptor[] {reference, literal});
    }
}
