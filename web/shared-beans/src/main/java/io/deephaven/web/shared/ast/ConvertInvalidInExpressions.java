...
public class ConvertInvalidInExpressions extends ReplacingVisitor {
    ...
    private FilterDescriptor handle(FilterDescriptor descriptor, FilterDescriptor.FilterOperation replacementOperation) {
        if (descriptor.getChildren().length != 2 || descriptor.getChildren()[0].getOperation() != descriptor.getChildren()[1].getOperation()) {
            return descriptor;
        }
