...
/**
 * Rewrites NOT expressions, with the goal of removing as many as possible and simplifying the expression tree. This
 * visitor delegates its work to {@link FilterDescriptor#not()}, which uses the following rules:
 * <ul>
 * <li>Distribute NOTs to children of AND/OR expressions, via DeMorgan's law.</li>
 * <li>{@code NOT(NOT(A))} is replaced with A.</li>
 * <li>Replace any operation with its opposite, if any. For example, {@code NOT(A >= B)} is replaced with {@code A < B},
 * and likewise for all the other inequality operators, {@code EQ}, and {@code IN}.</li>
