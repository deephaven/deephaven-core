...
/**
 * Enforces that all IN-type expressions have a reference on the left side and only literals
 ...
 *      by {@link ConvertInvalidInExpressions}.
 *
...
public class FlipNonReferenceMatchExpression extends ReplacingVisitor {
    ...
    private FilterDescriptor handleIn(final FilterDescriptor descriptor, final FilterDescriptor.FilterOperation op) {
        // check each child - if we pass all checks we will give up
        boolean rewrite = descriptor.getChildren()[0].getOperation() != FilterDescriptor.FilterOperation.REFERENCE;
        for (int i = 1; !rewrite && i < descriptor.getChildren().length; i++) {
            if (descriptor.getChildren()[i].getOperation() == FilterDescriptor.FilterOperation.REFERENCE) {
                rewrite = true;
            }
        }
        if (!rewrite) {
        ...
        if (count == 1) {
            ...
            return new FilterDescriptor(descriptor.getOperation(), null, null, new FilterDescriptor[] { descriptor.getChildren()[1], descriptor.getChildren()[0] });
        }
        ...
        for (int i = 0; i < count; i++) {
            replacement.getChildren()[i] = new FilterDescriptor(descriptor.getOperation(), null, null, new FilterDescriptor[] { descriptor.getChildren()[i + 1], descriptor.getChildren()[0] });
        }
