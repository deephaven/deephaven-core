 ...
 * them together into a single expression. This may change the order of expressions, but should
 * not have side effects - execution order of null checks vs invoke for example, should not be
 * affected by this. All IN expressions are moved to the front of the list.
 *
 * Examples:
 *   o  A in 1 AND B in 2 - left as is
 *   o  A not in 1 AND A not in 2 - rewritten as A not in 1, 2
 *   o  A == B OR A in 1 OR A == C OR A in 2 - rewritten as A in 1, 2 OR A == B OR A == C
 *
 * This assumes that all nested ORs and ANDs have been flattened already, NOTs normalized,
 * and that we're happy with EQs vs INs and their children.
 */
public class MergeRelatedSiblingExpressions extends ReplacingVisitor {
    ...
    public FilterDescriptor onAnd(final FilterDescriptor descriptor) {
        FilterDescriptor result = mergeChildren(descriptor, FilterDescriptor.FilterOperation.NOT_IN);
    ...
    private FilterDescriptor mergeChildren(FilterDescriptor descriptor, FilterDescriptor.FilterOperation op) {
        ...
        for (final FilterDescriptor child : descriptor.getChildren()) {
            if (child.getOperation() == op) {
                assert child.getChildren()[0].getOperation() == FilterDescriptor.FilterOperation.REFERENCE;
                collected.computeIfAbsent(child.getChildren()[0], ignore -> new LinkedHashSet<>())
                        .addAll(Arrays.stream(child.getChildren()).skip(1).collect(Collectors.toCollection(LinkedHashSet::new)));
            } else {
                leftover.add(child);
            }
        }
        ...
        if (newChildCount == 1) {
            assert leftover.isEmpty() : "Must be empty since collected is non-empty since the new and old child counts differ";
            // only one expression remains from the collection work

            Map.Entry<FilterDescriptor, Set<FilterDescriptor>> nameAndValues = collected.entrySet().iterator().next();
        ...
        int i = 0;
        for (Map.Entry<FilterDescriptor, Set<FilterDescriptor>> nameAndValues : collected.entrySet()) {
            replacementChildren[i++] = mergedFilterDescriptor(op, nameAndValues.getKey(), nameAndValues.getValue());
        }
    ...
    private FilterDescriptor mergedFilterDescriptor(FilterDescriptor.FilterOperation op, FilterDescriptor column, Set<FilterDescriptor> values) {
