...
import io.deephaven.engine.structures.chunk.util.chunkfillers.ObjectChunkFiller;
...
public class WritableObjectChunk<T, ATTR extends Any> extends ObjectChunk<T, ATTR> implements WritableChunk<ATTR> {
    ...
    public final void sort(int start, int length) {
        //noinspection unchecked
        Arrays.sort(data, offset + start, offset + start + length, (Comparator<T>) Comparator.nullsFirst(Comparator.naturalOrder()));
    }
    // endregion sort
