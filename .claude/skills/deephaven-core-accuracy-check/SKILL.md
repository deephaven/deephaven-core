...
description: >
  Review deephaven-core (Community) documentation for technical accuracy, verify claims against source code, and validate internal links. **Invoke this skill whenever:** someone asks to review/check/verify a doc, fact-check claims, confirm code examples work, verify method signatures match the API, check configuration property names, validate that described behavior matches implementation, or asks "is this technically correct?" for any docs/python or docs/groovy file. Also use when someone mentions a doc "might be wrong" or asks to "verify against source." This skill verifies against actual source code in engine/, py/server/, server/, extensions/. Do NOT use for: deephaven-ent/iris docs (use deephaven-enterprise-accuracy-check), style/formatting issues like backticks or capitalization (use deephaven-writing-style), or document reorganization (use deephaven-doc-structure-review). For quick single-paragraph checks use deephaven-core-accuracy-spot-check; for full review covering accuracy+structure+style use deephaven-docs-review-full.
