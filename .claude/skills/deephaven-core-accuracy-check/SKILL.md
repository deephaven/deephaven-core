...
4. **Style guide proofreading:** Apply the `deephaven-writing-style` skill for the full style guide — tone, quotes, links, page structure, proper noun capitalization, code formatting conventions, backticks, prose quality (active voice, jargon, audience calibration). That skill is the single source of truth; don't maintain a separate proper-noun, formatting, or punctuation list here. The mechanical grep patterns below operationalize specific `deephaven-writing-style` rules for this step — if a pattern here and that skill's stated rule ever disagree, the skill's rule wins and this list needs updating, not the other way around. Organizational concerns (heading order, duplicated explanations, topic flow, orphaned sections) are out of scope for this skill entirely — see `deephaven-doc-structure-review`.
