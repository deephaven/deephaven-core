...
3. **Technical accuracy review:**
   ...
   **Common accuracy pitfalls to check specifically:**
   ...
   - **Constants presented as universal:** Any specific technical number (chunk size, buffer size, timeout, cycle duration, a throughput multiplier) needs to be checked against every place it's actually defined in source — Configuration properties, per-class constants — not assumed to be a single value. If the number varies by code path, say so or drop the specific figure; if a claimed benchmark multiplier ("4-8x throughput") has no repo citation, drop it rather than presenting it as fact.
...
- [ ] Every corrected claim re-searched for duplicate occurrences — tables, cheat sheets, summaries — in both language files
