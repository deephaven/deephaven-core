...
description: >
  Critique the structure, organization, and readability of a deephaven-core (Community) documentation page — will a new reader find it easy to follow, are concepts introduced in a sensible order, is content duplicated or interleaved oddly? **Use this skill when:** someone says "I need to write a crash course," "I need to write a concept guide," "I need to write a how-to," "I need a high-level overview," "this doc feels hard to follow," "review organization/flow/readability," "how should I structure this doc," "is this outline right," or when drafting, restructuring, or outlining any long doc. Also use when a reviewer's feedback is about the document's shape rather than its content. Complements `deephaven-core-accuracy-check` (is it true?) and `deephaven-writing-style` (is each sentence styled correctly?) — this skill asks whether the document as a whole is organized so a reader can follow it. Calibrates checks by doc category via `ref-deephaven-doc-categories`. **Do NOT use for:** fact-checking (use accuracy-check), prose style (use writing-style), or Enterprise/deephaven-ent docs.
