...
public class BarrageMessageRoundTripTest extends LiveTableTestCase {
    private static final long UPDATE_INTERVAL = 1000; // arbitrary; we enforce coalescing on both
                                                      // sides
    ...
    private class RemoteClient {
        ...
        // The replicated table's TableUpdateValidator will be confused if the table is a viewport.
        // Instead we rely on
        ...
        RemoteClient(final Index viewport, final BitSet subscribedColumns,
            ...
            if (viewport == null) {
                ...
                // the TUV is unaware of the viewport and gets confused about which data should be
                // valid.
                // instead we rely on the validation of the content in the viewport between the
                // consumer and expected table.
                replicatedTUV = null;
        ...
        public void validate(final String msg, QueryTable expected) {
            ...
            // We expect all messages from original table to have been propagated to the replicated
            // table at this point.
            ...
            if (subscribedColumns.cardinality() != expected.getColumns().length) {
                ...
                for (int i = subscribedColumns.nextSetBit(0); i >= 0; i =
                    subscribedColumns.nextSetBit(i + 1)) {
            ...
            // Since key-space needs to be kept the same, the indexes should also be identical
            // between producer and consumer
            // (not the indexes between expected and consumer; as the consumer maintains the entire
            // index).
        ...
        public void show(QueryTable expected) {
            ...
            final Pair<String, Long> diffPair = TableTools.diffPair(toCheck, expected, maxLines,
            ...
            if (diffPair.getFirst().equals("")) {
                ...
                final long numTableRows =
                    Math.min(maxLines, Math.max(toCheck.size(), expected.size()));
                ...
                System.out
                    .println("Recomputed Table (" + name + ") Differs:\n" + diffPair.getFirst()
        ...
        public void setViewportAndColumns(final Index newViewport, final BitSet newColumns) {
            viewport = newViewport;
            subscribedColumns = newColumns;
            barrageMessageProducer.updateViewportAndColumns(dummyObserver, viewport,
                subscribedColumns);
        }
    }
    ...
    private class RemoteNugget implements EvalNuggetInterface {
        ...
        RemoteNugget(final Supplier<Table> makeTable) {
            ...
            this.barrageMessageProducer =
                originalTable.getResult(new BarrageMessageProducer.Operation<>(scheduler,
                    daggerRoot.getStreamGeneratorFactory(), originalTable, UPDATE_INTERVAL,
                    this::onGetSnapshot));
        ...
        public RemoteClient newClient(final Index viewport, final BitSet subscribedColumns,
            final String name) {
            clients
                .add(new RemoteClient(viewport, subscribedColumns, barrageMessageProducer, name));
    ...
    private abstract class TestHelper {
        ...
        TestHelper(final int numProducerCoalesce, final int numConsumerCoalesce, final int size,
        ...
        public void createTable() {
            sourceTable = getTable(size / 4, random,
                columnInfo = initColumnInfos(
                    new String[] {"Sym", "intCol", "doubleCol", "Keys", "boolCol", "TimeStamp"},
        ...
        public void createNuggets() {
            ...
            createNuggetsForTableMaker(sourceTable::flatten); // test shift aggressive version of
                                                              // these updates
            createNuggetsForTableMaker(() -> sourceTable.sort("doubleCol")); // test updates in the
                                                                             // middle of the
                                                                             // keyspace
            createNuggetsForTableMaker(() -> sourceTable.where("intCol % 12 < 5")); // test
                                                                                    // sparse(r)
                                                                                    // updates
        }
    ...
    private class OneProducerPerClient extends TestHelper {
        OneProducerPerClient(final int numProducerCoalesce, final int numConsumerCoalesce,
            final int size, final int seed, final MutableInt numSteps) {
        ...
        void createNuggetsForTableMaker(final Supplier<Table> makeTable) {
            ...
            subscribedColumns.set(0,
                nuggets.get(nuggets.size() - 1).originalTable.getColumns().length);
            nuggets.get(nuggets.size() - 1).newClient(null, subscribedColumns, "full");

            nuggets.add(new RemoteNugget(makeTable));
            nuggets.get(nuggets.size() - 1).newClient(Index.FACTORY.getIndexByRange(0, size / 10),
                subscribedColumns, "header");
            nuggets.add(new RemoteNugget(makeTable));
            nuggets.get(nuggets.size() - 1).newClient(
                Index.FACTORY.getIndexByRange(size / 2, size * 3 / 4), subscribedColumns,
                "floating");
    ...
    private class SharedProducerForAllClients extends TestHelper {
        SharedProducerForAllClients(final int numProducerCoalesce, final int numConsumerCoalesce,
            final int size, final int seed, final MutableInt numSteps) {
            super(numProducerCoalesce, numConsumerCoalesce, size, seed, numSteps);
        }
        ...
        void createNuggetsForTableMaker(final Supplier<Table> makeTable) {
            ...
            nugget.newClient(Index.FACTORY.getIndexByRange(0, size / 10), subscribedColumns,
                "header");
            nugget.newClient(Index.FACTORY.getIndexByRange(size / 2, size * 3L / 4),
    ...
    public void testAppendIncremental() {
        ...
        final Consumer<TestHelper> runOne = helper -> {
            final int maxSteps =
                MAX_STEPS * helper.numConsumerCoalesce * helper.numProducerCoalesce;
            helper.runTest(() -> {
                final long lastKey = (Math.abs(helper.random.nextLong()) % 16)
                    + (helper.sourceTable.getIndex().nonempty()
                        ? helper.sourceTable.getIndex().lastKey()
                        : -1);
        ...
        for (final int size : new int[] {10, 100, 1000}) {
            for (final int numProducerCoalesce : new int[] {1, 10}) {
                for (final int numConsumerCoalesce : new int[] {1, 10}) {
                    runOne.accept(new OneProducerPerClient(numProducerCoalesce, numConsumerCoalesce,
                        size, 0, new MutableInt(MAX_STEPS)));
                }
            }
        }
    }
    ...
    public void testPrependIncremental() {
        ...
        final Consumer<TestHelper> runOne = helper -> {
            final int maxSteps =
                MAX_STEPS * helper.numConsumerCoalesce * helper.numProducerCoalesce;
            helper.runTest(() -> {
                final long lastKey = helper.sourceTable.getIndex().nonempty()
                    ? helper.sourceTable.getIndex().lastKey()
                    : -1;
                final ShiftAwareListener.Update update = new ShiftAwareListener.Update();
                ...
                if (lastKey >= 0) {
                    shifted.shiftRange(0, lastKey,
                        stepSize + (Math.abs(helper.random.nextLong()) % 16));
                }
        ...
        for (final int size : new int[] {10, 100, 1000}) {
            for (final int numProducerCoalesce : new int[] {1, 2, 10}) {
                for (final int numConsumerCoalesce : new int[] {1, 2, 10}) {
                    runOne.accept(new SharedProducerForAllClients(numProducerCoalesce,
                        numConsumerCoalesce, size, 0, new MutableInt(MAX_STEPS)));
                }
            }
        }
    }
    ...
    public void testPrependIncrementalSharedProducer() {
        ...
        for (final int size : new int[] {10, 100, 1000}) {
            for (final int numProducerCoalsce : new int[] {1, 10}) {
                for (final int numConsumerCoalesce : new int[] {1, 10}) {
                    runOne.accept(new SharedProducerForAllClients(numProducerCoalsce,
                        numConsumerCoalesce, size, 0, new MutableInt(MAX_STEPS)));
                }
            }
        }
    ...
    public void testRoundTripIncrementalSharedProducer() {
        ...
        for (final int size : new int[] {10, 100, 1000}) {
            for (final int numProducerCoalesce : new int[] {1, 10}) {
                for (final int numConsumerCoalesce : new int[] {1, 10}) {
                    runOne.accept(new SharedProducerForAllClients(numProducerCoalesce,
                        numConsumerCoalesce, size, 0, new MutableInt(100)));
                }
            }
        }
    }
    ...
    private abstract class SubscriptionChangingHelper extends SharedProducerForAllClients {
        SubscriptionChangingHelper(final int numProducerCoalesce, final int numConsumerCoalesce,
            final int size, final int seed, final MutableInt numSteps) {
            super(numProducerCoalesce, numConsumerCoalesce, size, seed, numSteps);
        }
            ...
            for (numSteps.setValue(0); numSteps.intValue() < maxSteps; numSteps.increment()) {
                for (int rt = 0; rt < numConsumerCoalesce; ++rt) {
                    ...
                    for (int pt = 0; pt < numProducerCoalesce; ++pt) {
                        ...
                        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> GenerateTableUpdates
                            .generateShiftAwareTableUpdates(GenerateTableUpdates.DEFAULT_PROFILE,
                                size, random, sourceTable, columnInfo));
                    }
                ...
                for (final int numConsumerCoalesce : new int[] {1, 4}) {
                    for (int subProducerCoalesce =
                        ...
                        for (int subConsumerCoalesce =
                            ...
                            new SubscriptionChangingHelper(numProducerCoalesce, numConsumerCoalesce,
                                size, 0, new MutableInt(4)) {
                                {
                                    for (final RemoteNugget nugget : nuggets) {
                                        final BitSet columns = new BitSet();
                                        columns.set(0,
                                            nugget.originalTable.getColumns().length / 2);
                                        nugget.clients.add(new RemoteClient(
                                            Index.FACTORY.getIndexByRange(size / 5, 2 * size / 5),
                                ...
                                void maybeChangeSub(final int step, final int rt, final int pt) {
                                    ...
                                    for (final RemoteNugget nugget : nuggets) {
                                        final RemoteClient client =
                                            nugget.clients.get(nugget.clients.size() - 1);
                                        final BitSet columns = new BitSet();
                                        final int numColumns =
                                            nugget.originalTable.getColumns().length;
                        ...
                        for (int subConsumerCoalesce =
                            ...
                            new SubscriptionChangingHelper(numProducerCoalesce, numConsumerCoalesce,
                                ...
                                void createNuggetsForTableMaker(final Supplier<Table> makeTable) {
                                    ...
                                    columns.set(0, 4);
                                    nugget.clients.add(
                                        new RemoteClient(Index.FACTORY.getIndexByRange(0, size / 5),
                                            columns, nugget.barrageMessageProducer, "sub-changer"));
                                }
    ...
    public void testViewportSubscribeMidCycle() {
        // This is a regression test for IDS-6392. It catches a race between when a subscription
        // becomes active and
        // when the viewport becomes active post-snapshot.
        for (final int size : new int[] {10, 100, 1000}) {
            for (final int numProducerCoalesce : new int[] {2, 3, 4}) {
                for (final int numConsumerCoalesce : new int[] {1, 4}) {
                    new SubscriptionChangingHelper(numProducerCoalesce, numConsumerCoalesce, size,
                        0, new MutableInt(4)) {

                        void maybeChangeSub(final int step, final int rt, final int pt) {
                            if (step != 0 || rt != 0 || pt != 1) {
                                // Only subscribe after we have sent at least one update, but no
                                // need to subscribe again.
                                return;
                            }

                            nuggets.forEach(
                                (nugget) -> nugget.clients.forEach(RemoteClient::doSubscribe));
                        }

                        @Override
                        void createNuggetsForTableMaker(final Supplier<Table> makeTable) {
                            if (!nuggets.isEmpty()) {
                                return; // we can only have a single nugget since they all share a
                                        // single source table
                            }
                                ...
                                public void onGetSnapshot() {
                                    LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(
                                        () -> GenerateTableUpdates.generateShiftAwareTableUpdates(
                                            GenerateTableUpdates.DEFAULT_PROFILE, size, random,
                                            sourceTable, columnInfo));
                                }
                            };
                            nuggets.add(nugget);

                            // we can only have viewport subscriptions or else we won't tickle the
                            // original bug
                            final BitSet columns = new BitSet();
                            columns.set(0, 4);
                            final boolean deferSubscription = true;
                            nugget.clients.add(new RemoteClient(
                                Index.FACTORY.getIndexByRange(size / 5, 2 * size / 5), columns,
                                nugget.barrageMessageProducer, "sub-changer", deferSubscription));
                            ...
                            new SubscriptionChangingHelper(numProducerCoalesce, numConsumerCoalesce,
                                ...
                                public void createNuggets() {
                                    ...
                                    for (final RemoteNugget nugget : nuggets) {
                                        ...
                                        columns.set(0, 4);
                                        nugget.clients.add(new RemoteClient(
                                            Index.FACTORY.getIndexByRange(size / 5, 3 * size / 5),
                                            columns, nugget.barrageMessageProducer, "sub-changer"));
                                    }
                                }
                ...
                new SubscriptionChangingHelper(numProducerCoalesce, numConsumerCoalesce, size, 0,
                    new MutableInt(4)) {
                    {
                        for (final RemoteNugget nugget : nuggets) {
                            ...
                            nugget.clients.add(new RemoteClient(
                                Index.FACTORY.getIndexByRange(size / 5, 2 * size / 5), columns,
                                nugget.barrageMessageProducer, "sub-changer"));
                        }
                    }
    ...
    public void testUsePrevOnSnapshot() {
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            ...
            remoteClient.setValue(
                remoteNugget.newClient(Index.FACTORY.getIndexByRange(0, 1), cols, "prevSnapshot"));

            // flush producer in the middle of the cycle -- but we need a different thread to
            // usePrev
        ...
        Assert.equals(remoteClient.getValue().commandQueue.size(),
            "remoteClient.getValue().commandQueue.size()", 2);
    ...
    public void testRegressModificationsInPrevView() {
        ...
        final QueryTable queryTable =
            TstUtils.testRefreshingTable(i(5, 10, 12), c("intCol", 5, 10, 12));
        final RemoteNugget remoteNugget = new RemoteNugget(() -> queryTable);
        ...
        // Do not allow the two updates to coalesce; we must force the consumer to apply the
        // modification. (An allowed race.)
        flushProducerTable();
        ...
        flushProducerTable();
        Assert.equals(remoteClient.commandQueue.size(),
            "remoteClient.getValue().commandQueue.size()", 3); // mod, add, snaphot
        remoteNugget.flushClientEvents();
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(liveTableRegistrar::refresh);
        ...
        for (int size : new int[] {10, 1000, 10000}) {
            SharedProducerForAllClients helper =
                new SharedProducerForAllClients(1, 1, size, 0, new MutableInt(MAX_STEPS)) {
                    ...
                    public void createTable() {
                        columnInfo = initColumnInfos(
                            new String[] {"longCol", "intCol", "objCol", "byteCol", "doubleCol",
                                    "floatCol", "shortCol", "charCol", "boolCol", "strArrCol",
                                    "datetimeCol"},
                            new TstUtils.SortedLongGenerator(0, Long.MAX_VALUE - 1),
                            ...
                            new TstUtils.BooleanGenerator(0.2),
                            new TstUtils.SetGenerator<>(new String[] {"a", "b"},
                                new String[] {"0", "1"}, new String[] {}, null),
                            new TstUtils.UnsortedDateTimeGenerator(
                                DBTimeUtils.convertDateTime("2020-02-14T00:00:00 NY"),
                                DBTimeUtils.convertDateTime("2020-02-25T00:00:00 NY")));
            ...
            helper.runTest(() -> {
                ...
                final int stepSize = Math.max(1, helper.size / maxSteps);
                update.added =
                    Index.CURRENT_FACTORY.getIndexByRange(lastKey + 1, lastKey + stepSize);
                update.removed = i();
                if (helper.sourceTable.isEmpty()) {
                    update.modified = i();
                } else {
                    update.modified = Index.CURRENT_FACTORY
                        .getIndexByRange(Math.max(0, lastKey - stepSize), lastKey);
            ...
            SharedProducerForAllClients helper =
                new SharedProducerForAllClients(1, 1, size, 0, new MutableInt(MAX_STEPS)) {
                    ...
                    public void createTable() {
                        columnInfo = initColumnInfos(
                            new String[] {"longCol", "intCol", "objCol", "byteCol", "doubleCol",
                                    "floatCol", "shortCol", "charCol", "boolCol", "strCol",
                                    "strArrCol", "datetimeCol"},
                            new TstUtils.SortedLongGenerator(0, Long.MAX_VALUE - 1),
                            new TstUtils.IntGenerator(10, 100, 0.1),
    ...
    public static class DummyObserver implements StreamObserver<BarrageStreamGenerator.View> {
        ...
        DummyObserver(final BarrageMarshaller marshaller,
            ...
            try {
                messageView.forEachStream(inputStream -> {
                    try (final BarrageProtoUtil.ExposedByteArrayOutputStream baos =
                        ...
                        inputStream.close();
                        final BarrageMessage message = marshaller
                            .parse(new ByteArrayInputStream(baos.peekBuffer(), 0, baos.size()));
                        // we skip schema messages, but can't suppress without propagating
                        // something...
                        if (message != null) {
