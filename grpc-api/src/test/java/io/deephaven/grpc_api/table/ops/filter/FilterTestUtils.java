...
public class FilterTestUtils {
    ...
    public static Value reference(String columnName) {
        return Value.newBuilder()
            .setReference(Reference.newBuilder().setColumnName(columnName).build()).build();
    }

    public static Value literal(String stringValue) {
        return Value.newBuilder()
            .setLiteral(Literal.newBuilder().setStringValue(stringValue).build()).build();
    }
    ...
    public static Value literal(long longValue) {
        return Value.newBuilder().setLiteral(Literal.newBuilder().setLongValue(longValue).build())
            .build();
    }
    ...
    public static Value literal(double doubleValue) {
        return Value.newBuilder()
            .setLiteral(Literal.newBuilder().setDoubleValue(doubleValue).build()).build();
    }
    ...
    public static Condition eq(String columnName, int value) {
        return NormalizeFilterUtil.doComparison(CompareCondition.CompareOperation.EQUALS,
    ...
    public static Condition in(String columnName, int... values) {
        return NormalizeFilterUtil.doIn(reference(columnName), literals(values),
            CaseSensitivity.MATCH_CASE, MatchType.REGULAR);
    }

    public static Condition in(Value lhs, Value... rhs) {
        return NormalizeFilterUtil.doIn(lhs, Arrays.asList(rhs), CaseSensitivity.MATCH_CASE,
            MatchType.REGULAR);
    }

    public static Condition inICase(Value lhs, Value... rhs) {
        return NormalizeFilterUtil.doIn(lhs, Arrays.asList(rhs), CaseSensitivity.IGNORE_CASE,
            MatchType.REGULAR);
    }
    ...
    public static Condition compare(CompareCondition.CompareOperation operation, Value lhs,
        Value rhs) {
        return NormalizeFilterUtil.doComparison(operation, CaseSensitivity.MATCH_CASE, lhs, rhs);
    }

    public static Condition compare(CompareCondition.CompareOperation operation,
