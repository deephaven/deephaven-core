...
public class FilterTestUtils {
    ...
    public static Condition eq(String columnName, int value) {
        return NormalizeFilterUtil.doComparison(CompareCondition.CompareOperation.EQUALS, CaseSensitivity.MATCH_CASE, reference(columnName), literal(value));
    }
    public static Condition notEq(String columnName, int value) {
        return NormalizeFilterUtil.doComparison(CompareCondition.CompareOperation.NOT_EQUALS, CaseSensitivity.MATCH_CASE, reference(columnName), literal(value));
    }
    public static Condition in(String columnName, int... values) {
        return NormalizeFilterUtil.doIn(reference(columnName), literals(values), CaseSensitivity.MATCH_CASE, MatchType.REGULAR);
    }
    ...
    public static Condition notIn(String columnName, int... values) {
        return NormalizeFilterUtil.doIn(reference(columnName), literals(values), CaseSensitivity.MATCH_CASE, MatchType.INVERTED);
    }
    ...
    public static Condition compare(CompareCondition.CompareOperation operation, CaseSensitivity caseSensitivity, Value lhs, Value rhs) {
