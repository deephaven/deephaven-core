...
public class SessionServiceTest {
    ...
    public void testTokenRotationHasSpamProtection() {
        ...
        Assert.eq(sessionService.refreshToken(session), "sessionService.refreshToken(session)", initialToken, "initialToken");
    }
    ...
    public void testTokenLookup() {
        final SessionState session = sessionService.newSession(AUTH_CONTEXT);
        final SessionService.TokenExpiration initialToken = session.getExpiration();
        Assert.eq(sessionService.getSessionForToken(initialToken.token), "sessionService.getSessionForToken(initialToken.token)", session, "session");
    ...
    public void testSessionsAreIndependent() {
        ...
        Assert.lt(expiration2.deadline.getNanos(), "expiration2.deadline", expiration1.deadline.getNanos(), "expiration1.deadline");
