...
import io.deephaven.engine.plot.datasets.AbstractDataSeries;
import io.deephaven.engine.plot.datasets.category.AbstractCategoryDataSeries;
import io.deephaven.engine.plot.datasets.category.CategoryDataSeriesMap;
import io.deephaven.engine.plot.datasets.category.CategoryDataSeriesSwappableTableMap;
import io.deephaven.engine.plot.datasets.category.CategoryDataSeriesTableMap;
...
import io.deephaven.engine.plot.datasets.multiseries.AbstractMultiSeries;
import io.deephaven.engine.plot.datasets.multiseries.AbstractTableMapHandleMultiSeries;
import io.deephaven.engine.plot.datasets.multiseries.MultiCatSeries;
...
import io.deephaven.engine.plot.datasets.xy.AbstractXYDataSeries;
...
public class FigureWidgetTranslator {
                        ...
                        if (seriesInternal instanceof AbstractDataSeries) {
                            ...
                            if (multiSeries instanceof AbstractTableMapHandleMultiSeries) {
                                ...
                                if (tableMapMultiSeries instanceof MultiXYSeries) {
                                    ...
                                    clientAxes.add(makeTableMapSourceDescriptor(plotHandleId, multiXYSeries.getXCol(),
                                            SourceType.X, xAxis));
                                    clientAxes.add(makeTableMapSourceDescriptor(plotHandleId, multiXYSeries.getYCol(),
                                            SourceType.Y, yAxis));
                                    ...
                                    clientAxes.add(
                                            makeTableMapSourceDescriptor(plotHandleId, multiCatSeries.getCategoryCol(),
                                                    catAxis == xAxis ? SourceType.X : SourceType.Y, catAxis));
                                    clientAxes.add(
                                            makeTableMapSourceDescriptor(plotHandleId, multiCatSeries.getNumericCol(),
                                                    numAxis == xAxis ? SourceType.X : SourceType.Y, numAxis));
                                    clientSeries.setLineColor(stringMapWithDefault(mergeColors(
    ...
    private MultiSeriesSourceDescriptor makeTableMapSourceDescriptor(int plotHandleId, String columnName,
    ...
    private OneClickDescriptor makeOneClick(SwappableTableOneClickAbstract swappableTable) {
        ...
        oneClick.addAllColumnTypes(swappableTable.getByColumns()
                .stream()
                .map(colName -> swappableTable.getTableMapHandle().getTableDefinition().getColumn(colName).getDataType()
                        .getCanonicalName())
