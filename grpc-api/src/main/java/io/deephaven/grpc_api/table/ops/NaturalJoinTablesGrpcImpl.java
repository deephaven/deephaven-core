...
@Singleton
public class NaturalJoinTablesGrpcImpl extends GrpcTableOperation<NaturalJoinTablesRequest> {
    ...
    public NaturalJoinTablesGrpcImpl(final LiveTableMonitor liveTableMonitor) {
        super(BatchTableRequest.Operation::getNaturalJoin, NaturalJoinTablesRequest::getResultId, EXTRACT_DEPS);
        this.liveTableMonitor = liveTableMonitor;
    }

    @Override
    public void validateRequest(final NaturalJoinTablesRequest request) throws StatusRuntimeException {
        try {
    ...
    @Override
    public Table create(final NaturalJoinTablesRequest request, final List<SessionState.ExportObject<Table>> sourceTables) {
        ...
        final FunctionalInterfaces.ThrowingSupplier<Table, RuntimeException> doJoin =
                () -> lhs.naturalJoin(rhs, columnsToMatch, columnsToAdd);
