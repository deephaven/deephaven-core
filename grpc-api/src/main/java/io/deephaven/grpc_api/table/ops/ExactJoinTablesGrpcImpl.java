...
@Singleton
public class ExactJoinTablesGrpcImpl extends GrpcTableOperation<ExactJoinTablesRequest> {
    ...
    public ExactJoinTablesGrpcImpl(final LiveTableMonitor liveTableMonitor) {
        super(BatchTableRequest.Operation::getExactJoin, ExactJoinTablesRequest::getResultId, EXTRACT_DEPS);
        this.liveTableMonitor = liveTableMonitor;
    }

    @Override
    public void validateRequest(final ExactJoinTablesRequest request) throws StatusRuntimeException {
        try {
    ...
    @Override
    public Table create(final ExactJoinTablesRequest request, final List<SessionState.ExportObject<Table>> sourceTables) {
        ...
        final FunctionalInterfaces.ThrowingSupplier<Table, RuntimeException> doJoin =
                () -> lhs.exactJoin(rhs, columnsToMatch, columnsToAdd);
