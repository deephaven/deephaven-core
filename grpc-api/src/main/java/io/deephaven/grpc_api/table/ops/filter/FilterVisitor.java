...
public interface FilterVisitor<R> {
    ...
    R onComparison(CompareCondition.CompareOperation operation, CaseSensitivity caseSensitivity,
        Value lhs, Value rhs);

    R onIn(Value target, List<Value> candidatesList, CaseSensitivity caseSensitivity,
        MatchType matchType);
    ...
    R onContains(Reference reference, String searchString, CaseSensitivity caseSensitivity,
        MatchType matchType);

    R onMatches(Reference reference, String regex, CaseSensitivity caseSensitivity,
        MatchType matchType);
    ...
    static <R> R accept(Condition condition, FilterVisitor<R> visitor) {
        switch (condition.getDataCase()) {
            ...
            case IN:
                return visitor.onIn(condition.getIn().getTarget(),
                    condition.getIn().getCandidatesList(), condition.getIn().getCaseSensitivity(),
                    condition.getIn().getMatchType());
            case INVOKE:
                return visitor.onInvoke(condition.getInvoke().getMethod(),
                    condition.getInvoke().getTarget(), condition.getInvoke().getArgumentsList());
            case IS_NULL:
                return visitor.onIsNull(condition.getIsNull().getReference());
            case MATCHES:
                return visitor.onMatches(condition.getMatches().getReference(),
                    condition.getMatches().getRegex(), condition.getMatches().getCaseSensitivity(),
                    condition.getMatches().getMatchType());
            case CONTAINS:
                return visitor.onContains(condition.getContains().getReference(),
                    condition.getContains().getSearchString(),
                    condition.getContains().getCaseSensitivity(),
