...
public class FlipNonReferenceMatchExpression extends AbstractNormalizeFilters {
    private static final FlipNonReferenceMatchExpression INSTANCE =
        new FlipNonReferenceMatchExpression();
    ...
    public Condition onIn(Value target, List<Value> candidatesList, CaseSensitivity caseSensitivity,
        ...
        if (candidatesList.size() == 1) {
            ...
            return NormalizeFilterUtil.doIn(candidatesList.get(0),
                Collections.singletonList(target), caseSensitivity, matchType);
        }
        ...
        for (Value candidate : candidatesList) {
            replacementChildren.add(NormalizeFilterUtil.doIn(candidate,
                Collections.singletonList(target), caseSensitivity, matchType));
        }
