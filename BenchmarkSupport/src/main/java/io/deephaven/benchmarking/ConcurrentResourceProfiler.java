...
/*
 * Collects resource utilization stats using a separate thread and StatsGatherer class. Since CPU
 * Load is averaged over the last minute (by Java), this profiler doesn't provide much different CPU
 * information than sampling in afterIteration; but memory and thread stats may be more accurate
 * with this approach.
 */
public class ConcurrentResourceProfiler implements InternalProfiler {
    ...
    public Collection<? extends Result> afterIteration(BenchmarkParams benchmarkParams,
        ...
        results
            .add(new ScalarResult("Max free heap", stats.freeHeap, "bytes", AggregationPolicy.MAX));
        results
            .add(new ScalarResult("Max used heap", stats.usedHeap, "bytes", AggregationPolicy.MAX));
        results.add(
            new ScalarResult("Max threads", stats.activeThreads, "threads", AggregationPolicy.MAX));
