 ...
 * wrapping a unique {@link Random} as the RNG source for all underlying data generation.  This simplifies overall data generation
 * and makes it easier to guarantee that it produces identical tables given identical seeds.
 */
public class InMemoryBenchmarkTable extends AbstractGeneratedTable {
    /**
     * Create a {@link BenchmarkTable} that produces in memory only tables based on the inputset of {@link ColumnGenerator}s
     ...
     * @param columnGenerators The set of {@link ColumnGenerator}s used to create the internal {@link TableDefinition} and data.
     */
