 ...
 * over the last minute (by Java), this profiler doesn't provide much different CPU information than
 * sampling concurrently on another thread as is done in the ConcurrentResourceProfiler.
 */
public class ResourceProfiler implements InternalProfiler {
    ...
    public Collection<? extends Result> afterIteration(BenchmarkParams benchmarkParams,
        ...
        results
            .add(new ScalarResult("Max threads", activeThreads, "threads", AggregationPolicy.MAX));
