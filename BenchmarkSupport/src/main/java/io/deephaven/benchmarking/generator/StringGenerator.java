...
public class StringGenerator implements DataGenerator<String> {
    ...
    public StringGenerator(int minLen, int maxLen, ExtendedRandom random) {
        if (minLen <= 0 || maxLen <= 0) {
            throw new IllegalArgumentException("minLen and maxLen must be positive! (minLen=" + minLen + ", maxLen-" + maxLen + ')');
        }
        if (minLen > maxLen) {
            throw new IllegalArgumentException("minLen cannot be greater than maxLen (minLen=" + minLen + ", maxLen-" + maxLen + ')');
        }
