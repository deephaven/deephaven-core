...
public class WebsocketStreamImpl extends AbstractServerStream {
    ...
    private static final byte[] TRAILER_PREFIX = {(byte) 0x80, 0x00, 0x00, 0x00, 0x00};
    private static final byte[] TRAILER_LINE_ENDING = {'\r', '\n'};
    ...
    public void createStream(WebSocketAdapter adapter, String methodName, Metadata headers) {
        adapter.getTransportListener().streamCreated(this, methodName, headers);
    ...
    private final class Sink implements AbstractServerStream.Sink {
        ...
        public void writeHeaders(Metadata headers) {
            ...
            // trailer response must be prefixed with 0x80 (0r 0x81 if compressed), followed by four 0x00s
        ...
        public void writeFrame(@Nullable WritableBuffer frame, boolean flush, int numMessages) {
            ...
            try {
                if (frame != null) {
                    ...
                    // ByteBuffer prefix = ByteBuffer.allocate(5);
                    // prefix.put((byte) 0x00);
                    // prefix.putInt(frame.readableBytes() - 5);
                    // prefix.flip();
                    // System.out.println("payload");
                    // System.out.println(Arrays.toString(prefix.array()));

                    ByteBuffer payload =
                            ByteBuffer.wrap(((ByteArrayWritableBuffer) frame).bytes, 0, frame.readableBytes());
                    // payload.position(5);//skip first 5 bytes, we might frame differently
                    // System.out.println(Arrays.toString(payload.array()));
                    // System.out.println(new String(payload.array(), StandardCharsets.US_ASCII));
                    // logMessage("payload", prefix, payload);
                    // websocketSession.getBasicRemote().sendBinary(prefix);
        ...
        private void logMessage(String type, ByteBuffer prefix, ByteBuffer payload) {
            System.out.println(type);
            prefix = prefix.duplicate();
            payload = payload.duplicate();
            byte[] prefixBytes = new byte[prefix.remaining()];
            prefix.position(0);
            prefix.get(prefixBytes);
            System.out.println(Arrays.toString(prefixBytes));
        ...
        public void writeTrailers(Metadata trailers, boolean headersSent, Status status) {
            ...
            try {
                ...
                // System.out.println(new String(message.array(), StandardCharsets.US_ASCII));
                websocketSession.getBasicRemote().sendBinary(message);
