...
public class IntegerChunkedNumericalStats implements ChunkedNumericalStatsKernel<Integer> {
    ...
    public Result processChunks(final RowSet index, final ColumnSource<?> columnSource, boolean usePrev) {
        ...
        try (final ChunkSource.GetContext getContext = columnSource.makeGetContext(CHUNK_SIZE)) {
            ...
            while (okIt.hasMore()) {
                ...
                final IntChunk<? extends Values> chunk = (usePrev ? columnSource.getPrevChunk(getContext, nextKeys) : columnSource.getChunk(getContext, nextKeys)).asIntChunk();
