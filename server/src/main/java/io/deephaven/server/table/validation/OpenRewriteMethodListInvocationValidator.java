 ...
 * </table>
 */
public class OpenRewriteMethodListInvocationValidator implements MethodInvocationValidator {
     ...
     */
    public OpenRewriteMethodListInvocationValidator(final Collection<String> pointCuts) {
        methodMatchers = pointCuts.stream().map(MethodMatcher::new).collect(Collectors.toUnmodifiableList());
    }
     ...
     */
    private JavaType.Method toJavaType(final Method method) {
        ...
        JavaType[] parameterTypes = Arrays.stream(method.getParameters()).map(Parameter::getType).map(this::classType)
    ...
    private JavaType classType(Class<?> declaringClass) {
