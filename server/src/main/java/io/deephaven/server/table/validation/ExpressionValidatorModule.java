...
public class ExpressionValidatorModule {
    @Provides
    ColumnExpressionValidator provideMethodNameValidator() {
    ...
    public static ParsingColumnExpressionValidator getParsingColumnExpressionValidatorFromConfiguration(
        ...
        final Set<Class<?>> allowedInstanceTargets =
                getClasses(configuration, "ColumnExpressionValidator.instanceTargets.");
        final Set<Class<?>> allowedStaticTargets =
                getClasses(configuration, "ColumnExpressionValidator.staticTargets.");
        final Set<Method> allowedInstanceMethods =
                getMethods(configuration, "ColumnExpressionValidator.instanceMethods.", false);
        final Set<Method> allowedStaticMethods =
                getMethods(configuration, "ColumnExpressionValidator.staticMethods.", true);
        final Set<String> allowedAnnotationSets =
                getAnnotationSets(configuration, "ColumnExpressionValidator.annotationSets.");

        return new ParsingColumnExpressionValidator(allowedInstanceTargets, allowedStaticTargets,
                allowedInstanceMethods, allowedStaticMethods, allowedAnnotationSets);
    }
    ...
    private static Set<Method> getMethods(final Configuration configuration, final String prefix,
        ...
        configuration.getProperties(prefix).forEach((k, v) -> {
            try {
                allowedMethods.add(ParsingColumnExpressionValidator.toMethod((String) v, isStatic));
