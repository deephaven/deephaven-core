...
/**
 * Utility class to provide an enhanced view and common access point for java properties files, as well as
 * common configuration pieces such as log directories and workspace-related properties.
 */
...
public class Configuration extends PropertyFile {
    ...
    /**
     * Get the process name based on the standard process name property {@link #PROCESS_NAME_PROPERTY}.
     * Throw an exception if the property name does not exist.
     *
    ...
    /**
     * Get the process name based on the standard process name property {@link #PROCESS_NAME_PROPERTY}.
     * If the property does not exist and requireProcessName is true, throw an exception. If the property does not exist
     * and requireProcessName is false, return null.
     *
     ...
     * <ul>
     * <li>If the system property {@link #LOGDIR_PROPERTY} is set, the assumption is that a JVM parameter has been defined and it takes precedence</li>
     * <li>Otherwise, if the process name is defined, the properties {@link #LOGDIR_PROPERTY}.processName is used</li>
     * <li>If no value has been determined, the default log directory property {@link #LOGDIR_DEFAULT_PROPERTY} is used</li>
     * <li>If no value has been determined, "{@link #WORKSPACE_PROPERTY}/../logs" is used</li>
     * </ul>
     * In all cases except the first (where {@link #LOGDIR_PROPERTY} is defined with the system property), the directory is
     * normalized with {@link #normalizeLogDirectoryPath(String)}. If validateOrCreateDirectory is true, then if the filesystem
     * contains the determined directory, it is validated to be a directory, or if it does not exit it is created.
     *
     * @param validateOrCreateDirectory if true, then if the directory exists validate that it is a directory, and create it if it doesn't exist
    ...
    public String normalizeLogDirectoryPath(@NotNull final String directoryName) {
        ...
        if (directoryName.contains(LOGROOT_TOKEN)) {
            ...
            if (logDirRoot == null) {
                throw new IllegalArgumentException("Directory " + directoryName + " contains " + LOGROOT_TOKEN + " but " + LOGDIR_ROOT_PROPERTY + " property is not defined");
            }
        ...
        if (substitutedPath.contains(PROCESS_NAME_TOKEN)) {
            final String processName = getStringWithDefault(PROCESS_NAME_PROPERTY, null);
            if (processName == null) {
                throw new IllegalArgumentException("Directory " + substitutedPath + " (original path " + directoryName + ") contains " + PROCESS_NAME_TOKEN + " but " + PROCESS_NAME_PROPERTY + " property is not defined");
            } else {
        ...
        if (substitutedPath.contains(WORKSPACE_TOKEN)) {
            if (workspace == null) {
                throw new IllegalArgumentException("Directory " + substitutedPath + " (original path " + directoryName + ") contains " + WORKSPACE_TOKEN + " but " + WORKSPACE_PROPERTY + " property is not defined");
            } else {
        ...
        if (substitutedPath.contains(DEVROOT_TOKEN)) {
            if (devroot == null) {
                throw new IllegalArgumentException("Directory " + substitutedPath + " (original path " + directoryName + ") contains " + DEVROOT_TOKEN + " but " + DEVROOT_PROPERTY + " property is not defined");
            } else {
        ...
        if (substitutedPath.contains(DB_ROOT_TOKEN)) {
            ...
            if (dbRoot == null) {
                throw new IllegalArgumentException("Directory " + substitutedPath + " (original path " + directoryName + ") contains " + DB_ROOT_TOKEN + " but " + DB_ROOT_PROPERTY + " property is not defined");
            }
    ...
    public void checkDirectory(final String dir, final boolean createDirectory, final String message) throws IOException {
    ...
    /**
     * Find the name of the property specifying the root configuration file.
     * The first property set in the ordered list of candidates is returned, or null if none is set.
    ...
    protected Configuration() {
        ...
        try {
            ...
            throw new ConfigurationException("Could not process configuration from file " + confFileName + " in CLASSPATH.", x);
        }
        ...
        // The quiet property is available because things like shell scripts may be parsing our System.out and they don't
    ...
    private void load(String fileName, boolean ignoreScope) throws IOException, ConfigurationException {
        ...
        // we explicitly want to set 'properties' here so that if we get an error while loading, anything before that error shows up.
    ...
    /**
     * Return the configuration contexts for this process. This is the list of properties that may have been
     * used to parse the configuration file. If the configuration has not been parsed, this collection may be empty.
    ...
    public String lookupPath(String propertyName) {
        String result = System.getProperty(propertyName);      // In case it's been set with System.setProperty after the Configuration instance was created
        if (result == null) {
     ...
     * <ul>
     * <li>
     * Change linux-style absolute paths to platform independent absolute.
     * If the path starts with "/", replace "/" with the current directory's root (e.g. "C:\" on Windows.
     * </li>
     * <li>
    ...
    public static String expandLinuxPath(String path) {
        ...
        if (path.startsWith("~/")) {
            ...
            if (index != -1) path = path.replaceAll("~", System.getProperty("user.name"));
        }
     ...
     *
     * @param ignoreScope True if scope declarations in the property file should be ignored, false otherwise.
    ...
    void reloadProperties(boolean ignoreScope) throws IOException, ConfigurationException {
        ...
        // If any system properties exist with the same name as a property that's been declared final, that will generate
        // an exception the same way it would inside the properties file.
        properties.putAll(System.getProperties());
    }

    /**
     * ONLY the service factory is allowed to get null properties and
     * ONLY for the purposes of using default profiles when one doesn't exist.
     * This has been relocated here after many people are using defaults/nulls in the code
    ...
    /**
     * The following main method compares two directories of prop files and outputs a CSV report of the
     * differences.  Usually run before the release of a new version into prod
     *
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            out.print(propFileDiffReport(diffSet, oldEtcPath, "ise-prod.prop", newEtcPath, "ise-prod.prop", "", "", false));
            out.print('\n');
            out.print(propFileDiffReport(diffSet, oldEtcPath, "ise-stage.prop", newEtcPath, "ise-stage.prop", "", "", false));
            out.print('\n');
            out.print(propFileDiffReport(diffSet, oldEtcPath, "ise-simulation.prop", newEtcPath, "ise-simulation.prop", "", "", false));
            out.print('\n');
            out.print(propFileDiffReport(diffSet, newEtcPath, "ise-prod.prop", newEtcPath, "ise-stage.prop", newEtcPath, "ise-simulation.prop", true));
            out.print('\n');
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static String propFileDiffReport(Set<String> includedProperties, String dir1, String file1, String dir2, String file2, String dir3, String file3, boolean useDiffKeys) throws IOException {
        ...
        out.append("key,").append(dir1).append(File.separator).append(file1).append(",").
                append(dir2).append(File.separator).append(file2).append(",").
                append(dir3).append(File.separator).append(file3).append("\n");
        for (String sKey : keynames) {
            ...
            boolean bSame;
            if (dir3.length() > 0) {
                bSame = sLeftValue.equals(sRightValue) && sLeftValue.equals(sRightValue2) && sRightValue.equals(sRightValue2);
            } else {
    ...
    private static void writeLine(StringBuilder out, String sKey, String sLeftValue, String sRightValue, String sRightValue2) {
        out.append(sKey).append(", \"").append(sLeftValue).
                append("\", \"").append(sRightValue).
                append("\", \"").append(sRightValue2).
                append("\"\n");
    }
}
