...
public class PropertyFile {
    ...
    /** Gets the value of the given property, aborting if the value is
     * not specified in the system config files. Note that it is by
     * design that there is no overloaded method taking a default
     * value. Rather than scattering default values through all the
     * source files, all properties should be in one config file (as
     * much as possible). Put default values in <U>common.prop</U>. */
    ...
    private static class BooleanParser implements Parser {
        public java.lang.Boolean parse(String s) throws NumberFormatException { return Boolean.valueOf(s); }
        public String typeName() { return "boolean"; }
    }
    private static class IntegerParser implements Parser {
        public java.lang.Integer parse(String s) throws NumberFormatException { return Integer.valueOf(s); }
        public String typeName() { return "int"; }
    }
    private static class ShortParser implements Parser {
        public java.lang.Short parse(String s) throws NumberFormatException { return Short.valueOf(s); }
        public String typeName() { return "short"; }
    }
    private static class LongParser implements Parser {
        public java.lang.Long parse(String s) throws NumberFormatException { return Long.valueOf(s); }
        public String typeName() { return "long"; }
    }
    private static class DoubleParser implements Parser {
        public java.lang.Double parse(String s) throws NumberFormatException { return Double.valueOf(s); }
        public String typeName() { return "double"; }
    }

    private static class StringParser implements Parser {
        public String parse(String s) throws NumberFormatException { return s; }
        public String typeName() { return "String"; }
    }

    private <T extends Parser>
    java.lang.Object get(T parser, String propertyName,
    ...
    public Map<String, String> getNameStringMapFromProperty(final String propertyName) {
        ...
        final Matcher propertyMapMatcher = Pattern.compile("\\A\\(\\s*((?:[a-zA-Z0-9\\-\\. ]+=>[a-zA-Z0-9\\-\\. ]+,?)*)\\s*\\)\\Z").matcher(propertyValue);
        ...
        for ( String pair : propertyMapMatcher.group(1).split(",") ) {
            ...
            Require.eqNull(result.put(values[0].trim(), values[1].trim()), "result.put(values[0].trim(), values[1].trim())");
        }
        return result;
    }
     ...
     * Format is comma-separated individual values and ranges of the form start-end.
    ...
    public TIntSet getNonNegativeIntSetWithRangeSupport(final String propertyName) {
        ...
        for (int ti = 0; ti < tokens.length; ++ti) {
            ...
            if (rangeMatcher.matches()) {
                ...
                Assert.assertion(0 <= rangeBegin && rangeBegin <= rangeEnd, "0 <= rangeBegin && rangeBegin <= rangeEnd");
