...
public class ParquetFileWriter {
    ...
    public ParquetFileWriter(
        ...
        writeChannel = channelsProvider.getWriteChannel(filePath, false); // TODO add support for
                                                                          // appending
        this.type = type;
    ...
    RowGroupWriter addRowGroup(final String path, final boolean append) throws IOException {
        RowGroupWriterImpl rowGroupWriter = new RowGroupWriterImpl(path, append, channelsProvider,
            type, pageSize, allocator, compressor);
    ...
    public RowGroupWriter addRowGroup(final long size) {
        RowGroupWriterImpl rowGroupWriter =
            new RowGroupWriterImpl(writeChannel, type, pageSize, allocator, compressor);
    ...
    public void close() throws IOException {
        try (final OutputStream os = Channels.newOutputStream(writeChannel)) {
            ...
            ParquetMetadata footer = new ParquetMetadata(
                new FileMetaData(type, extraMetaData, Version.FULL_VERSION), blocks);
            serializeFooter(footer, os);
        }
        // os (and thus writeChannel) are closed at this point.
    }

    private void serializeFooter(final ParquetMetadata footer, final OutputStream os)
        throws IOException {
    ...
    private void serializeOffsetIndexes(
        ...
        for (int bIndex = 0, bSize = blocks.size(); bIndex < bSize; ++bIndex) {
            ...
            for (int cIndex = 0, cSize = columns.size(); cIndex < cSize; ++cIndex) {
                ...
                Util.writeOffsetIndex(ParquetMetadataConverter.toParquetOffsetIndex(offsetIndex),
                    os);
                column.setOffsetIndexReference(
                    new IndexReference(offset, (int) (writeChannel.position() - offset)));
            }
        }
    }
}
