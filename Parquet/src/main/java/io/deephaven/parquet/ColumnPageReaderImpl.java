    ...
    private int readRowCountFromDataPage(SeekableByteChannel file) throws IOException {
        ...
        switch (pageHeader.type) {
            case DATA_PAGE:
                ...
                BytesInput decompressedInput = decompressor.get().decompress(BytesInput.from(payload), pageHeader.getUncompressed_page_size());
    ...
    private IntBuffer readKeyFromDataPage(IntBuffer keyDest, int nullPlaceholder, SeekableByteChannel file) throws IOException {
        ...
        switch (pageHeader.type) {
            ...
            case DATA_PAGE_V2:
                ...
                BytesInput data = decompressor.get().decompress(BytesInput.from(readFully(file, dataSize)), pageHeader.getUncompressed_page_size() - dataHeaderV2.getRepetition_levels_byte_length() - dataHeaderV2.getDefinition_levels_byte_length());
