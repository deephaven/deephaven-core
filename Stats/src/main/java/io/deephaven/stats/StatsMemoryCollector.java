...
public class StatsMemoryCollector {
    ...
    private static final DirectMemoryStats DIRECT_MEMORY_STATS = AccessController.doPrivileged((PrivilegedAction<DirectMemoryStats>) JdkInternalsLoader.getInstance()::getDirectMemoryStats);
     ...
     * that the ServerStatus had an opportunity to call System.currentTimeMillis().  Because the StatsDriver is
     * generally created, we'll just make our own uptime calcuation. */
    ...
    private static class PoolState {
        ...
        public static KeyedObjectKey<String, PoolState> keyDef = new KeyedObjectKey<String, PoolState>() {
            public String getKey(PoolState v) { return v.bean.getName(); }
            public int hashKey(String k) { return k.hashCode(); }
            public boolean equalKey(String k, PoolState v) { return k.equals(v.bean.getName()); }
        };
    ...
    private static class CollectorState {
        ...
        public static KeyedObjectKey<String, CollectorState> keyDef = new KeyedObjectKey<String, CollectorState>() {
            public String getKey(CollectorState v) { return v.bean.getName(); }
            public int hashKey(String k) { return k.hashCode(); }
            public boolean equalKey(String k, CollectorState v) { return k.equals(v.bean.getName()); }
        };
                ...
                if ("ConcurrentMarkSweep".equals(bean.getName())) {
                    if (timeSample>1000 && getStatsUptime()>300000){
                        ...
                        if (enableCmsAlerts.getAsBoolean()) {
                            log.log(SimpleMailAppender.MAIL("Long GC detected -- " + System.getProperty("process.name")), "GC Time=" + timeSample);
                        }
                    }
                    ...
                    if (c - lastCount > 0 && getStatsUptime()>600000) {
                        ...
                        if (now - lastCMSOccurrence < 30000) {  // twice in 30 seconds seems like a bit much
                            if (now - lastCMSOccurrenceMailSent > TimeConstants.HOUR) {  // send at most one an hour
                                ...
                                final String message = "Last CMS collection at " + dateFormat.format(new Date(lastCMSOccurrence));
                                log.log(SimpleMailAppender.MAIL("Rapid CMS detected -- " + System.getProperty("process.name")), message);
    ...
    private final KeyedObjectHash<String, CollectorState> collectors = new KeyedObjectHash<String, CollectorState>(CollectorState.keyDef);
    ...
    public void update() {
        ...
        AccessController.doPrivileged((PrivilegedAction<Void>) () -> {
            // Obviously we are using internal Sun APIs here.  Worst case is Java 9 breaks this and we have to do it a different way
