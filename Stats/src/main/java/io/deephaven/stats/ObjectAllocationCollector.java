...
/**
 * Use the allocation instrumenter from http://code.google.com/p/java-allocation-instrumenter/ to
 * produce stats
 *
...
public class ObjectAllocationCollector {
    ...
    static {
        ...
        if (null != sEnable
            && (sEnable.toLowerCase().contains("y") || sEnable.toLowerCase().contains("t"))) {
            DUMP_STACK = true;
        }
    }
    ...
    private static class AllocationState {
        ...
        public static KeyedObjectKey<Class, AllocationState> keyDef =
            new KeyedObjectKey<Class, AllocationState>() {
                @Override
    ...
    public ObjectAllocationCollector() {
        ...
        AllocationRecorder.addSampler(new Sampler() {
            public void sampleAllocation(int count, String desc,
                ...
                // unfortunately, we can't use this b/c it syncs around the putIfAbsent AND the
                // Stats.makeItem which causes deadlocks!
                // classAllocationStates.putIfAbsent(newObj.getClass(), STATE_FACTORY).sample(size);

                final Class clazz = newObj.getClass();
                if (clazz.getName().startsWith("sun.")
                    || clazz.getName().endsWith("AllocationState"))
                    return;

                final AllocationState allocationState = classAllocationStates.get(clazz);
                if (allocationState == null) {
                    final Value value =
                        Stats.makeItem("GAllocation", clazz.getName(), State.FACTORY).getValue();
                    final AllocationState state = new AllocationState(value, clazz);
                    final AllocationState existing =
                        classAllocationStates.putIfAbsent(clazz, state);
