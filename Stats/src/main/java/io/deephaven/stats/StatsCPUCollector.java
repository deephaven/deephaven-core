...
public class StatsCPUCollector {
    ...
    private static class ThreadState {
        ...
        public static KeyedLongObjectKey<ThreadState> keyDef = new KeyedLongObjectKey<ThreadState>() {
            public Long getKey(ThreadState v) { return v.id; }
            public long getLongKey(ThreadState v) { return v.id; }
            public int hashKey(Long k) { return (int) k.longValue(); }
            public int hashLongKey(long k) { return (int) k; }
            public boolean equalKey(Long k, ThreadState v) { return k == v.id; }
            public boolean equalLongKey(long k, ThreadState v) { return k == v.id; }
        };

        public static KeyedLongObjectHash.ValueFactory<ThreadState> factory = new KeyedLongObjectHash.ValueFactory<ThreadState>() {
            public ThreadState newValue(long key) { return new ThreadState(key); }
            public ThreadState newValue(Long key) { return new ThreadState(key); }
        };
    }
        ...
        if ( hasProcStat ) {
            try {
                ...
                while (statBufferIndex < nb) {
                    while (statBufferIndex < nb && statBuffer[statBufferIndex] < 33) { statBufferIndex++; }
                    if (startsWith("cpu ", nb)) {
                        if (skipNextField(nb) && peekNextLong(nb)) {
                            ...
                            if (getNextFieldDeltaJiffies(statSysUserJiffies, nb) && skipNextField(nb) && peekNextLong(nb)) {
                                if (getNextFieldDeltaJiffies(statSysSystemJiffies, nb) && skipNextField(nb) && skipNextField(nb) && peekNextLong(nb)) {
                        ...
                        if (skipNextField(nb) && peekNextLong(nb)) {
                            if ( statSysPageIn == null ) {
                                statSysPageIn = Stats.makeItem("Kernel", "PageIn", Counter.FACTORY, "Number of pages read in from disk").getValue();
                                statSysPageOut = Stats.makeItem("Kernel", "PageOut", Counter.FACTORY, "Number of pages written to disk").getValue();
                            }
                        ...
                        if ( statSysSwapIn == null ) {
                            statSysSwapIn = Stats.makeItem("Kernel", "SwapIn", Counter.FACTORY, "Number of pages read from swap space").getValue();
                            statSysSwapOut = Stats.makeItem("Kernel", "SwapOut", Counter.FACTORY, "Number of pages written to swap space").getValue();
                        }
                        ...
                        if ( statSysCtxt == null ) {
                            statSysCtxt = Stats.makeItem("Kernel", "Ctxt", Counter.FACTORY, "Number of context switches").getValue();
                        }
                        if (skipNextField(nb)) {
        ...
        if ( hasProcPidStat ) {
            try {
                ...
                if ( statProcMinorFaults == null ) {
                    statProcMinorFaults = Stats.makeItem("Proc", "MinorFaults", Counter.FACTORY, "Minor faults the process has incurred").getValue();
                    statProcMajorFaults = Stats.makeItem("Proc", "MajorFaults", Counter.FACTORY, "Major faults the process has incurred").getValue();
                    statProcUserJiffies = Stats.makeItem("Proc", "UserJiffies", Counter.FACTORY, "User jiffies per 10 second interval (1000 equals 1 full CPU)").getValue();
                    ...
                    statProcVSZ = Stats.makeItem("Proc", "VSZ", State.FACTORY, "Virtual size of the process in kilobytes").getValue();
                    statProcRSS = Stats.makeItem("Proc", "RSS", State.FACTORY, "Resident set size of the process in pages").getValue();
                }
                statBufferIndex = 0;
        ...
        if ( hasProcFd ) {
            try {
                ...
                if ( statProcNumFDs == null ) {
                    statProcNumFDs = Stats.makeItem("Proc", "NumFDs", State.FACTORY, "Number of open file descriptors in the process").getValue();
                    statProcMaxFD = Stats.makeItem("Proc", "MaxFD", State.FACTORY, "Highest-numbered file descriptors in the process").getValue();
                }
                int maxFd = -1;
            ...
            for ( ThreadInfo tinfo : threadInfos ) {
                ...
                if ( state.name == null ) {
                    ...
                    state.userTime = Stats.makeItem("CPU", state.name+"-userTime", State.FACTORY,
                                                    "Per-thread CPU usage in user mode, 1000 equals 1 full CPU, as reported by Java").getValue();
                    state.systemTime = Stats.makeItem("CPU", state.name+"-systemTime", State.FACTORY,
                                                      "Per-thread CPU usage in system mode, 1000 equals 1 full CPU, as reported by Java").getValue();
                }
                else {
