...
public class TestLongSparseArraySource {
    ...
    private void testGet(Random random, int chunkSize) {
        ...
        final LongChunk<Values> result = source.getChunk(getContext, TrackingMutableRowSet.FACTORY.getRowSetByRange(0, 1023)).asLongChunk();
        for (int ii = 0; ii < 1024; ++ii) {
    ...
    private void checkRandomFill(int chunkSize, LongSparseArraySource source, ColumnSource.FillContext fillContext,
                                 WritableLongChunk<Values> dest, long[] expectations, TrackingMutableRowSet rowSet, boolean usePrev) {
