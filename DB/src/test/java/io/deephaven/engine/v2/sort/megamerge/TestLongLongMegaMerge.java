...
public class TestLongLongMegaMerge {
    ...
    private void testMerge(boolean ascending) {
        final LongArraySource valuesSource = new LongArraySource();
        ...
        try (final WritableLongChunk<Values> allValues = WritableLongChunk.makeWritableChunk(totalSize);
            ...
            for (int chunk = 0; chunk < chunkCount; ++chunk) {
                ...
                try (final WritableLongChunk<Values> valuesChunk = WritableLongChunk.makeWritableChunk(chunkSize);
                     final WritableLongChunk<Attributes.KeyIndices> keysChunk = WritableLongChunk.makeWritableChunk(chunkSize)) {

                    final Random random = new Random(0);

                    for (int ii = 0; ii < chunkSize; ++ii) {
                        valuesChunk.set(ii, MegaMergeTestUtils.getRandomLong(random));
                    ...
                    if (ascending) {
                        LongLongMegaMergeKernel.merge(keySource, valuesSource, 0, sizeAfterAddition - chunkSize, keysChunk, valuesChunk);
                    } else {
                        LongLongMegaMergeDescendingKernel.merge(keySource, valuesSource, 0, sizeAfterAddition - chunkSize, keysChunk, valuesChunk);
                    }
                ...
                try (final ChunkSource.GetContext valueContext = valuesSource.makeGetContext(sizeAfterAddition);
                    ...
                    final LongChunk<Values> checkValues = valuesSource.getChunk(valueContext, orderedKeys).asLongChunk();
