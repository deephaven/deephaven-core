...
public abstract class BaseTestDoubleTimSortKernel extends TestTimSortKernel {
    ...
    public static class DoublePartitionKernelStuff extends PartitionKernelStuff<DoubleLongTuple> {
        ...
        public DoublePartitionKernelStuff(List<DoubleLongTuple> javaTuples, TrackingMutableRowSet rowSet, int chunkSize, int nPartitions, boolean preserveEquality) {
            super(javaTuples.size());
            this.rowSet = rowSet;
            final int size = javaTuples.size();
...
//            dumpOffsets(offsetsOut, lengthsOut);
    ...
    static private void prepareDoubleChunks(List<DoubleLongTuple> javaTuples, WritableDoubleChunk valueChunk, WritableLongChunk<RowKeys> indexKeys) {
        for (int ii = 0; ii < javaTuples.size(); ++ii) {
    ...
    static private void prepareMultiDoubleChunks(List<DoubleLongLongTuple> javaTuples, WritableDoubleChunk valueChunk, WritableLongChunk secondaryChunk, WritableLongChunk<RowKeys> indexKeys) {
        for (int ii = 0; ii < javaTuples.size(); ++ii) {
            valueChunk.set(ii, javaTuples.get(ii).getFirstElement());
...
//        dumpChunk(valuesChunk);
    ...
    static private void verifyPartition(DoublePartitionKernel.PartitionKernelContext context, TrackingMutableRowSet source, int size, List<DoubleLongTuple> javaTuples, DoubleChunk doubleChunk, LongChunk indexKeys, ColumnSource<Double> columnSource) {
...
//        }
    ...
    static private void verify(int size, List<DoubleLongLongTuple> javaTuples, DoubleChunk primaryChunk, LongChunk secondaryChunk, LongChunk<RowKeys> indexKeys) {
//        System.out.println("Verify: " + javaTuples);
