...
public class TestDoubleSparseArraySource {
    ...
    private void testGet(Random random, int chunkSize) {
        ...
        final DoubleChunk<Values> result = source.getChunk(getContext, TrackingMutableRowSet.FACTORY.getRowSetByRange(0, 1023)).asDoubleChunk();
        for (int ii = 0; ii < 1024; ++ii) {
    ...
    private void checkRandomFill(int chunkSize, DoubleSparseArraySource source, ColumnSource.FillContext fillContext,
                                 WritableDoubleChunk<Values> dest, double[] expectations, TrackingMutableRowSet rowSet, boolean usePrev) {
