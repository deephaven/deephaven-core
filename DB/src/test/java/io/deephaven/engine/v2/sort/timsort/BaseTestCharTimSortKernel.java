...
public abstract class BaseTestCharTimSortKernel extends TestTimSortKernel {
    ...
    public static class CharPartitionKernelStuff extends PartitionKernelStuff<CharLongTuple> {
        ...
        public CharPartitionKernelStuff(List<CharLongTuple> javaTuples, TrackingMutableRowSet rowSet, int chunkSize, int nPartitions, boolean preserveEquality) {
            super(javaTuples.size());
            this.rowSet = rowSet;
            final int size = javaTuples.size();
...
//        dumpChunk(valuesChunk);
    ...
    static private void verifyPartition(CharPartitionKernel.PartitionKernelContext context, TrackingMutableRowSet source, int size, List<CharLongTuple> javaTuples, CharChunk charChunk, LongChunk indexKeys, ColumnSource<Character> columnSource) {
