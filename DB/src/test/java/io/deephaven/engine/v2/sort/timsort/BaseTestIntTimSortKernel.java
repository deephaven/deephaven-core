...
public abstract class BaseTestIntTimSortKernel extends TestTimSortKernel {
    ...
    public static class IntPartitionKernelStuff extends PartitionKernelStuff<IntLongTuple> {
        ...
        public IntPartitionKernelStuff(List<IntLongTuple> javaTuples, TrackingMutableRowSet rowSet, int chunkSize, int nPartitions, boolean preserveEquality) {
            super(javaTuples.size());
            this.rowSet = rowSet;
            final int size = javaTuples.size();
...
//            dumpOffsets(offsetsOut, lengthsOut);
    ...
    static private void prepareIntChunks(List<IntLongTuple> javaTuples, WritableIntChunk valueChunk, WritableLongChunk<RowKeys> indexKeys) {
        for (int ii = 0; ii < javaTuples.size(); ++ii) {
    ...
    static private void prepareMultiIntChunks(List<IntLongLongTuple> javaTuples, WritableIntChunk valueChunk, WritableLongChunk secondaryChunk, WritableLongChunk<RowKeys> indexKeys) {
        for (int ii = 0; ii < javaTuples.size(); ++ii) {
            valueChunk.set(ii, javaTuples.get(ii).getFirstElement());
...
//        dumpChunk(valuesChunk);
    ...
    static private void verifyPartition(IntPartitionKernel.PartitionKernelContext context, TrackingMutableRowSet source, int size, List<IntLongTuple> javaTuples, IntChunk intChunk, LongChunk indexKeys, ColumnSource<Integer> columnSource) {
...
//        }
    ...
    static private void verify(int size, List<IntLongLongTuple> javaTuples, IntChunk primaryChunk, LongChunk secondaryChunk, LongChunk<RowKeys> indexKeys) {
//        System.out.println("Verify: " + javaTuples);
