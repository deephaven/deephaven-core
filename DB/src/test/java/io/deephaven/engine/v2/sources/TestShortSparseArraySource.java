...
public class TestShortSparseArraySource {
    ...
    private void testGet(Random random, int chunkSize) {
        ...
        final ShortChunk<Values> result = source.getChunk(getContext, TrackingMutableRowSet.FACTORY.getRowSetByRange(0, 1023)).asShortChunk();
        for (int ii = 0; ii < 1024; ++ii) {
    ...
    private void checkRandomFill(int chunkSize, ShortSparseArraySource source, ColumnSource.FillContext fillContext,
                                 WritableShortChunk<Values> dest, short[] expectations, TrackingMutableRowSet rowSet, boolean usePrev) {
