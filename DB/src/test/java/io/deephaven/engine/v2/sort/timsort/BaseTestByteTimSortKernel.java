...
import io.deephaven.engine.v2.sort.findruns.ByteFindRunsKernel;
import io.deephaven.engine.v2.sort.partition.BytePartitionKernel;
...
public abstract class BaseTestByteTimSortKernel extends TestTimSortKernel {
    ...
    public static class BytePartitionKernelStuff extends PartitionKernelStuff<ByteLongTuple> {
        ...
        public BytePartitionKernelStuff(List<ByteLongTuple> javaTuples, TrackingMutableRowSet rowSet, int chunkSize, int nPartitions, boolean preserveEquality) {
            super(javaTuples.size());
            this.rowSet = rowSet;
            final int size = javaTuples.size();
...
//        dumpChunk(valuesChunk);
    ...
    static private void verifyPartition(BytePartitionKernel.PartitionKernelContext context, TrackingMutableRowSet source, int size, List<ByteLongTuple> javaTuples, ByteChunk byteChunk, LongChunk indexKeys, ColumnSource<Byte> columnSource) {
