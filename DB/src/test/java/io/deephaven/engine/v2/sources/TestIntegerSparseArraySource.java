...
public class TestIntegerSparseArraySource {
    ...
    private void testGet(Random random, int chunkSize) {
        ...
        final IntChunk<Values> result = source.getChunk(getContext, TrackingMutableRowSet.FACTORY.getRowSetByRange(0, 1023)).asIntChunk();
        for (int ii = 0; ii < 1024; ++ii) {
    ...
    private void checkRandomFill(int chunkSize, IntegerSparseArraySource source, ColumnSource.FillContext fillContext,
                                 WritableIntChunk<Values> dest, int[] expectations, TrackingMutableRowSet rowSet, boolean usePrev) {
    ...
    private void checkRangeResults(int[] expectations, int offset, RowSequence nextOk, IntChunk<Values> result) {
