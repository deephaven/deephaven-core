...
public class TestByteSparseArraySource {
    @Before
    ...
    private void testFill(Random random, int chunkSize) {
        ...
        final WritableByteChunk<Values> dest = WritableByteChunk.makeWritableChunk(chunkSize);
    ...
    private void testGet(Random random, int chunkSize) {
        ...
        final ByteChunk<Values> result = source.getChunk(getContext, TrackingMutableRowSet.FACTORY.getRowSetByRange(0, 1023)).asByteChunk();
        for (int ii = 0; ii < 1024; ++ii) {
    ...
    private void checkRandomFill(int chunkSize, ByteSparseArraySource source, ColumnSource.FillContext fillContext,
                                 WritableByteChunk<Values> dest, byte[] expectations, TrackingMutableRowSet rowSet, boolean usePrev) {
    ...
    private void checkRangeFill(int chunkSize, ByteSparseArraySource source, ColumnSource.FillContext fillContext,
                                WritableByteChunk<Values> dest, byte[] expectations, int firstKey, int lastKey, boolean usePrev) {
        int offset;
    ...
    public void testSerialization() throws Exception {
        ...
        final byte[] randomValues = ArrayGenerator.randomBytes(random, numValues);
        // Make every one out of 10 null
        for (int ii = 0; ii < numValues; ii += 10) {
            // region arrayFill
            randomValues[ii] = NULL_BYTE;
            // endregion arrayFill
        }
        ...
        final ByteSparseArraySource dest;
        try (final ByteArrayInputStream bis = new ByteArrayInputStream(bytes);
            final ObjectInputStream ois = new ObjectInputStream(bis)) {
            dest = (ByteSparseArraySource) ois.readObject();
        }
