...
public class QueryTableStaticNaturalJoinRandomTest extends QueryTableTestBase {
    ...
    private static void testNaturalJoinRandomStatic(int seed, int leftSize, int rightSize,
        Class dataType, boolean grouped, boolean flattenLeft, @Nullable JoinControl control) {
        ...
        if (dataType == int.class) {
            ...
            final TstUtils.UniqueStringGenerator uniqueStringGenerator =
            ...
            final List<TstUtils.Generator<String, String>> generatorList =
                Arrays.asList(uniqueStringGenerator, new TstUtils.SetGenerator<>(
                    duplicateRights.toArray(CollectionUtil.ZERO_LENGTH_STRING_ARRAY)));
            ...
            final TstUtils.UniqueSmartKeyGenerator uniqueSmartKeyGenerator =
                new TstUtils.UniqueSmartKeyGenerator(
                    new TstUtils.LongGenerator(0, 2 * (long) Math.sqrt(rightSize)),
                    new TstUtils.IntGenerator(0, 2 * (int) Math.sqrt(rightSize)));
            final TstUtils.SmartKeyGenerator defaultGenerator = new TstUtils.SmartKeyGenerator(
                new TstUtils.LongGenerator(0, (long) Math.sqrt(rightSize)),
        ...
        if (dataType == int.class) {
            final TIntIntMap rightMap =
                new TIntIntHashMap(rightTable.intSize(), 0.5f, -1, NULL_INT);
            ...
            final TShortIntMap rightMap =
                new TShortIntHashMap(rightTable.intSize(), 0.5f, (short) -1, NULL_INT);
            ...
            final TByteIntMap rightMap =
                new TByteIntHashMap(rightTable.intSize(), 0.5f, (byte) -1, NULL_INT);
            ...
            final TCharIntMap rightMap =
                new TCharIntHashMap(rightTable.intSize(), 0.5f, (char) -1, NULL_INT);
        ...
        if (dataType == SmartKey.class) {
            ...
            leftJoinTable = leftTable.update("JoinLong=JoinKey.get(0)", "JoinInt=JoinKey.get(1)")
                .dropColumns("JoinKey");
            rightJoinTable = rightTable.update("JoinLong=JoinKey.get(0)", "JoinInt=JoinKey.get(1)")
                .dropColumns("JoinKey");
            matchKeys = "JoinLong,JoinInt";
            updateFixup = x -> x.update("JoinLong=JoinKey.get(0)", "JoinInt=JoinKey.get(1)")
        ...
        if (control == null) {
            ...
            joined = NaturalJoinHelper.naturalJoin((QueryTable) leftJoinTable,
                (QueryTable) rightJoinTable,
                ...
                for (int seed = 0; seed < 2; ++seed) {
                    for (Class dataType : Arrays.asList(String.class, int.class, SmartKey.class)) {
                        for (boolean grouped : Arrays.asList(Boolean.TRUE, Boolean.FALSE)) {
                            System.out.println(
                                "Seed = " + seed + ", leftSize=" + leftSize + ", rightSize="
                                    + rightSize + ", type=" + dataType + ", grouped=" + grouped);
                            testNaturalJoinRandomStatic(seed, leftSize, rightSize, dataType,
                                grouped, false, null);
                        }
                    }
                }
    ...
    public void testNaturalJoinRandomStaticOverflow() {
        for (int leftSize = 10_000; leftSize <= 100_000; leftSize *= 10) {
            for (int rightSize = 10_000; rightSize <= 100_000; rightSize *= 10) {
                for (int seed = 0; seed < 2; ++seed) {
                    for (Class dataType : Arrays.asList(String.class, int.class, SmartKey.class)) {
                        System.out.println("Seed = " + seed + ", leftSize=" + leftSize
                            + ", rightSize=" + rightSize + ", type=" + dataType);
                        testNaturalJoinRandomStatic(seed, leftSize, rightSize, dataType, false,
                            false, QueryTableJoinTest.SMALL_LEFT_CONTROL);
                        testNaturalJoinRandomStatic(seed, leftSize, rightSize, dataType, false,
                            false, QueryTableJoinTest.SMALL_RIGHT_CONTROL);
                    }
                }
            }
        }
            ...
            for (int rightSize = 10_000; rightSize <= 10_000; rightSize *= 10) {
                for (int seed = 0; seed < 2; ++seed) {
                    for (Class dataType : Collections.singletonList(int.class)) {
                        for (boolean grouped : Arrays.asList(Boolean.TRUE, Boolean.FALSE)) {
                            for (JoinControl.RedirectionType redirectionType : JoinControl.RedirectionType
                                .values()) {
                                System.out.println(
                                    "Seed = " + seed + ", leftSize=" + leftSize + ", rightSize="
                                        + rightSize + ", type=" + dataType + ", grouped=" + grouped
                                ...
                                testNaturalJoinRandomStatic(seed, leftSize, rightSize, dataType,
                                    grouped,
                                    redirectionType == JoinControl.RedirectionType.Contiguous,
                                    new JoinControl() {
                                        @Override
