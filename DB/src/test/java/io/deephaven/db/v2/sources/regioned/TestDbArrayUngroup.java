...
public class TestDbArrayUngroup {
    ...
    public void testUngroup() {
        final Table theTable =
            TableTools.emptyTable(20).update("A=`a`+i%10", "B=`b`+i%5", "C=`i`+i");
        assertEquals(String.class, theTable.getDefinition().getColumn("C").getDataType());

        final Table groupedTable = theTable.by("A", "B");
        assertTrue(DbArray.class
            .isAssignableFrom(groupedTable.getDefinition().getColumn("C").getDataType()));
        ...
        assertTrue(
            DbArray.class.isAssignableFrom(actual.getDefinition().getColumn("C").getDataType()));
        assertEquals(String.class, actual.getDefinition().getColumn("C").getComponentType());

        Table ungroupedActual = actual.ungroup();
        assertFalse(DbArray.class
            .isAssignableFrom(ungroupedActual.getDefinition().getColumn("C").getDataType()));
