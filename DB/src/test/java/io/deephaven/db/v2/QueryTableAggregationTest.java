...
public class QueryTableAggregationTest {
    ...
    private static Table individualStaticByTest(@NotNull final Table input,
        @Nullable final AggregationControl aggregationControl,
        ...
        {
            // NB: We can't re-use SelectColumns across calls, hence the duplicate extraction for
            // expressions and key names.
            final SelectColumn[] keySelectColumns = SelectColumnFactory.getExpressions(keyColumns);
            final String[] keyNames = Arrays.stream(keySelectColumns).map(SelectColumn::getName)
            ...
            if (keyColumns.length == 0) {
                ...
                final SelectColumn[] allSelectColumns = Stream
                    .concat(Arrays.stream(keySelectColumns),
                        retainedColumns.stream().map(SourceColumn::new))
    ...
    public void testStaticNoKeyByWithChunks() {
        ...
        individualStaticByTest(emptyTable(10000).update("A=Integer.toString(i % 5)", "B=i / 5"),
            null);
    }
    ...
    public void testStaticReinterpretableKeyByWithChunks() {
        ...
        final Table input = emptyTable(10000).update(
            "A=ii % 100 == 0 ? null : plus(" + nowName + ", (long) (ii / 5))",
    ...
    public void testStaticByWithChunksAndAggressiveOverflow() {
        ...
        final Table input5 =
            emptyTable(10000).update("E=(ii & 1) == 0 ? ii : (ii - 1 + 0xFFFFFFFFL)");
        final Table input6 = emptyTable(10000).update("A=i", "B=i%2", "C=i%3", "D=ii");

        individualStaticByTest(
            individualStaticByTest(individualStaticByTest(input1, control, "C"), control, "B"),
            control, "A");
        individualStaticByTest(
            individualStaticByTest(individualStaticByTest(input2, control, "C"), control, "B"),
            control, "A");
    ...
    public void testStaticNameReusingByWithChunks() {
        individualStaticByTest(emptyTable(10000).update("A=i"), null, "A=Integer.toString(A % 5)",
            "A=A.hashCode()", "A=A / 2");
    }
    ...
    private static class IncrementalFirstStaticAfterByResultSupplier implements Supplier<Table> {
        ...
        private IncrementalFirstStaticAfterByResultSupplier(
        ...
        /**
         * Return an incremental by() result on first invocation, in order to establish the
         * enclosing {@link EvalNugget}'s baseline "original table". Return a static by() result on
         * subsequent invocations, in order to use the static implementation to validate the
         * incremental implementation. Note that the static implementation is well tested by its own
         * unit tests that don't rely on by().
         *
        ...
        public final Table get() {
            ...
            return ByAggregationFactory.by(control, (QueryTable) input.silent(), keySelectColumns)
                .sort(keyNames);
        }
    }
    ...
    private static EvalNugget incrementalByEvalNugget(@NotNull final AggregationControl control,
        ...
        final Supplier<Table> tableSupplier =
            new IncrementalFirstStaticAfterByResultSupplier(control, input, columns);
    ...
    private static EvalNugget incrementalByEvalNugget(@NotNull final QueryTable input,
        @NotNull String... columns) {
    ...
    public void testIncrementalByDownstreamFromMerge() {
        ...
        final EvalNugget[] ens = new EvalNugget[] {
                ...
                new EvalNugget() {
                    @Override
                    protected final Table e() {
                        return ByAggregationFactory.by(merged, "StrCol")
                            .update("IntColSum=sum(IntCol)");
                    }
                }
        };
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            ...
            inputs[1].notifyListeners(i(), ir(mergeChunkMultiple - 1_000, mergeChunkMultiple - 1),
                i());
        });
        TstUtils.validate(ens);
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            ...
            inputs[4].notifyListeners(
                new ShiftAwareListener.Update(i(), i(), ir(0, mergeChunkMultiple / 2),
    ...
    public void testIncrementalNoKeyBy() {
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            input2.notifyListeners(new ShiftAwareListener.Update(i(0, 1), i(0, 1), i(),
                IndexShiftData.EMPTY, ModifiedColumnSet.EMPTY));
        });
        TstUtils.validate(ens);

        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            input2.notifyListeners(new ShiftAwareListener.Update(i(), i(), i(2, 3),
                IndexShiftData.EMPTY, ModifiedColumnSet.ALL));
        });
        TstUtils.validate(ens);

        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            input2.notifyListeners(new ShiftAwareListener.Update(i(), i(), i(),
                IndexShiftData.EMPTY, ModifiedColumnSet.EMPTY));
        });
        TstUtils.validate(ens);
    }
    ...
    public void testStaticBy() {
        ...
        try {
            ...
            TestCase.assertEquals("Failed to get expression for all matched patterns",
                e.getMessage());
            TestCase.assertNotNull(e.getCause());
            TestCase.assertEquals("Invalid column name \"i\": \"i\" is a reserved keyword",
        ...
        TestCase.assertEquals(Arrays.asList("c", "e", "g"),
            Arrays.asList(table.by("S").getColumn("S").get(0, 3)));
        ...
        TestCase.assertEquals(Arrays.asList(5, 4, 9),
            Arrays.asList(table.by("Z=X+Y").getColumn("Z").get(0, 3)));
        ...
        TestCase.assertEquals(Arrays.asList(8, 4),
            Arrays.asList(table.by("Z=X+Y").getColumn("Z").get(0, 2)));
    ...
    public void testLastByIterative() {
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            addToTable(queryTable, i(7, 9), c("Sym", "aa", "aa"), c("intCol", 20, 10),
                c("doubleCol", 2.1, 2.2));
    ...
    public void testFirstByLastByIncremental() {
        ...
        final QueryTable table = getTable(size, random,
            columnInfo = initColumnInfos(
                new String[] {"Sym", "Sym2", "IntSet", "boolCol", "intCol", "doubleCol"},
        ...
        final EvalNuggetInterface en[] = new EvalNuggetInterface[] {
                ...
                EvalNugget.from(() -> table.sort("Sym", "intCol")
                    .by(new TrackingLastByStateFactoryImpl(), "Sym").sort("Sym")),
                EvalNugget.from(() -> table.sort("Sym", "intCol")
                    .by(new TrackingFirstByStateFactoryImpl(), "Sym").sort("Sym")),
                ...
                EvalNugget.from(() -> table.firstBy("Sym", "Sym2", "IntSet", "boolCol").sort("Sym",
                    "Sym2", "IntSet", "boolCol")),
        };
    ...
    private void testFirstOrLastByStatic(int seed, int size) {
        ...
        final Table expected =
            table.update("First=firstSet.add(Sym)").where("First").dropColumns("First");
        final Table firstBy = table.firstBy("Sym");
        assertTableEquals(expected, firstBy);

        firstSet.clear();
        final Table lastBy = table.lastBy("Sym").sort("Sym");
        final Table expectedLast = table.reverse().update("First=firstSet.add(Sym)").where("First")
            .dropColumns("First").sort("Sym");
        assertTableEquals(expectedLast, lastBy);

        final Table expectedFirstComposite = table
            .update("First=skSet.add(new io.deephaven.datastructures.util.SmartKey(Sym, intCol))")
        ...
        final Table expectedLastComposite = table.reverse()
            .update("First=skSet.add(new io.deephaven.datastructures.util.SmartKey(Sym, intCol))")
            .where("First").dropColumns("First").sort("Sym", "intCol")
            .moveUpColumns("Sym", "intCol");
    ...
    private <T> void powerSetInternal(int depth, boolean[] included, T[] elements,
        Consumer<T[]> consumer) {
        if (depth == included.length) {
            // noinspection unchecked
            consumer.accept(IntStream.range(0, included.length).filter(i -> included[i])
    ...
    public void testKeyColumnTypes() {
        ...
        final String[] columns =
            table.getColumnSourceMap().keySet().toArray(CollectionUtil.ZERO_LENGTH_STRING_ARRAY);
    ...
    public void testIncrementalBy() {
        ...
        TstUtils.validate(en);
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            addToTable(queryTable, i(7, 9), c("Sym", "bc", "bc"), c("intCol", 21, 11),
                c("doubleCol", 2.2, 2.3));
    ...
    private static void incrementalByTestSuite2() {
        ...
        final EvalNuggetInterface en[] = new EvalNuggetInterface[] {
                ...
                new EvalNugget() {
                    public Table e() {
                        return table.by("Sym").update("intColSum=sum(intCol)").ungroup()
                            .sort("Sym");
                    }
                },
                ...
                new UpdateValidatorNugget(
                    table.by("Sym").update("intColSum=sum(intCol)").ungroup()),
                new UpdateValidatorNugget(
                    table.by("Sym").update("intColSum=cumsum(intCol)").ungroup()),
                new UpdateValidatorNugget(
                    table.by("Sym").update("doubleColSum=cumsum(doubleCol)").ungroup()),
        };
    ...
    public void testApplyToAllBy() {
        ...
        TestCase.assertEquals(Arrays.asList("aa", "bc"),
            Arrays.asList(result.getColumn("Sym").get(0, 2)));
        TestCase.assertEquals(Arrays.asList(30.0, 20.0),
            Arrays.asList(result.getColumn("intCol").get(0, 2)));
        ...
        TestCase.assertEquals(Arrays.asList(10, 20, 30, 50),
            Arrays.asList(result.getColumn("intCol").get(0, 4)));
        ...
        TestCase.assertEquals(Arrays.asList("aa", "bc", "aa", "aa"),
            Arrays.asList(result.getColumn("Sym").get(0, 4)));
        TestCase.assertEquals(Arrays.asList("aa", "bc", "aa", "aa"),
            Arrays.asList(result.getColumn("Sym1").get(0, 4)));
        ...
        TestCase.assertEquals(Collections.singletonList(27.5),
            Arrays.asList(result.getColumn("intCol").get(0, 1)));
    ...
    private void testSumByStatic(int size, boolean lotsOfStrings, boolean grouped) {
        ...
        final QueryTable queryTable = getTable(false, size, random, initColumnInfos(
            new String[] {"Sym",
                    "charCol", "byteCol",
            ...
            Arrays.asList(
                grouped ? Collections.singletonList(ColumnInfo.ColAttributes.Grouped) : ea, ea, ea,
        ...
        final List<String> updates = queryTable.getDefinition().getColumnNames().stream()
            .filter(c -> !c.equals("Sym")).map(c -> c + "="
                + QueryTableAggregationTestFormulaStaticMethods.sumFunction(c) + "(" + c + ")")
            .collect(Collectors.toList());
        final Table updateResult =
            queryTable.dropColumns("Sym").by().update(Selectable.from(updates));
        ...
        final List<String> updatesAbs = queryTable.getDefinition().getColumnNames().stream()
            .filter(c -> !c.equals("Sym"))
            .map(c -> c + "=" + QueryTableAggregationTestFormulaStaticMethods.absSumFunction(c, c))
            .collect(Collectors.toList());
        final Table updateResultAbs =
            queryTable.dropColumns("Sym").by().update(Selectable.from(updatesAbs));
        ...
        final List<String> updateKeyedAbs = queryTable.getDefinition().getColumnNames().stream()
            .filter(c -> !c.equals("Sym"))
            .map(c -> c + "=" + QueryTableAggregationTestFormulaStaticMethods.absSumFunction(c, c))
            .collect(Collectors.toList());
        final Table updateKeyedResultAbs =
            queryTable.by("Sym").update(Selectable.from(updateKeyedAbs));
    ...
    private void testMinMaxByStatic(int size, boolean lotsOfStrings) {
        ...
        final List<String> updateKeyedMax =
            queryTable.getDefinition().getColumnNames().stream().filter(c -> !c.equals("Sym"))
                .map(c -> c + "=" + QueryTableAggregationTestFormulaStaticMethods.maxFunction(c))
                .collect(Collectors.toList());
        final Table updateKeyedResultMax =
    ...
    private void testAvgByStatic(int size, boolean lotsOfStrings) {
        ...
        final Table updateResult = queryTable.dropColumns("Sym").by()
            .update(Selectable.from(updates)).dropColumns(sumsAndCounts);
        assertTableEquals(updateResult, result, TableDiff.DiffItems.DoublesExact);
    ...
    private String avgExpr(String c) {
        if ("bigI".equals(c)) {
            return c + "=" + c + "_Count == 0 ? null : new java.math.BigDecimal(" + c
                + "_Sum).divide(java.math.BigDecimal.valueOf(" + c
                + "_Count), java.math.BigDecimal.ROUND_HALF_UP)";
        }
        if ("bigD".equals(c)) {
            return c + "=" + c + "_Count == 0 ? null : " + c
                + "_Sum.divide(java.math.BigDecimal.valueOf(" + c
                + "_Count), java.math.BigDecimal.ROUND_HALF_UP)";
        }
        // I would expect us to return a null for an average of nothing, but we instead return a NaN
        // return c + "=" + c + "_Count == 0 ? null : ((double)" + c + "_Sum / (double)" + c +
        // "_Count)";
        return c + "=((double)(" + c + "_Count == 0 ? 0.0 : " + c + "_Sum) / (double)" + c
            + "_Count)";
    }
    ...
    private void testSumByIncremental(final int size, final int seed, boolean grouped,
        boolean lotsOfStrings) {
        try (final SafeCloseable ignored = LivenessScopeStack.open(new LivenessScope(true), true)) {
            doTestSumByIncremental(size, seed, grouped, lotsOfStrings);
        }
    }

    private void doTestSumByIncremental(final int size, final int seed, boolean grouped,
        boolean lotsOfStrings) {
        final Random random = new Random(seed);
        final ColumnInfo[] columnInfo;
        final List<ColumnInfo.ColAttributes> ea = Collections.emptyList();
        final List<ColumnInfo.ColAttributes> ga =
            Collections.singletonList(ColumnInfo.ColAttributes.Grouped);
        final QueryTable queryTable = getTable(size, random,
            columnInfo = initColumnInfos(new String[] {"Sym",
                    "charCol",
                    "byteCol", "shortCol", "intCol", "longCol", "bigI", "bigD", "doubleCol",
                    "doubleNanCol", "boolCol"
            },
                Arrays.asList(grouped ? ga : ea, ea, ea, ea, ea, ea, ea, ea, ea, ea, ea),
                lotsOfStrings ? new StringGenerator(1000000)
        ...
        final EvalNugget[] en = new EvalNugget[] {
                ...
                EvalNugget.Sorted.from(
                    () -> queryTable.dropColumns("Sym").sort("intCol").sumBy("intCol"), "intCol"),
                EvalNugget.Sorted.from(
                    () -> queryTable.sort("Sym", "intCol").sumBy("Sym", "intCol"), "Sym", "intCol"),
                EvalNugget.Sorted
                    .from(() -> queryTable.sort("Sym").update("x=intCol+1").sumBy("Sym"), "Sym"),
                EvalNugget.Sorted.from(() -> queryTable.sortDescending("intCol")
                    .update("x=intCol+1").dropColumns("Sym").sumBy("intCol"), "intCol"),
                EvalNugget.Sorted.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .sumBy("Sym", "intCol"), "Sym", "intCol"),
                EvalNugget.Sorted.from(
                    () -> queryTable.sort("Sym", "intCol").update("x=intCol+1").sumBy("Sym"),
                    "Sym"),
                EvalNugget.Sorted.from(() -> queryTable.sort("Sym").absSumBy("Sym"), "Sym"),
                EvalNugget.Sorted.from(
                    () -> queryTable.dropColumns("Sym").sort("intCol").absSumBy("intCol"),
                    "intCol"),
                EvalNugget.Sorted.from(
                    () -> queryTable.sort("Sym", "intCol").absSumBy("Sym", "intCol"), "Sym",
                    "intCol"),
                EvalNugget.Sorted
                    .from(() -> queryTable.sort("Sym").update("x=intCol+1").absSumBy("Sym"), "Sym"),
                EvalNugget.Sorted.from(() -> queryTable.sortDescending("intCol")
                    .update("x=intCol+1").dropColumns("Sym").absSumBy("intCol"), "intCol"),
                EvalNugget.Sorted.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .absSumBy("Sym", "intCol"), "Sym", "intCol"),
                EvalNugget.Sorted.from(
                    () -> queryTable.sort("Sym", "intCol").update("x=intCol+1").absSumBy("Sym"),
                    "Sym"),
        };
    ...
    public void testAbsSumBySimple() {
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            TstUtils.addToTable(table, i(8), col("BigI", BigInteger.valueOf(5)),
                col("DoubleCol", 5.0), col("BoolCol", true));
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            TstUtils.addToTable(table, i(8), col("BigI", BigInteger.valueOf(4)),
                col("DoubleCol", 4.0), col("BoolCol", false));
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            TstUtils.addToTable(table, i(10), col("BigI", BigInteger.valueOf(0)),
                col("DoubleCol", Double.NaN), col("BoolCol", true));
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            TstUtils.addToTable(table, i(12, 14),
                col("BigI", BigInteger.valueOf(0), BigInteger.valueOf(0)),
    ...
    public void testAbsSumByNull() {
        final QueryTable table =
            TstUtils.testRefreshingTable(i(2), intCol("IntCol", QueryConstants.NULL_INT),
    ...
    public void testAvgInfinities() {
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            TstUtils.addToTable(table, i(9), col("IntCol", 6),
                floatCol("FloatCol", Float.POSITIVE_INFINITY));
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            TstUtils.addToTable(table, i(10), col("IntCol", 7),
                floatCol("FloatCol", Float.NEGATIVE_INFINITY));
            table.notifyListeners(i(10), i(), i());
        });
        show(result);
        avg = result.getColumn("IntCol").getDouble(0);
    ...
    private void testAvgByIncremental(int size) {
        ...
        final EvalNugget en[] = new EvalNugget[] {
                ...
                EvalNugget.from(() -> queryTable.dropColumns("Sym").sort("intCol").avgBy("intCol")
                    .sort("intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").avgBy("Sym", "intCol")
                    .sort("Sym", "intCol")),
                EvalNugget.from(
                    () -> queryTable.sort("Sym").update("x=intCol+1").avgBy("Sym").sort("Sym")),
                EvalNugget.from(() -> queryTable.sortDescending("intCol").update("x=intCol+1")
                    .dropColumns("Sym").avgBy("intCol").sort("intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .avgBy("Sym", "intCol").sort("Sym", "intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .avgBy("Sym").sort("Sym")),
        };
        for (int i = 0; i < 50; i++) {
    ...
    private void testStdVarByIncremental(int size) {
        ...
        final QueryTable queryTable = getTable(size, random,
            columnInfo = initColumnInfos(
                new String[] {"Sym", "byteCol", "charCol", "shortCol", "intCol", "longCol",
                        "floatCol", "doubleCol", "bigI", "bigD"},
                new SetGenerator<>("a", "b", "c", "d"),
        ...
        final String decimalCmp = integerCmp.replaceAll("DiffI", "DiffD")
            .replaceAll("doubleI", "doubleD").replaceAll("bigI", "bigD");
        ...
        final Table bigAsDouble = queryTable
            .view("Sym", "bigI", "bigD", "doubleI=bigI.doubleValue()", "doubleD=bigD.doubleValue()")
            .sort("Sym");
    ...
    public void testWeightedAvgByLong() {
        final QueryTable table = TstUtils.testRefreshingTable(i(2, 4, 6), col("Long1", 2L, 4L, 6L),
            col("Long2", 1L, 2L, 3L));
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            TstUtils.addToTable(table, i(8), col("Long1", (long) Integer.MAX_VALUE),
                col("Long2", 7L));
            table.notifyListeners(i(8), i(), i());
        });
        show(result);
    ...
    private void testWeightedAvgByIncremental(int size, int seed) {
        ...
        final EvalNuggetInterface[] en = new EvalNuggetInterface[] {
                ...
                EvalNugget.Sorted.from(
                    () -> queryTable.view("intCol", "Sym", "doubleCol").wavgBy("intCol", "Sym"),
                    "Sym"),
                EvalNugget.Sorted.from(() -> queryTable
                    .view("doubleCol", "intCol", "intCol2", "Sym").wavgBy("doubleCol", "Sym"),
                    "Sym"),
    ...
    private void testWeightedSumByIncremental(int size, int seed) {
        ...
        final EvalNuggetInterface[] en = new EvalNuggetInterface[] {
                ...
                EvalNugget.Sorted.from(
                    () -> queryTable.view("intCol", "Sym", "doubleCol").wsumBy("intCol", "Sym"),
                    "Sym"),
                EvalNugget.Sorted.from(() -> queryTable
                    .view("doubleCol", "intCol", "intCol2", "Sym").wsumBy("doubleCol", "Sym"),
                    "Sym"),
                ...
                new TableComparator(queryTable.view("intCol2", "intCol").wsumBy("intCol2"), "wsum",
                    queryTable.updateView("W=intCol*intCol2").by().update("intCol=(long)sum(W)")
                        .view("intCol"),
                    "update"),
                new TableComparator(queryTable.view("intCol2", "doubleCol").wsumBy("intCol2"),
                    "wsum",
                    queryTable.updateView("W=doubleCol*intCol2").by().update("doubleCol=sum(W)")
                        .view("doubleCol"),
                    "update"),
                new TableComparator(queryTable.view("doubleCol", "intCol").wsumBy("doubleCol"),
                    "wsum",
                    queryTable.updateView("W=doubleCol*intCol").by().update("intCol=sum(W)")
                        .view("intCol"),
                    "update"),
                new TableComparator(queryTable.view("doubleCol", "doubleCol2").wsumBy("doubleCol2"),
                    "wsum",
    ...
    private void testCountByIncremental(int size) {
        ...
        final QueryTable queryTable = getTable(size, random,
            columnInfo = initColumnInfos(new String[] {"Sym", "intCol", "doubleCol"},
                new SetGenerator<>("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m",
                    "n", "o", "p"),
                new IntGenerator(10, 100),
                new SetGenerator<>(10.1, 20.1, 30.1)));
        final EvalNuggetInterface[] en = new EvalNuggetInterface[] {
                EvalNugget.from(() -> queryTable.countBy("Count", "Sym").sort("Sym")),
                new UpdateValidatorNugget(queryTable.sort("intCol").countBy("Count", "Sym")
                    .view("Count=Count * 2", "Sym")),
                new UpdateValidatorNugget(queryTable.sort("doubleCol").avgBy("Sym")
                    .view("doubleCol=doubleCol*2", "intCol")),
        };
    ...
    private void testMinMaxByIncremental(int size, int seed) {
        ...
        final QueryTable queryTable = getTable(size, random,
            columnInfo = initColumnInfos(
                new String[] {"Sym", "intCol", "shortCol", "byteCol", "doubleCol", "Timestamp",
                        "boolCol", "betterDoubleCol", "floatCol"},
                new SetGenerator<>("a", "b", "c", "d"),
                new IntGenerator(10, 100, 0.1),
        ...
        final EvalNuggetInterface[] en = new EvalNuggetInterface[] {
                ...
                EvalNugget.from(() -> queryTable.dropColumns("Sym").sort("intCol").maxBy("intCol")
                    .sort("intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").maxBy("Sym", "intCol")
                    .sort("Sym", "intCol")),
                EvalNugget.from(
                    () -> queryTable.sort("Sym").update("x=intCol+1").maxBy("Sym").sort("Sym")),
                EvalNugget.from(() -> queryTable.sortDescending("intCol").update("x=intCol+1")
                    .dropColumns("Sym").maxBy("intCol").sort("intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .maxBy("Sym", "intCol").sort("Sym", "intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .maxBy("Sym").sort("Sym")),
                EvalNugget.from(() -> queryTable.minBy("Sym").sort("Sym")),
                EvalNugget.from(() -> queryTable.sort("Sym").minBy("Sym")),
                EvalNugget.from(() -> queryTable.dropColumns("Sym").sort("intCol").minBy("intCol")
                    .sort("intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").minBy("Sym", "intCol")
                    .sort("Sym", "intCol")),
                EvalNugget.from(
                    () -> queryTable.sort("Sym").update("x=intCol+1").minBy("Sym").sort("Sym")),
                EvalNugget.from(() -> queryTable.sortDescending("intCol").update("x=intCol+1")
                    .dropColumns("Sym").minBy("intCol").sort("intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .minBy("Sym", "intCol").sort("Sym", "intCol")),
                EvalNugget.from(() -> queryTable.sort("Sym", "intCol").update("x=intCol+1")
                    .minBy("Sym").sort("Sym")),
    ...
    private void testMinMaxByAppend(int size) {
        ...
        final EvalNuggetInterface[] en = new EvalNuggetInterface[] {
                new EvalNugget() {
                    public Table e() {
                        return queryTable.by(new AddOnlyMinMaxByStateFactoryImpl(false), "Sym")
                            .sort("Sym");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return queryTable.dropColumns("Sym").update("x = k")
                            .by(new AddOnlyMinMaxByStateFactoryImpl(false), "intCol")
                            .sort("intCol");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return queryTable.updateView("x = k")
                            .by(new AddOnlyMinMaxByStateFactoryImpl(false), "Sym", "intCol")
                            .sort("Sym", "intCol");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return queryTable.update("x=intCol+1")
                            .by(new AddOnlyMinMaxByStateFactoryImpl(false), "Sym").sort("Sym");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                ...
                new EvalNugget() {
                    public Table e() {
                        return queryTable.by(new AddOnlyMinMaxByStateFactoryImpl(true), "Sym")
                            .sort("Sym");
                    }
                },
                new EvalNugget() {
                ...
                new EvalNugget() {
                    public Table e() {
                        return queryTable.updateView("x = k")
                            .by(new AddOnlyMinMaxByStateFactoryImpl(true), "Sym", "intCol")
                            .sort("Sym", "intCol");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return queryTable.update("x=intCol+1")
                            .by(new AddOnlyMinMaxByStateFactoryImpl(true), "Sym").sort("Sym");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                ...
                new TableComparator(
                    queryTable.by(new AddOnlyMinMaxByStateFactoryImpl(false), "Sym").sort("Sym"),
                    queryTable.applyToAllBy("max(each)", "Sym").sort("Sym")),
                new TableComparator(
                    queryTable.by(new AddOnlyMinMaxByStateFactoryImpl(true), "Sym").sort("Sym"),
                    queryTable.applyToAllBy("min(each)", "Sym").sort("Sym")),
        };
        for (int step = 0; step < 50; step++) {
            LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
                final Index keysToAdd =
                    TstUtils.newIndex(random.nextInt(size / 2 + 1), queryTable.getIndex(), random);
    ...
    private void testMedianByIncremental(int size) {
        ...
        final EvalNuggetInterface[] en = new EvalNuggetInterface[] {
                ...
                EvalNugget.from(() -> withoutFloats
                    .by(new PercentileByStateFactoryImpl(0.25), "Sym").sort("Sym")),
                EvalNugget.from(() -> withoutFloats
                    .by(new PercentileByStateFactoryImpl(0.75), "Sym").sort("Sym")),
                EvalNugget.from(() -> withoutFloats.by(new PercentileByStateFactoryImpl(0.1), "Sym")
                    .sort("Sym")),
                EvalNugget.from(() -> withoutFloats
                    .by(new PercentileByStateFactoryImpl(0.99), "Sym").sort("Sym")),
                EvalNugget.from(() -> withoutFloats.where("Sym=`a`")
                    .by(new PercentileByStateFactoryImpl(0.99), "Sym").sort("Sym"))
        };
        for (int step = 0; step < 50; step++) {
            if (LiveTableTestCase.printTableUpdates) {
    ...
    public void testTDigest() {
        ...
        final Table aggregated =
            ApproximatePercentile.approximatePercentile(queryTable.dropColumns("Sym"), 0.99);
        TableTools.showWithIndex(aggregated);

        final Table aggregatedBySym =
            ApproximatePercentile.approximatePercentile(queryTable, 0.99, "Sym");
        ...
        for (final String sym : new String[] {"a", "b", "c", "d"}) {
            ...
            checkTableP99(queryTable.where("Sym=`" + sym + "`"),
    ...
    public void testTDigestMulti() {
        ...
        final ApproximatePercentile.PercentileDefinition definition =
            new ApproximatePercentile.PercentileDefinition("doubleCol").add(0.75, "DP75")
                .add(0.95, "DP95").add(0.99, "DP99").add(0.999, "DP999").nextColumn("floatCol")
                .add(0.75, "FP75").add(0.99, "FP99");
        final Table aggregated =
            ApproximatePercentile.approximatePercentiles(queryTable.dropColumns("Sym"), definition);
        TableTools.showWithIndex(aggregated);

        final Table aggregatedBySym =
            ApproximatePercentile.approximatePercentiles(queryTable, definition, "Sym");
        TableTools.showWithIndex(aggregatedBySym);
    ...
    public void testTDigestAccumulation() {
        ...
        final ApproximatePercentile.PercentileDefinition definition =
            new ApproximatePercentile.PercentileDefinition("doubleCol").exposeDigest("Digest")
                .add(0.95, "P95").setCompression(33);
        final Table aggregated =
            ApproximatePercentile.approximatePercentiles(queryTable.dropColumns("Sym"), definition);
        TableTools.showWithIndex(aggregated);

        final Table aggregatedBySym = ApproximatePercentile.approximatePercentiles(queryTable,
            definition.setCompression(100), "Sym");
        TableTools.showWithIndex(aggregatedBySym);
    ...
    private void checkTableP99(Table queryTable, Table aggregated) {
        final double[] dValues =
            (double[]) queryTable.where("!Double.isNaN(doubleCol) && !isNull(doubleCol)")
        ...
        final float[] fValues = (float[]) queryTable
            .where("!Float.isNaN(floatCol) && !isNull(floatCol)").getColumn("floatCol").getDirect();
        Arrays.sort(fValues);
        ...
        final int[] iValues =
            (int[]) queryTable.where("!isNull(intCol)").getColumn("intCol").getDirect();
        Arrays.sort(iValues);
    ...
    private void checkTableComboPercentiles(Table queryTable, Table aggregated) {
        ...
        System.out
            .println("Double 99.9:  " + dValue999 + ", " + dtValue999 + ", Error: " + derror999);
    ...
    public void testTDigestIncremental() {
        ...
        final ApproximatePercentile.PercentileDefinition definition =
            new ApproximatePercentile.PercentileDefinition("doubleCol").add(0.75, "DP75")
                .add(0.95, "DP95").add(0.99, "DP99").add(0.999, "DP999").nextColumn("longCol")
                .add(0.75, "LP75").add(0.95, "LP95").add(0.99, "FP99").add(0.999, "LP999");
                    ...
                    void checkDifferences(String msg, Table recomputed) {
                        ...
                        for (final Map.Entry<String, ? extends ColumnSource> columnSourceEntry : rc
                            ...
                            final double error = Math.abs(
                                (recomputedPercentile - originalPercentile) / recomputedPercentile);
                            if (error > .01) {
                        ...
                        for (final Map.Entry<String, ? extends ColumnSource> columnSourceEntry : rc
                            ...
                            if (name.equals("Sym")) {
                                ...
                                for (int ii = 0; ii < 4; ++ii) {
                                    ...
                                    final double error =
                                        Math.abs((recomputedPercentile - originalPercentile)
                                            / recomputedPercentile);
                                    if (error > .025) {
                                        throw new ComparisonFailure(
                                            "Bad percentile for " + name + ", error=" + error,
                                            Double.toString(recomputedPercentile),
                                            Double.toString(originalPercentile));
                                    }
                                }
        ...
        for (int step = 0; step < 10; ++step) {
            ...
            LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
                final Index added =
                    Index.FACTORY.getIndexByRange(size * (fstep + 1), size * (fstep + 2) - 1);
    ...
    public void testMedianTypes() {
        ...
        final IncrementalReleaseFilter incrementalReleaseFilter =
            new IncrementalReleaseFilter(5, 10);
        final Table updated =
            table.update("KeyCol=`KeyCol`").where(incrementalReleaseFilter).select();
    ...
    public void testSelectDistinct() {
        ...
        TestCase.assertEquals(1, result.getColumns().length);
        TestCase.assertEquals(Arrays.asList(1, 2, 3),
            Arrays.asList(result.getColumn("x").get(0, 3)));
        ...
        TestCase.assertEquals(1, result.getColumns().length);
        TestCase.assertEquals(Arrays.asList(1, 2, 3),
            Arrays.asList(result.getColumn("y").get(0, 3)));
        ...
        TestCase.assertEquals(Arrays.asList(1, 2, 2, 3),
            Arrays.asList(result.getColumn("x").get(0, 4)));
        TestCase.assertEquals(Arrays.asList(1, 2, 3, 3),
            Arrays.asList(result.getColumn("y").get(0, 4)));
    }
    ...
    private class SelectDistinctEvalNugget implements EvalNuggetInterface {
        ...
        SelectDistinctEvalNugget(Table sourceTable, String... columns) {
            ...
            ((QueryTable) originalValue)
                .listenForUpdates(new InstrumentedShiftAwareListener("Failure Listener") {
                    @Override
        ...
        public void validate(final String msg) {
            ...
            final String diff1 =
                diff(originalValue, check1, 10, EnumSet.of(TableDiff.DiffItems.DoublesExact));
            Assert.assertEquals(msg, "", diff1);

            // we can also check the table's validity against a countBy
            final Table check2 =
                sourceTable.countBy("__TEMP__", columns).dropColumns("__TEMP__").sort(columns);
            final String diff2 =
                diff(originalValue, check2, 10, EnumSet.of(TableDiff.DiffItems.DoublesExact));
    ...
    public void testSelectDistinctUpdates() {
        ...
        // now let's remove one of our rows, but not the last one with a given value, also expecting
        // no changes
        ...
        // now modify it so that we generate a new key, but don't change the existing key's
        // existence
        // and modify something, but keep the key the same
    ...
    public void testFirstByShift() {
        final QueryTable table = TstUtils.testRefreshingTable(i(1, 2, 4097),
            intCol("Sentinel", 1, 2, 4097), col("Bucket", "A", "B", "A"));
    ...
    public void testFirstLastByAttributes() {
        ...
        if (SystemicObjectTracker.isSystemicObjectMarkingEnabled()) {
            ...
            TestCase.assertEquals(new LinkedHashSet<>(
    ...
    public void testIds6203() {
        ...
        final QueryTable table = TstUtils.testRefreshingTable(
            Index.FACTORY.getIndexByRange(100, 100 + keyValues.length - 1),
        ...
        final FuzzerPrintListener subPrintListener =
            new FuzzerPrintListener("subTable", subTable, 0);
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            ...
            final Index removals =
                Index.FACTORY.getIndexByRange(6000 + newSize, 6000 + newSize * 3);
            TstUtils.addToTable(table, additions, col("Key", keys2), intCol("IntCol", sentinel2));
            TstUtils.removeRows(table, removals);
            table.notifyListeners(additions, removals, i());
        });
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            ...
            final Index removals =
                Index.FACTORY.getIndexByRange(6000 + newSize * 4, 6000 + newSize * 5 - 1);
    ...
    public void testIds6321() {
        ...
        final FuzzerPrintListener printListener =
            new FuzzerPrintListener("exposedLastBy", exposedLastBy);
    ...
    public void testIds6593() {
        ...
        final boolean substitutionWasEnabled =
        ...
        try {
            for (final boolean substituteForThisIteration : new boolean[] {false, true}) {
                ...
                LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
                    ...
                    source.notifyListeners(new ShiftAwareListener.Update(ir(50, 100), i(), i(),
                        IndexShiftData.EMPTY, ModifiedColumnSet.EMPTY));
                });
            }
        } finally {
