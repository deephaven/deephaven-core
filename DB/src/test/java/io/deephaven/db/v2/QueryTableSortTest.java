...
public class QueryTableSortTest extends QueryTableTestBase {
    ...
    public void testSort() {
        final Table result0 =
            newTable(c("Unsorted", 3.0, null, 2.0), c("DataToSort", "c", "a", "b"));
        ...
        Table result =
            newTable(c("Unsorted", 3, 1, 2), c("DataToSort", "c", "a", "b")).sort("DataToSort");
        assertEquals(Arrays.asList(1, 2, 3), Arrays.asList(result.getColumn("Unsorted").get(0, 3)));
        assertEquals(Arrays.asList("a", "b", "c"),
            Arrays.asList(result.getColumn("DataToSort").get(0, 3)));
        result = newTable(c("Unsorted", 3, 1, 2), c("DataToSort", "c", "a", "b"))
        ...
        result =
            newTable(c("Unsorted", '3', '1', '2'), c("DataToSort", "c", "a", "b")).sort("Unsorted");
        assertEquals(Arrays.asList('1', '2', '3'),
            Arrays.asList(result.getColumn("Unsorted").get(0, 3)));
        assertEquals(Arrays.asList("a", "b", "c"),
            Arrays.asList(result.getColumn("DataToSort").get(0, 3)));
        result = newTable(c("Unsorted", '3', '1', '2'), c("DataToSort", "c", "a", "b"))
        ...
        final Table input = newTable(c("C1", 2, 4, 2, 4), c("C2", '1', '1', '2', '2'),
            c("Witness", "a", "b", "c", "d"));
        ...
        assertEquals(Arrays.asList('1', '2', '1', '2'),
            Arrays.asList(result.getColumn("C2").get(0, 4)));
        assertEquals(Arrays.asList("a", "c", "b", "d"),
            Arrays.asList(result.getColumn("Witness").get(0, 4)));

        result = newTable(c("C1", 2, 4, 2, 4), c("C2", '2', '2', '1', '1'),
            c("Witness", "a", "b", "c", "d")).sort("C2", "C1");
        ...
        result = newTable(c("C1", 2, 4, 2, 4), c("C2", '1', '1', '2', '2'),
            c("Witness", "a", "b", "c", "d")).sortDescending("C1", "C2");
        ...
        result = newTable(c("C1", 2, 4, 2, 4), c("C2", '2', '2', '1', '1'),
            c("Witness", "a", "b", "c", "d")).sortDescending("C2", "C1");
        ...
        assertEquals(Arrays.asList(null, '1', '2', '3'),
            Arrays.asList(result.getColumn("Unsorted").get(0, 4)));
        assertEquals(Arrays.asList("d", "a", "b", "c"),
            Arrays.asList(result.getColumn("DataToSort").get(0, 4)));
    ...
    public void testSort2() {
        final QueryTable table =
            testRefreshingTable(i(10, 20, 30), c("A", 3, 1, 2), c("B", "c", "a", "b"));
        ...
        assertTableEquals(testRefreshingTable(c("A", 1, 2, 3, 3), c("B", "A3", "b", "c", "C2")),
            sorted);
    ...
    public void testGroupedSortRefreshing() {
        final Table table = testRefreshingTable(Index.FACTORY.getFlatIndex(9),
            cG("A", "Apple", "Apple", "Apple", "Banana", "Banana", "Banana", "Canteloupe",
        ...
        show(sorted);
        assertTableEquals(
            newTable(col("A", "Canteloupe", "Canteloupe", "Canteloupe", "Banana", "Banana",
                "Banana", "Apple", "Apple", "Apple"), col("Sentinel", 6, 7, 8, 3, 4, 5, 0, 1, 2)),
            sorted);

        final QueryTable sorted2 =
            (QueryTable) table.sort(SortPair.descending("A"), SortPair.ascending("Secondary"));
        show(sorted2);
        assertTableEquals(newTable(
            col("A", "Canteloupe", "Canteloupe", "Canteloupe", "Banana", "Banana", "Banana",
                "Apple", "Apple", "Apple"),
    ...
    private void testGroupedSortHistorical(int size) {
        ...
        final Table grouped =
            testTable(Index.FACTORY.getFlatIndex(values.length), cG("Captain", values))
                .update("Sentinel=i");
        final Table nogroups =
            testTable(Index.FACTORY.getFlatIndex(values.length), c("Captain", values))
                .update("Sentinel=i");
    ...
    public void testSortBool() {
        final QueryTable table = testRefreshingTable(i(10, 20, 30, 40, 50),
            c("boolCol", false, true, null, true, false));

        final QueryTable sorted = (QueryTable) table.sort("boolCol");
        show(sorted);
        assertEquals("",
            diff(sorted, testRefreshingTable(c("boolCol", null, false, false, true, true)), 10));
        final QueryTable descending = (QueryTable) table.sort(SortPair.descending("boolCol"));
        show(descending);
        assertEquals("", diff(descending,
            testRefreshingTable(c("boolCol", true, true, false, false, null)), 10));
    }
    ...
    public void testMultiColumnRuns() {
        ...
        final QueryTable queryTable = getTable(10000, random,
            columnInfo = initColumnInfos(
                new String[] {"bool1", "bool2", "bool3", "bool4", "bool5", "Sentinel"},
        ...
        final EvalNugget en[] = new EvalNugget[] {
                ...
                new EvalNugget() {
                    public Table e() {
                        return queryTable.sortDescending("bool1", "bool2", "bool3", "bool4",
                            "bool5");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return queryTable.sortDescending("bool5", "bool4", "bool3", "bool2",
                            "bool1");
                    }
                },
        };
    ...
    private ColumnInfo[] getIncrementalColumnInfo() {
        return initColumnInfos(
            new String[] {"Sym", "intCol", "doubleCol", "floatCol", "longCol", "shortCol",
                    "byteCol", "charCol", "boolCol", "bigI", "bigD", "Keys"},
    ...
    private void testSortIncremental(final String ctxt, final int size, int seed,
        MutableInt numSteps) {
        ...
        final EvalNugget en[] = new EvalNugget[] {
                ...
                EvalNugget.from(() -> queryTable.by("Sym", "intCol").sort("Sym", "intCol")
                    .update("x=intCol+1")),
                EvalNugget.from(() -> queryTable.sort(SortPair.ascending("Sym"),
                    SortPair.descending("intCol"))),
                EvalNugget.from(() -> queryTable.sort(SortPair.ascending("Sym"),
                    SortPair.descending("intCol"), SortPair.ascending("doubleCol"))),
        ...
        for (numSteps.setValue(0); numSteps.intValue() < maxSteps; numSteps.increment()) {
            simulateShiftAwareStep(ctxt + " step == " + numSteps.getValue(), size, random,
                queryTable, columnInfo, en);
        }
    }
    ...
    public void testSortPerformance() {
        ...
        // (both increasing and decreasing sort) alternates between adding at end and adding at
        // beginning
        // Expected performance: very fast. Expect add ratio of 100%, others 0.
        ...
        // Expected performance: modest (sometimes getting lucky, sometimes being forced to move
        // elements, sometimes
        // being forced to re-spread).
        // Expected performance: medium? Not really sure what numbers to use here. Just going to say
        // 20, 20, 75.
        ...
        // Rationale:
        // In the sort-ascending case, (once things get going), the new elements are to the right of
        // the median, so the
        // code will be operating in the forward direction and always simply push [large...large+9]
        // to the right.
        //
        // In the sort-descending case, (once things get going), the new elements are to the left of
        // the median, so
        // the code will be operating in the reverse direction and always simply push
        // [large+9...large] to the left.
        ...
        // Expected performance: Should be very fast (just moving 10 elements each time). Similar
    ...
    private void performTests(String what, long addedRatioLimit, long removedRatioLimit,
        long modifiedRatioLimit,
        LongUnaryOperator generator) {
        performTestsInDirection(what, addedRatioLimit, removedRatioLimit, modifiedRatioLimit,
            generator, true);
        performTestsInDirection(what, addedRatioLimit, removedRatioLimit, modifiedRatioLimit,
            generator, false);
    }
    ...
    private void performTestsInDirection(String what, long addedRatioLimit, long removedRatioLimit,
        ...
        final String description =
            String.format("\"%s\": sort %s", what, ascending ? "ascending" : "descending");
    ...
    public void testSortIncremental() {
        ...
        final EvalNuggetInterface[] en = new EvalNuggetInterface[] {
                ...
                EvalNugget.from(() -> queryTable.by("Sym", "intCol").sort("Sym", "intCol")
                    .update("x=intCol+1")),
                new TableComparator(
                    queryTable.updateView("ok=k").sort(SortPair.ascending("Sym"),
                        SortPair.descending("intCol")),
                    "Single Sort",
                    queryTable.updateView("ok=k").sortDescending("intCol").sort("Sym"),
                    "Double Sort")
        };

        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            addToTable(queryTable, i(3, 9), c("Sym", "aa", "aa"), c("intCol", 20, 10),
                c("doubleCol", 2.1, 2.2));
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            addToTable(queryTable, i(1, 9), c("Sym", "bc", "aa"), c("intCol", 30, 11),
                c("doubleCol", 2.1, 2.2));
    ...
    public void testSortFloatIncremental() {
        ...
        for (int i = 0; i < steps; i++) {
            simulateShiftAwareStep("floatSort step == " + i, 100, random, queryTable, columnInfo,
                en);
        }
    }
    ...
    public void testGrowingMergeReinterpret() {
        ...
        final Table viewed =
            table.update("Timestamp='2019-04-11T09:30 NY' + (ii * 60L * 1000000000L)");
        final Table sorted = TableTools.merge(viewed, viewed).sortDescending("Timestamp");

        for (int ii = 2; ii < 10000; ++ii) {
            // Use large enough indices that we blow beyond merge's initially reserved 64k
            // key-space.
    ...
    private void doReinterpretTest(Table table) {
        ...
        mergeSorted.columnIterator("Sentinel")
            .forEachRemaining(sentinel -> sentinels.add((int) sentinel));
        assertEquals("sentinels", new TIntArrayList(new int[] {100, 0, 104, 4, 108, 8, 101, 1, 105,
                5, 109, 9, 102, 2, 106, 6, 103, 3, 107, 7}),
            sentinels);
        sentinels.clear();

        final Table mergeSorted2 = merged.sort("Timestamp");

        mergeSorted2.columnIterator("Sentinel")
            .forEachRemaining(sentinel -> sentinels.add((int) sentinel));
        assertEquals("sentinels", new TIntArrayList(new int[] {100, 0, 104, 4, 108, 8, 101, 1, 105,
                5, 109, 9, 102, 2, 106, 6, 103, 3, 107, 7}),
            sentinels);
        ...
        boolSorted.columnIterator("Sentinel")
            .forEachRemaining(sentinel -> sentinels.add((int) sentinel));
        assertEquals("sentinels", new TIntArrayList(new int[] {102, 105, 108, 2, 5, 8, 101, 104,
                107, 1, 4, 7, 100, 103, 106, 109, 0, 3, 6, 9}),
            sentinels);
        sentinels.clear();
        ...
        boolInverseSorted.columnIterator("Sentinel")
            .forEachRemaining(sentinel -> sentinels.add((int) sentinel));
        assertEquals("sentinels", new TIntArrayList(new int[] {107, 7, 103, 3, 106, 6, 102, 2, 109,
                9, 105, 5, 101, 1, 108, 8, 104, 4, 100, 0}),
            sentinels);
        sentinels.clear();
    }
    ...
    private void doSymbolTableTest(Table table) {
        ...
        final TIntList sentinels = new TIntArrayList();
        symbolSorted.columnIterator("Sentinel")
            .forEachRemaining(sentinel -> sentinels.add((int) sentinel));
        assertEquals("sentinels", new TIntArrayList(new int[] {0, 3, 6, 9, 1, 4, 7, 2, 5, 8}),
            sentinels);
        sentinels.clear();

        final Table tsSorted = table.sort("Timestamp");
        TableTools.showWithIndex(tsSorted);
        tsSorted.columnIterator("Sentinel")
            .forEachRemaining(sentinel -> sentinels.add((int) sentinel));
        assertEquals("sentinels", new TIntArrayList(new int[] {0, 4, 8, 1, 5, 9, 2, 6, 3, 7}),
            sentinels);
        sentinels.clear();
        ...
        boolSorted.columnIterator("Sentinel")
            .forEachRemaining(sentinel -> sentinels.add((int) sentinel));
        assertEquals("sentinels", new TIntArrayList(new int[] {2, 5, 8, 1, 4, 7, 0, 3, 6, 9}),
            sentinels);
        sentinels.clear();
    }
