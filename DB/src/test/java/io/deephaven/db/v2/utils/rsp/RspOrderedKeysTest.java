...
public class RspOrderedKeysTest extends OrderedKeysTestBase {
    ...
    public void testIteratorSimpleByKeys() {
        ...
        try (final OrderedKeys.Iterator it = rb.getOrderedKeysIterator()) {
            ...
            while (it.hasMore()) {
                ...
                if (oks.size() != 0) {
                    try (final WritableLongChunk<OrderedKeyRanges> chunk = WritableLongChunk.makeWritableChunk(65536 * 6)) {
    ...
    public void testIteratorSimpleByPosition() {
        ...
        try (final OrderedKeys.Iterator it = rb.getOrderedKeysIterator()) {
            ...
            while (it.hasMore()) {
                ...
                if (oks.size() != 0) {
                    try (final WritableLongChunk<OrderedKeyRanges> chunk = WritableLongChunk.makeWritableChunk(2 * (int) len)) {
                        oks.fillKeyRangesChunk(chunk);
                        assertEquals(0, chunk.size() % 2);
                        int i = 0;
                        while (i < chunk.size()) {
                ...
                for (int dk1 = -1; dk1 <= 1; ++dk1) {
                    for (int dk2 = -1; dk2 <= 1; ++dk2) {
                        ...
                        try (final OrderedKeys ok = rb.getOrderedKeysByKeyRange(startKey, endKey)) {
                            ...
                            try (final WritableLongChunk<OrderedKeyIndices> chunk = WritableLongChunk.makeWritableChunk(chunkSz)) {
                                while (it.hasMore()) {
                ...
                for (int dk1 = -1; dk1 <= 1; ++dk1) {
                    for (int dk2 = -1; dk2 <= 1; ++dk2) {
                        ...
                        try (final OrderedKeys ok = rb.getOrderedKeysByKeyRange(startKey, endKey)) {
                            ...
                            final OrderedKeys.Iterator it = ok.getOrderedKeysIterator();
                            final int chunkSz = 13;
                            try (final WritableLongChunk<OrderedKeyRanges> chunk = WritableLongChunk.makeWritableChunk(chunkSz)) {
                                while (it.hasMore()) {
    ...
    @Test public void testForEachLongRangeInSpanRegression0() {
        final RspBitmap rb = new RspBitmap();
        // We need a number of ranges that matches bufSz/2 in the implementation for RspArray.forEachLongRangeInSpanWithOffsetAndMaxCardinality.
