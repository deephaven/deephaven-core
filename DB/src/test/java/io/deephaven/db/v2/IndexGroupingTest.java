...
public class IndexGroupingTest extends LiveTableTestCase {
    ...
    public void testGrouping(final boolean immutableColumns, final Random random,
        ...
        if (immutableColumns) {
            ...
            columnInfo[0] =
                new TstUtils.ColumnInfo(new TstUtils.SetGenerator<>("a", "b", "c", "d", "e", "f"),
                    "Sym", TstUtils.ColumnInfo.ColAttributes.Immutable);
            // noinspection unchecked
            columnInfo[1] = new TstUtils.ColumnInfo(new TstUtils.IntGenerator(10, 100), "intCol",
                TstUtils.ColumnInfo.ColAttributes.Immutable);
        } else {
            // noinspection unchecked
            columnInfo[0] = new TstUtils.ColumnInfo(
                new TstUtils.SetGenerator<>("a", "b", "c", "d", "e", "f"), "Sym");
            // noinspection unchecked
            columnInfo[1] = new TstUtils.ColumnInfo(new TstUtils.IntGenerator(10, 100), "intCol");
        }
        // noinspection unchecked
        ...
        final EvalNugget[] en = new EvalNugget[] {
                new EvalNugget() {
                    public Table e() {
                        return LiveTableMonitor.DEFAULT.exclusiveLock()
                            .computeLocked(() -> queryTable.head(0));
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return LiveTableMonitor.DEFAULT.exclusiveLock()
                            .computeLocked(() -> queryTable.head(1));
                    }
                },
                new EvalNugget() {
                    public Table e() {
                ...
                new EvalNugget() {
                    public Table e() {
                        return LiveTableMonitor.DEFAULT.exclusiveLock()
                            .computeLocked(() -> queryTable.by("Sym", "intCol")
                                .sort("Sym", "intCol").view("doubleCol=max(doubleCol)"));
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return LiveTableMonitor.DEFAULT.exclusiveLock()
                            .computeLocked(() -> queryTable.avgBy("Sym", "doubleCol")
                                .sort("Sym", "doubleCol").view("intCol=min(intCol)"));
                    }
                },
        };
        ...
        Table by =
            LiveTableMonitor.DEFAULT.exclusiveLock().computeLocked(() -> queryTable.avgBy("Sym"));
        addGroupingValidator((DynamicTable) by, "by");
        Table avgBy =
            LiveTableMonitor.DEFAULT.exclusiveLock().computeLocked(() -> queryTable.avgBy("Sym"));
        addGroupingValidator((DynamicTable) avgBy, "avgBy");
        Table avgBy1 = LiveTableMonitor.DEFAULT.exclusiveLock()
            .computeLocked(() -> queryTable.avgBy("Sym", "intCol"));
        addGroupingValidator((DynamicTable) avgBy1, "avgBy1");

        Table merged = Require.neqNull(LiveTableMonitor.DEFAULT.exclusiveLock()
            .computeLocked(() -> TableTools.merge(queryTable)), "TableTools.merge(queryTable)");
        ...
        for (numSteps.setValue(0); numSteps.intValue() < maxSteps; numSteps.increment()) {
            simulateShiftAwareStep("step == " + numSteps.intValue(), size, random, queryTable,
                columnInfo, en);
        }
    ...
    private void addGroupingValidator(DynamicTable originalValue, String context) {
        ArrayList<ArrayList<String>> columnSets2 =
            powerSet(originalValue.getColumnSourceMap().keySet());
    ...
    public void testCombinedGrouping() throws IOException {
        ...
        // noinspection unchecked
        columnInfo[1] = new TstUtils.ColumnInfo(new TstUtils.SetGenerator<>("q", "r", "s", "t"),
            "Sym2", TstUtils.ColumnInfo.ColAttributes.Immutable,
            TstUtils.ColumnInfo.ColAttributes.Grouped);
        // noinspection unchecked
        ...
        GroupingValidator.validateGrouping(new String[] {"Sym"}, countingTable.getIndex(),
            countingTable, "sym", symGrouping);
        ...
        GroupingValidator.validateGrouping(new String[] {"intCol"}, countingTable.getIndex(),
            countingTable, "intCol", intGrouping);
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        final TupleSource intSymTupleSource =
        ...
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("getInt"));
        GroupingValidator.validateGrouping(new String[] {"intCol", "Sym"}, countingTable.getIndex(),
        ...
        Map<Object, Index> intSymSym2Grouping =
            countingTable.getIndex().getGrouping(intSymSym2TupleSource);
        assertEquals(0, ((CountingTable.MethodCounter) symColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) sym2ColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("getInt"));
        GroupingValidator.validateGrouping(new String[] {"intCol", "Sym", "Sym2"},
            countingTable.getIndex(), countingTable, "intCol+sym+sym2", intSymSym2Grouping);
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        final TupleSource intSymDoubleTupleSource = TupleSourceFactory
            .makeTupleSource(intColumnSource, symColumnSource, doubleColumnSource);
        Map<Object, Index> intSymDoubleGrouping =
            countingTable.getIndex().getGrouping(intSymDoubleTupleSource);
        assertEquals(0, ((CountingTable.MethodCounter) symColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("getInt"));
        assertEquals(countingTable.size(),
            ((CountingTable.MethodCounter) doubleColumnSource).getMethodCount("get"));
        assertEquals(0,
            ((CountingTable.MethodCounter) doubleColumnSource).getMethodCount("getDouble"));
        GroupingValidator.validateGrouping(new String[] {"intCol", "Sym", "doubleCol"},
            countingTable.getIndex(), countingTable, "intCol+sym+doubleCol", intSymDoubleGrouping);
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        final TupleSource intSymSym2DoubleTupleSource = TupleSourceFactory.makeTupleSource(
            intColumnSource, symColumnSource, sym2ColumnSource, doubleColumnSource);
        Map<Object, Index> intSymSym2DoubleGrouping =
            countingTable.getIndex().getGrouping(intSymSym2DoubleTupleSource);
        assertEquals(0, ((CountingTable.MethodCounter) symColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) sym2ColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("getInt"));
        ...
        GroupingValidator.validateGrouping(new String[] {"intCol", "Sym", "Sym2", "doubleCol"},
            countingTable.getIndex(), countingTable, "intCol+sym+sym2+doubleCol",
            intSymSym2DoubleGrouping);
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());
    }
    ...
    public void testRestrictedGrouping() throws IOException {
        ...
        columnInfo[0] = new TstUtils.ColumnInfo(
            ...
            TstUtils.ColumnInfo.ColAttributes.Immutable, TstUtils.ColumnInfo.ColAttributes.Grouped);
        // noinspection unchecked
        columnInfo[1] = new TstUtils.ColumnInfo(
            new TstUtils.SetGenerator<>("q", "r", "s", "t", "u", "v"), "Sym2",
            TstUtils.ColumnInfo.ColAttributes.Immutable, TstUtils.ColumnInfo.ColAttributes.Grouped);
        // noinspection unchecked
        columnInfo[2] = new TstUtils.ColumnInfo(new TstUtils.IntGenerator(10, 100), "intCol",
            TstUtils.ColumnInfo.ColAttributes.Immutable, TstUtils.ColumnInfo.ColAttributes.Grouped);
        ...
        final Map<Object, Index> symGrouping =
            countingTable.getIndex().getGroupingForKeySet(keySet, symColumnSource);
        assertEquals(0, ((CountingTable.MethodCounter) symColumnSource).getMethodCount("get"));

        GroupingValidator.validateRestrictedGrouping(new String[] {"Sym"}, countingTable.getIndex(),
            countingTable, "sym", symGrouping, keySet);
        ((CountingTable.MethodCounter) symColumnSource).clear();
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        keySet.clear();
        keySet.addAll(Arrays.asList(10, 20, 30, 40, 50, 60, 70, 80, 90));
        final Map<Object, Index> intGrouping =
            countingTable.getIndex().getGroupingForKeySet(keySet, intColumnSource);
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("getInt"));
        GroupingValidator.validateRestrictedGrouping(new String[] {"intCol"},
            countingTable.getIndex(), countingTable, "intCol", intGrouping, keySet);
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        keySet.clear();
        final TupleSource intSymFactory =
            TupleSourceFactory.makeTupleSource(intColumnSource, symColumnSource);
        TstUtils.selectSubIndexSet(5, countingTable.getIndex(), random)
            .forEach(row -> keySet.add(intSymFactory.createTuple(row)));
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        final Map<Object, Index> intSymGrouping = countingTable.getIndex().getGroupingForKeySet(
            keySet, TupleSourceFactory.makeTupleSource(intColumnSource, symColumnSource));
        assertEquals(0, ((CountingTable.MethodCounter) symColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("get"));
        assertEquals(0, ((CountingTable.MethodCounter) intColumnSource).getMethodCount("getInt"));
        GroupingValidator.validateRestrictedGrouping(new String[] {"intCol", "Sym"},
            countingTable.getIndex(), countingTable, "intCol+sym", intSymGrouping, keySet);
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        keySet.clear();
        final TupleSource intSymDoubleFactory = TupleSourceFactory.makeTupleSource(intColumnSource,
            symColumnSource, doubleColumnSource);
        TstUtils.selectSubIndexSet(5, countingTable.getIndex(), random)
            .forEach(row -> keySet.add(intSymDoubleFactory.createTuple(row)));
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        final Map<Object, Index> intSymDoubleGrouping =
        ...
        keySet.clear();
        final TupleSource intSymSym2DoubleFactory = TupleSourceFactory.makeTupleSource(
            intColumnSource, symColumnSource, sym2ColumnSource, doubleColumnSource);
        TstUtils.selectSubIndexSet(5, countingTable.getIndex(), random)
            .forEach(row -> keySet.add(intSymSym2DoubleFactory.createTuple(row)));
        countingTable.getColumnSources().forEach(x -> ((CountingTable.MethodCounter) x).clear());

        final Map<Object, Index> intSymSym2DoubleGrouping = countingTable.getIndex()
            .getGroupingForKeySet(keySet, TupleSourceFactory.makeTupleSource(intColumnSource,
        ...
        GroupingValidator.validateRestrictedGrouping(
            new String[] {"intCol", "Sym", "Sym2", "doubleCol"}, countingTable.getIndex(),
            countingTable, "intCol+sym+sym2+doubleCol", intSymSym2DoubleGrouping, keySet);
    }
