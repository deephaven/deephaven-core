...
public class TestSourceSink {
    /**
     * A variety of tests are possible here. As a first pass, we make a chunk of size 1000, fill
     * elements 0-249 and 500-749 with random values, and then see if they come back.
     */
    public static void runTests(ChunkType chunkType, IntFunction<WritableChunkSink> makeSink) {
        final ChunkType containerType =
            chunkType == ChunkType.Boolean ? ChunkType.Object : chunkType;
        ...
        final OrderedKeys keysA = OrderedKeys
            .wrapKeyRangesChunkAsOrderedKeys(LongChunk.chunkWrap(new long[] {0, chunkSize - 1}));
        final OrderedKeys keysB = OrderedKeys.wrapKeyRangesChunkAsOrderedKeys(
        ...
        final OrderedKeys keysAll = OrderedKeys.wrapKeyRangesChunkAsOrderedKeys(
            LongChunk.chunkWrap(new long[] {0, 4 * chunkSize - 1}));
    ...
    private static void equalsHelper(String what, final ChunkEquals ce,
        final Chunk<Values> expected,
