...
public class TestCharacterDeltaAwareColumnSource {
    ...
    private static void checkUsingChunk(DeltaAwareColumnSource<Character> dacs, Map<Long, Character> expectedCurrent,
        ...
        try (ChunkSource.GetContext context = dacs.makeGetContext(chunkSize)) {
            CharChunk<Values> chunk;

            chunk = dacs.getChunk(context, index0).asCharChunk();
            checkChunk(chunk, expectedCurrent, index0);
            chunk = dacs.getChunk(context, index1).asCharChunk();
            checkChunk(chunk, expectedCurrent, index1);

            chunk = dacs.getPrevChunk(context, index0).asCharChunk();
            checkChunk(chunk, expectedPrev, index0);
            chunk = dacs.getPrevChunk(context, index1).asCharChunk();
            checkChunk(chunk, expectedPrev, index1);
        }
    }

    private static void checkChunk(CharChunk<Values> values, Map<Long, Character> expected, Index keys) {
        int sliceOffset = 0;
