...
public class TestDownsampledWhereFilter {
    ...
    public void testDownsampledWhere() throws IOException {
        ...
        final QueryTable table = getTable(false, size, random, initColumnInfos(new String[]{"Timestamp", "doubleCol"},
                new SortedDateTimeGenerator(DBTimeUtils.convertDateTime("2015-09-11T09:30:00 NY"), DBTimeUtils.convertDateTime("2015-09-11T10:00:00 NY")),
                new DoubleGenerator(0, 100)
        ));

        Table downsampled = table.where(new DownsampledWhereFilter("Timestamp", 60_000_000_000L));
        Table standardWay = table.updateView("TimeBin=upperBin(Timestamp, 60000000000)").lastBy("TimeBin").dropColumns("TimeBin");
    ...
    public void testDownsampledWhereLowerFirst() throws IOException {
        ...
        Table downsampled = table.where(new DownsampledWhereFilter("Timestamp", 60_000_000_000L, DownsampledWhereFilter.SampleOrder.LOWERFIRST));
        Table standardWay = table.updateView("TimeBin=lowerBin(Timestamp, 60000000000)").firstBy("TimeBin").dropColumns("TimeBin");
