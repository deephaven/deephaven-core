...
public class GroupingValidator extends InstrumentedShiftAwareListenerAdapter {
    ...
    public static void validateGrouping(String[] groupingToCheck, Index index, DynamicTable source, String context, Map<Object, Index> grouping) {
        final ColumnSource[] groupColumns = getColumnSources(groupingToCheck, source);
        ...
        for (Index.Iterator it = index.iterator(); it.hasNext(); ) {
            ...
            if (keyIndex != null) {
                Assert.assertion(keyIndex.find(next) >= 0, "keyIndex.find(next) >= 0", next, "next", key, "key", keyIndex, "keyIndex", context, "context");
            }
        }
    }

    public static void validateRestrictedGrouping(String[] groupingToCheck, Index index, DynamicTable source, String context, Map<Object, Index> grouping, Set<Object> validKeys) {
        ColumnSource[] groupColumns = getColumnSources(groupingToCheck, source);
        for (Map.Entry<Object, Index> objectIndexEntry : grouping.entrySet()) {
            final Object groupKey = objectIndexEntry.getKey();
            Assert.assertion(validKeys.contains(groupKey), "validKeys.contains(objectIndexEntry.getKey())", groupKey, "groupKey", validKeys, "validKeys");
            for (Index.Iterator it = objectIndexEntry.getValue().iterator(); it.hasNext(); ) {
                long next = it.nextLong();
    ...
    static private void checkGroupKey(final ColumnSource[] groupColumns, final long next, final Object key, final String context) {
        final Object value = getValue(groupColumns, next);
        Assert.assertion(Objects.equals(value, key), "value.equals(key)", value, "value", key, "key", context, "context");
    }

    static private void checkGroupPrevKey(final ColumnSource[] groupColumns, final long next, final Object key, final String context) {
        Object value = getPrevValue(groupColumns, next);
        Assert.assertion(value == key || value.equals(key), "value.equals(key)", value, "value", key, "key", context, "context");
    }
