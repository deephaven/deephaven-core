...
public class IndexShiftDataExpanderTest {
    ...
    /**
     * These tests names have a few qualities worth defining:
     * - Major Shift: shift with no overlap in keyspace before and after (i.e. modified index from shift alone is empty)
     * - Minor Shift: shift with overlap in keyspace before and after (i.e. modified index from shift alone is non-empty)
     * - Inner Shift: shift occurs with valid elements below and above keyspace both before and after
     * - NoAdd / NoRm: Other than the shift, either the added or removed (respectively) indexes will be empty.
     * - WithGaps: There will be a gap on the leading edges of the shift. (need to carefully categorize items near gap)
     * - Modified Inner/Outer: Rows inside/outside of a shift were modified.
    ...
    public void testGappedRShift() {
        ...
        final long s = 25; final long e = 49; final long dt = 100;
    ...
    public void testGappedLShift() {
        // not all keys in shift range are real
        final long s = 125; final long e = 149; final long dt = -100;
        c.sourceIndex.insertRange(s, e);
    ...
    /**
     * 1. Add initial index state to {@code sourceIndex}.
     * 2. Setup added/removed/modified/shifted as inputs to IndexShiftDataExpander.
     * 3. Modify expected output ranges to expectAdded / expectRemoved / expectModified.
     * 4. Profit by letting @Before / @After clear context and run validate automagically.
     */
