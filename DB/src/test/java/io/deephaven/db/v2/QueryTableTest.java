 ...
 *
 * This test used to be a catch all, but at over 7,000 lines became unwieldy.  It is still somewhat of a catch-all,
 * but some specific classes of tests have been broken out.
 *
...
public class QueryTableTest extends QueryTableTestBase {
    ...
    /**
     * Test that the formula can see the internal variable that DBTimeUtils introduces here.
     * (Prior to IDS-6532 this threw an exception).
     */
    ...
    /**
     * Test that the formula behaves correctly when there are are two initDefs() without an intervening
     * request to compile the formula. Prior to the second update to IDS-6532, this threw an exception,
     * although typically only by OpenAPI code (because that code uses validateSelect() whereas other
     * code tends not to). The issue is that this sequence of operations works:
     *
     * initDef()
     ...
     * get compiled formula
     *
     * But (prior to this change) this sequence of operations would not work:
     * initDef()
     * initDef()
     * get compiled formula
     *
     * The reason the second one breaks is that (prior to this change), when using certain Time literals,
     * the second initDef() changes Formula state in such a way that a subsequent compilation would not succeed.
     * The reason this break was not observed in practice is that most usages are like the first example: there
     * is a compilation request interposed between initDefs() and, thanks to formula caching, the second
     * compilation uses the cached Formula object from the first compilation and doesn't actually invoke
     * the compiler again.
     */
    ...
    /**
     * The formula generation code used to create internal variables called "__chunk" + columnName; it also
     * created an internal variable called "__chunkPos". Prior to the change that fixed this, a formula
     * compilation error can happen if the customer names their column "Pos".
     */
    ...
    /**
     * Confirm that the system behaves correctly with select validation and the new "flatten"
     * code QueryTable#select(). Prior to the change that fixed this, "validateSelect" would
     * cause the SelectColumn to get associated with one index, but then select() would want to
     * flatten that index, so a later initDef would try to associate it with a different index,
     * and then the assertion would fail at AbstractFormulaColumn.java:86.
     * The simple fix is that validateSelect() should copy its select columns before using
     * them and then throw away the copies.
     */
    ...
    public void testIds1822() {
        ...
        assertEquals("", io.deephaven.db.tables.utils.TableTools.diff(t2, t4, 10));
        assertEquals("", io.deephaven.db.tables.utils.TableTools.diff(t3, t4, 10));
        ...
        assertEquals("", io.deephaven.db.tables.utils.TableTools.diff(u2, u4, 10));
        assertEquals("", io.deephaven.db.tables.utils.TableTools.diff(u3, u4, 10));
        ...
        assertEquals("", io.deephaven.db.tables.utils.TableTools.diff(v3, v4, 10));
    }
    ...
    public void testViewIncremental() {
        ...
        final EvalNugget[] en = new EvalNugget[]{
                ...
                EvalNugget.from(() -> queryTable.updateView("newCol=intCol / 2", "newCol2=newCol_[i-1] * newCol"))
                        .hasUnstableColumns("newCol2"),
                EvalNugget.from(() -> queryTable.updateView("repeatedCol=doubleCol - 0.5", "newCol=intCol / 2", "repeatedCol=newCol_[i-1] * repeatedCol"))
                ...
                EvalNugget.from(() -> sortedTable.updateView("newCol=intCol / 2", "newCol2=newCol_[i-1] * newCol"))
                        .hasUnstableColumns("newCol2"),
                EvalNugget.from(() -> sortedTable.updateView("repeatedCol=doubleCol - 0.5", "newCol=intCol / 2", "repeatedCol=newCol_[i-1] * repeatedCol"))
                        .hasUnstableColumns("repeatedCol"),
                ...
                EvalNugget.from(() -> queryTable.view("newCol=intCol / 2", "newCol2=newCol_[i-1] * newCol"))
                        .hasUnstableColumns("newCol2"),
                EvalNugget.from(() -> queryTable.view("repeatedCol=doubleCol - 0.5", "newCol=intCol / 2", "repeatedCol=newCol_[i-1] * repeatedCol"))
                        .hasUnstableColumns("repeatedCol"),
                ...
                EvalNugget.from(() -> sortedTable.view("newCol=intCol / 2", "newCol2=newCol_[i-1] * newCol"))
                        .hasUnstableColumns("newCol2"),
                EvalNugget.from(() -> sortedTable.view("repeatedCol=doubleCol - 0.5", "newCol=intCol / 2", "repeatedCol=newCol_[i-1] * repeatedCol"))
                        .hasUnstableColumns("repeatedCol"),
                EvalNugget.from(() -> sortedTable.view("newCol2=intCol / 2", "newCol=newCol2_[i-1] + 7"))
                        .hasUnstableColumns("newCol"),
                EvalNugget.from(() -> queryTable.updateView("newCol2=intCol / 2", "newCol=newCol2", "newCol=newCol_[i-1] + 7"))
                        .hasUnstableColumns("newCol"),
    ...
    public void testView() {
        ...
        assertEquals("", io.deephaven.db.tables.utils.TableTools.diff(table2, table1.update("z = x", "x = z + 1", "t = x - 3"), 10));
        ...
        assertEquals("", io.deephaven.db.tables.utils.TableTools.diff(table4, table3.select("z = x", "x = z + 1", "t = x - 3"), 10));
        assertEquals(added, i(7, 9));
        assertEquals(modified, i());
        assertEquals(removed, i());
    ...
    public void testView1() {
        ...
        Table table = newTable(3, Arrays.asList("String", "Int"), Arrays.asList(TableTools.objColSource("c", "e", "g"), TableTools.colSource(2, 4, 6)));
        assertEquals(2, table.view(CollectionUtil.ZERO_LENGTH_STRING_ARRAY).getColumns().length);
        assertEquals(table.getColumns()[0].getName(), table.view(CollectionUtil.ZERO_LENGTH_STRING_ARRAY).getColumns()[0].getName());
        assertEquals(table.getColumns()[1].getName(), table.view(CollectionUtil.ZERO_LENGTH_STRING_ARRAY).getColumns()[1].getName());
        ...
        assertSame(table.getColumns()[0].getClass(), table.view("Int1=Int", "String1=String").getColumns()[1].getClass());
        assertSame(table.getColumns()[1].getClass(), table.view("Int1=Int", "String1=String").getColumns()[0].getClass());
        ...
        table = io.deephaven.db.tables.utils.TableTools.emptyTable(3);
    ...
    public void testDropColumns() {
        ...
        final List<ColumnSource> colSources = Arrays.asList(TableTools.objColSource("c", "e", "g"), colSource(2, 4, 6), colSource(1.0, 2.0, 3.0));
        ...
        ColumnSource[] columnsAfterDrop = columnSourcesAfterDrop.toArray(ColumnSource.ZERO_LENGTH_COLUMN_SOURCE_ARRAY);
        ...
        assertSame(table.getColumnSource("Double"), table.dropColumns("String").dropColumns("Int").getColumnSource("Double"));
        try {
    ...
    public void testRenameColumns() {
        ...
        final Collection<? extends ColumnSource> renamedColumnSources = table.renameColumns(CollectionUtil.ZERO_LENGTH_STRING_ARRAY).getColumnSources();
        final ColumnSource[] renamedColumns = renamedColumnSources.toArray(ColumnSource.ZERO_LENGTH_COLUMN_SOURCE_ARRAY);
        assertSame(columns[0], renamedColumns[0]);
        assertSame(columns[1], renamedColumns[1]);
        assertSame(columns[2], renamedColumns[2]);
        assertSame(table.getColumnSource("String"), table.renameColumns(CollectionUtil.ZERO_LENGTH_STRING_ARRAY).getColumnSource("String"));
        assertSame(table.getColumnSource("Int"), table.renameColumns(CollectionUtil.ZERO_LENGTH_STRING_ARRAY).getColumnSource("Int"));
        assertSame(table.getColumnSource("Double"), table.renameColumns(CollectionUtil.ZERO_LENGTH_STRING_ARRAY).getColumnSource("Double"));

        assertEquals(3, table.renameColumns("NewInt=Int").getColumns().length);
        assertEquals(table.getColumnSources().toArray()[0], table.renameColumns("NewInt=Int").getColumnSources().toArray()[0]);
        assertTrue(Arrays.equals((int[]) table.getColumns()[1].getDirect(), (int[]) table.renameColumns("NewInt=Int").getColumns()[1].getDirect()));
        assertEquals(table.getColumnSources().toArray()[2], table.renameColumns("NewInt=Int").getColumnSources().toArray()[2]);
        assertEquals(table.getColumnSource("String"), table.renameColumns("NewInt=Int").getColumnSource("String"));
        assertTrue(Arrays.equals((int[]) table.getColumn("Int").getDirect(), (int[]) table.renameColumns("NewInt=Int").getColumn("NewInt").getDirect()));
        ...
        assertTrue(Arrays.equals((String[]) table.getColumns()[0].getDirect(), (String[]) table.renameColumns("NewInt=Int", "NewString=String").getColumns()[0].getDirect()));
        assertTrue(Arrays.equals((int[]) table.getColumns()[1].getDirect(), (int[]) table.renameColumns("NewInt=Int").getColumns()[1].getDirect()));
        assertEquals(table.getColumnSources().toArray()[2], table.renameColumns("NewInt=Int", "NewString=String").getColumnSources().toArray()[2]);
        assertTrue(Arrays.equals((String[]) table.getColumn("String").getDirect(), (String[]) table.renameColumns("NewInt=Int", "NewString=String").getColumn("NewString").getDirect()));
        assertTrue(Arrays.equals((int[]) table.getColumn("Int").getDirect(), (int[]) table.renameColumns("NewInt=Int").getColumn("NewInt").getDirect()));
        assertEquals(table.getColumnSource("Double"), table.renameColumns("NewInt=Int", "NewString=String").getColumnSource("Double"));
        try {
        ...
        assertTrue(Arrays.equals((String[]) table.getColumns()[0].getDirect(), (String[]) table.renameColumns("NewInt=Int", "NewString=String").getColumns()[0].getDirect()));
        assertTrue(Arrays.equals((int[]) table.getColumns()[1].getDirect(), (int[]) table.renameColumns("NewInt=Int").getColumns()[1].getDirect()));
        assertEquals(table.getColumnSources().toArray()[2], table.renameColumns("NewInt=Int").renameColumns("NewString=String").getColumnSources().toArray()[2]);
        assertTrue(Arrays.equals((String[]) table.getColumn("String").getDirect(), (String[]) table.renameColumns("NewInt=Int", "NewString=String").getColumn("NewString").getDirect()));
        assertTrue(Arrays.equals((int[]) table.getColumn("Int").getDirect(), (int[]) table.renameColumns("NewInt=Int").getColumn("NewInt").getDirect()));
        assertEquals(table.getColumnSource("Double"), table.renameColumns("NewInt=Int").renameColumns("NewString=String").getColumnSource("Double"));
        try {
    ...
    public void testStringContainsFilter() {
        ...
        final EvalNuggetInterface en[] = new EvalNuggetInterface[]{
                new TableComparator(table.where(filter.apply("S1.contains(`aab`)")), table.where(new StringContainsFilter("S1", "aab"))),
                new TableComparator(table.where(filter.apply("S2.contains(`m`)")), table.where(new StringContainsFilter("S2", "m"))),
                new TableComparator(table.where(filter.apply("!S2.contains(`ma`)")), table.where(new StringContainsFilter(MatchFilter.MatchType.Inverted, "S2", "ma"))),
                new TableComparator(table.where(filter.apply("S2.toLowerCase().contains(`ma`)")), table.where(new StringContainsFilter(MatchFilter.CaseSensitivity.IgnoreCase, MatchFilter.MatchType.Regular, "S2", "mA"))),
                new TableComparator(table.where(filter.apply("S2.contains(`mA`)")), table.where(new StringContainsFilter("S2", "mA"))),
        };
    ...
    public void testDoubleRangeFilterSimple() {
        final Table t = TableTools.newTable(doubleCol("DV", 1.0, 2.0, -3.0, Double.NaN, QueryConstants.NULL_DOUBLE, 6.0, Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY, 9.0)).update("IV=i+1");
        Table leq1 = t.where("DV <= 1.0");
        Table leq1b = t.where("DV <= 1.0 && true");

        assertTableEquals(leq1b, leq1);
        assertTableEquals(TableTools.newTable(doubleCol("DV", 1.0, -3.0, QueryConstants.NULL_DOUBLE, Double.NEGATIVE_INFINITY), intCol("IV", 1, 3, 5, 8)), leq1);
        ...
        assertTableEquals(TableTools.newTable(doubleCol("DV", 1.0, 2.0, Double.NaN, 6.0, Double.POSITIVE_INFINITY, 9.0), intCol("IV", 1, 2, 4, 6, 7, 9)), geq1);
    }

    public void testLongRangeFilterSimple() {
        final Table t = TableTools.newTable(longCol("LV", 1, 2, -3, Long.MAX_VALUE, QueryConstants.NULL_LONG, 6)).update("IV=i+1");
        Table leq1 = t.where("LV <= 1");
        Table leq1b = t.where("LV <= 1 && true");

        assertTableEquals(leq1b, leq1);
        assertTableEquals(TableTools.newTable(longCol("LV", 1, -3, QueryConstants.NULL_LONG), intCol("IV", 1, 3, 5)), leq1);
    ...
    public void testComparableRangeFilterSimple() {
        final Table t = TableTools.newTable(longCol("LV", 1, 2, -3, Long.MAX_VALUE, QueryConstants.NULL_LONG, 6)).update("IV=i+1", "LV=LV==null ? null : java.math.BigInteger.valueOf(LV)");
        Table leq1 = t.where("LV <= 1");
    ...
    public void testDoubleRangeFilter() {
        ...
        final ColumnInfo[] columnInfo;
        final QueryTable table = getTable(size, random, columnInfo = initColumnInfos(new String[]{"D1", "D2", "F1", "F2"},
                new DoubleGenerator(),
        ...
        final EvalNuggetInterface en[] = new EvalNuggetInterface[]{
                new TableComparator(table.where(filter.apply("D1 > 500 && D1 <= 501")), table.where(new DoubleRangeFilter("D1", 500, 501, false, true))),
                new TableComparator(table.where(filter.apply("D1 > 500.7 && D1 <= 500.8")), table.where(new DoubleRangeFilter("D1", 500.7, 500.8, false, true))),
                new TableComparator(table.where(filter.apply("D2 >= 250.02 && D2 < 250.03")), table.where(DoubleRangeFilter.makeRange("D2", "250.02"))),
                new TableComparator(table.where(filter.apply("F1 > 500 && F1 <= 501")), table.where(new FloatRangeFilter("F1", 500, 501, false, true))),
                new TableComparator(table.where(filter.apply("F1 > 500.7 && F1 <= 500.8")), table.where(new FloatRangeFilter("F1", 500.7f, 500.8f, false, true))),
                new TableComparator(table.where(filter.apply("F2 >= 250.02 && F2 < 250.03")), table.where(FloatRangeFilter.makeRange("F2", "250.02"))),

                new TableComparator(table.where(filter.apply("F1 <= -250.02 && F1 > -250.03")), table.where(FloatRangeFilter.makeRange("F1", "-250.02"))),
                new TableComparator(table.where(filter.apply("F1 <= -37.0002 && F1 > -37.0003")), table.where(FloatRangeFilter.makeRange("F1", "-37.0002"))),
                new TableComparator(table.where(filter.apply("D1 <= -250.02 && D1 > -250.03")), table.where(DoubleRangeFilter.makeRange("D1", "-250.02"))),
                new TableComparator(table.where(filter.apply("D1 <= -37.0002 && D1 > -37.0003")), table.where(DoubleRangeFilter.makeRange("D1", "-37.0002")))
        };
    ...
    public void testDateTimeRangeFilter() {
        ...
        final ColumnInfo[] columnInfo;
        final QueryTable table = getTable(size, random, columnInfo = initColumnInfos(new String[]{"Timestamp", "Ts2", "Sentinel"},
        ...
        final EvalNuggetInterface en[] = new EvalNuggetInterface[]{
                new TableComparator(table.where(filter.apply("Timestamp >= '" + lower.toString() + "' && Timestamp <= '" + upper.toString() + "'")), "Condition", table.where(new DateTimeRangeFilter("Timestamp", lower, upper, true, true)), "Range"),
                new TableComparator(table.where(filter.apply("Timestamp >= '" + lower.toString() + "' && Timestamp < '" + upper.toString() + "'")), table.where(new DateTimeRangeFilter("Timestamp", lower, upper, true, false))),
                new TableComparator(table.where(filter.apply("Timestamp > '" + lower.toString() + "' && Timestamp <= '" + upper.toString() + "'")), table.where(new DateTimeRangeFilter("Timestamp", lower, upper, true, true))),
                new TableComparator(table.where(filter.apply("Timestamp > '" + lower.toString() + "' && Timestamp < '" + upper.toString() + "'")), table.where(new DateTimeRangeFilter("Timestamp", lower, upper, false, false))),

                new TableComparator(table.where(filter.apply("Ts2 >= '" + lower.toString() + "' && Ts2 <= '" + upper.toString() + "'")), "Condition", table.where(new DateTimeRangeFilter("Ts2", lower, upper, true, true)), "Range"),
                new TableComparator(table.where(filter.apply("Ts2 >= '" + lower.toString() + "' && Ts2 < '" + upper.toString() + "'")), table.where(new DateTimeRangeFilter("Ts2", lower, upper, true, false))),
                new TableComparator(table.where(filter.apply("Ts2 > '" + lower.toString() + "' && Ts2 <= '" + upper.toString() + "'")), table.where(new DateTimeRangeFilter("Ts2", lower, upper, true, true))),
                new TableComparator(table.where(filter.apply("Ts2 > '" + lower.toString() + "' && Ts2 < '" + upper.toString() + "'")), table.where(new DateTimeRangeFilter("Ts2", lower, upper, false, false)))
        };
    ...
    public void testDateTimeRangeFilterNulls() {
        ...
        final ColumnInfo[] columnInfo;
        final QueryTable table = getTable(size, random, columnInfo = initColumnInfos(new String[]{"Timestamp", "Sentinel"},
    ...
    public void testReverse() {
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            final ColumnHolder[] columnAdditions = new ColumnHolder[]{c("Ticker", "SPY", "VXX"), c("Timestamp", 60L, 70L)};
        ...
        final ColumnSource<Long> longIdentityColumnSource = new AbstractColumnSource.DefaultedImmutable<Long>(long.class) {
            @Override
        ...
        final QueryTable bigTable = new QueryTable(i(0, (long) Integer.MAX_VALUE, (long) Integer.MAX_VALUE * 2L), Collections.singletonMap("LICS", longIdentityColumnSource));
    ...
    public void testReverseIncremental() throws ParseException {
        ...
        final ColumnInfo[] columnInfo;
        final QueryTable table = getTable(size, random, columnInfo = initColumnInfos(new String[]{"Date", "C1", "C2", "KEY"},
    ...
    public void testSnapshotHistorical() {
        ...
        assertEquals("",diff(left1.snapshotHistory(right), testRefreshingTable(c("T",1,1,1),c("A", 3, 1, 2), c("B", "c", "a", "b")),10));

        final QueryTable left2 = testRefreshingTable(c("T",1,2));
        final Table snapshot = left2.snapshotHistory(right);
        show(snapshot);
        assertEquals("", diff(snapshot, testRefreshingTable(c("T", 1, 1, 1, 2, 2, 2), c("A", 3, 1, 2, 3, 1, 2), c("B", "c", "a", "b", "c", "a", "b")), 10));
        ...
        show(snapshot, 50);
        assertEquals("", diff(snapshot, testRefreshingTable(c("T", 1, 1, 1, 2, 2, 2, 5, 5, 5, 5, 5), c("A", 3, 1, 2, 3, 1, 2, 3, 30, 1, 2, 50), c("B", "c", "a", "b", "c", "a", "b", "c", "aa", "a", "b", "bc")), 10));
    ...
    public void testWhereInScope() {
        ...
        LiveTableMonitor.DEFAULT.exclusiveLock().doLocked(setScope::close);
        ...
        LiveTableMonitor.DEFAULT.exclusiveLock().doLocked(singletonManager::release);
    ...
    public void testSnapshotIncremental() {
        ...
        assertEquals("",diff(snapshot, testRefreshingTable(c("A", 11,50, 34),c("B", "A","bc", "Q"), c("T", 8, 5, 8)),10));
        assertEquals(listener.getCount(), 1);
    ...
    public void testSnapshotIncrementalPrev() {
        ...
        show(snapshot,50);
        final QueryTable secondResult = testRefreshingTable(c("A", 3, 30, 1, 2, 50), c("B", "c", "aa", "a", "b", "bc"), c("T", 2, 5, 2, 2, 5));
        ...
        final QueryTable fourthResult = testRefreshingTable(c("A", 12, 50, 34), c("B", "R", "bc", "Q"), c("T", 9, 5, 8));
    ...
    public void testSnapshotIncrementalRandom() {
        ...
        final QueryTable rightTable = getTable(stampSize, random, rightInfo = initColumnInfos(new String[]{"Sym", "intCol", "doubleCol"},
        ...
        try {
            for (int step = 0; step < 200; step++) {
                ...
                LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
                    if (printTableUpdates) {
                        System.out.println("Step = " + fstep + ", modStamp=" + modStamp + ", modRight=" + modRight + ", modifyRightFirst=" + modifyRightFirst);
                    }
                    ...
                    if (modStamp) {
                        ...
                        final Index stampsToAdd = Index.FACTORY.getIndexByRange(lastStamp + 1, lastStamp + numAdditions);
                        ...
                        stampTable.notifyListeners(stampsToAdd, Index.FACTORY.getEmptyIndex(), Index.FACTORY.getEmptyIndex());
                    }
                ...
                if (modStamp) {
                    ...
                    if (coalescingListener.getCount() > 0) {
                        ...
                        final int lastStamp = stampTable.getColumnSource("Stamp").getInt(stampTable.getIndex().lastKey());
                        @SuppressWarnings("unchecked") final ColumnSource<Integer> stamps = snapshot.getColumnSource("Stamp");
    ...
    static void testLegacyFlattenModifications(io.deephaven.base.Function.Unary<QueryTable, QueryTable>function) {
        ...
        Assert.assertEquals("simpleListener.update.shifted.getBeginRange(0) = 3", 3, simpleListener.update.shifted.getBeginRange(0));
        Assert.assertEquals("simpleListener.update.shifted.getEndRange(0) = 3", 3, simpleListener.update.shifted.getEndRange(0));
        Assert.assertEquals("simpleListener.update.shifted.getShiftDelta(0) = 1", 1, simpleListener.update.shifted.getShiftDelta(0));
        ...
        Assert.assertEquals("simpleListener.update.shifted.size() = 1", 1, simpleListener.update.shifted.size());
        Assert.assertEquals("simpleListener.update.shifted.getBeginRange(0) = 3", 3, simpleListener.update.shifted.getBeginRange(0));
        Assert.assertEquals("simpleListener.update.shifted.getEndRange(0) = 4", 4, simpleListener.update.shifted.getEndRange(0));
        Assert.assertEquals("simpleListener.update.shifted.getShiftDelta(0) = -1", -1, simpleListener.update.shifted.getShiftDelta(0));
...
//        Assert.assertEquals("simpleListener.update.shifted.getShiftDelta(0) = -1", -1, simpleListener.update.shifted.getShiftDelta(0));
    ...
    public void testDbDateTimeColumns() {
        final QueryTable queryTable = TstUtils.testRefreshingTable(
                c("Sym", "aa", "bc", "aa", "aa"),
                c("Timestamp", DBTimeUtils.currentTime(), DBTimeUtils.currentTime(), DBTimeUtils.currentTime(), DBTimeUtils.currentTime()));
    ...
    public void testUngroupingAgnostic() {
        Table table = testRefreshingTable(c("X", 1, 2, 3), c("Y", new String[]{"a", "b", "c"}, CollectionUtil.ZERO_LENGTH_STRING_ARRAY, new String[]{"d", "e"}),
        ...
        table = testRefreshingTable(c("X", 1, 2, 3), c("Y", new String[]{"a", "b", "c"}, new String[]{"d", "e"}, CollectionUtil.ZERO_LENGTH_STRING_ARRAY),
                c("Z", new int[]{4, 5, 6}, new int[0], new int[]{7, 8}));
        try {
            table.ungroup();
        } catch (Exception e) {
            assertEquals("Assertion failed: asserted sizes[i] == Array.getLength(arrayColumn.get(i)), instead referenceColumn == \"Y\", name == \"Z\", row == 1.",e.getMessage());
        }
        ...
        assertEquals(Arrays.asList("a", "a", "a", "b", "b", "b", "c", "c", "c"), Arrays.asList(t1.getColumn("Y").get(0, 9)));
    ...
    public void testUngroupableColumnSources() {
        final Table table = testRefreshingTable(c("X", 1, 1, 2, 2, 3, 3, 4, 4), c("Int", 1, 2, 3, 4, 5, 6, 7, null), c("Double", 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, null, 0.45), c("String", "a", "b", "c", "d", "e", "f", "g", null));
        ...
        int [] intPrevDirect = (int[])IndexedDataColumn.makePreviousColumn(t2.getIndex(), t2.getColumnSource("Int")).getDirect();
        ...
        intPrevDirect = (int[])IndexedDataColumn.makePreviousColumn(t2.getIndex(), t2.getColumnSource("Int")).getDirect();
        if (!Arrays.equals(expected, intPrevDirect)) {
            System.out.println("Expected: " + Arrays.toString(expected));
            System.out.println("Prev: " + Arrays.toString(intPrevDirect));
            fail("Expected does not match previous value!");
    ...
    public void testUngroupOverflow() {
        try (final ErrorExpectation errorExpectation = new ErrorExpectation()) {
            final QueryTable table = testRefreshingTable(i(5, 7), c("X", 1, 2), c("Y", new String[]{"a", "b", "c"}, new String[]{"d", "e"}));
    ...
    public void testUngroupWithRebase() {
        ...
        try {
            ...
            assertEquals(Arrays.asList("a", "b", "c", "d", "e"), Arrays.asList((String[])t1.getColumn("Y").getDirect()));
            ...
            assertEquals(Arrays.asList("X", "Y"), t1.getDefinition().getColumnNames());
            assertEquals(Arrays.asList(1, 1, 1, 2, 2, 3, 3, 3, 3, 3, 3), Ints.asList((int[])(t1.getColumn("X").getDirect())));
            assertEquals(Arrays.asList("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k"), Arrays.asList((String[])t1.getColumn("Y").getDirect()));

            assertEquals(Arrays.asList(1, 1, 1, 2, 2), Ints.asList((int[])IndexedDataColumn.makePreviousColumn(t1.getIndex(), t1.getColumnSource("X")).getDirect()));
            assertEquals(Arrays.asList("a", "b", "c", "d", "e"), Arrays.asList((String[])IndexedDataColumn.makePreviousColumn(t1.getIndex(), t1.getColumnSource("Y")).getDirect()));
            ...
            assertEquals(Arrays.asList(1, 1, 1, 2, 2, 3, 3, 3, 3, 3, 3), Ints.asList((int[])t1.getColumn("X").getDirect()));
            assertEquals(Arrays.asList("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k"), Arrays.asList((String[])t1.getColumn("Y").getDirect()));
            assertEquals(Arrays.asList(1, 1, 1, 2, 2, 3, 3, 3, 3, 3, 3), Ints.asList((int[])IndexedDataColumn.makePreviousColumn(t1.getIndex(), t1.getColumnSource("X")).getDirect()));
            assertEquals(Arrays.asList("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k"), Arrays.asList((String[])IndexedDataColumn.makePreviousColumn(t1.getIndex(), t1.getColumnSource("Y")).getDirect()));
        } finally {
    ...
    private void testUngroupIncremental(int tableSize, boolean nullFill) throws ParseException {
        ...
        final ColumnInfo[] columnInfo;
        final QueryTable table = getTable(tableSize, random, columnInfo = initColumnInfos(new String[]{"Date", "C1", "C2", "C3"},
                new DateGenerator(format.parse("2011-02-02"), format.parse("2011-02-03")),
                new SetGenerator<>("a", "b"),
                new SetGenerator<>(10, 20, 30),
                new SetGenerator<>(CollectionUtil.ZERO_LENGTH_STRING_ARRAY, new String[]{"a", "b"}, new String[]{"a", "b", "c"})));
        ...
        final EvalNuggetInterface[] en = new EvalNuggetInterface[]{
                ...
                EvalNugget.from(() -> table.by().update("Date=Date.toArray()","C1=C1.toArray()","C2=C2.toArray()").ungroup(nullFill)),
                EvalNugget.from(() -> table.by("C1").update("Date=Date.toArray()","C2=C2.toArray()").sort("C1").ungroup(nullFill)),
                EvalNugget.from(() -> table.by().update("Date=Date.toArray()","C1=C1.toArray()","C2=C2.toArray()").ungroup(nullFill, "C1")),
                EvalNugget.from(() -> table.by("C1").update("Date=Date.toArray()","C2=C2.toArray()").sort("C1").ungroup(nullFill, "C2")),
                EvalNugget.from(() -> table.by("C1").update("Date=Date.toArray()","C2=C2.toArray()").sort("C1").ungroup(nullFill, "C2").ungroup(nullFill, "Date")),
                EvalNugget.from(() -> table.by("C1").update("Date=Date.toArray()","C2=C2.toArray()").sort("C1").ungroup(nullFill, "C2").ungroup(nullFill)),
                EvalNugget.from(() -> table.by("C1","C2").update("Date=Date.toArray()").sort("C1","C2").ungroup(nullFill)),
                EvalNugget.from(() -> table.by("C1").update("Date=Date.toArray()","C2=C2.toArray()").sort("C1").ungroup(nullFill)),
    ...
    private void testUngroupMismatch(int size, boolean nullFill) {
        ...
        for (int q = 0; q < 10;q++) {
            final ColumnInfo[] columnInfo;
            final QueryTable table = getTable(size, random, columnInfo = initColumnInfos(new String[]{"Sym", "intCol",},
...
//                show(mismatch, 100);
    ...
    public void testUngroupJoined_IDS6311() throws IOException, ClassNotFoundException {
        final QueryTable left = TstUtils.testRefreshingTable(c("Letter", 'a', 'b', 'c', 'd'), intCol("Value", 0, 1, 2, 3));
        ...
        QueryTable expected = TstUtils.testRefreshingTable(c("Letter", 'a', 'b', 'c', 'd'),
                c("Value", (DbIntArray)new DbIntArrayDirect(0), (DbIntArray)new DbIntArrayDirect(1), (DbIntArray)new DbIntArrayDirect(2), (DbIntArray)new DbIntArrayDirect(3)),
        ...
        expected = TstUtils.testRefreshingTable(c("Letter", 'a', 'b', 'c', 'd'),
                c("Value", 0, 1, 2, 3),
                byteCol("BValue", io.deephaven.util.QueryConstants.NULL_BYTE , (byte)1, io.deephaven.util.QueryConstants.NULL_BYTE, io.deephaven.util.QueryConstants.NULL_BYTE),
                shortCol("SValue", io.deephaven.util.QueryConstants.NULL_SHORT, (short)1, io.deephaven.util.QueryConstants.NULL_SHORT, io.deephaven.util.QueryConstants.NULL_SHORT),
                intCol("EulavI", io.deephaven.util.QueryConstants.NULL_INT, 1, io.deephaven.util.QueryConstants.NULL_INT, io.deephaven.util.QueryConstants.NULL_INT),
                longCol("LValue", io.deephaven.util.QueryConstants.NULL_LONG, (long)1, io.deephaven.util.QueryConstants.NULL_LONG, io.deephaven.util.QueryConstants.NULL_LONG),
                floatCol("FValue", io.deephaven.util.QueryConstants.NULL_FLOAT, 1.1f, io.deephaven.util.QueryConstants.NULL_FLOAT, io.deephaven.util.QueryConstants.NULL_FLOAT),
                doubleCol("DValue", io.deephaven.util.QueryConstants.NULL_DOUBLE, 1.1d, io.deephaven.util.QueryConstants.NULL_DOUBLE, io.deephaven.util.QueryConstants.NULL_DOUBLE),
                charCol("CCol", io.deephaven.util.QueryConstants.NULL_CHAR, 'b', io.deephaven.util.QueryConstants.NULL_CHAR, io.deephaven.util.QueryConstants.NULL_CHAR),
        ...
        assertEquals(io.deephaven.util.QueryConstants.NULL_SHORT, ungrouped.getColumnSource("SValue").getShort(firstKey));
        ...
        assertEquals(io.deephaven.util.QueryConstants.NULL_FLOAT, ungrouped.getColumnSource("FValue").getFloat(firstKey));
        assertEquals(1.1f, ungrouped.getColumnSource("FValue").getFloat(secondKey));

        assertEquals(io.deephaven.util.QueryConstants.NULL_DOUBLE, ungrouped.getColumnSource("DValue").getDouble(firstKey));
        ...
        // repeat with prev
        assertEquals(io.deephaven.util.QueryConstants.NULL_BYTE, ungrouped.getColumnSource("BValue").getPrevByte(firstKey));
        assertEquals((byte)1, ungrouped.getColumnSource("BValue").getPrevByte(secondKey));

        assertEquals(io.deephaven.util.QueryConstants.NULL_SHORT, ungrouped.getColumnSource("SValue").getPrevShort(firstKey));
        assertEquals((short)1, ungrouped.getColumnSource("SValue").getPrevShort(secondKey));

        assertEquals(io.deephaven.util.QueryConstants.NULL_INT, ungrouped.getColumnSource("EulavI").getPrevInt(firstKey));
        assertEquals(1, ungrouped.getColumnSource("EulavI").getPrevInt(secondKey));

        assertEquals(io.deephaven.util.QueryConstants.NULL_LONG, ungrouped.getColumnSource("LValue").getPrevLong(firstKey));
        assertEquals(1, ungrouped.getColumnSource("LValue").getPrevLong(secondKey));

        assertEquals(io.deephaven.util.QueryConstants.NULL_FLOAT, ungrouped.getColumnSource("FValue").getPrevFloat(firstKey));
        assertEquals(1.1f, ungrouped.getColumnSource("FValue").getPrevFloat(secondKey));

        assertEquals(io.deephaven.util.QueryConstants.NULL_DOUBLE, ungrouped.getColumnSource("DValue").getPrevDouble(firstKey));
        ...
        final Table snappy = InitialSnapshotTable.setupInitialSnapshotTable(ungrouped, ConstructSnapshot.constructInitialSnapshot(this, (QueryTable)ungrouped));
    ...
    private void testNoMemoize(Table source, io.deephaven.base.Function.Unary<Table, Table> op1, io.deephaven.base.Function.Unary<Table, Table> op2) {
        final Table result = op1.call(source);
        final Table result2 = op2.call(source);
        Assert.assertNotSame(result, result2);
    }

    private void testMemoize(Table source, io.deephaven.base.Function.Unary<Table, Table> op1, io.deephaven.base.Function.Unary<Table, Table> op2) {
        final Table result = op1.call(source);
        final Table result2 = op2.call(source);
        Assert.assertSame(result, result2);
    }
    ...
    public void testMemoize() {
        ...
        try {
            ...
            testMemoize(source, t -> t.whereOneOf(Collections.singletonList(SelectFilterFactory.getExpression("Sym in `aa`, `bb`")), Collections.singletonList(SelectFilterFactory.getExpression("intCol=7"))));
            testMemoize(source, t -> t.where(DisjunctiveFilter.makeDisjunctiveFilter(SelectFilterFactory.getExpressions("Sym in `aa`, `bb`","intCol=7"))));
            testMemoize(source, t -> t.where(ConjunctiveFilter.makeConjunctiveFilter(SelectFilterFactory.getExpressions("Sym in `aa`, `bb`","intCol=7"))));
            testNoMemoize(source, t -> t.where(ConjunctiveFilter.makeConjunctiveFilter(SelectFilterFactory.getExpressions("Sym in `aa`, `bb`","intCol=7"))), t-> t.where(DisjunctiveFilter.makeDisjunctiveFilter(SelectFilterFactory.getExpressions("Sym in `aa`, `bb`","intCol=7"))));
            ...
            testMemoize(source, t -> t.by(AggCombo(AggSum("intCol"), AggAbsSum("absInt=intCol"), AggMax("doubleCol"), AggFirst("Sym"), AggCountDistinct("UniqueCountSym=Sym"), AggDistinct("UniqueSym=Sym"))));
            ...
            testNoMemoize(source, t -> t.by(AggCombo(AggCountDistinct("UniqueCountSym=Sym"))), t -> t.by(AggCombo(AggCountDistinct(true, "UniqueCountSym=Sym"))));
            ...
            testNoMemoize(source, t -> t.by(AggCombo(AggCountDistinct("UniqueCountSym=Sym"))), t -> t.by(AggCombo(AggDistinct( "UniqueCountSym=Sym"))));
            testNoMemoize(source, t -> t.by(AggCombo(AggDistinct("UniqueSym=Sym"))), t -> t.by(AggCombo(AggDistinct(true, "UniqueSym=Sym"))));
            ...
            testNoMemoize(source, t -> t.where("Sym in memoizeTestVariable"), t -> { value.add("cc"); return t.where("Sym in memoizeTestVariable"); });
    ...
    public void testWhereInGrouped() throws IOException {
        diskBackedTestHarness(t -> {
            ...
            final Table result = t.whereIn(Table.GroupStrategy.CREATE_GROUPS, t.where("Truthiness=true"), "Symbol", "Timestamp");
    ...
    public void testIds7153() {
        final QueryTable lTable = testRefreshingTable(Index.FACTORY.getIndexByValues(10, 12, 14, 16), c("X", "a", "b", "c", "d"));
    ...
    public void testNoCoalesceOnNotification() {
        ...
        // coalesced and for the new result table to receive and propagate that update. IDS-7153 made it explicitly illegal
        // to publish an update on a table's very first cycle if it was initiated under the LTM lock. It's also not great
        // to coalesce a table and immediately make it garbage.

        // This regression check verifies that we do not see a lastNotificationStep != LogicalClock.DEFAULT.currentStep()
