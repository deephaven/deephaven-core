...
public class TestDoubleDeltaAwareColumnSource {
    ...
    private static void checkUsingChunk(DeltaAwareColumnSource<Double> dacs, Map<Long, Double> expectedCurrent,
        ...
        try (ChunkSource.GetContext context = dacs.makeGetContext(chunkSize)) {
            DoubleChunk<Values> chunk;

            chunk = dacs.getChunk(context, index0).asDoubleChunk();
            checkChunk(chunk, expectedCurrent, index0);
            chunk = dacs.getChunk(context, index1).asDoubleChunk();
            checkChunk(chunk, expectedCurrent, index1);

            chunk = dacs.getPrevChunk(context, index0).asDoubleChunk();
            checkChunk(chunk, expectedPrev, index0);
            chunk = dacs.getPrevChunk(context, index1).asDoubleChunk();
            checkChunk(chunk, expectedPrev, index1);
        }
    }

    private static void checkChunk(DoubleChunk<Values> values, Map<Long, Double> expected, Index keys) {
        int sliceOffset = 0;
