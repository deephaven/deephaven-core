...
public class TickSuppressorTest extends QueryTableTestBase {
    public void testModifyToAddRemoves() {
        ...
        final EvalNugget[] en = new EvalNugget[] {
                EvalNugget
                    .from(() -> TickSuppressor.convertModificationsToAddsAndRemoves(queryTable)),
                EvalNugget
                    .from(() -> TickSuppressor.convertModificationsToAddsAndRemoves(sortedTable)),
                EvalNugget.from(() -> TickSuppressor.convertModificationsToAddsAndRemoves(
                    TableTools.merge(queryTable, sortedTable))),
                EvalNugget.from(() -> TickSuppressor.convertModificationsToAddsAndRemoves(
                    queryTable.naturalJoin(queryTable.lastBy("Sym"), "Sym",
                        "intCol2=intCol,doubleCol2=doubleCol")))
        };
    ...
    private void testRemoveSpuriousModificationsIterative(int seed, int size, int maxSteps) {
        ...
        final QueryTable queryTable = getTable(size, random,
            columnInfo =
                initColumnInfos(new String[] {"Sym", "intCol", "doubleCol", "boolCol", "boolCol2"},
        ...
        final EvalNugget[] en = new EvalNugget[] {
                ...
                EvalNugget.from(
                    () -> TickSuppressor.removeSpuriousModifications(queryTable.view("boolCol"))),
                EvalNugget.from(
                    () -> TickSuppressor.removeSpuriousModifications(queryTable.view("boolCol2"))),
                EvalNugget.from(() -> TickSuppressor
                    .removeSpuriousModifications(queryTable.view("boolCol", "boolCol2"))),
                EvalNugget.from(() -> TickSuppressor.removeSpuriousModifications(sortedTable)),
                EvalNugget.from(
                    () -> TickSuppressor.removeSpuriousModifications(sortedTable.view("boolCol"))),
                EvalNugget.from(
                    () -> TickSuppressor.removeSpuriousModifications(sortedTable.view("boolCol2"))),
                EvalNugget.from(() -> TickSuppressor
                    .removeSpuriousModifications(sortedTable.view("boolCol", "boolCol2"))),
                EvalNugget.from(() -> TickSuppressor
                    .removeSpuriousModifications(TableTools.merge(queryTable, sortedTable))),
                EvalNugget
                    .from(() -> TickSuppressor.removeSpuriousModifications(queryTable.naturalJoin(
                        queryTable.lastBy("Sym"), "Sym", "intCol2=intCol,doubleCol2=doubleCol"))),
                EvalNugget
                    .from(() -> queryTable.naturalJoin(
                        TickSuppressor.removeSpuriousModifications(
                            queryTable.view("Sym", "boolCol").lastBy("Sym")),
                        "Sym", "jbc=boolCol")),
                EvalNugget
                    .from(() -> queryTable.naturalJoin(
                        TickSuppressor.removeSpuriousModifications(
                            queryTable.view("Sym", "boolCol2").lastBy("Sym")),
                        "Sym", "jbc2=boolCol2")),
    ...
    public void testRemoveSpuriousModifications() {
        ...
        final DynamicTable suppressed =
            (DynamicTable) TickSuppressor.removeSpuriousModifications(input);
        ...
        assertFalse(listener.update.modifiedColumnSet
            .containsAny(suppressed.newModifiedColumnSet("SentinelA")));
        assertTrue(listener.update.modifiedColumnSet
            .containsAny(suppressed.newModifiedColumnSet("SentinelB")));
