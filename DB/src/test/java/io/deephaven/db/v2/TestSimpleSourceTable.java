...
public class TestSimpleSourceTable extends LiveTableTestCase {
        ...
        checking(new Expectations() {
            {
                ...
                allowing(locationProvider)
                    .getTableLocation(with(StandaloneTableLocationKey.getInstance()));
    ...
    private static ColumnDefinition<?>[] getIncludedColumnDefs(final int... indices) {
        return IntStream.of(indices).mapToObj(ci -> TABLE_DEFINITION.getColumns()[ci])
            .toArray(ColumnDefinition[]::new);
    }

    private static String[] getIncludedColumnNames(final int... indices) {
        return IntStream.of(indices).mapToObj(ci -> TABLE_DEFINITION.getColumns()[ci].getName())
            .toArray(String[]::new);
    }

    private static String[] getExcludedColumnNames(final TableDefinition currentDef,
        final int... indices) {
        final Set<String> includedNames =
            IntStream.of(indices).mapToObj(ci -> TABLE_DEFINITION.getColumns()[ci].getName())
                .collect(Collectors.toSet());
        return currentDef.getColumnStream().map(ColumnDefinition::getName)
            .filter(n -> !includedNames.contains(n)).toArray(String[]::new);
    }
    ...
    private void doTestRedefinition() {
        ...
        final Table dropColumnsResult2 = dropColumnsResult1.dropColumns(
            getExcludedColumnNames(dropColumnsResult1.getDefinition(), includedColumnIndices2));
        assertIsSatisfied();
        ...
        final Table viewResult1 =
            dropColumnsResult2.view(getIncludedColumnNames(includedColumnIndices3));
        assertIsSatisfied();
