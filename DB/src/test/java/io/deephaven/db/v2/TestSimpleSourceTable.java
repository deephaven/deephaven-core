...
public class TestSimpleSourceTable extends LiveTableTestCase {
    ...
    public void setUp() throws Exception {
        ...
        checking(new Expectations() {{
            oneOf(componentFactory).createColumnSourceManager(with(false), ColumnToCodecMappings.EMPTY, with(equal(TABLE_DEFINITION.getColumns())));
    ...
    private void doTestRedefinition() {
        ...
        checking(new Expectations() {{
            oneOf(componentFactory).createColumnSourceManager(with(false), ColumnToCodecMappings.EMPTY, with(equal(getIncludedColumnDefs(includedColumnIndices1))));
            will(returnValue(columnSourceManager));
        }});
        ...
        checking(new Expectations() {{
            oneOf(componentFactory).createColumnSourceManager(with(false), ColumnToCodecMappings.EMPTY, with(equal(getIncludedColumnDefs(includedColumnIndices2))));
            will(returnValue(columnSourceManager));
        }});
        ...
        checking(new Expectations() {{
            oneOf(componentFactory).createColumnSourceManager(with(false), ColumnToCodecMappings.EMPTY, with(equal(getIncludedColumnDefs(includedColumnIndices3))));
            will(returnValue(columnSourceManager));
        }});
