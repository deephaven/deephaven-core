...
public class ChunkUtilsTest {
    ...
    public void testChunkMaxSizeDoesntOverflowLongAccumulator() {
        // Note we accumulate using longs and need to verify adding the next value to the
        // accumulator will not overflow.
    ...
    private void tryMaxChunkSizeTest(final long limit) {
        ...
        final LongChunk<OrderedKeyRanges> newChunk =
    ...
    public void testConvertToRangesSingleton() {
        validateChunk(ChunkUtils.convertToOrderedKeyRanges(createChunk(
            42)),
            42, 42);
    }
    ...
    public void testConvertToIndicesSingleton() {
        validateChunk(ChunkUtils.convertToOrderedKeyIndices(createChunk(
            1022, 1022)),
            1022);
    }
    ...
    public void testConvertRoundTripEveryOther() {
        ...
        validateChunk(createChunk(indices),
            ChunkUtils.convertToOrderedKeyIndices(createChunk(ranges)));
    }
        ...
        for (final int indexCount : new int[] {100, 1_000_000}) {
            for (final int avgElementsPerRange : new int[] {1, 4, 64, 1_000_000}) {
                for (final int sparsityFactor : new int[] {1, 10, 1_000}) {
                    ...
                    validateChunk(chunks.second,
                        ChunkUtils.convertToOrderedKeyIndices(chunks.first));
                    validateChunk(chunks.first,
                        ChunkUtils.convertToOrderedKeyRanges(chunks.second));
                }
            }
        }
    }
    ...
    private <ATTR extends Any> void validateChunk(final LongChunk<ATTR> chunk,
        final long... values) {
    ...
    private <ATTR extends Any> void validateChunk(final LongChunk<ATTR> expected,
    ...
    private Pair<LongChunk<OrderedKeyRanges>, LongChunk<OrderedKeyIndices>> generateChunks(
        ...
        final int rangeCount =
            Math.max(1, (indexCount + avgElementsPerRange / 2) / avgElementsPerRange);
        ...
        for (int i = 0; i < rangeCount - 1; i++) {
            ...
            indexRanges[2 * i] =
                lastPos + 2 + random.nextInt(2 * avgElementsPerRange - 1) * sparsityFactor;
