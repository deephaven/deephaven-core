...
public class TestShortSegmentedSortedArray extends LiveTableTestCase {
    ...
    private void testShifts(final int seed, final int tableSize, final int nodeSize) {
        ...
        final ShortSegmentedSortedArray ssa = new ShortSegmentedSortedArray(nodeSize);
        ...
        checkSsaInitial(asShort, ssa, valueSource);

        ((DynamicTable)asShort).listenForUpdates(new InstrumentedShiftAwareListenerAdapter((DynamicTable) asShort) {
            @Override
            public void onUpdate(Update upstream) {
                try (final ColumnSource.GetContext checkContext = valueSource.makeGetContext(asShort.getIndex().getPrevIndex().intSize())) {
                    ...
                    checkSsa(ssa, valueSource.getPrevChunk(checkContext, relevantIndices).asShortChunk(), relevantIndices.asKeyIndicesChunk());
                }
                ...
                try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(size)) {
                    ...
                    try (final ColumnSource.GetContext checkContext = valueSource.makeGetContext(asShort.intSize())) {
                        ...
                        checkSsa(ssa, valueSource.getChunk(checkContext, relevantIndices).asShortChunk(), relevantIndices.asKeyIndicesChunk());
                    }
        ...
        for (int step = 0; step < 50; ++step) {
            ...
            try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(asShort.intSize())) {
                checkSsa(ssa, valueSource.getChunk(getContext, asShort.getIndex()).asShortChunk(), asShort.getIndex().asKeyIndicesChunk());
            }
        }
    }
    ...
    private void checkSsa(ShortSegmentedSortedArray ssa, ShortChunk<? extends Values> valueChunk, LongChunk<? extends KeyIndices> tableIndexChunk) {
        ssa.validate();
