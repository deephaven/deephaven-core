...
/**
 * Utility for generating a table that counts the operations performed on its ColumnSource's. Used
 * by the grouping tests to verify that we are not doing unnecessary work.
 */
class CountingTable {
    @NotNull
    ...
    private static <T> ColumnSource<T> getCountingColumnSource(
        final ColumnSource<T> inputColumnSource) {
    ...
    static private class CountingColumnSourceInvocationHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            if (method.getDeclaringClass().equals(MethodCounter.class)) {
                switch (method.getName()) {
                    ...
                    case "getMethodCount":
                        ...
                        Require.eq(method.getParameterCount(), "method.getParameterCount()", 1,
                            "1");
                        ...
                        if (method.getParameterTypes()[0].equals(Method.class)) {
                            ...
                            return methodCounts.entrySet().stream()
                                .filter(x -> x.getKey().getName().equals(args[0]))
            ...
            return wrappedColumnSource.getClass()
