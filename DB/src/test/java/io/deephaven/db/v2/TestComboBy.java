...
public class TestComboBy extends LiveTableTestCase {
    ...
    public void testBy() {
        ...
        AggregationFormulaStateFactory minFactory = new AggregationFormulaStateFactory("min(each)", "each");
        AggregationFormulaStateFactory maxFactory = new AggregationFormulaStateFactory("max(each)", "each");

        ComboAggregateFactory minMaxFactory = new ComboAggregateFactory(new ComboAggregateFactory.ComboByImpl(minFactory, "Min=B"), new ComboAggregateFactory.ComboByImpl(maxFactory, "Max=B"));
        ...
        ComboAggregateFactory doubleCountFactory = new ComboAggregateFactory(new ComboAggregateFactory.CountComboBy("Count1"),new ComboAggregateFactory.CountComboBy("Count2"));
...
//        show(percentiles);
    ...
    public void testComboByMinMaxTypes() {
        ...
        final QueryTable queryTable = getTable(size, random, columnInfo = initColumnInfos(new String[]{"Sym", "intCol", "shortCol", "byteCol", "longCol", "charCol", "doubleCol", "floatCol", "DateTime", "BoolCol", "bigI", "bigD"},
                ...
                new TstUtils.UnsortedDateTimeGenerator(convertDateTime("2020-03-17T12:00:00 NY"), convertDateTime("2020-03-18T12:00:00 NY")),
        ...
        final EvalNuggetInterface [] en = new EvalNuggetInterface[]{
                EvalNugget.from(() ->  queryTable.by(AggCombo(AggMin(queryTable.getDefinition().getColumnNamesArray())))),
                EvalNugget.from(() ->  queryTable.by(AggCombo(AggMax(queryTable.getDefinition().getColumnNamesArray())))),
                new QueryTableTest.TableComparator(
                        queryTable.by(AggCombo(AggMin(queryTable.getDefinition().getColumnNamesArray()))),
                        "AggCombo",
                        queryTable.minBy(),
                        "MinBy"),
                EvalNugget.Sorted.from(() ->  queryTable.by(AggCombo(AggMin(queryTable.getDefinition().getColumnNamesArray())), "Sym"), "Sym"),
                new QueryTableTest.TableComparator(
                        queryTable.by(AggCombo(AggMin(queryTable.getDefinition().getColumnNamesArray())), "Sym").sort("Sym"),
                        "AggCombo",
    ...
    private void testComboByIncremental(final String ctxt, final int size) {
        ...
        final QueryTable queryTable = getTable(size, random, columnInfo = initColumnInfos(new String[]{"Sym", "intCol", "intColNulls", "doubleCol", "doubleColNulls"},
                new TstUtils.SetGenerator<>("a", "b", "c", "d"),
                new TstUtils.IntGenerator(10, 100),
        ...
        EvalNuggetInterface en[] = new EvalNuggetInterface[]{
                new EvalNugget() {
                    public Table e() {
                        return queryTable.by(AggCombo(Agg(AggType.Avg, "MeanI=intCol", "MeanD=doubleCol"), Agg(AggType.Std, "StdI=intCol", "StdD=doubleCol")), "Sym").sort("Sym");
                    }
                },
                new EvalNugget() {
                    public Table e() {
                        return queryTable.by(AggCombo(AggFormula("min(each)", "each", "MinI=intCol", "MinD=doubleCol"), AggFormula("max(each)", "each", "MaxI=intCol")), "Sym").sort("Sym");
                    }
                },
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym", "MinI=min(intCol)", "MinD=min(doubleCol)").sort("Sym"),
                        "UpdateView",
                        queryTable.by(new ComboAggregateFactory(Agg(new MinMaxByStateFactoryImpl(true), "MinI=intCol", "MinD=doubleCol")), "Sym").sort("Sym"),
                        "ComboBy"),
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym", "MaxI=max(intCol)", "MaxD=max(doubleCol)").sort("Sym"),
                        "UpdateView",
                        queryTable.by(AggCombo(Agg(AggType.Max, "MaxI=intCol", "MaxD=doubleCol")), "Sym").sort("Sym"),
                        "ComboBy"),
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym", "MinI=min(intCol)", "MaxI=max(intCol)").sort("Sym"),
                        "UpdateView",
                        queryTable.by(new ComboAggregateFactory(Agg(new MinMaxByStateFactoryImpl(true), "MinI=intCol"), Agg(new MinMaxByStateFactoryImpl(false), "MaxI=intCol")), "Sym").sort("Sym"),
                        "ComboBy"),
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym", "MinD=min(doubleCol)", "MaxD=max(doubleCol)").sort("Sym"),
                        "UpdateView",
                        queryTable.by(AggCombo(Agg(new MinMaxByStateFactoryImpl(true), "MinD=doubleCol"), Agg(new MinMaxByStateFactoryImpl(false), "MaxD=doubleCol")), "Sym").sort("Sym"),
                        "ComboBy"),
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym", "MinD=min(doubleCol)", "MaxI=max(intCol)", "FirstD=first(doubleCol)", "LastI=last(intCol)").sort("Sym"),
                        "UpdateView",
                        ...
                        "ComboBy"),
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym", "MinD=min(doubleCol)", "MaxD=max(doubleCol)", "MinI=min(intCol)", "MaxI=max(intCol)", "LastD=last(doubleCol)", "FirstD=first(doubleCol)", "FirstI=first(intCol)", "LastI=last(intCol)").sort("Sym"),
                        "UpdateView",
                        queryTable.by(AggCombo(
                                AggMin("MinD=doubleCol"),
                        ...
                        "ComboBy"),
                new QueryTableTest.TableComparator(
                        queryTable.by().view("MinD=min(doubleCol)", "MaxI=max(intCol)", "MaxD=max(doubleCol)", "MinI=min(intCol)",
                                "FirstD=first(doubleCol)", "LastI=last(intCol)", "LastD=last(doubleCol)", "FirstI=first(intCol)"),
                        "UpdateView",
                        queryTable.by(AggCombo(
                                AggMin("MinD=doubleCol"),
                                AggMax("MaxI=intCol"),
                        ...
                        "ComboBy"),
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym", "AvgD=avg(doubleCol)", "SumD=sum(doubleCol)", "VarD=var(doubleCol)", "StdD=std(doubleCol)", "intCol").sort("Sym"),
                        "UpdateView",
                ...
                new QueryTableTest.TableComparator(
                        queryTable.by("Sym").view("Sym",
                                ...
                                "Pct01D=percentile(doubleCol, 0.01)", "Pct01I=(int)TestComboBy.percentile(intCol, 0.01)",
                                "Pct05D=percentile(doubleCol, 0.05)", "Pct05I=(int)TestComboBy.percentile(intCol, 0.05)",
                                "Pct25D=percentile(doubleCol, 0.25)", "Pct25I=(int)TestComboBy.percentile(intCol, 0.25)",
                                "Pct50D=percentile(doubleCol, 0.50)", "Pct50I=(int)TestComboBy.percentile(intCol, 0.50)",
                                "Pct65D=percentile(doubleCol, 0.65)", "Pct65I=(int)TestComboBy.percentile(intCol, 0.65)",
                                "Pct90D=percentile(doubleCol, 0.90)", "Pct90I=(int)TestComboBy.percentile(intCol, 0.90)",
                                "Pct99D=percentile(doubleCol, 0.99)", "Pct99I=(int)TestComboBy.percentile(intCol, 0.99)"
                        ).sort("Sym"),
                        queryTable.by(AggCombo(
                                ...
                                AggPct(0.99, "Pct99D=doubleCol", "Pct99I=intCol")
                        ), "Sym").sort("Sym")
                ),
                new QueryTableTest.TableComparator(
                        queryTable.view("Sym", "intCol", "doubleCol").wavgBy("doubleCol", "Sym").renameColumns("WAvg=intCol"), "WAvgBy",
                        queryTable.by(AggCombo(
                                AggWAvg("doubleCol", "WAvg=intCol")
                        ), "Sym"), "AggWAvg"
                ),
                new QueryTableTest.TableComparator(
                        queryTable.view("Sym", "intCol", "doubleCol").countBy("Count"), "Count",
                        queryTable.by(AggCombo(reusedCount), CollectionUtil.ZERO_LENGTH_STRING_ARRAY), "AggCount"
                ),
                new QueryTableTest.TableComparator(
                        queryTable.view("Sym", "intCol", "doubleCol").countBy("Count"), "Count",
                        queryTable.by(AggCombo(reusedCount), CollectionUtil.ZERO_LENGTH_STRING_ARRAY), "AggCount"
    ...
    public void testComboByCountDistinct() {
        ...
        Table result = dataTable.by(AggCombo(AggCountDistinct("Account", "Qty")), "USym").sort("USym");
        Table countNulls = dataTable.by(AggCombo(AggCountDistinct(true,"Account", "Qty")), "USym").sort("USym");
        assertEquals(4, result.size());
    ...
    public void testAggUniqueDefaultValues() {
        ...
        Table result;
        expectException(IllegalArgumentException.class, "Attempted to use no key/non unique values of incorrect types for aggregated columns!",
                () -> dataTable.by(AggCombo(AggUnique(false, -1, -2, "StringCol", "BoolCol", "DatTime", "CharCol", "ByteCol", "ShortCol", "IntCol", "LongCol", "FloatCol", "DoubleCol")), "USym").sort("USym"));

        result = dataTable.by(AggCombo(AggUnique(false, -1, -2, "ByteCol", "ShortCol", "IntCol", "LongCol", "FloatCol", "DoubleCol")), "USym").sort("USym");

        // Byte out of range
        testUniqueOutOfRangeParams(Byte.class, dataTable, ((short)Byte.MIN_VALUE - 1), Byte.MIN_VALUE, ((short)Byte.MAX_VALUE+1), Byte.MAX_VALUE, "ByteCol", "ShortCol", "IntCol", "LongCol", "FloatCol", "DoubleCol");
        testUniqueOutOfRangeParams(Short.class, dataTable, ((int)Short.MIN_VALUE - 1), Short.MIN_VALUE, ((int)Short.MAX_VALUE+1), Short.MAX_VALUE, "ShortCol", "IntCol", "LongCol", "FloatCol", "DoubleCol");
        testUniqueOutOfRangeParams(Integer.class, dataTable, ((long)Integer.MIN_VALUE - 1), Integer.MIN_VALUE, ((long)Integer.MAX_VALUE+1), Integer.MAX_VALUE, "IntCol", "LongCol", "FloatCol", "DoubleCol");
        testUniqueOutOfRangeParams(Long.class, dataTable, BigInteger.valueOf(Long.MIN_VALUE).subtract(BigInteger.ONE), Long.MIN_VALUE, BigInteger.valueOf(Long.MAX_VALUE).add(BigInteger.ONE), Long.MAX_VALUE, "LongCol", "FloatCol", "DoubleCol");

    }

    private void testUniqueOutOfRangeParams(Class<?> type, Table dataTable, Number invalidLow, Number validLow, Number invalidHigh, Number validHigh, String... aggCols) {
        // Byte out of range
        expectException(IllegalArgumentException.class, "Attempted to use no key values too small for " + type.getName() + "!",
                () -> dataTable.by(AggCombo(AggUnique(false, invalidLow, -1, aggCols)), "USym").sort("USym"));

        expectException(IllegalArgumentException.class, "Attempted to use no key values too large for " + type.getName() + "!",
                () -> dataTable.by(AggCombo(AggUnique(false, invalidHigh, -1, aggCols)), "USym").sort("USym"));

        expectException(IllegalArgumentException.class, "Attempted to non unique values too small for " + type.getName() + "!",
                () -> dataTable.by(AggCombo(AggUnique(false, -1, invalidLow, aggCols)), "USym").sort("USym"));

        expectException(IllegalArgumentException.class, "Attempted to use non unique values too large for " + type.getName() + "!",
                () -> dataTable.by(AggCombo(AggUnique(false, -1, invalidHigh, aggCols)), "USym").sort("USym"));

        dataTable.by(AggCombo(AggUnique(false, validLow, validLow, aggCols)), "USym").sort("USym");
        dataTable.by(AggCombo(AggUnique(false, validHigh, validHigh, aggCols)), "USym").sort("USym");
    }
