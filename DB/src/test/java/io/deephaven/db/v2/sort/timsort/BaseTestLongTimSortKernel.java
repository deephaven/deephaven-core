...
import io.deephaven.db.v2.sort.findruns.LongFindRunsKernel;
import io.deephaven.db.v2.sort.partition.LongPartitionKernel;
...
//            dumpOffsets(offsetsOut, lengthsOut);
    ...
    static private void prepareLongChunks(List<LongLongTuple> javaTuples, WritableLongChunk valueChunk, WritableLongChunk<Attributes.KeyIndices> indexKeys) {
        for (int ii = 0; ii < javaTuples.size(); ++ii) {
    ...
    static private void prepareMultiLongChunks(List<LongLongLongTuple> javaTuples, WritableLongChunk valueChunk, WritableLongChunk secondaryChunk, WritableLongChunk<Attributes.KeyIndices> indexKeys) {
        for (int ii = 0; ii < javaTuples.size(); ++ii) {
            valueChunk.set(ii, javaTuples.get(ii).getFirstElement());
...
//        }
    ...
    static private void verify(int size, List<LongLongLongTuple> javaTuples, LongChunk primaryChunk, LongChunk secondaryChunk, LongChunk<Attributes.KeyIndices> indexKeys) {
//        System.out.println("Verify: " + javaTuples);
