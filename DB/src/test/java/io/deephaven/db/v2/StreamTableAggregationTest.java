...
public class StreamTableAggregationTest extends JUnit4QueryTableTestBase {
    ...
    private void doOperatorTest(@NotNull final UnaryOperator<Table> operator,
        ...
        TestCase.assertFalse(((BaseTable) streamExpected).isStream()); // Aggregation results are
                                                                       // never stream tables
                ...
                LiveTableMonitor.DEFAULT.refreshLiveTableForUnitTests(() -> {
                    if (normalStepInserted.nonempty()) {
                        ...
                        normal.notifyListeners(
                            new Update(normalStepInserted, Index.CURRENT_FACTORY.getEmptyIndex(),
                                Index.CURRENT_FACTORY.getEmptyIndex(), IndexShiftData.EMPTY,
    ...
    public void testComboBy() {
        doOperatorTest(table -> table.by(AggCombo(
            ...
            AggSortedLast("Sym", "SymSortedLastPrice=Price", "SymSortedLastSize=Size")), "Sym"),
            false);
    }
