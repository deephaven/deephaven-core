...
public class ReplicateSourcesAndChunkTest {
    public static void main(String[] args) throws IOException {
        ...
        ReplicatePrimitiveCode.charToAllButBoolean(TestCharacterArraySource.class,
            ReplicatePrimitiveCode.TEST_SRC);
        fixupBooleanColumnSourceTest(
            ReplicatePrimitiveCode.charToBooleanAsByte(TestCharacterArraySource.class,
                ReplicatePrimitiveCode.TEST_SRC, Collections.emptyMap()));
        fixupObjectColumnSourceTest(ReplicatePrimitiveCode
            .charToObject(TestCharacterArraySource.class, ReplicatePrimitiveCode.TEST_SRC));

        ReplicatePrimitiveCode.charToAllButBoolean(TestCharacterSparseArraySource.class,
            ReplicatePrimitiveCode.TEST_SRC);
        fixupBooleanColumnSourceTest(
            ReplicatePrimitiveCode.charToBooleanAsByte(TestCharacterSparseArraySource.class,
                ReplicatePrimitiveCode.TEST_SRC, Collections.emptyMap()));
        fixupObjectColumnSourceTest(ReplicatePrimitiveCode
            .charToObject(TestCharacterSparseArraySource.class, ReplicatePrimitiveCode.TEST_SRC));

        ReplicatePrimitiveCode.charToAll(TestCharChunk.class, ReplicatePrimitiveCode.TEST_SRC);
        fixupChunkTest(ReplicatePrimitiveCode.charToObject(TestCharChunk.class,
            ReplicatePrimitiveCode.TEST_SRC));

        ReplicatePrimitiveCode.charToAllButBoolean(TestCharacterDeltaAwareColumnSource.class,
            ReplicatePrimitiveCode.TEST_SRC);
        fixupBooleanDeltaAwareColumnSourceTest(
            ReplicatePrimitiveCode.charToBooleanAsByte(TestCharacterDeltaAwareColumnSource.class,
                ReplicatePrimitiveCode.TEST_SRC, Collections.emptyMap()));
        fixupObjectDeltaAwareColumnSourceTest(ReplicatePrimitiveCode.charToObject(
            TestCharacterDeltaAwareColumnSource.class, ReplicatePrimitiveCode.TEST_SRC));
    }
    ...
    private static void fixupBooleanColumnSourceTest(String booleanPath) throws IOException {
        ...
        lines =
            simpleFixup(lines, "arrayFill", "NULL_BOOLEAN", "BooleanUtils.NULL_BOOLEAN_AS_BYTE");
        lines = simpleFixup(lines, "testsourcesink", "ChunkType.Boolean", "ChunkType.Object");

        lines =
            applyFixup(lines, "fromsource", "(.*)checkFromSource\\((.*)byte fromSource(.*)\\) \\{",
                m -> Collections.singletonList(m.group(1) + "checkFromSource(" + m.group(2)
                    + "Boolean fromSource" + m.group(3) + ") {"));
        lines =
            applyFixup(lines, "fromsource", "(.*)checkFromSource\\((.*)byte fromChunk(.*)\\) \\{",
                m -> Collections.singletonList(m.group(1) + "checkFromSource(" + m.group(2)
                    + "Boolean fromChunk" + m.group(3) + ") {"));
        lines =
            applyFixup(lines, "fromvalues", "(.*)checkFromValues\\((.*)byte fromChunk(.*)\\) \\{",
                m -> Collections.singletonList(m.group(1) + "checkFromValues(" + m.group(2)
                    + "Boolean fromChunk" + m.group(3) + ") {"));
        lines = applyFixup(lines, "fromvalues", "(.*)fromValues, fromChunk\\);",
            m -> Collections.singletonList(m.group(1)
    ...
    private static void fixupObjectDeltaAwareColumnSourceTest(String objectPath)
        throws IOException {
        fixupObjectColumnSourceTest(objectPath);
    }

    private static void fixupBooleanDeltaAwareColumnSourceTest(String booleanPath)
        throws IOException {
        final File booleanFile = new File(booleanPath);
        List<String> lines = FileUtils.readLines(booleanFile, Charset.defaultCharset());
        lines = globalReplacements(lines,
            ...
            "asBooleanChunk", "asObjectChunk",
            "values.get\\((.*)\\)",
            "io.deephaven.db.util.BooleanUtils.booleanAsByte(values.get($1))");
        lines = addImport(lines, BooleanUtils.class);
        lines = addImport(lines, ObjectChunk.class);
        FileUtils.writeLines(booleanFile, lines);
    }
