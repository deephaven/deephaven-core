...
public class TestTableAssertions {
    ...
    public void testStatic() {
        ...
        try {
            ...
            TestCase.assertEquals("Table violates sorted assertion, table description=test, column=D1, Ascending, 3.141592653589793 is out of order with respect to -Infinity!", saf.getMessage());
        }

        TestCase.assertEquals(SortingOrder.Ascending, SortedColumnsAttribute.getOrderForColumn(test, "Plant").orElse(null));
        TestCase.assertEquals(SortingOrder.Descending, SortedColumnsAttribute.getOrderForColumn(test, "Int").orElse(null));
        TestCase.assertEquals(Optional.empty(), SortedColumnsAttribute.getOrderForColumn(test, "D1"));
    }

    @Test
    public void testRefreshing() {
        final QueryTable test = TstUtils.testRefreshingTable(i(10, 11, 12, 17), stringCol("Plant", "Apple", "Banana", "Carrot", "Daffodil"),
                intCol("Int", 9, 7, 5, 3));
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            addToTable(test, i(9, 13, 18), stringCol("Plant", "Aaple", "DAFODIL", "Forsythia"), intCol("Int", 10, 4, 0));
    ...
    public void testIncrementalRandom(int seed, int size) {
        ...
        final QueryTable table = getTable(true, size, random, columnInfo = initColumnInfos(new String[]{"SortValue", "Sentinel"},
...
//            final Table badTable = LiveTableMonitor.DEFAULT.sharedLock().computeLocked(() -> table.update("RV=random1.nextDouble() < 0.00001 ? -1L : SortValue"));
...
//                    },
                    new EvalNugget() {
                        @Override
                        protected Table e() {
                            return TableAssertions.assertSorted("table sorted by sentinel", table.sortDescending("Sentinel"), "Sentinel", SortingOrder.Descending);
                        }
                    },
            };

            for (int step = 0; step < maxSteps; step++) {
                LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> GenerateTableUpdates.generateShiftAwareTableUpdates(GenerateTableUpdates.DEFAULT_PROFILE, size, random, table, columnInfo));
                validate(en);
            }
//        } finally {
