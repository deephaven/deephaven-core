...
import io.deephaven.engine.v2.sources.sparse.DoubleOneOrN;
...
public class DoubleSparseArraySource extends SparseArrayColumnSource<Double> implements MutableColumnSourceGetDefaults.ForDouble {
    ...
    public void remove(Index toRemove) {
        toRemove.forEachLong((i) -> { set(i, NULL_DOUBLE); return true; });
    }
    ...
    public void startTrackingPrevValues() {
        ...
        prevFlusher = new UpdateCommitter<>(this, DoubleSparseArraySource::commitUpdates);
    }
    ...
    */
    ...
    void fillByUnorderedKeys(@NotNull WritableChunk<? super Values> dest, @NotNull LongChunk<? extends KeyIndices> keys) {
        ...
        for (int ii = 0; ii < keys.size(); ) {
            ...
            if (firstKey == Index.NULL_KEY) {
                doubleChunk.set(ii++, NULL_DOUBLE);
                continue;
            }
            ...
            if (block == null) {
                doubleChunk.fillWithNullValue(ii, lastII - ii + 1);
                ii = lastII + 1;
                continue;
            }
            while (ii <= lastII) {
                final int indexWithinBlock = (int) (keys.get(ii) & INDEX_MASK);
                doubleChunk.set(ii++, block[indexWithinBlock]);
            }
        }
    ...
    void fillPrevByUnorderedKeys(@NotNull WritableChunk<? super Values> dest, @NotNull LongChunk<? extends KeyIndices> keys) {
        ...
        for (int ii = 0; ii < keys.size(); ) {
            ...
            while (ii <= lastII) {
                ...
                doubleChunk.set(ii++, blockToUse == null ? NULL_DOUBLE : blockToUse[indexWithinBlock]);
            }
        }
        dest.setSize(keys.size());
    }
    ...
    void fillFromChunkByKeys(@NotNull OrderedKeys orderedKeys, Chunk<? extends Values> src) {
        ...
        for (int ii = 0; ii < keys.size(); ) {
            ...
            final double[] prevBlock = hasPrev ? ensurePrevBlock(firstKey, block0, block1, block2) : null;
    ...
    // region getChunk
    @Override
    public DoubleChunk<Values> getChunk(@NotNull GetContext context, @NotNull OrderedKeys orderedKeys) {
        if (orderedKeys.size() == 0) {
        ...
        return getChunkByFilling(context, orderedKeys).asDoubleChunk();
    }
    // endregion getChunk

    // region getPrevChunk
    @Override
    public DoubleChunk<Values> getPrevChunk(@NotNull GetContext context, @NotNull OrderedKeys orderedKeys) {
        if (prevFlusher == null) {
            return getChunk(context, orderedKeys);
        }
        return getPrevChunkByFilling(context, orderedKeys).asDoubleChunk();
    }
