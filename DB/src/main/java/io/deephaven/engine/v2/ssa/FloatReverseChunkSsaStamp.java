...
public class FloatReverseChunkSsaStamp implements ChunkSsaStamp {
    ...
    private void applyShift(FloatChunk<Values> leftStampValues, LongChunk<Attributes.RowKeys> leftStampKeys, FloatChunk<? extends Values> rightStampChunk, LongChunk<Attributes.RowKeys> rightStampKeys, long shiftDelta, MutableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final float rightStampValue = rightStampChunk.get(ii);
