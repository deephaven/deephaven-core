...
public final class DoubleChunkPool implements ChunkPool {
    ...
    private final WritableDoubleChunk<Any> EMPTY = WritableDoubleChunk.writableChunkWrap(ArrayUtils.EMPTY_DOUBLE_ARRAY);
    ...
    DoubleChunkPool() {
        ...
        for (int pcci = 0; pcci < NUM_POOLED_CHUNK_CAPACITIES; ++pcci) {
            ...
            writableDoubleChunks[pcci] = new SegmentedSoftPool<>(
                    ...
                    () -> QueryPerformanceRecorder.recordPoolAllocation(() -> WritableDoubleChunk.makeWritableChunkForPool(chunkCapacity)),
                    (final WritableDoubleChunk chunk) -> chunk.setSize(chunkCapacity)
            );
        }
        resettableDoubleChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableDoubleChunk::makeResettableChunkForPool),
                ResettableDoubleChunk::clear
        );
        resettableWritableDoubleChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableWritableDoubleChunk::makeResettableChunkForPool),
