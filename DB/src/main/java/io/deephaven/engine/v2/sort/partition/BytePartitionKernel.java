...
//    }
    ...
    public static PartitionKernelContext createContext(TrackingMutableRowSet rowSet, ColumnSource<Byte> columnSource, int chunkSize, int nPartitions, boolean preserveEquality) {
    ...
    private static void samplePivots(TrackingMutableRowSet rowSet, int nPartitions, WritableLongChunk<RowKeys> pivotKeys, WritableByteChunk<Any> pivotValues, ColumnSource<Byte> columnSource) {
        pivotKeys.setSize(0);
    ...
    private static int binarySearchTieIndex(ByteChunk pivotValues, LongChunk<RowKeys> pivotKeys, int lo, int hi, byte searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final byte compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
