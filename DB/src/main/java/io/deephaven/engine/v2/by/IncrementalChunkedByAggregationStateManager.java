...
import static io.deephaven.engine.v2.by.IncrementalByAggregationUpdateTracker.NULL_COOKIE;
...
class IncrementalChunkedByAggregationStateManager
    // region extensions
    // endregion extensions
{
    // region constants
    static final int CHUNK_SIZE = 1 << 12;
    static final int MINIMUM_INITIAL_HASH_SIZE = 1 << 11; // io.deephaven.engine.v2.sources.ArrayBackedColumnSource.BLOCK_SIZE
    static final int MAX_TABLE_SIZE = HashTableColumnSource.MINIMUM_OVERFLOW_HASH_SLOT;
    // endregion constants
    ...
    @FunctionalInterface
    interface ShiftAppliedCallback {

        /**
         * Invoke a callback that will allow external trackers to record an applied shift.
         *
         * @param cookie    The current cookie for the state, or 0 on insert
         * @param stateSlot The state slot (in main table space)
         * @return The new cookie for the state
         */
        long invoke(long cookie, int stateSlot);
    }

    @FunctionalInterface
    interface StateMoveCallback {

        /**
         * Invoke a callback that will allow external trackers to record a state's move from one slot to another on
         * rehash.
         *
         * @param cookie       The current cookie for the state
         * @param newStateSlot The new state slot (in main table space)
         */
        void invoke(long cookie, int newStateSlot);
    }
    ...
    static final int MINIMUM_OVERFLOW_HASH_SLOT = MAX_TABLE_SIZE;
    ...
    // These are re-used in all build and probe passes for interaction with the IncrementalByAggregationUpdateTracker
    private final LongArraySource cookieSource = new LongArraySource();
    private final LongArraySource overflowCookieSource = new LongArraySource();
    // endregion extra variables

    IncrementalChunkedByAggregationStateManager(ColumnSource<?>[] tableKeySources
                                         , int tableSize
    ...
    void buildInitialTableFromCurrent(@NotNull final Table sourceTable, @NotNull final ColumnSource<?>[] keySources, @NotNull final IncrementalByAggregationUpdateTracker aggregationUpdateTracker) {
    ...
    private void buildTable(final BuildContext bc,
                            ...
                            , final StateMoveCallback moveCallback
            ...
            while (rsIt.hasMore()) {
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ++ii) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        cookieSource.set(tableLocation, trackingCallback.invoke(cookieSource.getLong(tableLocation), (int) tableLocation, sourceChunkIndexKeys.get(ii)));
                ...
                for (int ii = 0; ii < bc.insertPositionsInSourceChunk.size(); ) {
                    ...
                    // region main insert
                    indexSource.set(currentHashLocation, RowSetFactory.empty().convertToTracking());
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        trackingCallback.invoke(cookieSource.getLong(tableLocation), (int) tableLocation, sourceChunkIndexKeys.get(chunkPosition));
                ...
                if (bc.overflowPositionInSourceChunk.size() > 0) {
                    ...
                    while (bc.overflowPositionInSourceChunk.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.equalValues.size(); ++ii) {
                            ...
                            if (bc.equalValues.get(ii)) {
                                ...
                                overflowCookieSource.set(overflowLocation, trackingCallback.invoke(overflowCookieSource.getLong(overflowLocation), overflowLocationToHashLocation((int) overflowLocation), sourceChunkIndexKeys.get(chunkPosition)));
                    ...
                    while (bc.chunkPositionsToInsertInOverflow.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.chunkPositionsToInsertInOverflow.size(); ) {
                            ...
                            overflowIndexSource.set(allocatedOverflowLocation, RowSetFactory.empty().convertToTracking());
                        ...
                        for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                            ...
                            if (bc.equalValues.get(ii)) {
                                ...
                                trackingCallback.invoke(overflowCookieSource.getLong(insertedOverflowLocation), overflowLocationToHashLocation((int) insertedOverflowLocation), sourceChunkIndexKeys.get(chunkPosition));
    ...
    public void doRehash(BuildContext bc
                          ...
                          , StateMoveCallback moveCallback
            ...
            while (rsIt.hasMore()) {
                ...
                for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                    if (pc.equalValues.get(ii)) {
                        ...
                        cookieSource.set(tableLocation, trackingCallback.invoke(cookieSource.getLong(tableLocation), (int) tableLocation, pc.keyIndices.get(ii)));
                ...
                while (pc.overflowLocationsToFetch.size() > 0) {
                    ...
                    for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                        ...
                        if (pc.equalValues.get(ii)) {
                            ...
                            overflowCookieSource.set(overflowLocation, trackingCallback.invoke(overflowCookieSource.getLong(overflowLocation), overflowLocationToHashLocation((int) overflowLocation), pc.keyIndices.get(chunkPosition)));
    ...
    void clearCookies() {
        for (int si = 0; si < tableSize; ++si) {
            cookieSource.set(si, NULL_COOKIE);
        }
        for (int osi = 0; osi < nextOverflowLocation; ++ osi) {
            overflowCookieSource.set(osi, NULL_COOKIE);
        }
    }

    void applyShift(final int stateSlot,
