...
// endregion extensions
{
    ...
    private void addSymbols(final Index index, ColumnSource<String> symbolSource, ColumnSource<Long> idSource,
    ...
    class BuildContext implements Context {
        ...
        // @WritableStateChunkType@ from \QWritableIntChunk<Values>\E, @WritableStateChunkName@ from
        // \QWritableIntChunk\E
    ...
    // endregion extra build arguments
        ...
        // endregion build initialization try
            ...
            final WritableIntChunk<Attributes.KeyIndices> sourceResultIdentifiers =
                    WritableIntChunk.makeWritableChunk((int) Math.min(CHUNK_SIZE, buildIndex.size()));
            ...
            while (okIt.hasMore()) {
                ...
                uniqueIdentifierSource.fillChunkUnordered(bc.stateSourceFillContext, bc.workingStateEntries,
    ...
    // endregion extra rehash arguments
        ...
        while (rehashRequired()) {
            ...
            overflowLocationSource.fillChunk(bc.overflowFillContext, bc.overflowLocations,
                    OrderedKeys.wrapKeyIndicesChunkAsOrderedKeys(LongChunk.downcast(bc.rehashLocations)));
            ...
            for (int loop = 0; loop < 2; loop++) {
                ...
                overflowLocationSource.fillChunk(bc.overflowFillContext, bc.overflowLocations,
                        OrderedKeys.wrapKeyIndicesChunkAsOrderedKeys(bc.overflowLocationForPromotionLoop));
                ...
                overflowUniqueIdentifierSource.fillChunkUnordered(bc.overflowStateSourceFillContext,
                        bc.workingStateEntries, bc.overflowLocationsAsKeyIndices);
                    ...
                    if ((firstLoop && tableLocation < tableHashPivot)
                        ...
                        if (tableLocation > lastBackingChunkLocation) {
                            if (bc.sourcePositions.size() > 0) {
                                ...
                                // mixin allowUpdateWriteThroughState
                                // @StateChunkTypeEnum@ from \QInt\E
                                IntPermuteKernel.permute(bc.sourcePositions, bc.workingStateEntries,
    ...
    private void createOverflowPartitions(WritableLongChunk<KeyIndices> overflowLocationsToFetch,
            WritableLongChunk<KeyIndices> rehashLocations, WritableBooleanChunk<Any> shouldMoveBucket, int moves) {
    ...
    private void compactOverflowLocations(IntChunk<Values> overflowLocations,
            WritableLongChunk<KeyIndices> overflowLocationsToFetch) {
    ...
    private void swapOverflowPointers(LongChunk<KeyIndices> tableLocationsChunk,
            LongChunk<KeyIndices> overflowLocationsToFetch) {
    ...
    // mixin allowUpdateWriteThroughState
    // @WritableStateChunkType@ from \QWritableIntChunk<Values>\E
    private void updateWriteThroughState(ResettableWritableIntChunk<Values> writeThroughState, long firstPosition,
            long expectedLastPosition) {
        final long firstBackingChunkPosition =
                uniqueIdentifierSource.resetWritableChunkToBackingStore(writeThroughState, firstPosition);
    ...
    private static long updateWriteThroughChunks(ResettableWritableChunk<Values>[] writeThroughChunks,
            long currentHashLocation, ArrayBackedColumnSource<?>[] sources) {
        final long firstBackingChunkPosition =
                sources[0].resetWritableChunkToBackingStore(writeThroughChunks[0], currentHashLocation);
        for (int jj = 1; jj < sources.length; ++jj) {
            if (sources[jj].resetWritableChunkToBackingStore(writeThroughChunks[jj],
                    currentHashLocation) != firstBackingChunkPosition) {
                throw new IllegalStateException("ArrayBackedColumnSources have different block sizes!");
            }
    ...
    private void fillKeys(ColumnSource.FillContext[] fillContexts, WritableChunk<Values>[] keyChunks,
            WritableLongChunk<KeyIndices> tableLocationsChunk) {
    ...
    private void fillOverflowKeys(ColumnSource.FillContext[] fillContexts, WritableChunk<Values>[] keyChunks,
            WritableLongChunk<KeyIndices> overflowLocationsChunk) {
        fillKeys(overflowKeySources, fillContexts, keyChunks, overflowLocationsChunk);
    }

    private static void fillKeys(ArrayBackedColumnSource<?>[] keySources, ColumnSource.FillContext[] fillContexts,
            WritableChunk<Values>[] keyChunks, WritableLongChunk<KeyIndices> keyIndices) {
    ...
    private void getKeyChunks(ColumnSource<?>[] sources, ColumnSource.GetContext[] contexts,
            Chunk<? extends Values>[] chunks, OrderedKeys orderedKeys) {
    ...
    private void decorationProbe(ProbeContext pc, OrderedKeys probeIndex, final ColumnSource<?>[] probeSources
    ...
    // endregion additional probe arguments
        ...
        // endregion probe additional try resources
            ...
            final WritableIntChunk<Attributes.KeyIndices> workingSymbolValues =
                    WritableIntChunk.makeWritableChunk(pc.chunkSize);
            ...
            while (okIt.hasMore()) {
                ...
                uniqueIdentifierSource.fillChunkUnordered(pc.stateSourceFillContext, pc.workingStateEntries,
    ...
    private void convertHashToTableLocations(WritableIntChunk<HashCode> hashChunk,
            WritableLongChunk<KeyIndices> tablePositionsChunk) {
        // mixin rehash
    ...
    private void convertHashToTableLocations(WritableIntChunk<HashCode> hashChunk,
            WritableLongChunk<KeyIndices> tablePositionsChunk, int pivotPoint) {
        // endmixin rehash
    ...
    private void dumpTable(boolean showBadMain, boolean showBadOverflow) {
        ...
        final WritableIntChunk<Values> overflowOverflowLocationChunk =
        ...
        final OrderedKeys overflowLocations =
                nextOverflowLocation > 0 ? Index.FACTORY.getIndexByRange(0, nextOverflowLocation - 1)
                        : OrderedKeys.EMPTY;
        ...
        for (int ii = 0; ii < keyColumnCount; ++ii) {
            ...
            try (final ColumnSource.FillContext fillContext =
                    overflowKeySources[ii].makeFillContext(nextOverflowLocation)) {
        ...
        try (final ColumnSource.FillContext fillContext =
                overflowOverflowLocationSource.makeFillContext(nextOverflowLocation)) {
            overflowOverflowLocationSource.fillChunk(fillContext, overflowOverflowLocationChunk, overflowLocations);
        }
        try (final ColumnSource.FillContext fillContext =
                overflowUniqueIdentifierSource.makeFillContext(nextOverflowLocation)) {
        ...
        for (int ii = 0; ii < tableSize; ++ii) {
            ...
            if (stateValue == EMPTY_SYMBOL_VALUE) {
                ...
                System.out.println(ChunkUtils.extractKeyStringFromChunks(keyChunkTypes, dumpChunks, ii) + " (hash="
                        + hashCode + ") = " + stateValue);
            ...
            while (overflowLocation != QueryConstants.NULL_INT) {
                ...
                System.out.println("\tOF[" + overflowLocation + "] "
                        + ChunkUtils.extractKeyStringFromChunks(keyChunkTypes, overflowDumpChunks, overflowLocation)
                        + " (hash=" + overflowHashCode + ") = " + overflowStateValue);
