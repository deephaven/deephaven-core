    ...
    private PreAndPostFilters applyFilterRenamings(SelectFilter[] filters) {
        ...
        for (SelectFilter filter : filters) {
            ...
            if (postFilter) {
                ...
                if (!renames.isEmpty()) {
                    if (filter instanceof io.deephaven.engine.v2.select.MatchFilter) {
                        io.deephaven.engine.v2.select.MatchFilter matchFilter =
                                (io.deephaven.engine.v2.select.MatchFilter) filter;
