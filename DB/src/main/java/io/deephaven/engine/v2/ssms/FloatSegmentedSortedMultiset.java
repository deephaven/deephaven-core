...
import io.deephaven.engine.structures.vector.DbFloatArrayDirect;
...
public final class FloatSegmentedSortedMultiset implements SegmentedSortedMultiSet<Float>, DbFloatArray {
    ...
    private transient DbFloatArray prevValues;
    // endregion Deltas
    ...
    public boolean insert(WritableChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        ...
        insert(valuesToInsert.asWritableFloatChunk(), counts);
        return beforeSize != size();
    }

    private int insertExistingIntoLeaf(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, float [] leafValues, long [] leafCounts, float maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        float nextValue;
        while (rlpos < leafSize && ripos < valuesToInsert.size() && (leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
            if (gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    ...
                    valuesToInsert.copyFromTypedChunk((WritableFloatChunk)valuesToInsert, ripos, wipos.intValue(), lastInsert - ripos);
    ...
    private void distributeNewIntoLeaves(FloatChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, final int insertStart, final int insertCount, int firstLeaf, int requiredLeaves, int newLeafSize) {
    ...
    private void insertNewIntoLeaf(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, float [] leafValues, long [] leafCounts) {
    ...
    private void copyRemainingValuesToLeaf(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, float[] leafValues, long[] leafCounts, int ripos) {
    ...
    private void maybeCompact(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, int wipos) {
        if (wipos == ripos) {
            return;
        }
    ...
    private void insertExisting(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        if (leafCount == 0) {
            return;
        }
        if (leafCount == 1) {
            final MutableInt wipos = new MutableInt(0);
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, 0, wipos, size, directoryValues, directoryCount, NULL_FLOAT, true);
    ...
    private void insert(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        validate();
        ...
        if (leafCount > 1 && gt(valuesToInsert.get(0), getMaxFloat())) {
    ...
    private void makeLeavesInitial(FloatChunk<? extends Values> values, IntChunk<ChunkLengths> counts) {
    ...
    private void packValuesIntoLeaves(FloatChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int rpos, int startLeaf, int valuesPerLeaf) {
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(FloatChunk<? extends Attributes.Any> valuesToSearch, int lo, int hi, float searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
    ...
    public boolean remove(RemoveContext removeContext, WritableChunk<? extends Values> valuesToRemove, WritableIntChunk<ChunkLengths> counts) {
        final long beforeSize = size();
        remove(removeContext, valuesToRemove.asFloatChunk(), counts);
        return beforeSize != size();
    }

    private void remove(RemoveContext removeContext, FloatChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts) {
        validate();
    ...
    private int removeFromLeaf(RemoveContext removeContext, FloatChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts, int ripos, int end, float[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
    ...
    private void validateInputs(FloatChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        if (!SEGMENTED_SORTED_MULTISET_VALIDATION) {
            return;
        }
...
//                }
    ...
    private static void validateLeaf(float[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])", (Float)thisValue, "values[ii]", (Float)nextValue, "values[ii + 1]", ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(geq(getMinFloat(), destination.getMaxFloat()), "geq(getMinFloat(), destination.getMaxFloat())");
            }
        }
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(leq(getMaxFloat(), destination.getMinFloat()), "leq(getMaxFloat(), destination.getMinFloat())");
            }
        }
        ...
        if (completeLeavesToMove > 0) {
            if (leafCount == 1) {
                ...
                if (directoryMoves > 0) {
                    Arrays.fill(directoryValues, firstLeafTozero, firstLeafTozero + directoryMoves - (completeLeavesToMove - numberOfLeavesToRemove), NULL_FLOAT);
                }
                maybePromoteLastLeaf();
            }
            destination.maybePromoteLastLeaf();
    ...
    private void maybeAccumulateAdditions(WritableFloatChunk<? extends Values> valuesToInsert) {
        if (!accumulateDeltas || valuesToInsert.size() == 0) {
            return;
        }

        if(prevValues == null) {
            prevValues = new DbFloatArrayDirect(keyArray());
        }
    ...
    public DbFloatArray getPrevValues() {
    ...
    public DbFloatArray subArray(long fromIndex, long toIndex) {
        return new DbFloatArrayDirect(keyArray(fromIndex, toIndex));
    }
    ...
    private boolean equalsArray(DbFloatArray o) {
        if(size() != o.size()) {
            return false;
        }
    ...
    private boolean equalsArray(DbArray<?> o) {
        ...
        if(leafCount == 1) {
            for(int ii = 0; ii < size; ii++) {
                final Float val = (Float)o.get(ii);
                //region DbArrayEquals
                if(directoryValues[ii] == NULL_FLOAT && val != null && val != NULL_FLOAT) {
                    return false;
                }
                //endregion DbArrayEquals
        ...
        for (int li = 0; li < leafCount; ++li) {
            for(int ai = 0; ai < leafSizes[li]; ai++) {
                final Float val = (Float)o.get(nCompared++);
                //region DbArrayEquals
                if(leafValues[li][ai] == NULL_FLOAT && val != null && val != NULL_FLOAT) {
                    return false;
                }
                //endregion DbArrayEquals
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof FloatSegmentedSortedMultiset)) {
            //region DbArrayEquals
            if(o instanceof DbFloatArray) {
                return equalsArray((DbFloatArray)o);
            }
            //endregion DbArrayEquals
