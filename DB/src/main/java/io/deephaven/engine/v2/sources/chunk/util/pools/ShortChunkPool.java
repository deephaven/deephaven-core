...
public final class ShortChunkPool implements ChunkPool {
    ...
    private final WritableShortChunk<Any> EMPTY = WritableShortChunk.writableChunkWrap(ArrayUtils.EMPTY_SHORT_ARRAY);
    ...
    ShortChunkPool() {
        ...
        for (int pcci = 0; pcci < NUM_POOLED_CHUNK_CAPACITIES; ++pcci) {
            ...
            writableShortChunks[pcci] = new SegmentedSoftPool<>(
                    ...
                    () -> QueryPerformanceRecorder.recordPoolAllocation(() -> WritableShortChunk.makeWritableChunkForPool(chunkCapacity)),
                    (final WritableShortChunk chunk) -> chunk.setSize(chunkCapacity)
            );
        }
        resettableShortChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableShortChunk::makeResettableChunkForPool),
                ResettableShortChunk::clear
        );
        resettableWritableShortChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableWritableShortChunk::makeResettableChunkForPool),
