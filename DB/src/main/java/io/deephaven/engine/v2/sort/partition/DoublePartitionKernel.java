...
import io.deephaven.engine.v2.sort.timsort.DoubleLongTimsortKernel;
...
public class DoublePartitionKernel {
    public static class PartitionKernelContext {
        ...
        private final WritableDoubleChunk<Any> pivotValues;
        ...
        private PartitionKernelContext(int chunkSize, int numPartitions, boolean preserveEquality) {
            ...
            pivotValues = WritableDoubleChunk.makeWritableChunk(numPartitions - 1);
...
//            System.out.println("[" + IntStream.range(0, pivotValues.size()).mapToObj(pivotValues::get).map(DoublePartitionKernel::format).collect(Collectors.joining(",")) + "]");
...
//    }
    ...
    public static PartitionKernelContext createContext(TrackingMutableRowSet rowSet, ColumnSource<Double> columnSource, int chunkSize, int nPartitions, boolean preserveEquality) {
        ...
        final WritableDoubleChunk<Any> tempPivotValues = WritableDoubleChunk.makeWritableChunk(nPartitions * 3);
    ...
    private static void samplePivots(TrackingMutableRowSet rowSet, int nPartitions, WritableLongChunk<RowKeys> pivotKeys, WritableDoubleChunk<Any> pivotValues, ColumnSource<Double> columnSource) {
        pivotKeys.setSize(0);
        ...
        try (final LongSortKernel sortContext = DoubleLongTimsortKernel.createContext(samplesRequired)) {
     ...
     */
    public static void partition(PartitionKernelContext context, LongChunk<RowKeys> indexKeys, DoubleChunk values) {
        ...
        for (int ii = 0; ii < values.size(); ii += accumulatedChunkSize) {
            ...
            for (int jj = ii; jj < last; jj++) {
                ...
                final double searchValue = values.get(jj);
    ...
    private static int binarySearchPreserve(DoubleChunk pivotValues, int lo, int hi, double searchValue) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final double compareValue = pivotValues.get(mid);
    ...
    private static int binarySearchTieIndex(DoubleChunk pivotValues, LongChunk<RowKeys> pivotKeys, int lo, int hi, double searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final double compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
