...
//    }
    ...
    public static PartitionKernelContext createContext(Index index, ColumnSource<Double> columnSource, int chunkSize, int nPartitions, boolean preserveEquality) {
    ...
    private static void samplePivots(Index index, int nPartitions, WritableLongChunk<KeyIndices> pivotKeys, WritableDoubleChunk<Any> pivotValues, ColumnSource<Double> columnSource) {
        pivotKeys.setSize(0);
     ...
     */
    public static void partition(PartitionKernelContext context, LongChunk<KeyIndices> indexKeys, DoubleChunk values) {
    ...
    private static int binarySearchTieIndex(DoubleChunk pivotValues, LongChunk<KeyIndices> pivotKeys, int lo, int hi, double searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final double compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
