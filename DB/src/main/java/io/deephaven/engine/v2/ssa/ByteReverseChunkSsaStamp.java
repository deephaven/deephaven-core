...
public class ByteReverseChunkSsaStamp implements ChunkSsaStamp {
    ...
    private void applyShift(ByteChunk<Values> leftStampValues, LongChunk<Attributes.RowKeys> leftStampKeys, ByteChunk<? extends Values> rightStampChunk, LongChunk<Attributes.RowKeys> rightStampKeys, long shiftDelta, MutableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final byte rightStampValue = rightStampChunk.get(ii);
