...
import io.deephaven.engine.v2.utils.rsp.container.BitmapContainer;
import io.deephaven.engine.v2.utils.rsp.container.TwoValuesContainer;
import io.deephaven.engine.v2.utils.rsp.container.SingleRangeContainer;
import io.deephaven.engine.v2.utils.rsp.container.RunContainer;
...
public abstract class RspArray<T extends RspArray> extends RefCountedCow<T> {
    ...
    public long rangesCountUpperBound(final int startIdx, final int endIdx) {
        ...
        for (int idx = startIdx; idx <= endIdx; ++idx) {
            ...
            if (isSingletonSpan(s)) {
                ++nRanges;
            } else if (s instanceof io.deephaven.engine.v2.utils.rsp.container.RunContainer) {
                nRanges += ((io.deephaven.engine.v2.utils.rsp.container.RunContainer) s).numberOfRanges();
            } else if (s instanceof io.deephaven.engine.v2.utils.rsp.container.SingleRangeContainer) {
                nRanges += 1;
            } else if (s instanceof io.deephaven.engine.v2.utils.rsp.container.TwoValuesContainer) {
                nRanges += 2;
