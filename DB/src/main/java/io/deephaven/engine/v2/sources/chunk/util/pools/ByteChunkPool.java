    ...
    ByteChunkPool() {
        ...
        for (int pcci = 0; pcci < NUM_POOLED_CHUNK_CAPACITIES; ++pcci) {
            ...
            writableByteChunks[pcci] = new SegmentedSoftPool<>(
                    ...
                    () -> QueryPerformanceRecorder.recordPoolAllocation(() -> WritableByteChunk.makeWritableChunkForPool(chunkCapacity)),
                    (final WritableByteChunk chunk) -> chunk.setSize(chunkCapacity)
            );
        }
        resettableByteChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableByteChunk::makeResettableChunkForPool),
                ResettableByteChunk::clear
        );
        resettableWritableByteChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableWritableByteChunk::makeResettableChunkForPool),
