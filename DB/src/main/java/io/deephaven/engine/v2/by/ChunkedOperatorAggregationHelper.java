...
import io.deephaven.engine.v2.sort.timsort.IntIntTimsortKernel;
...
public class ChunkedOperatorAggregationHelper {
    ...
    private static QueryTable aggregation(AggregationControl control, SwapListener swapListener,
        ...
        final TrackingMutableRowSet resultRowSet =
                RowSetFactoryImpl.INSTANCE.flat(outputPosition.intValue()).convertToTracking();
        if (withView.isRefreshing()) {
            ...
            final Listener listener =
                    new BaseTable.ListenerImpl("by(" + aggregationContextFactory + ")", withView, result) {
                        ...
                        public void onUpdate(@NotNull final Update upstream) {
                            ...
                            result.getRowSet().asMutable().update(downstream.added, downstream.removed);
    ...
    private static Update adjustForStreaming(@NotNull final Update upstream) {
        ...
        return new Update(upstream.added, RowSetFactoryImpl.INSTANCE.empty(), upstream.modified, upstream.shifted,
    ...
    private static class KeyedUpdateContext implements SafeCloseable {
        ...
        private KeyedUpdateContext(@NotNull final AggregationContext ac,
            ...
            emptiedStatesBuilder = RowSetFactoryImpl.INSTANCE.builderRandom();
            modifiedStatesBuilder = RowSetFactoryImpl.INSTANCE.builderRandom();
            reincarnatedStatesBuilder = RowSetFactoryImpl.INSTANCE.builderRandom();
        ...
        private Update computeDownstreamIndicesAndCopyKeys(
            ...
            try (final RowSet newStates = makeNewStatesIndex(previousLastState, outputPosition.intValue() - 1)) {
                ...
                try (final RowSet addedBack = downstream.added.intersect(downstream.removed)) {
                    downstream.added.asMutable().remove(addedBack);
                    downstream.removed.asMutable().remove(addedBack);

                    if (newStates.isNonempty()) {
                        downstream.added.asMutable().insert(newStates);
                    ...
                    downstream.modified.asMutable().remove(downstream.removed);
                    if (ac.addedBackModified()) {
                        downstream.modified.asMutable().insert(addedBack);
                    }
                }
        ...
        private void doProcessShiftBucketed(@NotNull final WritableLongChunk<OrderedRowKeys> preKeyIndices,
            ...
            try (final RowSequence preShiftChunkKeys =
                    RowSequenceUtil.wrapRowKeysChunkAsRowSequence(WritableLongChunk.downcast(preKeyIndices));
                    final RowSequence postShiftChunkKeys =
                            RowSequenceUtil.wrapRowKeysChunkAsRowSequence(WritableLongChunk.downcast(postKeyIndices))) {
                sharedContext.reset();
        ...
        private static class ModifySplitResult implements SafeCloseable {
            ...
            /**
             * This is a partition of same-slot modifies for rowSet keys that were not shifted. Needed for modifyChunk of
        ...
        private ModifySplitResult splitKeyModificationsAndDoKeyChangeRemoves() {
            ...
            final RowSetBuilderSequential unshiftedSameSlotIndicesBuilder =
                    needUnshiftedSameSlotIndices ? RowSetFactoryImpl.INSTANCE.builderSequential() : null;
            final RowSetBuilderSequential sameSlotIndicesPreShiftBuilder =
                    needSameSlotIndicesPreShift ? RowSetFactoryImpl.INSTANCE.builderSequential() : null;
            final RowSetBuilderSequential sameSlotIndicesPostShiftBuilder =
                    needSameSlotIndicesPostShift ? RowSetFactoryImpl.INSTANCE.builderSequential() : null;
            final RowSetBuilderSequential keyChangeIndicesPostShiftBuilder =
                    RowSetFactoryImpl.INSTANCE.builderSequential();
            ...
            try (final RowSequence.Iterator modifiedPreShiftIterator =
                ...
                while (modifiedPreShiftIterator.hasMore()) {
                    ...
                    try (final RowSequence keyIndicesToRemoveChunk =
                            RowSequenceUtil.wrapRowKeysChunkAsRowSequence(removedKeyIndices)) {
                        propagateRemovesToOperators(keyIndicesToRemoveChunk, slots);
                    }
                }
            }
    ...
    private static RowSet extractUnshiftedModifiesFromUpstream(@NotNull final Update upstream) {
        final RowSetBuilderSequential unshiftedModifiesBuilder = RowSetFactoryImpl.INSTANCE.builderSequential();
    ...
    private static QueryTable staticGroupedAggregation(QueryTable withView, String keyName, ColumnSource<?> keySource,
        ...
        final QueryTable result = new QueryTable(RowSetFactoryImpl.INSTANCE.flat(responsiveGroups).convertToTracking(),
    ...
    private static void initialGroupedKeyAddition(QueryTable withView,
        ...
        try (final SafeCloseable bc =
                ...
                final RowSequence rs = RowSequenceUtil.forRange(0, responsiveGroups - 1);
    ...
    private static RowSet makeNewStatesIndex(final int first, final int last) {
        return first > last ? RowSetFactoryImpl.INSTANCE.empty()
                : RowSetFactoryImpl.INSTANCE.fromRange(first, last);
    }
    ...
    private static QueryTable noKeyAggregation(SwapListener swapListener,
        ...
        final QueryTable result = new QueryTable(RowSetFactoryImpl.INSTANCE.flat(initialResultSize).convertToTracking(),
                resultColumnSourceMap);
        ac.propagateInitialStateToOperators(result);
                        ...
                        private void processNoKeyUpdate(@NotNull final Update upstream) {
                            ...
                            try (final SafeCloseable ignored = new SafeCloseableArray<>(opContexts)) {
                                ...
                                if ((lastSize == 0 && newResultSize == 1)) {
                                    downstream.added = RowSetFactoryImpl.INSTANCE.fromKeys(0);
                                    downstream.removed = RowSetFactoryImpl.INSTANCE.empty();
                                    downstream.modified = RowSetFactoryImpl.INSTANCE.empty();
                                    result.getRowSet().asMutable().insert(0);
                                } else if (lastSize == 1 && newResultSize == 0) {
                                    downstream.added = RowSetFactoryImpl.INSTANCE.empty();
                                    downstream.removed = RowSetFactoryImpl.INSTANCE.fromKeys(0);
                                    downstream.modified = RowSetFactoryImpl.INSTANCE.empty();
                                    result.getRowSet().asMutable().remove(0);
                                } else {
                                    ...
                                    downstream.added = RowSetFactoryImpl.INSTANCE.empty();
                                    downstream.removed = RowSetFactoryImpl.INSTANCE.empty();
                                    downstream.modified = RowSetFactoryImpl.INSTANCE.fromKeys(0);
                                }
    ...
    private static void doProcessShiftNoKey(AggregationContext ac,
        ...
        try (final RowSequence preChunkOk = RowSequenceUtil.wrapRowKeysChunkAsRowSequence(preKeyIndices);
                final RowSequence postChunkOk = RowSequenceUtil.wrapRowKeysChunkAsRowSequence(postKeyIndices)) {
            sharedContext.reset();
            postSharedContext.reset();
            Arrays.fill(workingPreChunks, null);
            Arrays.fill(workingPostChunks, null);
