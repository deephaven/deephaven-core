...
import io.deephaven.engine.v2.sort.timsort.FloatLongTimsortKernel;
...
//    }
    ...
    public static PartitionKernelContext createContext(TrackingMutableRowSet rowSet, ColumnSource<Float> columnSource, int chunkSize, int nPartitions, boolean preserveEquality) {
    ...
    private static void samplePivots(TrackingMutableRowSet rowSet, int nPartitions, WritableLongChunk<RowKeys> pivotKeys, WritableFloatChunk<Any> pivotValues, ColumnSource<Float> columnSource) {
        pivotKeys.setSize(0);
    ...
    private static int binarySearchTieIndex(FloatChunk pivotValues, LongChunk<RowKeys> pivotKeys, int lo, int hi, float searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final float compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
