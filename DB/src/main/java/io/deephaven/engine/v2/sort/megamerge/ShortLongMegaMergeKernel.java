...
public class ShortLongMegaMergeKernel {
    private ShortLongMegaMergeKernel() {
        throw new UnsupportedOperationException();
    }

    // region Context
    public static class ShortLongMegaMergeKernelContext<ATTR extends Any, KEY_INDICES extends Keys> implements LongMegaMergeKernel<ATTR, KEY_INDICES> {
        ...
        public void merge(LongArraySource indexDestinationSource, ArrayBackedColumnSource<?> valuesDestinationSource, long destinationOffset, long destinationSize, LongChunk<KEY_INDICES> indexKeys, Chunk<ATTR> valuesToMerge) {
            ShortLongMegaMergeKernel.merge(indexDestinationSource, (ShortArraySource)valuesDestinationSource, destinationOffset, destinationSize, indexKeys, valuesToMerge.asShortChunk());
        }
    }
    // endregion Context

    public static <ATTR extends Any, KEY_INDICES extends Keys> ShortLongMegaMergeKernelContext<ATTR, KEY_INDICES> createContext() {
        //noinspection unchecked
        return ShortLongMegaMergeKernelContext.INSTANCE;
    }

    static public <ATTR extends Any, KEY_INDICES extends Keys> void merge(LongArraySource destinationKeys,
                                                                           ShortArraySource destinationValues,
                                                                           long destinationOffset,
                                                                           long destinationSize,
                                                                           LongChunk<KEY_INDICES> keysChunk,
                                                                           ShortChunk<ATTR> valuesChunk
    ) {
        ...
        final short run2lo = valuesChunk.get(0);
        ...
        short val1 = destinationValues.getUnsafe(destCursor);
        short val2 = valuesChunk.get(chunkCursor);
    ...
    private static <ATTR extends Any, KEY_INDICES extends Keys> void copyChunkToDest(LongChunk<KEY_INDICES> keysChunk, ShortChunk<ATTR> valuesChunk, LongArraySource destinationKeys, ShortArraySource destinationValues, int sourceStart, long destStart, int length) {
    ...
    private static void moveInDest(LongArraySource destinationKeys, ShortArraySource destinationValues, long sourceStart, long destStart, long length) {
    ...
    private static long upperBound(ShortArraySource values, long lo, long hi, short searchValue) {
        return bound(values, lo, hi, searchValue, false);
    }

    private static long bound(ShortArraySource valuesToSort, long lo, long hi, short searchValue, @SuppressWarnings("SameParameterValue") final boolean lower) {
        final int compareLimit = lower ? -1 : 0;  // lt or leq

        while (lo < hi) {
            final long mid = (lo + hi) >>> 1;
            final short testValue = valuesToSort.getUnsafe(mid);
    ...
    private static int lowerBound(ShortChunk<?> valuesToSort, @SuppressWarnings("SameParameterValue") int lo, int hi, short searchValue) {
        return bound(valuesToSort, lo, hi, searchValue, true);
    }

    private static int bound(ShortChunk<?> valuesToSort, int lo, int hi, short searchValue, @SuppressWarnings("SameParameterValue") final boolean lower) {
        final int compareLimit = lower ? -1 : 0;  // lt or leq
