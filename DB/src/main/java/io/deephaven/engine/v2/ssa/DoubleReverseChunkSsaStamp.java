...
public class DoubleReverseChunkSsaStamp implements ChunkSsaStamp {
    ...
    private void applyShift(DoubleChunk<Values> leftStampValues, LongChunk<Attributes.RowKeys> leftStampKeys, DoubleChunk<? extends Values> rightStampChunk, LongChunk<Attributes.RowKeys> rightStampKeys, long shiftDelta, MutableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final double rightStampValue = rightStampChunk.get(ii);
