...
public class DoubleReverseChunkSsaStamp implements ChunkSsaStamp {
    ...
    private static int findModified(int leftLowIdx, DoubleChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys, RedirectionIndex redirectionIndex, DoubleChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightStampIndices, RowSetBuilderRandom modifiedBuilder, boolean disallowExactMatch) {
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final double rightStampValue = rightStampChunk.get(ii);
    ...
    private void applyShift(DoubleChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys, DoubleChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightStampKeys, long shiftDelta, RedirectionIndex redirectionIndex, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final double rightStampValue = rightStampChunk.get(ii);
