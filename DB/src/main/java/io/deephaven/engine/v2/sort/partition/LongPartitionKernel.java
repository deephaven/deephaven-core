...
//    }
    ...
    public static PartitionKernelContext createContext(TrackingMutableRowSet rowSet, ColumnSource<Long> columnSource, int chunkSize, int nPartitions, boolean preserveEquality) {
    ...
    private static void samplePivots(TrackingMutableRowSet rowSet, int nPartitions, WritableLongChunk<RowKeys> pivotKeys, WritableLongChunk<Any> pivotValues, ColumnSource<Long> columnSource) {
        pivotKeys.setSize(0);
    ...
    private static int binarySearchTieIndex(LongChunk pivotValues, LongChunk<RowKeys> pivotKeys, int lo, int hi, long searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final long compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
