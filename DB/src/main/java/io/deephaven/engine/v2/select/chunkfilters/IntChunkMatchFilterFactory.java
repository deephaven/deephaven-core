...
public class IntChunkMatchFilterFactory {
    ...
    private static class SingleValueIntChunkFilter implements ChunkFilter.IntChunkFilter {
        ...
        @Override
        public void filter(IntChunk<? extends Values> values, LongChunk<Attributes.OrderedRowKeys> keys, WritableLongChunk<Attributes.OrderedRowKeys> results) {
            results.setSize(0);
    ...
    private static class InverseSingleValueIntChunkFilter implements ChunkFilter.IntChunkFilter {
        ...
        @Override
        public void filter(IntChunk<? extends Values> values, LongChunk<Attributes.OrderedRowKeys> keys, WritableLongChunk<OrderedRowKeys> results) {
            results.setSize(0);
            for (int ii = 0; ii < values.size(); ++ii) {
                if (values.get(ii) != value) {
                    results.add(keys.get(ii));
