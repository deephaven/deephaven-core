 ...
 * ------------------------------------------------------------------------------------------------------------------ */
package io.deephaven.engine.structures.chunk.util.pools;
...
import static io.deephaven.engine.structures.chunk.util.pools.ChunkPoolConstants.*;
...
public final class BooleanChunkPool implements ChunkPool {
    ...
    private final WritableBooleanChunk<Any> EMPTY = WritableBooleanChunk.writableChunkWrap(ArrayUtils.EMPTY_BOOLEAN_ARRAY);
    ...
    BooleanChunkPool() {
        ...
        for (int pcci = 0; pcci < NUM_POOLED_CHUNK_CAPACITIES; ++pcci) {
            ...
            writableBooleanChunks[pcci] = new SegmentedSoftPool<>(
                    ...
                    () -> QueryPerformanceRecorder.recordPoolAllocation(() -> WritableBooleanChunk.makeWritableChunkForPool(chunkCapacity)),
                    (final WritableBooleanChunk chunk) -> chunk.setSize(chunkCapacity)
            );
        }
        resettableBooleanChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableBooleanChunk::makeResettableChunkForPool),
                ResettableBooleanChunk::clear
        );
        resettableWritableBooleanChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableWritableBooleanChunk::makeResettableChunkForPool),
