...
public final class IntChunkPool implements ChunkPool {
    ...
    private final WritableIntChunk<Any> EMPTY = WritableIntChunk.writableChunkWrap(ArrayUtils.EMPTY_INT_ARRAY);
    ...
    IntChunkPool() {
        ...
        for (int pcci = 0; pcci < NUM_POOLED_CHUNK_CAPACITIES; ++pcci) {
            ...
            writableIntChunks[pcci] = new SegmentedSoftPool<>(
                    ...
                    () -> QueryPerformanceRecorder.recordPoolAllocation(() -> WritableIntChunk.makeWritableChunkForPool(chunkCapacity)),
                    (final WritableIntChunk chunk) -> chunk.setSize(chunkCapacity)
            );
        }
        resettableIntChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableIntChunk::makeResettableChunkForPool),
                ResettableIntChunk::clear
        );
        resettableWritableIntChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableWritableIntChunk::makeResettableChunkForPool),
