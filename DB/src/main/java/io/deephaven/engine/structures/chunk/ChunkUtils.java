...
public class ChunkUtils {
    ...
    /**
     * Copy data from {@code src} to {@code dest}, which must be of sufficient capacity to accept all row keys in
     * {@code destAllKeys}.
     *
    ...
    public static void copyData(ChunkSource.WithPrev<? extends Attributes.Values> src, OrderedKeys srcAllKeys,
            ChunkSink<? super Attributes.Values> dest, OrderedKeys destAllKeys, boolean usePrev) {
        if (src == dest) {
    ...
    /**
     * Copy data from sources to destinations for the provided source and destination keys. All destinations must be of
     * sufficient capacity to accept all row keys in {@code destAllKeys}.
     *
    ...
    public static void copyData(ChunkSource.WithPrev<? extends Attributes.Values>[] sources, OrderedKeys srcAllKeys,
            ChunkSink<? super Attributes.Values>[] destinations, OrderedKeys destAllKeys, boolean usePrev) {
        if (srcAllKeys.size() != destAllKeys.size()) {
            final String msg = String.format("Expected srcAllKeys.size() == destAllKeys.size(), but got %d and %d",
                    srcAllKeys.size(), destAllKeys.size());
            throw new IllegalArgumentException(msg);
        ...
        try (final SharedContext sharedContext = SharedContext.makeSharedContext();
            ...
            for (int ss = 0; ss < sources.length; ++ss) {
                for (ChunkSink<?> destination : destinations) {
    ...
    public static <T extends Attributes.Values> void fillWithNullValue(ChunkSink<T> dest, OrderedKeys allKeys) {
