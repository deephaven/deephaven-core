...
public class DateTimeConverter extends JFrame implements ActionListener, FocusListener {
    ...
    public DateTimeConverter() {
        ...
        output = new JTextField[TimeZone.values().length];
        timezones = new JComboBox[TimeZone.values().length];

        for (int i = 0; i < TimeZone.values().length; i++) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e != null && e.getSource() == currentTimeButton) {
            input.setText(String.valueOf(DateTimeUtils.millisToTime(System.currentTimeMillis())));
            input.setSelectionStart(0);
            input.setSelectionEnd(input.getText().length());
        }
        ...
        for (int i = 0; i < output.length; i++) {
            try {
                if (time == null) {
                    ...
                    output[i].setText(time.toString(TimeZone.values()[timezones[i].getSelectedIndex()]));
                }
            } catch (Exception ex) {
                // who cares
            }
    ...
    private DateTime getTime() {
        ...
        if (s.matches("[0-9]+[ ]*ms")) {
            ...
            return DateTimeUtils.millisToTime(Long.parseLong(s));
        }

        if (s.matches("[0-9]+[ ]*ns")) {
            s = s.substring(0, s.indexOf("n")).trim();

            return DateTimeUtils.nanosToTime(Long.parseLong(s));
        }

        try {
            return DateTimeUtils.convertDateTime(getText());
