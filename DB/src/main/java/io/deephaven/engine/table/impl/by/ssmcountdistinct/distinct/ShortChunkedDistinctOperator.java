...
import io.deephaven.engine.table.impl.utils.compact.ShortCompactKernel;
 ...
 */
public class ShortChunkedDistinctOperator implements IterativeChunkedAggregationOperator {
    private final String name;

    private final ShortSsmBackedSource internalResult;
    private final ColumnSource<?> externalResult;
    private final Supplier<SegmentedSortedMultiSet.RemoveContext> removeContextFactory;
    private final boolean countNull;
    private final boolean exposeInternal;
    private WritableRowSet touchedStates;
    private UpdateCommitter<ShortChunkedDistinctOperator> prevFlusher = null;

    public ShortChunkedDistinctOperator(
            // region Constructor
        ...
        // region SsmCreation
        this.internalResult = new ShortSsmBackedSource();
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues, Chunk<? extends Values> postValues, LongChunk<? extends RowKeys> postShiftIndices, IntChunk<Attributes.RowKeys> destinations, IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
        ...
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            if (runLength == 0) {
                final ShortSegmentedSortedMultiset ssm = internalResult.getCurrentSsm(destination);
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues, Chunk<? extends Values> postValues, LongChunk<? extends RowKeys> postShiftIndices, long destination) {
        ...
        ShortSegmentedSortedMultiset ssm = internalResult.getCurrentSsm(destination);
        if (context.valueCopy.size() > 0) {
            if (ssm == null) {
    ...
    private static void flushPrevious(ShortChunkedDistinctOperator op) {
    ...
    public void startTrackingPrevValues() {
        ...
        if(exposeInternal) {
            ...
            prevFlusher = new UpdateCommitter<>(this, ShortChunkedDistinctOperator::flushPrevious);
