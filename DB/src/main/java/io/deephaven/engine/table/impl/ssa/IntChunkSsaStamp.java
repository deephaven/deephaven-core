...
public class IntChunkSsaStamp implements ChunkSsaStamp {
    ...
    private void applyShift(IntChunk<Values> leftStampValues, LongChunk<Attributes.RowKeys> leftStampKeys, IntChunk<? extends Values> rightStampChunk, LongChunk<Attributes.RowKeys> rightStampKeys, long shiftDelta, WritableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final int rightStampValue = rightStampChunk.get(ii);
