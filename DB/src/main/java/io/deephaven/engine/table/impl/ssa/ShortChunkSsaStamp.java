...
public class ShortChunkSsaStamp implements ChunkSsaStamp {
    ...
    private static void processEntry(ShortChunk<Values> leftStampValues, Chunk<Attributes.RowKeys> leftStampKeys, ShortSegmentedSortedArray ssa, WritableLongChunk<Attributes.RowKeys> rightKeysForLeft, boolean disallowExactMatch) {
    ...
    private static void processInsertion(ShortChunk<Values> leftStampValues, LongChunk<Attributes.RowKeys> leftStampKeys, ShortChunk<? extends Values> rightStampChunk, LongChunk<Attributes.RowKeys> rightKeys, ShortChunk<Values> nextRightValue, WritableRowRedirection rowRedirection, RowSetBuilderRandom modifiedBuilder, boolean endsWithLastValue, boolean disallowExactMatch) {
    ...
    private void applyShift(ShortChunk<Values> leftStampValues, LongChunk<Attributes.RowKeys> leftStampKeys, ShortChunk<? extends Values> rightStampChunk, LongChunk<Attributes.RowKeys> rightStampKeys, long shiftDelta, WritableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final short rightStampValue = rightStampChunk.get(ii);
