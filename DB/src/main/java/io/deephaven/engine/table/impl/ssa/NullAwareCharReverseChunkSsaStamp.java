 ...
 */
public class NullAwareCharReverseChunkSsaStamp implements ChunkSsaStamp {
    static NullAwareCharReverseChunkSsaStamp INSTANCE = new NullAwareCharReverseChunkSsaStamp();

    private NullAwareCharReverseChunkSsaStamp() {} // use the instance
    ...
    public void processEntry(Chunk<Values> leftStampValues, Chunk<Attributes.RowKeys> leftStampKeys, SegmentedSortedArray ssa, WritableLongChunk<Attributes.RowKeys> rightKeysForLeft, boolean disallowExactMatch) {
        processEntry(leftStampValues.asCharChunk(), leftStampKeys, (NullAwareCharReverseSegmentedSortedArray)ssa, rightKeysForLeft, disallowExactMatch);
    }

    private static void processEntry(CharChunk<Values> leftStampValues, Chunk<Attributes.RowKeys> leftStampKeys, NullAwareCharReverseSegmentedSortedArray ssa, WritableLongChunk<Attributes.RowKeys> rightKeysForLeft, boolean disallowExactMatch) {
        ...
        final NullAwareCharReverseSegmentedSortedArray.Iterator ssaIt = ssa.iterator(disallowExactMatch, true);
    ...
    private void applyShift(CharChunk<Values> leftStampValues, LongChunk<Attributes.RowKeys> leftStampKeys, CharChunk<? extends Values> rightStampChunk, LongChunk<Attributes.RowKeys> rightStampKeys, long shiftDelta, WritableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final char rightStampValue = rightStampChunk.get(ii);
