 ...
 *
 * Overrides many default {@link Calendar} and BusinessCalendar
...
public class DefaultBusinessCalendar extends AbstractBusinessCalendar implements Serializable {
    ...
    private DefaultBusinessCalendar(final String calendarName, final DBTimeZone timeZone, final long lengthOfDefaultDayNanos, final List<String> defaultBusinessPeriodStrings, final Set<DayOfWeek> weekendDays, final Map<LocalDate, BusinessSchedule> dates, final Map<LocalDate, BusinessSchedule> holidays) {
    ...
    static CalendarElements constructCalendarElements(@NotNull final File calendarFile) {
        ...
        final Pair<Map<LocalDate, BusinessSchedule>, Map<LocalDate, BusinessSchedule>> datePair = getDates(root, calendarElements);
    ...
    private static Pair<Map<LocalDate, BusinessSchedule>, Map<LocalDate, BusinessSchedule>> getDates(final Element root, final CalendarElements calendarElements) {
        ...
        for (Element holidayElement : holidayElements) {
            ...
            if (date != null) {
                ...
                if (businessPeriodStrings.isEmpty()) {
                    ...
                    holidays.put(localDate, new BusinessSchedule(parseBusinessPeriods(calendarElements.timeZone, localDate, businessPeriodStrings)));
                }
            }
        }
        ...
        if(minYear != Integer.MAX_VALUE && maxYear != Integer.MIN_VALUE) {
            for (int i = 0; i < maxYear - minYear; i++) {
                ...
                for (int j = 0; j < numDays; j++) {
                    dates.computeIfAbsent(LocalDate.ofYearDay(year, j + 1), date -> newBusinessDay(date, calendarElements.weekendDays, calendarElements.timeZone, calendarElements.defaultBusinessPeriodStrings));
                }
            }
        }
    ...
    private static List<String> getDefaultBusinessPeriodStrings(@NotNull final Element defaultElement, final String filePath) {
        ...
        if (businessPeriods != null) {
            ...
            throw new IllegalArgumentException("Missing the 'businessPeriod' tag in the 'default' section in calendar file " + filePath);
        }
    ...
    private static Element getRootElement(File calendarFile) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Could not initialize business calendar: Error parsing " + calendarFile.getName());
        } catch (IOException e) {
            throw new RuntimeException("Could not initialize business calendar: " + calendarFile.getName() + " could not be loaded");
        }
    ...
    private static LocalDate parseLocalDate(final String date) {
        try {
            ...
            try {
                ...
                throw new IllegalArgumentException("Malformed date string. Acceptable formats are yyyy-MM-dd and yyyyMMdd. s=" + date);
            }
        }
    }
        ...
        for (String businessPeriodString : defaultBusinessPeriodStrings) {
            ...
            if (openClose.length == 2) {
                ...
                if (hhmm.matcher(open).matches() && hhmm.matcher(close).matches()) {
                    ...
                    long defOpenTimeNanos = (Integer.parseInt(openingTimeHHMM[0]) * DBTimeUtils.HOUR) + (Integer.parseInt(openingTimeHHMM[1]) * DBTimeUtils.MINUTE);
                    long defClosingTimeNanos = (Integer.parseInt(closingTimeHHMM[0]) * DBTimeUtils.HOUR) + (Integer.parseInt(closingTimeHHMM[1]) * DBTimeUtils.MINUTE);
    ...
    private static BusinessPeriod[] parseBusinessPeriods(final DBTimeZone timeZone, final LocalDate date, final List<String> businessPeriodStrings) {
            ...
            if(openClose.length == 2) {
                ...
                if(hhmm.matcher(open).matches() && hhmm.matcher(close).matches()) {
                    ...
                    if(close.equals("24:00")) { //midnight closing time
                        ...
                        close = "00:00";
                    } else if(Integer.parseInt(open.replaceAll(":", "")) > Integer.parseInt(close.replaceAll(":", ""))) {
                        throw new IllegalArgumentException("Can not parse business periods; open = " + open + " is greater than close = " + close);
                    } else {
                        closeDate = date;
                    }
    ...
    @Override
    public long standardBusinessDayLengthNanos() { return lengthOfDefaultDayNanos; }
    ...
    private static BusinessSchedule newBusinessDay(final LocalDate date, final Set<DayOfWeek> weekendDays, final DBTimeZone timeZone, final List<String> businessPeriodStrings){
    ...
    @Override
    public int hashCode() {
        return Objects.hash(calendarName, timeZone, lengthOfDefaultDayNanos, defaultBusinessPeriodStrings, weekendDays, holidays);
    }
    ...
    private static class Holiday extends BusinessSchedule implements Serializable {
        ...
        @Override
        public boolean isBusinessDay() { return false; }
    }
}
