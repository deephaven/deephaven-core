...
public class TableAssertions {
     ...
     * This can be used to ensure the safety and stability of stateful operations.
     *
     * @param description An optional description which will be included in the exception message if the assertion
     *                    is violated.
     * @param table       The table to apply the assertion to
     * @return The provided {@code table}.
     */
    public static Table assertAppendOnly(String description, @NotNull Table table) {
        ...
        return QueryPerformanceRecorder.withNuggetThrowing(
                ...
                () -> {
                    ...
                    final DynamicTable result = new QueryTable(dynamicTable.getDefinition(), dynamicTable.getIndex(), dynamicTable.getColumnSourceMap());
                    final ShiftAwareListener listener = new AppendOnlyAssertionInstrumentedListenerAdapter(description, dynamicTable, result);
     ...
     * @return The provided {@code table}.
     */
    public static Table assertSorted(String description, @NotNull Table table, @NotNull final String column, SortingOrder order) {
        //noinspection ConstantConditions
        if (table == null) {
            throw new IllegalArgumentException("The table cannot be null!");
        }
        ...
        SortedAssertionInstrumentedListenerAdapter.doCheckStatic(table.getIndex(), columnSource, SortCheck.make(columnSource.getChunkType(), order.isDescending()), description, column, order);
        ...
        return QueryPerformanceRecorder.withNuggetThrowing(
                ...
                () -> {
                    final DynamicTable result = new QueryTable(dynamicTable.getIndex(), dynamicTable.getColumnSourceMap());
                    final ShiftAwareListener listener = new SortedAssertionInstrumentedListenerAdapter(description, dynamicTable, result, column, order);
                    dynamicTable.listenForUpdates(listener);
    ...
    assertIsNormalDecimal(Table, String... cols)
    assertPositive(Table, String... cols)
    assertNegative(Table, String... cols)
    assertNull(Table, String... cols)
    assertNotNull(Table, String... cols)
    assertFormula(Table, String... formulas)

    assertColumnEqual(Table, String col1, String col2)
    assertColumnGreater(Table, String col1, String col2)
    assertColumnLess(Table, String col1, String col2)
    assertColumnGeq(Table, String col1, String col2)
    ...
    assertColumnLess(Table, double tolerance, String col1, String col2)
    assertColumnGeq(Table, double tolerance, String col1, String col2)
    assertColumnLeq(Table, double tolerance, String col1, String col2)
