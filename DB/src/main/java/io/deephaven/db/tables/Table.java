...
public interface Table extends LongSizedDataStructure, LivenessNode, TableOperations<Table, Table> {
     ...
     * {@code columnNames}.
     *
     * @param columnNames The array of column names to be checked for inclusion in this table. Must
     *        not be {@code null}.
     * @return {@code true} if this Table contains a column for each and every string in the
     *         {@code columnNames} array; {@code false} if any element of {@code columnNames} is
     *         <b>not</b> the name of a column in this table
     */
    @AsyncMethod
    ...
    /**
     * Determines whether this Table contains a column for each string in the specified collection
     * of {@code columnNames}.
     *
     * @param columnNames The collection of column names to be checked for inclusion in this table.
     *        Must not be {@code null}.
     * @return {@code true} if this Table contains a column for each and every string in the
     *         {@code columnNames} collection; {@code false} if any element of {@code columnNames}
     *         is <b>not</b> the name of a column in this table
     */
    @AsyncMethod
     ...
     * <p>
     * A stream table is a sequence of additions that represent rows newly received from a stream;
     * on the cycle after the stream table is refreshed the rows are removed. Note that this means
     * any particular row of data (not to be confused with an index key) never exists for more than
     * one cycle.
     * <p>
     * Most operations are supported as normal on stream tables, but aggregation operations are
     * treated specially, producing aggregate results that are valid over the entire observed stream
     * from the time the operation is initiated. These semantics necessitate a few exclusions, i.e.
     * unsupported operations:
     * <ol>
     * <li>{@link #by(SelectColumn...) by()} as an index-aggregation is unsupported. This means any
     * of the overloads for {@link #by(AggregationStateFactory, SelectColumn...)} or
     * {@link #by(Collection, Collection)} using {@link AggregationIndexStateFactory},
     * {@link AggregationFormulaStateFactory}, or {@link Array}.
     * {@link io.deephaven.db.v2.by.ComboAggregateFactory#AggArray(java.lang.String...)}, and
     * {@link ComboAggregateFactory#AggFormula(java.lang.String, java.lang.String, java.lang.String...)}
     * are also unsupported.
     * <li>{@link #byExternal(boolean, String...) byExternal()} is unsupported</li>
     * <li>{@link #rollup(ComboAggregateFactory, boolean, SelectColumn...) rollup()} is unsupported
     * if {@code includeConstituents == true}</li>
    ...
    /**
     * The query engine may set or read this attribute to determine if a table is sorted by a
     * particular column.
     */
    ...
    /**
     * This attribute is used internally by TableTools.merge to detect successive merges. Its
     * presence indicates that it is safe to decompose the table into its multiple constituent
     * parts.
     */
     ...
     * <ul>
     * <li>True for post-{@link #coalesce()} source tables and their children if the source table is
     * empty.</li>
     * <li>False for post-{@link #coalesce()} source tables and their children if the source table
     * is non-empty.</li>
    ...
    /**
     * Explicitly ensure that any work needed to make a table indexable, iterable, or queryable has
     * been done, and return the coalesced child table if appropriate.
     * 
    ...
    /**
     * Retrieves a {@code ColumnSource} and {@link ColumnSource#cast casts} is to to the target
     * class {@code clazz}.
     *
    ...
    default DataColumn[] getColumns() {
        return getDefinition().getColumnStream().map(c -> getColumn(c.getName()))
    ...
    /**
     * A table operation that applies the supplied predicate to each row in the table and produces
     * columns containing the pass/fail result of the predicate application. This is similar to
     * {@link #where(String...)} except that instead of selecting only rows that meet the criteria,
     * new columns are added with the result of the comparison.
     *
    ...
    /**
     * Filters this table based on the set of values in the rightTable. Note that when the right
     * table ticks, all of the rows in the left table are going to be re-evaluated, thus the
     * intention is that the right table is fairly slow moving compared with the left table.
     *
     * @param rightTable the filtering table.
     * @param inclusion whether things included in rightTable should be passed through (they are
     *        exluded if false)
     * @param columnsToMatch the columns to match between the two tables
     * @return a new table filtered on right table
     */
    Table whereIn(GroupStrategy groupStrategy, Table rightTable, boolean inclusion,
    ...
    default Table whereIn(Table rightTable, boolean inclusion, String... columnsToMatch) {
        return whereIn(GroupStrategy.DEFAULT, rightTable, inclusion,
            MatchPairFactory.getExpressions(columnsToMatch));
    }

    default Table whereIn(Table rightTable, String... columnsToMatch) {
        return whereIn(GroupStrategy.DEFAULT, rightTable, true,
            MatchPairFactory.getExpressions(columnsToMatch));
    }
    ...
    default Table whereNotIn(Table rightTable, String... columnsToMatch) {
        return whereIn(GroupStrategy.DEFAULT, rightTable, false,
            MatchPairFactory.getExpressions(columnsToMatch));
    }
    ...
    default Table whereIn(GroupStrategy groupStrategy, Table rightTable,
        MatchPair... columnsToMatch) {
        return whereIn(groupStrategy, rightTable, true, columnsToMatch);
    }

    default Table whereNotIn(GroupStrategy groupStrategy, Table rightTable,
    ...
    default Table whereIn(GroupStrategy groupStrategy, Table rightTable, boolean inclusion,
        String... columnsToMatch) {
        return whereIn(groupStrategy, rightTable, inclusion,
            MatchPairFactory.getExpressions(columnsToMatch));
    }
     ...
     *
     * @param filtersToApply each inner collection is a set of filters, all of must which match for
     *        the clause to be true. If any one of the collections in the array evaluates to true,
     *        the row is part of the output table.
    ...
    default Table whereOneOf(String... filtersToApplyStrings) {
        ...
        final Collection<SelectFilter>[] filtersToApplyArrayOfCollections =
            (Collection<SelectFilter>[]) Arrays
                .stream(SelectFilterFactory.getExpressions(filtersToApplyStrings))
     ...
     *
     * <p>
     * Lazy update defers computation until required for a set of values, and caches the results for
     * a set of input values. This uses less RAM than an update statement when you have a smaller
     * set of unique values. Less computation than an updateView is needed, because the results are
     * saved in a cache.
     * </p>
     *
     * <p>
     * If you have many unique values, you should instead use an update statement, which will have
     * more memory efficient structures. Values are never removed from the lazyUpdate cache, so it
     * should be used judiciously on a ticking table.
     * </p>
     *
    ...
    default Table formatColumns(String... columnFormats) {
        final SelectColumn[] selectColumns =
            SelectColumnFactory.getFormatExpressions(columnFormats);
        ...
        final String[] unknownColumns = Arrays.stream(selectColumns)
            ...
            .filter(column -> (column != null && !column.equals("*")
    ...
    default Table formatColumnWhere(String columnName, String condition, String formula) {
        return formatColumns(
            columnName + " = (" + condition + ") ? io.deephaven.db.util.DBColorUtil.toLong("
                + formula + ") : io.deephaven.db.util.DBColorUtil.toLong(NO_FORMATTING)");
    }

    /**
     * Produce a new table with the specified columns moved to the leftmost position. Columns can be
     * renamed with the usual syntax, i.e. {@code "NewColumnName=OldColumnName")}.
     *
    ...
    /**
     * Produce a new table with the specified columns moved to the rightmost position. Columns can
     * be renamed with the usual syntax, i.e. {@code "NewColumnName=OldColumnName")}.
     *
    ...
    default Table moveDownColumns(String... columnsToMove) {
        return moveColumns(getDefinition().getColumns().length - columnsToMove.length, true,
            columnsToMove);
    }

    /**
     * Produce a new table with the specified columns moved to the specified {@code index}. Column
     * indices begin at 0. Columns can be renamed with the usual syntax, i.e.
     * {@code "NewColumnName=OldColumnName")}.
     *
    ...
    default Table moveColumns(int index, boolean moveToEnd, String... columnsToMove) {
        ...
        for (final String columnToMove : columnsToMove) {
            ...
            if (!leftColsToMove.add(left) || !currentColumns.contains(left)
                || (rightColsToMove.contains(left) && !left.equals(right)
                    && leftColsToMove.stream().anyMatch(col -> col.equals(right)))) {
                extraCols++;
            }
            if (currentColumns.stream().anyMatch(currentColumn -> currentColumn.equals(right))
                && !left.equals(right) && rightColsToMove.add(right)
                && !rightColsToMove.contains(left)) {
                extraCols--;
            }
        }
        ...
        for (int vci = 0, cci = 0, ctmi = 0; vci < viewColumns.length;) {
            if (vci >= index && ctmi < columnsToMove.length) {
                ...
                // Don't add the column if it's one of the columns we're moving or if it has been
                // renamed.
                final String currentColumn = currentColumns.get(cci++);
                if (!leftColsToMove.contains(currentColumn)
                    && Arrays.stream(viewColumns)
                        .noneMatch(viewCol -> viewCol != null
                            && viewCol.getMatchPair().leftColumn.equals(currentColumn))
                    && Arrays.stream(columnsToMove).noneMatch(
                        colToMove -> MatchPairFactory.getExpression(colToMove).rightColumn
    ...
    /**
     * Produce a new table with the same columns as this table, but with a new column presenting the
     * specified DBDateTime column as a Long column (with each DBDateTime represented instead as the
     * corresponding number of nanos since the epoch).
     * <p>
     * NOTE: This is a really just an updateView(), and behaves accordingly for column ordering and
     * (re)placement. This doesn't work on data that has been brought fully into memory (e.g. via
    ...
    default Table dateTimeColumnAsNanos(String dateTimeColumnName, String nanosColumnName) {
        // noinspection unchecked
        return updateView(new ReinterpretedColumn(dateTimeColumnName, DBDateTime.class,
            nanosColumnName, long.class));
    }
     ...
     *
     * If both firstPosition and lastPosition are positive, then the rows are counted from the
     * beginning of the table. The firstPosition is inclusive, and the lastPosition is exclusive.
     * The {@link #head}(N) call is equivalent to slice(0, N). The firstPosition must be less than
     * or equal to the lastPosition.
     *
     * If firstPosition is positive and lastPosition is negative, then the firstRow is counted from
     * the beginning of the table, inclusively. The lastPosition is counted from the end of the
     * table. For example, slice(1, -1) includes all rows but the first and last. If the
     * lastPosition would be before the firstRow, the result is an emptyTable.
     *
     * If firstPosition is negative, and lastPosition is zero, then the firstRow is counted from the
     * end of the table, and the end of the slice is the size of the table. slice(-N, 0) is
     * equivalent to {@link #tail}(N).
     *
     * If the firstPosition is nega tive and the lastPosition is negative, they are both counted
     * from the end of the table. For example, slice(-2, -1) returns the second to last row of the
     * table.
     *
     ...
     *
     * @param percent the fraction of the table to return (0..1), the number of rows will be rounded
     *        up. For example if there are 3 rows, headPct(50) returns the first two rows.
     */
    @AsyncMethod
    ...
    /**
     * GroupStrategy is used for joins and other operations that can choose one of several ways to
     * make use of grouping information.
     */
    enum GroupStrategy {
    ...
    /**
     * Returns a table that has one column for each original table's columns, and one column
     * corresponding to each of the input table (right table) columns listed in the columns to add
     * (or all the columns whose names don't overlap with the name of a column from the source table
     * if the columnsToAdd is length zero). The new columns (those corresponding to the input table)
     * contain an aggregation of all values from the left side that match the join criteria.
     * Consequently the types of all right side columns not involved in a join criteria, is an array
     * of the original column type. If the two tables have columns with matching names then the
     * method will fail with an exception unless the columns with corresponding names are found in
     ...
     *         corresponding to each column listed in columnsToAdd. If columnsToAdd.length==0 one
     *         column corresponding to each column of the input table (right table) columns whose
     *         names don't overlap with the name of a column from the source table is added. The new
     *         columns (those corresponding to the input table) contain an aggregation of all values
     *         from the left side that match the join criteria.
     */
    ...
    /**
     * Looks up the columns in the rightTable that meet the match conditions in the columnsToMatch
     * list. Matching is done exactly for the first n-1 columns and via a binary search for the last
     * match pair. The columns of the original table are returned intact, together with the columns
     ...
     * Looks up the columns in the rightTable that meet the match conditions in the columnsToMatch
     * list. Matching is done exactly for the first n-1 columns and via a binary search for the last
     * match pair. The columns of the original table are returned intact, together with all the
     * columns from rightTable.
     *
     * @param rightTable The right side table on the join.
     * @param columnsToMatch A comma separated list of match conditions ("leftColumn=rightColumn" or
     *        "columnFoundInBoth")
    ...
    default Table aj(Table rightTable, Collection<String> columnsToMatch) {
        Pair<MatchPair[], AsOfMatchRule> expressions =
            AjMatchPairFactory.getExpressions(false, columnsToMatch);
        return aj(
            rightTable,
    ...
    /**
     * Just like .aj(), but the matching on the last column is in reverse order, so that you find
     * the row after the given timestamp instead of the row before.
     * <p>
     * Looks up the columns in the rightTable that meet the match conditions in the columnsToMatch
     * list. Matching is done exactly for the first n-1 columns and via a binary search for the last
     * match pair. The columns of the original table are returned intact, together with the columns
     ...
     * Looks up the columns in the rightTable that meet the match conditions in the columnsToMatch
     * list. Matching is done exactly for the first n-1 columns and via a binary search for the last
     * match pair. The columns of the original table are returned intact, together with the all
     * columns from rightTable.
     *
     * @param rightTable The right side table on the join.
     * @param columnsToMatch A comma separated list of match conditions ("leftColumn=rightColumn" or
    ...
    default Table raj(Table rightTable, Collection<String> columnsToMatch) {
        Pair<MatchPair[], AsOfMatchRule> expressions =
            AjMatchPairFactory.getExpressions(true, columnsToMatch);
        return raj(
            rightTable,
            expressions.getFirst(),
            MatchPair.ZERO_LENGTH_MATCH_PAIR_ARRAY,
     ...
     * each of the left table's columns, and one column corresponding to each of the right table's
     * columns. The rows are ordered first by the left table then by the right table.
     * <p>
     ...
     * <p>
     * An {@link io.deephaven.db.v2.utils.OutOfKeySpaceException} is thrown when the total number of
     * bits needed to express the result table exceeds that needed to represent Long.MAX_VALUE.
     * There are a few work arounds: - If the left table is sparse, consider flattening the left
     * table. - If there are no key-columns and the right table is sparse, consider flattening the
     * right table. - If the maximum size of a right table's group is small, you can reserve fewer
     * bits by setting numRightBitsToReserve on initialization.
     * <p>
     * Note: If you can prove that a given group has at most one right-row then you should prefer
     * using {@link #naturalJoin}.
     *
     * @param rightTable The right side table on the join.
     * @return a new table joined according to the specification with zero key-columns and includes
     *         all right columns
     */
     ...
     * Returns a table that is the cartesian product of left rows X right rows, with one column for
     * each of the left table's columns, and one column corresponding to each of the right table's
     * columns that are not key-columns. The rows are ordered first by the left table then by the
     * right table. If columnsToMatch is non-empty then the product is filtered by the supplied
     * match conditions.
     * <p>
     * To efficiently produce updates, the bits that represent a key for a given row are split into
     * two. Unless specified, join reserves 16 bits to represent a right row. When there are too few
     ...
     * Returns a table that is the cartesian product of left rows X right rows, with one column for
     * each of the left table's columns, and one column corresponding to each of the right table's
     * columns that are included in the columnsToAdd argument. The rows are ordered first by the
     * left table then by the right table. If columnsToMatch is non-empty then the product is
     * filtered by the supplied match conditions.
     * <p>
     * To efficiently produce updates, the bits that represent a key for a given row are split into
     * two. Unless specified, join reserves 16 bits to represent a right row. When there are too few
     * bits to represent all of the right rows for a given aggregation group the table will shift a
     ...
     *
     * @param rightTable The right side table on the join.
     * @param columnsToMatch An array of match pair conditions ("leftColumn=rightColumn" or
     *        "columnFoundInBoth")
     * @param columnsToAdd An array of the columns from the right side that need to be added to the
     *        left side as a result of the match.
     * @return a new table joined according to the specification in columnsToMatch and columnsToAdd
     */
    default Table join(Table rightTable, MatchPair[] columnsToMatch, MatchPair[] columnsToAdd) {
        return join(rightTable, columnsToMatch, columnsToAdd,
    ...
    default Table by(Collection<? extends Selectable> groupByColumns,
        ...
        List<ColumnName> userOrder =
            AggregationOutputs.of(aggregations).collect(Collectors.toList());
    ...
    /**
     * Groups data according to groupByColumns and applies formulaColumn to each of columns not
     * altered by the grouping operation. <code>columnParamName</code> is used as place-holder for
     * the name of each column inside <code>formulaColumn</code>.
     *
     ...
     */
    @AsyncMethod
    Table applyToAllBy(String formulaColumn, String columnParamName,
        SelectColumn... groupByColumns);

    /**
     * Groups data according to groupByColumns and applies formulaColumn to each of columns not
     * altered by the grouping operation.
     *
     * @param formulaColumn Formula applied to each column, uses parameter <i>each</i> to refer to
     *        each colum it being applied to
     * @param groupByColumns The grouping columns {@link Table#by(SelectColumn...)}
     */
    @AsyncMethod
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the sum for the
     * rest of the fields
     *
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the sum of the
     * absolute values for the rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     */
    @AsyncMethod
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the average for
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the weighted
     * average using weightColumn for the rest of the fields
     *
    ...
    /**
     * Produces a single row table with the weighted average using weightColumn for the rest of the
     * fields
     *
     * When the input table is empty, zero output rows are produced.
     *
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the weighted sum
     * using weightColumn for the rest of the fields
     *
     * If the weight column is a floating point type, all result columns will be doubles. If the
     * weight column is an integral type, all integral input columns will have long results and all
     * floating point input columns will have double results.
     *
     * @param weightColumn the column to use for the weight
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     */
    ...
    /**
     * Computes the weighted sum for all rows in the table using weightColumn for the rest of the
     * fields
     *
     * If the weight column is a floating point type, all result columns will be doubles. If the
     * weight column is an integral type, all integral input columns will have long results and all
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the standard
     * deviation for the rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     */
    @AsyncMethod
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the variance for
     * the rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     */
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and retrieves the last for
     * the rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     */
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and retrieves the first for
     * the rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     */
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the min for the
     * rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     */
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the max for the
     * rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     *        }
    ...
    /**
     * Groups the data column according to <code>groupByColumns</code> and computes the median for
     * the rest of the fields
     *
     * @param groupByColumns The grouping columns {@link io.deephaven.db.tables.Table#by(String...)}
     *        }
    ...
    /**
     * If this table is a stream table, i.e. it has {@link #STREAM_TABLE_ATTRIBUTE} set to
     * {@code true}, return a child without the attribute, restoring standard semantics for
     * aggregation operations.
     *
     ...
     *
     * @param nullFill indicates if the ungrouped table should allow disparate sized arrays filling
     *        shorter columns with null values. If set to false, then all arrays should be the same
     *        length.
    ...
    default Table ungroupAllBut(String... columnsNotToUngroup) {
        final Set<String> columnsNotToUnwrapSet =
            Arrays.stream(columnsNotToUngroup).collect(Collectors.toSet());
    ...
    default Table ungroup(boolean nullFill) {
        return ungroup(nullFill, getDefinition().getColumnStream()
            .filter(c -> c.getDataType().isArray() || DBLanguageParser.isDbArray(c.getDataType()))
            .map(ColumnDefinition::getName).toArray(String[]::new));
    }
     ...
     *
     * <p>
     * The returned TableMap contains each row in this table in exactly one of the tables within the
     * map. If you have exactly one key column the TableMap is keyed by the value in that column. If
     * you have zero key columns, then the TableMap is keyed by
     * {@code io.deephaven.datastructures.util.SmartKey.EMPTY} (and will contain this table as the
     * value). If you have multiple key columns, then the TableMap is keyed by a
     * {@code io.deephaven.datastructures.util.SmartKey}. The SmartKey will have one value for each
     * of your column values, in the order specified by keyColumnNames.
     * </p>
     *
     * <p>
     * For example if you have a Table keyed by a String column named USym, and a DBDateTime column
     * named Expiry; a value could be retrieved from the TableMap with
     * {@code tableMap.get(new SmartKey("SPY";, DBTimeUtils.convertDateTime("2020-06-19T16:15:00 NY")))}.
     * For a table with an Integer column named Bucket, you simply use the desired value as in
     * {@code tableMap.get(1)}.
     * </p>
     *
     ...
     *
     * A rollup table aggregates by the specified columns, and then creates a hierarchical table
     * which re-aggregates using one less aggregation column on each level. The column that is no
     * longer part of the aggregation key is replaced with null on each level.
     *
    ...
    default Table rollup(ComboAggregateFactory comboAggregateFactory, boolean includeConstituents,
        ...
        return rollup(comboAggregateFactory, includeConstituents,
            SelectColumnFactory.getExpressions(columns));
    }
     ...
     *
     * The structure of the table is encoded by an "id" and a "parent" column. The id column should
     * represent a unique identifier for a given row, and the parent column indicates which row is
     * the parent for a given row. Rows that have a null parent, are shown in the main table. It is
     * possible for rows to be "orphaned", if their parent reference is non-null and does not exist
     * in the table.
     *
     * @param idColumn the name of a column containing a unique identifier for a particular row in
     *        the table
     * @param parentColumn the name of a column containing the parent's identifier, null for
     *        elements that are part of the root table
     ...
     *
     * <p>
     * Note that this table operates on the table it was invoked on and does not create a new table.
     * So in the following code <code>T1 = baseTable.where(...)
    ...
    @Override
    default Table snapshot(Table baseTable, boolean doInitialSnapshot,
     ...
     * <p>
     * This is useful if you have a reference to a table or a proxy and want to run a series of
     * operations against the table without each individual operation resulting in an RMI.
     *
     * @implNote If the LTM is not required the {@link Function.Unary#call(Object)} method should be
     *           annotated with {@link AsyncMethod}.
     *
    ...
    /**
     * Return true if this table is guaranteed to be flat. The index of a flat table will be from
     * 0...numRows-1.
     */
    @AsyncMethod
    boolean isFlat();
     ...
     *
     * @return The same table this method was invoked on, with the keyColumns and unique attributes
     *         set
     */
    @AsyncMethod
    ...
    /**
     * Add a description for a specific column. You may use {@link #withColumnDescription(Map)} to
     * set several descriptions at once.
     *
    ...
    /**
     * Release resources held by this table, possibly destructively. This may render the table
     * unsuitable or unsafe for further use.
     *
    ...
    /**
     * Attempt to release cached resources held by this table. Unlike {@link #close()}, this must
     * not render the table unusable for subsequent read operations. Implementations should be sure
     * to call {@code super.releaseCachedResources()}.
     */
