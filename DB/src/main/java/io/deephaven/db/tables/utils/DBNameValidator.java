...
public class DBNameValidator {
    private static final Logger log = LoggerFactory.getLogger(DBNameValidator.class);
    ...
    private static final boolean ENABLE_COLUMN_VALIDATION = Configuration.getInstance().getBooleanWithDefault("enableColumnNameValidation", true);
    ...
    private enum ValidationCode {
        ...
        private String getErrorMessage(String name, String type){
            return message.replace("<name>", name == null ? "null" : name).replaceAll("<type>", type);
        }

        private boolean isValid() {return this == OK;}
    }

    //table names should not start with numbers. Partition names should be able to
    private final static Pattern TABLE_NAME_PATTERN = Pattern.compile("([a-zA-Z_$])[a-zA-Z0-9_$[-][+]@]*");
    private final static Pattern PARTITION_NAME_PATTERN = Pattern.compile("([a-zA-Z0-9$])[a-zA-Z0-9_$[-][+]@\\.]*");
    ...
    public enum Type{
        ...
        Type(boolean checkReservedVariableNames, boolean checkValidJavaWord, String type, Pattern pattern) {
        ...
        private String validate(String name) {
            ValidationCode code = getCode(name, pattern, checkReservedVariableNames, checkValidJavaWord);
            if(!code.isValid()) {
    ...
    static {
        ...
        tempSet.add("ii");
        tempSet.add("k");
        DB_RESERVED_VARIABLE_NAMES = Collections.unmodifiableSet(tempSet);
    }
    ...
    public static String validateColumnName(String name) {
        try {
            ...
            if (ENABLE_COLUMN_VALIDATION) {
                throw ine;
            }
            log.warn().append("Invalid column name \"").append(name).append("\": ").append(ine).endl();
            return name;
        }
    }
    ...
    public static class LegalizeNameException extends RuntimeException {
        /**
     ...
     *
     *  To avoid duplicated names, anything in the set {@code takenNames} will not be returned. Theses duplicates
     *  are resolved by adding sequential digits at the end of the variable name.
     *
     *
     *  Column names
    ...
    public static String legalizeColumnName(String name, Function<String, String> customReplace, Set<String> takenNames) {
        return legalizeName(name, customReplace, takenNames, "Can not legalize column name " + name, COLUMN_PREFIX, STERILE_COLUMN_AND_QUERY_REGEX, true, true, DBNameValidator::validateColumnName);
    }
    ...
    public static String[] legalizeColumnNames(String[] names, Function<String, String> customReplace, boolean resolveConflicts) {
        return legalizeNames(names, customReplace, resolveConflicts, DBNameValidator::legalizeColumnName);
    }
    ...
    public static String legalizeQueryParameterName(String name, Function<String, String> customReplace, Set<String> takenNames) {
        return legalizeName(name, customReplace, takenNames, "Can not legalize table name " + name, QUERY_PREFIX, STERILE_COLUMN_AND_QUERY_REGEX, true, true, DBNameValidator::validateQueryParameterName);
    }
    ...
    public static String[] legalizeQueryParameterNames(String[] names, Function<String, String> customReplace, boolean resolveConflicts) {
        return legalizeNames(names, customReplace, resolveConflicts, DBNameValidator::legalizeQueryParameterName);
    }
    ...
    public static String legalizeTableName(String name, Function<String, String> customReplace, Set<String> takenNames) {
        return legalizeName(name, customReplace, takenNames, "Can not legalize table name " + name, TABLE_PREFIX, STERILE_TABLE_AND_NAMESPACE_REGEX, false, true, DBNameValidator::validateTableName);
    }
    ...
    public static boolean isLegalTableName(String name, Function<String, String> customReplace, Set<String> takenNames) {
        return isLegal(name, customReplace, takenNames, STERILE_TABLE_AND_NAMESPACE_REGEX, false, true, DBNameValidator::validateTableName);
    }
    ...
    public static String[] legalizeTableNames(String[] names, Function<String, String> customReplace, boolean resolveConflicts) {
        return legalizeNames(names, customReplace, resolveConflicts, DBNameValidator::legalizeTableName);
    }
    ...
    public static String legalizeNamespaceName(String name, Function<String, String> customReplace, Set<String> takenNames) {
        return legalizeName(name, customReplace, takenNames, "Can not legalize namespace name " + name, null, STERILE_TABLE_AND_NAMESPACE_REGEX, false, false, DBNameValidator::validateNamespaceName);
    }
    ...
    public static boolean isLegalNamespaceName(String name, Function<String, String> customReplace, Set<String> takenNames) {
        return isLegal(name, customReplace, takenNames, STERILE_TABLE_AND_NAMESPACE_REGEX, false, false, DBNameValidator::validateNamespaceName);
    }
    ...
    public static String[] legalizeNamespaceNames(String[] names, Function<String, String> customReplace, boolean resolveConflicts) {
        return legalizeNames(names, customReplace, resolveConflicts, DBNameValidator::legalizeNamespaceName);
    }
    ...
    private static String legalizeName(String name, Function<String, String> customReplace, Set<String> takenNames, String error, String prefix, String regex, boolean checkReserved, boolean checkFirstIsNumber, Consumer<String> validation) {
        ...
        Require.neqNull(customReplace, "customReplace");
        Require.neqNull(takenNames, "takenNames");
    ...
    private static boolean isReserved(String replacedName) {
        return DB_RESERVED_VARIABLE_NAMES.contains(replacedName) || SourceVersion.isKeyword(replacedName);
    }

    private static String[] legalizeNames(final String[] names, final Function<String, String> customReplace, final boolean resolveConflicts, final FunctionalInterfaces.TriFunction<String, Function<String, String>, Set<String>, String> legalizer) {
        //if null, throw an exception
        if(names == null) {
            throw new LegalizeNameException("Can not legalize a null name array");
        }
        Require.neqNull(customReplace, "customReplace");
        Require.neqNull(legalizer, "legalizer");
