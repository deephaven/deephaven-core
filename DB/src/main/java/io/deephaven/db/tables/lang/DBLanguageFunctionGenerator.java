...
public class DBLanguageFunctionGenerator {
    ...
    public static void main(String args[]){
         ...
         * and the other is a primitive, Java's inclination is to unbox the one that's boxed.
         *
         * Since the DB uses {@code Boolean} to store booleans while supporting {@code null}, we must manually
         * box the result of boolean operations if we wish to support nulls.
         *
         * See JLS Chapter 15 section 25 -- https://docs.oracle.com/javase/specs/jls/se8/html/jls-15.html#jls-15.25"
         *
         * Note that we do not provide null-safe handling for the "conditional-and" and "conditional-or"
         * operators ("&&" and "||"). To do so while maintaining their short-circuit behavior would
         * require different parser changes.
         */
        ...
        String[] operatorDescriptions = new String[]{"add", "subtract", "multiply", "divide", "calculate remainder of"};
        ...
        for (Class classA : classes){
            for (Class classB : classes){
                appendTest(testBuf, varVarCompareTestFormatter, classA, classB, getSmallLiteral(classA), getBiggerLiteral(classB));
            }
        }
        ...
        System.out.println("Finished generating DBLanguageFunctionUtil in " + new DecimalFormat().format(System.currentTimeMillis() - start) + " millis");
    ...
    private static void append(StringBuilder buf, MessageFormat messageFormat, BinaryExpr.Operator op, Class type1, Class type2){
        append(buf, messageFormat, op, null, type1, type2);
    }

    private static void append(StringBuilder buf, MessageFormat messageFormat, BinaryExpr.Operator op, String opDescription, Class type1, Class type2){
        append(buf, messageFormat, op, opDescription, type1, type2, null, null);
    }

    private static void appendTest(StringBuilder buf, MessageFormat messageFormat, BinaryExpr.Operator op, Class type1, Class type2){
        append(buf, messageFormat, op, null, type1, type2, getLiteral(type1), getLiteral(type2));
    }

    private static void appendTest(StringBuilder buf, MessageFormat messageFormat, Class type1, Class type2, String literal1, String literal2){
    ...
    /**
     * Returns a String of an bigger example literal value of {@code type}.
     * Must be larger than the value produced by getSmallLiteral (across all types).
     * Used for generating comparison tests.
     */
