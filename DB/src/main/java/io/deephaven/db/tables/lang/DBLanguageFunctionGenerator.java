...
public class DBLanguageFunctionGenerator {
    ...
    public static void main(String args[]) {
        ...
        for (Class classA : classes) {
            for (Class classB : classes) {
                ...
                if (classA.equals(long.class) && classB.equals(double.class)) {
                    append(buf, longDoubleCompareToFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus is
                                                                                                         // just to
                                                                                                         // avoid a npe
                } else if (classA.equals(double.class) && classB.equals(long.class)) {
                    append(buf, inverseCompareToFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus is
                                                                                                      // just to avoid a
                                                                                                      // npe
                    ...
                    append(buf, varDoubleCompareToFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus is
                                                                                                        // just to avoid
                                                                                                        // a npe
                    ...
                    if (classA.isAssignableFrom(float.class) && classB.isAssignableFrom(float.class)) {
                        append(buf, varFloatCompareToFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus
                                                                                                           // is just to
                                                                                                           // avoid a
                                                                                                           // npe
                    } else {
                        append(buf, varDoubleCompareToFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus
                                                                                                            // is just
                                                                                                            // to avoid
                                                                                                            // a npe
                    }
                } else {
                    append(buf, varVarCompareToFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus is just
                                                                                                     // to avoid a npe
                }

                append(buf, varVarEqualsFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus is just to
                                                                                              // avoid a npe
        ...
        for (Class classA : classes) { // Functions for null-safe casts between primitive types
            for (Class classB : classes) {
                if (classA != classB) { // don't create functions for redundant casts (e.g. intCast(int))
                    append(buf, castFormatter, BinaryExpr.Operator.plus, classA, classB); // the plus is just so we
                                                                                          // don't get a npe
                }
            }
        }
        ...
        for (Class c : classes) { // Functions for null-safe casts from Object to primitive types
            append(buf, castFromObjFormatter, BinaryExpr.Operator.plus, c, Object.class); // the plus and Object are
                                                                                          // just so we don't get a npe
        }
        ...
        for (Class clazz : classes) {
            append(buf, negateFormatter, BinaryExpr.Operator.plus, clazz, clazz); // the plus is just so we don't get a
                                                                                  // npe
        }
    ...
    private static void append(StringBuilder buf, MessageFormat messageFormat, BinaryExpr.Operator op, Class type1,
            Class type2) {
    ...
    private static void appendTest(StringBuilder buf, MessageFormat messageFormat, BinaryExpr.Operator op, Class type1,
            Class type2) {
        append(buf, messageFormat, op, null, type1, type2, getLiteral(type1), getLiteral(type2));
    }

    private static void appendTest(StringBuilder buf, MessageFormat messageFormat, Class type1, Class type2,
    ...
    private static void append(StringBuilder buf, MessageFormat messageFormat, BinaryExpr.Operator op,
        ...
        if (op == BinaryExpr.Operator.equals || op == BinaryExpr.Operator.less || op == BinaryExpr.Operator.greater
                || op == BinaryExpr.Operator.lessEquals || op == BinaryExpr.Operator.greaterEquals) {
