...
public class CsvParserFormat {
    ...
    public static String[] getFormatNames() {
        return new String[]{FORMAT_DEFAULT, FORMAT_TRIM, FORMAT_EXCEL, FORMAT_TDF, FORMAT_MYSQL, FORMAT_RFC4180, FORMAT_BPIPE};
    }
     ...
     */
    private static CSVFormat getCsvFormat(@NotNull final String fileFormat, final boolean noHeader, final List<String> columnNames) {
        CSVFormat result;
        switch (fileFormat) {
            case FORMAT_TRIM:
     ...
     * Returns an Apache CSVFormat based on the fileFormat String.
     * @param fileFormat The string for which a format should be created
     * @param delimiter A single character to use as a delimiter - comma when format will be controlled by fileFormat
     ...
     * @throws RuntimeException if fileFormat is unrecognized
     */
    public static CSVFormat getCsvFormat(final String fileFormat, final char delimiter, final boolean trim, final boolean noHeader,@Nullable final List<String> columnNames) {
        // First figure out the CSVFormat
        final CSVFormat csvFormat;
        if (fileFormat == null || delimiter != ',') {
            if (!noHeader) {
                csvFormat = CSVFormat.newFormat(delimiter).withTrim(trim).withFirstRecordAsHeader().withQuote('"');
            } else if (columnNames != null && columnNames.size() > 0) {
                csvFormat = CSVFormat.newFormat(delimiter).withTrim(trim).withHeader(columnNames.toArray(new String[0])).withQuote('"');
            } else {
