...
public class CsvParserFormat {
    ...
    public static String[] getFormatNames() {
        return new String[] {FORMAT_DEFAULT, FORMAT_TRIM, FORMAT_EXCEL, FORMAT_TDF, FORMAT_MYSQL, FORMAT_RFC4180,
                FORMAT_BPIPE};
    }
     ...
     * @throws RuntimeException if fileFormat is unrecognized
     */
    public static CSVFormat getCsvFormat(final String fileFormat, final char delimiter, final boolean trim,
            final boolean noHeader, @Nullable final List<String> columnNames) {
        ...
        if (fileFormat == null || delimiter != ',') {
            if (!noHeader) {
                ...
                csvFormat = CSVFormat.newFormat(delimiter).withTrim(trim).withHeader(columnNames.toArray(new String[0]))
                        .withQuote('"');
            } else {
