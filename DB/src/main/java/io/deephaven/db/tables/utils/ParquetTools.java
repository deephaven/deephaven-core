...
public class ParquetTools {
     ...
     * @param destPath destination file path; the file name should end in ".parquet" extension If
     *        the path includes non-existing directories they are created If there is an error any
     *        intermediate directories previously created are removed; note this makes this method
     *        unsafe for concurrent use
     */
    public static void writeTable(
        @NotNull final Table sourceTable,
        @NotNull final String destPath) {
        writeTable(sourceTable, new File(destPath), sourceTable.getDefinition(),
     ...
     *
     * @param sourceTable source table
     * @param destFile destination file; the file name should end in ".parquet" extension If the
     *        path includes non-existing directories they are created
     */
    public static void writeTable(
        @NotNull final Table sourceTable,
     ...
     *
     * @param sourceTable source table
     * @param destFile destination file; its path must end in ".parquet". Any non existing
     *        directories in the path are created If there is an error any intermediate directories
     ...
     *
     * @param sourceTable source table
     * @param destPath destination path; it must end in ".parquet". Any non existing directories in
     *        the path are created If there is an error any intermediate directories previously
    ...
    public static void writeTable(@NotNull final Table sourceTable,
        ...
        try {
            writeParquetTableImpl(
                sourceTable, definition, writeInstructions, destFile,
    ...
    private static File prepareDestinationFileLocation(@NotNull File destination) {
        ...
        if (!destination.getPath().endsWith(PARQUET_FILE_EXTENSION)) {
            throw new UncheckedDeephavenException("Destination " + destination + " does not end in "
                + PARQUET_FILE_EXTENSION + " extension");
        }
        if (destination.exists()) {
            if (destination.isDirectory()) {
                throw new UncheckedDeephavenException(
                    "Destination " + destination + " exists and is a directory");
            }
            if (!destination.canWrite()) {
                throw new UncheckedDeephavenException(
                    "Destination " + destination + " exists but is not writable");
            }
            return null;
        }
        ...
        if (firstParent.isDirectory()) {
            ...
            throw new UncheckedDeephavenException(
                "Destination " + destination + " has non writable parent directory");
        }
        ...
        if (!firstParent.mkdirs()) {
            throw new UncheckedDeephavenException(
                "Couldn't (re)create destination directory " + firstParent);
        }
        return firstCreated;
    }

    /**
     * Writes tables to disk in parquet format to a supplied set of destinations. If you specify
     * grouping columns, there must already be grouping information for those columns in the
     * sources. This can be accomplished with {@code .by(<grouping columns>).ungroup()} or
     * {@code .sort(<grouping column>)}.
     *
     * @param sources The tables to write
     * @param tableDefinition The common schema for all the tables to write
     * @param writeInstructions Write instructions for customizations while writing
     * @param destinations The destinations paths. Any non existing directories in the paths
     *        provided are created. If there is an error any intermediate directories previously
     *        created are removed; note this makes this method unsafe for concurrent use
    ...
    public static void writeParquetTables(@NotNull final Table[] sources,
        ...
        for (int i = 0; i < sources.length; i++) {
            ...
            try {
                writeParquetTableImpl(source, tableDefinition, writeInstructions, destinations[i],
                    groupingColumns);
    ...
    /**
     * This method attempts to "do the right thing." It examines the source to determine if it's a
     * single parquet file, a metadata file, or a directory. If it's a directory, it additionally
     * tries to guess the layout to use. Unless a metadata file is supplied or discovered in the
     * directory, the first found parquet file will be used to infer schema.
     *
    ...
    private static Table readTableInternal(@NotNull final File source,
        ...
        if (sourceAttr.isRegularFile()) {
            if (sourceFileName.endsWith(PARQUET_FILE_EXTENSION)) {
                final ParquetTableLocationKey tableLocationKey =
                    new ParquetTableLocationKey(source, 0, null);
        ...
        if (sourceAttr.isDirectory()) {
            final Path metadataPath =
                sourcePath.resolve(ParquetMetadataFileLayout.METADATA_FILE_NAME);
            ...
            if (firstEntryAttr.isDirectory() && firstEntryFileName.contains("=")) {
                return readPartitionedTableInferSchema(
                    KeyValuePartitionLayout.forParquet(source, 32), instructions);
            }
            if (firstEntryAttr.isRegularFile()
                && firstEntryFileName.endsWith(PARQUET_FILE_EXTENSION)) {
        ...
        throw new TableDataException(
            "Source " + source + " is neither a directory nor a regular file");
    }
    ...
    /**
     * Reads in a table from files discovered with {@code locationKeyFinder} using the provided
     * table definition.
     *
     * @param locationKeyFinder The source of {@link ParquetTableLocationKey location keys} to
     *        include
     * @param readInstructions Instructions for customizations while reading
     * @param tableDefinition The table's {@link TableDefinition definition}
     * @return The table
    ...
    public static Table readPartitionedTable(
        ...
        final TableLocationProvider locationProvider = new PollingTableLocationProvider<>(
            ...
            new ParquetTableLocationFactory(readInstructions),
            null);
        return new PartitionAwareSourceTable(tableDefinition,
            "Read multiple parquet files with " + locationKeyFinder,
            RegionedTableComponentFactoryImpl.INSTANCE, locationProvider, null);
    }

    /**
     * Reads in a table from files discovered with {@code locationKeyFinder} using a definition
     * built from the first location found, which must have non-null partition values for all
     * partition keys.
     *
     * @param locationKeyFinder The source of {@link ParquetTableLocationKey location keys} to
     *        include
    ...
    public static Table readPartitionedTableInferSchema(
        ...
        final List<ParquetTableLocationKey> foundKeys =
            recordingLocationKeyFinder.getRecordedKeys();
        if (foundKeys.isEmpty()) {
            return TableTools.emptyTable(0);
        }
        // TODO (https://github.com/deephaven/deephaven-core/issues/877): Support schema merge when
        // discovering multiple parquet files
        ...
        for (final String partitionKey : firstKey.getPartitionKeys()) {
            ...
            allColumns.add(ColumnDefinition.fromGenericType(partitionKey,
                getUnboxedTypeIfBoxed(partitionValue.getClass()),
    ...
    public static Table readPartitionedTableWithMetadata(
        ...
        final ParquetMetadataFileLayout layout =
            new ParquetMetadataFileLayout(directory, readInstructions);
    ...
    private static final SimpleTypeMap<Class<?>> DB_ARRAY_TYPE_MAP = SimpleTypeMap.create(
        null, DbCharArray.class, DbByteArray.class, DbShortArray.class, DbIntArray.class,
        DbLongArray.class,
        DbFloatArray.class, DbDoubleArray.class, DbArray.class);

    private static Class<?> loadClass(final String colName, final String desc,
        final String className) {
        try {
            return ClassUtil.lookupClass(className);
        } catch (ClassNotFoundException e) {
            throw new UncheckedDeephavenException(
                "Column " + colName + " with " + desc + "=" + className
                    + " that can't be found in classloader");
        }
    }
        ...
        return (final ParquetSchemaReader.ParquetMessageDefinition parquetColDef) -> {
            ...
            if (parquetColDef.codecType != null && !parquetColDef.codecType.isEmpty()) {
                final Class<?> componentType =
                    (parquetColDef.codecComponentType != null
                        && !parquetColDef.codecComponentType.isEmpty())
                            ? loadClass(parquetColDef.name, "codecComponentType",
                                parquetColDef.codecComponentType)
                            : null;
                final Class<?> dataType =
                    loadClass(parquetColDef.name, "codecType", parquetColDef.codecType);
                colDef =
                    ColumnDefinition.fromGenericType(parquetColDef.name, dataType, componentType);
            } else if (parquetColDef.dhSpecialType != null) {
                if (parquetColDef.dhSpecialType == ColumnTypeInfo.SpecialType.StringSet) {
                    colDef =
                        ColumnDefinition.fromGenericType(parquetColDef.name, StringSet.class, null);
                    ...
                    if (dbArrayType != null) {
                        colDef = ColumnDefinition.fromGenericType(parquetColDef.name, dbArrayType,
                            baseType);
                    } else {
                        colDef = ColumnDefinition.fromGenericType(parquetColDef.name, DbArray.class,
                            baseType);
                    }
                } else {
                    throw new UncheckedDeephavenException(
                        "Unhandled dbSpecialType=" + parquetColDef.dhSpecialType);
                }
            } else {
                if (parquetColDef.isArray) {
                    if (baseType == byte.class && parquetColDef.noLogicalType) {
                        colDef = ColumnDefinition.fromGenericType(parquetColDef.name, byte[].class,
                            byte.class);
                    } else {
                        ...
                        colDef = ColumnDefinition.fromGenericType(parquetColDef.name, dataType,
                            componentType);
                    }
                } else {
    ...
    /**
     * Convert schema information from a {@link ParquetMetadata} into {@link ColumnDefinition
     * ColumnDefinitions}.
     *
     ...
     * @return A {@link Pair} with {@link ColumnDefinition ColumnDefinitions} and adjusted
    ...
    public static Pair<List<ColumnDefinition>, ParquetInstructions> convertSchema(
        ...
        final ParquetSchemaReader.ColumnDefinitionConsumer colConsumer =
            makeSchemaReaderConsumer(cols);
        return new Pair<>(cols, ParquetSchemaReader.readParquetSchema(
            schema,
            keyValueMetadata,
            readInstructionsIn,
            colConsumer,
            (final String colName, final Set<String> takenNames) -> NameValidator
                .legalizeColumnName(colName, s -> s.replace(" ", "_"), takenNames)));
    }
    ...
    public static final ParquetInstructions LZ4 =
        ParquetInstructions.builder().setCompressionCodecName("LZ4").build();
    public static final ParquetInstructions LZO =
        ParquetInstructions.builder().setCompressionCodecName("LZO").build();
    ...
    public static final ParquetInstructions LEGACY =
        ParquetInstructions.builder().setIsLegacyParquet(true).build();
