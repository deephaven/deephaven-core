...
public class SortedAssertionInstrumentedListenerAdapter extends BaseTable.ShiftAwareListenerImpl {
    ...
    public SortedAssertionInstrumentedListenerAdapter(String description,
        ...
        SortingOrder order) {
        super(
            "assertSorted(" + (description == null ? "" : description) + ", " + columnName + ", "
                + order + ')',
            parent, dependent);
        ...
        sortCheck =
            SortCheck.make(parentColumnSource.getChunkType(), order == SortingOrder.Descending);
    }
    ...
    public void onUpdate(final Update upstream) {
        ...
        if (upstream.added.nonempty() || modifiedRows) {
            final Index rowsOfInterest =
                modifiedRows ? upstream.added.union(upstream.modified) : upstream.added;
    ...
    public static void doCheckStatic(Index toProcess, ColumnSource<?> parentColumnSource,
        SortCheck sortCheck, String description, String column, SortingOrder order) {
        final int contextSize = (int) Math.min(CHUNK_SIZE, toProcess.size());

        try (
            final ChunkSource.GetContext getContext =
                parentColumnSource.makeGetContext(contextSize);
            ...
            while (okIt.hasMore()) {
                ...
                final Chunk<? extends Attributes.Values> valuesChunk =
                    parentColumnSource.getChunk(getContext, chunkOk);
                final int firstUnsorted = sortCheck.sortCheck(valuesChunk);
                if (firstUnsorted >= 0) {
                    final String value1 = ChunkUtils.extractKeyStringFromChunk(
                        valuesChunk.getChunkType(), valuesChunk, firstUnsorted);
                    final String value2 = ChunkUtils.extractKeyStringFromChunk(
                        valuesChunk.getChunkType(), valuesChunk, firstUnsorted + 1);
    ...
    private Index makeAdjacentIndex(Index rowsOfInterest) {
        try (final Index inverted = parentIndex.invert(rowsOfInterest)) {
            final Index.SequentialBuilder processBuilder =
                Index.CURRENT_FACTORY.getSequentialBuilder();
            long lastPosition = parentIndex.size() - 1;
            long lastUsedPosition = 0;
            for (ReadOnlyIndex.RangeIterator rangeIterator = inverted.rangeIterator(); rangeIterator
                .hasNext();) {
                rangeIterator.next();
