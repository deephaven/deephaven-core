...
public class CharLongMegaMergeKernel {
    private CharLongMegaMergeKernel() {
        throw new UnsupportedOperationException();
    }

    // region Context
    public static class CharLongMegaMergeKernelContext<ATTR extends Any, KEY_INDICES extends Keys> implements LongMegaMergeKernel<ATTR, KEY_INDICES> {
        @SuppressWarnings("rawtypes")
        private static final CharLongMegaMergeKernelContext INSTANCE = new CharLongMegaMergeKernelContext();
        ...
        public void merge(LongArraySource indexDestinationSource, ArrayBackedColumnSource<?> valuesDestinationSource, long destinationOffset, long destinationSize, LongChunk<KEY_INDICES> indexKeys, Chunk<ATTR> valuesToMerge) {
            CharLongMegaMergeKernel.merge(indexDestinationSource, (CharacterArraySource)valuesDestinationSource, destinationOffset, destinationSize, indexKeys, valuesToMerge.asCharChunk());
        }
    }
    // endregion Context

    public static <ATTR extends Any, KEY_INDICES extends Keys> CharLongMegaMergeKernelContext<ATTR, KEY_INDICES> createContext() {
        //noinspection unchecked
        return CharLongMegaMergeKernelContext.INSTANCE;
    }
    ...
    private static void moveInDest(LongArraySource destinationKeys, CharacterArraySource destinationValues, long sourceStart, long destStart, long length) {
    ...
    private static long bound(CharacterArraySource valuesToSort, long lo, long hi, char searchValue, @SuppressWarnings("SameParameterValue") final boolean lower) {
    ...
    private static int lowerBound(CharChunk<?> valuesToSort, @SuppressWarnings("SameParameterValue") int lo, int hi, char searchValue) {
        return bound(valuesToSort, lo, hi, searchValue, true);
    }

    private static int bound(CharChunk<?> valuesToSort, int lo, int hi, char searchValue, @SuppressWarnings("SameParameterValue") final boolean lower) {
        final int compareLimit = lower ? -1 : 0;  // lt or leq
