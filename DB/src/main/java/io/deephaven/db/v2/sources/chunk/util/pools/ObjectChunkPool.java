...
public final class ObjectChunkPool implements ChunkPool {
    ...
    private final WritableObjectChunk<?, Any> EMPTY = WritableObjectChunk.writableChunkWrap(ArrayUtils.EMPTY_OBJECT_ARRAY);
    ...
    ObjectChunkPool() {
        ...
        for (int pcci = 0; pcci < NUM_POOLED_CHUNK_CAPACITIES; ++pcci) {
            ...
            writableObjectChunks[pcci] = new SegmentedSoftPool<>(
                    ...
                    () -> QueryPerformanceRecorder.recordPoolAllocation(() -> WritableObjectChunk.makeWritableChunkForPool(chunkCapacity)),
        ...
        resettableObjectChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableObjectChunk::makeResettableChunkForPool),
                ResettableObjectChunk::clear
        );
        resettableWritableObjectChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableWritableObjectChunk::makeResettableChunkForPool),
