...
public class CumulativeUtil {
    ...
    /**
     * General purpose core method for executing running and cumulative aggregations. Used by helper methods
     * which provide template formulae for specific types of aggregations.
     ...
     *            {@link CumulativeHelper} map. Note that a non-null literal key must be enclosed in backticks
     *            since it will be used in a formula. Typically a column expression, rather than a literal, is
     *            used to allow operations based on the grouping of the expression.
     ...
     * @param formula A formula indicating how to calculate the aggregation. This is normally provides by
     *                a helper method. An example would be something like: "_prev+A" to cumulatively sum A.
     * @return A {@link Table} with the new aggregation column added. Note that this column will be a double
     * column, regardless of the numeric type(s) of the formula input(s).
     */
    public static Table accumulate(Table t, String key, double startValue, String newCol, String formula){
        ...
        formula=newCol+"=__CumulativeUtil_map.put(" + key + "," + formula.replaceAll("_prev", "__CumulativeUtil_map.get("+key+")") + ")";
     ...
     * @param t The {@link Table} to use as input to the aggregation.
     * @param newCol The name of the aggregation column to add to the table.
     * @param formula A formula for the source value on which to calculate a running minimum. This can be
     *                as simple as the column name or a more complex expression.
     * @return A {@link Table} with the new aggregation column added. Note that this column will be a double
     * column, regardless of the numeric type(s) of the formula input(s).
     */
     ...
     *            used to allow operations based on the grouping of the expression.
     * @param newCol The name of the aggregation column to add to the table.
     * @param formula A formula for the source value on which to calculate a running sum. This can be
     *                as simple as the column name or a more complex expression.
     * @return A {@link Table} with the new aggregation column added. Note that this column will be a double
     * column, regardless of the numeric type(s) of the formula input(s).
     */
    ...
    public static Table rollingSum(Table t, int windowSize, String newCol, String formula){
        return accumulate(t, null, 0, newCol, "_prev+(" + formula + ")").update(newCol + "=" + newCol + "-" + newCol + "_[i-" + windowSize + "]");
    }
     ...
     * @return A {@link Table} of only rows where the selected value has changed from the value in the
     * previous row.
     */
