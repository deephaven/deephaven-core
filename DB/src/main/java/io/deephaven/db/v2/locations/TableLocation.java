...
/**
 * Building block for Deephaven "source" tables, with helper methods for discovering locations and
 * their sizes. A location allows access to columns, size, and possibly other metadata for a single
 * partition that may be included in a source table.
 */
public interface TableLocation
    extends NamedImplementation, LogOutputAppendable, TableLocationState {
    ...
    interface Listener extends BasicTableDataListener {
        ...
        /**
         * Notify the listener that the table location has been updated. This may be called
         * "spuriously," i.e. in cases where there has been no substantive update since the last
         * handleUpdate() invocation. Implementations should use appropriate measures to avoid
         * reacting to spurious updates.
         */
        void handleUpdate();
    }
    ...
    /**
     * <p>
     * Subscribe to pushed location updates. Subscribing more than once with the same listener
     * without an intervening unsubscribe is an error, and may result in undefined behavior.
     * <p>
     * This is a possibly asynchronous operation - listener will receive 1 or more handleUpdate
     * callbacks, followed by 0 or 1 handleException callbacks during invocation and continuing
     * after completion, on a thread determined by the implementation. Don't hold a lock that
     * prevents notification delivery while subscribing!
     * <p>
