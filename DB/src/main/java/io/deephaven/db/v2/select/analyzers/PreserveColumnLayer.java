...
final public class PreserveColumnLayer extends DependencyLayerBase {
    PreserveColumnLayer(SelectAndViewAnalyzer inner, String name, SelectColumn sc, ColumnSource cs,
        String[] deps,
    ...
    public void applyUpdate(ShiftAwareListener.Update upstream, ReadOnlyIndex toClear,
        UpdateHelper helper) {
        // Nothing to do at this level, but need to recurse because my inner layers might need to be
        // called (e.g. because they are SelectColumnLayers)
        inner.applyUpdate(upstream, toClear, helper);
    }

    @Override
    Map<String, ColumnSource> getColumnSourcesRecurse(GetMode mode) {
        // our column is not a new column, so we need to make sure that we do not double enable
        // previous tracking
