...
/**
 * Base {@link ImmutableTableLocationKey} implementation for table locations that may be enclosed by
 * partitions. Sub-classes should be sure to invoke the partition-map comparator at higher priority
 * than other comparisons when implementing {@link #compareTo(Object)}, and to include the
 * partitions in their {@link #equals(Object)} implementations.
 */
public abstract class PartitionedTableLocationKey implements ImmutableTableLocationKey {

    @SuppressWarnings("StringOperationCanBeSimplified")
    private static final Comparable<?> MISSING_PARTITION_VALUE =
        new String("MISSING PARTITION VALUE");
     ...
     *        mutate the map after this call completes, but the partition keys and values themselves
     *        <em>must</em> be effectively immutable.
     */
     ...
     */
    protected static final class PartitionsFormatter
        implements LogOutput.ObjFormatter<Map<String, Comparable<?>>> {

        public static final LogOutput.ObjFormatter<Map<String, Comparable<?>>> INSTANCE =
            new PartitionsFormatter();

        private PartitionsFormatter() {}

        @Override
        public void format(@NotNull final LogOutput logOutput,
            ...
            for (final Iterator<Map.Entry<String, Comparable<?>>> pi =
                partitions.entrySet().iterator(); pi.hasNext();) {
    ...
    /**
     * Compares two maps of partitions by key-value pairs. {@code p1}'s entry order determines the
     * priority of each partition in the comparison, and it's assumed that {@code p2} will have the
     * same entry order. {@link #compare(Map, Map)} will throw an
     * {@link UnknownPartitionKeyException} if one of the maps is missing keys found in the other.
     */
    protected static final class PartitionsComparator
        implements Comparator<Map<String, Comparable<?>>> {

        public static final Comparator<Map<String, Comparable<?>>> INSTANCE =
            new PartitionsComparator();
        ...
        @Override
        public int compare(final Map<String, Comparable<?>> p1,
            final Map<String, Comparable<?>> p2) {
            ...
            for (final Map.Entry<String, Comparable<?>> p1Entry : p1.entrySet()) {
                ...
                final Comparable<?> p2Value =
                    p2.getOrDefault(partitionKey, MISSING_PARTITION_VALUE);
