...
public class PerformanceQueries {
    ...
    /**
     * <p>Takes in a query id and returns a view for that query's performance data.</p>
     *
     * <p>The query performance log contains data on how long each query takes to run. Examples of what constitutes
     * one individual query, for performance logging purposes, include:
     * <ul>
    ...
    public static Table queryPerformance(final long evaluationNumber) {
        ...
        Table queryPerformanceLog = TableLoggers.queryPerformanceLog()
                ...
                .updateView(
                        ...
                        "TimeSecs = nanosToMillis(EndTime - StartTime) / 1000d",  // How long this query ran for, in seconds
                        "NetMemoryChange = FreeMemoryChange - TotalMemoryChange",
                        "QueryMemUsed = TotalMemoryUsed - TotalMemoryFree",     // Memory in use by the query. (Only includes active heap memory.)
                        "QueryMemUsedPct = QueryMemUsed / WorkerHeapSize",      // Memory usage as a percenage of max heap size (-Xmx)
                        "QueryMemFree = WorkerHeapSize - QueryMemUsed"          // Remaining memory until the query runs into the max heap size
                )
                .moveUpColumns(
    ...
    /**
     * <p>Takes in a query id and returns a view for that query's individual operations's performance data.</p>
     *
     * <p>The query operation performance log contains data on how long each individual operation of a query (where(),
    ...
    public static Table queryOperationPerformance(final long evaluationNumber) {
        final Table queryOps = TableLoggers.queryOperationPerformanceLog()
                ...
                .updateView(
                        ...
                        "NetMemoryChange = FreeMemoryChange - TotalMemoryChange" // Change in memory usage delta while this query was executing
                )
                .moveUpColumns(
    ...
    public static Table queryUpdatePerformance(final long evaluationNumber) {
        ...
        Table queryUpdatePerformance = TableLoggers.updatePerformanceLog()
                ...
                .updateView(
                        "WorkerHeapSize = " + workerHeapSizeBytes + "L",
                        "Ratio = EntryIntervalUsage / IntervalDurationNanos",  // % of time during this interval that the operation was using CPU
                        "QueryMemUsed = TotalMemoryUsed - TotalMemoryFree",     // Memory in use by the query. (Only includes active heap memory.)
                        "QueryMemUsedPct = QueryMemUsed / WorkerHeapSize",  // Memory usage as a percenage of the max heap size (-Xmx)
                        "QueryMemFree = WorkerHeapSize - QueryMemUsed",         // Remaining memory until the query runs into the max heap size
                        "NRows = EntryIntervalAdded + EntryIntervalRemoved + EntryIntervalModified",    // Total number of changed rows
                        "RowsPerSec = round(NRows / IntervalDurationNanos * 1.0e9)",         // Average rate data is ticking at
                        "RowsPerCPUSec = round(NRows / EntryIntervalUsage * 1.0e9)"     // Approximation of how fast CPU handles row changes
                )
                .moveUpColumns(
                        "ProcessUniqueId", "EvaluationNumber", "OperationNumber",
