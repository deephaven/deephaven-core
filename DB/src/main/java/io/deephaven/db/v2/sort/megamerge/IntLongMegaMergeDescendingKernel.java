...
import io.deephaven.db.v2.sources.chunk.Attributes.Keys;
...
public class IntLongMegaMergeDescendingKernel {
    ...
    // region Context
    public static class IntLongMegaMergeDescendingKernelContext<ATTR extends Any, KEY_INDICES extends Keys> implements LongMegaMergeKernel<ATTR, KEY_INDICES> {
        ...
        public void merge(LongArraySource indexDestinationSource, ArrayBackedColumnSource<?> valuesDestinationSource, long destinationOffset, long destinationSize, LongChunk<KEY_INDICES> indexKeys, Chunk<ATTR> valuesToMerge) {
            IntLongMegaMergeDescendingKernel.merge(indexDestinationSource, (IntegerArraySource)valuesDestinationSource, destinationOffset, destinationSize, indexKeys, valuesToMerge.asIntChunk());
        }
    }
    // endregion Context

    public static <ATTR extends Any, KEY_INDICES extends Keys> IntLongMegaMergeDescendingKernelContext<ATTR, KEY_INDICES> createContext() {
    ...
    private static <ATTR extends Any, KEY_INDICES extends Keys> void copyChunkToDest(LongChunk<KEY_INDICES> keysChunk, IntChunk<ATTR> valuesChunk, LongArraySource destinationKeys, IntegerArraySource destinationValues, int sourceStart, long destStart, int length) {
    ...
    private static void moveInDest(LongArraySource destinationKeys, IntegerArraySource destinationValues, long sourceStart, long destStart, long length) {
    ...
    private static int doComparison(int lhs, int rhs) {
        return -1 * DhIntComparisons.compare(lhs, rhs);
    }
    ...
    private static long bound(IntegerArraySource valuesToSort, long lo, long hi, int searchValue, @SuppressWarnings("SameParameterValue") final boolean lower) {
    ...
    private static int lowerBound(IntChunk<?> valuesToSort, @SuppressWarnings("SameParameterValue") int lo, int hi, int searchValue) {
        return bound(valuesToSort, lo, hi, searchValue, true);
    }

    private static int bound(IntChunk<?> valuesToSort, int lo, int hi, int searchValue, @SuppressWarnings("SameParameterValue") final boolean lower) {
        final int compareLimit = lower ? -1 : 0;  // lt or leq
