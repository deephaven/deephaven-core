...
/**
 * <p>{@link ResettableContext} used as a holder for other {@link ResettableContext}s that may be shared across
 * components.
 *
 * <p>This serves as a place to cache re-usable computations or resources, but must be {@link #reset()} for every step
 * of an operation (usually a chunk of ordered keys).
 *
 * <p>For example, {@link io.deephaven.db.v2.sources.ReadOnlyRedirectedColumnSource}s that share the same
 * {@link io.deephaven.db.v2.utils.RedirectionIndex} cache a chunk of redirections for the most recent chunk
 * of ordered keys they have been handed.
 *
 * <p>It's important that "nested" usage follows the convention of creating a new instance and passing that instance
...
public class SharedContext implements ResettableContext {
     ...
     *
     * @param key          The key
     * @param valueFactory The value factory, to be invoked if {@code key} is not found within this {@link SharedContext}
     * @return The value associated with {@code key}, possibly newly-created
     */
    public final <V extends ResettableContext, K extends Key<V>> V getOrCreate(final K key, @NotNull final Supplier<V> valueFactory) {
        //noinspection unchecked
        return (V) entries.computeIfAbsent(key, k -> valueFactory.get());
    }

    /**
     * <p>Reset implementation which invokes {@link ResettableContext#reset()} on all values registered via
     * {@link #getOrCreate(Key, Supplier)}.
     *
     * <p>Sub-classes should be sure to call {@code super.reset()}.
     */
    @Override
    ...
    /**
     * <p>Close implementation which invokes {@link SafeCloseable#close()} on all values registered via
     * {@link #getOrCreate(Key, Supplier)}, and then forgets all registered values.
     *
     * <p>Sub-classes should be sure to call {@code super.close()}.
     */
    @Override
    @OverridingMethodsMustInvokeSuper
    public void close() {
