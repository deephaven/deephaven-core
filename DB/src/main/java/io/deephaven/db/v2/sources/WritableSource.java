...
public interface WritableSource<T> extends ColumnSource<T>, WritableChunkSink<Attributes.Values> {
     ...
     * @param nullFilled Whether data should be "null-filled". If true, get operations at index keys
     *        that have not been set will return the appropriate null value; otherwise such gets
     *        produce undefined results.
     */
    void ensureCapacity(long capacity, boolean nullFilled);

    // WritableSource provides a slow, default implementation of fillFromChunk. Inheritors who care
    // should provide
     ...
     */
    @Override
    default void fillFromChunk(@NotNull FillFromContext context,
    ...
    @Override
    default void fillFromChunkUnordered(@NotNull FillFromContext context,
