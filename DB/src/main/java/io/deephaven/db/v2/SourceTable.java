...
public abstract class SourceTable extends RedefinableTable {
    ...
    private static final Logger log =
        ProcessEnvironment.tryGet() == null ? LoggerFactory.getLogger(SourceTable.class)
    ...
    SourceTable(@NotNull final TableDefinition tableDefinition,
        ...
        columnSourceManager =
            componentFactory.createColumnSourceManager(isLive, ColumnToCodecMappings.EMPTY,
                definition.getColumns() /*
                                         * NB: this is the *re-written* definition passed to the
    ...
    /**
     * Force this table to determine its initial state (available locations, size, index) if it
            ...
            QueryPerformanceRecorder.withNugget(description + ".initializeAvailableLocations()",
                () -> {
                    if (isRefreshing()) {
                        ...
                        liveTableRegistrar.addTable(
                            locationChangePoller = new LocationChangePoller(locationBuffer));
                    } else {
    ...
    private void maybeAddLocations(
    ...
    private void initializeLocationSizes() {
        ...
        synchronized (this) {
            ...
            QueryPerformanceRecorder.withNugget(description + ".initializeLocationSizes()",
    ...
    private class LocationChangePoller extends InstrumentedLiveTable {
        ...
        private LocationChangePoller(
            @NotNull final TableLocationSubscriptionBuffer locationBuffer) {
        ...
        protected void instrumentedRefresh() {
            try {
                ...
                // "location listeners", but it was never used - resurrect from git history if
                // needed.
                if (!locationSizesInitialized) {
                    // We don't want to start polling size changes until the initial Index has been
                    // computed.
                    return;
                }
                ...
                index.insert(added);
                notifyListeners(added, Index.FACTORY.getEmptyIndex(),
                    Index.FACTORY.getEmptyIndex());
            } catch (Exception e) {
                // Notify listeners to the SourceTable when we had an issue refreshing available
                // locations.
    ...
    /**
     * Hook to allow found location keys to be filtered (e.g. according to a where-clause on the
     * partitioning columns of a {@link PartitionAwareSourceTable}. The default implementation
     * returns its input unmolested.
     *
    ...
    protected final QueryTable doCoalesce() {
        ...
        final ShiftAwareSwapListener swapListener = createSwapListenerIfRefreshing(
            (final BaseTable parent) -> new ShiftAwareSwapListener(parent) {

                @Override
                public final void destroy() {
                    // NB: We can't call super.destroy() because we don't want to try to remove
                    // ourselves from the
                    // coalesced table (see override for removeUpdateListener), but we are probably
                    // not missing
        ...
        initializeWithSnapshot("SourceTable.coalesce", swapListener,
            (usePrev, beforeClockValue) -> {
                final QueryTable resultTable =
                    new QueryTable(definition, index, columnSourceManager.getColumnSources());
                    ...
                    final ShiftAwareListenerImpl listener =
                        new ShiftAwareListenerImpl("SourceTable.coalesce", this, resultTable) {
                            ...
                            protected final void destroy() {
                                // NB: This implementation cannot call super.destroy() for the same
                                // reason as the swap listener
                                removeUpdateListenerUncoalesced(this);
                            }
                        };
