...
/**
 * <p>
 * {@link TableLocationKeyFinder Location finder} that will examine a parquet metadata file to
 * discover locations.
 *
 * <p>
 * Note that we expect to find the following files:
 * <ul>
 * <li>{@code _metadata} - A file containing Parquet metadata for all {@link RowGroup row groups} in
 * all {@code .parquet} files for the entire data set, including schema information non-partitioning
 * columns and key-value metadata</li>
 * <li>{@code _common_metadata} <i>(optional)</i> - A file containing Parquet metadata with schema
 * information that applies to the entire data set, including partitioning columns that are inferred
 * from file paths rather than explicitly written in {@link org.apache.parquet.format.ColumnChunk
 * column chunks} within {@code .parquet} files</li>
 * </ul>
 */
public class ParquetMetadataFileLayout implements TableLocationKeyFinder<ParquetTableLocationKey> {
    ...
    public ParquetMetadataFileLayout(@NotNull final File directory,
        ...
        this(new File(directory, METADATA_FILE_NAME),
            new File(directory, COMMON_METADATA_FILE_NAME), inputInstructions);
    }
    ...
    public ParquetMetadataFileLayout(@NotNull final File metadataFile,
        ...
        if (!metadataFile.exists()) {
            throw new TableDataException(
                "Parquet metadata file " + metadataFile + " does not exist");
        }
        final ParquetFileReader metadataFileReader =
            ParquetTools.getParquetFileReader(metadataFile);

        final ParquetMetadataConverter converter = new ParquetMetadataConverter();
        final ParquetMetadata metadataFileMetadata =
            convertMetadata(metadataFile, metadataFileReader, converter);
        final Pair<List<ColumnDefinition>, ParquetInstructions> leafSchemaInfo =
            ParquetTools.convertSchema(
        ...
        if (commonMetadataFile != null && commonMetadataFile.exists()) {
            final ParquetFileReader commonMetadataFileReader =
                ParquetTools.getParquetFileReader(commonMetadataFile);
            final Pair<List<ColumnDefinition>, ParquetInstructions> fullSchemaInfo =
                ParquetTools.convertSchema(
                    commonMetadataFileReader.getSchema(),
                    convertMetadata(commonMetadataFile, commonMetadataFileReader, converter)
            ...
            final Map<String, ColumnDefinition> leafDefinitionsMap = leafSchemaInfo.getFirst()
                .stream().collect(toMap(ColumnDefinition::getName, Function.identity()));
            for (final ColumnDefinition fullDefinition : fullSchemaInfo.getFirst()) {
                final ColumnDefinition leafDefinition =
                    leafDefinitionsMap.get(fullDefinition.getName());
                if (leafDefinition == null) {
                    ...
                    adjustedColumnDefinitions.add(fullDefinition); // No adjustments to apply in
                                                                   // this case
                } else {
                    final List<String> differences = new ArrayList<>();
                    // noinspection unchecked
                    fullDefinition.describeDifferences(differences, leafDefinition, "full schema",
                        "file schema", "");
                    throw new TableDataException(String.format(
                        "Schema mismatch between %s and %s for column %s: %s",
        ...
        final Map<String, ColumnDefinition> partitioningColumnsMap =
            partitioningColumns.stream().collect(
                toMap(ColumnDefinition::getName, Function.identity(), Assert::neverInvoked,
                    LinkedHashMap::new));
        ...
        for (int rgi = 0; rgi < numRowGroups; ++rgi) {
            fileNameToRowGroupIndices
                .computeIfAbsent(rowGroups.get(rgi).getColumns().get(0).getFile_path(),
                    fn -> new TIntArrayList())
                .add(rgi);
        }
        ...
        keys = fileNameToRowGroupIndices.entrySet().stream().map(entry -> {
            ...
            if (partitions != null) {
                ...
                if (numPartitions != partitioningColumns.size()) {
                    throw new TableDataException("Unexpected number of path elements in "
                        + filePathString + " for partitions " + partitions.keySet());
                }
                ...
                for (int pi = 0; pi < numPartitions; ++pi) {
                    ...
                    if (useHiveStyle) {
                        ...
                        if (pathComponents.length != 2) {
                            throw new TableDataException(
                                "Unexpected path format found for hive-style partitioning from "
                                    + filePathString + " for " + metadataFile);
                        }
                        partitionKey = instructions
                            .getColumnNameFromParquetColumnNameOrDefault(pathComponents[0]);
                    ...
                    final Comparable<?> partitionValue = CONVERSION_FUNCTIONS
                        .get(columnDefinition.getDataType()).apply(partitionValueRaw);
                    if (partitions.containsKey(partitionKey)) {
                        throw new TableDataException(
                            "Unexpected duplicate partition key " + partitionKey + " when parsing "
                                + filePathString + " for " + metadataFile);
                    }
                    partitions.put(partitionKey, partitionValue);
                }
            }
            final ParquetTableLocationKey tlk = new ParquetTableLocationKey(
                new File(directory, filePathString), partitionOrder.getAndIncrement(), partitions);
    ...
    public String toString() {
        return ParquetMetadataFileLayout.class.getSimpleName() + '[' + metadataFile + ','
            + commonMetadataFile + ']';
    }
    ...
    private static ColumnDefinition adjustPartitionDefinition(
        ...
        // NB: This fallback includes any kind of timestamp; we don't have a strong grasp of
        // required parsing support at
