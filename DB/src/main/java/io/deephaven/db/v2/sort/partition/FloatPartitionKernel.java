...
import io.deephaven.db.v2.sort.timsort.FloatLongTimsortKernel;
...
public class FloatPartitionKernel {
    public static class PartitionKernelContext {
        ...
        private PartitionKernelContext(int chunkSize, int numPartitions, boolean preserveEquality) {
            ...
            pivotValues = WritableFloatChunk.makeWritableChunk(numPartitions - 1);
...
//            System.out.println("[" + IntStream.range(0, pivotValues.size()).mapToObj(pivotValues::get).map(FloatPartitionKernel::format).collect(Collectors.joining(",")) + "]");
...
//    }
    ...
    public static PartitionKernelContext createContext(Index index, ColumnSource<Float> columnSource, int chunkSize, int nPartitions, boolean preserveEquality) {
        ...
        final WritableFloatChunk<Any> tempPivotValues = WritableFloatChunk.makeWritableChunk(nPartitions * 3);
    ...
    private static void samplePivots(Index index, int nPartitions, WritableLongChunk<KeyIndices> pivotKeys, WritableFloatChunk<Any> pivotValues, ColumnSource<Float> columnSource) {
        pivotKeys.setSize(0);
        ...
        try (final LongSortKernel sortContext = FloatLongTimsortKernel.createContext(samplesRequired)) {
     ...
     */
    public static void partition(PartitionKernelContext context, LongChunk<KeyIndices> indexKeys, FloatChunk values) {
        ...
        for (int ii = 0; ii < values.size(); ii += accumulatedChunkSize) {
            ...
            for (int jj = ii; jj < last; jj++) {
                ...
                final float searchValue = values.get(jj);
    ...
    private static int binarySearchPreserve(FloatChunk pivotValues, int lo, int hi, float searchValue) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final float compareValue = pivotValues.get(mid);
    ...
    private static int binarySearchTieIndex(FloatChunk pivotValues, LongChunk<KeyIndices> pivotKeys, int lo, int hi, float searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final float compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
