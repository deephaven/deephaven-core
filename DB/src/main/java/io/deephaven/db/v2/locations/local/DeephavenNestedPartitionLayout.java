...
/**
 * {@link TableLocationKeyFinder} that will traverse a directory hierarchy laid out in Deephaven's
 * "nested-partitioned" format, e.g.
 * 
 * <pre>
 * tableRootDirectory/internalPartitionValue/columnPartitionValue/tableName/...
 * </pre>
 * 
 * , producing {@link FileTableLocationKey}'s with two partitions, for keys
 * {@value INTERNAL_PARTITION_KEY} and the specified {@code columnPartitionKey}.
 */
public abstract class DeephavenNestedPartitionLayout<TLK extends FileTableLocationKey>
    ...
    public static DeephavenNestedPartitionLayout<ParquetTableLocationKey> forParquet(
        ...
        return new DeephavenNestedPartitionLayout<ParquetTableLocationKey>(tableRootDirectory,
            tableName, columnPartitionKey, internalPartitionValueFilter) {
            @Override
            protected ParquetTableLocationKey makeKey(@NotNull Path tableLeafDirectory,
                @NotNull Map<String, Comparable<?>> partitions) {
                return new ParquetTableLocationKey(
                    tableLeafDirectory.resolve(PARQUET_FILE_NAME).toFile(), 0, partitions);
            }
        };
    }
     ...
     * @param internalPartitionValueFilter Filter to control which internal partitions are included,
     *        {@code null} for all
     */
    ...
    public String toString() {
        return DeephavenNestedPartitionLayout.class.getSimpleName() + '[' + tableRootDirectory + ','
            + tableName + ']';
    }
        ...
        PrivilegedFileAccessUtil.doFilesystemAction(() -> {
            try (final DirectoryStream<Path> internalPartitionStream =
                ...
                for (final Path internalPartition : internalPartitionStream) {
                    final String internalPartitionValue =
                        internalPartition.getFileName().toString();
                    ...
                    try (final DirectoryStream<Path> columnPartitionStream =
                        ...
                        for (final Path columnPartition : columnPartitionStream) {
                            partitions.put(columnPartitionKey,
                                columnPartition.getFileName().toString());
                            if (needToUpdateInternalPartitionValue) {
                                // Partition order dictates comparison priority, so we need to
                                // insert the internal partition after the column partition.
                                partitions.put(INTERNAL_PARTITION_KEY, internalPartitionValue);
                                needToUpdateInternalPartitionValue = false;
                            }
                            locationKeyObserver
                                .accept(makeKey(columnPartition.resolve(tableName), partitions));
                        }
                    }
                }
