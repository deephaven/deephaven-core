...
public class TreeIndex extends SortedIndex implements ImplementedByTreeIndexImpl, Externalizable {
    ...
    /**
     * Protects prevImpl.
     * Only updated in checkPrev() and initializePreviousValue() (this later supposed
     * to be used only right after the constructor, in special cases).
     */
    ...
    @VisibleForTesting @Override public int refCount() {
    ...
    @Override public void insert(final long key) {
        if (trace) pre("insert(" + key + ")");
        checkPrevForWrite();
    ...
    @Override public void insertRange(final long startKey, final long endKey) {
        if (trace) pre("insertRange(" + startKey + ", " + endKey + ")");
        checkPrevForWrite();
    ...
    @Override public void insert(final LongChunk<OrderedKeyIndices> keys, final int offset, final int length) {
        Assert.leq(offset + length, "offset + length", keys.size(), "keys.size()");
        if (trace) pre("insert(chunk)");
        checkPrevForWrite();
    ...
    @Override public void remove(final long key) {
        if (trace) pre("remove(" + key + ")");
        checkPrevForWrite();
    ...
    @Override public void removeRange(final long startKey, final long endKey) {
        if (trace) pre("removeRange(" + startKey + ", " + endKey + ")");
        checkPrevForWrite();
    ...
    @Override public void remove(final LongChunk<OrderedKeyIndices> keys, final int offset, final int length) {
        Assert.leq(offset + length, "offset + length", keys.size(), "keys.size()");
        if (trace) pre("remove(chunk)");
        checkPrevForWrite();
    ...
    @Override public boolean forEachLong(LongAbortableConsumer lc) {
        if (trace) pre("forEachLong");
    ...
    // endPos is exclusive.
    @Override public Index subindexByPos(final long startPos, final long endPos) {
        if (trace) pre("subindexByPos(" + startPos + "," + endPos + ")");
        final TreeIndex ans = new TreeIndex(impl.ixSubindexByPosOnNew(startPos, endPos));
        if (trace) pos();
        return ans;
    }

    @Override public Index subindexByKey(final long startKey, final long endKey) {
        if (trace) pre("subindexByKey(" + startKey + "," + endKey + ")");
        final TreeIndex ans = new TreeIndex(impl.ixSubindexByKeyOnNew(startKey, endKey));
        if (trace) pos();
        return ans;
    }

    @Override public long get(final long pos) {
        if (trace) pre("get(" + pos + ")");
    ...
    @Override public void getKeysForPositions(PrimitiveIterator.OfLong positions, LongConsumer outputKeys) {
        if (trace) pre("getKeysForPositions");
        impl.ixGetKeysForPositions(positions, outputKeys);
        if (trace) pos();
    }

    @Override public long getPrev(final long pos) {
        if (pos < 0) {
            return -1;
        }
        if (trace) pre("getPrev(" + pos + ")");
        final long ans = checkAndGetPrev().ixGet(pos);
        if (trace) pos();
        return ans;
    }

    @Override public long sizePrev() {
        if (trace) pre("sizePrev()");
        final long ans = checkAndGetPrev().ixCardinality();
        if (trace) pos();
        return ans;
    }

    @Override public Index getPrevIndex() {
        if (trace) pre("getPrevIndex");
    ...
    public void initializePreviousValue() {
        if (trace) pre("initializePreviousValue");
        initializePreviousValueInternal();
        if (trace) pos();
    }

    @Override public long find(long key) {
        if (trace) pre("find(" + key + ")");
    ...
    @Override public long findPrev(long key) {
        if (trace) pre("findPrev(" + key + ")");
        final long ans = checkAndGetPrev().ixFind(key);
        if (trace) pos();
        return ans;
    }

    @Override public long firstKeyPrev() {
        if (trace) pre("firstKeyPrev");
        final long ans = checkAndGetPrev().ixFirstKey();
        if (trace) pos();
        return ans;
    }

    @Override public long lastKeyPrev() {
        if (trace) pre("firstKeyPrev");
    ...
    @Override public SearchIterator searchIterator() {
        if (trace) pre("searchIiterator");
    ...
    @NotNull
    @Override public Index.Iterator iterator() {
        if (trace) pre("iterator");
        final Index.Iterator ans = impl.ixIterator();
        if (trace) pos();
        return ans;
    }

    @Override public SearchIterator reverseIterator() {
        if (trace) pre("reverseIterator");
        final SearchIterator ans = impl.ixReverseIterator();
        if (trace) pos();
        return ans;
    }

    @Override public RangeIterator rangeIterator() {
        if (trace) pre("rangeIterator");
    ...
    @Override public boolean containsRange(final long start, final long end) {
        if (trace) pre("containsRange");
    ...
    @Override public void retainRange(final long startKey, final long endKey) {
        if (trace) pre("retainRange");
        checkPrevForWrite();
    ...
    @Override public boolean overlapsRange(final long start, final long end) {
        if (trace) pre("overlapsRange");
    ...
    private static class IndexRandomBuilder extends TreeIndexImplRandomBuilder implements Index.RandomBuilder {
        @Override public Index getIndex() {
    ...
    private static class CurrentOnlyIndexRandomBuilder extends TreeIndexImplRandomBuilder implements Index.RandomBuilder {
        @Override public Index getIndex() {
    ...
    @Override public Index shift(final long shiftAmount) {
        if (trace) pre("shift(" + shiftAmount + ")");
        final TreeIndex ans = new TreeIndex(impl.ixShiftOnNew(shiftAmount));
        if (trace) pos();
        return ans;
    }

    @Override public void shiftInPlace(final long shiftAmount) {
        if (trace) pre("shiftInPlace(" + shiftAmount + ")");
    ...
    public void validate(final String failMsg) {
        ...
        while (it.hasNext()) {
            ...
            Assert.assertion(start > lastEnd + 1, m + "start > lastEnd + 1", start, "start", lastEnd, "lastEnd", this, "index");
            lastEnd = end;
    ...
    @Override public long getAverageRunLengthEstimate() {
