...
/**
 * A source table that can filter partitions before coalescing. Refer to {@link TableLocationKey}
 * for an explanation of partitioning.
 */
public class PartitionAwareSourceTable extends SourceTable {
    ...
    protected PartitionAwareSourceTable newInstance(@NotNull final TableDefinition tableDefinition,
        ...
        @NotNull final Map<String, ColumnDefinition> partitioningColumnDefinitions,
        @Nullable final SelectFilter... partitioningColumnFilters) {
        return new PartitionAwareSourceTable(tableDefinition, description, componentFactory,
            locationProvider, liveTableRegistrar, partitioningColumnDefinitions,
    ...
    private PartitionAwareSourceTable getFilteredTable(
        ...
        SelectFilter[] resultPartitioningColumnFilters =
            new SelectFilter[partitioningColumnFilters.length
                + additionalPartitioningColumnFilters.length];
    ...
    private static class PartitionAwareQueryTableReference extends QueryTableReference {
        ...
        public TableAndRemainingFilters getWithWhere(SelectFilter... selectFilters) {
            ...
            for (SelectFilter filter : selectFilters) {
                ...
                } else if (((PartitionAwareSourceTable) table)
                    .isValidAgainstColumnPartitionTable(columns, filter.getColumnArrays())) {
            ...
            final Table result = partitionFilters.isEmpty() ? table.coalesce()
                : table
                    .where(partitionFilters.toArray(SelectFilter.ZERO_LENGTH_SELECT_FILTER_ARRAY));

            // put the other filters onto the end of the grouping filters, this means that the group
            // filters should
            // go first, which should be preferable to having them second. This is basically the
            // first query
            // optimization that we're doing for the user, so maybe it is a good thing but maybe
            // not. The reason we do
            // it, is that we have deferred the filters for the users permissions, and they did not
    ...
    protected final BaseTable redefine(@NotNull final TableDefinition newDefinition) {
        ...
        if (newDefinition.getColumns().length == definition.getColumns().length || newDefinition
            .getPartitioningColumns().size() == partitioningColumnDefinitions.size()) {
            // Nothing changed except ordering, *or* some columns were dropped but the partitioning
            // column was retained.
            return newInstance(newDefinition,
                description + "-retainColumns",
                componentFactory, locationProvider, liveTableRegistrar,
                partitioningColumnDefinitions, partitioningColumnFilters);
        }
        // Some partitioning columns are gone - defer dropping them.
        final List<ColumnDefinition> newColumnDefinitions =
            new ArrayList<>(newDefinition.getColumnList());
        ...
        final Collection<ColumnDefinition> droppedPartitioningColumnDefinitions =
            partitioningColumnDefinitions.values().stream()
                .filter(cd -> !retainedPartitioningColumnDefinitions.containsKey(cd.getName()))
                .collect(Collectors.toList());
        newColumnDefinitions.addAll(droppedPartitioningColumnDefinitions);
        final PartitionAwareSourceTable redefined =
            newInstance(new TableDefinition(newColumnDefinitions),
                description + "-retainColumns",
                componentFactory, locationProvider, liveTableRegistrar,
                partitioningColumnDefinitions, partitioningColumnFilters);
        final DeferredViewTable deferredViewTable = new DeferredViewTable(newDefinition,
            description + "-retainColumns", new PartitionAwareQueryTableReference(redefined),
            droppedPartitioningColumnDefinitions.stream().map(ColumnDefinition::getName)
                .toArray(String[]::new),
            null, null);
    ...
    protected final Table redefine(TableDefinition newDefinitionExternal,
        ...
        DeferredViewTable deferredViewTable = new DeferredViewTable(newDefinitionExternal,
            description + "-redefined", reference, null, viewColumns, null);
        deferredViewTable.setRefreshing(isRefreshing());
        return deferredViewTable;
    }

    private static final String LOCATION_KEY_COLUMN_NAME =
        "__PartitionAwareSourceTable_TableLocationKey__";

    @SuppressWarnings("unchecked")
    private static <T> ColumnSource makePartitionSource(
        @NotNull final ColumnDefinition<T> columnDefinition,
        ...
        locationKeys.stream()
            ...
            .forEach((final T partitionValue) -> result.set(nextIndex.getAndIncrement(),
                partitionValue));
        return result;
    }
    ...
    protected final Collection<ImmutableTableLocationKey> filterLocationKeys(
        ...
        // TODO (https://github.com/deephaven/deephaven-core/issues/867): Refactor around a ticking
        // partition table
        ...
        for (final ColumnDefinition columnDefinition : partitioningColumnDefinitions.values()) {
            ...
            partitionTableColumnSources
                .add(makePartitionSource(columnDefinition, foundLocationKeys));
        }
        partitionTableColumnSources.add(ArrayBackedColumnSource
            .getMemoryColumnSource(foundLocationKeys, ImmutableTableLocationKey.class, null));
        final Table filteredColumnPartitionTable = TableTools
            .newTable(foundLocationKeys.size(), partitionTableColumnNames,
                partitionTableColumnSources)
    ...
    public final Table where(SelectFilter... filters) {
        ...
        // if there was nothing that actually required the partition, defer the result. This is
        // different than V1, and
        ...
        SelectFilter[] partitionFilterArray =
            partitionFilters.toArray(SelectFilter.ZERO_LENGTH_SELECT_FILTER_ARRAY);
        final String filteredTableDescription =
            "getFilteredTable(" + Arrays.toString(partitionFilterArray) + ")";
        ...
        if (groupFilters.isEmpty()) {
            return QueryPerformanceRecorder.withNugget(
                description + filteredTableDescription + ".coalesce()", filteredTable::coalesce);
        }

        return QueryPerformanceRecorder.withNugget(
            description + ".coalesce().where(" + groupFilters + ")", () -> filteredTable.coalesce()
    ...
    public final Table selectDistinct(@NotNull final SelectColumn... columns) {
        for (SelectColumn selectColumn : columns) {
            ...
            if (!isValidAgainstColumnPartitionTable(selectColumn.getColumns(),
        ...
        final List<String> partitionTableColumnNames =
            new ArrayList<>(partitioningColumnDefinitions.keySet());
        final List<ColumnSource> partitionTableColumnSources =
            new ArrayList<>(partitioningColumnDefinitions.size());
        for (final ColumnDefinition columnDefinition : partitioningColumnDefinitions.values()) {
            // noinspection unchecked
            partitionTableColumnSources
                .add(makePartitionSource(columnDefinition, existingLocationKeys));
        }
        return TableTools
            .newTable(existingLocationKeys.size(), partitionTableColumnNames,
                partitionTableColumnSources)
            .selectDistinct(columns);
        // TODO (https://github.com/deephaven/deephaven-core/issues/867): Refactor around a ticking
        // partition table
        // TODO: Maybe just get rid of this implementation and coalesce? Partitioning columns are
        // automatically grouped. Needs lazy region allocation.
    }
