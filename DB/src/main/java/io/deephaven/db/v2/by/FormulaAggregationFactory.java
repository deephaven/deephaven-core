...
public class FormulaAggregationFactory implements AggregationContextFactory {
    ...
    public FormulaAggregationFactory(@NotNull final String formula,
    ...
    public AggregationContext makeAggregationContext(@NotNull final Table inputTable,
        ...
        final Set<String> groupByColumnNameSet =
            Arrays.stream(groupByColumnNames).collect(Collectors.toSet());
        final MatchPair[] resultColumns = inputTable.getDefinition().getColumnNames().stream()
            .filter(cn -> !groupByColumnNameSet.contains(cn)).map(MatchPairFactory::getExpression)
        ...
        final FormulaChunkedOperator formulaChunkedOperator = new FormulaChunkedOperator(
            byChunkedOperator, true, formula, columnParamName, resultColumns);
    ...
    public static QueryTable applyToAllBy(@NotNull final QueryTable inputTable,
        ...
        @NotNull final String columnParamName,
        @NotNull final String... groupByColumnNames) {
        return applyToAllBy(AggregationControl.DEFAULT_FOR_OPERATOR, inputTable, formula,
            columnParamName, groupByColumnNames);
    }
    ...
    public static QueryTable applyToAllBy(@NotNull final QueryTable inputTable,
        ...
        return applyToAllBy(AggregationControl.DEFAULT_FOR_OPERATOR, inputTable, formula,
            columnParamName, groupByColumns);
    }
