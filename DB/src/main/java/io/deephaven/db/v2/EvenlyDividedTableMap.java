...
public class EvenlyDividedTableMap {
     ...
     *
     * <p>This method is intended to be used to enable parallelism by creating a TableMap without the cost of a full
     * byExternal.  It is important to note that any natural boundaries in the data are not respected, thus when
     * doing operations in parallel and calling {@link TransformableTableMap#merge()} the result is very likely
     * not the same as doing the operations individually.  Care must be taken to preserve your desired semantics.</p>
     *
     * @param table       the table to divide
     * @param divisions   how many divisions should be created, you may want to align this value with {@link TableMapTransformThreadPool#TRANSFORM_THREADS}.
    ...
    static TableMap makeEvenlyDividedTableMap(Table table, int divisions, int minimumSize) {
        ...
        if (queryTable.isRefreshing()) {
            ...
            final ShiftAwareListener listener = new InstrumentedShiftAwareListenerAdapter("tablemap division", queryTable, false) {
                ...
                public void onUpdate(Update upstream) {
                    if (queryTable.getIndex().size() > currentEnd) {
                        ...
                        while (currentEnd < queryTable.getIndex().size()) {
                            localTableMap.put(new Division(currentEnd, currentEnd + divisionSize), queryTable.slice(currentEnd, currentEnd + divisionSize));
