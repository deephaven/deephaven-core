...
public interface ChunkSource<ATTR extends Attributes.Any> extends FillContextMaker, GetContextMaker {
     ...
     *
     * @apiNote <p>The returned chunk belongs to the ColumnSource and may be mutated as result of calling getChunk again
     *             under the same context or as a result of the column source itself being mutated. The callee is not
     *             supposed to keep references to the chunk beyond the scope of the call.</p>
     *          <p>Post-condition: The retrieved values start at position 0 in the chunk.</p>
     *          <p>Post-condition: destination.size() will be equal to orderedKeys.size()</p>
     */
    Chunk<? extends ATTR> getChunk(@NotNull GetContext context, @NotNull OrderedKeys orderedKeys);

    /**
     * Same as {@link #getChunk(GetContext, OrderedKeys)}, except that you pass in the begin and last keys
     * representing the begin and last (inclusive) keys of a single range rather than an {@link OrderedKeys}.
     * Typically you want to call this only if you don't have an {@link OrderedKeys}, such as during an
     * {@link OrderedKeys#forAllLongRanges(LongRangeConsumer)} call.  In this case, it allows you to avoid
     * creating an intermediary {@link OrderedKeys} object.
     *
     * @param context     A context containing all mutable/state related data used in retrieving the Chunk.
     *                    In particular, the Context may be used to provide a Chunk data pool
     ...
     *
     * @apiNote <p>[beginKey,lastKey] must be a range that exists in this ChunkSource.  This is unchecked.</p>
     *          <p>Post-condition: The retrieved values start at position 0 in the chunk.</p>
     *          <p>Post-condition: destination.size() will be equal to lastKey - beginKey + 1</p>
     */
     ...
     *                    [0,orderedKeys.size()).
     * @param orderedKeys An {@link OrderedKeys} representing the keys to be fetched
     * @apiNote <p>Post-condition: The retrieved values start at position 0 in the chunk.</p>
     *          <p>Post-condition: destination.size() will be equal to orderedKeys.size()</p>
     */
    void fillChunk(@NotNull FillContext context, @NotNull WritableChunk<? super ATTR> destination, @NotNull OrderedKeys orderedKeys);
    ...
    interface WithPrev<ATTR extends Attributes.Any> extends ChunkSource<ATTR> {
         ...
         * @param orderedKeys An {@link OrderedKeys} representing the keys to be fetched
         * @return A chunk of data corresponding to the keys from the given {@link OrderedKeys}
         * @apiNote <p>The returned chunk belongs to the ColumnSource and may be mutated as result of calling getChunk
         *             again under the same context or as a result of the column source itself being mutated. The callee
         *             is not supposed to keep references to the chunk beyond the scope of the call.</p>
         *          <p>Post-condition: The retrieved values start at position 0 in the chunk.
         *          <p>Post-condition: destination.size() will be equal to orderedKeys.size()
         */
         ...
         *          <p>Post-condition: destination.size() will be equal to orderedKeys.size()</p>
         */
        void fillPrevChunk(@NotNull FillContext context, @NotNull WritableChunk<? super ATTR> destination, @NotNull OrderedKeys orderedKeys);
