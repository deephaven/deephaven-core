...
public class ReplicateFreezeBy {
    public static void main(String[] args) throws IOException {
        final List<String> results = ReplicatePrimitiveCode
            .charToAllButBoolean(CharFreezeByHelper.class, ReplicatePrimitiveCode.MAIN_SRC);

        final Optional<String> longResult =
            results.stream().filter(s -> s.contains("Long")).findFirst();
        // noinspection OptionalGetWithoutIsPresent
        fixupLong(longResult.get());
        final String objectResult = ReplicatePrimitiveCode.charToObject(CharFreezeByHelper.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        fixupObject(objectResult);

        final String booleanResult = ReplicatePrimitiveCode.charToBoolean(CharFreezeByHelper.class,
            ReplicatePrimitiveCode.MAIN_SRC);
    ...
    private static void fixupObject(String objectResult) throws IOException {
        ...
        final List<String> newLines =
            ReplicateUtilities.replaceRegion(lines, "clearIndex", Collections
                .singletonList("        removed.forAllLongs(idx -> resultSource.set(idx, null));"));
    ...
    private static void fixupBoolean(String booleanResult) throws IOException {
        ...
        final List<String> newLines = ReplicateUtilities.globalReplacements(lines,
            "final BooleanChunk asBoolean = values.asBooleanChunk",
    ...
    private static void fixupLong(String longResult) throws IOException {
        ...
        final List<String> newLines = ReplicateUtilities.globalReplacements(0, lines,
            "LongArraySource", "AbstractLongArraySource");
