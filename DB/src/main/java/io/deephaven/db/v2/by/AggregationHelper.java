...
/**
 * Implementation for chunk-oriented aggregation operations, including {@link Table#by} and
 * {@link Table#byExternal}.
 */
...
public class AggregationHelper {
    ...
    public static QueryTable by(@NotNull final AggregationControl aggregationControl,
        ...
        {
            final Map<String, ColumnSource> keyColumnSourceMap =
                new LinkedHashMap<>(keyColumns.length);
            final Map<String, ColumnSource> fullColumnSourceMap =
                new LinkedHashMap<>(existingColumnSourceMap);
            ...
            aggregatedColumnNames = existingColumnSourceMap.keySet().stream()
                .filter(columnSource -> !keyColumnSourceMap.containsKey(columnSource))
                .toArray(String[]::new);
            keyColumnSources =
                keyColumnSourceMap.values().toArray(ColumnSource.ZERO_LENGTH_COLUMN_SOURCE_ARRAY);
        }
        ...
        if (inputTable.isRefreshing()) {
            return incrementalHashedBy(aggregationControl, inputTable, existingColumnSourceMap,
                keyColumnNames, aggregatedColumnNames, keyColumnSources,
                keyColumnUpstreamInputColumnNames);
        }
        return staticHashedBy(aggregationControl, inputTable, existingColumnSourceMap,
            keyColumnNames, aggregatedColumnNames, keyColumnSources);
    }
    ...
    private static QueryTable noKeyBy(@NotNull final QueryTable inputTable) {
        ...
        inputTable.initializeWithSnapshot("by()-Snapshot", swapListener,
            (final boolean usePrev, final long beforeClockValue) -> {
                ...
                final boolean empty =
                    usePrev ? inputTable.getIndex().firstKeyPrev() == Index.NULL_KEY
                        : inputTable.isEmpty();
                    ...
                    inputTable.getColumnSourceMap().entrySet().stream().collect(Collectors.toMap(
                        ...
                        (final Map.Entry<String, ColumnSource> columnNameToSourceEntry) -> {
                            ...
                            final AggregateColumnSource aggregateColumnSource =
                                AggregateColumnSource.make(columnNameToSourceEntry.getValue(),
                    ...
                    final ModifiedColumnSet.Transformer transformer =
                        inputTable.newModifiedColumnSetTransformer(
                            ...
                            resultTable.getDefinition().getColumnNames().stream()
                                .map(resultTable::newModifiedColumnSet)
                        ...
                        new BaseTable.ShiftAwareListenerImpl("by()", inputTable, resultTable) {
                            ...
                            public final void onUpdate(@NotNull final Update upstream) {
                                final boolean wasEmpty =
                                    inputTable.getIndex().firstKeyPrev() == Index.NULL_KEY;
                                ...
                                if (wasEmpty) {
                                    if (isEmpty) {
                                        // empty -> empty: No change to report, we probably
                                        // shouldn't even be notified
                                        return;
                                    }
                                    ...
                                    modified = Index.FACTORY.getEmptyIndex();
                                    modifiedColumnSet = ModifiedColumnSet.EMPTY;
                                } else if (upstream.added.nonempty()
                                    || upstream.removed.nonempty()) {
                                    added = Index.FACTORY.getEmptyIndex();
                                    removed = Index.FACTORY.getEmptyIndex();
                                    ...
                                    // Only shifts: Nothing to report downstream, our data has not
                                    // changed
                                    return;
                                }
    ...
    @NotNull
    private static QueryTable staticGroupedBy(
        ...
        final ObjectArraySource<Index> resultIndexColumnSource =
            flatResultColumnSources.getSecond();
        ...
        existingColumnSourceMap.entrySet().stream()
            .filter((
                final Map.Entry<String, ColumnSource> columnNameToSourceEntry) -> !columnNameToSourceEntry
    ...
    private static QueryTable staticHashedBy(@NotNull final AggregationControl aggregationControl,
        ...
        final int numGroups = stateManager.buildTable(inputTable,
            maybeReinterpretedKeyColumnSources, groupIndexToHashSlot);
        stateManager.convertBuildersToIndexes(groupIndexToHashSlot, numGroups);

        // TODO: Consider selecting the hash inputTable sources, in order to truncate them to size
        // and improve density
        ...
        final RedirectionIndex resultIndexToHashSlot =
            new IntColumnSourceRedirectionIndex(groupIndexToHashSlot);
        ...
        for (int kci = 0; kci < keyHashTableSources.length; ++kci) {
            ...
            if (keyColumnSources[kci] != maybeReinterpretedKeyColumnSources[kci]) {
                resultKeyColumnSource = ReinterpretUtilities
                    .convertToOriginal(keyColumnSources[kci].getType(), resultKeyColumnSource);
            }
        ...
        Arrays.stream(aggregatedColumnNames)
            .forEachOrdered((final String aggregatedColumnName) -> resultColumnSourceMap
                .put(aggregatedColumnName, AggregateColumnSource.make(
    ...
    @NotNull
    private static QueryTable incrementalHashedBy(
        ...
        inputTable.initializeWithSnapshot("by(" + String.join(",", keyColumnNames) + "-Snapshot",
            swapListener, (final boolean usePrev, final long beforeClockValue) -> {
                // Reinterpret key column sources as primitives where possible
                final ColumnSource[] maybeReinterpretedKeyColumnSources =
                    maybeReinterpretKeyColumnSources(keyColumnSources);
                ...
                final IncrementalChunkedByAggregationStateManager stateManager =
                    new IncrementalChunkedByAggregationStateManager(
                ...
                if (usePrev) {
                    stateManager.buildInitialTableFromPrevious(inputTable,
                        maybeReinterpretedKeyColumnSources, updateTracker);
                } else {
                    stateManager.buildInitialTableFromCurrent(inputTable,
                        maybeReinterpretedKeyColumnSources, updateTracker);

                }
                // Compute result index and redirection to hash slots
                final RedirectionIndex resultIndexToHashSlot = RedirectionIndexLockFreeImpl.FACTORY
                    .createRedirectionIndex(updateTracker.size());
                final Index resultIndex =
                    updateTracker.applyAddsAndMakeInitialIndex(stateManager.getIndexSource(),
                ...
                // Gather the result aggregate columns
                final ColumnSource<Index> resultIndexColumnSource =
                    new ReadOnlyRedirectedColumnSource<>(resultIndexToHashSlot,
                        stateManager.getIndexHashTableSource());
                Arrays.stream(aggregatedColumnNames)
                    .forEachOrdered((final String aggregatedColumnName) -> {
                        // noinspection unchecked
                        final AggregateColumnSource aggregatedColumnSource = AggregateColumnSource
                            .make(existingColumnSourceMap.get(aggregatedColumnName),
                                resultIndexColumnSource);
                ...
                final ModifiedColumnSet upstreamKeyColumnInputs = inputTable.newModifiedColumnSet(
                    ...
                    new BaseTable.ShiftAwareListenerImpl(
                        ...
                        public final void onUpdate(@NotNull final Update upstream) {
                            ...
                            if (keyColumnsModified) {
                                try (final Index toRemove =
                                    upstream.removed.union(upstream.getModifiedPreShift())) {
                                    stateManager.processRemoves(maybeReinterpretedKeyColumnSources,
                                        toRemove, updateTracker);
                                }
                            } else {
                                stateManager.processRemoves(maybeReinterpretedKeyColumnSources,
                                    upstream.removed, updateTracker);
                            }
                            updateTracker.applyRemovesToStates(stateManager.getIndexSource(),
                                stateManager.getOverflowIndexSource());

                            if (upstream.shifted.nonempty()) {
                                upstream.shifted.apply((final long beginRange, final long endRange,
                                    final long shiftDelta) -> {
                                    final Index shiftedPreviousIndex;
                                    try (final Index previousIndex =
                                        inputTable.getIndex().getPrevIndex()) {
                                        shiftedPreviousIndex =
                                            previousIndex.subindexByKey(beginRange, endRange);
                                    }
                                    try {
                                        if (aggregationControl.shouldProbeShift(
                                            shiftedPreviousIndex.size(), resultIndex.intSize())) {
                                            stateManager.processShift(
                                                maybeReinterpretedKeyColumnSources,
                                                shiftedPreviousIndex, updateTracker);
                                            updateTracker.applyShiftToStates(
                                                stateManager.getIndexSource(),
                                                stateManager.getOverflowIndexSource(), beginRange,
                                                endRange, shiftDelta);
                                        } else {
                                            resultIndex.forAllLongs((final long stateKey) -> {
                                                final int stateSlot =
                                                    (int) resultIndexToHashSlot.get(stateKey);
                                                stateManager.applyShift(stateSlot, beginRange,
                                                    endRange, shiftDelta,
                                                    updateTracker::processAppliedShift);
                                            });
                                        }
                                    } finally {
                            ...
                            if (keyColumnsModified) {
                                try (final Index toAdd = upstream.added.union(upstream.modified)) {
                                    stateManager.processAdds(maybeReinterpretedKeyColumnSources,
                                        toAdd, updateTracker);
                                }
                            } else {
                                stateManager.processModifies(maybeReinterpretedKeyColumnSources,
                                    upstream.modified, updateTracker);
                                stateManager.processAdds(maybeReinterpretedKeyColumnSources,
                                    upstream.added, updateTracker);
                            }
                            ...
                            final Update downstream = updateTracker.makeUpdateFromStates(
                                stateManager.getIndexSource(),
                                stateManager.getOverflowIndexSource(), resultIndex,
                                resultIndexToHashSlot,
                                (final boolean someKeyHasAddsOrRemoves,
                                    final boolean someKeyHasModifies) -> {
                                    if (someKeyHasAddsOrRemoves) {
                                        return downstreamAllAggregatedColumns;
                                    }
                                    if (someKeyHasModifies) {
                                        aggregatedColumnsTransformer.clearAndTransform(
    ...
    public static LocalTableMap byExternal(@NotNull final AggregationControl aggregationControl,
        ...
        final boolean dropKeyColumns,
        @NotNull final String... keyColumnNames) {
        // If there are no key columns, return a map with just the input table; there's nothing to
        // be aggregated
        ...
        final ColumnSource[] keyColumnSources = Arrays.stream(keyColumnNames)
            .map(inputTable::getColumnSource).toArray(ColumnSource[]::new);
        ...
        final Map<Object, Index> groupingForAggregation =
            maybeGetGroupingForAggregation(aggregationControl, inputTable, keyColumnSources);
        if (groupingForAggregation != null) {
            final LocalTableMap staticGroupedResult =
                new LocalTableMap(null, inputTable.getDefinition());
            AbstractColumnSource.forEachResponsiveGroup(groupingForAggregation,
                inputTable.getIndex(),
        ...
        if (inputTable.isRefreshing()) {
            return incrementalHashedByExternal(aggregationControl, inputTable, keyColumnSources,
                subTableSource);
        }

        return staticHashedByExternal(aggregationControl, inputTable, keyColumnSources,
            subTableSource);
    }

    @NotNull
    private static LocalTableMap staticHashedByExternal(
        @NotNull final AggregationControl aggregationControl,
        @NotNull final QueryTable inputTable,
        @NotNull final ColumnSource[] keyColumnSources,
        ...
        // Build our table map
        final LocalTableMap staticHashedResult =
            new LocalTableMap(null, inputTable.getDefinition());

        final TupleSource<?> inputKeyIndexToMapKeySource =
            keyColumnSources.length == 1 ? keyColumnSources[0]
                : new SmartKeySource(keyColumnSources);
        final ColumnSource<Index> hashSlotToIndexSource = stateManager.getIndexHashTableSource();
        final int chunkSize =
            Math.min(numGroups, IncrementalChunkedByAggregationStateManager.CHUNK_SIZE);
        ...
        try (final OrderedKeys groupIndices = CurrentOnlyIndex.FACTORY.getFlatIndex(numGroups);
            ...
            final ChunkSource.GetContext hashSlotGetContext =
                groupIndexToHashSlot.makeGetContext(chunkSize);
            ...
            final ChunkSource.GetContext mapKeyGetContext =
                inputKeyIndexToMapKeySource.makeGetContext(chunkSize)) {
            while (groupIndicesIterator.hasMore()) {
                ...
                final LongChunk<Values> hashSlots = groupIndexToHashSlot
                    .getChunk(hashSlotGetContext, groupIndexesForThisChunk).asLongChunk();
                ...
                try (final OrderedKeys inputKeyIndices =
                    ...
                    mapKeys = inputKeyIndexToMapKeySource
                        .getChunk(mapKeyGetContext, inputKeyIndices).asObjectChunk();
                }
                for (int gi = 0; gi < groupsInThisChunk; ++gi) {
                    staticHashedResult.put(mapKeys.get(gi),
                        subTableSource.getSubTable(aggregatedIndexes.get(gi)));
                }
            }
        }
    ...
    @NotNull
    private static LocalTableMap incrementalHashedByExternal(
        @NotNull final AggregationControl aggregationControl,
        @NotNull final QueryTable inputTable,
        @NotNull final ColumnSource[] keyColumnSources,
        @NotNull final QueryTable subTableSource) {
    ...
    private static Map<Object, Index> maybeGetGroupingForAggregation(
        ...
        @NotNull final QueryTable inputTable,
        @NotNull final ColumnSource[] keyColumnSources) {
        // If we have one grouped key column and the input table is not refreshing use the existing
        // grouping
    ...
    private static ColumnSource[] maybeReinterpretKeyColumnSources(
        ...
        final ColumnSource[] maybeReinterpretedKeyColumnSources =
            new ColumnSource[keyColumnSources.length];
        for (int kci = 0; kci < keyColumnSources.length; ++kci) {
