...
public class AggregationHelper {
    ...
    public static QueryTable by(@NotNull final AggregationControl aggregationControl,
        ...
        final Map<Object, Index> groupingForAggregation = maybeGetGroupingForAggregation(aggregationControl, inputTable, keyColumnSources);
        if (groupingForAggregation != null) {
            return staticGroupedBy(existingColumnSourceMap, keyColumnNames[0], keyColumnSources[0], groupingForAggregation);
        }
        ...
        return staticHashedBy(aggregationControl, inputTable, existingColumnSourceMap, keyColumnNames, aggregatedColumnNames, keyColumnSources);
    }
    ...
    private static QueryTable noKeyBy(@NotNull final QueryTable inputTable) {
        ...
        final ShiftAwareSwapListener swapListener = inputTable.createSwapListenerIfRefreshing(ShiftAwareSwapListener::new);
        inputTable.initializeWithSnapshot("by()-Snapshot", swapListener, (final boolean usePrev, final long beforeClockValue) -> {
            final ColumnSource<Index> resultIndexColumnSource = new SingleValueObjectColumnSource<>(inputTable.getIndex());
            final boolean empty = usePrev ? inputTable.getIndex().firstKeyPrev() == Index.NULL_KEY : inputTable.isEmpty();
            final QueryTable resultTable = new QueryTable(
                    ...
                    inputTable.getColumnSourceMap().entrySet().stream().collect(Collectors.toMap(
                            ...
                            (final Map.Entry<String, ColumnSource<?>> columnNameToSourceEntry) -> {
                                final AggregateColumnSource<DbArrayBase, ?> aggregateColumnSource = AggregateColumnSource.make(columnNameToSourceEntry.getValue(), resultIndexColumnSource);
            ...
            if (swapListener != null) {
                final ModifiedColumnSet.Transformer transformer = inputTable.newModifiedColumnSetTransformer(
                        ...
                        resultTable.getDefinition().getColumnNames().stream().map(resultTable::newModifiedColumnSet).toArray(ModifiedColumnSet[]::new));
                final ShiftAwareListener aggregationUpdateListener = new BaseTable.ShiftAwareListenerImpl("by()", inputTable, resultTable) {
                    @Override
                    public final void onUpdate(@NotNull final Update upstream) {
                        ...
                        if (wasEmpty) {
                            if (isEmpty) {
                                // empty -> empty: No change to report, we probably shouldn't even be notified
                                return;
                            }
                            ...
                            removed = Index.FACTORY.getEmptyIndex();
                            modified = Index.FACTORY.getFlatIndex(1);
                            transformer.clearAndTransform(upstream.modifiedColumnSet, modifiedColumnSet = resultTable.getModifiedColumnSetForUpdates());
                        } else {
                            // Only shifts: Nothing to report downstream, our data has not changed
                            return;
                        }
                        final Update downstream = new Update(added, removed, modified, IndexShiftData.EMPTY, modifiedColumnSet);
    ...
    private static QueryTable staticGroupedBy(@NotNull final Map<String, ColumnSource<?>> existingColumnSourceMap,
        ...
        final Pair<ArrayBackedColumnSource<?>, ObjectArraySource<Index>> flatResultColumnSources = AbstractColumnSource.groupingToFlatSources(keyColumnSource, groupToIndex);
        ...
        existingColumnSourceMap.entrySet().stream()
                .filter((final Map.Entry<String, ColumnSource<?>> columnNameToSourceEntry) -> !columnNameToSourceEntry.getKey().equals(keyColumnName))
                .forEachOrdered((final Map.Entry<String, ColumnSource<?>> columnNameToSourceEntry) ->
                        resultColumnSourceMap.put(columnNameToSourceEntry.getKey(), AggregateColumnSource.make(columnNameToSourceEntry.getValue(), resultIndexColumnSource)));
    ...
    private static QueryTable staticHashedBy(@NotNull final AggregationControl aggregationControl,
        ...
        final StaticChunkedByAggregationStateManager stateManager = new StaticChunkedByAggregationStateManager(maybeReinterpretedKeyColumnSources,
                aggregationControl.initialHashTableSize(inputTable), aggregationControl.getTargetLoadFactor(), aggregationControl.getMaximumLoadFactor());
        ...
        final int numGroups = stateManager.buildTable(inputTable, maybeReinterpretedKeyColumnSources, groupIndexToHashSlot);
        ...
        for (int kci = 0; kci < keyHashTableSources.length; ++kci) {
            ...
            if (keyColumnSources[kci] != maybeReinterpretedKeyColumnSources[kci]) {
                resultKeyColumnSource = ReinterpretUtilities.convertToOriginal(keyColumnSources[kci].getType(), resultKeyColumnSource);
            }
            resultColumnSourceMap.put(keyColumnNames[kci], new ReadOnlyRedirectedColumnSource<>(resultIndexToHashSlot, resultKeyColumnSource));
        }

        // Gather the result aggregate columns
        final ColumnSource<Index> resultIndexColumnSource = new ReadOnlyRedirectedColumnSource<>(resultIndexToHashSlot, stateManager.getIndexHashTableSource());
        Arrays.stream(aggregatedColumnNames)
                .forEachOrdered((final String aggregatedColumnName) ->
                        resultColumnSourceMap.put(aggregatedColumnName, AggregateColumnSource.make(existingColumnSourceMap.get(aggregatedColumnName), resultIndexColumnSource)));
    ...
    private static QueryTable incrementalHashedBy(@NotNull final AggregationControl aggregationControl,
        ...
        inputTable.initializeWithSnapshot("by(" + String.join(",", keyColumnNames) + "-Snapshot", swapListener, (final boolean usePrev, final long beforeClockValue) -> {
            // Reinterpret key column sources as primitives where possible
            final ColumnSource[] maybeReinterpretedKeyColumnSources = maybeReinterpretKeyColumnSources(keyColumnSources);

            // Prepare our state manager
            final IncrementalChunkedByAggregationStateManager stateManager = new IncrementalChunkedByAggregationStateManager(maybeReinterpretedKeyColumnSources,
                    aggregationControl.initialHashTableSize(inputTable), aggregationControl.getTargetLoadFactor(), aggregationControl.getMaximumLoadFactor());

            // Prepare our update tracker
            final IncrementalByAggregationUpdateTracker updateTracker = new IncrementalByAggregationUpdateTracker();

            // Perform the initial aggregation pass
            if (usePrev) {
                stateManager.buildInitialTableFromPrevious(inputTable, maybeReinterpretedKeyColumnSources, updateTracker);
            } else {
                stateManager.buildInitialTableFromCurrent(inputTable, maybeReinterpretedKeyColumnSources, updateTracker);

            }
            // Compute result index and redirection to hash slots
            final RedirectionIndex resultIndexToHashSlot = RedirectionIndexLockFreeImpl.FACTORY.createRedirectionIndex(updateTracker.size());
            final Index resultIndex = updateTracker.applyAddsAndMakeInitialIndex(stateManager.getIndexSource(), stateManager.getOverflowIndexSource(), resultIndexToHashSlot);
            ...
            Arrays.stream(aggregatedColumnNames)
                    .forEachOrdered((final String aggregatedColumnName) -> {
                        final AggregateColumnSource aggregatedColumnSource = AggregateColumnSource.make(existingColumnSourceMap.get(aggregatedColumnName), resultIndexColumnSource);
            ...
            final ModifiedColumnSet upstreamKeyColumnInputs = inputTable.newModifiedColumnSet(keyColumnUpstreamInputColumnNames.toArray(ZERO_LENGTH_STRING_ARRAY));
            final ModifiedColumnSet downstreamAllAggregatedColumns = resultTable.newModifiedColumnSet(aggregatedColumnNames);
            final ModifiedColumnSet.Transformer aggregatedColumnsTransformer = inputTable.newModifiedColumnSetTransformer(
                    aggregatedColumnNames,
                    Arrays.stream(aggregatedColumnNames).map(resultTable::newModifiedColumnSet).toArray(ModifiedColumnSet[]::new));

            // Handle updates
            final ShiftAwareListener aggregationUpdateListener = new BaseTable.ShiftAwareListenerImpl("by(" + String.join(",", keyColumnNames) + ')', inputTable, resultTable) {
                @Override
                public final void onUpdate(@NotNull final Update upstream) {
                    ...
                    final boolean keyColumnsModified = upstream.modifiedColumnSet.containsAny(upstreamKeyColumnInputs);

                    if (keyColumnsModified) {
                        try (final Index toRemove = upstream.removed.union(upstream.getModifiedPreShift())) {
                            stateManager.processRemoves(maybeReinterpretedKeyColumnSources, toRemove, updateTracker);
                        }
                    } else {
                        stateManager.processRemoves(maybeReinterpretedKeyColumnSources, upstream.removed, updateTracker);
                    }
                    updateTracker.applyRemovesToStates(stateManager.getIndexSource(), stateManager.getOverflowIndexSource());

                    if (upstream.shifted.nonempty()) {
                        upstream.shifted.apply((final long beginRange, final long endRange, final long shiftDelta) -> {
                            final Index shiftedPreviousIndex;
                            try (final Index previousIndex = inputTable.getIndex().getPrevIndex()) {
                                shiftedPreviousIndex = previousIndex.subindexByKey(beginRange, endRange);
                            }
                            try {
                                if (aggregationControl.shouldProbeShift(shiftedPreviousIndex.size(), resultIndex.intSize())) {
                                    stateManager.processShift(maybeReinterpretedKeyColumnSources, shiftedPreviousIndex, updateTracker);
                                    updateTracker.applyShiftToStates(stateManager.getIndexSource(), stateManager.getOverflowIndexSource(), beginRange, endRange, shiftDelta);
                                } else {
                                    resultIndex.forAllLongs((final long stateKey) -> {
                                        final int stateSlot = (int) resultIndexToHashSlot.get(stateKey);
                                        stateManager.applyShift(stateSlot, beginRange, endRange, shiftDelta, updateTracker::processAppliedShift);
                                    });
                                }
                            } finally {
                    ...
                    if (keyColumnsModified) {
                        ...
                        stateManager.processModifies(maybeReinterpretedKeyColumnSources, upstream.modified, updateTracker);
                        stateManager.processAdds(maybeReinterpretedKeyColumnSources, upstream.added, updateTracker);
                    }
                    updateTracker.applyAddsToStates(stateManager.getIndexSource(), stateManager.getOverflowIndexSource());

                    final Update downstream = updateTracker.makeUpdateFromStates(
                            stateManager.getIndexSource(), stateManager.getOverflowIndexSource(), resultIndex, resultIndexToHashSlot,
                            (final boolean someKeyHasAddsOrRemoves, final boolean someKeyHasModifies) -> {
                                ...
                                if (someKeyHasModifies) {
                                    aggregatedColumnsTransformer.clearAndTransform(upstream.modifiedColumnSet, resultTable.getModifiedColumnSetForUpdates());
    ...
    public static LocalTableMap byExternal(@NotNull final AggregationControl aggregationControl,
        ...
        final ColumnSource[] keyColumnSources = Arrays.stream(keyColumnNames).map(inputTable::getColumnSource).toArray(ColumnSource[]::new);
        final QueryTable subTableSource = dropKeyColumns ? (QueryTable) inputTable.dropColumns(keyColumnNames) : inputTable;
        ...
        if (groupingForAggregation != null) {
            ...
            AbstractColumnSource.forEachResponsiveGroup(groupingForAggregation, inputTable.getIndex(),
                    (final Object key, final Index index) -> staticGroupedResult.put(key, subTableSource.getSubTable(index)));
    ...
    private static LocalTableMap staticHashedByExternal(@NotNull final AggregationControl aggregationControl,
        ...
        final TupleSource<?> inputKeyIndexToMapKeySource = keyColumnSources.length == 1 ? keyColumnSources[0] : new SmartKeySource(keyColumnSources);
        ...
        try (final OrderedKeys groupIndices = CurrentOnlyIndex.FACTORY.getFlatIndex(numGroups);
             ...
             final WritableObjectChunk<Index, Values> aggregatedIndexes = WritableObjectChunk.makeWritableChunk(chunkSize);
             final WritableLongChunk<OrderedKeyIndices> mapKeySourceIndices = WritableLongChunk.makeWritableChunk(chunkSize);
            ...
            while (groupIndicesIterator.hasMore()) {
                final OrderedKeys groupIndexesForThisChunk = groupIndicesIterator.getNextOrderedKeysWithLength(chunkSize);
                final int groupsInThisChunk = groupIndexesForThisChunk.intSize();
                final LongChunk<Values> hashSlots = groupIndexToHashSlot.getChunk(hashSlotGetContext, groupIndexesForThisChunk).asLongChunk();
                ...
                try (final OrderedKeys inputKeyIndices = OrderedKeys.wrapKeyIndicesChunkAsOrderedKeys(mapKeySourceIndices)) {
    ...
    @Nullable
    private static Map<Object, Index> maybeGetGroupingForAggregation(@NotNull final AggregationControl aggregationControl,
                                                                     @NotNull final QueryTable inputTable,
                                                                     @NotNull final ColumnSource[] keyColumnSources) {
    ...
    private static ColumnSource[] maybeReinterpretKeyColumnSources(@NotNull final ColumnSource[] keyColumnSources) {
        ...
        for (int kci = 0; kci < keyColumnSources.length; ++kci) {
            maybeReinterpretedKeyColumnSources[kci] = ReinterpretUtilities.maybeConvertToPrimitive(keyColumnSources[kci]);
        }
