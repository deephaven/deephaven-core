...
public interface WritableChunkSink<ATTR extends Attributes.Any> extends ChunkSource<ATTR> {
    ...
    /**
     * Fills the ChunkSink with data from the source, with data corresponding to the keys from the given {@link OrderedKeys}.
     ...
     */
    void fillFromChunk(@NotNull FillFromContext context, @NotNull Chunk<? extends ATTR> src, @NotNull OrderedKeys orderedKeys);
     ...
     */
    void fillFromChunkUnordered(@NotNull FillFromContext context, @NotNull Chunk<? extends ATTR> src, @NotNull LongChunk<KeyIndices> keys);
