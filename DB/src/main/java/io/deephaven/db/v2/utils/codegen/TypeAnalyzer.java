...
/**
 * When {@code type} is either a primitive type or a boxed type, {@code dbPrimitiveType} is the type
 * used by the DB to represent it -- that is, Boolean for either boolean or Boolean results, but the
 * primitive version of all other primitive/boxed types. For all other types (e.g. misc Objects),
 * {@code dbPrimitiveType} is null.
 **/
public class TypeAnalyzer {
    public static TypeAnalyzer create(Class type) {
        ...
        if (type == Boolean.class || type == boolean.class) {
            ...
            if (dbPrimitiveType == null) {
                chunkVariableBase = "ObjectChunk";
                chunkExtendsGenericArgs =
                    String.format("<%s, ? extends Attributes.Values>", type.getCanonicalName());
                chunkSuperGenericArgs =
                    String.format("<%s, ? super Attributes.Values>", type.getCanonicalName());
                chunkTypeString = "Object";
            } else {
                final String simpleName = dbPrimitiveType.getSimpleName();
                final String camelCasedName =
        ...
        final String writeChunkVariableType =
            "Writable" + chunkVariableBase + chunkSuperGenericArgs;
        final String asReadMethod = "as" + chunkVariableBase;
        final String asWritableMethod = "asWritable" + chunkVariableBase;
        return new TypeAnalyzer(type, dbPrimitiveType, returnTypeName, chunkTypeString,
            readChunkVariableType,
    ...
    /**
     * A DhFormulaColumn will unbox the result of any formula that returns a boxed type (except
     * Boolean). If the formula returns null, this could trigger a NullPointerException.
     *
    ...
    public String wrapWithCastIfNecessary(String formulaString) {
        ...
        return DBLanguageFunctionUtil.class.getCanonicalName() + '.' + dbPrimitiveType.getName()
            + "Cast(" + formulaString + ')';
    }
}
