...
public abstract class MergedListener extends LivenessArtifact implements NotificationQueue.Dependency {
    ...
    public void notifyChanges() {
        ...
        LiveTableMonitor.DEFAULT.addNotification(new AbstractNotification(false) {
            ...
            public void run() {
                try {
                    if (queuedNotificationClock != LogicalClock.DEFAULT.currentStep()) {
                        throw Assert.statementNeverExecuted("Notification step mismatch: listener=" + System.identityHashCode(MergedListener.this) + ": queuedNotificationClock=" + queuedNotificationClock + ", step=" + LogicalClock.DEFAULT.currentStep());
                    }
                    ...
                    try {
                        synchronized (MergedListener.this) {
                            if (notificationClock == queuedNotificationClock) {
                                throw Assert.statementNeverExecuted("Multiple notifications in the same step: listener=" + System.identityHashCode(MergedListener.this) + ", queuedNotificationClock=" + queuedNotificationClock);
                            }
                            notificationClock = queuedNotificationClock;
                        }
                        process();
                        LiveTableMonitor.DEFAULT.logDependencies().append("MergedListener has completed execution ").append(this).endl();
                    } finally {
                        entry.onUpdateEnd();
            ...
            public LogOutput append(LogOutput logOutput) {
                return logOutput.append("Merged Notification ").append(System.identityHashCode(MergedListener.this)).append(" ").append(listenerDescription);
            }
    ...
    protected boolean canExecute(final long step) {
        return Stream.concat(recorders.stream(), dependencies.stream()).allMatch((final NotificationQueue.Dependency dep) -> dep.satisfied(step));
    }

    @Override
    public boolean satisfied(final long step) {
        if (lastCompletedStep == step) {
            LiveTableMonitor.DEFAULT.logDependencies().append("MergedListener has previously been completed ").append(this).endl();
            return true;
        }
        if (queuedNotificationClock == step) {
            LiveTableMonitor.DEFAULT.logDependencies().append("MergedListener has queued notification ").append(this).endl();
            return false;
        }
        if (canExecute(step)) {
            LiveTableMonitor.DEFAULT.logDependencies().append("MergedListener has dependencies satisfied ").append(this).endl();
            // mark this node as completed, because both our parents have been satisfied; but we are not enqueued; so we can never actually execute
