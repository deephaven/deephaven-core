...
public final class CharSegmentedSortedMultiset implements SegmentedSortedMultiSet<Character>, DbCharArray {
    ...
    private int insertExistingIntoLeaf(WritableCharChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, char [] leafValues, long [] leafCounts, char maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        char nextValue;
        while (rlpos < leafSize && ripos < valuesToInsert.size() && (leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
            if (gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    ...
                    valuesToInsert.copyFromTypedChunk((WritableCharChunk)valuesToInsert, ripos, wipos.intValue(), lastInsert - ripos);
    ...
    private void distributeNewIntoLeaves(CharChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, final int insertStart, final int insertCount, int firstLeaf, int requiredLeaves, int newLeafSize) {
        ...
        for (int li = firstLeaf + 1; li < firstLeaf + requiredLeaves; ++li) {
            leafValues[li] = new char[leafSize];
        ...
        while (remaining-- > 0) {
            final char insertValue = valuesToInsert.get(ripos);
            final char leafValue = leafValues[firstLeaf][rlpos];
    ...
    private void insertNewIntoLeaf(WritableCharChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, char [] leafValues, long [] leafCounts) {
        ...
        while (wpos >= 0) {
            final char insertValue = valuesToInsert.get(ripos);
            final char leafValue = leafValues[rlpos];
    ...
    private void copyRemainingValuesToLeaf(WritableCharChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, char[] leafValues, long[] leafCounts, int ripos) {
    ...
    private void maybeCompact(WritableCharChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, int wipos) {
        if (wipos == ripos) {
            return;
        }
    ...
    private void insertExisting(WritableCharChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        if (leafCount == 0) {
            return;
        }
        if (leafCount == 1) {
            final MutableInt wipos = new MutableInt(0);
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, 0, wipos, size, directoryValues, directoryCount, NULL_CHAR, true);
    ...
    private void insert(WritableCharChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        ...
        if (leafCount > 1 && gt(valuesToInsert.get(0), getMaxChar())) {
        ...
        do {
            final char insertValue = valuesToInsert.get(rpos);
    ...
    private void moveDirectoryToLeaf(int desiredLeafCount, int directoryLocation) {
        leafValues = new char[desiredLeafCount][];
    ...
    private void makeLeafHole(int holePosition, int holeSize) {
        ...
        if (holePosition + holeSize < leafValues.length) {
            Arrays.fill(directoryValues, holePosition, holePosition + holeSize, NULL_CHAR);
        } else {
            Arrays.fill(directoryValues, holePosition, holePosition + holeSize - 1, NULL_CHAR);
        }
        // endregion fillValue
    }
    ...
    private void allocateLeafArrays(int newSize) {
        leafValues = new char[newSize][];
        leafCounts = new long[newSize][];
        leafSizes = new int[newSize];
        directoryValues = new char[newSize - 1];
    }
    ...
    private void packValuesIntoLeaves(CharChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int rpos, int startLeaf, int valuesPerLeaf) {
     ...
     */
    private static int lowerBound(char [] valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(CharChunk<? extends Attributes.Any> valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(char [] valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch[mid];
            final boolean moveLo = leq(testValue, searchValue);
     ...
     */
    private static int lowerBoundExclusive(char [] valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch[mid];
    ...
    private void remove(RemoveContext removeContext, CharChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts) {
        validate();
        ...
        if (leafCount == 1) {
            ...
            do {
                ...
                final char firstValueToRemove = valuesToRemove.get(rpos);
    ...
    private int removeFromLeaf(RemoveContext removeContext, CharChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts, int ripos, int end, char[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
            final char removeValue = valuesToRemove.get(ripos);
    ...
    private int compactValues(RemoveContext removeContext, char[] leafValues, long[] leafCounts, int sz, int cl) {
        int removed = 0;
        for (int cli = 0; cli <= cl; cli++) {
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    if (ii < leafCount - 2) {
                        final char nextDirectoryValue = directoryValues[ii + 1];
...
//                }
    ...
    private static void validateLeaf(char[] values, long[] counts, int size) {
        Assert.gtZero(size, "size");
        for (int ii = 0; ii < size - 1; ++ii) {
            Assert.gtZero(counts[ii], "counts[ii]");
            final char thisValue = values[ii];
            final char nextValue = values[ii + 1];
            Assert.assertion(lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])", (Character)thisValue, "values[ii]", (Character)nextValue, "values[ii + 1]", ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        final CharSegmentedSortedMultiset destination = (CharSegmentedSortedMultiset)untypedDestination;
        validate();
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(geq(getMinChar(), destination.getMaxChar()), "geq(getMinChar(), destination.getMaxChar())");
            }
        }
            ...
            if (leafCount == 1) {
                ...
                for (int rli = 0; rli < rleaf; ++rli) {
                    ...
                    if (rli == rleaf - 1 && leftOver > 0 && partialUnique == 0) {
                        ...
                        final char [] tmpValues = new char[leafSize];
        ...
        if (partialUnique > 0) {
            final char [] sourceValues;
            final long [] sourceCounts;
            final char [] destinationValues;
            ...
            if (appendToExtra) {
                ...
                destinationValues = destination.leafValues[wleaf] = new char[leafSize];
    ...
    private boolean prepareAppend(int finalSlots, int completeLeaves) {
        ...
        if (completeLeaves == 0) {
            ...
            if (leafCount == 0) {
                directoryValues = new char[finalSlots];
            ...
            if (extraLeaf) {
                leafValues[leafCount + completeLeaves] = new char[leafSize];
    ...
    private boolean preparePrepend(int initialSlots, int completeLeaves) {
        ...
        if (completeLeaves > 0) {
            ...
            if (leafCount == 0) {
                ...
                extraLeafCount = 1;
                directoryValues = new char[initialSlots];
        ...
        if (extraLeafCount == 0 && initialSlots > 0) {
            ...
            if (directoryCount != null) {
                final char [] tmpValues = new char[initialSlots + size];
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(leq(getMaxChar(), destination.getMinChar()), "leq(getMaxChar(), destination.getMinChar())");
            }
        }
        ...
        if (slotsInPartialLeaf > 0) {
            ...
            final char [] destValues;
            final long [] destCounts;

            final char [] srcValues;
            ...
            if (destination.directoryCount != null) {
                ...
                if (extraLeaf) {
                    destination.leafValues[0] = new char[leafSize];
        ...
        if (completeLeavesToMove > 0) {
            if (leafCount == 1) {
                ...
                if (hasLeftOverSlot) {
                    ...
                    leafValues[rleaf + 1] = new char[leafSize];
                ...
                if (directoryMoves > 0) {
                    Arrays.fill(directoryValues, firstLeafTozero, firstLeafTozero + directoryMoves - (completeLeavesToMove - numberOfLeavesToRemove), NULL_CHAR);
                }
                maybePromoteLastLeaf();
            }
            destination.maybePromoteLastLeaf();
    ...
    @Override
    public CharChunk<?> keyChunk() {
        final WritableCharChunk keyChunk = WritableCharChunk.makeWritableChunk(intSize());
    ...
    public void fillKeyChunk(WritableChunk<?> keyChunk, int offset) {
        fillKeyChunk(keyChunk.asWritableCharChunk(), offset);
    }

    private void fillKeyChunk(WritableCharChunk<?> keyChunk, int offset) {
    ...
    private void maybeAccumulateRemoval(char valueRemoved) {
        if(!accumulateDeltas) {
            return;
        }
    ...
    public boolean equals(Object o) {
        ...
        final CharSegmentedSortedMultiset that = (CharSegmentedSortedMultiset) o;
