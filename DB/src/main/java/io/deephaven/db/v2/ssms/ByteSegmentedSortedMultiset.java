...
public final class ByteSegmentedSortedMultiset implements SegmentedSortedMultiSet<Byte>, DbByteArray {
    ...
    public boolean insert(WritableChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        ...
        insert(valuesToInsert.asWritableByteChunk(), counts);
        ...
        while (rlpos < leafSize && ripos < valuesToInsert.size() && (leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
            if (gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    ...
                    valuesToInsert.copyFromTypedChunk((WritableByteChunk)valuesToInsert, ripos, wipos.intValue(), lastInsert - ripos);
    ...
    private void distributeNewIntoLeaves(ByteChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, final int insertStart, final int insertCount, int firstLeaf, int requiredLeaves, int newLeafSize) {
        ...
        for (int li = firstLeaf + 1; li < firstLeaf + requiredLeaves; ++li) {
            leafValues[li] = new byte[leafSize];
    ...
    private void insertNewIntoLeaf(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, byte [] leafValues, long [] leafCounts) {
    ...
    private void copyRemainingValuesToLeaf(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, byte[] leafValues, long[] leafCounts, int ripos) {
    ...
    private void maybeCompact(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, int wipos) {
        if (wipos == ripos) {
            return;
        }
        ...
        valuesToInsert.copyFromTypedChunk((ByteChunk)valuesToInsert, ripos, wipos, toCopy);
        counts.copyFromChunk(counts, ripos, wipos, toCopy);
        valuesToInsert.setSize(wipos + toCopy);
        counts.setSize(wipos + toCopy);
    }

    private void insertExisting(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        if (leafCount == 0) {
            return;
        }
        if (leafCount == 1) {
            final MutableInt wipos = new MutableInt(0);
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, 0, wipos, size, directoryValues, directoryCount, NULL_BYTE, true);
        ...
        while (ripos < valuesToInsert.size()) {
            final byte startValue = valuesToInsert.get(ripos);
            ...
            final byte maxValue = lastLeaf ? NULL_BYTE : directoryValues[nextLeaf];
    ...
    private void insert(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        validate();
        ...
        do {
            final byte insertValue = valuesToInsert.get(rpos);
            ...
            if (nextLeaf == leafCount - 1) {
                ...
                final byte lastLeafValue = directoryValues[nextLeaf];
    ...
    private void moveDirectoryToLeaf(int desiredLeafCount, int directoryLocation) {
        leafValues = new byte[desiredLeafCount][];
        ...
        directoryCount = null;
        directoryValues = new byte[desiredLeafCount - 1];
    }

    private void doAppend(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
    ...
    private void makeLeafHole(int holePosition, int holeSize) {
        ...
        if (holePosition + holeSize < leafValues.length) {
            Arrays.fill(directoryValues, holePosition, holePosition + holeSize, NULL_BYTE);
        } else {
            Arrays.fill(directoryValues, holePosition, holePosition + holeSize - 1, NULL_BYTE);
        }
        // endregion fillValue
    }
    ...
    private void allocateLeafArrays(int newSize) {
        leafValues = new byte[newSize][];
        leafCounts = new long[newSize][];
        leafSizes = new int[newSize];
        directoryValues = new byte[newSize - 1];
    }
    ...
    private void makeLeavesInitial(ByteChunk<? extends Values> values, IntChunk<ChunkLengths> counts) {
    ...
    private void packValuesIntoLeaves(ByteChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int rpos, int startLeaf, int valuesPerLeaf) {
        while (rpos < values.size()) {
            final int thisLeafSize = Math.min(valuesPerLeaf, values.size() - rpos);
            leafSizes[startLeaf] = thisLeafSize;
            leafValues[startLeaf] = new byte[leafSize];
     ...
     */
    private static int lowerBound(byte [] valuesToSearch, int lo, int hi, byte searchValue) {
        while (lo < hi) {
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(ByteChunk<? extends Attributes.Any> valuesToSearch, int lo, int hi, byte searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(byte [] valuesToSearch, int lo, int hi, byte searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final byte testValue = valuesToSearch[mid];
            final boolean moveLo = leq(testValue, searchValue);
     ...
     */
    private static int lowerBoundExclusive(byte [] valuesToSearch, int lo, int hi, byte searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final byte testValue = valuesToSearch[mid];
    ...
    public boolean remove(RemoveContext removeContext, WritableChunk<? extends Values> valuesToRemove, WritableIntChunk<ChunkLengths> counts) {
        final long beforeSize = size();
        remove(removeContext, valuesToRemove.asByteChunk(), counts);
        return beforeSize != size();
    }

    private void remove(RemoveContext removeContext, ByteChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts) {
        validate();
        ...
        if (leafCount == 1) {
            ...
            do {
                ...
                final byte firstValueToRemove = valuesToRemove.get(rpos);
    ...
    private int removeFromLeaf(RemoveContext removeContext, ByteChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts, int ripos, int end, byte[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
            final byte removeValue = valuesToRemove.get(ripos);
    ...
    private int compactValues(RemoveContext removeContext, byte[] leafValues, long[] leafCounts, int sz, int cl) {
        int removed = 0;
        for (int cli = 0; cli <= cl; cli++) {
    ...
    private void validateInputs(ByteChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        if (!SEGMENTED_SORTED_MULTISET_VALIDATION) {
            return;
        }
...
//                }
    ...
    private static void validateLeaf(byte[] values, long[] counts, int size) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        final ByteSegmentedSortedMultiset destination = (ByteSegmentedSortedMultiset)untypedDestination;
        validate();
            ...
            if (leafCount == 1) {
                ...
                for (int rli = 0; rli < rleaf; ++rli) {
                    ...
                    if (rli == rleaf - 1 && leftOver > 0 && partialUnique == 0) {
                        ...
                        final byte [] tmpValues = new byte[leafSize];
        ...
        if (partialUnique > 0) {
            final byte [] sourceValues;
            final long [] sourceCounts;
            final byte [] destinationValues;
            ...
            if (appendToExtra) {
                ...
                destinationValues = destination.leafValues[wleaf] = new byte[leafSize];
    ...
    private boolean prepareAppend(int finalSlots, int completeLeaves) {
        ...
        if (completeLeaves == 0) {
            ...
            if (leafCount == 0) {
                directoryValues = new byte[finalSlots];
            ...
            if (extraLeaf) {
                leafValues[leafCount + completeLeaves] = new byte[leafSize];
    ...
    private boolean preparePrepend(int initialSlots, int completeLeaves) {
        ...
        if (completeLeaves > 0) {
            ...
            if (leafCount == 0) {
                ...
                extraLeafCount = 1;
                directoryValues = new byte[initialSlots];
        ...
        if (extraLeafCount == 0 && initialSlots > 0) {
            ...
            if (directoryCount != null) {
                final byte [] tmpValues = new byte[initialSlots + size];
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (slotsInPartialLeaf > 0) {
            ...
            final byte [] destValues;
            final long [] destCounts;

            final byte [] srcValues;
            ...
            if (destination.directoryCount != null) {
                ...
                if (extraLeaf) {
                    destination.leafValues[0] = new byte[leafSize];
        ...
        if (completeLeavesToMove > 0) {
            if (leafCount == 1) {
                ...
                if (hasLeftOverSlot) {
                    ...
                    leafValues[rleaf + 1] = new byte[leafSize];
                ...
                if (directoryMoves > 0) {
                    Arrays.fill(directoryValues, firstLeafTozero, firstLeafTozero + directoryMoves - (completeLeavesToMove - numberOfLeavesToRemove), NULL_BYTE);
                }
                maybePromoteLastLeaf();
            }
            destination.maybePromoteLastLeaf();
    ...
    public void fillKeyChunk(WritableChunk<?> keyChunk, int offset) {
        fillKeyChunk(keyChunk.asWritableByteChunk(), offset);
    }

    private void fillKeyChunk(WritableByteChunk<?> keyChunk, int offset) {
     ...
     * @return
     */
    private byte[] keyArray(long first, long last) {
        if(isEmpty()) {
            return ArrayUtils.EMPTY_BYTE_ARRAY;
        }

        final int totalSize = (int)(last - first + 1);
        final byte[] keyArray = new byte[totalSize];
        if (leafCount == 1) {
    ...
    private void maybeAccumulateAdditions(WritableByteChunk<? extends Values> valuesToInsert) {
        ...
        if (added == null) {
            added = new TByteHashSet(valuesToInsert.size());
        }
        ...
        if(removed == null) {
            ...
            for (int ii = 0; ii < valuesToInsert.size(); ii++) {
                byte val = valuesToInsert.get(ii);
    ...
    private void maybeAccumulateRemoval(byte valueRemoved) {
        if(!accumulateDeltas) {
            return;
        }
    ...
    public boolean equals(Object o) {
        ...
        final ByteSegmentedSortedMultiset that = (ByteSegmentedSortedMultiset) o;
