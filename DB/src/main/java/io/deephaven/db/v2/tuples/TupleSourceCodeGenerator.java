...
public class TupleSourceCodeGenerator {
    ...
    private static final File OUTPUT_RELATIVE_PATH = new File(new File("DB", MAIN_SRC), OUTPUT_PACKAGE.replace('.', File.separatorChar));
    ...
    enum ColumnSourceType {
        ...
        ColumnSourceType(@NotNull final String nameText,
            ...
            this.internalClass = overrideInternalClass == null ? elementClass : overrideInternalClass;
            ...
            this.imports = Stream.concat(Stream.of(chunkClass), Arrays.stream(importClasses)).map(Class::getName).toArray(String[]::new);
        }
        ...
        private String getExportElementText(@NotNull final String valueName) {
            return elementClass != internalClass || isReinterpreted || this == OBJECT ? "(ELEMENT_TYPE) " + getBoxingText(valueName) : valueName;
        }
    ...
    private static ColumnSourceType forPrimitive(final Class<?> clazz) {
        return Arrays.stream(ColumnSourceType.values()).filter(cst -> cst.getElementClass() == clazz).findFirst().orElseThrow(() -> new RuntimeException("Could not find type to: " + clazz));
    }

    private static String generateSimpleClassName(@NotNull final ColumnSourceType... types) {
        return Arrays.stream(types).map(ColumnSourceType::getNameText).collect(Collectors.joining()) + CLASS_NAME_SUFFIX;
    }
    ...
    private static void addReinterpretedUnboxing(@NotNull ColumnSourceType type1, Indenter indenter, StringBuilder code, String valueName, boolean comma) {
        if (type1.isReinterpreted()) {
            code.append(indenter).append(forPrimitive(type1.elementClass).getUnboxingText(valueName)).append(comma ? "," : "").append(NEW_LINE);
        } else {
            code.append(indenter).append(type1.getUnboxingText(valueName)).append(comma ? "," : "").append(NEW_LINE);
        }
    }

    private static void addReinterpretedExport(@NotNull ColumnSourceType type1, Indenter indenter, StringBuilder code, String valueName) {
        if (type1.isReinterpreted()) {
            code.append(indenter.increaseLevel()).append("return ").append(forPrimitive(type1.elementClass).getBoxingText(valueName)).append(";").append(NEW_LINE);
        } else {
            code.append(indenter.increaseLevel()).append("return ").append(type1.getBoxingText(valueName)).append(";").append(NEW_LINE);

        }
    }

    private String generateTwoColumnTupleSource(@NotNull final String className, @NotNull final ColumnSourceType type1, @NotNull final ColumnSourceType type2) {
        final Indenter indenter = new Indenter();
        final StringBuilder code = new StringBuilder(1024);
        final String sourceClass1Name = TypeUtils.getBoxedType(type1.getElementClass()).getSimpleName();
        final String sourceClass2Name = TypeUtils.getBoxedType(type2.getElementClass()).getSimpleName();
        final String tupleClassName = TupleCodeGenerator.getTupleClassName(type1.getInternalClass(), type2.getInternalClass());
        final String [] extraImports = new String[] { TupleCodeGenerator.getTupleImport(tupleClassName), TwoColumnTupleSourceFactory.class.getName() };

        code.append("package ").append(OUTPUT_PACKAGE).append(';').append(NEW_LINE);

        code.append(NEW_LINE);

        Stream.of(DEFAULT_IMPORTS, type1.getImports(), type2.getImports(), extraImports).flatMap(Arrays::stream).filter(i -> !i.startsWith("java.")).sorted().distinct().forEachOrdered(
        ...
        code.append(" * <p>{@link TupleSource} that produces key column values from {@link ColumnSource} types ").append(sourceClass1Name).append(" and ").append(sourceClass2Name).append('.').append(NEW_LINE);
        code.append(" * <p>Generated by {@link ").append(TupleSourceCodeGenerator.class.getName()).append("}.").append(NEW_LINE);
        code.append(" */").append(NEW_LINE);
        code.append("@SuppressWarnings({\"unused\", \"WeakerAccess\"})").append(NEW_LINE);
        code.append("public class ").append(className).append(" extends AbstractTupleSource<").append(tupleClassName).append("> {").append(NEW_LINE);

        code.append(NEW_LINE);

        code.append(indenter).append("/** {@link ").append(TwoColumnTupleSourceFactory.class.getSimpleName()).append("} instance to create instances of {@link ").append(className).append("}. **/").append(NEW_LINE);
        code.append(indenter).append("public static final ").append(TwoColumnTupleSourceFactory.class.getSimpleName()).append('<')
        ...
        code.append(indenter).append("private final ColumnSource<").append(sourceClass1Name).append("> ").append(CS1).append(';').append(NEW_LINE);
        code.append(indenter).append("private final ColumnSource<").append(sourceClass2Name).append("> ").append(CS2).append(';').append(NEW_LINE);

        code.append(NEW_LINE);

        code.append(indenter).append("public ").append(className).append('(').append(NEW_LINE);
        indenter.increaseLevel(2);
        code.append(indenter).append("@NotNull final ColumnSource<").append(sourceClass1Name).append("> ").append(CS1).append(',').append(NEW_LINE);
        code.append(indenter).append("@NotNull final ColumnSource<").append(sourceClass2Name).append("> ").append(CS2).append(NEW_LINE);
        ...
        code.append(indenter).append("public final ").append(tupleClassName).append(" createTuple(final long ").append(IK).append(") {").append(NEW_LINE);
        indenter.increaseLevel();
        ...
        code.append(indenter).append("@Override").append(NEW_LINE);
        code.append(indenter).append("public final ").append(tupleClassName).append(" createPreviousTuple(final long ").append(IK).append(") {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("return new ").append(tupleClassName).append('(').append(NEW_LINE);
        indenter.increaseLevel(2);
        ...
        code.append(indenter).append("@Override").append(NEW_LINE);
        code.append(indenter).append("public final ").append(tupleClassName).append(" createTupleFromValues(@NotNull final Object... values) {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("return new ").append(tupleClassName).append('(').append(NEW_LINE);
        indenter.increaseLevel(2);
        ...
        code.append(indenter).append("@Override").append(NEW_LINE);
        code.append(indenter).append("public final ").append(tupleClassName).append(" createTupleFromReinterpretedValues(@NotNull final Object... values) {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("return new ").append(tupleClassName).append('(').append(NEW_LINE);
        indenter.increaseLevel(2);
        ...
        code.append(indenter).append("@Override").append(NEW_LINE);
        code.append(indenter).append("public final <ELEMENT_TYPE> void exportElement(@NotNull final ").append(tupleClassName).append(" tuple, final int elementIndex, @NotNull final WritableSource<ELEMENT_TYPE> writableSource, final long destinationIndexKey) {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("if (elementIndex == 0) {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("writableSource.set(destinationIndexKey, ").append(type1.getExportElementText("tuple.getFirstElement()")).append(");").append(NEW_LINE);
        code.append(indenter).append("return;").append(NEW_LINE);
        indenter.decreaseLevel();
        code.append(indenter).append('}').append(NEW_LINE);
        code.append(indenter).append("if (elementIndex == 1) {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("writableSource.set(destinationIndexKey, ").append(type2.getExportElementText("tuple.getSecondElement()")).append(");").append(NEW_LINE);
        code.append(indenter).append("return;").append(NEW_LINE);
        indenter.decreaseLevel();
        code.append(indenter).append('}').append(NEW_LINE);
        code.append(indenter).append("throw new IndexOutOfBoundsException(\"Invalid element index \" + elementIndex + \" for export\");").append(NEW_LINE);
        indenter.decreaseLevel();
        code.append(indenter).append('}').append(NEW_LINE);
        ...
        code.append(indenter).append("@Override").append(NEW_LINE);
        code.append(indenter).append("public final Object exportToExternalKey(@NotNull final ").append(tupleClassName).append(" tuple) {").append(NEW_LINE);
        indenter.increaseLevel();
        ...
        code.append(indenter).append("@Override").append(NEW_LINE);
        code.append(indenter).append("public final Object exportElement(@NotNull final ").append(tupleClassName).append(" tuple, int elementIndex) {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("if (elementIndex == 0) {").append(NEW_LINE);
        code.append(indenter.increaseLevel()).append("return ").append(type1.getBoxingText("tuple.getFirstElement()")).append(";").append(NEW_LINE);
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(indenter).append("if (elementIndex == 1) {").append(NEW_LINE);
        code.append(indenter.increaseLevel()).append("return ").append(type2.getBoxingText("tuple.getSecondElement()")).append(";").append(NEW_LINE);
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(indenter).append("throw new IllegalArgumentException(\"Bad elementIndex for 2 element tuple: \" + elementIndex);").append(NEW_LINE);
        indenter.decreaseLevel();
        code.append(indenter).append('}').append(NEW_LINE);
        ...
        code.append(indenter).append("@Override").append(NEW_LINE);
        code.append(indenter).append("public final Object exportElementReinterpreted(@NotNull final ").append(tupleClassName).append(" tuple, int elementIndex) {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("if (elementIndex == 0) {").append(NEW_LINE);
        ...
        code.append(indenter).append(type2.getValuesChunkTypeString()).append(" chunk2 = chunks[1].as").append(type2.chunkType).append("Chunk();").append(NEW_LINE);
        code.append(indenter).append("for (int ii = 0; ii < chunkSize; ++ii) {").append(NEW_LINE);
        code.append(indenter.increaseLevel()).append("destinationObjectChunk.set(ii, new ").append(tupleClassName).append("(").append(type1.getFromChunkText("chunk1.get(ii)")).append(", ").append(type2.getFromChunkText("chunk2.get(ii)")).append("));").append(NEW_LINE);
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(indenter).append("destination.setSize(chunkSize);").append(NEW_LINE);
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(NEW_LINE);

        code.append(indenter).append("/** {@link ").append(TwoColumnTupleSourceFactory.class.getSimpleName()).append("} for instances of {@link ").append(className).append("}. **/").append(NEW_LINE);
        code.append(indenter).append("private static final class Factory implements ").append(TwoColumnTupleSourceFactory.class.getSimpleName())
                .append('<').append(tupleClassName).append(", ").append(sourceClass1Name).append(", ").append(sourceClass2Name).append("> {").append(NEW_LINE);
        indenter.increaseLevel();
    ...
    private String generateThreeColumnTupleSource(@NotNull final String className, @NotNull final ColumnSourceType type1, @NotNull final ColumnSourceType type2, @NotNull final ColumnSourceType type3) {
        final Indenter indenter = new Indenter();
        final StringBuilder code = new StringBuilder(1024);
        final String sourceClass1Name = TypeUtils.getBoxedType(type1.getElementClass()).getSimpleName();
        final String sourceClass2Name = TypeUtils.getBoxedType(type2.getElementClass()).getSimpleName();
        final String sourceClass3Name = TypeUtils.getBoxedType(type3.getElementClass()).getSimpleName();
        final String tupleClassName = TupleCodeGenerator.getTupleClassName(type1.getInternalClass(), type2.getInternalClass(), type3.getInternalClass());
        final String [] extraImports = new String[] { TupleCodeGenerator.getTupleImport(tupleClassName), ThreeColumnTupleSourceFactory.class.getName() };

        code.append("package ").append(OUTPUT_PACKAGE).append(';').append(NEW_LINE);

        code.append(NEW_LINE);

        Stream.of(DEFAULT_IMPORTS, type1.getImports(), type2.getImports(), type3.getImports(), extraImports).flatMap(Arrays::stream).filter(i -> !i.startsWith("java.")).sorted().distinct().forEachOrdered(
                i -> code.append("import ").append(i).append(';').append(NEW_LINE)
        );
        ...
        code.append("/**").append(NEW_LINE);
        code.append(" * <p>{@link TupleSource} that produces key column values from {@link ColumnSource} types ").append(sourceClass1Name).append(", ").append(sourceClass2Name).append(", and ").append(sourceClass3Name).append('.').append(NEW_LINE);
        code.append(" * <p>Generated by {@link ").append(TupleSourceCodeGenerator.class.getName()).append("}.").append(NEW_LINE);
        code.append(" */").append(NEW_LINE);
        code.append("@SuppressWarnings({\"unused\", \"WeakerAccess\"})").append(NEW_LINE);
        code.append("public class ").append(className).append(" extends AbstractTupleSource<").append(tupleClassName).append("> {").append(NEW_LINE);
        ...
        code.append(indenter).append("/** {@link ").append(ThreeColumnTupleSourceFactory.class.getSimpleName()).append("} instance to create instances of {@link ").append(className).append("}. **/").append(NEW_LINE);
        code.append(indenter).append("public static final ").append(ThreeColumnTupleSourceFactory.class.getSimpleName()).append('<')
                .append(tupleClassName).append(", ").append(sourceClass1Name).append(", ").append(sourceClass2Name).append(", ").append(sourceClass3Name)
                .append("> FACTORY = new Factory();").append(NEW_LINE);
        ...
        code.append(indenter).append("private final ColumnSource<").append(sourceClass1Name).append("> ").append(CS1).append(';').append(NEW_LINE);
        code.append(indenter).append("private final ColumnSource<").append(sourceClass2Name).append("> ").append(CS2).append(';').append(NEW_LINE);
        code.append(indenter).append("private final ColumnSource<").append(sourceClass3Name).append("> ").append(CS3).append(';').append(NEW_LINE);
        ...
        indenter.increaseLevel(2);
        code.append(indenter).append("@NotNull final ColumnSource<").append(sourceClass1Name).append("> ").append(CS1).append(',').append(NEW_LINE);
        code.append(indenter).append("@NotNull final ColumnSource<").append(sourceClass2Name).append("> ").append(CS2).append(',').append(NEW_LINE);
        code.append(indenter).append("@NotNull final ColumnSource<").append(sourceClass3Name).append("> ").append(CS3).append(NEW_LINE);
        indenter.decreaseLevel(2);
        code.append(indenter).append(") {").append(NEW_LINE);
        indenter.increaseLevel();
        code.append(indenter).append("super(").append(CS1).append(", ").append(CS2).append(", ").append(CS3).append(");").append(NEW_LINE);
        code.append(indenter).append("this.").append(CS1).append(" = ").append(CS1).append(';').append(NEW_LINE);
        code.append(indenter).append("this.").append(CS2).append(" = ").append(CS2).append(';').append(NEW_LINE);
        ...
        indenter.increaseLevel();
        code.append(indenter).append("writableSource.set(destinationIndexKey, ").append(type3.getExportElementText("tuple.getThirdElement()")).append(");").append(NEW_LINE);
        code.append(indenter).append("return;").append(NEW_LINE);
        indenter.decreaseLevel();
        code.append(indenter).append('}').append(NEW_LINE);
        code.append(indenter).append("throw new IndexOutOfBoundsException(\"Invalid element index \" + elementIndex + \" for export\");").append(NEW_LINE);
        ...
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(indenter).append("if (elementIndex == 2) {").append(NEW_LINE);
        code.append(indenter.increaseLevel()).append("return ").append(type3.getBoxingText("tuple.getThirdElement()")).append(";").append(NEW_LINE);
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(indenter).append("throw new IllegalArgumentException(\"Bad elementIndex for 3 element tuple: \" + elementIndex);").append(NEW_LINE);
        indenter.decreaseLevel();
        code.append(indenter).append('}').append(NEW_LINE);
        ...
        code.append(indenter).append(type1.getValuesChunkTypeString()).append(" chunk1 = chunks[0].as").append(type1.chunkType).append("Chunk();").append(NEW_LINE);
        code.append(indenter).append(type2.getValuesChunkTypeString()).append(" chunk2 = chunks[1].as").append(type2.chunkType).append("Chunk();").append(NEW_LINE);
        code.append(indenter).append(type3.getValuesChunkTypeString()).append(" chunk3 = chunks[2].as").append(type3.chunkType).append("Chunk();").append(NEW_LINE);
        code.append(indenter).append("for (int ii = 0; ii < chunkSize; ++ii) {").append(NEW_LINE);
        code.append(indenter.increaseLevel()).append("destinationObjectChunk.set(ii, new ").append(tupleClassName).append("(").append(type1.getFromChunkText("chunk1.get(ii)")).append(", ").append(type2.getFromChunkText("chunk2.get(ii)")).append(", ").append(type3.getFromChunkText("chunk3.get(ii)")).append("));").append(NEW_LINE);
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(indenter).append("destinationObjectChunk.setSize(chunkSize);").append(NEW_LINE);
        code.append(indenter.decreaseLevel()).append("}").append(NEW_LINE);
        code.append(NEW_LINE);

        code.append(indenter).append("/** {@link ").append(ThreeColumnTupleSourceFactory.class.getSimpleName()).append("} for instances of {@link ").append(className).append("}. **/").append(NEW_LINE);
        code.append(indenter).append("private static final class Factory implements ").append(ThreeColumnTupleSourceFactory.class.getSimpleName())
                .append('<').append(tupleClassName).append(", ").append(sourceClass1Name).append(", ").append(sourceClass2Name).append(", ").append(sourceClass3Name).append("> {").append(NEW_LINE);
        indenter.increaseLevel();
    ...
    private void writeClass(@NotNull final String className, @NotNull final String classBody) {
        try (final PrintStream destination = new PrintStream(new FileOutputStream(new File(OUTPUT_RELATIVE_PATH, className + ".java")))) {
    ...
    public static void main(@NotNull final String... args) {
        ...
        Arrays.stream(ColumnSourceType.values()).forEach(t1 ->
                Arrays.stream(ColumnSourceType.values()).forEach(t2 -> {
                    final String twoColumnTupleSourceName = generateSimpleClassName(t1, t2);
                    final String twoColumnTupleSourceBody = generator.generateTwoColumnTupleSource(twoColumnTupleSourceName, t1, t2);
                    ...
                    Arrays.stream(ColumnSourceType.values()).forEach(t3 -> {
                        ...
                        final String threeColumnTupleSourceBody = generator.generateThreeColumnTupleSource(threeColumnTupleSourceName, t1, t2, t3);
