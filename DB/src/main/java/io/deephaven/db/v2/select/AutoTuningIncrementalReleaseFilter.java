 ...
 * On each cycle, the number of rows per second is computed; and then the number of rows released is the LTM's target cycle
 * multiplied by the rows per second multiplied by the target factor.
 *
 *
 * <p>The AutotuningIncrementalReleaseFilter can be used to benchmark how many rows of data a query can process.  In its
 * simplest form we can measure how many rows a lastBy statement can process.  For example:</p>
 * <pre>
 ...
 * <pre>12:55:49.985 WARN Completed release 6.97 seconds, rows=19630961, rows/second=2,817,053.86</pre>
 * If verbose mode is enabled, progress is displayed for each cycle at the INFO level.</p>
 *
 * <p>You may specify a StreamLoggerImpl() to send the data to STDOUT, as follows:</p>
 * <pre>
 * import io.deephaven.db.v2.select.AutoTuningIncrementalReleaseFilter
 *
 * quotes = db.t("FeedOS", "EquityQuoteL1").where("Date=lastBusinessDateNy()")
 ...
 * currentQuote = quotesFiltered.lastBy("LocalCodeStr").update("Mid=(Bid + Ask)/2")
 * </pre>
 * <p>The verbose information and the final report are easily visible on your console.</p>
 *
 * <p>The AutotuningIncrementalReleaseFilter is best suited for queries that have a single source table with arbitrary
...
public class AutoTuningIncrementalReleaseFilter extends BaseIncrementalReleaseFilter {
     ...
     */
    @ScriptApi
    public AutoTuningIncrementalReleaseFilter(long initialSize, long initialRelease, double targetFactor, boolean verbose) {
     ...
     */
    @ScriptApi
    public AutoTuningIncrementalReleaseFilter(Logger logger, long initialSize, long initialRelease, double targetFactor, boolean verbose) {
     ...
     */
    @ScriptApi
    public AutoTuningIncrementalReleaseFilter(long initialSize, long initialRelease, double targetFactor, boolean verbose, TimeProvider timeProvider) {
     ...
     */
    @ScriptApi
    public AutoTuningIncrementalReleaseFilter(Logger logger, long initialSize, long initialRelease, double targetFactor, boolean verbose, TimeProvider timeProvider) {
        super(initialSize);
        this.logger = logger;
    ...
    public void refresh() {
        ...
        if (nextSize == 0) {
            ...
            if (verbose) {
                ...
                logger.info().append("Releasing: ").append(nextSize).append(" rows, last rows/second: ").append(decimalFormat.format(rowsPerNanoSecond * 1_000_000_000L)).append(", duration=").append(cycleDuration / 1000000L).append(" ms, total rows/second=").append(decimalFormat.format(totalRowsPerSecond)).append(", ETA ").append(decimalFormat.format(eta)).append(" sec").endl();
            }
        }
        LiveTableMonitor.DEFAULT.addNotification(new TerminalNotification() {
            ...
            public void run() {
                ...
                if (!captureReleasedAll && releasedAll) {
                    ...
                    logger.warn().append("Completed release ").append(decimalFormat.format(durationSeconds)).append(" seconds, rows=").append(rows).append(", rows/second=").append(decimalFormat.format(rowsPerSecond)).endl();
                }
            }
        });
        lastRefresh = now;
    ...
    public AutoTuningIncrementalReleaseFilter copy() {
        return new AutoTuningIncrementalReleaseFilter(getInitialSize(), initialRelease, targetFactor, verbose, timeProvider);
    }
}
