...
class NaturalJoinHelper {
    ...
    static Table naturalJoin(QueryTable leftTable, QueryTable rightTable,
        MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, boolean exactMatch) {
        return naturalJoin(leftTable, rightTable, columnsToMatch, columnsToAdd, exactMatch,
            new JoinControl());
    }
    ...
    static Table naturalJoin(QueryTable leftTable, QueryTable rightTable,
        MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, boolean exactMatch,
        JoinControl control) {
        final Table result = naturalJoinInternal(leftTable, rightTable, columnsToMatch,
            columnsToAdd, exactMatch, control);
    ...
    private static Table naturalJoinInternal(QueryTable leftTable, QueryTable rightTable,
        MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, boolean exactMatch,
        JoinControl control) {
        try (final BucketingContext bucketingContext = new BucketingContext("naturalJoin",
            leftTable, rightTable, columnsToMatch, columnsToAdd, control)) {

            // if we have a single column of unique values, and the range is small, we can use a
            // simplified table
            ...
            if (!rightTable.isLive() && control.useUniqueTable(bucketingContext.uniqueValues,
                ...
                final SimpleUniqueStaticNaturalJoinStateManager jsm =
                    new SimpleUniqueStaticNaturalJoinStateManager(
                ...
                jsm.decorateLeftSide(leftTable.getIndex(), bucketingContext.leftSources,
                    leftRedirections);

                final RedirectionIndex redirectionIndex = jsm.buildRedirectionIndex(leftTable,
                    exactMatch, leftRedirections, control.getRedirectionType(leftTable));

                final QueryTable result =
                    makeResult(leftTable, rightTable, columnsToAdd, redirectionIndex, true);

                leftTable.listenForUpdates(new LeftTickingListener(
                    bucketingContext.listenerDescription, columnsToMatch, columnsToAdd, leftTable,
                    result, redirectionIndex, jsm, bucketingContext.leftSources));
            ...
            if (rightTable.isLive()) {
                if (leftTable.isLive()) {
                    ...
                    final IncrementalChunkedNaturalJoinStateManager jsm =
                        new IncrementalChunkedNaturalJoinStateManager(bucketingContext.leftSources,
                            tableSize, bucketingContext.originalLeftSources);
                    ...
                    redirectionIndex = jsm.buildRedirectionIndexFromRedirections(leftTable,
                        exactMatch, leftHashSlots, control.getRedirectionType(leftTable));

                    final QueryTable result =
                        makeResult(leftTable, rightTable, columnsToAdd, redirectionIndex, true);

                    final JoinListenerRecorder leftRecorder = new JoinListenerRecorder(true,
                        bucketingContext.listenerDescription, leftTable, result);
                    final JoinListenerRecorder rightRecorder = new JoinListenerRecorder(false,
                        bucketingContext.listenerDescription, rightTable, result);

                    jsm.setMaximumLoadFactor(control.getMaximumLoadFactor());
                    jsm.setTargetLoadFactor(control.getTargetLoadFactor());

                    final ChunkedMergedJoinListener mergedJoinListener =
                        new ChunkedMergedJoinListener(
                            leftTable, rightTable, bucketingContext.leftSources,
                            bucketingContext.rightSources, columnsToMatch,
                            columnsToAdd, leftRecorder, rightRecorder, result, redirectionIndex,
                            jsm, exactMatch, bucketingContext.listenerDescription);
                    ...
                    if (bucketingContext.useLeftGrouping) {
                        ...
                        final Pair<ArrayBackedColumnSource<?>, ObjectArraySource<Index>> flatResultColumnSources =
                            AbstractColumnSource.groupingToFlatSources(
                                (ColumnSource) bucketingContext.leftSources[0], grouping,
                                leftTable.getIndex(), groupingSize);
                        final ArrayBackedColumnSource groupSource =
                            flatResultColumnSources.getFirst();
                        ...
                        jsm.buildFromLeftSide(leftTable, bucketingContext.leftSources,
                            leftHashSlots);
                        indexSource = null;
                    }
                    ...
                    if (bucketingContext.useLeftGrouping) {
                        redirectionIndex = jsm.buildRedirectionIndexFromHashSlotGrouped(leftTable,
                            indexSource, groupingSize.intValue(), exactMatch, leftHashSlots,
                            control.getRedirectionType(leftTable));
                    } else {
                        redirectionIndex = jsm.buildRedirectionIndexFromHashSlot(leftTable,
                            exactMatch, leftHashSlots, control.getRedirectionType(leftTable));
                    }
                ...
                if (bucketingContext.useLeftGrouping) {
                    ...
                    final StaticChunkedNaturalJoinStateManager jsm =
                        new StaticChunkedNaturalJoinStateManager(groupedSourceArray,
                            StaticChunkedNaturalJoinStateManager
                                .hashTableSize(groupingSize.intValue()),
                            groupedSourceArray);
                    jsm.buildFromLeftSide(leftTableGrouped, groupedSourceArray, leftHashSlots);
                    jsm.decorateWithRightSide(rightTable, bucketingContext.rightSources);
                    redirectionIndex = jsm.buildGroupedRedirectionIndex(leftTable, exactMatch,
                        leftTableGrouped.size(), leftHashSlots, indexSource,
                        control.getRedirectionType(leftTable));
                    ...
                    jsm.decorateWithRightSide(rightTable, bucketingContext.rightSources);
                    redirectionIndex = jsm.buildRedirectionIndexFromHashSlot(leftTable, exactMatch,
                        leftHashSlots, control.getRedirectionType(leftTable));
                } else {
                    final StaticChunkedNaturalJoinStateManager jsm =
                        new StaticChunkedNaturalJoinStateManager(bucketingContext.leftSources,
                            control.tableSizeForRightBuild(rightTable),
                    ...
                    leftTable.listenForUpdates(new LeftTickingListener(
                        bucketingContext.listenerDescription, columnsToMatch, columnsToAdd,
    ...
    private static Table zeroKeyColumnsJoin(QueryTable leftTable, QueryTable rightTable,
        MatchPair[] columnsToAdd, boolean exactMatch, String listenerDescription) {
        ...
        if (rightTable.size() > 1) {
            ...
            redirectionIndex =
                getSingleValueRedirectionIndex(rightRefreshing, rightTable.getIndex().firstKey());
        } else {
        ...
        final QueryTable result =
            makeResult(leftTable, rightTable, columnsToAdd, redirectionIndex, rightRefreshing);
                ...
                final MergedListener mergedListener =
                    new MergedListener(Arrays.asList(leftRecorder, rightRecorder),
                        ...
                        protected void process() {
                            ...
                            if (rightChanged) {
                                final boolean rightUpdated =
                                    updateRightRedirection(rightTable, redirectionIndex);
                                if (rightUpdated) {
                            ...
                            if (leftChanged) {
                                ...
                                result.notifyListeners(new ShiftAwareListener.Update(
                                    leftRecorder.getAdded().clone(),
                                    leftRecorder.getRemoved().clone(), modified,
                ...
                rightTable.listenForUpdates(
                    new BaseTable.ShiftAwareListenerImpl(listenerDescription, rightTable, result) {
                        ...
                        public void onUpdate(final Update upstream) {
                            ...
                            result.notifyListeners(new Update(Index.FACTORY.getEmptyIndex(),
    ...
    @NotNull
    private static SingleValueRedirectionIndex getSingleValueRedirectionIndex(boolean refreshing,
        long value) {
        return refreshing ? new TickingSingleValueRedirectionIndexImpl(value)
            : new StaticSingleValueRedirectionIndexImpl(value);
    }

    private static boolean updateRightRedirection(QueryTable rightTable,
        SingleValueRedirectionIndex redirectionIndex) {
        final boolean changed;
    ...
    private static void checkRightTableSizeZeroKeys(final Table leftTable, final Table rightTable,
        boolean exactMatch) {
        if (leftTable.size() != 0) {
            if (rightTable.size() > 1) {
                throw new RuntimeException(
        ...
        for (MatchPair mp : columnsToAdd) {
            ...
            final ReadOnlyRedirectedColumnSource redirectedColumnSource =
                new ReadOnlyRedirectedColumnSource(redirectionIndex,
                    rightTable.getColumnSource(mp.right()));
     ...
     *
     * The symbol sources are reinterpreted to longs, and then the SymbolCombiner produces an
     * IntegerSparseArraySource for each side. To convert from the symbol table value, we simply
     * look it up in the symbolLookup source and use that as our chunked result.
     */
    static class SymbolTableToUniqueIdSource extends AbstractColumnSource<Integer>
        ...
        SymbolTableToUniqueIdSource(ColumnSource<Long> symbolSource,
    ...
    private static class LeftTickingListener extends BaseTable.ShiftAwareListenerImpl {
        ...
        LeftTickingListener(String description, MatchPair[] columnsToMatch,
            MatchPair[] columnsToAdd, QueryTable leftTable, QueryTable result,
            RedirectionIndex redirectionIndex, StaticNaturalJoinStateManager jsm,
    ...
    private static class RightTickingListener extends BaseTable.ShiftAwareListenerImpl {
        ...
        private final NaturalJoinModifiedSlotTracker modifiedSlotTracker =
            new NaturalJoinModifiedSlotTracker();
        ...
        public void onUpdate(final Update upstream) {
            ...
            final int maxSize = UpdateSizeCalculator.chunkSize(upstream,
            ...
            try (final RightIncrementalChunkedNaturalJoinStateManager.ProbeContext pc =
                ...
                final boolean rightKeysChanged =
                    upstream.modifiedColumnSet.containsAny(rightKeyColumns);
                ...
                if (upstream.shifted.nonempty()) {
                    ...
                    if (rightKeysChanged) {
                        previousToShift = getParent().getIndex().getPrevIndex()
                            .minus(modifiedPreShift).minus(upstream.removed);
                    } else {
                        previousToShift =
                            getParent().getIndex().getPrevIndex().minus(upstream.removed);
                    }
                    ...
                    while (sit.hasNext()) {
                        ...
                        jsm.applyRightShift(pc, rightSources, shiftedIndex, sit.shiftDelta(),
                            modifiedSlotTracker);
                    }
                }
                ...
                if (rightKeysChanged) {
                    // It should make us somewhat sad that we have to add/remove, because we are
                    // doing two hash lookups for keys that have not actually changed.
                    // The alternative would be to do an initial pass that would filter out key
                    // columns that have not actually changed.
            ...
            final ModifiedSlotUpdater slotUpdater = new ModifiedSlotUpdater(jsm,
                modifiedLeftBuilder, redirectionIndex, exactMatch, addedRightColumnsChanged);
    ...
    private static class ModifiedSlotUpdater
        implements NaturalJoinModifiedSlotTracker.ModifiedSlotConsumer {
        ...
        private ModifiedSlotUpdater(IncrementalNaturalJoinStateManager jsm,
            Index.RandomBuilder modifiedLeftBuilder, RedirectionIndex redirectionIndex,
            boolean exactMatch, boolean rightAddedColumnsChanged) {
            this.jsm = jsm;
        ...
        public void accept(long updatedSlot, long originalRightValue, byte flag) {
            ...
            if (rightIndex == StaticNaturalJoinStateManager.DUPLICATE_RIGHT_VALUE) {
                throw new IllegalStateException(
                    "Duplicate right key for " + jsm.keyString(updatedSlot));
            }

            final boolean unchangedRedirection = rightIndex == originalRightValue;

            // if we have no right columns that have changed, and our redirection is identical we
            // can quit here
            ...
            final byte notShift = (~NaturalJoinModifiedSlotTracker.FLAG_RIGHT_SHIFT)
                & NaturalJoinModifiedSlotTracker.FLAG_MASK;
            if ((flag & notShift) != 0) {
                // we do not want to mark the state as modified if the only thing that changed was a
                // shift
    ...
    private static class ChunkedMergedJoinListener extends MergedListener {
        ...
        private ChunkedMergedJoinListener(QueryTable leftTable,
            ...
            leftTransformer = leftTable.newModifiedColumnSetTransformer(result, leftTable
        ...
        protected void process() {
            ...
            if (rightRecorder.recordedVariablesAreValid()) {
                ...
                final long probeSize = UpdateSizeCalculator.chunkSize(
                    Math.max(rightRemoved.size(), rightModified.size()), rightRecorder.getShifted(),
                    IncrementalChunkedNaturalJoinStateManager.CHUNK_SIZE);
                final long buildSize =
                    Math.max(rightAdded.size(), rightKeysModified ? rightModified.size() : 0);

                // process right updates
                try (
                    final IncrementalChunkedNaturalJoinStateManager.ProbeContext pc =
            ...
            if (leftRecorder.recordedVariablesAreValid()) {
                ...
                final long buildSize =
                    Math.max(leftAdded.size(), leftKeyModifications ? leftModified.size() : 0);
                final long probeSize = UpdateSizeCalculator.chunkSize(
                    Math.max(leftRemoved.size(), leftKeyModifications ? leftModified.size() : 0),
                    leftShifted, IncrementalChunkedNaturalJoinStateManager.CHUNK_SIZE);

                final LongArraySource leftRedirections =
                    newLeftRedirections ? new LongArraySource() : null;
                ...
                try (
                    ...
                    if (leftShifted.nonempty()) {
                        try (final Index prevIndex =
                            leftRecorder.getParent().getIndex().getPrevIndex()) {
                            ...
                            while (sit.hasNext()) {
                                sit.next();
                                try (final Index shiftedIndex =
                                    prevIndex.subindexByKey(sit.beginRange(), sit.endRange())
                                        .shift(sit.shiftDelta())) {
                                    jsm.applyLeftShift(pc, leftSources, shiftedIndex,
                                        sit.shiftDelta());
                                }
                            }
                    ...
                    if (leftKeyModifications) {
                        ...
                        jsm.addLeftSide(bc, leftModified, leftSources, leftRedirections,
                            modifiedSlotTracker);
                        copyRedirections(leftModified, leftRedirections);

                        // TODO: This column mask could be made better if we were to keep more
                        // careful track of the original left hash slots during removal.
                        // We are almost able to fix this, because we know the hash slot and the
                        // result redirection for the left modified row; which is the new value.
                        // We could get the hash slot from the removal, and compare them, but the
                        // hash slot outside of a modified slot tracker is unstable [and we don’t
                        // want two of them].
                        // On removal, we could ask our modified slot tracker if, (i) our cookie is
                        // valid, and if so (ii) what the original right value was what the right
                        // value was
                        // [presuming we add that for right side point 1]. This would let us report
                        // our original redirection index as part of the jsm.removeLeft. We could
                        // then compare
                        // the old redirections to the new redirections, only lighting up
                        // allRightColumns if there was indeed a change.
                        result.modifiedColumnSet.setAll(allRightColumns);
                    }

                    if (leftAdditions) {
                        jsm.addLeftSide(bc, leftAdded, leftSources, leftRedirections,
                            modifiedSlotTracker);
            ...
            result.notifyListeners(
                new ShiftAwareListener.Update(leftAdded.clone(), leftRemoved.clone(), modifiedLeft,
                    leftShifted, result.modifiedColumnSet));
        }

        private void copyRedirections(final Index leftRows,
            @NotNull final LongArraySource leftRedirections) {
            final MutableInt position = new MutableInt(0);
