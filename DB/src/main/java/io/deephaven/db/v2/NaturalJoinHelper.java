...
class NaturalJoinHelper {
    ...
    private static Table naturalJoinInternal(QueryTable leftTable, QueryTable rightTable, MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, boolean exactMatch, JoinControl control) {
        try (final BucketingContext bucketingContext = new BucketingContext("naturalJoin", leftTable, rightTable, columnsToMatch, columnsToAdd, control)) {
            ...
            if (!rightTable.isLive() && control.useUniqueTable(bucketingContext.uniqueValues, bucketingContext.maximumUniqueValue, bucketingContext.minimumUniqueValue)) {
                ...
                final SimpleUniqueStaticNaturalJoinStateManager jsm = new SimpleUniqueStaticNaturalJoinStateManager(
                        bucketingContext.originalLeftSources, bucketingContext.uniqueValuesRange(), bucketingContext.uniqueFunctor);
                ...
                final RedirectionIndex redirectionIndex = jsm.buildRedirectionIndex(leftTable, exactMatch, leftRedirections, control.getRedirectionType(leftTable));

                final QueryTable result = makeResult(leftTable, rightTable, columnsToAdd, redirectionIndex, true);

                leftTable.listenForUpdates(new LeftTickingListener(bucketingContext.listenerDescription, columnsToMatch, columnsToAdd, leftTable, result, redirectionIndex, jsm, bucketingContext.leftSources));
            ...
            if (rightTable.isLive()) {
                if (leftTable.isLive()) {
                    ...
                    final int tableSize = Math.max(control.tableSizeForLeftBuild(leftTable), control.tableSizeForRightBuild(rightTable));

                    final IncrementalChunkedNaturalJoinStateManager jsm = new IncrementalChunkedNaturalJoinStateManager(bucketingContext.leftSources, tableSize, bucketingContext.originalLeftSources);
                    ...
                    redirectionIndex = jsm.buildRedirectionIndexFromRedirections(leftTable, exactMatch, leftHashSlots, control.getRedirectionType(leftTable));

                    final QueryTable result = makeResult(leftTable, rightTable, columnsToAdd, redirectionIndex, true);

                    final JoinListenerRecorder leftRecorder = new JoinListenerRecorder(true, bucketingContext.listenerDescription, leftTable, result);
                    final JoinListenerRecorder rightRecorder = new JoinListenerRecorder(false, bucketingContext.listenerDescription, rightTable, result);
                    ...
                    final RightIncrementalChunkedNaturalJoinStateManager jsm = new RightIncrementalChunkedNaturalJoinStateManager(
                            bucketingContext.leftSources, control.tableSizeForLeftBuild(leftTable), bucketingContext.originalLeftSources);

                    final ObjectArraySource<Index> indexSource;
                    final MutableInt groupingSize = new MutableInt();
                    if (bucketingContext.useLeftGrouping) {
                        final Map<?, Index> grouping = bucketingContext.leftSources[0].getGroupToRange(leftTable.getIndex());

                        //noinspection unchecked,rawtypes
                        final Pair<ArrayBackedColumnSource<?>, ObjectArraySource<Index>> flatResultColumnSources = AbstractColumnSource.groupingToFlatSources((ColumnSource) bucketingContext.leftSources[0], grouping, leftTable.getIndex(), groupingSize);
                        final ArrayBackedColumnSource<?> groupSource = flatResultColumnSources.getFirst();
                        indexSource = flatResultColumnSources.getSecond();

                        final Table leftTableGrouped = new QueryTable(Index.FACTORY.getFlatIndex(groupingSize.intValue()), Collections.singletonMap(columnsToMatch[0].left(), groupSource));
                    ...
                    if (bucketingContext.useLeftGrouping) {
                        redirectionIndex = jsm.buildRedirectionIndexFromHashSlotGrouped(leftTable, indexSource, groupingSize.intValue(), exactMatch, leftHashSlots, control.getRedirectionType(leftTable));
                    } else {
                        redirectionIndex = jsm.buildRedirectionIndexFromHashSlot(leftTable, exactMatch, leftHashSlots, control.getRedirectionType(leftTable));
                    }

                    final QueryTable result = makeResult(leftTable, rightTable, columnsToAdd, redirectionIndex, true);

                    rightTable.listenForUpdates(new RightTickingListener(bucketingContext.listenerDescription, rightTable,
                            columnsToMatch, columnsToAdd, result, redirectionIndex, jsm, bucketingContext.rightSources, exactMatch));
                    return result;
                }
            } else {
                if (bucketingContext.useLeftGrouping) {
                    ...
                    final ColumnSource<?>[] groupedSourceArray = {groupSource};
                    final StaticChunkedNaturalJoinStateManager jsm = new StaticChunkedNaturalJoinStateManager(groupedSourceArray, StaticChunkedNaturalJoinStateManager.hashTableSize(groupingSize.intValue()), groupedSourceArray);
                    jsm.buildFromLeftSide(leftTableGrouped, groupedSourceArray, leftHashSlots);
                    jsm.decorateWithRightSide(rightTable, bucketingContext.rightSources);
                    redirectionIndex = jsm.buildGroupedRedirectionIndex(leftTable, exactMatch, leftTableGrouped.size(), leftHashSlots, indexSource, control.getRedirectionType(leftTable));
                } else if (control.buildLeft(leftTable, rightTable)) {
                    final StaticChunkedNaturalJoinStateManager jsm = new StaticChunkedNaturalJoinStateManager(bucketingContext.leftSources, control.tableSizeForLeftBuild(leftTable), bucketingContext.originalLeftSources);
                    jsm.buildFromLeftSide(leftTable, bucketingContext.leftSources, leftHashSlots);
                    jsm.decorateWithRightSide(rightTable, bucketingContext.rightSources);
                    redirectionIndex = jsm.buildRedirectionIndexFromHashSlot(leftTable, exactMatch, leftHashSlots, control.getRedirectionType(leftTable));
                } else {
                    final StaticChunkedNaturalJoinStateManager jsm = new StaticChunkedNaturalJoinStateManager(bucketingContext.leftSources, control.tableSizeForRightBuild(rightTable), bucketingContext.originalLeftSources);
                    jsm.buildFromRightSide(rightTable, bucketingContext.rightSources);
                    ...
                    protected void process() {
                        ...
                        if (rightChanged) {
                            ...
                            if (rightUpdated) {
                                ...
                                rightTransformer.transform(rightRecorder.getModifiedColumnSet(), result.modifiedColumnSet);
                            }
                        }
    ...
    private static SingleValueRedirectionIndex getSingleValueRedirectionIndex(boolean refreshing, long value) {
        return refreshing ? new TickingSingleValueRedirectionIndexImpl(value) : new StaticSingleValueRedirectionIndexImpl(value);
    }
    ...
    private static QueryTable makeResult(@NotNull final QueryTable leftTable,
        ...
        for (MatchPair mp : columnsToAdd) {
            final ReadOnlyRedirectedColumnSource<?> redirectedColumnSource = new ReadOnlyRedirectedColumnSource<>(redirectionIndex, (ColumnSource<?>) rightTable.getColumnSource(mp.right()));
     ...
     */
    static class SymbolTableToUniqueIdSource extends AbstractColumnSource<Integer> implements ImmutableColumnSourceGetDefaults.ForInt {
    ...
    private static class LeftTickingListener extends BaseTable.ShiftAwareListenerImpl {
        ...
        LeftTickingListener(String description, MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, QueryTable leftTable, QueryTable result, RedirectionIndex redirectionIndex, StaticNaturalJoinStateManager jsm, ColumnSource<?>[] leftSources) {
            ...
            leftTransformer = leftTable.newModifiedColumnSetTransformer(result, leftTable.getDefinition().getColumnNamesArray());
        }
    ...
    private static class RightTickingListener extends BaseTable.ShiftAwareListenerImpl {
        ...
        RightTickingListener(String description, QueryTable rightTable, MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, QueryTable result, RedirectionIndex redirectionIndex, RightIncrementalChunkedNaturalJoinStateManager jsm, ColumnSource<?>[] rightSources, boolean exactMatch) {
        ...
        public void onUpdate(final Update upstream) {
            ...
            final int maxSize = UpdateSizeCalculator.chunkSize(upstream, IncrementalChunkedNaturalJoinStateManager.CHUNK_SIZE);
            if (maxSize == 0) {
                Assert.assertion(upstream.empty(), "upstream.empty()");
                return;
            }

            try (final RightIncrementalChunkedNaturalJoinStateManager.ProbeContext pc = jsm.makeProbeContext(rightSources, maxSize)) {
                ...
                if (upstream.shifted.nonempty()) {
                    ...
                    if (rightKeysChanged) {
                        previousToShift = getParent().getIndex().getPrevIndex().minus(modifiedPreShift).minus(upstream.removed);
                    } else {
                    ...
                    while (sit.hasNext()) {
                        sit.next();
                        final Index shiftedIndex = previousToShift.subindexByKey(sit.beginRange(), sit.endRange()).shift(sit.shiftDelta());
                ...
                if (rightKeysChanged) {
                    // It should make us somewhat sad that we have to add/remove, because we are doing two hash lookups for keys that have not actually changed.
                    // The alternative would be to do an initial pass that would filter out key columns that have not actually changed.
            ...
            final ModifiedSlotUpdater slotUpdater = new ModifiedSlotUpdater(jsm, modifiedLeftBuilder, redirectionIndex, exactMatch, addedRightColumnsChanged);
    ...
    private static class ModifiedSlotUpdater implements NaturalJoinModifiedSlotTracker.ModifiedSlotConsumer {
        ...
        private ModifiedSlotUpdater(IncrementalNaturalJoinStateManager jsm, Index.RandomBuilder modifiedLeftBuilder, RedirectionIndex redirectionIndex, boolean exactMatch, boolean rightAddedColumnsChanged) {
            this.jsm = jsm;
        ...
        public void accept(long updatedSlot, long originalRightValue, byte flag) {
            ...
            if (unchangedRedirection && !rightAddedColumnsChanged && (flag & NaturalJoinModifiedSlotTracker.FLAG_RIGHT_ADD) == 0) {
                return;
            }

            final byte notShift = (~NaturalJoinModifiedSlotTracker.FLAG_RIGHT_SHIFT) & NaturalJoinModifiedSlotTracker.FLAG_MASK;
            if ((flag & notShift) != 0) {
    ...
    private static class ChunkedMergedJoinListener extends MergedListener {
        ...
        private ChunkedMergedJoinListener(QueryTable leftTable,
            ...
            leftTransformer = leftTable.newModifiedColumnSetTransformer(result, leftTable.getColumnSourceMap().keySet().toArray(CollectionUtil.ZERO_LENGTH_STRING_ARRAY));
            rightTransformer = rightTable.newModifiedColumnSetTransformer(result, columnsToAdd);
        ...
        protected void process() {
            ...
            if (rightRecorder.recordedVariablesAreValid()) {
                ...
                final boolean rightKeysModified = rightModified.nonempty() && rightModifiedColumns.containsAny(rightKeyColumns);

                final long probeSize = UpdateSizeCalculator.chunkSize(Math.max(rightRemoved.size(), rightModified.size()), rightRecorder.getShifted(), IncrementalChunkedNaturalJoinStateManager.CHUNK_SIZE);
                final long buildSize = Math.max(rightAdded.size(), rightKeysModified ? rightModified.size() : 0);

                // process right updates
                try (final IncrementalChunkedNaturalJoinStateManager.ProbeContext pc = probeSize == 0 ? null : jsm.makeProbeContext(rightSources, probeSize);
                     final IncrementalChunkedNaturalJoinStateManager.BuildContext bc = buildSize == 0 ? null : jsm.makeBuildContext(rightSources, buildSize)
                ) {
                    final Index modifiedPreShift;
                    ...
                    if (rightShifted.nonempty()) {
                        final Index previousToShift = rightRecorder.getParent().getIndex().getPrevIndex().minus(rightRemoved);
            ...
            if (leftRecorder.recordedVariablesAreValid()) {
                ...
                final boolean leftKeyModifications = leftModified.nonempty() && leftModifiedColumns.containsAny(leftKeyColumns);
                ...
                final long probeSize = UpdateSizeCalculator.chunkSize(Math.max(leftRemoved.size(), leftKeyModifications ? leftModified.size() : 0), leftShifted, IncrementalChunkedNaturalJoinStateManager.CHUNK_SIZE);
                ...
                try (final IncrementalChunkedNaturalJoinStateManager.ProbeContext pc = probeSize == 0 ? null : jsm.makeProbeContext(leftSources, probeSize);
                     final IncrementalChunkedNaturalJoinStateManager.BuildContext bc = buildSize == 0 ? null : jsm.makeBuildContext(leftSources, buildSize)
                ) {
                    ...
                    if (leftShifted.nonempty()) {
                        try (final Index prevIndex = leftRecorder.getParent().getIndex().getPrevIndex()) {
                            ...
                            while (sit.hasNext()) {
                                sit.next();
                                try (final Index shiftedIndex = prevIndex.subindexByKey(sit.beginRange(), sit.endRange()).shift(sit.shiftDelta())) {
                    ...
                    if (leftKeyModifications) {
                        ...
                        // TODO: This column mask could be made better if we were to keep more careful track of the original left hash slots during removal.
                        // We are almost able to fix this, because we know the hash slot and the result redirection for the left modified row; which is the new value.
                        // We could get the hash slot from the removal, and compare them, but the hash slot outside of a modified slot tracker is unstable [and we don’t want two of them].
                        // On removal, we could ask our modified slot tracker if, (i) our cookie is valid, and if so (ii) what the original right value was what the right value was
                        // [presuming we add that for right side point 1].  This would let us report our original redirection index as part of the jsm.removeLeft.  We could then compare
                        // the old redirections to the new redirections, only lighting up allRightColumns if there was indeed a change.
                        result.modifiedColumnSet.setAll(allRightColumns);
                    }
