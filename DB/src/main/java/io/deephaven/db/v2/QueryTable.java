...
public class QueryTable extends BaseTable {
    ...
    public interface Operation<T extends DynamicNode & NotificationStepReceiver> {
        ...
        class Result<T extends DynamicNode & NotificationStepReceiver> {
            ...
            /**
             * Construct the result of an operation. The listener may be null if the parent is
             * non-ticking and the table does not need to respond to ticks from other sources.
             *
         ...
         * @param beforeClock the clock value that we captured before the function began; the
         *        function can use this value to bail out early if it notices something has gone
         *        wrong.
    ...
    public interface MemoizableOperation<T extends DynamicNode & NotificationStepReceiver>
        extends Operation<T> {
        /**
    ...
    /**
     * If set to true, then use a RedirectedColumnSource wrapping an ArrayBackedColumnSource for
     * update() calls. Otherwise, the default of a SparseArraySource is used.
     */
    ...
    /**
     * If set to true, then use a RedirectedColumnSource wrapping an ArrayBackedColumnSource for
     * select() calls. Otherwise, the default of a SparseArraySource is used.
     */
    ...
    /**
     * For a static select(), we would prefer to flatten the table to avoid using memory
     * unnecessarily (because the data may be spread out across many blocks depending on the input
     * Index). However, the select() can become slower because it must look things up in a
     * redirection index.
     *
     ...
     */
    private static final double MAXIMUM_STATIC_SELECT_MEMORY_OVERHEAD = Configuration.getInstance()
        .getDoubleWithDefault("QueryTable.maximumStaticSelectMemoryOverhead", 1.1);
     ...
     * @param columns the column source map for the table, which will be copied into a new column
     *        source map
     */
    public QueryTable(TableDefinition definition, Index index,
     ...
     * @param modifiedColumnSet optional {@link ModifiedColumnSet} that should be re-used if
     *        supplied
     */
    private QueryTable(TableDefinition definition, Index index,
     ...
     *             were being used at the {@link ColumnDefinition} level. Do not use this method
     *             without good reason.
     */
    @Deprecated
    ...
    public ColumnSource getColumnSource(String sourceName) {
        ...
        if (columnSource == null) {
            throw new NoSuchColumnException(columns.keySet(),
    ...
    /**
     * Producers of tables should use the modified column set embedded within the table for their
     * result.
     *
     * You must not mutate the result of this method if you are not generating the updates for this
     * table. Callers should not rely on the dirty state of this modified column set.
     *
    ...
    public Table rollup(ComboAggregateFactory comboAggregateFactory, boolean includeConstituents,
        ...
        return memoizeResult(
            MemoizedOperationKey.rollup(comboAggregateFactory, columns, includeConstituents),
            () -> {
                final ComboAggregateFactory withRollup =
                    comboAggregateFactory.forRollup(includeConstituents);
                ...
                final List<SelectColumn> reaggregateColumns =
                ...
                while (!reaggregateColumns.isEmpty()) {
                    final SelectColumn removedColumn =
                        reaggregateColumns.remove(reaggregateColumns.size() - 1);

                    nullColumns.add(0, removedColumn.getName());

                    final Map<String, Class> nullColumnsMap =
                        new LinkedHashMap<>(nullColumns.size());
                    final Table fLastLevel = lastLevel;
                    nullColumns.forEach(nc -> nullColumnsMap.put(nc,
                        fLastLevel.getDefinition().getColumn(nc).getDataType()));
                ...
                final String[] rollupsToDrop = lastLevel.getColumnSourceMap().keySet().stream()
                    .filter(cn -> cn.endsWith(ComboAggregateFactory.ROLLUP_COLUMN_SUFFIX))
                    .toArray(String[]::new);
                final QueryTable finalTable = (QueryTable) lastLevel.dropColumns(rollupsToDrop);
                final Object reverseLookup = Require.neqNull(
                    lastLevel.getAttribute(REVERSE_LOOKUP_ATTRIBUTE), "REVERSE_LOOKUP_ATTRIBUTE");
                finalTable.setAttribute(Table.REVERSE_LOOKUP_ATTRIBUTE, reverseLookup);

                final Table result = HierarchicalTable.createFrom(finalTable,
                    new RollupInfo(comboAggregateFactory, columns,
                        includeConstituents ? RollupInfo.LeafType.Constituent
                            : RollupInfo.LeafType.Normal));
    ...
    public Table treeTable(String idColumn, String parentColumn) {
        ...
        return memoizeResult(MemoizedOperationKey.treeTable(idColumn, parentColumn), () -> {
            final LocalTableMap byExternalResult = ByExternalAggregationFactory.byExternal(this,
                false, (pt, st) -> pt.copyAttributes(st, CopyAttributeOperation.ByExternal),
            ...
            if (hasAttribute(PREPARED_RLL_ATTRIBUTE)) {
                ...
                if (listenerCols.length != 1 || !listenerCols[0].equals(idColumn)) {
                    final String listenerColError = StringUtils
                        .joinStrings(Arrays.stream(listenerCols).map(col -> "'" + col + "'"), ", ");
                    throw new IllegalStateException(
                        "Table was prepared for Tree table with a different Id column. Expected `"
                            + idColumn + "`, Actual " + listenerColError);
                }
            } else {
                reverseLookup = ReverseLookupListener
                    .makeReverseLookupListenerWithSnapshot(QueryTable.this, idColumn);
            }
    ...
    public Table slice(final long firstPositionInclusive, final long lastPositionExclusive) {
        ...
        return getResult(
            SliceLikeOperation.slice(this, firstPositionInclusive, lastPositionExclusive, "slice"));
    }
    ...
    @Override
    public Table leftJoin(Table table, final MatchPair[] columnsToMatch,
        final MatchPair[] columnsToAdd) {
        return QueryPerformanceRecorder.withNugget("leftJoin(" + table + ","
            + matchString(columnsToMatch) + "," + matchString(columnsToAdd) + ")",
            sizeForInstrumentation(), () -> {
    ...
    public Table exactJoin(Table table, MatchPair columnsToMatch[], MatchPair[] columnsToAdd) {
        return QueryPerformanceRecorder.withNugget(
            "exactJoin(" + table + "," + Arrays.toString(columnsToMatch) + ","
                + Arrays.toString(columnsToMatch) + ")",
            sizeForInstrumentation(),
    ...
    public Table lastBy(SelectColumn... selectColumns) {
        return QueryPerformanceRecorder.withNugget("lastBy(" + Arrays.toString(selectColumns) + ")",
            sizeForInstrumentation(),
            () -> {
                final Table result = by(TRACKED_LAST_BY ? new TrackingLastByStateFactoryImpl()
                    : new LastByStateFactoryImpl(), selectColumns);
    ...
    public Table firstBy(SelectColumn... selectColumns) {
        return QueryPerformanceRecorder.withNugget(
            "firstBy(" + Arrays.toString(selectColumns) + ")", sizeForInstrumentation(),
            () -> {
                final Table result = by(TRACKED_FIRST_BY ? new TrackingFirstByStateFactoryImpl()
                    : new FirstByStateFactoryImpl(), selectColumns);
    ...
    public Table medianBy(SelectColumn... selectColumns) {
        return QueryPerformanceRecorder.withNugget(
            "medianBy(" + Arrays.toString(selectColumns) + ")", sizeForInstrumentation(),
    ...
    public Table countBy(String countColumnName, SelectColumn... groupByColumns) {
        return QueryPerformanceRecorder.withNugget(
            "countBy(" + countColumnName + "," + Arrays.toString(groupByColumns) + ")",
            sizeForInstrumentation(), () -> {
                if (!COLUMN_NAME.matcher(countColumnName).matches()) { // TODO: Test more columns
                                                                       // this way
    ...
    private QueryTable byNoMemo(AggregationStateFactory inputAggregationStateFactory,
        ...
        final String description =
            "by(" + inputAggregationStateFactory + ", " + Arrays.toString(groupByColumns) + ")";
        ...
        return QueryPerformanceRecorder.withNugget(description, sizeForInstrumentation(), () -> {
            ...
            final boolean isCount =
                inputAggregationStateFactory.getClass() == CountByStateFactoryImpl.class;
            ...
            final boolean isSortedFirstOrLast =
                inputAggregationStateFactory instanceof SortedFirstOrLastByFactoryImpl;
            final boolean isFirst = inputAggregationStateFactory
                .getClass() == FirstByStateFactoryImpl.class
                || inputAggregationStateFactory.getClass() == TrackingFirstByStateFactoryImpl.class;
            final boolean isLast = inputAggregationStateFactory
                .getClass() == LastByStateFactoryImpl.class
            ...
            if (isBy) {
                ...
                final String formula =
                    ((AggregationFormulaStateFactory) inputAggregationStateFactory).getFormula();
                final String columnParamName =
                    ((AggregationFormulaStateFactory) inputAggregationStateFactory)
                        .getColumnParamName();
                return FormulaAggregationFactory.applyToAllBy(this, formula, columnParamName,
                    groupByColumns);
            } else if (isNumeric) {
                return ChunkedOperatorAggregationHelper.aggregation(
                    new NonKeyColumnAggregationFactory(
                        (IterativeChunkedOperatorFactory) inputAggregationStateFactory),
                    this, groupByColumns);
                ...
                return ChunkedOperatorAggregationHelper
                    .aggregation(new SortedFirstOrLastByAggregationFactory(isSortedFirst, false,
                        ((SortedFirstOrLastByFactoryImpl) inputAggregationStateFactory)
                            .getSortColumnNames()),
                        this, groupByColumns);
            } else if (isFirst || isLast) {
                return ChunkedOperatorAggregationHelper.aggregation(
                    new FirstOrLastByAggregationFactory(isFirst), this, groupByColumns);
            } else if (isMinMax) {
                final boolean isMin =
                ...
                final double percentile =
                    ((PercentileByStateFactoryImpl) inputAggregationStateFactory).getPercentile();
                ...
                if (isWeightedAvg) {
                    ...
                    weightName = ((WeightedSumStateFactoryImpl) inputAggregationStateFactory)
                        .getWeightName();
                }
                return ChunkedOperatorAggregationHelper.aggregation(
                    new WeightedAverageSumAggregationFactory(weightName, isWeightedSum), this,
                    groupByColumns);
            } else if (isCount) {
                return ChunkedOperatorAggregationHelper.aggregation(
                ...
                if (getColumnSourceMap().isEmpty()) {
                    // if we have no input columns, then the only thing we can do is have an empty
                    // result
                    return new QueryTable(Index.FACTORY.getEmptyIndex(), Collections.emptyMap());
                }
                return ChunkedOperatorAggregationHelper.aggregation(new KeyOnlyAggregationFactory(),
                    this, groupByColumns);
            } else if (isCombo) {
                return ChunkedOperatorAggregationHelper
                    .aggregation(((ComboAggregateFactory) inputAggregationStateFactory)
                        .makeAggregationContextFactory(), this, groupByColumns);
            }

            throw new RuntimeException(
                "Unknown aggregation factory: " + inputAggregationStateFactory);
        });
    }
    ...
    public Table headBy(long nRows, String... groupByColumns) {
        return QueryPerformanceRecorder.withNugget(
            "headBy(" + nRows + ", " + Arrays.toString(groupByColumns) + ")",
    ...
    public Table tailBy(long nRows, String... groupByColumns) {
        return QueryPerformanceRecorder.withNugget(
            "tailBy(" + nRows + ", " + Arrays.toString(groupByColumns) + ")",
    ...
    private Table headOrTailBy(long nRows, boolean head, String... groupByColumns) {
        ...
        // Iterate through the columns and build updateView() arguments that will trim the columns
        // to nRows rows
            ...
            if (!groupByColsSet.contains(colName)) {
                ...
                if (head)
                    updates[j++] =
                        ...
                        // : colName
                        colName + '=' + "isNull(" + colName + ") ? null" +
                            ':' + colName + ".size() > " + nRows + " ? " + colName + ".subArray(0, "
                            + nRows + ')' +
                            ':' + colName;
                else
                    updates[j++] =
                        ...
                        // colName = isNull(colName) ? null
                        // : colName.size() > nRows ? colName.subArray(colName.size() - nRows,
                        // colName.size())
                        // : colName
                        colName + '=' + "isNull(" + colName + ") ? null" +
                            ':' + colName + ".size() > " + nRows + " ? " + colName + ".subArray("
                            + colName + ".size() - " + nRows + ", " + colName + ".size())" +
                            ':' + colName;
            }
        }
    ...
    public Table applyToAllBy(String formulaColumn, String columnParamName,
        ...
        return QueryPerformanceRecorder.withNugget(
            "applyToAllBy(" + formulaColumn + ',' + columnParamName + ','
                + Arrays.toString(groupByColumns) + ")",
            sizeForInstrumentation(),
            () -> noFormattingColumnsTable.by(
                new AggregationFormulaStateFactory(formulaColumn, columnParamName),
                groupByColumns));
    }
    ...
    public Table absSumBy(SelectColumn... groupByColumns) {
        return QueryPerformanceRecorder.withNugget(
            "absSumBy(" + Arrays.toString(groupByColumns) + ")", sizeForInstrumentation(),
    ...
    public Table wavgBy(String weightColumn, SelectColumn... groupByColumns) {
        return QueryPerformanceRecorder.withNugget(
            "wavgBy(" + weightColumn + ", " + Arrays.toString(groupByColumns) + ")",
            sizeForInstrumentation(),
    ...
    public Table wsumBy(String weightColumn, SelectColumn... groupByColumns) {
        return QueryPerformanceRecorder.withNugget(
            "wsumBy(" + weightColumn + ", " + Arrays.toString(groupByColumns) + ")",
            sizeForInstrumentation(),
    ...
    public static class FilteredTable extends QueryTable implements SelectFilter.RecomputeListener {
        ...
        public FilteredTable(final Index currentMapping, final QueryTable source,
        ...
        /**
         * Called if something about the filters has changed such that all matched rows of the
         ...
         * @param modifiedColumnSet the set of columns that have any changes to indices in
         *        {@code modified}
         */
        private void doRefilter(final Index upstreamAdded, final Index upstreamRemoved,
            final Index upstreamModified,
            ...
            // Remove upstream keys first, so that keys at rows that were removed and then added are
            // propagated as such.
            // Note that it is a failure to propagate these as modifies, since modifiedColumnSet may
            // not mark that all
            ...
            if (refilterMatchedRequested && refilterUnmatchedRequested) {
                newMapping =
                    whereInternal(source.getIndex().clone(), source.getIndex(), false, filters);
                ...
                // we need to take removed rows out of our index so we do not read them; and also
                // examine added or modified rows
                ...
                throw new IllegalStateException(
                    "Refilter called when a refilter was not requested!");
            }
            ...
            sizeForInstrumentation(), () -> {
                for (int fi = 0; fi < filters.length; ++fi) {
                    ...
                    if (last && !reindexingFilter.requiresSorting()) {
                        // If this is the last (or only) filter, we can just run it as normal unless
                        // it requires sorting.
                        break;
                    }
                    Table result = this;
                ...
                return memoizeResult(MemoizedOperationKey.filter(filters), () -> {
                    ...
                    initializeWithSnapshot("where", swapListener,
                        (prevRequested, beforeClock) -> {
                            ...
                            currentMapping =
                                whereInternal(indexToUse.clone(), indexToUse, usePrev, filters);
                            ...
                            if (!refreshingFilters && isAddOnly()) {
                                filteredTable.setAttribute(Table.ADD_ONLY_TABLE_ATTRIBUTE,
                                    Boolean.TRUE);
                            }
                            ...
                            final List<NotificationQueue.Dependency> dependencies = Stream.concat(
                                ...
                                Stream.of(filters)
                                    .filter(f -> f instanceof DependencyStreamProvider).flatMap(
                                        f -> ((DependencyStreamProvider) f).getDependencyStream()))
    ...
    protected Index whereInternal(Index currentMapping, Index fullSet, boolean usePrev,
    ...
    @Override
    public Table whereIn(final GroupStrategy groupStrategy, final Table rightTable,
        final boolean inclusion, final MatchPair... columnsToMatch) {
        return QueryPerformanceRecorder.withNugget("whereIn(" + groupStrategy + " , rightTable, "
            + inclusion + ", " + matchString(columnsToMatch) + ")", sizeForInstrumentation(),
            () -> {
                checkInitiateOperation(rightTable);

                final Table distinctValues =
                    rightTable.selectDistinct(MatchPair.getRightColumns(columnsToMatch));
                final DynamicWhereFilter dynamicWhereFilter = new DynamicWhereFilter(groupStrategy,
                    distinctValues, inclusion, columnsToMatch);
    ...
    public Table flatten() {
        if (!isFlat() && !isRefreshing() && index.size() - 1 == index.lastKey()) {
            // We're already flat, and we'll never update; so we can just return ourselves, after
            // setting ourselves flat
            setFlat();
        }
    ...
    public boolean isFlat() {
        if (flat) {
            Assert.assertion(index.lastKey() == index.size() - 1,
                "index.lastKey() == index.size() - 1", index, "index");
        }
        return flat;
    }
    ...
    public Table select(SelectColumn... selectColumns) {
        if (!isRefreshing() && !isFlat() && exceedsMaximumStaticSelectOverhead()) {
            // if we are static, we will pass the select through a flatten call, to ensure that our
            // result is as
    ...
    private boolean exceedsMaximumStaticSelectOverhead() {
        ...
        final long requiredBlocks =
            (size() + SparseConstants.BLOCK_SIZE - 1) / SparseConstants.BLOCK_SIZE;
        final long acceptableBlocks =
            (long) (MAXIMUM_STATIC_SELECT_MEMORY_OVERHEAD * (double) requiredBlocks);
    ...
    /**
     * This does a certain amount of validation and can be used to get confidence that the formulas
     * are valid. If it is not valid, you will get an exception. Positive test (should pass
     * validation): "X = 12", "Y = X + 1") Negative test (should fail validation): "X = 12", "Y = Z
     * + 1")
     */
    @Override
    public SelectValidationResult validateSelect(final SelectColumn... selectColumns) {
        final SelectColumn[] clones =
            Arrays.stream(selectColumns).map(SelectColumn::copy).toArray(SelectColumn[]::new);
        SelectAndViewAnalyzer analyzer =
            SelectAndViewAnalyzer.create(SelectAndViewAnalyzer.Mode.SELECT_STATIC, columns,
    ...
    private Table selectOrUpdate(Flavor flavor, final SelectColumn... selectColumns) {
        ...
        final String updateDescription =
            humanReadablePrefix + '(' + selectColumnString(selectColumns) + ')';
        return memoizeResult(
            MemoizedOperationKey.selectUpdateViewOrUpdateView(selectColumns, flavor),
            () -> QueryPerformanceRecorder.withNugget(updateDescription, sizeForInstrumentation(),
                () -> {
                    ...
                    if (isRefreshing()) {
                        ...
                        for (final ColumnSource columnSource : analyzer.getNewColumnSources()
                            .values()) {
    ...
    private void propagateGrouping(SelectColumn[] selectColumns, QueryTable resultTable) {
        ...
        for (SelectColumn selectColumn : selectColumns) {
            ...
            if (sourceColumn != null && !usedOutputColumns.contains(sourceColumn.getSourceName())) {
                ...
                final ColumnSource selectedColumnSource =
                ...
                if (originalColumnSource != selectedColumnSource) {
                    if (originalColumnSource instanceof DeferredGroupingColumnSource) {
                        ...
                        final GroupingProvider groupingProvider =
                            ((DeferredGroupingColumnSource) originalColumnSource)
                                .getGroupingProvider();
                        if (groupingProvider != null) {
                            ...
                            deferredGroupingSelectedSource
                                .setGroupToRange(originalColumnSource.getGroupToRange());
                        }
                    } else if (originalColumnSource.getGroupToRange() != null) {
                        final DeferredGroupingColumnSource deferredGroupingSelectedSource =
                            (DeferredGroupingColumnSource) selectedColumnSource;
    ...
    private Table viewOrUpdateView(Flavor flavor, final SelectColumn... viewColumns) {
        ...
        final String updateDescription =
            humanReadablePrefix + '(' + selectColumnString(viewColumns) + ')';
        ...
        return memoizeResult(MemoizedOperationKey.selectUpdateViewOrUpdateView(viewColumns, flavor),
            () -> QueryPerformanceRecorder.withNugget(
                updateDescription, sizeForInstrumentation(), () -> {
                    ...
                    final ShiftAwareSwapListener swapListener =
                        createSwapListenerIfRefreshing(ShiftAwareSwapListener::new);
                    initializeWithSnapshot(humanReadablePrefix, swapListener,
                        (usePrev, beforeClockValue) -> {
                            ...
                            final SelectAndViewAnalyzer analyzer =
                                SelectAndViewAnalyzer.create(SelectAndViewAnalyzer.Mode.VIEW_EAGER,
                                    columns, index, modifiedColumnSet, publishTheseSources,
                                    viewColumns);
                            final QueryTable queryTable =
                                new QueryTable(index, analyzer.getPublishedColumnSources());
                            if (swapListener != null) {
                                final Map<String, String[]> effects = analyzer.calcEffects();
                                final ShiftAwareListener listener = new ViewOrUpdateViewListener(
                                    updateDescription, this, queryTable, effects);
    ...
    /**
     * A Shift-Aware listener for {Update,}View. It uses the LayeredColumnReferences class to
     * calculate how columns affect other columns, then creates a column set transformer which will
     * be used by onUpdate to transform updates.
     */
    ...
    /**
     * A Shift-Aware listener for Select or Update. It uses the SelectAndViewAnalyzer to calculate
     * how columns affect other columns, then creates a column set transformer which will be used by
     * onUpdate to transform updates.
     */
    private static class SelectOrUpdateListener extends ShiftAwareListenerImpl {
        ...
        public void onUpdate(final Update upstream) {
            ...
            // - create parallel arrays of pre-shift-keys and post-shift-keys so we can move them in
            // chunks
    ...
    public Table lazyUpdate(SelectColumn... selectColumns) {
        return QueryPerformanceRecorder.withNugget(
            "lazyUpdate(" + selectColumnString(selectColumns) + ")", sizeForInstrumentation(),
            () -> {
                checkInitiateOperation();
    ...
    public Table dropColumns(String... columnNames) {
        return memoizeResult(MemoizedOperationKey.dropColumns(columnNames),
            () -> QueryPerformanceRecorder.withNugget(
                "dropColumns(" + Arrays.toString(columnNames) + ")", sizeForInstrumentation(),
                () -> {
                    final Mutable<Table> result = new MutableObject<>();
                    ...
                    if (!existingColumns.containsAll(columnNamesToDrop)) {
                        ...
                        throw new RuntimeException(
                            "Unknown columns: " + columnNamesToDrop.toString()
                    ...
                    initializeWithSnapshot("dropColumns", swapListener,
                        (usePrev, beforeClockValue) -> {
                            ...
                            if (swapListener != null) {
                                ...
                                final ShiftAwareListenerImpl listener = new ShiftAwareListenerImpl(
                                    "dropColumns(" + Arrays.deepToString(columnNames) + ')', this,
                                    resultTable) {
                                    @Override
                                    public void onUpdate(final Update upstream) {
                                        final Update downstream = upstream.copy();
                                        ...
                                        if (upstream.modified.empty()
                                            || resultTable.modifiedColumnSet.empty()) {
            ...
            sizeForInstrumentation(), () -> {
                ...
                for (MatchPair pair : pairs) {
                    ...
                    if (null == columns.get(pair.rightColumn)) {
                        throw new IllegalArgumentException(
                            "Column \"" + pair.rightColumn + "\" not found");
                    }
                ...
                if (isRefreshing()) {
                    ...
                    listenForUpdates(new ShiftAwareListenerImpl(
                        "renameColumns(" + Arrays.deepToString(pairs) + ')', this, queryTable) {
                        @Override
                        public void onUpdate(final Update upstream) {
                            final Update downstream = upstream.copy();
    ...
    @Override
    public Table aj(final Table rightTable, final MatchPair[] columnsToMatch,
        final MatchPair[] columnsToAdd, AsOfMatchRule asOfMatchRule) {
        if (rightTable == null) {
        ...
        return QueryPerformanceRecorder.withNugget(
            "aj(" + "rightTable, " + matchString(columnsToMatch) + ", " + matchString(columnsToAdd)
                + ")",
            () -> ajInternal(rightTableCoalesced, columnsToMatch, columnsToAdd,
                SortingOrder.Ascending, asOfMatchRule));
    }

    @Override
    public Table raj(final Table rightTable, final MatchPair[] columnsToMatch,
        final MatchPair[] columnsToAdd, AsOfMatchRule asOfMatchRule) {
        if (rightTable == null) {
        ...
        return QueryPerformanceRecorder.withNugget(
            ...
            () -> ajInternal(rightTableCoalesced.reverse(), columnsToMatch, columnsToAdd,
                SortingOrder.Descending, asOfMatchRule));
    }
    ...
    private Table ajInternal(Table rightTable, MatchPair[] columnsToMatch, MatchPair[] columnsToAdd,
        ...
        switch (asOfMatchRule) {
            case LESS_THAN:
                if (order != SortingOrder.Ascending) {
                    throw new IllegalArgumentException(
                        "Invalid as of match rule for raj: " + asOfMatchRule);
                }
            ...
            case GREATER_THAN:
                if (order != SortingOrder.Descending) {
                    throw new IllegalArgumentException(
                        "Invalid as of match rule for aj: " + asOfMatchRule);
                }
                disallowExactMatch = true;
                break;
        ...
        return AsOfJoinHelper.asOfJoin(this, (QueryTable) rightTable, columnsToMatch, columnsToAdd,
            order, disallowExactMatch);
    }

    @Override
    public Table naturalJoin(final Table rightTable, final MatchPair[] columnsToMatch,
    ...
    private Table naturalJoinInternal(final Table rightTable, final MatchPair[] columnsToMatch,
        ...
        return NaturalJoinHelper.naturalJoin(this, rightTableCoalesced, columnsToMatch,
            columnsToAdd, exactMatch);
    }

    private MatchPair[] createColumnsToAddIfMissing(Table rightTable, MatchPair[] columnsToMatch,
        MatchPair[] columnsToAdd) {
        if (columnsToAdd.length == 0) {
            final Set<String> matchColumns =
                Arrays.stream(columnsToMatch).map(matchPair -> matchPair.leftColumn)
    ...
    private static String selectColumnString(final SelectColumn[] selectColumns) {
        ...
        result.append('[');
        final Iterable<String> scs = Arrays.stream(selectColumns).map(SelectColumn::getName)
            .filter(name -> name.length() > 0)::iterator;
    ...
    @Override
    public Table join(final Table rightTableCandidate, MatchPair[] columnsToMatch,
        MatchPair[] columnsToAdd, int numRightBitsToReserve) {
        return memoizeResult(
            MemoizedOperationKey.crossJoin(rightTableCandidate, columnsToMatch, columnsToAdd,
                numRightBitsToReserve),
            () -> joinNoMemo(rightTableCandidate, columnsToMatch, columnsToAdd,
                numRightBitsToReserve));
    }

    private Table joinNoMemo(final Table rightTableCandidate, MatchPair[] columnsToMatch,
        MatchPair[] columnsToAdd, int numRightBitsToReserve) {
        ...
        if (columnsToAdd.length == 0) {
            ...
            try {
                ...
                throw new UnsupportedOperationException(
                    "Can not join a V2 table to a V1 table on the right side.", uoe);
            }

            realColumnsToAdd =
                rightColumnNames.stream().filter(x -> !columnsForMatching.contains(x))
        ...
        if (USE_CHUNKED_CROSS_JOIN) {
            ...
            return QueryPerformanceRecorder.withNugget(
                "join(" + matchString(columnsToMatch) + ", " + matchString(realColumnsToAdd) + ", "
                    + numRightBitsToReserve + ")",
                () -> CrossJoinHelper.join(this, coalescedRightTable, columnsToMatch,
                    realColumnsToAdd, numRightBitsToReserve));
        }

        final Set<String> columnsToMatchSet = Arrays.stream(columnsToMatch).map(MatchPair::right)
            .collect(Collectors.toCollection(HashSet::new));
        ...
        final ArrayList<MatchPair> columnsToAddAfterRename =
            new ArrayList<>(realColumnsToAdd.length);
        for (MatchPair matchPair : realColumnsToAdd) {
            ...
            columnsToAddSelectColumns
                .add(new SourceColumn(matchPair.rightColumn, matchPair.leftColumn));
        }
        ...
        return QueryPerformanceRecorder.withNugget(
            ...
            () -> {
                ...
                final Table rightGrouped =
                    rightTable.by(rightColumnsToMatch).view(columnsToAddSelectColumns
                        .toArray(SelectColumn.ZERO_LENGTH_SELECT_COLUMN_ARRAY));
                ...
                maybeCopyColumnDescriptions(ungroupedResult, rightTable, columnsToMatch,
                    realColumnsToAdd);

                return sentinelAdded ? ungroupedResult.dropColumns("__sentinel__")
                    : ungroupedResult;
            });
    }

    /**
     * The leftColumns are the column sources for the snapshot-triggering table. The rightColumns
     * are the column sources for the table being snapshotted. The leftIndex refers to snapshots
     * that we want to take. Typically this index is expected to have size 1, but in some cases it
     * could be larger. The rightIndex refers to the index of the current table. Therefore we want
     * to take leftIndex.size() snapshots, each of which being rightIndex.size() in size.
     * 
     ...
     * @param dest The ColumnSources in which to store the data. The keys are drawn from
     *        leftColumns.keys() and\ rightColumns.keys()
     ...
     *         {@code destOffset + leftIndex.size() * rightIndex.size()}
     */
    private static long snapshotHistoryInternal(
        ...
        leftIndex.forAllLongs(snapshotKey -> {
            ...
            try (
                final Index destIndex = Index.FACTORY.getIndexByRange(doff, doff + rightSize - 1)) {
    ...
    public Table snapshotHistory(final Table rightTable) {
        return QueryPerformanceRecorder.withNugget("snapshotHistory",
            rightTable.sizeForInstrumentation(), () -> {
                checkInitiateOperation();

                // resultColumns initially contains the left columns, then we insert the right
                // columns into it
                final Map<String, ArrayBackedColumnSource> resultColumns =
                    SnapshotUtils.<ArrayBackedColumnSource>createColumnSourceMap(
                ...
                final long initialSize =
                    snapshotHistoryInternal(leftTable.getColumnSourceMap(), leftTable.getIndex(),
                ...
                if (isRefreshing()) {
                    listenForUpdates(new ListenerImpl(
                        "snapshotHistory" + resultColumns.keySet().toString(), this, result) {
                        private long lastKey = index.lastKey();

                        @Override
                        public void onUpdate(final Index added, final Index removed,
                            ...
                            Assert.assertion(added.firstKey() > lastKey,
                                "added.firstKey() > lastKey",
                                lastKey, "lastKey", added, "added");
                            final long oldSize = resultIndex.size();
                            final long newSize =
                                snapshotHistoryInternal(leftTable.getColumnSourceMap(), added,
                                    rightTable.getColumnSourceMap(), rightTable.getIndex(),
                                    resultColumns, oldSize);
                            final Index addedSnapshots =
                                Index.FACTORY.getIndexByRange(oldSize, newSize - 1);
                        ...
                        public boolean canExecute(final long step) {
                            return ((NotificationQueue.Dependency) rightTable).satisfied(step)
                                && super.canExecute(step);
                        }
                    });
                }
                result.setFlat();
    ...
    public void addColumnGrouping(String columnName) {
        // NB: This used to set the group to range map on the column source, but that's not a safe
        // thing to do.
    ...
    public Table snapshot(Table baseTable, boolean doInitialSnapshot, String... stampColumns) {
        return QueryPerformanceRecorder.withNugget(
            ...
            baseTable.sizeForInstrumentation(), () -> {
                ...
                // 'stampColumns' specifies a subset of this table's columns to use, but if
                // stampColumns is empty, we get
                // a view containing all of the columns (in that case, basically we get this table
                // back).
                QueryTable viewTable = (QueryTable) view(stampColumns);
                // Due to the above logic, we need to pull the actual set of column names back from
                // the viewTable.
    ...
    private Table snapshotInternal(Table tableToSnapshot, boolean doInitialSnapshot,
        String... stampColumns) {
        // TODO: we would like to make this operation LTM safe, instead of requiring the lock here;
        // there are two tables
        // but we do only need to listen to one of them; however we are dependent on two of them
        checkInitiateOperation();

        // There are no LazySnapshotTableProviders in the system currently, but they may be used for
        // multicast
        // distribution systems and similar integrations.

        // If this table provides a lazy snapshot version, we should use that instead for the
        // snapshot, this allows us
        // to refresh the table only immediately before the snapshot occurs. Because we know that we
        // are uninterested
        // in things like previous values, it can save a significant amount of CPU to only refresh
        ...
        // Establish the "right" columns using the same names and types as the table being
        // snapshotted
        final Map<String, ArrayBackedColumnSource> resultRightColumns =
            SnapshotUtils.<ArrayBackedColumnSource>createColumnSourceMap(
                tableToSnapshot.getColumnSourceMap(),
                ArrayBackedColumnSource::getMemoryColumnSource);

        // Now make the "left" columns (namely, the "snapshot key" columns). Because this flavor of
        // "snapshot" only
        // keeps a single snapshot, each snapshot key column will have the same value in every row.
        // So for efficiency we
        ...
        for (String stampColumn : stampColumns) {
            ...
            resultLeftColumns.put(stampColumn,
                SingleValueColumnSource.getSingleValueColumnSource(stampColumnType));
        }
        ...
        final SnapshotInternalListener listener =
            new SnapshotInternalListener(this, lazySnapshot, tableToSnapshot,
        ...
        if (doInitialSnapshot) {
            if (!isRefreshing() && tableToSnapshot.isLive() && !lazySnapshot) {
                // if we are making a static copy of the table, we must ensure that it does not
    ...
    public Table snapshotIncremental(final Table tableToSnapshot, final boolean doInitialSnapshot,
        ...
        return QueryPerformanceRecorder.withNugget(
            "snapshotIncremental(tableToSnapshot, " + doInitialSnapshot + ", "
                + Arrays.toString(stampColumns) + ")",
            tableToSnapshot.sizeForInstrumentation(), () -> {
                checkInitiateOperation();

                final QueryTable rightTable =
                    (QueryTable) (tableToSnapshot instanceof UncoalescedTable
                        ? tableToSnapshot.coalesce()
                        : tableToSnapshot);
                rightTable.checkInitiateOperation();

                // Use the given columns (if specified); otherwise an empty array means all of my
                // columns
                ...
                final Map<String, SparseArrayColumnSource> resultLeftColumns =
                    new LinkedHashMap<>();
                for (Map.Entry<String, ColumnSource> entry : leftColumns.entrySet()) {
                    ...
                    final SparseArrayColumnSource stampDest =
                        DbArrayBase.class.isAssignableFrom(type)
                ...
                final Map<String, SparseArrayColumnSource> resultRightColumns =
                    SnapshotUtils.<SparseArrayColumnSource>createColumnSourceMap(
                        rightTable.getColumnSourceMap(),
                ...
                if (resultColumns.size() != resultLeftColumns.size() + resultRightColumns.size()) {
                    throwColumnConflictMessage(resultLeftColumns.keySet(),
                        resultRightColumns.keySet());
                }
                ...
                if (isRefreshing() && rightTable.isRefreshing()) {
                    ...
                    // What's happening here: the left table gets "listener" (some complicated logic
                    // that has access to the
                    // coalescer) whereas the right table (above) gets the one-liner above (but
                    // which also has access to the
                    // same coalescer). So when the right table sees updates they are simply fed to
                    // the coalescer. The
                    // coalescer's job is just to remember what rows have changed. When the *left*
                    // table gets updates, then
                    // the SnapshotIncrementalListener gets called, which does all the snapshotting
                    // work.

                    final ListenerRecorder rightListenerRecorder = new ListenerRecorder(
                        "snapshotIncremental (rightTable)", rightTable, resultTable);
                    ...
                    SnapshotIncrementalListener.copyRowsToResult(rightTable.getIndex(), this,
                        rightTable, leftColumns, resultColumns);
                    resultTable.getIndex().insert(rightTable.getIndex());
                    resultTable.getIndex().initializePreviousValue();
                } else if (isRefreshing()) {
                    // we are not doing an initial snapshot, but are refreshing so need to take a
                    // snapshot of our (static)
                    // right table on the very first tick of the leftTable
                    listenForUpdates(new ShiftAwareListenerImpl("snapshotIncremental (leftTable)",
                        this, resultTable) {
                        @Override
    ...
    private static void throwColumnConflictMessage(Set<String> left, Set<String> right) {
        ...
        throw new RuntimeException(
            "Column name conflicts: " + IterableUtils.makeCommaSeparatedList(conflicts));
    }

    /**
     * If we have a column source that is a complex type, but can be reinterpreted or transformed
     * into a simpler type, do the transformation, otherwise return the original column source.
     *
     * @param columnSource the column source to possibly reinterpret
     * @return the transformed column source, or the original column source if there is not a
    ...
    static ColumnSource maybeTransformToPrimitive(final ColumnSource<?> columnSource) {
        if (DBDateTime.class.isAssignableFrom(columnSource.getType())) {
            if (columnSource.allowsReinterpret(long.class)) {
                ...
                // noinspection unchecked
                final ColumnSource<DBDateTime> columnSourceAsDateTime =
                    (ColumnSource<DBDateTime>) columnSource;
        ...
        if (Boolean.class.isAssignableFrom(columnSource.getType())) {
            if (columnSource.allowsReinterpret(byte.class)) {
                ...
                // noinspection unchecked
                final ColumnSource<Boolean> columnSourceAsBoolean =
    ...
    /**
     * This is the smallest "base" that is used by the ungroup function. Each row from the input
     * table is allocated 2^minimumUngroupBase rows in the output table at startup. If rows are
     * added to the table, this base may need to grow. If a single row in the input has more than
     * 2^base rows, then the base must change for all of the rows.
     */
    ...
    /**
     * The reverse operation returns a new table that is the same as the original table, but the
     * first row is last, and the last row is first. This is an internal API to be used by .raj(),
     * but is accessible for unit tests.
     *
    ...
    public Table ungroup(boolean nullFill, String... columnsToUngroupBy) {
        return QueryPerformanceRecorder.withNugget(
            "ungroup(" + Arrays.toString(columnsToUngroupBy) + ")", sizeForInstrumentation(),
            () -> {
                ...
                long maxSize =
                    computeMaxSize(index, arrayColumns, dbArrayColumns, null, sizes, nullFill);
                final int initialBase =
                    Math.max(64 - Long.numberOfLeadingZeros(maxSize), minimumUngroupBase);
                ...
                final QueryTable result = new QueryTable(
                    getUngroupIndex(sizes, Index.FACTORY.getRandomBuilder(), initialBase, index)
                        .getIndex(),
                    resultMap);
                if (isRefreshing()) {
                    ...
                    listenForUpdates(new ListenerImpl(
                        "ungroup(" + Arrays.deepToString(columnsToUngroupBy) + ')', this, result) {
                        ...
                        public void onUpdate(final Index added, final Index removed,
                            ...
                            newBase = evaluateModified(modified, ungroupModified, ungroupAdded,
                                ungroupRemoved, newBase);
                            if (newBase > shiftState.getNumShiftBits()) {
                                ...
                                if (!modifiedIndex.subsetOf(result.getIndex())) {
                                    final Index missingModifications =
                                        modifiedIndex.minus(result.getIndex());
                                    log.error().append("Result Index: ")
                                        .append(result.getIndex().toString()).endl();
                                    log.error().append("Missing modifications: ")
                                        .append(missingModifications.toString()).endl();
                                    log.error().append("Added: ").append(addedIndex.toString())
                                        .endl();
                                    log.error().append("Modified: ")
                                        .append(modifiedIndex.toString()).endl();
                                    log.error().append("Removed: ").append(removedIndex.toString())
                                        .endl();

                                    for (Map.Entry<String, ColumnSource> es : arrayColumns
                                        .entrySet()) {
                                        ...
                                        for (int i = 0; i < index.size(); i++) {
                                            ...
                                            log.error().append(name).append("[").append(i)
                                                .append("] ").append(size).append(" -> ")
                                                .append(prevSize).endl();
                                        }
                                    }

                                    for (Map.Entry<String, ColumnSource> es : dbArrayColumns
                                        .entrySet()) {
                                        ColumnSource arrayColumn = es.getValue();
                                        String name = es.getKey();
                                        Index.Iterator iterator = index.iterator();
                                    ...
                                    Assert.assertion(false,
                                        "modifiedIndex.subsetOf(result.getIndex())",
                                        modifiedIndex, "modifiedIndex", result.getIndex(),
                                        "result.getIndex()",
                                        shiftState.getNumShiftBits(),
                                        "shiftState.getNumShiftBits()", newBase, "newBase");
                                }
                        ...
                        private int evaluateIndex(final Index index,
                            final Index.RandomBuilder ungroupBuilder, final int newBase) {
                            if (index.size() > 0) {
                                final long[] modifiedSizes = new long[index.intSize("ungroup")];
                                final long maxSize = computeMaxSize(index, arrayColumns,
                                    dbArrayColumns, null, modifiedSizes, nullFill);
                                final int minBase = 64 - Long.numberOfLeadingZeros(maxSize);
                                getUngroupIndex(modifiedSizes, ungroupBuilder,
                                    shiftState.getNumShiftBits(), index);
                        ...
                        private void evaluateRemovedIndex(final Index index,
                            ...
                            if (index.size() > 0) {
                                final long[] modifiedSizes = new long[index.intSize("ungroup")];
                                computePrevSize(index, arrayColumns, dbArrayColumns, modifiedSizes,
                                    nullFill);
                                getUngroupIndex(modifiedSizes, ungroupBuilder,
                                    shiftState.getNumShiftBits(), index);
                            }
                        ...
                        private int evaluateModified(final Index index,
                            ...
                            final int newBase) {
                            if (index.size() > 0) {
                                final long maxSize = computeModifiedIndicesAndMaxSize(index,
                                    arrayColumns, dbArrayColumns, null, modifyBuilder, addedBuilded,
                                    removedBuilder, shiftState.getNumShiftBits(), nullFill);
                                final int minBase = 64 - Long.numberOfLeadingZeros(maxSize);
                                return Math.max(newBase, minBase);
                            }
                            return newBase;
    ...
    private long computeModifiedIndicesAndMaxSize(Index index,
        Map<String, ColumnSource> arrayColumns, Map<String, ColumnSource> dbArrayColumns,
        String referenceColumn, Index.RandomBuilder modifyBuilder, Index.RandomBuilder addedBuilded,
        Index.RandomBuilder removedBuilder, long base, boolean nullFill) {
        if (nullFill) {
            return computeModifiedIndicesAndMaxSizeNullFill(index, arrayColumns, dbArrayColumns,
                referenceColumn, modifyBuilder, addedBuilded, removedBuilder, base);
        }
        return computeModifiedIndicesAndMaxSizeNormal(index, arrayColumns, dbArrayColumns,
            referenceColumn, modifyBuilder, addedBuilded, removedBuilder, base);
    }

    private long computeModifiedIndicesAndMaxSizeNullFill(Index index,
        Map<String, ColumnSource> arrayColumns, Map<String, ColumnSource> dbArrayColumns,
        String referenceColumn, Index.RandomBuilder modifyBuilder, Index.RandomBuilder addedBuilded,
        Index.RandomBuilder removedBuilder, long base) {
        long maxSize = 0;
        ...
        for (int i = 0; i < index.size(); i++) {
            ...
            maxSize = maxAndIndexUpdateForRow(modifyBuilder, addedBuilded, removedBuilder, maxSize,
                maxCur, next, maxPrev, base);
        }
        return maxSize;
    }

    private long computeModifiedIndicesAndMaxSizeNormal(Index index,
        Map<String, ColumnSource> arrayColumns, Map<String, ColumnSource> dbArrayColumns,
        String referenceColumn, Index.RandomBuilder modifyBuilder, Index.RandomBuilder addedBuilded,
        Index.RandomBuilder removedBuilder, long base) {
        long maxSize = 0;
        ...
        for (Map.Entry<String, ColumnSource> es : arrayColumns.entrySet()) {
            ...
            if (!sizeIsInitialized) {
                ...
                for (int i = 0; i < index.size(); i++) {
                    ...
                    maxSize = maxAndIndexUpdateForRow(modifyBuilder, addedBuilded, removedBuilder,
                        maxSize, sizes[i], next, prevSize, base);
                }
            } else {
                Index.Iterator iterator = index.iterator();
                for (int i = 0; i < index.size(); i++) {
            ...
            if (!sizeIsInitialized) {
                ...
                for (int i = 0; i < index.size(); i++) {
                    ...
                    Assert.assertion(sizes[i] == 0 && arrayColumn.get(next) == null ||
                        ...
                        referenceColumn, "referenceColumn", name, "arrayColumn.getName()", i,
                        "row");
                }
            }
        }
    ...
    private long maxAndIndexUpdateForRow(Index.RandomBuilder modifyBuilder,
        Index.RandomBuilder addedBuilded, Index.RandomBuilder removedBuilder, long maxSize,
    ...
    private static long computeMaxSizeNormal(Index index, Map<String, ColumnSource> arrayColumns,
        ...
        for (Map.Entry<String, ColumnSource> es : arrayColumns.entrySet()) {
            ...
            if (!sizeIsInitialized) {
                ...
                for (int i = 0; i < index.size(); i++) {
                    Assert.assertion(
                        sizes[i] == Array.getLength(arrayColumn.get(iterator.nextLong())),
        ...
        for (Map.Entry<String, ColumnSource> es : dbArrayColumns.entrySet()) {
            ...
            if (!sizeIsInitialized) {
                ...
                for (int ii = 0; ii < index.size(); ii++) {
                    if (isUngroupable) {
                        sizes[ii] = ((UngroupableColumnSource) arrayColumn)
                            .getUngroupedSize(iterator.nextLong());
                    } else {
                        final DbArrayBase dbArrayBase =
                            (DbArrayBase) arrayColumn.get(iterator.nextLong());
                ...
                for (int i = 0; i < index.size(); i++) {
                    ...
                    if (isUngroupable) {
                        expectedSize = ((UngroupableColumnSource) arrayColumn)
                            .getUngroupedSize(iterator.nextLong());
                    } else {
                        final DbArrayBase dbArrayBase =
                            (DbArrayBase) arrayColumn.get(iterator.nextLong());
                    ...
                    Assert.assertion(sizes[i] == expectedSize,
                        "sizes[i] == ((DbArrayBase)arrayColumn.get(i)).size()",
                        referenceColumn, "referenceColumn", name, "arrayColumn.getName()", i,
                        "row");
                }
    ...
    private static void computePrevSizeNormal(Index index, Map<String, ColumnSource> arrayColumns,
        ...
        for (ColumnSource arrayColumn : dbArrayColumns.values()) {
            ...
            for (int i = 0; i < index.size(); i++) {
                if (isUngroupable) {
                    sizes[i] = ((UngroupableColumnSource) arrayColumn)
                        .getUngroupedPrevSize(iterator.nextLong());
                } else {
    ...
    private IndexBuilder getUngroupIndex(
        final long[] sizes, final Index.RandomBuilder indexBuilder, final long base,
        final Index index) {
        ...
        for (Index.Iterator iterator = index.iterator(); iterator.hasNext();) {
            ...
            if (sizes[pos] != 0) {
                Assert.assertion(nextShift >= 0, "nextShift >= 0", nextShift, "nextShift", base,
                    "base", next, "next");
    ...
    public Table selectDistinct(SelectColumn... columns) {
        return QueryPerformanceRecorder.withNugget(
            "selectDistinct(" + Arrays.toString(columns) + ")", sizeForInstrumentation(),
    ...
    public QueryTable getSubTable(@NotNull final Index index,
        ...
        return QueryPerformanceRecorder.withNugget("getSubTable", sizeForInstrumentation(), () -> {
            // there is no operation check here, because byExternal calls it internally; and the
            // Index results are
            // not updated internally, but rather externally.
            final QueryTable result =
                new QueryTable(definition, index, columns, resultModifiedColumnSet);
    ...
    static class CopiedTable extends QueryTable {
        ...
        public <R> R memoizeResult(MemoizedOperationKey memoKey, Supplier<R> operation) {
            ...
            final boolean attributesCompatible =
                memoKey.attributesCompatible(parent.attributes, attributes);
            final Supplier<R> computeCachedOperation = attributesCompatible ? () -> {
                ...
                if (parentResult instanceof QueryTable) {
                    ...
                    copyAttributes((QueryTable) parentResult, myResult,
                        memoKey.getParentCopyType());
    ...
    private static <R> MemoizedResult<R> getMemoizedResult(MemoizedOperationKey memoKey,
        ...
        // noinspection unchecked
        return (MemoizedResult<R>) cachedOperations.computeIfAbsent(memoKey,
            k -> new MemoizedResult<>());
    }
    ...
    private static class MemoizedResult<R> {
        ...
        private void maybeMarkSystemic(R cachedResult) {
            if (cachedResult instanceof SystemicObject
                && SystemicObjectTracker.isSystemicThread()) {
        ...
        R getIfValid() {
            if (reference != null) {
                ...
                if (!isFailedTable(cachedResult)
                    && Liveness.verifyCachedObjectForReuse(cachedResult)) {
                    return cachedResult;
                }
            }
            return null;
    ...
    public <T extends DynamicNode & NotificationStepReceiver> T getResult(
    ...
    private <T extends DynamicNode & NotificationStepReceiver> T getResultNoMemo(
        final Operation<T> operation) {
        return QueryPerformanceRecorder.withNugget(operation.getDescription(),
            sizeForInstrumentation(), () -> {
                ...
                initializeWithSnapshot(operation.getLogPrefix(), swapListener,
                    (usePrev, beforeClockValue) -> {
                        final Operation.Result<T> result =
                            operation.initialize(usePrev, beforeClockValue);
                        if (result == null) {
                            return false;
                        }
                        ...
                        if (swapListener != null) {
                            swapListener.setListenerAndResult(
                                Require.neqNull(result.resultListener, "resultListener"),
                                result.resultNode);
    ...
    private class WhereListener extends MergedListener {
        ...
        private WhereListener(ListenerRecorder recorder,
            Collection<NotificationQueue.Dependency> dependencies, FilteredTable result) {
            super(Collections.singleton(recorder), dependencies,
                "where(" + Arrays.toString(result.filters) + ")", result);
        ...
        public void process() {
            ...
            if (result.refilterRequested()) {
                result.doRefilter(recorder.getAdded(), recorder.getRemoved(),
                    recorder.getModified(), recorder.getShifted(),
                    sourceModColumns);
                return;
            }
            ...
            // check which modified keys match filters (Note: filterColumns will be null if we must
            // always check)
            final boolean runFilters =
                filterColumns == null || sourceModColumns.containsAny(filterColumns);
    ...
    private static class StaticWhereListener extends MergedListener {
        ...
        private StaticWhereListener(Collection<NotificationQueue.Dependency> dependencies,
            FilteredTable result) {
            super(Collections.emptyList(), dependencies,
                "where(" + Arrays.toString(result.filters) + ")", result);
            this.result = result;
        }
