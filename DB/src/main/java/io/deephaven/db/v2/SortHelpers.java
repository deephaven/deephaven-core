...
public class SortHelpers {
    ...
    /**
     * If we have more than this many entries per group, instead of creating a large flat
     * redirection Index, we create a redirection index that is composed of the group indices and an
     * accumulated cardinality cache. This can save a significant amount of memory when the groups
     * are large and storing them using our Index structure is more efficient.
     */
    public static int groupedRedirectionThreshold = Configuration.getInstance()
        .getIntegerWithDefault("SortHelpers.groupedRedirectionThreshold", 32);

    /**
     * When the sort is greater than or equal to than megaSortSize, instead of sorting one large
     * chunk, we will sort individual chunks of sortChunkSize and then merge them into
     * ArrayBackedColumnSources with the LongMegaMergeKernel.
     *
     * There are some boundary conditions in the chunk sizing math that make Integer.MAX_VALUE fail;
     * you could probably back off to Integer.MAX_VALUE - 32 safely. We're being very conservative
     * with 1 << 30 instead.
     */
    @VisibleForTesting
    static int megaSortSize =
        Configuration.getInstance().getIntegerWithDefault("QueryTable.sortChunkSize", 1 << 30);
    /**
     * The size of each chunk of a sort in a mega-merge based sort.
     */
    @VisibleForTesting
    static int sortChunkSize =
        Configuration.getInstance().getIntegerWithDefault("QueryTable.sortChunkSize", 1 << 30);
    ...
    static private final SortMapping EMPTY_SORT_MAPPING =
        new ArraySortMapping(CollectionUtil.ZERO_LENGTH_LONG_ARRAY);

    /**
     * Note that if usePrev is true, then indexToSort is the previous index; not the current index,
     * and we should not need to call getPrevIndex.
     */
    static SortMapping getSortedKeys(SortingOrder[] order,
        ColumnSource<Comparable<?>>[] columnsToSortBy, ReadOnlyIndex indexToSort, boolean usePrev) {
        ...
        if (columnsToSortBy.length == 1) {
            if (indexToSort.hasGrouping(columnsToSortBy[0])) {
                ...
                if (allowSymbolTable && columnsToSortBy[0] instanceof SymbolTableSource
                    && ((SymbolTableSource<Comparable<?>>) columnsToSortBy[0])
                        .hasSymbolTable(indexToSort)) {
    ...
    private static class SparseSymbolMapping {
        ...
        private static SparseSymbolMapping createMapping(LongChunk originalSymbol,
            IntChunk mappedIndex) {
            ...
            if (minTrailing == 32) {
                // this means we only found a zero region, in which case we do not want to shift by
                // 64, but rather by zero to just truncate the region entirely
            ...
            // maxMapping ends up being the number of unique string values that we have. We compute
            // it so that we can
            // map symbol IDs to these unique integers using the narrowest primitive sorting kernel
            // possible.
            int maxMapping = 0;
    ...
    private static SortMapping doSymbolTableMapping(SortingOrder order,
        ColumnSource<Comparable<?>> columnSource, ReadOnlyIndex index, boolean usePrev) {
        ...
        final Table symbolTable =
            ((SymbolTableSource) columnSource).getStaticSymbolTable(index, true);

        if (symbolTable.size() >= sortSize) {
            // the very first thing we will do is sort the symbol table, using a regular sort; if it
            // is larger than the
        ...
        try (
            final WritableLongChunk<Values> originalSymbol =
                WritableLongChunk.makeWritableChunk(symbolEntries);
            final WritableIntChunk<Values> mappedIndex =
                WritableIntChunk.makeWritableChunk(symbolEntries)) {
            final ColumnSource idSource =
                idMapping.getColumnSource(SymbolTableSource.ID_COLUMN_NAME);
            try (final ColumnSource.FillContext idContext =
                idSource.makeFillContext(symbolEntries)) {
                idSource.fillChunk(idContext, originalSymbol, idMapping.getIndex());
            }

            final ColumnSource sortedIndexSource =
                idMapping.getColumnSource(SORTED_INDEX_COLUMN_NAME);
            try (final ColumnSource.FillContext sortedIndexContext =
                sortedIndexSource.makeFillContext(symbolEntries)) {
        ...
        // Read the symbol table values into the unmappedValues chunk. The reinterpreted source
        // provides the region and
        // the symbol ID within the region as a packed long, which we then unpack in the
        // type-specific loops below.
        try (final WritableLongChunk<Values> unmappedValues =
            makeAndFillValues(usePrev, index, reinterpreted).asWritableLongChunk()) {

            // Mapped values generic is the chunk of unique integer keys for the sort operation,
            // using the narrowest
            ...
            if (mapping.getMaxMapping() <= Byte.MAX_VALUE) {
                ...
                for (int ii = 0; ii < unmappedValues.size(); ++ii) {
                    // symTabId is the packed ID, nulls get converted to the right kind of null,
                    // other values are pulled
                    // apart into region and id; and then used as offsets in the lookup table to get
                    // the unique sorted id
                ...
                final WritableShortChunk<Any> mappedValues =
                    WritableShortChunk.makeWritableChunk(index.intSize());
                for (int ii = 0; ii < unmappedValues.size(); ++ii) {
                    // symTabId is the packed ID, nulls get converted to the right kind of null,
                    // other values are pulled
                    // apart into region and id; and then used as offsets in the lookup table to get
                ...
                final WritableIntChunk<Any> mappedValues =
                    WritableIntChunk.makeWritableChunk(index.intSize());
                for (int ii = 0; ii < unmappedValues.size(); ++ii) {
                    // symTabId is the packed ID, nulls get converted to the right kind of null,
                    // other values are pulled
                    // apart into region and id; and then used as offsets in the lookup table to get
            ...
            // Fill a chunk that is Writable, and does not have an ordered tag with the index keys
            // that we are sorting, the
            // index would does something very similar inside of
            // io.deephaven.db.v2.utils.OrderedKeys.asKeyIndicesChunk;
            // but provides a LongChunk<OrderedKeyIndices> as its return.
            final long[] indexKeysArray = new long[sortSize];
            final WritableLongChunk<KeyIndices> indexKeys =
                WritableLongChunk.writableChunkWrap(indexKeysArray);
    ...
    private static SortMapping getSortMappingOne(SortingOrder order,
        ColumnSource<Comparable<?>> columnSource, ReadOnlyIndex index, boolean usePrev) {
        ...
        if (sortSize >= megaSortSize) {
            ...
            return new ArraySortMapping(
                doChunkSortingOne(order, columnSource, index, usePrev, (int) sortSize));
        }
    }

    @NotNull
    private static SortMapping doMegaSortOne(SortingOrder order,
        ColumnSource<Comparable<?>> columnSource, ReadOnlyIndex index, boolean usePrev,
        long sortSize) {
        ...
        try (
            final LongSortKernel<Values, KeyIndices> sortContext =
            ...
            while (okit.hasMore()) {
                ...
                try (final WritableChunk<Values> partialValues =
                    makeAndFillValues(usePrev, chunkOk, columnSource)) {
                    ...
                    longMegaMergeKernel.merge(resultIndices, valuesToMerge, 0, accumulatedSize,
                        partialKeys, partialValues);
    ...
    @NotNull
    private static long[] doChunkSortingOne(SortingOrder order,
        ColumnSource<Comparable<?>> columnSource, OrderedKeys index, boolean usePrev,
        int chunkSize) {
    ...
    private static SortMapping getSortMappingGrouped(SortingOrder order,
        ColumnSource<Comparable<?>> columnSource, ReadOnlyIndex index) {
        final Map<Object, Index> groupToRange = index.getGrouping(columnSource);
        final Object[] keys = groupToRange.keySet()
            .toArray((Object[]) Array.newInstance(
                io.deephaven.util.type.TypeUtils.getBoxedType(columnSource.getType()),
                groupToRange.size()));
    ...
    private static SortMapping getSortMappingMulti(SortingOrder[] order,
        ColumnSource<Comparable<?>>[] columnSources, ReadOnlyIndex index, boolean usePrev) {
        ...
        WritableIntChunk<ChunkPositions> offsetsOut =
            WritableIntChunk.makeWritableChunk((sortSize + 1) / 2);
        ...
        if (index.hasGrouping(columnSources[0])) {
            ...
            final Object[] keys = groupToRange.keySet().toArray((Object[]) Array
                .newInstance(TypeUtils.getBoxedType(columnSource.getType()), groupToRange.size()));
        ...
        final WritableLongChunk<KeyIndices> indicesToFetch =
            WritableLongChunk.makeWritableChunk(totalRunLength);
        final WritableIntChunk<ChunkPositions> originalPositions =
            WritableIntChunk.makeWritableChunk(totalRunLength);
        ...
        int maximumSecondarySize = computeIndicesToFetch(indexKeys, offsetsOut, lengthsOut,
            indicesToFetch, originalPositions);
        WritableChunk<Values> values = fetchSecondaryValues(usePrev, columnSources[1],
            indicesToFetch, originalPositions, sortIndexContext, maximumSecondarySize);
        try (final LongSortKernel<Values, KeyIndices> sortContext = LongSortKernel
            .makeContext(chunkType, order[1], indicesToFetch.size(), columnSources.length != 2)) {
        ...
        if (columnSources.length > 2) {
            ...
            for (int columnIndex = 2; columnIndex < columnSources.length; ++columnIndex) {
                ...
                maximumSecondarySize = computeIndicesToFetch(indexKeys, offsetsOut, lengthsOut,
                    indicesToFetch, originalPositions);

                values.close();
                values = fetchSecondaryValues(usePrev, columnSources[columnIndex], indicesToFetch,
                    originalPositions, sortIndexContext, maximumSecondarySize);

                try (final LongSortKernel<Values, KeyIndices> sortContext =
                    LongSortKernel.makeContext(chunkType, order[columnIndex], indicesToFetch.size(),
    ...
    private static WritableChunk<Values> fetchSecondaryValues(boolean usePrev,
        ColumnSource columnSource, WritableLongChunk<KeyIndices> indicesToFetch,
        WritableIntChunk<ChunkPositions> originalPositions,
        ...
        try (final WritableChunk<Values> secondaryValues =
            makeAndFillValues(usePrev, OrderedKeys.wrapKeyIndicesChunkAsOrderedKeys(
                WritableLongChunk.downcast(indicesToFetch)), columnSource)) {

            final ChunkType chunkType = columnSource.getChunkType();

            // make the big chunk that can hold all the relevant secondary values, in their desired
            // position
    ...
    private static int computeIndicesToFetch(WritableLongChunk<KeyIndices> indexKeys,
        ...
        WritableLongChunk<KeyIndices> indicesToFetch,
        WritableIntChunk<ChunkPositions> originalPositions) {
    ...
    private static WritableChunk<Values> makeAndFillValues(boolean usePrev, OrderedKeys ok,
        ...
        final int sortSize =
            LongSizedDataStructure.intSize("SortHelper.makeAndFillValues", ok.size());

        final WritableChunk<Values> values =
            columnSource.getChunkType().makeWritableChunk(sortSize);

        try (final ColumnSource.FillContext primaryColumnSourceContext =
            columnSource.makeFillContext(sortSize)) {
            if (usePrev) {
