...
public class SortHelpers {
    ...
    /**
     * When the sort is greater than or equal to than megaSortSize, instead of sorting one large chunk, we will sort individual chunks of
     * sortChunkSize and then merge them into ArrayBackedColumnSources with the LongMegaMergeKernel.
     *
    ...
    private static class SparseSymbolMapping {
        ...
        private static SparseSymbolMapping createMapping(LongChunk originalSymbol, IntChunk mappedIndex) {
            ...
            if (minTrailing == 32) {
                // this means we only found a zero region, in which case we do not want to shift by 64, but rather by zero to just truncate the region entirely
    ...
    private static SortMapping doSymbolTableMapping(SortingOrder order, ColumnSource<Comparable<?>> columnSource, ReadOnlyIndex index, boolean usePrev) {
        ...
        final Table idMapping = symbolTable.sort(SymbolTableSource.SYMBOL_COLUMN_NAME).by(SymbolTableSource.SYMBOL_COLUMN_NAME).update(SORTED_INDEX_COLUMN_UPDATE).ungroup().view(SymbolTableSource.ID_COLUMN_NAME, SORTED_INDEX_COLUMN_NAME);
        ...
        try (final WritableLongChunk<Values> unmappedValues = makeAndFillValues(usePrev, index, reinterpreted).asWritableLongChunk()) {
            ...
            if (mapping.getMaxMapping() <= Byte.MAX_VALUE) {
                ...
                for (int ii = 0; ii < unmappedValues.size(); ++ii) {
                    ...
                    // apart into region and id; and then used as offsets in the lookup table to get the unique sorted id
            ...
            // Fill a chunk that is Writable, and does not have an ordered tag with the index keys that we are sorting, the
    ...
    private static SortMapping doMegaSortOne(SortingOrder order, ColumnSource<Comparable<?>> columnSource, ReadOnlyIndex index, boolean usePrev, long sortSize) {
        ...
        final LongMegaMergeKernel<Values, KeyIndices> longMegaMergeKernel = LongMegaMergeKernel.makeContext(columnSource.getChunkType(), order);
        try (final LongSortKernel<Values, KeyIndices> sortContext = LongSortKernel.makeContext(columnSource.getChunkType(), order, sortChunkSize, true);
            ...
            while (okit.hasMore()) {
                ...
                try (final WritableChunk<Values> partialValues = makeAndFillValues(usePrev, chunkOk, columnSource)) {
                    ...
                    final WritableLongChunk<KeyIndices> partialKeys = WritableLongChunk.writableChunkWrap(partialKeysArray);
                    chunkOk.fillKeyIndicesChunk(partialKeys);

                    sortContext.sort(partialKeys, partialValues);

                    longMegaMergeKernel.merge(resultIndices, valuesToMerge, 0, accumulatedSize, partialKeys, partialValues);
    ...
    @NotNull
    private static long [] doChunkSortingOne(SortingOrder order, ColumnSource<Comparable<?>> columnSource, OrderedKeys index, boolean usePrev, int chunkSize) {
        try (final WritableChunk<Values> values = makeAndFillValues(usePrev, index, columnSource)) {
            ...
            try (final LongSortKernel<Values, KeyIndices> sortContext = LongSortKernel.makeContext(columnSource.getChunkType(), order, chunkSize, false)) {
    ...
    private static SortMapping getSortMappingMulti(SortingOrder [] order, ColumnSource<Comparable<?>> [] columnSources, ReadOnlyIndex index, boolean usePrev) {
        ...
        if (index.hasGrouping(columnSources[0])) {
            ...
            final Object [] keys = groupToRange.keySet().toArray((Object[]) Array.newInstance(TypeUtils.getBoxedType(columnSource.getType()), groupToRange.size()));
            ...
            for (final Object key : keys) {
                ...
                group.forAllLongs(indexKey -> { indexKeysArray[outputIdx.intValue()] = indexKey; outputIdx.increment(); });
            }
        } else {
            index.fillKeyIndicesChunk(indexKeys);
            ...
            try (final LongSortKernel<Values, KeyIndices> sortContext = LongSortKernel.makeContext(chunkType, order[0], sortSize, true)) {
                sortContext.sort(indexKeys, values);
            }
        ...
        final LongIntTimsortKernel.LongIntSortKernelContext<KeyIndices, ChunkPositions> sortIndexContext = LongIntTimsortKernel.createContext(totalRunLength);

        ChunkType chunkType = columnSources[1].getChunkType();
        int maximumSecondarySize = computeIndicesToFetch(indexKeys, offsetsOut, lengthsOut, indicesToFetch, originalPositions);
        WritableChunk<Values> values = fetchSecondaryValues(usePrev, columnSources[1], indicesToFetch, originalPositions, sortIndexContext, maximumSecondarySize);
        try (final LongSortKernel<Values, KeyIndices> sortContext = LongSortKernel.makeContext(chunkType, order[1], indicesToFetch.size(), columnSources.length != 2)) {
        ...
        if (columnSources.length > 2) {
            ...
            for (int columnIndex = 2; columnIndex < columnSources.length; ++columnIndex) {
                ...
                maximumSecondarySize = computeIndicesToFetch(indexKeys, offsetsOut, lengthsOut, indicesToFetch, originalPositions);

                values.close();
                values = fetchSecondaryValues(usePrev, columnSources[columnIndex], indicesToFetch, originalPositions, sortIndexContext, maximumSecondarySize);

                try (final LongSortKernel<Values, KeyIndices> sortContext = LongSortKernel.makeContext(chunkType, order[columnIndex], indicesToFetch.size(), columnIndex != columnSources.length - 1)) {
                    // and we can sort the stuff within the run now
                    sortContext.sort(indexKeys, values, offsetsOut, lengthsOut);
                }
    ...
    private static WritableChunk<Values> fetchSecondaryValues(boolean usePrev, ColumnSource columnSource, WritableLongChunk<KeyIndices> indicesToFetch, WritableIntChunk<ChunkPositions> originalPositions, LongIntTimsortKernel.LongIntSortKernelContext<KeyIndices, ChunkPositions> sortIndexContext, int maximumSecondarySize) {
        sortIndexContext.sort(originalPositions, indicesToFetch);

        try (final WritableChunk<Values> secondaryValues = makeAndFillValues(usePrev, OrderedKeys.wrapKeyIndicesChunkAsOrderedKeys(WritableLongChunk.downcast(indicesToFetch)), columnSource)) {
    ...
    private static int computeIndicesToFetch(WritableLongChunk<KeyIndices> indexKeys, WritableIntChunk<ChunkPositions> offsetsOut, WritableIntChunk<ChunkLengths> lengthsOut, WritableLongChunk<KeyIndices> indicesToFetch, WritableIntChunk<ChunkPositions> originalPositions) {
    ...
    @NotNull
    private static WritableChunk<Values> makeAndFillValues(boolean usePrev, OrderedKeys ok, ColumnSource<?> columnSource) {
