...
public class ZeroKeyChunkedAjMergedListener extends MergedListener {
    ...
    public void process() {
        ...
        final boolean leftAdditionsOrRemovals = leftStampModified || (leftTicked
        ...
        try (
            ...
            final LongSortKernel<Values, KeyIndices> sortKernel = LongSortKernel.makeContext(
                stampChunkType, order, Math.max(leftChunkSize, rightChunkSize), true)) {
            ...
            if (leftTicked) {
                ...
                if (leftRestampRemovals.nonempty()) {
                    ...
                    try (final OrderedKeys.Iterator leftOkIt =
                        leftRestampRemovals.getOrderedKeysIterator()) {
                ...
                if (leftShifted.nonempty()) {
                    ...
                    ChunkedAjUtilities.bothIncrementalLeftSsaShift(leftShifted, leftSsa,
                        leftRestampRemovals, leftTable, leftChunkSize, leftStampSource);
                }
            } else {
                ...
                try (
                    ...
                    try (
                        final OrderedKeys.Iterator removeit =
                            rightRestampRemovals.getOrderedKeysIterator();
                        ...
                        while (removeit.hasMore()) {
                            ...
                            ssaSsaStamp.processRemovals(leftSsa, rightStampValues, rightStampKeys,
                                priorRedirections, redirectionIndex, modifiedBuilder,
                                disallowExactMatch);
                        }
                    }
                    ...
                    if (rightShifted.nonempty()) {
                        try (final Index fullPrevIndex = rightTable.getIndex().getPrevIndex();
                            ...
                            final SizedChunk<Values> shiftRightStampValues =
                                new SizedChunk<>(stampChunkType);
                            final SizedLongChunk<KeyIndices> shiftRightStampKeys =
                                new SizedLongChunk<>()) {
                            ...
                            while (sit.hasNext()) {
                                ...
                                if (sit.polarityReversed()) {
                                    ...
                                    rightStampSource.fillPrevChunk(
                                        shiftFillContext.ensureCapacity(shiftSize),
                                        shiftRightStampValues.ensureCapacity(shiftSize),
                                        indexToShift);
                                    indexToShift.fillKeyIndicesChunk(
                                        shiftRightStampKeys.ensureCapacity(shiftSize));
                                    shiftSortContext.ensureCapacity(shiftSize).sort(
                                        shiftRightStampKeys.get(), shiftRightStampValues.get());

                                    ssaSsaStamp.applyShift(leftSsa, shiftRightStampValues.get(),
                                        shiftRightStampKeys.get(), sit.shiftDelta(),
                                        redirectionIndex, disallowExactMatch);
                                    rightSsa.applyShiftReverse(shiftRightStampValues.get(),
                                        shiftRightStampKeys.get(), sit.shiftDelta());
                                } else {
                                    try (final OrderedKeys.Iterator shiftIt =
                                        indexToShift.getOrderedKeysIterator()) {
                                        while (shiftIt.hasMore()) {
                                            final OrderedKeys chunkOk = shiftIt
                                                .getNextOrderedKeysWithLength(rightChunkSize);
                                            rightStampSource.fillPrevChunk(fillContext,
                                                rightStampValues, chunkOk);
                                            chunkOk.fillKeyIndicesChunk(rightStampKeys);
                                            sortKernel.sort(rightStampKeys, rightStampValues);

                                            rightSsa.applyShift(rightStampValues, rightStampKeys,
                                                sit.shiftDelta());
                                            ssaSsaStamp.applyShift(leftSsa, rightStampValues,
                                                rightStampKeys, sit.shiftDelta(), redirectionIndex,
                                                disallowExactMatch);
                                        }
                                    }
                                }
                    ...
                    try (
                        ...
                        final int chunks =
                            (rightRestampAdditions.intSize() + rightChunkSize - 1) / rightChunkSize;
                        for (int ii = 0; ii < chunks; ++ii) {
                            final int startChunk = chunks - ii - 1;
                            try (final Index chunkOk = rightRestampAdditions.subindexByPos(
                                ...
                                final int valuesWithNext = rightSsa.insertAndGetNextValue(
                                    stampChunk, insertedIndices, nextRightValue);
                    ...
                    if (!rightStampModified && rightModified.nonempty()) {
                        try (final OrderedKeys.Iterator modit =
                            ...
                            while (modit.hasMore()) {
                                ...
                                chunkOk.fillKeyIndicesChunk(rightStampKeys);
                                sortKernel.sort(rightStampKeys, rightStampValues);
                                ssaSsaStamp.findModified(leftSsa, redirectionIndex,
                                    rightStampValues, rightStampKeys, modifiedBuilder,
                                    disallowExactMatch);
                            }
                        }
                    }
            ...
            if (leftTicked) {
                ...
                if (leftStampModified) {
                    leftRestampAdditions =
                        leftRecorder.getAdded().union(leftRecorder.getModified());
                } else {
                    leftRestampAdditions = leftRecorder.getAdded();
                }

                try (
                    final OrderedKeys.Iterator leftOkIt =
                        leftRestampAdditions.getOrderedKeysIterator();
