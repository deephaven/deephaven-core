...
public class ZeroKeyChunkedAjMergedListener extends MergedListener {
    ...
    public void process() {
        ...
        final boolean leftAdditionsOrRemovals = leftStampModified || (leftTicked && (leftRecorder.getAdded().nonempty() || leftRecorder.getRemoved().nonempty()));

        try (final ColumnSource.FillContext leftFillContext = leftAdditionsOrRemovals ? leftStampSource.makeFillContext(leftChunkSize) : null;
            ...
            if (leftTicked) {
                ...
                if (leftShifted.nonempty()) {
                    ...
                    ChunkedAjUtilities.bothIncrementalLeftSsaShift(leftShifted, leftSsa, leftRestampRemovals, leftTable, leftChunkSize, leftStampSource);
                }
            } else {
                ...
                try (final ColumnSource.FillContext fillContext = rightStampSource.makeFillContext(rightChunkSize);
                    ...
                    if (rightShifted.nonempty()) {
                        try (final Index fullPrevIndex = rightTable.getIndex().getPrevIndex();
                             ...
                             final SizedSafeCloseable<ColumnSource.FillContext> shiftFillContext = new SizedSafeCloseable<>(rightStampSource::makeFillContext);
                             final SizedSafeCloseable<LongSortKernel<Values, KeyIndices>> shiftSortContext = new SizedSafeCloseable<>(sz -> LongSortKernel.makeContext(stampChunkType, order, sz, true));
                            ...
                            while (sit.hasNext()) {
                                ...
                                if (sit.polarityReversed()) {
                                    ...
                                    rightStampSource.fillPrevChunk(shiftFillContext.ensureCapacity(shiftSize), shiftRightStampValues.ensureCapacity(shiftSize), indexToShift);
                                    indexToShift.fillKeyIndicesChunk(shiftRightStampKeys.ensureCapacity(shiftSize));
                                    shiftSortContext.ensureCapacity(shiftSize).sort(shiftRightStampKeys.get(), shiftRightStampValues.get());

                                    ssaSsaStamp.applyShift(leftSsa, shiftRightStampValues.get(), shiftRightStampKeys.get(), sit.shiftDelta(), redirectionIndex, disallowExactMatch);
                                    rightSsa.applyShiftReverse(shiftRightStampValues.get(), shiftRightStampKeys.get(), sit.shiftDelta());
                                } else {
                                    try (final OrderedKeys.Iterator shiftIt = indexToShift.getOrderedKeysIterator()) {
                                        while (shiftIt.hasMore()) {
                                            ...
                                            ssaSsaStamp.applyShift(leftSsa, rightStampValues, rightStampKeys, sit.shiftDelta(), redirectionIndex, disallowExactMatch);
                                        }
                                    }
                                }
                    ...
                    if (!rightStampModified && rightModified.nonempty()) {
                        try (final OrderedKeys.Iterator modit = rightModified.getOrderedKeysIterator()) {
                            while (modit.hasMore()) {
                                ...
                                chunkOk.fillKeyIndicesChunk(rightStampKeys);
                                sortKernel.sort(rightStampKeys, rightStampValues);
                                ssaSsaStamp.findModified(leftSsa, redirectionIndex, rightStampValues, rightStampKeys, modifiedBuilder, disallowExactMatch);
                            }
                        }
                    }
                    ...
                    if (rightStampModified || rightRecorder.getAdded().nonempty() || rightRecorder.getRemoved().nonempty()) {
