...
/**
 * PageStores are a collection of non-overlapping pages, which provides a single {@link ChunkSource}
...
public interface PageStore<ATTR extends Attributes.Any, INNER_ATTR extends ATTR, PAGE extends Page<INNER_ATTR>>
    ...
    /**
     * This is a helper which is the same as a call to {@link #fillChunkAppend}, except that some of
     * the initial work has already been done for the first call to
     * {@link Page#fillChunkAppend(FillContext, WritableChunk, OrderedKeys.Iterator)} which we don't
     * want to repeat.
     */
    // Should be private
    @FinalDefault
    default void doFillChunkAppend(@NotNull final FillContext context,
        ...
        try (
            final OrderedKeys.Iterator orderedKeysIterator = orderedKeys.getOrderedKeysIterator()) {
            page.fillChunkAppend(context, destination, orderedKeysIterator);
