...
public class RspOrderedKeys extends OrderedKeysAsChunkImpl {
        ...
        if (RspBitmap.debug) {
            if (endIdx < startIdx ||
                ...
                throw new IllegalArgumentException(
                    "Empty " + RspOrderedKeys.class.getSimpleName() + " :" +
    ...
    public OrderedKeys getOrderedKeysByKeyRange(long startKeyInclusive, long endKeyInclusive) {
        return arr.getOrderedKeysByKeyRangeConstrainedToIndexAndOffsetRange(startKeyInclusive,
            endKeyInclusive,
    ...
    public void fillKeyIndicesChunk(final WritableLongChunk<? extends KeyIndices> chunkToFill) {
        final RspIterator it =
            new RspIterator(new RspArray.SpanCursorForwardImpl(arr, startIdx), startOffset);
    ...
    public void fillKeyRangesChunk(final WritableLongChunk<OrderedKeyRanges> chunkToFill) {
        ...
        final RspRangeBatchIterator it =
            new RspRangeBatchIterator(new RspArray.SpanCursorForwardImpl(arr, startIdx),
                startOffset, size());
        int nRanges = 0;
        while (it.hasNext()) {
    ...
    public boolean forEachLongRange(final LongRangeAbortableConsumer lrac) {
        if (startIdx == endIdx) {
            ...
            return arr.forEachLongRangeInSpanWithOffsetAndMaxCardinality(startIdx, startOffset,
                remaining, lrac);
        }

        final long[] pendingRange = new long[2];
        final LongRangeAbortableConsumer wrapper =
            RspArray.makeAdjacentRangesCollapsingWrapper(pendingRange, lrac);
    ...
    // Note unlike Index.Iterator, this Iterator will /not/ automatically release its underlying
    ...
    static class Iterator implements OrderedKeys.Iterator {
        ...
        public long peekNextKey() {
            ...
            if (nextKey == -1) {
                ...
                if (currEndIdx == -1) {
                    ...
                    final long spanCardinalityAtCurrEndIdx =
                        arr.getSpanCardinalityAtIndexMaybeAcc(currEndIdx);
        ...
        private int endIndex(
            ...
            if (prevCardMu == null) {
                int j = RspArray.unsignedBinarySearch(idx -> arr.acc[idx], fromIndex, arr.size,
                    cardTarget);
                if (j < 0) {
                    j = -j - 1;
                    if (j == arr.size) {
        ...
        private long nextOrderedKeysWithLength(final long desiredNumberOfKeys) {
            ...
            if (currEndIdx == -1) {
                currEndIdx = endIndex(currStartIdx, currStartOffset, currCardBeforeStartIdx,
                    boundedNumberOfKeys, prevCardMu);
                if (currEndIdx == currStartIdx) {
                    currCardBeforeEndIdx = currCardBeforeStartIdx;
                    currEndOffset = currStartOffset + boundedNumberOfKeys - 1;
                } else {
                    currCardBeforeEndIdx = (prevCardMu != null) ? prevCardMu.longValue()
                        : arr.cardinalityBeforeWithAcc(currEndIdx);
                    final long spanCardAtStartIdx =
                        arr.getSpanCardinalityAtIndexMaybeAcc(currStartIdx);
                    ...
                    final long remainingForEndSpan =
                        boundedNumberOfKeys - firstSpanCount - deltaCount;
            ...
            if (currEndOffset + 1 < spanCardinality) {
                ...
                // currEndIdx + 1 < arr.size, otherwise we would have returned on the
                // bounderNumberOfKeys <= 0 check.
                if (RspArray.debug) {
                ...
                keysAvailableInStartSpan = currStartIdxSpanCardinality =
                    arr.getSpanCardinalityAtIndexMaybeAcc(currStartIdx);
            }
        ...
        public boolean advance(final long toKey) {
            ...
            currCardBeforeStartIdx =
                arr.cardinalityBeforeMaybeAcc(currStartIdx, savedStartIdx, currCardBeforeStartIdx);
            currEndIdx = -1;
            currEndOffset = -1;
            currCardBeforeEndIdx = -1;
        ...
        private boolean updateCurrThrough(final long toKey) {
            ...
            if (currEndIdx != -1) {
                ...
                if (currEndOffset + 1 < spanCardinality) {
                    ...
                    // currEndIdx + 1 < arr.size, otherwise we would have returned on the sizeLeft
                    // <= 0 check.
                    if (RspArray.debug) {
                        Assert.lt(currEndIdx + 1, "currEndIdx + 1",
                            arr.size, "arr.size");
            ...
            if (savedEndIdx != -1) {
                currCardBeforeStartIdx =
                    arr.cardinalityBeforeMaybeAcc(currStartIdx, savedEndIdx, currCardBeforeEndIdx);
            }
            currCardBeforeEndIdx =
                arr.cardinalityBeforeMaybeAcc(currEndIdx, currStartIdx, currCardBeforeStartIdx);
            final long cardinalityBeforeStart = currCardBeforeStartIdx + currStartOffset;
