    ...
    public synchronized boolean processPending() {
        if (!subscribed) {
            if (tableLocation.supportsSubscriptions()) {
                ...
                // NB: Locations that don't support subscriptions don't tick - this single call to refresh is sufficient.
        ...
        synchronized (updateLock) {
            if (observedNonNullSize) {
                if (tableLocation.getSize() == TableLocationState.NULL_SIZE) {
                    pendingException = new TableDataException("Location " + tableLocation + " is no longer available, data has been removed or replaced");
