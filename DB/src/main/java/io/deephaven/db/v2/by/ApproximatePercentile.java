 ...
 *
 * <p>The underlying data structure and algorithm used is a t-digest as described at https://github.com/tdunning/t-digest,
 ...
 * smallish (usually less than 10) multiple of this number.&quote;</p>
 *
 * <p>All input columns are cast to doubles and the result columns are doubles.</p>
 *
 * <p>The input table must be add only, if modifications or removals take place; then an UnsupportedOperationException
 * is thrown.  For tables with adds and removals you must use exact percentiles with
 * {@link ComboAggregateFactory#AggPct(double, java.lang.String...)}.</p>
 *
 * <p>You may compute either one approximate percentile or several approximate percentiles at once.  For example, to
 * compute the 95th percentile of all other columns, by the "Sym" column you would call:
 * <pre>ApproximatePercentile.approximatePercentile(input, 0.95, "Sym")</pre></p>
 *
 * <p>If you need to compute several percentiles, it is more efficient to compute them simultaneously.  For example, this
 ...
 * </pre></p>
 *
 * <p>When parallelizing a workload, you may want to divide it based on natural partitioning and then compute an overall
 * percentile.  In these cases, you should use the {@link PercentileDefinition#exposeDigest} method to expose the internal
 * t-digest structure as a column.  If you then perform an array aggregation ({@link Table#by}), you can call the
...
public class ApproximatePercentile {
     ...
     * @return a single row table with double columns representing the approximate percentile for each column of the input table
     */
    public static Table approximatePercentile(Table input, double percentile) {
        return approximatePercentile(input, DEFAULT_COMPRESSION, percentile, SelectColumn.ZERO_LENGTH_SELECT_COLUMN_ARRAY);
    }
    ...
    public static Table approximatePercentile(Table input, double percentile, String... groupByColumns) {
        return approximatePercentile(input, DEFAULT_COMPRESSION, percentile, SelectColumnFactory.getExpressions(groupByColumns));
    }
    ...
    public static Table approximatePercentile(Table input, double compression, double percentile, SelectColumn... groupByColumns) {
        final NonKeyColumnAggregationFactory aggregationContextFactory = new NonKeyColumnAggregationFactory((type, resultName, exposeInternalColumns) -> new TDigestPercentileOperator(type, compression, percentile, resultName));
        return ChunkedOperatorAggregationHelper.aggregation(aggregationContextFactory, (QueryTable) input, groupByColumns);
    }
     ...
     */
    public static Table approximatePercentiles(Table input, PercentileDefinition percentileDefinitions, SelectColumn... groupByColumns) {
        ...
        final NonKeyColumnAggregationFactory aggregationContextFactory = new NonKeyColumnAggregationFactory((type, resultName, exposeInternalColumns) -> {
            for (final PercentileDefinition percentileDefinition : flatDefs) {
                if (percentileDefinition.inputColumn.equals(resultName)) {
                    return new TDigestPercentileOperator(type, compression, percentileDefinition.digestColumnName, percentileDefinition.percentiles.toArray(), percentileDefinition.resultNames.toArray(CollectionUtil.ZERO_LENGTH_STRING_ARRAY));
                }
            }
            return null;
        });
     ...
     * @return a table containing the groupByColumns and the approximate percentiles
     */
    public static Table approximatePercentiles(Table input, PercentileDefinition percentileDefinitions, String... groupByColumns) {
     ...
     *
     * <p>Accumulate the digests within the DbArray into a single TDigest.  The compression factor is one third of the
     * compression factor of the first digest within the array.  If the array has only a single element, then that
     * element is returned.  If a null array is passed in, null is returned.</p>
     *
     * <p>This function is intended to be used for parallelization.  The first step is to independently compute
     * approximate percentiles with an exposed digest column using your desired buckets.  Next, call
     * {@link Table#by(String...)} to produce arrays of Digests for each relevant bucket.  Once the arrays are created,
     * use this function to accumulate the arrays of digests within an {@link Table#update(String...)} statement.
     * Finally, you may call the TDigest quantile function (or others) to produce the desired approximate percentile.
     * </p>
     *
