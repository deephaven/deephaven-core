 ...
 *
 * <p>
 * The underlying data structure and algorithm used is a t-digest as described at
 * https://github.com/tdunning/t-digest, which has a "compression" parameter that determines the
 * size of the retained values. From the t-digest documentation, &quot;100 is a common value for
 * normal uses. 1000 is extremely large. The number of centroids retained will be a smallish
 * (usually less than 10) multiple of this number.&quote;
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * The input table must be add only, if modifications or removals take place; then an
 * UnsupportedOperationException is thrown. For tables with adds and removals you must use exact
 * percentiles with {@link ComboAggregateFactory#AggPct(double, java.lang.String...)}.
 * </p>
 *
 * <p>
 * You may compute either one approximate percentile or several approximate percentiles at once. For
 * example, to compute the 95th percentile of all other columns, by the "Sym" column you would call:
 * 
 * <pre>
 ...
 *
 * <p>
 * If you need to compute several percentiles, it is more efficient to compute them simultaneously.
 * For example, this example computes the 75th, 95th, and 99th percentiles of the "Latency" column
 * using a builder pattern, and the 95th and 99th percentiles of the "Size" column by "Sym":
 * 
 * <pre>
 ...
 *
 * <p>
 * When parallelizing a workload, you may want to divide it based on natural partitioning and then
 * compute an overall percentile. In these cases, you should use the
 * {@link PercentileDefinition#exposeDigest} method to expose the internal t-digest structure as a
 * column. If you then perform an array aggregation ({@link Table#by}), you can call the
 * {@link #accumulateDigests} function to produce a single digest that represents all of the
 * constituent digests. The amount of error introduced is related to the compression factor that you
 * have selected for the digests. Once you have a combined digest object, you can call the quantile
 * or other functions to extract the desired percentile.
 * </p>
 */
public class ApproximatePercentile {
    public static double DEFAULT_COMPRESSION = Configuration.getInstance()
        .getDoubleWithDefault("ApproximatePercentile.defaultCompression", 100.0);
     ...
     * @return a single row table with double columns representing the approximate percentile for
     ...
     * @return a with the groupByColumns and double columns representing the approximate percentile
     *         for each remaining column of the input table
     */
    public static Table approximatePercentile(Table input, double percentile,
    ...
    public static Table approximatePercentile(Table input, double compression, double percentile,
        ...
        final NonKeyColumnAggregationFactory aggregationContextFactory =
            new NonKeyColumnAggregationFactory(
                (type, resultName, exposeInternalColumns) -> new TDigestPercentileOperator(type,
                    compression, percentile, resultName));
        return ChunkedOperatorAggregationHelper.aggregation(aggregationContextFactory,
            (QueryTable) input, groupByColumns);
    }

    /**
     * A builder class for an approximate percentile definition to be used with
     * {@link #approximatePercentiles}.
     */
    public static class PercentileDefinition {
         ...
         *
         * @param inputColumn the name of the input column that subsequent calls to {@link #add}
         *        operate on.
         *
         * @return a (possibly new) PercentileDefinition
         */
    ...
    /**
     * Compute a set of approximate percentiles for input according to the definitions in
     * percentileDefinitions.
     *
     * @param input the table to compute approximate percentiles for
     * @param percentileDefinitions the compression factor, and map of input columns to output
     *        columns
     * @param groupByColumns the columns to group by
     * @return a table containing the groupByColumns and the approximate percentiles
     */
    public static Table approximatePercentiles(Table input,
        PercentileDefinition percentileDefinitions, SelectColumn... groupByColumns) {
        final List<PercentileDefinition> flatDefs =
            PercentileDefinition.flatten(percentileDefinitions);
        if (flatDefs.isEmpty()) {
                ...
                for (final PercentileDefinition percentileDefinition : flatDefs) {
                    if (percentileDefinition.inputColumn.equals(resultName)) {
                        return new TDigestPercentileOperator(type, compression,
                            percentileDefinition.digestColumnName,
                            percentileDefinition.percentiles.toArray(),
                            percentileDefinition.resultNames
     ...
     */
    public static Table approximatePercentiles(Table input,
        PercentileDefinition percentileDefinitions, String... groupByColumns) {
        return approximatePercentiles(input, percentileDefinitions,
            SelectColumnFactory.getExpressions(groupByColumns));
    }
     ...
     */
    public static Table approximatePercentiles(Table input,
        PercentileDefinition percentileDefinitions) {
        return approximatePercentiles(input, percentileDefinitions,
            SelectColumn.ZERO_LENGTH_SELECT_COLUMN_ARRAY);
    }
     ...
     *
     * <p>
     * Accumulate the digests within the DbArray into a single TDigest. The compression factor is
     * one third of the compression factor of the first digest within the array. If the array has
     * only a single element, then that element is returned. If a null array is passed in, null is
     * returned.
     * </p>
     *
     * <p>
     * This function is intended to be used for parallelization. The first step is to independently
     * compute approximate percentiles with an exposed digest column using your desired buckets.
     * Next, call {@link Table#by(String...)} to produce arrays of Digests for each relevant bucket.
     * Once the arrays are created, use this function to accumulate the arrays of digests within an
     * {@link Table#update(String...)} statement. Finally, you may call the TDigest quantile
     * function (or others) to produce the desired approximate percentile.
     * </p>
     *
