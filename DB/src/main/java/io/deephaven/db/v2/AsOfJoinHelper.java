...
public class AsOfJoinHelper {
    ...
    static Table asOfJoin(QueryTable leftTable, QueryTable rightTable, MatchPair[] columnsToMatch,
        ...
        return asOfJoin(joinControl, leftTable, rightTable, columnsToMatch, columnsToAdd, order,
            disallowExactMatch);
    }

    static Table asOfJoin(JoinControl control, QueryTable leftTable, QueryTable rightTable,
        MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, SortingOrder order,
        ...
        if (!leftTable.isLive() && leftTable.size() == 0) {
            return makeResult(leftTable, rightTable,
                new StaticSingleValueRedirectionIndexImpl(Index.NULL_KEY), columnsToAdd, false);
        }
        ...
        final ColumnSource<?>[] originalLeftSources =
            Arrays.stream(columnsToMatch).limit(keyColumnCount)
        ...
        for (int ii = 0; ii < leftSources.length; ++ii) {
            rightSources[ii] =
                ReinterpretUtilities.maybeConvertToPrimitive(originalRightSources[ii]);
        }

        final ColumnSource<?> leftStampSource = ReinterpretUtilities
            .maybeConvertToPrimitive(leftTable.getColumnSource(stampPair.left()));
        final ColumnSource originalRightStampSource = rightTable.getColumnSource(stampPair.right());
        final ColumnSource<?> rightStampSource =
            ReinterpretUtilities.maybeConvertToPrimitive(originalRightStampSource);
        ...
        final RedirectionIndex redirectionIndex =
            JoinRedirectionIndex.makeRedirectionIndex(control, leftTable);
        if (keyColumnCount == 0) {
            return zeroKeyAj(control, leftTable, rightTable, columnsToAdd, stampPair,
                leftStampSource, originalRightStampSource, rightStampSource, order,
                disallowExactMatch, redirectionIndex);
        }

        if (rightTable.isLive()) {
            if (leftTable.isLive()) {
                return bothIncrementalAj(control, leftTable, rightTable, columnsToMatch,
                    columnsToAdd, order, disallowExactMatch, stampPair,
                    leftSources, rightSources, leftStampSource, rightStampSource, redirectionIndex);
            }
            return rightTickingLeftStaticAj(control, leftTable, rightTable, columnsToMatch,
                columnsToAdd, order, disallowExactMatch, stampPair, leftSources, rightSources,
                leftStampSource, rightStampSource, redirectionIndex);
        } else {
            return rightStaticAj(control, leftTable, rightTable, columnsToMatch, columnsToAdd,
                order, disallowExactMatch, stampPair, originalLeftSources, leftSources,
    ...
    private static Table rightStaticAj(JoinControl control,
        ...
        if (leftGrouping != null) {
            ...
            leftGroupedSources = AbstractColumnSource.groupingToFlatSources(
                (ColumnSource) leftSources[0], leftGrouping, leftTable.getIndex(), groupSize);
        ...
        if (rightGrouping != null) {
            final MutableInt groupSize = new MutableInt();
            // noinspection unchecked
            rightGroupedSources = AbstractColumnSource.groupingToFlatSources(
                (ColumnSource) rightSources[0], rightGrouping, rightTable.getIndex(), groupSize);
        ...
        if (buildLeft) {
            if (leftGroupedSources == null) {
                slotCount = asOfJoinStateManager.buildFromLeftSide(leftTable.getIndex(),
                    leftSources, slots);
            } else {
                slotCount = asOfJoinStateManager.buildFromLeftSide(
                    Index.CURRENT_FACTORY.getFlatIndex(leftGroupingSize),
            ...
            if (rightGroupedSources == null) {
                ...
                asOfJoinStateManager.probeRight(
                    Index.CURRENT_FACTORY.getFlatIndex(rightGroupingSize),
                    new ColumnSource[] {rightGroupedSources.getFirst()});
            }
        } else {
            if (rightGroupedSources == null) {
                slotCount = asOfJoinStateManager.buildFromRightSide(rightTable.getIndex(),
                    rightSources, slots);
            } else {
                slotCount = asOfJoinStateManager.buildFromRightSide(
                    Index.CURRENT_FACTORY.getFlatIndex(rightGroupingSize),
        ...
        try (
            final AsOfStampContext stampContext = new AsOfStampContext(order, disallowExactMatch,
                ...
                if (leftGroupedSources != null) {
                    if (leftIndex.size() != 1) {
                        throw new IllegalStateException(
                            "Groupings should have exactly one index key!");
                    }
                    leftIndex = leftGroupedSources.getSecond().get(leftIndex.get(0));
                }

                if (arrayValuesCache != null) {
                    processLeftSlotWithRightCache(stampContext, leftIndex, rightIndex,
                        redirectionIndex, rightStampSource, keyChunk, valuesChunk, arrayValuesCache,
                        slot);
                } else {
        ...
        final QueryTable result = makeResult(leftTable, rightTable, redirectionIndex, columnsToAdd,
            leftTable.isRefreshing());
        ...
        leftTable.listenForUpdates(new BaseTable.ShiftAwareListenerImpl(
            makeListenerDescription(columnsToMatch, stampPair, columnsToAdd,
                order == SortingOrder.Descending, disallowExactMatch),
            leftTable, result) {
            @Override
            public void onUpdate(Update upstream) {
                ...
                final boolean keysModified =
                    upstream.modifiedColumnSet.containsAny(leftKeysOrStamps);
                ...
                if (restampKeys.nonempty()) {
                    ...
                    final int slotCount = asOfJoinStateManager.probeLeft(restampKeys, leftSources,
                        updatedSlots, foundBuilder);
                    ...
                    try (
                        ...
                        for (int ii = 0; ii < slotCount; ++ii) {
                            ...
                            processLeftSlotWithRightCache(stampContext, leftIndex, rightIndex,
                                redirectionIndex, rightStampSource, keyChunk, valuesChunk,
    ...
    private static void checkColumnConflicts(QueryTable leftTable, MatchPair[] columnsToAdd) {
        final Set<String> rightColumnsToAdd =
            new HashSet<>(Arrays.asList(MatchPair.getLeftColumns(columnsToAdd)));
    ...
    private static class ArrayValuesCache {
        ...
        void setKeysAndValues(long slot, long[] keyIndices, Object StampArray) {
            if (StaticChunkedAsOfJoinStateManager.isOverflowLocation(slot)) {
                final long overflowLocation =
                    StaticChunkedAsOfJoinStateManager.hashLocationToOverflowLocation(slot);
    ...
    private static void processLeftSlotWithRightCache(AsOfStampContext stampContext,
        ...
        ColumnSource<?> rightStampSource,
        ResettableWritableLongChunk<KeyIndices> keyChunk,
        ResettableWritableChunk<Values> valuesChunk,
        ...
        if (rightStampKeys == null) {
            ...
            if (keyChunk.size() < rightSize) {
                // we will hold onto these things "forever", so we would like to avoid making them
                // too large
                keyIndices = Arrays.copyOf(keyIndices, keyChunk.size());
                final Object compactedRightValues =
                    rightStampSource.getChunkType().makeArray(keyChunk.size());
    ...
    /**
     * If the asOfJoinStateManager is null, it means we are passing in the leftIndex. If the
     * leftIndex is null; we are passing in the asOfJoinStateManager and should obtain the leftIndex
     * from the state manager if necessary.
     */
    private static void getCachedLeftStampsAndKeys(
    ...
    private static Table zeroKeyAj(JoinControl control, QueryTable leftTable, QueryTable rightTable,
        ...
        ColumnSource<?> originalRightStampSource, ColumnSource<?> rightStampSource,
        SortingOrder order, boolean disallowExactMatch, final RedirectionIndex redirectionIndex) {
        if (rightTable.isLive() && leftTable.isLive()) {
            return zeroKeyAjBothIncremental(control, leftTable, rightTable, columnsToAdd, stampPair,
                leftStampSource, rightStampSource, order, disallowExactMatch, redirectionIndex);
        } else if (rightTable.isLive()) {
            return zeroKeyAjRightIncremental(control, leftTable, rightTable, columnsToAdd,
                stampPair, leftStampSource, rightStampSource, order, disallowExactMatch,
                redirectionIndex);
        } else {
            return zeroKeyAjRightStatic(leftTable, rightTable, columnsToAdd, stampPair,
                leftStampSource, originalRightStampSource, rightStampSource, order,
                disallowExactMatch, redirectionIndex);
        }
    }
    ...
    private static Table rightTickingLeftStaticAj(JoinControl control,
        ...
        final LongArraySource slots = new LongArraySource();
        final int slotCount =
            asOfJoinStateManager.buildFromLeftSide(leftTable.getIndex(), leftSources, slots);
        asOfJoinStateManager.probeRightInitial(rightTable.getIndex(), rightSources);

        final ArrayValuesCache leftValuesCache =
        ...
        try (
            ...
            for (int slotIndex = 0; slotIndex < slotCount; ++slotIndex) {
                ...
                final SegmentedSortedArray rightSsa =
                    asOfJoinStateManager.getRightSsa(slot, (rightIndex) -> {
                        final SegmentedSortedArray ssa = ssaFactory.get();
                        final int slotSize = rightIndex.intSize();
                        if (slotSize > 0) {
                            rightStampSource.fillChunk(
                                rightStampFillContext.ensureCapacity(slotSize),
                                rightValues.ensureCapacity(slotSize), rightIndex);
                            rightIndex
                                .fillKeyIndicesChunk(rightKeyIndices.ensureCapacity(slotSize));
                            sortContext.ensureCapacity(slotSize).sort(rightKeyIndices.get(),
                                rightValues.get());
                ...
                getCachedLeftStampsAndKeys(null, leftIndex, leftStampSource, leftStampFillContext,
                ...
                chunkSsaStamp.processEntry(leftValuesChunk, leftKeyChunk, rightSsa,
                    rightKeysForLeftChunk, disallowExactMatch);
        ...
        SafeCloseable.closeArray(sortContext, leftStampFillContext, rightStampFillContext,
            rightValues, rightKeyIndices, rightKeysForLeft);

        final QueryTable result =
            makeResult(leftTable, rightTable, redirectionIndex, columnsToAdd, true);

        final ModifiedColumnSet rightMatchColumns =
            rightTable.newModifiedColumnSet(MatchPair.getRightColumns(columnsToMatch));
        final ModifiedColumnSet rightStampColumn =
            rightTable.newModifiedColumnSet(stampPair.right());
        ...
        rightTable.listenForUpdates(
            new BaseTable.ShiftAwareListenerImpl(makeListenerDescription(columnsToMatch, stampPair,
                columnsToAdd, reverse, disallowExactMatch), rightTable, result) {
                @Override
                public void onUpdate(Update upstream) {
                    ...
                    final boolean keysModified =
                        upstream.modifiedColumnSet.containsAny(rightMatchColumns);
                    final boolean stampModified =
                        upstream.modifiedColumnSet.containsAny(rightStampColumn);
                    ...
                    sequentialBuilders
                        .ensureCapacity(Math.max(restampRemovals.size(), restampAdditions.size()));

                    // We first do a probe pass, adding all of the removals to a builder in the as
                    // of join state manager
                    final int removedSlotCount = asOfJoinStateManager
                        .markForRemoval(restampRemovals, rightSources, slots, sequentialBuilders);

                    // Now that everything is marked, process the removals state by state, just as
                    // if we were doing the zero
                    // key case: when removing a row, record the stamp, redirection key, and prior
                    // redirection key. Binary search
                    // in the left for the removed key to find the smallest value geq the removed
                    // right. Update all rows
                    ...
                    try (
                        ...
                        final ResettableWritableChunk<Values> leftValuesChunk =
                            rightStampSource.getChunkType().makeResettableWritableChunk();
                        final SizedLongChunk<KeyIndices> priorRedirections =
                            new SizedLongChunk<>()) {
                        for (int slotIndex = 0; slotIndex < removedSlotCount; ++slotIndex) {
                            ...
                            chunkSsaStamp.processRemovals(leftValuesChunk, leftKeyChunk,
                                rightValues.get(), rightKeyIndices.get(), priorRedirections.get(),
                            ...
                            if (previousToShift.nonempty()) {
                                try (
                                    final ResettableWritableLongChunk<KeyIndices> leftKeyChunk =
                                        ResettableWritableLongChunk.makeResettableChunk();
                                    final ResettableWritableChunk<Values> leftValuesChunk =
                                        rightStampSource.getChunkType()
                                            .makeResettableWritableChunk()) {
                                    final IndexShiftData.Iterator sit =
                                        upstream.shifted.applyIterator();
                                    while (sit.hasNext()) {
                                        sit.next();
                                        final Index indexToShift = previousToShift
                                            .subindexByKey(sit.beginRange(), sit.endRange());
                                        ...
                                        final int shiftedSlots =
                                            asOfJoinStateManager.gatherShiftIndex(indexToShift,
                                                rightSources, slots, sequentialBuilders);
                                        indexToShift.close();

                                        for (int slotIndex =
                                            0; slotIndex < shiftedSlots; ++slotIndex) {
                                            final long slot = slots.getLong(slotIndex);
                                            try (final Index slotShiftIndex =
                                                ...
                                                getCachedLeftStampsAndKeys(asOfJoinStateManager,
                                                    null, leftStampSource, leftStampFillContext,
                                                    sortContext, leftKeyChunk, leftValuesChunk,
                                                    leftValuesCache, slot);

                                                rightStampSource.fillPrevChunk(
                                                    rightStampFillContext.ensureCapacity(shiftSize),
                                                    rightValues.ensureCapacity(shiftSize),
                                                    slotShiftIndex);

                                                final SegmentedSortedArray rightSsa =
                                                    asOfJoinStateManager.getRightSsa(slot);

                                                slotShiftIndex.fillKeyIndicesChunk(
                                                    rightKeyIndices.ensureCapacity(shiftSize));
                                                sortContext.ensureCapacity(shiftSize)
                                                    .sort(rightKeyIndices.get(), rightValues.get());

                                                if (sit.polarityReversed()) {
                                                    chunkSsaStamp.applyShift(leftValuesChunk,
                                                        leftKeyChunk, rightValues.get(),
                    ...
                    final int addedSlotCount = asOfJoinStateManager.probeAdditions(restampAdditions,
                        rightSources, slots, sequentialBuilders);
                    ...
                    try (final SizedChunk<Values> nextRightValue = new SizedChunk<>(stampChunkType);
                        ...
                        final CompactKernel stampCompact =
                            CompactKernel.makeCompact(stampChunkType);

                        // When adding a row to the right hand side: we need to know which left hand
                        // side might be
                        // responsive. If we are a duplicate stamp and not the last one, we ignore
                        // it. Next, we should binary
                        // search in the left for the first value >=, everything up until the next
                        // extant right value should be
                        ...
                        for (int slotIndex = 0; slotIndex < addedSlotCount; ++slotIndex) {
                            ...
                            rightStampSource.fillChunk(
                                rightStampFillContext.ensureCapacity(rightSize),
                                rightStampChunk.ensureCapacity(rightSize), rightAdded);
                            rightAdded
                                .fillKeyIndicesChunk(insertedIndices.ensureCapacity(rightSize));
                            sortContext.ensureCapacity(rightSize).sort(insertedIndices.get(),
                                rightStampChunk.get());

                            final int valuesWithNext = rightSsa.insertAndGetNextValue(
                                rightStampChunk.get(), insertedIndices.get(),
                                nextRightValue.ensureCapacity(rightSize));

                            final boolean endsWithLastValue =
                                valuesWithNext != rightStampChunk.get().size();
                            if (endsWithLastValue) {
                                Assert.eq(valuesWithNext, "valuesWithNext",
                                    rightStampChunk.get().size() - 1, "rightStampChunk.size() - 1");
                                rightStampChunk.get().setSize(valuesWithNext);
                                stampChunkEquals.notEqual(rightStampChunk.get(),
                                    nextRightValue.get(), retainStamps.ensureCapacity(rightSize));
                            ...
                            chunkSsaStamp.processInsertion(leftValuesChunk, leftKeyChunk,
                                rightStampChunk.get(), insertedIndices.get(), nextRightValue.get(),
                                redirectionIndex, modifiedBuilder, endsWithLastValue,
                                disallowExactMatch);
                        }

                        // and then finally we handle the case where the keys and stamps were not
                        // modified, but we must identify
                        ...
                        if (!keysModified && !stampModified && upstream.modified.nonempty()) {
                            ...
                            final int modifiedSlotCount = asOfJoinStateManager.gatherModifications(
                                upstream.modified, rightSources, slots, sequentialBuilders);

                            for (int slotIndex = 0; slotIndex < modifiedSlotCount; ++slotIndex) {
                                final long slot = slots.getLong(slotIndex);

                                try (final Index rightModified =
                                    sequentialBuilders.get(slotIndex).getIndex()) {
                                    sequentialBuilders.set(slotIndex, null);
                                    ...
                                    rightModified.fillKeyIndicesChunk(
                                        rightKeyIndices.ensureCapacity(rightSize));
                                    sortContext.ensureCapacity(rightSize)
                                        .sort(rightKeyIndices.get(), rightValues.get());

                                    getCachedLeftStampsAndKeys(asOfJoinStateManager, null,
                                        leftStampSource, leftStampFillContext, sortContext,
                                        leftKeyChunk, leftValuesChunk, leftValuesCache, slot);

                                    chunkSsaStamp.findModified(0, leftValuesChunk, leftKeyChunk,
                                        redirectionIndex, rightValues.get(), rightKeyIndices.get(),
                    ...
                    SafeCloseable.closeArray(sortContext, leftStampFillContext,
                        rightStampFillContext, rightValues, rightKeyIndices, rightKeysForLeft);

                    downstream.modified = modifiedBuilder.getIndex();

                    final boolean processedAdditionsOrRemovals =
                        removedSlotCount > 0 || addedSlotCount > 0;
    ...
    private static Table bothIncrementalAj(JoinControl control,
        ...
        final RightIncrementalChunkedAsOfJoinStateManager asOfJoinStateManager =
            new RightIncrementalChunkedAsOfJoinStateManager(leftSources,
                control.initialBuildSize());

        final LongArraySource slots = new LongArraySource();
        int slotCount =
            asOfJoinStateManager.buildFromLeftSide(leftTable.getIndex(), leftSources, slots);
        slotCount = asOfJoinStateManager.buildFromRightSide(rightTable.getIndex(), rightSources,
            slots, slotCount);

        // These contexts and chunks will be closed when the SSA factory itself is closed by the
        // destroy function of the BucketedChunkedAjMergedListener
        final SizedSafeCloseable<ColumnSource.FillContext> rightStampFillContext =
            new SizedSafeCloseable<>(rightStampSource::makeFillContext);
        ...
        final SsaFactory rightSsaFactory = new SsaFactory() {
            ...
            public SegmentedSortedArray apply(Index rightIndex) {
                ...
                if (slotSize > 0) {
                    ...
                    sortKernel.ensureCapacity(slotSize).sort(rightStampKeys.get(),
        ...
        final SsaFactory leftSsaFactory = new SsaFactory() {
            @Override
            public void close() {
                SafeCloseable.closeArray(sortKernel, leftStampFillContext, leftStampValues,
                    leftStampKeys);
            }
            ...
            public SegmentedSortedArray apply(Index leftIndex) {
                ...
                if (slotSize > 0) {
                    ...
                    sortKernel.ensureCapacity(slotSize).sort(leftStampKeys.get(),
                        leftStampValues.get());
        ...
        final QueryTable result;
        // if we fail to create the table, then we should make sure to close the ssa factories,
        // which contain a context.
        // if we are successful, then the mergedJoinListener will own them and be responsible for
        // closing them
        try (final SafeCloseableList closeableList =
            new SafeCloseableList(leftSsaFactory, rightSsaFactory)) {
            for (int slotIndex = 0; slotIndex < slotCount; ++slotIndex) {
                final long slot = slots.getLong(slotIndex);

                // if either initial state is empty, we would prefer to leave things as an index
                // rather than process them into an ssa
        ...
        final String listenerDescription = makeListenerDescription(columnsToMatch, stampPair,
        ...
        final BucketedChunkedAjMergedListener mergedJoinListener =
            new BucketedChunkedAjMergedListener(leftRecorder, rightRecorder,
                listenerDescription, result, leftTable, rightTable, columnsToMatch, stampPair,
                columnsToAdd, leftSources,
    ...
    private static Table zeroKeyAjBothIncremental(JoinControl control, QueryTable leftTable,
        QueryTable rightTable, MatchPair[] columnsToAdd, MatchPair stampPair,
        ColumnSource<?> leftStampSource, ColumnSource<?> rightStampSource, SortingOrder order,
        ...
        final SegmentedSortedArray leftSsa =
            SegmentedSortedArray.make(stampChunkType, reverse, leftNodeSize);
        final SegmentedSortedArray rightSsa =
            SegmentedSortedArray.make(stampChunkType, reverse, rightNodeSize);
        ...
        final String listenerDescription =
            makeListenerDescription(MatchPair.ZERO_LENGTH_MATCH_PAIR_ARRAY, stampPair, columnsToAdd,
                reverse, disallowExactMatch);
        final JoinListenerRecorder leftRecorder =
            new JoinListenerRecorder(true, listenerDescription, leftTable, result);
        final JoinListenerRecorder rightRecorder =
            new JoinListenerRecorder(false, listenerDescription, rightTable, result);
    ...
    private static String makeListenerDescription(MatchPair[] columnsToMatch, MatchPair stampPair,
        ...
        final String stampString =
            disallowExactMatch ? makeDisallowExactStampString(stampPair, reverse)
                : MatchPair.matchString(stampPair);
        return (reverse ? "r" : "") + "aj([" + MatchPair.matchString(columnsToMatch) + ", "
            + stampString + "], [" + MatchPair.matchString(columnsToAdd) + "])";
    }
    ...
    private static void fillSsaWithSort(QueryTable rightTable, ColumnSource<?> stampSource,
        int nodeSize, SegmentedSortedArray ssa, SortingOrder order) {
        try (final ColumnSource.FillContext context = stampSource.makeFillContext(nodeSize);
            ...
            final WritableChunk<Values> stampChunk =
                stampSource.getChunkType().makeWritableChunk(nodeSize);
            final WritableLongChunk<KeyIndices> keyChunk =
                WritableLongChunk.makeWritableChunk(nodeSize);
    ...
    private static Table zeroKeyAjRightIncremental(JoinControl control, QueryTable leftTable,
        QueryTable rightTable, MatchPair[] columnsToAdd, MatchPair stampPair,
        ColumnSource<?> leftStampSource, ColumnSource<?> rightStampSource, SortingOrder order,
        boolean disallowExactMatch, final RedirectionIndex redirectionIndex) {
        final boolean reverse = order == SortingOrder.Descending;
        ...
        final WritableLongChunk<KeyIndices> leftStampKeys =
            WritableLongChunk.makeWritableChunk(leftSize);
        ...
        final ChunkSsaStamp chunkSsaStamp = ChunkSsaStamp.make(stampChunkType, reverse);
        try (final WritableLongChunk<KeyIndices> rightKeysForLeft =
            WritableLongChunk.makeWritableChunk(leftSize)) {
            chunkSsaStamp.processEntry(leftStampValues, leftStampKeys, ssa, rightKeysForLeft,
                disallowExactMatch);
        ...
        rightTable.listenForUpdates(new BaseTable.ShiftAwareListenerImpl(
            ...
            public void onUpdate(Update upstream) {
                ...
                try (
                    ...
                    // When removing a row, record the stamp, redirection key, and prior redirection
                    // key. Binary search
                    // in the left for the removed key to find the smallest value geq the removed
                    // right. Update all rows
                    // with the removed redirection to the previous key.
                    try (
                        final OrderedKeys.Iterator removeit =
                            restampRemovals.getOrderedKeysIterator();
                        ...
                        while (removeit.hasMore()) {
                            ...
                            ssa.removeAndGetPrior(rightStampChunk, rightKeyIndices,
                                priorRedirections);
                            chunkSsaStamp.processRemovals(leftStampValues, leftStampKeys,
                                rightStampChunk, rightKeyIndices, priorRedirections,
                                redirectionIndex, modifiedBuilder, disallowExactMatch);
                        }
                    }

                    if (upstream.shifted.nonempty()) {
                        rightIncrementalApplySsaShift(upstream.shifted, ssa, sortKernel,
                            fillContext, restampRemovals, rightTable, rightChunkSize,
                            rightStampSource, chunkSsaStamp, leftStampValues, leftStampKeys,
                            redirectionIndex, disallowExactMatch);
                    }

                    // When adding a row to the right hand side: we need to know which left hand
                    // side might be
                    // responsive. If we are a duplicate stamp and not the last one, we ignore it.
                    // Next, we should binary
                    // search in the left for the first value >=, everything up until the next
                    // extant right value should be
                    // restamped with our value
                    try (
                        ...
                        final int chunks =
                            (restampAdditions.intSize() + control.rightChunkSize() - 1)
                                / control.rightChunkSize();
                        for (int ii = 0; ii < chunks; ++ii) {
                            final int startChunk = chunks - ii - 1;
                            try (final Index chunkOk = restampAdditions.subindexByPos(
                                ...
                                final int valuesWithNext = ssa.insertAndGetNextValue(stampChunk,
                                    insertedIndices, nextRightValue);

                                final boolean endsWithLastValue =
                                    valuesWithNext != stampChunk.size();
                                if (endsWithLastValue) {
                                    Assert.eq(valuesWithNext, "valuesWithNext",
                                        stampChunk.size() - 1, "stampChunk.size() - 1");
                                    stampChunk.setSize(valuesWithNext);
                                    stampChunkEquals.notEqual(stampChunk, nextRightValue,
                                        retainStamps);
                                ...
                                chunkSsaStamp.processInsertion(leftStampValues, leftStampKeys,
                                    stampChunk, insertedIndices, nextRightValue, redirectionIndex,
                                    modifiedBuilder, endsWithLastValue, disallowExactMatch);
                            }
                        }
                    }

                    // if the stamp was not modified, then we need to figure out the responsive rows
                    // to mark as modified
                    if (!stampModified && upstream.modified.nonempty()) {
                        try (
                            final OrderedKeys.Iterator modit =
                            ...
                            while (modit.hasMore()) {
                                ...
                                chunkSsaStamp.findModified(0, leftStampValues, leftStampKeys,
                                    redirectionIndex, rightStampChunk, rightStampIndices,
                                    modifiedBuilder, disallowExactMatch);
                            }
                        }
                    }
                ...
                if (stampModified || upstream.added.nonempty() || upstream.removed.nonempty()) {
                    // If we kept track of whether or not something actually changed, then we could
                    // skip painting all
                    // the right columns as modified. It is not clear whether it is worth the
    ...
    private static void rightIncrementalApplySsaShift(IndexShiftData shiftData,
        SegmentedSortedArray ssa,
            ...
            while (sit.hasNext()) {
                ...
                try (final Index indexToShift =
                    ...
                    if (sit.polarityReversed()) {
                        ...
                        chunkSsaStamp.applyShift(leftStampValues, leftStampKeys,
                            rightStampValues.get(), rightStampKeys.get(), sit.shiftDelta(),
                            redirectionIndex, disallowExactMatch);
                        ...
                        if (indexToShift.size() > chunkSize) {
                            try (final OrderedKeys.Iterator shiftIt =
                                ...
                                while (shiftIt.hasMore()) {
                                    final OrderedKeys chunkOk =
                                        shiftIt.getNextOrderedKeysWithLength(chunkSize);
                                    stampSource.fillPrevChunk(fillContext,
                                        rightStampValues.ensureCapacity(chunkSize), chunkOk);

                                    chunkOk.fillKeyIndicesChunk(
                                        rightStampKeys.ensureCapacity(chunkSize));

                                    sortKernel.sort(rightStampKeys.get(), rightStampValues.get());

                                    ssa.applyShift(rightStampValues.get(), rightStampKeys.get(),
                                        sit.shiftDelta());
                                    chunkSsaStamp.applyShift(leftStampValues, leftStampKeys,
                                        rightStampValues.get(), rightStampKeys.get(),
                                        sit.shiftDelta(), redirectionIndex, disallowExactMatch);
                                }
                            }
                        } else {
                            stampSource.fillPrevChunk(fillContext,
                                rightStampValues.ensureCapacity(indexToShift.intSize()),
                                indexToShift);
                            indexToShift.fillKeyIndicesChunk(
                                rightStampKeys.ensureCapacity(indexToShift.intSize()));
    ...
    private static Table zeroKeyAjRightStatic(QueryTable leftTable, Table rightTable,
        MatchPair[] columnsToAdd, MatchPair stampPair, ColumnSource<?> leftStampSource,
        ColumnSource<?> originalRightStampSource, ColumnSource<?> rightStampSource,
        SortingOrder order, boolean disallowExactMatch, final RedirectionIndex redirectionIndex) {
        ...
        final WritableLongChunk<KeyIndices> rightStampKeys =
            WritableLongChunk.makeWritableChunk(rightIndex.intSize());
        final WritableChunk<Values> rightStampValues =
            rightStampSource.getChunkType().makeWritableChunk(rightIndex.intSize());

        try (final SafeCloseableList chunksToClose =
            new SafeCloseableList(rightStampKeys, rightStampValues)) {
            final Supplier<String> keyStringSupplier = () -> "[] (zero key columns)";
            try (final AsOfStampContext stampContext = new AsOfStampContext(order,
                disallowExactMatch, leftStampSource, rightStampSource, originalRightStampSource)) {
                stampContext.getAndCompactStamps(rightIndex, rightStampKeys, rightStampValues);
                stampContext.processEntry(leftTable.getIndex(), rightStampValues, rightStampKeys,
                    redirectionIndex);
            }
            final QueryTable result = makeResult(leftTable, rightTable, redirectionIndex,
                columnsToAdd, leftTable.isRefreshing());
            if (!leftTable.isRefreshing()) {
                return result;
            }

            final ModifiedColumnSet leftStampColumn =
                leftTable.newModifiedColumnSet(stampPair.left());
            final ModifiedColumnSet allRightColumns =
                result.newModifiedColumnSet(MatchPair.getLeftColumns(columnsToAdd));
            final ModifiedColumnSet.Transformer leftTransformer =
                leftTable.newModifiedColumnSetTransformer(result,
            ...
            if (rightStampKeys.size() < rightIndex.size()) {
                compactedRightStampKeys =
                    WritableLongChunk.makeWritableChunk(rightStampKeys.size());
                compactedRightStampValues =
                    rightStampSource.getChunkType().makeWritableChunk(rightStampKeys.size());

                rightStampKeys.copyToChunk(0, compactedRightStampKeys, 0, rightStampKeys.size());
                rightStampValues.copyToChunk(0, compactedRightStampValues, 0,
                    rightStampKeys.size());
            } else {
                chunksToClose.clear();
    ...
    private static QueryTable makeResult(QueryTable leftTable, Table rightTable,
        RedirectionIndex redirectionIndex, MatchPair[] columnsToAdd, boolean refreshing) {
        ...
        Arrays.stream(columnsToAdd).forEach(mp -> {
            // noinspection unchecked
            final ReadOnlyRedirectedColumnSource rightSource = new ReadOnlyRedirectedColumnSource<>(
                redirectionIndex, rightTable.getColumnSource(mp.right()));
            if (refreshing) {
