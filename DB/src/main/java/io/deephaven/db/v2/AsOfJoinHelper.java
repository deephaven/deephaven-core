...
public class AsOfJoinHelper {
    ...
    static Table asOfJoin(JoinControl control, QueryTable leftTable, QueryTable rightTable, MatchPair[] columnsToMatch, MatchPair[] columnsToAdd, SortingOrder order, boolean disallowExactMatch) {
        ...
        if (!leftTable.isLive() && leftTable.size() == 0) {
            return makeResult(leftTable, rightTable, new StaticSingleValueRedirectionIndexImpl(Index.NULL_KEY), columnsToAdd, false);
        }
        ...
        final ColumnSource<?>[] originalLeftSources = Arrays.stream(columnsToMatch).limit(keyColumnCount).map(mp -> leftTable.getColumnSource(mp.leftColumn)).toArray(ColumnSource[]::new);
        ...
        final ColumnSource<?>[] originalRightSources = Arrays.stream(columnsToMatch).limit(keyColumnCount).map(mp -> rightTable.getColumnSource(mp.rightColumn)).toArray(ColumnSource[]::new);
        ...
        final ColumnSource<?> leftStampSource = ReinterpretUtilities.maybeConvertToPrimitive(leftTable.getColumnSource(stampPair.left()));
        ...
        if (rightTable.isLive()) {
            ...
            return rightStaticAj(control, leftTable, rightTable, columnsToMatch, columnsToAdd, order, disallowExactMatch, stampPair, originalLeftSources, leftSources, rightSources, leftStampSource, originalRightStampSource, rightStampSource, redirectionIndex);
        }
    }
    ...
    private static Table rightStaticAj(JoinControl control,
        ...
        final StaticChunkedAsOfJoinStateManager asOfJoinStateManager = new StaticChunkedAsOfJoinStateManager(leftSources, size, originalLeftSources);
        ...
        if (leftGrouping != null) {
            ...
            leftGroupedSources = AbstractColumnSource.groupingToFlatSources((ColumnSource) leftSources[0], leftGrouping, leftTable.getIndex(), groupSize);
        ...
        if (rightGrouping != null) {
            final MutableInt groupSize = new MutableInt();
            //noinspection unchecked
            rightGroupedSources = AbstractColumnSource.groupingToFlatSources((ColumnSource) rightSources[0], rightGrouping, rightTable.getIndex(), groupSize);
        ...
        if (buildLeft) {
            if (leftGroupedSources == null) {
                ...
                slotCount = asOfJoinStateManager.buildFromLeftSide(Index.CURRENT_FACTORY.getFlatIndex(leftGroupingSize), new ColumnSource[]{leftGroupedSources.getFirst()}, slots);
            }
            if (rightGroupedSources == null) {
                asOfJoinStateManager.probeRight(rightTable.getIndex(), rightSources);
            } else {
                asOfJoinStateManager.probeRight(Index.CURRENT_FACTORY.getFlatIndex(rightGroupingSize), new ColumnSource[]{rightGroupedSources.getFirst()});
            }
        } else {
            if (rightGroupedSources == null) {
                slotCount = asOfJoinStateManager.buildFromRightSide(rightTable.getIndex(), rightSources, slots);
            } else {
                slotCount = asOfJoinStateManager.buildFromRightSide(Index.CURRENT_FACTORY.getFlatIndex(rightGroupingSize), new ColumnSource[]{rightGroupedSources.getFirst()}, slots);
            }
            if (leftGroupedSources == null) {
                asOfJoinStateManager.probeLeft(leftTable.getIndex(), leftSources);
            } else {
                asOfJoinStateManager.probeLeft(Index.CURRENT_FACTORY.getFlatIndex(leftGroupingSize), new ColumnSource[]{leftGroupedSources.getFirst()});
            }
        }
        ...
        try (final AsOfStampContext stampContext = new AsOfStampContext(order, disallowExactMatch, leftStampSource, rightStampSource, originalRightStampSource);
             final ResettableWritableLongChunk<KeyIndices> keyChunk = ResettableWritableLongChunk.makeResettableChunk();
             final ResettableWritableChunk<Values> valuesChunk = rightStampSource.getChunkType().makeResettableWritableChunk()
        ) {
            for (int slotIndex = 0; slotIndex < slotCount; ++slotIndex) {
                ...
                if (arrayValuesCache != null) {
                    processLeftSlotWithRightCache(stampContext, leftIndex, rightIndex, redirectionIndex, rightStampSource, keyChunk, valuesChunk, arrayValuesCache, slot);
                } else {
        ...
        final QueryTable result = makeResult(leftTable, rightTable, redirectionIndex, columnsToAdd, leftTable.isRefreshing());
        if (!leftTable.isRefreshing()) {
            return result;
        }

        final ModifiedColumnSet leftKeysOrStamps = leftTable.newModifiedColumnSet(MatchPair.getLeftColumns(columnsToMatch));
        ...
        leftTable.listenForUpdates(new BaseTable.ShiftAwareListenerImpl(makeListenerDescription(columnsToMatch, stampPair, columnsToAdd, order == SortingOrder.Descending, disallowExactMatch), leftTable, result) {
            @Override
            public void onUpdate(Update upstream) {
                ...
                if (restampKeys.nonempty()) {
                    ...
                    final int slotCount = asOfJoinStateManager.probeLeft(restampKeys, leftSources, updatedSlots, foundBuilder);
    ...
    private static class ArrayValuesCache {
        ...
        long [] getKeys(long slot) {
            if (StaticChunkedAsOfJoinStateManager.isOverflowLocation(slot)) {
                return overflowCachedStampKeys.get(StaticChunkedAsOfJoinStateManager.hashLocationToOverflowLocation(slot));
            } else {
        ...
        Object getValues(long slot) {
            if (StaticChunkedAsOfJoinStateManager.isOverflowLocation(slot)) {
                return overflowCachedStampValues.get(StaticChunkedAsOfJoinStateManager.hashLocationToOverflowLocation(slot));
            } else {
    ...
    /**
     * If the asOfJoinStateManager is null, it means we are passing in the leftIndex.  If the leftIndex is null;
     * we are passing in the asOfJoinStateManager and should obtain the leftIndex from the state manager if necessary.
     */
    ...
    private static Table zeroKeyAj(JoinControl control, QueryTable leftTable, QueryTable rightTable, MatchPair[] columnsToAdd, MatchPair stampPair, ColumnSource<?> leftStampSource, ColumnSource<?> originalRightStampSource, ColumnSource<?> rightStampSource, SortingOrder order, boolean disallowExactMatch, final RedirectionIndex redirectionIndex) {
        if (rightTable.isLive() && leftTable.isLive()) {
            ...
            return zeroKeyAjRightStatic(leftTable, rightTable, columnsToAdd, stampPair, leftStampSource, originalRightStampSource, rightStampSource, order, disallowExactMatch, redirectionIndex);
        }
    }
    ...
    private static Table rightTickingLeftStaticAj(JoinControl control,
        ...
        final Supplier<SegmentedSortedArray> ssaFactory = SegmentedSortedArray.makeFactory(stampChunkType, reverse, control.rightSsaNodeSize());
        final ChunkSsaStamp chunkSsaStamp = ChunkSsaStamp.make(stampChunkType, reverse);

        final int tableSize = control.initialBuildSize();
        final RightIncrementalChunkedAsOfJoinStateManager asOfJoinStateManager = new RightIncrementalChunkedAsOfJoinStateManager(leftSources, tableSize);
        ...
        final SizedSafeCloseable<LongSortKernel<Values, KeyIndices>> sortContext = new SizedSafeCloseable<>(size -> LongSortKernel.makeContext(stampChunkType, order, size, true));
        final SizedSafeCloseable<ChunkSource.FillContext> leftStampFillContext = new SizedSafeCloseable<>(leftStampSource::makeFillContext);
        final SizedSafeCloseable<ChunkSource.FillContext> rightStampFillContext = new SizedSafeCloseable<>(rightStampSource::makeFillContext);
        ...
        try (final ResettableWritableLongChunk<KeyIndices> leftKeyChunk = ResettableWritableLongChunk.makeResettableChunk();
             final ResettableWritableChunk<Values> leftValuesChunk = rightStampSource.getChunkType().makeResettableWritableChunk()) {
            for (int slotIndex = 0; slotIndex < slotCount; ++slotIndex) {
                final long slot = slots.getLong(slotIndex);
                ...
                final SegmentedSortedArray rightSsa = asOfJoinStateManager.getRightSsa(slot, (rightIndex) -> {
                    ...
                    if (slotSize > 0) {
                        rightStampSource.fillChunk(rightStampFillContext.ensureCapacity(slotSize), rightValues.ensureCapacity(slotSize), rightIndex);
                ...
                getCachedLeftStampsAndKeys(null, leftIndex, leftStampSource, leftStampFillContext, sortContext, leftKeyChunk, leftValuesChunk, leftValuesCache, slot);

                if (rightSsa.size() == 0) {
                    continue;
                }

                final WritableLongChunk<KeyIndices> rightKeysForLeftChunk = rightKeysForLeft.ensureCapacity(leftIndex.intSize());

                chunkSsaStamp.processEntry(leftValuesChunk, leftKeyChunk, rightSsa, rightKeysForLeftChunk, disallowExactMatch);
        ...
        SafeCloseable.closeArray(sortContext, leftStampFillContext, rightStampFillContext, rightValues, rightKeyIndices, rightKeysForLeft);

        final QueryTable result = makeResult(leftTable, rightTable, redirectionIndex, columnsToAdd, true);

        final ModifiedColumnSet rightMatchColumns = rightTable.newModifiedColumnSet(MatchPair.getRightColumns(columnsToMatch));
        ...
        final ObjectArraySource<Index.SequentialBuilder> sequentialBuilders = new ObjectArraySource<>(Index.SequentialBuilder.class);

        rightTable.listenForUpdates(new BaseTable.ShiftAwareListenerImpl(makeListenerDescription(columnsToMatch, stampPair, columnsToAdd, reverse, disallowExactMatch), rightTable, result) {
            @Override
            public void onUpdate(Update upstream) {
                ...
                final int removedSlotCount = asOfJoinStateManager.markForRemoval(restampRemovals, rightSources, slots, sequentialBuilders);

                // Now that everything is marked, process the removals state by state, just as if we were doing the zero
                // key case: when removing a row, record the stamp, redirection key, and prior redirection key.  Binary search
                ...
                try (final ResettableWritableLongChunk<KeyIndices> leftKeyChunk = ResettableWritableLongChunk.makeResettableChunk();
                    ...
                    for (int slotIndex = 0; slotIndex < removedSlotCount; ++slotIndex) {
                        ...
                        rightStampSource.fillPrevChunk(rightStampFillContext.ensureCapacity(slotSize), rightValues.ensureCapacity(slotSize), rightRemoved);
                        rightRemoved.fillKeyIndicesChunk(rightKeyIndices.ensureCapacity(slotSize));
                        sortContext.ensureCapacity(slotSize).sort(rightKeyIndices.get(), rightValues.get());

                        getCachedLeftStampsAndKeys(asOfJoinStateManager, null, leftStampSource, leftStampFillContext, sortContext, leftKeyChunk, leftValuesChunk, leftValuesCache, slot);
                        ...
                        chunkSsaStamp.processRemovals(leftValuesChunk, leftKeyChunk, rightValues.get(), rightKeyIndices.get(), priorRedirections.get(), redirectionIndex, modifiedBuilder, disallowExactMatch);
                        ...
                        if (previousToShift.nonempty()) {
                            try (final ResettableWritableLongChunk<KeyIndices> leftKeyChunk = ResettableWritableLongChunk.makeResettableChunk();
                                ...
                                while (sit.hasNext()) {
                                    ...
                                    final int shiftedSlots = asOfJoinStateManager.gatherShiftIndex(indexToShift, rightSources, slots, sequentialBuilders);
                                    indexToShift.close();

                                    for (int slotIndex = 0; slotIndex < shiftedSlots; ++slotIndex) {
                                        final long slot = slots.getLong(slotIndex);
                                        try (final Index slotShiftIndex = sequentialBuilders.get(slotIndex).getIndex()) {
                                            sequentialBuilders.set(slotIndex, null);
                                            ...
                                            rightStampSource.fillPrevChunk(rightStampFillContext.ensureCapacity(shiftSize), rightValues.ensureCapacity(shiftSize), slotShiftIndex);

                                            final SegmentedSortedArray rightSsa = asOfJoinStateManager.getRightSsa(slot);

                                            slotShiftIndex.fillKeyIndicesChunk(rightKeyIndices.ensureCapacity(shiftSize));
                                            sortContext.ensureCapacity(shiftSize).sort(rightKeyIndices.get(), rightValues.get());

                                            if (sit.polarityReversed()) {
                                                chunkSsaStamp.applyShift(leftValuesChunk, leftKeyChunk, rightValues.get(), rightKeyIndices.get(), sit.shiftDelta(), redirectionIndex, disallowExactMatch);
                                                rightSsa.applyShiftReverse(rightValues.get(), rightKeyIndices.get(), sit.shiftDelta());
                                            } else {
                                                chunkSsaStamp.applyShift(leftValuesChunk, leftKeyChunk, rightValues.get(), rightKeyIndices.get(), sit.shiftDelta(), redirectionIndex, disallowExactMatch);
                                                rightSsa.applyShift(rightValues.get(), rightKeyIndices.get(), sit.shiftDelta());
                                            }
                                        }
                                    }
                                }
                ...
                final int addedSlotCount = asOfJoinStateManager.probeAdditions(restampAdditions, rightSources, slots, sequentialBuilders);
                ...
                try (final SizedChunk<Values> nextRightValue =  new SizedChunk<>(stampChunkType);
                     ...
                     final SizedSafeCloseable<ColumnSource.FillContext> rightStampFillContext = new SizedSafeCloseable<>(rightStampSource::makeFillContext);
                     final ResettableWritableLongChunk<KeyIndices> leftKeyChunk = ResettableWritableLongChunk.makeResettableChunk();
                     final ResettableWritableChunk<Values> leftValuesChunk = rightStampSource.getChunkType().makeResettableWritableChunk()
                ) {
                    ...
                    // responsive.  If we are a duplicate stamp and not the last one, we ignore it.  Next, we should binary
                    // search in the left for the first value >=, everything up until the next extant right value should be
                    ...
                    for (int slotIndex = 0; slotIndex < addedSlotCount; ++slotIndex) {
                        ...
                        rightStampSource.fillChunk(rightStampFillContext.ensureCapacity(rightSize), rightStampChunk.ensureCapacity(rightSize), rightAdded);
                        ...
                        final int valuesWithNext = rightSsa.insertAndGetNextValue(rightStampChunk.get(), insertedIndices.get(), nextRightValue.ensureCapacity(rightSize));

                        final boolean endsWithLastValue = valuesWithNext != rightStampChunk.get().size();
                        if (endsWithLastValue) {
                            Assert.eq(valuesWithNext, "valuesWithNext", rightStampChunk.get().size() - 1, "rightStampChunk.size() - 1");
                            rightStampChunk.get().setSize(valuesWithNext);
                            stampChunkEquals.notEqual(rightStampChunk.get(), nextRightValue.get(), retainStamps.ensureCapacity(rightSize));
                        ...
                        chunkSsaStamp.processInsertion(leftValuesChunk, leftKeyChunk, rightStampChunk.get(), insertedIndices.get(), nextRightValue.get(), redirectionIndex, modifiedBuilder, endsWithLastValue, disallowExactMatch);
                    }

                    // and then finally we handle the case where the keys and stamps were not modified, but we must identify
                    // the responsive modifications.
                    if (!keysModified && !stampModified && upstream.modified.nonempty()) {
                        // next we do the additions
                        final int modifiedSlotCount = asOfJoinStateManager.gatherModifications(upstream.modified, rightSources, slots, sequentialBuilders);
                        ...
                        for (int slotIndex = 0; slotIndex < modifiedSlotCount; ++slotIndex) {
                            ...
                            try (final Index rightModified = sequentialBuilders.get(slotIndex).getIndex()) {
                                ...
                                rightStampSource.fillChunk(rightStampFillContext.ensureCapacity(rightSize), rightValues.ensureCapacity(rightSize), rightModified);
                                ...
                                chunkSsaStamp.findModified(0, leftValuesChunk, leftKeyChunk, redirectionIndex, rightValues.get(), rightKeyIndices.get(), modifiedBuilder, disallowExactMatch);
                            }
                        }
    ...
    private static Table bothIncrementalAj(JoinControl control,
        ...
        final RightIncrementalChunkedAsOfJoinStateManager asOfJoinStateManager = new RightIncrementalChunkedAsOfJoinStateManager(leftSources, control.initialBuildSize());
        ...
        // These contexts and chunks will be closed when the SSA factory itself is closed by the destroy function of the BucketedChunkedAjMergedListener
        final SizedSafeCloseable<ColumnSource.FillContext> rightStampFillContext = new SizedSafeCloseable<>(rightStampSource::makeFillContext);
        final SizedSafeCloseable<LongSortKernel<Values, KeyIndices>> sortKernel = new SizedSafeCloseable<>(size -> LongSortKernel.makeContext(stampChunkType, order, size, true));
        final SizedSafeCloseable<ColumnSource.FillContext> leftStampFillContext = new SizedSafeCloseable<>(leftStampSource::makeFillContext);
        ...
        final SsaFactory rightSsaFactory = new SsaFactory() {
            ...
            public SegmentedSortedArray apply(Index rightIndex) {
                ...
                if (slotSize > 0) {
                    ...
                    rightStampSource.fillChunk(rightStampFillContext.ensureCapacity(slotSize), rightStampValues.ensureCapacity(slotSize), rightIndex);
        ...
        final SsaFactory leftSsaFactory = new SsaFactory() {
            ...
            public SegmentedSortedArray apply(Index leftIndex) {
                ...
                if (slotSize > 0) {
                    ...
                    leftStampSource.fillChunk(leftStampFillContext.ensureCapacity(slotSize), leftStampValues.ensureCapacity(slotSize), leftIndex);
        ...
        try (final SafeCloseableList closeableList = new SafeCloseableList(leftSsaFactory, rightSsaFactory)) {
            for (int slotIndex = 0; slotIndex < slotCount; ++slotIndex) {
                ...
                // if either initial state is empty, we would prefer to leave things as an index rather than process them into an ssa
                final byte state = asOfJoinStateManager.getState(slot);
                if ((state & RightIncrementalChunkedAsOfJoinStateManager.ENTRY_RIGHT_MASK) == RightIncrementalChunkedAsOfJoinStateManager.ENTRY_RIGHT_IS_EMPTY) {
                    continue;
                }
                if ((state & RightIncrementalChunkedAsOfJoinStateManager.ENTRY_LEFT_MASK) == RightIncrementalChunkedAsOfJoinStateManager.ENTRY_LEFT_IS_EMPTY) {
                    continue;
                }
        ...
        final String listenerDescription = makeListenerDescription(columnsToMatch, stampPair, columnsToAdd, reverse, disallowExactMatch);
    ...
    private static Table zeroKeyAjBothIncremental(JoinControl control, QueryTable leftTable, QueryTable rightTable, MatchPair[] columnsToAdd, MatchPair stampPair, ColumnSource<?> leftStampSource, ColumnSource<?> rightStampSource, SortingOrder order, boolean disallowExactMatch, final RedirectionIndex redirectionIndex) {
        ...
        final String listenerDescription = makeListenerDescription(MatchPair.ZERO_LENGTH_MATCH_PAIR_ARRAY, stampPair, columnsToAdd, reverse, disallowExactMatch);
        final JoinListenerRecorder leftRecorder = new JoinListenerRecorder(true, listenerDescription, leftTable, result);
        final JoinListenerRecorder rightRecorder = new JoinListenerRecorder(false, listenerDescription, rightTable, result);
    ...
    @NotNull
    private static String makeListenerDescription(MatchPair [] columnsToMatch, MatchPair stampPair, MatchPair[] columnsToAdd, boolean reverse, boolean disallowExactMatch) {
        final String stampString = disallowExactMatch ? makeDisallowExactStampString(stampPair, reverse) :  MatchPair.matchString(stampPair);
        return (reverse ? "r" : "") + "aj([" + MatchPair.matchString(columnsToMatch) + ", " + stampString + "], [" + MatchPair.matchString(columnsToAdd) + "])";
    }
    ...
    private static void fillSsaWithSort(QueryTable rightTable, ColumnSource<?> stampSource, int nodeSize, SegmentedSortedArray ssa, SortingOrder order) {
        try (final ColumnSource.FillContext context = stampSource.makeFillContext(nodeSize);
             ...
             final LongSortKernel<Values, KeyIndices> sortKernel = LongSortKernel.makeContext(stampSource.getChunkType(), order, nodeSize, true)) {
            while (okit.hasMore()) {
    ...
    private static Table zeroKeyAjRightIncremental(JoinControl control, QueryTable leftTable, QueryTable rightTable, MatchPair[] columnsToAdd, MatchPair stampPair, ColumnSource<?> leftStampSource, ColumnSource<?> rightStampSource, SortingOrder order, boolean disallowExactMatch, final RedirectionIndex redirectionIndex) {
        final boolean reverse = order == SortingOrder.Descending;
        ...
        try (final LongSortKernel<Values, KeyIndices> sortKernel = LongSortKernel.makeContext(stampChunkType, order, leftSize, true)) {
        ...
        rightTable.listenForUpdates(new BaseTable.ShiftAwareListenerImpl(makeListenerDescription(MatchPair.ZERO_LENGTH_MATCH_PAIR_ARRAY, stampPair, columnsToAdd, reverse, disallowExactMatch), rightTable, result) {
            @Override
            public void onUpdate(Update upstream) {
                final Update downstream = new Update();
                downstream.added = Index.FACTORY.getEmptyIndex();
                ...
                try (final ColumnSource.FillContext fillContext = rightStampSource.makeFillContext(rightChunkSize);
                     final LongSortKernel<Values, KeyIndices> sortKernel = LongSortKernel.makeContext(stampChunkType, order, rightChunkSize, true)
                ) {
                    ...
                    try (final OrderedKeys.Iterator removeit = restampRemovals.getOrderedKeysIterator();
                         final WritableLongChunk<KeyIndices> priorRedirections = WritableLongChunk.makeWritableChunk(rightChunkSize);
                         final WritableLongChunk<KeyIndices> rightKeyIndices = WritableLongChunk.makeWritableChunk(rightChunkSize);
                        ...
                        while (removeit.hasMore()) {
                            ...
                            chunkSsaStamp.processRemovals(leftStampValues, leftStampKeys, rightStampChunk, rightKeyIndices, priorRedirections, redirectionIndex, modifiedBuilder, disallowExactMatch);
                        }
                    }

                    if (upstream.shifted.nonempty()) {
                        rightIncrementalApplySsaShift(upstream.shifted, ssa, sortKernel, fillContext, restampRemovals, rightTable, rightChunkSize, rightStampSource, chunkSsaStamp, leftStampValues, leftStampKeys, redirectionIndex, disallowExactMatch);
                    }
                    ...
                    try (final WritableChunk<Values> stampChunk = stampChunkType.makeWritableChunk(rightChunkSize);
                         ...
                         final WritableLongChunk<KeyIndices> insertedIndices = WritableLongChunk.makeWritableChunk(rightChunkSize);
                         final WritableBooleanChunk<Any> retainStamps = WritableBooleanChunk.makeWritableChunk(rightChunkSize)
                         ) {
                        final int chunks = (restampAdditions.intSize() + control.rightChunkSize() - 1) / control.rightChunkSize();
                        for (int ii = 0; ii < chunks; ++ii) {
                            final int startChunk = chunks - ii - 1;
                            try (final Index chunkOk = restampAdditions.subindexByPos(startChunk * control.rightChunkSize(), (startChunk + 1) * control.rightChunkSize())) {
                                ...
                                final int valuesWithNext = ssa.insertAndGetNextValue(stampChunk, insertedIndices, nextRightValue);

                                final boolean endsWithLastValue = valuesWithNext != stampChunk.size();
                                if (endsWithLastValue) {
                                    Assert.eq(valuesWithNext, "valuesWithNext", stampChunk.size() - 1, "stampChunk.size() - 1");
                                ...
                                chunkSsaStamp.processInsertion(leftStampValues, leftStampKeys, stampChunk, insertedIndices, nextRightValue, redirectionIndex, modifiedBuilder, endsWithLastValue, disallowExactMatch);
                            }
                        }
                    }
                    ...
                    if (!stampModified && upstream.modified.nonempty()) {
                        try (final OrderedKeys.Iterator modit = upstream.modified.getOrderedKeysIterator();
                             final WritableLongChunk<KeyIndices> rightStampIndices = WritableLongChunk.makeWritableChunk(rightChunkSize);
                             final WritableChunk<Values> rightStampChunk = stampChunkType.makeWritableChunk(rightChunkSize)
                        ) {
                            while (modit.hasMore()) {
                                ...
                                chunkSsaStamp.findModified(0, leftStampValues, leftStampKeys, redirectionIndex, rightStampChunk, rightStampIndices, modifiedBuilder, disallowExactMatch);
                            }
                        }
                    }
                ...
                if (stampModified || upstream.added.nonempty() || upstream.removed.nonempty()) {
                    // If we kept track of whether or not something actually changed, then we could skip painting all
                    // the right columns as modified.  It is not clear whether it is worth the additional complexity.
                    downstream.modifiedColumnSet.setAll(allRightColumns);
                } else {
                    rightTransformer.transform(upstream.modifiedColumnSet, downstream.modifiedColumnSet);
                }
    ...
    private static void rightIncrementalApplySsaShift(IndexShiftData shiftData, SegmentedSortedArray ssa,
        ...
        try (final Index fullPrevIndex = table.getIndex().getPrevIndex();
             ...
             final SizedSafeCloseable<LongSortKernel<Values, KeyIndices>> shiftSortKernel = new SizedSafeCloseable<>(sz -> LongSortKernel.makeContext(stampSource.getChunkType(), ssa.isReversed() ? SortingOrder.Descending : SortingOrder.Ascending, sz, true));
                ...
                try (final Index indexToShift = previousToShift.subindexByKey(sit.beginRange(), sit.endRange())) {
                    ...
                    if (sit.polarityReversed()) {
                        ...
                        if (chunkSize >= shiftSize) {
                            stampSource.fillPrevChunk(fillContext, rightStampValues.ensureCapacity(shiftSize), indexToShift);
                            sortKernel.sort(rightStampKeys.get(), rightStampValues.get());
                        } else {
                            stampSource.fillPrevChunk(shiftFillContext.ensureCapacity(shiftSize), rightStampValues.ensureCapacity(shiftSize), indexToShift);
                            shiftSortKernel.ensureCapacity(shiftSize).sort(rightStampKeys.get(), rightStampValues.get());
                        }

                        chunkSsaStamp.applyShift(leftStampValues, leftStampKeys, rightStampValues.get(), rightStampKeys.get(), sit.shiftDelta(), redirectionIndex, disallowExactMatch);
                        ...
                        if (indexToShift.size() > chunkSize) {
                            try (final OrderedKeys.Iterator shiftIt = indexToShift.getOrderedKeysIterator()) {
                                while (shiftIt.hasMore()) {
                                    ...
                                    stampSource.fillPrevChunk(fillContext, rightStampValues.ensureCapacity(chunkSize), chunkOk);
                            ...
                            stampSource.fillPrevChunk(fillContext, rightStampValues.ensureCapacity(indexToShift.intSize()), indexToShift);
    ...
    private static Table zeroKeyAjRightStatic(QueryTable leftTable, Table rightTable, MatchPair[] columnsToAdd, MatchPair stampPair, ColumnSource<?> leftStampSource, ColumnSource<?> originalRightStampSource, ColumnSource<?> rightStampSource, SortingOrder order, boolean disallowExactMatch, final RedirectionIndex redirectionIndex) {
        ...
        final WritableChunk<Values> rightStampValues = rightStampSource.getChunkType().makeWritableChunk(rightIndex.intSize());
        ...
        try (final SafeCloseableList chunksToClose = new SafeCloseableList(rightStampKeys, rightStampValues)) {
            ...
            leftTable.listenForUpdates(new BaseTable.ShiftAwareListenerImpl(makeListenerDescription(MatchPair.ZERO_LENGTH_MATCH_PAIR_ARRAY, stampPair, columnsToAdd, order == SortingOrder.Descending, disallowExactMatch), leftTable, result) {
                @Override
                public void onUpdate(Update upstream) {
                    final Update downstream = upstream.copy();
                    ...
                    final boolean stampModified = upstream.modified.nonempty() && upstream.modifiedColumnSet.containsAny(leftStampColumn);
                    ...
                    try (final AsOfStampContext stampContext = new AsOfStampContext(order, disallowExactMatch, leftStampSource, rightStampSource, originalRightStampSource)) {
                        stampContext.processEntry(restampKeys, compactedRightStampValues, compactedRightStampKeys, redirectionIndex);
                    }
