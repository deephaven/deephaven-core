...
public class BitShiftingColumnSource<T> extends AbstractColumnSource<T> implements UngroupableColumnSource {
    ...
    public T getUngroupedPrev(long columnIndex, int arrayIndex) {
        //noinspection unchecked
        return (T) ((UngroupableColumnSource) innerSource).getUngroupedPrev(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }

    @Override
    public Boolean getUngroupedBoolean(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedBoolean(shiftState.getShifted(columnIndex), arrayIndex);
    }

    @Override
    public Boolean getUngroupedPrevBoolean(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevBoolean(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }

    @Override
    public double getUngroupedDouble(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedDouble(shiftState.getShifted(columnIndex), arrayIndex);
    }

    @Override
    public double getUngroupedPrevDouble(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevDouble(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }

    @Override
    public float getUngroupedFloat(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedFloat(shiftState.getShifted(columnIndex), arrayIndex);
    }

    @Override
    public float getUngroupedPrevFloat(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevFloat(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }
    ...
    public byte getUngroupedPrevByte(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevByte(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }
    ...
    public char getUngroupedPrevChar(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevChar(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }

    @Override
    public short getUngroupedShort(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedShort(shiftState.getShifted(columnIndex), arrayIndex);
    }

    @Override
    public short getUngroupedPrevShort(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevShort(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }
    ...
    public int getUngroupedPrevInt(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevInt(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }
    ...
    public long getUngroupedPrevLong(long columnIndex, int arrayIndex) {
        return ((UngroupableColumnSource) innerSource).getUngroupedPrevLong(shiftState.getPrevShifted(columnIndex), arrayIndex);
    }
    ...
    private class ReinterpretToOriginal<ALTERNATE_DATA_TYPE> extends BitShiftingColumnSource<ALTERNATE_DATA_TYPE> {
        private ReinterpretToOriginal(Class<ALTERNATE_DATA_TYPE> alternateDataType) {
            super(BitShiftingColumnSource.this.shiftState, BitShiftingColumnSource.this.innerSource.reinterpret(alternateDataType));
        }
    ...
    private static class FillContext implements ColumnSource.FillContext {
        ...
        private FillContext(final BitShiftingColumnSource cs, final int chunkCapacity, final SharedContext sharedContext) {
        ...
        private static final class Shareable extends SharedContext {
            ...
            private void ensureKeysAndLengthsInitialized(@NotNull final CrossJoinShiftState shiftState, final boolean usePrev, @NotNull final OrderedKeys orderedKeys) {
                ...
                orderedKeys.forAllLongs((final long indexKey) -> {
                    ...
                    final long innerIndexKey = usePrev ? shiftState.getPrevShifted(indexKey) : shiftState.getShifted(indexKey);
    ...
    private void doFillChunk(@NotNull final ColumnSource.FillContext context,
        ...
        try (final OrderedKeys innerOK = OrderedKeys.wrapKeyIndicesChunkAsOrderedKeys(effectiveContext.shareable.uniqueIndices)) {
            if (usePrev) {
        ...
        effectiveContext.dupExpandKernel.expandDuplicates(orderedKeys.intSize(), destination, effectiveContext.shareable.runLengths);
    }
}
