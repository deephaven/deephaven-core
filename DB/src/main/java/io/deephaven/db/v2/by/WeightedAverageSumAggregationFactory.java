...
public class WeightedAverageSumAggregationFactory implements AggregationContextFactory {
    ...
    private AggregationContext getAllColumnOperators(Table withView, String[] groupByNameArray) {
        ...
        final int operatorColumnCount =
            withView.getColumnSourceMap().size() - groupByNames.size() - 1;

        final List<IterativeChunkedAggregationOperator> operators =
            new ArrayList<>(operatorColumnCount + 1);
        ...
        boolean weightSourceIsFloatingPoint = weightSource.getChunkType() == ChunkType.Double
            || weightSource.getChunkType() == ChunkType.Float;
        boolean anyIntegerResults = !weightSourceIsFloatingPoint && isSum
            && withView.getColumnSourceMap().values().stream()
                .anyMatch(cs -> cs.getChunkType() == ChunkType.Long
                    || cs.getChunkType() == ChunkType.Int || cs.getChunkType() == ChunkType.Short
                    || cs.getChunkType() == ChunkType.Byte || cs.getChunkType() == ChunkType.Char);
        boolean anyFloatResults = weightSourceIsFloatingPoint || !isSum
            || withView.getColumnSourceMap().values().stream()
                .anyMatch(cs -> cs.getChunkType() == ChunkType.Float
                    || cs.getChunkType() == ChunkType.Double);
        ...
        if (anyIntegerResults) {
            longWeightOperator =
                new LongWeightRecordingInternalOperator(weightSource.getChunkType());
            // noinspection unchecked
            inputColumns.add(weightSource);
        ...
        withView.getColumnSourceMap().forEach((name, columnSource) -> {
            ...
            if (isSum) {
                    ...
                    switch (columnSource.getChunkType()) {
                        ...
                        default:
                            throw new UnsupportedOperationException(
                                "Invalid chunk type for weightedSum: "
                ...
                if (isInteger) {
                    ...
                    operators.add(new LongChunkedWeightedSumOperator(columnSource.getChunkType(),
                        longWeightOperator, name));
                } else {
