...
public class WeightedAverageSumAggregationFactory implements AggregationContextFactory {
    ...
    private AggregationContext getAllColumnOperators(Table withView, String [] groupByNameArray) {
        ...
        boolean weightSourceIsFloatingPoint = weightSource.getChunkType() == ChunkType.Double || weightSource.getChunkType() == ChunkType.Float;
        boolean anyIntegerResults = !weightSourceIsFloatingPoint && isSum && withView.getColumnSourceMap().values().stream().anyMatch(cs -> cs.getChunkType() == ChunkType.Long || cs.getChunkType() == ChunkType.Int || cs.getChunkType() == ChunkType.Short || cs.getChunkType() == ChunkType.Byte || cs.getChunkType() == ChunkType.Char);
        boolean anyFloatResults = weightSourceIsFloatingPoint || !isSum || withView.getColumnSourceMap().values().stream().anyMatch(cs -> cs.getChunkType() == ChunkType.Float || cs.getChunkType() == ChunkType.Double);
        ...
        withView.getColumnSourceMap().forEach((name, columnSource) -> {
            ...
            if (isSum) {
                ...
                if (weightSourceIsFloatingPoint) {
                    ...
                    switch (columnSource.getChunkType()) {
                        ...
                        case Float:
                            ...
                            break;
                        default:
                            throw new UnsupportedOperationException("Invalid chunk type for weightedSum: " + columnSource.getChunkType());
                    }
                }
                ...
                if (isInteger) {
                    ...
                    operators.add(new LongChunkedWeightedSumOperator(columnSource.getChunkType(), longWeightOperator, name));
                } else {
                    floatColumnNames.add(name);
                    operators.add(new DoubleChunkedWeightedSumOperator(columnSource.getChunkType(), doubleWeightOperator, name));
                }
            } else {
                isIntegerResult.add(false);
                floatColumnNames.add(name);
                operators.add(new ChunkedWeightedAverageOperator(columnSource.getChunkType(), doubleWeightOperator, name));
            }
        });
