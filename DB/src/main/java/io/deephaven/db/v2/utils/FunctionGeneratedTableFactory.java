 ...
 *
 * The table will refresh by regenerating the full values (using the tableGenerator Function passed
 * in). The resultant table's values are copied into the result table and appropriate listener
 * notifications are fired.
 *
 * All of the rows in the output table are modified on every tick, even if no actual changes
 * occurred. The output table also has a contiguous index.
 *
 * The generator function must produce a V2 table, and the table definition must not change between
 * invocations.
 *
 * If you are transforming a table, you should generally prefer to use the regular table operations
 * as opposed to this factory, because they are capable of performing some operations incrementally.
 * However, for small tables this might prove to require less development effort.
 */
public class FunctionGeneratedTableFactory {
    ...
    /**
     * Create a table that refreshes based on the value of your function, automatically called every
     * refreshIntervalMs.
     * 
     * @param tableGenerator a function returning a table to copy into the output table
     * @return a ticking table (assuming sourceTables have been specified) generated by
     *         tableGenerator
     */
    ...
    /**
     * Create a table that refreshes based on the value of your function, automatically called when
     * any of the sourceTables tick.
     * 
     * @param tableGenerator a function returning a table to copy into the output table
     * @param sourceTables The query engine does not know the details of your function inputs. If
     *        you are dependent on a ticking table tables in your tableGenerator function, you can
     *        add it to this list so that the function will be recomputed on each tick.
     * @return a ticking table (assuming sourceTables have been specified) generated by
     *         tableGenerator
     */
    public static Table create(Function.Nullary<Table> tableGenerator,
        DynamicTable... sourceTables) {
        final FunctionGeneratedTableFactory factory =
            new FunctionGeneratedTableFactory(tableGenerator, 0);

        final FunctionBackedTable result = factory.getTable();

        for (DynamicTable source : sourceTables) {
            source.listenForUpdates(
                new BaseTable.ShiftAwareListenerImpl("FunctionGeneratedTable", source, result) {
                    @Override
    ...
    private FunctionGeneratedTableFactory(final Function.Nullary<Table> tableGenerator,
        ...
        for (Map.Entry<String, ? extends ColumnSource> entry : initialTable.getColumnSourceMap()
            .entrySet()) {
            ColumnSource columnSource = entry.getValue();
            final ArrayBackedColumnSource memoryColumnSource = ArrayBackedColumnSource
                .getMemoryColumnSource(0, columnSource.getType(), columnSource.getComponentType());
    ...
    private class FunctionBackedTable extends QueryTable implements LiveTable {
        ...
        protected void doRefresh() {
            ...
            if (size > 0) {
                ...
                final Index modified = index.clone();
                notifyListeners(Index.FACTORY.getEmptyIndex(), Index.FACTORY.getEmptyIndex(),
                    modified);
            }
        }
