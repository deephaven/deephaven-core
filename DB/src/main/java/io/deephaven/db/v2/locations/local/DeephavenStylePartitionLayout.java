 ...
 * format, e.g. <pre>tableRootDirectory/internalPartitionValue/columnPartitionValue/tableName/...</pre>, producing
    ...
    public final void findKeys(@NotNull final Consumer<TLK> locationKeyObserver) {
        ...
        PrivilegedFileAccessUtil.doFilesystemAction(() -> {
            try (final DirectoryStream<Path> internalPartitionStream = Files.newDirectoryStream(tableRootDirectory.toPath(), Files::isDirectory)) {
                ...
                throw new TableDataException("Error finding locations for " + tableName + " under " + tableRootDirectory, e);
            }
        });
    }

    protected abstract TLK makeKey(@NotNull final Path tableLeafDirectory, @NotNull final Map<String, Comparable<?>> partitions);
}
