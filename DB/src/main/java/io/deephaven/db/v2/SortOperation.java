...
public class SortOperation implements QueryTable.MemoizableOperation<QueryTable> {
    ...
    public SortOperation(QueryTable parent, SortPair[] sortPairs) {
        ...
        this.sortOrder =
            Arrays.stream(sortPairs).map(SortPair::getOrder).toArray(SortingOrder[]::new);
        ...
        for (int ii = 0; ii < sortColumnNames.length; ++ii) {
            // noinspection unchecked
            sortColumns[ii] =
                QueryTable.maybeTransformToPrimitive(parent.getColumnSource(sortColumnNames[ii]));
            ...
            Require.requirement(
                ...
                sortColumnNames[ii], "sortColumnNames[ii]", sortColumns[ii].getType(),
    ...
    private static boolean alreadySorted(final QueryTable parent,
        @NotNull final SortHelpers.SortMapping sortedKeys) {
    ...
    private QueryTable historicalSort(SortHelpers.SortMapping sortedKeys) {
        ...
        for (Map.Entry<String, ColumnSource> stringColumnSourceEntry : this.parent
            .getColumnSourceMap().entrySet()) {
            // noinspection unchecked
            resultMap.put(stringColumnSourceEntry.getKey(), new ReadOnlyRedirectedColumnSource<>(
                sortMapping, stringColumnSourceEntry.getValue()));
        }
    ...
    @NotNull
    private Result<QueryTable> streamSort(
        @NotNull final SortHelpers.SortMapping initialSortedKeys) {
        ...
        final MutableObject<LongChunkColumnSource> recycledInnerRedirectionSource =
            new MutableObject<>();
        final SwitchColumnSource<Long> redirectionSource =
            new SwitchColumnSource<>(initialInnerRedirectionSource,
                (final ColumnSource<Long> previousInnerRedirectionSource) -> {
                    final LongChunkColumnSource recycled =
                        (LongChunkColumnSource) previousInnerRedirectionSource;
                    recycled.clear();
        ...
        final Map<String, ColumnSource<?>> resultMap = new LinkedHashMap<>();
        for (Map.Entry<String, ColumnSource> stringColumnSourceEntry : parent.getColumnSourceMap()
            .entrySet()) {
            // noinspection unchecked
            resultMap.put(stringColumnSourceEntry.getKey(), new ReadOnlyRedirectedColumnSource<>(
                sortMapping, stringColumnSourceEntry.getValue()));
        ...
        final ShiftAwareListener resultListener =
            new BaseTable.ShiftAwareListenerImpl("Stream sort listener", parent, resultTable) {
                ...
                public void onUpdate(@NotNull final Update upstream) {
                    ...
                    final LongChunkColumnSource recycled =
                        recycledInnerRedirectionSource.getValue();
                    ...
                    if (updateSortedKeys.size() > 0) {
                        updateInnerRedirectSource.addChunk(WritableLongChunk
                            .writableChunkWrap(updateSortedKeys.getArrayMapping()));
                    }
                    ...
                    final Index removed =
                        Index.CURRENT_FACTORY.getFlatIndex(upstream.removed.size());
                    ...
                    resultTable.notifyListeners(
                        new Update(added, removed, Index.CURRENT_FACTORY.getEmptyIndex(),
    ...
    public Result<QueryTable> initialize(boolean usePrev, long beforeClock) {
        ...
        if (parent.isStream()) {
            try (
                final ReadOnlyIndex prevIndex = usePrev ? parent.getIndex().getPrevIndex() : null) {
        ...
        try (final SafeCloseableList closer = new SafeCloseableList()) {
            ...
            final Index indexToSort =
                usePrev ? closer.add(parent.getIndex().getPrevIndex()) : parent.getIndex();

            if (indexToSort.size() >= Integer.MAX_VALUE) {
                throw new UnsupportedOperationException(
                    "Can not perform ticking sort for table larger than " + Integer.MAX_VALUE
                        + " rows, table is" + indexToSort.size());
            }

            final long[] sortedKeys = SortHelpers
                .getSortedKeys(sortOrder, sortColumns, indexToSort, usePrev).getArrayMapping();
            ...
            // fillFromChunk may convert the provided OrderedKeys to a KeyRanges (or KeyIndices)
            // chunk that is owned by
            ...
            for (Map.Entry<String, ColumnSource> stringColumnSourceEntry : parent
                .getColumnSourceMap().entrySet()) {
                // noinspection unchecked
                resultMap.put(stringColumnSourceEntry.getKey(),
                    new ReadOnlyRedirectedColumnSource<>(sortMapping,
                        stringColumnSourceEntry.getValue()));
            }
            ...
            // we also reinterpret our sortedColumnsToSortBy, which are guaranteed to be redirected
            // sources of the inner source
            for (int ii = 0; ii < sortedColumnsToSortBy.length; ++ii) {
                // noinspection unchecked
                sortedColumnsToSortBy[ii] =
                    QueryTable.maybeTransformToPrimitive(sortedColumnsToSortBy[ii]);
            }

            resultTable = new QueryTable(resultIndex, resultMap);
            parent.copyAttributes(resultTable, BaseTable.CopyAttributeOperation.Sort);

            final SortListener listener =
                new SortListener(parent, resultTable, reverseLookup, sortColumns, sortOrder,
                    sortMapping, sortedColumnsToSortBy,
