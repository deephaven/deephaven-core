...
public interface TableMap extends TransformableTableMap, LivenessNode {
     ...
     * Note that this causes the current {@link io.deephaven.db.util.liveness.LivenessManager}
     * (see {@link io.deephaven.db.util.liveness.LivenessScopeStack}) to manage the result if non-null.
     *
     * @param key key
    ...
    /**
     * When creating the table map, some of the keys that we would like to be there eventually may not exist.  This
     ...
     * @return a new TableMap where each table has had function applied
     */
    default TableMap transformTables(TableDefinition returnDefinition, java.util.function.Function<Table, Table> function) {
     ...
     * new keys are added.
     * <p>
     * The function may be passed a sentinel key, which can be checked with {@link TableMap#isSentinel(Object)}.
     * On the sentinel key, the function will be passed in an empty table, and is expected to return
     * an empty table of the proper definition. To avoid this sentinel invocation, callers can be
     * explicit and use {@link #transformTablesWithKey(TableDefinition, BiFunction)}.
     *
     ...
     * @return a new TableMap where each table has had function applied
     */
    TableMap transformTablesWithKey(TableDefinition returnDefinition, java.util.function.BiFunction<Object, Table, Table> function);
     ...
     * @param function the function to apply to each table in this TableMap, the tables in this map are the first argument
