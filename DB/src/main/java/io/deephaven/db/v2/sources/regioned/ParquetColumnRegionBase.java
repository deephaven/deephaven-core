...
public abstract class ParquetColumnRegionBase<ATTR extends Any>
    ...
    ParquetColumnRegionBase(final long pageMask,
        ...
        // We are making the following assumptions, so these basic functions are inlined rather than
        // virtual calls.
        Require.eq(columnChunkPageStore.mask(), "columnChunkPageStore.mask()", mask(),
            "ColumnRegion.mask()");
        Require.eq(columnChunkPageStore.firstRowOffset(), "columnChunkPageStore.firstRowOffset()",
            firstRowOffset(), "ColumnRegion.firstrRowOffset()");
    }
