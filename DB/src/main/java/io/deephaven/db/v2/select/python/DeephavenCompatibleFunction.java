...
import io.deephaven.db.v2.select.ConditionFilter.FilterKernel.Context;
 ...
 * information, used to help implement {@link io.deephaven.db.v2.select.AbstractConditionFilter} and
 * {@link FormulaColumnPython}.
 */
public class DeephavenCompatibleFunction {
    ...
    public static DeephavenCompatibleFunction create(
        ...
        // todo: python can't convert from java type to Class<?> (ie,
        // java_func_on_type(jpy.get_type('...')))
        ...
        return new DeephavenCompatibleFunction(function, (Class) returnedType,
            Arrays.asList(columnNames), isVectorized);
    }
    ...
    public FormulaKernel toFormulaKernel() {
        return isVectorized ? new FormulaKernelPythonChunkedFunction(function)
            : new io.deephaven.db.v2.select.python.FormulaKernelPythonSingularFunction(function);
    }

    public FilterKernel<Context> toFilterKernel() {
        if (returnedType != boolean.class) {
            throw new IllegalStateException(
                "FilterKernel functions must be annotated with a boolean return type");
        }
