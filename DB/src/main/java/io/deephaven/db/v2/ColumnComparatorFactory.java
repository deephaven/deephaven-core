...
public class ColumnComparatorFactory {
     ...
     * To do this we look at the underlying types of the column sources (actually we require those
     * underlying types to be the same) and we use this information to call the specific primitive
     * type getter method (whether {@link ColumnSource#getDouble}, {@link ColumnSource#getLong} and
     * so on). This approach allows us to avoid boxing on these calls. We use a similar approach in
     * order to test the null-ness of a given element.
     *
     * This method is not especially efficient, but (because we are typically not being called from
     * an inner loop), that fact is probably not relevant for performance. The point is that the
     * returned IComparator *is* rather efficient.
     * 
     * @param lcs The left-hand ColumnSource (uses current values)
     * @param rcs The right-hand ColumnSource (uses current values)
     * @return An AbstractColumnSource.IComparator designed to compare elements from the two column
     *         sources.
     */
    public static IComparator createComparator(final ColumnSource lcs,
        ...
        if (provides.test(byte.class)) {
            return (lKey, rKey) -> {
                ...
                return l == r ? 0
                    : (l == NULL_BYTE ? -1 : (r == NULL_BYTE ? 1 : Byte.compare(l, r)));
            };
        }
        ...
        if (provides.test(short.class)) {
            return (lKey, rKey) -> {
                ...
                return l == r ? 0
                    : (l == NULL_SHORT ? -1 : (r == NULL_SHORT ? 1 : Short.compare(l, r)));
            };
        }
        if (provides.test(int.class)) {
            return (lKey, rKey) -> {
                ...
                return l == r ? 0
                    : (l == NULL_INT ? -1 : (r == NULL_INT ? 1 : Integer.compare(l, r)));
            };
        }
        if (provides.test(long.class)) {
            return (lKey, rKey) -> {
                ...
                return l == r ? 0
                    : (l == NULL_LONG ? -1 : (r == NULL_LONG ? 1 : Long.compare(l, r)));
            };
        }
        if (provides.test(float.class)) {
            return (lKey, rKey) -> DhFloatComparisons.compare(lcs.getFloat(lKey),
                rcs.getFloat(rKey));
        }
        if (provides.test(double.class)) {
            return (lKey, rKey) -> DhDoubleComparisons.compare(lcs.getDouble(lKey),
                rcs.getDouble(rKey));
        }
     ...
     * This method is not especially efficient, but (because we are typically not being called from
     * an inner loop), that fact is probably not relevant for performance. The point is that the
     * returned IComparatorEnhanced *is* rather efficient.
     * 
     * @param lcs The left-hand ColumnSource (uses current values)
    ...
    public static IComparator createComparatorLeftCurrRightPrev(
        ...
        if (provides.test(char.class)) {
            return (lKey, rKey) -> {
                ...
                return l == r ? 0
                    : (l == NULL_CHAR ? -1 : (r == NULL_CHAR ? 1 : Character.compare(l, r)));
            };
        }
        if (provides.test(short.class)) {
            return (lKey, rKey) -> {
        ...
        if (provides.test(float.class)) {
            return (lKey, rKey) -> {
                ...
                return l == r ? 0
                    : (l == NULL_FLOAT ? -1 : (r == NULL_FLOAT ? 1 : Float.compare(l, r)));
            };
        }
        if (provides.test(double.class)) {
            return (lKey, rKey) -> {
                ...
                return l == r ? 0
                    : (l == NULL_DOUBLE ? -1 : (r == NULL_DOUBLE ? 1 : Double.compare(l, r)));
            };
        }
        // fall through to Object interface
        return (lKey, rKey) -> {
