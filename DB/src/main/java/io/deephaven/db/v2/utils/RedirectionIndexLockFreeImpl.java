...
public class RedirectionIndexLockFreeImpl implements RedirectionIndex {
    private static final float LOAD_FACTOR = (float) Configuration.getInstance().getDoubleWithDefault("RedirectionIndexK4V4Impl.loadFactor", 0.5);
    /**
     * The special "key not found" value used in the 'baseline' map is -1.
     * However, for the 'updates' map, the "key not found" value is -2. This allows us to use the updates map to
    ...
    /**
     * Commits the 'updates' map into the 'baseline' map, then resets the 'updates' map to empty. The only caller
     * should be Writer@Idle, via the TerminalNotification.
     */
    ...
    private static final int hashBucketWidth = Configuration.getInstance().getIntegerForClassWithDefault(RedirectionIndexLockFreeImpl.class, "hashBucketWidth", 1);
    ...
    @NotNull
    private static TNullableLongLongMap createMapWithCapacity(int initialCapacity, float loadFactor, long noEntryValue) {
        switch (hashBucketWidth) {
            case 1: return new HashMapLockFreeK1V1(initialCapacity, loadFactor, noEntryValue);
            case 2: return new HashMapLockFreeK2V2(initialCapacity, loadFactor, noEntryValue);
            case 4: return new HashMapLockFreeK4V4(initialCapacity, loadFactor, noEntryValue);
            default: throw new UnsupportedOperationException("Unsupported hashBucketWidth setting: " + hashBucketWidth);
        }
    }
}
