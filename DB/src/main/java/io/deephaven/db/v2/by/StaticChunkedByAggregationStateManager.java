...
import io.deephaven.db.v2.hashing.ObjectChunkIdentityEquals;
...
import io.deephaven.db.v2.ReplicateHashTable;
...
{
    // region constants
    private static final int CHUNK_SIZE = IncrementalChunkedByAggregationStateManager.CHUNK_SIZE;
    private static final int MINIMUM_INITIAL_HASH_SIZE = IncrementalChunkedByAggregationStateManager.MINIMUM_INITIAL_HASH_SIZE;
    private static final int MAX_TABLE_SIZE = IncrementalChunkedByAggregationStateManager.MAX_TABLE_SIZE;
    // endregion constants
    ...
    private static final int MINIMUM_OVERFLOW_HASH_SLOT = IncrementalChunkedByAggregationStateManager.MINIMUM_OVERFLOW_HASH_SLOT;
    ...
    private final ObjectArraySource<Object> indexSource
            // @StateColumnSourceConstructor@ from \QObjectArraySource<>(Object.class)\E
            = new ObjectArraySource<>(Object.class);
    ...
    // @StateColumnSourceType@ from \QObjectArraySource<Object>\E
    private final ObjectArraySource<Object> overflowIndexSource
            // @StateColumnSourceConstructor@ from \QObjectArraySource<>(Object.class)\E
            = new ObjectArraySource<>(Object.class);
    ...
    private final IntegerArraySource statePositionInInsertedHashSlots = new IntegerArraySource();
    private final IntegerArraySource overflowStatePositionInInsertedHashSlots = new IntegerArraySource();
    // endregion extra variables

    StaticChunkedByAggregationStateManager(ColumnSource<?>[] tableKeySources
                                         , int tableSize
    ...
    private void ensureCapacity(int tableSize) {
        ...
        statePositionInInsertedHashSlots.ensureCapacity(tableSize);
    ...
    private void ensureOverflowCapacity(WritableIntChunk<ChunkPositions> chunkPositionsToInsertInOverflow) {
        ...
        overflowStatePositionInInsertedHashSlots.ensureCapacity(newCapacity);
        // endregion ensureOverflowCapacity
    }

    // region build wrappers
    int buildTable(@NotNull final Table sourceTable, @NotNull final ColumnSource<?>[] keySources, @NotNull final IntegerArraySource insertedHashSlots) {
    ...
    class BuildContext implements Context {
        ...
        // @WritableStateChunkType@ from \QWritableObjectChunk<Object,Values>\E
        final WritableObjectChunk<Object,Values> workingStateEntries;
    ...
    private void buildTable(final BuildContext bc,
                            ...
                            , final IntegerArraySource insertedHashSlots
        ...
        try (final OrderedKeys.Iterator okIt = buildIndex.getOrderedKeysIterator();
            ...
            final WritableIntChunk<KeyIndices> sourceChunkInsertedHashSlots = WritableIntChunk.makeWritableChunk(bc.chunkSize);
            ...
            while (okIt.hasMore()) {
                ...
                chunkOk.fillKeyIndicesChunk(sourceChunkIndexKeys);
                sourceChunkInsertedHashSlots.setSize(bc.hashChunk.size());
                ...
                // @StateChunkIdentityName@ from \QObjectChunkIdentity\E
                ObjectChunkIdentityEquals.notEqual(bc.workingStateEntries, EMPTY_VALUE, bc.filledValues);
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ++ii) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        ((Index.SequentialBuilder) indexSource.get(tableLocation)).appendKey(sourceChunkIndexKeys.get(ii));
                ...
                for (int ii = 0; ii < bc.insertPositionsInSourceChunk.size(); ) {
                    ...
                    // region main insert
                    final Index.SequentialBuilder mainSlotBuilder = Index.CURRENT_FACTORY.getSequentialBuilder();
                    ...
                    sourceChunkInsertedHashSlots.set(firstChunkPositionForHashLocation, (int) currentHashLocation);
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        ((Index.SequentialBuilder) indexSource.get(tableLocation)).appendKey(sourceChunkIndexKeys.get(chunkPosition));
                ...
                if (bc.overflowPositionInSourceChunk.size() > 0) {
                    ...
                    while (bc.overflowPositionInSourceChunk.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.equalValues.size(); ++ii) {
                            ...
                            if (bc.equalValues.get(ii)) {
                                ...
                                ((Index.SequentialBuilder) overflowIndexSource.get(overflowLocation)).appendKey(sourceChunkIndexKeys.get(chunkPosition));
                                sourceChunkInsertedHashSlots.set(chunkPosition, EXISTING_HASH_SLOT);
                    ...
                    while (bc.chunkPositionsToInsertInOverflow.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.chunkPositionsToInsertInOverflow.size(); ) {
                            ...
                            final Index.SequentialBuilder overflowSlotBuilder = Index.CURRENT_FACTORY.getSequentialBuilder();
                            ...
                            sourceChunkInsertedHashSlots.set(chunkPosition, overflowLocationToHashLocation(allocatedOverflowLocation));
                        ...
                        for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                            ...
                            if (bc.equalValues.get(ii)) {
                                ...
                                ((Index.SequentialBuilder) overflowIndexSource.get(insertedOverflowLocation)).appendKey(sourceChunkIndexKeys.get(chunkPosition));
                                sourceChunkInsertedHashSlots.set(chunkPosition, EXISTING_HASH_SLOT);
                    ...
                    doRehash(bc, sourceChunkInsertedHashSlots, insertedHashSlots);
                    // endregion post-build rehash
                    // endmixin rehash
                }

                // region copy hash slots
                // We need to take care of setting our inserted hash slots before rehash can move them
                int insertedHashSlotPosition = insertedHashSlotsLength.intValue();
                insertedHashSlots.ensureCapacity(insertedHashSlotPosition + sourceChunkInsertedHashSlots.size());
                for (int ii = 0; ii < sourceChunkInsertedHashSlots.size(); ++ii) {
                    final int insertedHashSlot = sourceChunkInsertedHashSlots.get(ii);
                    if (insertedHashSlot != EXISTING_HASH_SLOT) {
                        insertedHashSlots.set(insertedHashSlotPosition, insertedHashSlot);
                        if (isOverflowLocation(insertedHashSlot)) {
                            overflowStatePositionInInsertedHashSlots.set(hashLocationToOverflowLocation(insertedHashSlot), insertedHashSlotPosition);
                        } else {
                            statePositionInInsertedHashSlots.set(insertedHashSlot, insertedHashSlotPosition);
                        }
                        ++insertedHashSlotPosition;
                    }
                }
                insertedHashSlotsLength.setValue(insertedHashSlotPosition);
                // endregion copy hash slots
                hashSlotOffset += chunkOk.size();
            }
            // region post build loop
            sourceChunkIndexKeys.close();
            sourceChunkInsertedHashSlots.close();
    ...
    public void doRehash(BuildContext bc
                          ...
                          , WritableIntChunk<KeyIndices> sourceChunkInsertedHashSlots
                          , IntegerArraySource insertedHashSlots
        ...
        while (rehashRequired()) {
            ...
            for (int ii = 0; ii < bc.shouldMoveBucket.size(); ++ii) {
                if (bc.shouldMoveBucket.get(ii)) {
                    ...
                    // @StateValueType@ from \QObject\E
                    final Object stateValueToMove = indexSource.getUnsafe(oldHashLocation);
                    indexSource.set(newHashLocation, stateValueToMove);
                    indexSource.set(oldHashLocation, EMPTY_VALUE);
                                // region rehash move values
                                final int positionInInsertedHashSlots = statePositionInInsertedHashSlots.getInt(oldHashLocation);
                                statePositionInInsertedHashSlots.set(newHashLocation, positionInInsertedHashSlots);
                                if (isChunkPosition(positionInInsertedHashSlots)) {
                                    final int chunkPosition = insertedHashSlotPositionToChunkPosition(positionInInsertedHashSlots);
                                    sourceChunkInsertedHashSlots.set(chunkPosition, (int) newHashLocation);
                                } else {
                                    insertedHashSlots.set(positionInInsertedHashSlots, (int) newHashLocation);
                                }
                ...
                for (int ii = 0; ii < bc.overflowLocationForPromotionLoop.size(); ++ii) {
                    ...
                    if ((firstLoop && tableLocation < tableHashPivot) || (!firstLoop && tableLocation >= tableHashPivot)) {
                        ...
                        bc.destinationLocationPositionInWriteThrough.add((int)(tableLocation - firstBackingChunkLocation));
                                    ...
                                    final int positionInInsertedHashSlots = overflowStatePositionInInsertedHashSlots.getInt(overflowLocation);
                                    statePositionInInsertedHashSlots.set(tableLocation, positionInInsertedHashSlots);
                                    if (isChunkPosition(positionInInsertedHashSlots)) {
                                        final int chunkPosition = insertedHashSlotPositionToChunkPosition(positionInInsertedHashSlots);
                                        sourceChunkInsertedHashSlots.set(chunkPosition, (int) tableLocation);
                                    } else {
                                        insertedHashSlots.set(positionInInsertedHashSlots, (int) tableLocation);
                                    }
    ...
    private void verifyKeyHashes() {
        ...
        try (final WritableLongChunk<KeyIndices> positions = WritableLongChunk.makeWritableChunk(maxSize);
            ...
            // @StateChunkIdentityName@ from \QObjectChunkIdentity\E
            ObjectChunkIdentityEquals.notEqual(stateChunk, EMPTY_VALUE, exists);
    ...
    private void nullOverflowObjectSources(IntChunk<Values> locationsToNull) {
        ...
        // NB: It's worth nulling out the overflowIndexSource because if it's never re-used its sequential builder will never be swapped to an index
        for (int ii = 0; ii < locationsToNull.size(); ++ii) {
    ...
    void convertBuildersToIndexes(@NotNull final IntegerArraySource insertedHashSlots, final int insertedHashSlotsLength) {
        for (int insertedHashSlotPosition = 0; insertedHashSlotPosition < insertedHashSlotsLength; ++insertedHashSlotPosition) {
            ...
            if (isOverflowLocation(insertedHashSlot)) {
                ...
                overflowIndexSource.set(insertedOverflowSlot, ((Index.SequentialBuilder) overflowIndexSource.get(insertedOverflowSlot)).getIndex());
            } else {
                indexSource.set(insertedHashSlot, ((Index.SequentialBuilder) indexSource.get(insertedHashSlot)).getIndex());
            }
        }
    }
    ...
    ColumnSource[] getKeyHashTableSources() {
        ...
        for (int kci = 0; kci < keyColumnCount; ++kci) {
            // noinspection unchecked
            keyHashTableSources[kci] = new HashTableColumnSource(keySources[kci], overflowKeySources[kci]);
        }
    ...
    ColumnSource<Index> getIndexHashTableSource() {
        //noinspection unchecked
        return new HashTableColumnSource(Index.class, indexSource, overflowIndexSource);
    }
    ...
    static boolean isOverflowLocation(final int hashSlot) {
        return HashTableColumnSource.isOverflowLocation(hashSlot);
    }

    static int hashLocationToOverflowLocation(final int hashSlot) {
        return HashTableColumnSource.hashLocationToOverflowLocation(hashSlot);
    }

    private static int overflowLocationToHashLocation(final int overflowSlot) {
        return (int)HashTableColumnSource.overflowLocationToHashLocation(overflowSlot);
    }

    private static boolean isChunkPosition(final int insertedHashSlotPosition) {
        return insertedHashSlotPosition < 0;
    }

    private static int insertedHashSlotPositionToChunkPosition(final int insertedHashSlotPosition) {
        return -(insertedHashSlotPosition + 1);
    }

    private static int chunkPositionToInsertedHashSlotPosition(final int chunkPosition) {
        return -chunkPosition - 1;
    }
