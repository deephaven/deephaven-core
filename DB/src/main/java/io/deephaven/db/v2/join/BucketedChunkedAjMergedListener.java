...
public class BucketedChunkedAjMergedListener extends MergedListener {
    ...
    public void process() {
        ...
        final boolean leftStampModified = leftTicked && leftRecorder.getModified().nonempty() && leftRecorder.getModifiedColumnSet().containsAny(leftStampColumn);
        final boolean leftKeysModified = leftTicked && leftRecorder.getModified().nonempty() && leftRecorder.getModifiedColumnSet().containsAny(leftKeyColumns);
        final boolean leftAdditionsOrRemovals = leftKeysModified || leftStampModified || (leftTicked && (leftRecorder.getAdded().nonempty() || leftRecorder.getRemoved().nonempty()));

        final ColumnSource.FillContext leftFillContext = leftAdditionsOrRemovals ? leftStampSource.makeFillContext(leftChunkSize) : null;
        final WritableChunk<Values> leftStampValues = leftAdditionsOrRemovals ? stampChunkType.makeWritableChunk(leftChunkSize) : null;
        final WritableLongChunk<KeyIndices> leftStampKeys = leftAdditionsOrRemovals ? WritableLongChunk.makeWritableChunk(leftChunkSize) : null;
        final LongSortKernel<Values, KeyIndices> sortKernel = LongSortKernel.makeContext(stampChunkType, order, Math.max(leftChunkSize, rightChunkSize), true);
        ...
        if (leftTicked) {
            ...
            if (leftRestampRemovals.nonempty()) {
                ...
                final int removedSlotCount = asOfJoinStateManager.markForRemoval(leftRestampRemovals, leftKeySources, slots, sequentialBuilders);
                ...
                for (int slotIndex = 0; slotIndex < removedSlotCount; ++slotIndex) {
                    ...
                    try (final OrderedKeys.Iterator leftOkIt = leftRemoved.getOrderedKeysIterator()) {
            ...
            if (leftShifted.nonempty()) {
                ...
                try (final Index fullPrevIndex = leftTable.getIndex().getPrevIndex();
                    ...
                    if (relevantShift.nonempty()) {
                        try (final SizedSafeCloseable<ColumnSource.FillContext> leftShiftFillContext = new SizedSafeCloseable<>(leftStampSource::makeFillContext);
                             final SizedSafeCloseable<LongSortKernel<Values, KeyIndices>> shiftSortContext = new SizedSafeCloseable<>(size -> LongSortKernel.makeContext(stampChunkType, order, size, true));
                            ...
                            final int shiftedSlotCount = asOfJoinStateManager.gatherShiftIndex(relevantShift, leftKeySources, slots, sequentialBuilders);
                            ...
                            for (int slotIndex = 0; slotIndex < shiftedSlotCount; ++slotIndex) {
                                ...
                                final IndexShiftData shiftDataForSlot = leftShifted.intersect(shiftedIndex);

                                if ((state & ENTRY_RIGHT_MASK) == ENTRY_RIGHT_IS_EMPTY) {
                                    // if the left is empty, we should be an index entry rather than an SSA, and we can not be empty, because we are responsive
                                ...
                                while (slotSit.hasNext()) {
                                    slotSit.next();
                                    final Index indexToShift = shiftedIndex.subindexByKey(slotSit.beginRange(), slotSit.endRange());
                                    ChunkedAjUtilities.applyOneShift(leftSsa, leftChunkSize, leftStampSource, leftShiftFillContext, shiftSortContext, stampKeys, stampValues, slotSit, indexToShift);
                                    indexToShift.close();
                                }
                            }
                        }
        ...
        if (rightTicked) {
            ...
            final int removedSlotCount = asOfJoinStateManager.markForRemoval(rightRestampRemovals, rightKeySources, slots, sequentialBuilders);

            final MutableObject<Index> indexOutput = new MutableObject<>();
            try (final WritableLongChunk<KeyIndices> priorRedirections = WritableLongChunk.makeWritableChunk(rightChunkSize);
                 ...
                 final WritableLongChunk<KeyIndices> rightStampKeys = WritableLongChunk.makeWritableChunk(rightChunkSize)
            ) {
                for (int slotIndex = 0; slotIndex < removedSlotCount; ++slotIndex) {
                    final long slot = slots.getLong(slotIndex);

                    try (final Index rightRemoved = indexFromBuilder(slotIndex)) {
                        final SegmentedSortedArray rightSsa = asOfJoinStateManager.getRightSsaOrIndex(slot, indexOutput);
                        if (rightSsa == null) {
                        ...
                        try (final OrderedKeys.Iterator removeIt = rightRemoved.getOrderedKeysIterator()) {
                            while (removeIt.hasMore()) {
                                ...
                                ssaSsaStamp.processRemovals(leftSsa, rightStampValues, rightStampKeys, priorRedirections, redirectionIndex, modifiedBuilder, disallowExactMatch);
                            }
                        }
                    }
                }
            ...
            if (rightShifted.nonempty()) {
                try (final Index fullPrevIndex = rightTable.getIndex().getPrevIndex();
                    ...
                    if (relevantShift.nonempty()) {
                        try (final SizedSafeCloseable<ColumnSource.FillContext> rightShiftFillContext = new SizedSafeCloseable<>(rightStampSource::makeFillContext);
                             final SizedSafeCloseable<LongSortKernel<Values, KeyIndices>> shiftSortKernel = new SizedSafeCloseable<>(sz -> LongSortKernel.makeContext(stampChunkType, order, sz, true));
                            ...
                            final int shiftedSlotCount = asOfJoinStateManager.gatherShiftIndex(relevantShift, rightKeySources, slots, sequentialBuilders);
                            ...
                            for (int slotIndex = 0; slotIndex < shiftedSlotCount; ++slotIndex) {
                                ...
                                while (slotSit.hasNext()) {
                                    ...
                                    try (final Index indexToShift = shiftedIndex.subindexByKey(slotSit.beginRange(), slotSit.endRange())) {
                                        if (slotSit.polarityReversed()) {
                                            final int shiftSize = indexToShift.intSize();
                                            rightStampSource.fillPrevChunk(rightShiftFillContext.ensureCapacity(shiftSize), rightStampValues.ensureCapacity(shiftSize), indexToShift);
                                            ...
                                            ssaSsaStamp.applyShift(leftSsa, rightStampValues.get(), rightStampKeys.get(), slotSit.shiftDelta(), redirectionIndex, disallowExactMatch);
                                            rightSsa.applyShiftReverse(rightStampValues.get(), rightStampKeys.get(), slotSit.shiftDelta());
                                        } else {
                                            try (final OrderedKeys.Iterator shiftIt = indexToShift.getOrderedKeysIterator()) {
                                                while (shiftIt.hasMore()) {
                                                    final OrderedKeys chunkOk = shiftIt.getNextOrderedKeysWithLength(rightChunkSize);
                                                    final int shiftSize = chunkOk.intSize();
                                                    chunkOk.fillKeyIndicesChunk(rightStampKeys.ensureCapacity(shiftSize));
                                                    rightStampSource.fillPrevChunk(rightShiftFillContext.ensureCapacity(shiftSize), rightStampValues.ensureCapacity(shiftSize), chunkOk);
                                                    sortKernel.sort(rightStampKeys.get(), rightStampValues.get());

                                                    rightSsa.applyShift(rightStampValues.get(), rightStampKeys.get(), slotSit.shiftDelta());
                                                    ssaSsaStamp.applyShift(leftSsa, rightStampValues.get(), rightStampKeys.get(), slotSit.shiftDelta(), redirectionIndex, disallowExactMatch);
                                                }
                                            }
                                        }
            ...
            final int addedSlotCount = asOfJoinStateManager.buildAdditions(false, rightRestampAdditions, rightKeySources, slots, sequentialBuilders);
                ...
                for (int slotIndex = 0; slotIndex < addedSlotCount; ++slotIndex) {
                    ...
                    for (int ii = 0; ii < chunks; ++ii) {
                        ...
                        try (final Index chunkOk = rightAdded.subindexByPos(startChunk * rightChunkSize, (startChunk + 1) * rightChunkSize)) {
                            ...
                            final int valuesWithNext = rightSsa.insertAndGetNextValue(stampChunk, insertedIndices, nextRightValue);
                            ...
                            ssaSsaStamp.processInsertion(leftSsa, stampChunk, insertedIndices, nextRightValue, redirectionIndex, modifiedBuilder, endsWithLastValue, disallowExactMatch);
                        }
                    }
                }
            }
            ...
            if (!rightStampModified && !rightKeysModified && rightRecorder.getModified().nonempty()) {
                ...
                final int modifiedSlotCount = asOfJoinStateManager.gatherModifications(rightRecorder.getModified(), rightKeySources, slots, sequentialBuilders);
                    ...
                    for (int slotIndex = 0; slotIndex < modifiedSlotCount; ++slotIndex) {
                        ...
                        try (final OrderedKeys.Iterator modit = rightModified.getOrderedKeysIterator()) {
                            while (modit.hasMore()) {
                                ...
                                ssaSsaStamp.findModified(leftSsa, redirectionIndex, rightStampChunk, rightStampIndices, modifiedBuilder, disallowExactMatch);
                            }
                        }
                    }
                }
            ...
            if (rightStampModified || rightKeysModified || rightRecorder.getAdded().nonempty() || rightRecorder.getRemoved().nonempty()) {
        ...
        if (leftTicked) {
            ...
            final int addedSlotCount = asOfJoinStateManager.buildAdditions(true, leftRestampAdditions, leftKeySources, slots, sequentialBuilders);
            ...
            for (int slotIndex = 0; slotIndex < addedSlotCount; ++slotIndex) {
                ...
                switch (state) {
                    ...
                    case ENTRY_LEFT_IS_BUILDER|ENTRY_RIGHT_IS_SSA:
                    case ENTRY_LEFT_IS_INDEX|ENTRY_RIGHT_IS_SSA:
                        throw new  IllegalStateException("Bad state: " + state + ", slot=" + slot + ", slotIndex=" + slotIndex);
                ...
                try (final OrderedKeys.Iterator leftOkIt = leftAdded.getOrderedKeysIterator();
                     final WritableLongChunk<KeyIndices> rightKeysForLeft = WritableLongChunk.makeWritableChunk(leftChunkSize)
                ) {
                    assert leftFillContext != null;
                    assert leftStampValues != null;
                    ...
                    while (leftOkIt.hasMore()) {
                        ...
                        chunkSsaStamp.processEntry(leftStampValues, leftStampKeys, rightSsa, rightKeysForLeft, disallowExactMatch);
        ...
        SafeCloseable.closeArray(sortKernel, leftStampKeys, leftStampValues, leftFillContext, leftSsaFactory, rightSsaFactory);
