...
public class FirstOrLastChunkedOperator implements IterativeChunkedAggregationOperator {
    ...
    FirstOrLastChunkedOperator(boolean isFirst, MatchPair[] resultPairs, Table originalTable, String exposeRedirectionAs) {
    ...
    public void shiftChunk(BucketedContext bucketedContext, Chunk<? extends Values> previousValues, Chunk<? extends Values> newValues, LongChunk<? extends KeyIndices> preShiftIndices, LongChunk<? extends KeyIndices> postShiftIndices, IntChunk<KeyIndices> destinations, IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
        ...
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            final long newValue = doShift(preShiftIndicesAsOrdered, postShiftIndicesAsOrdered, startPosition, runLength, destination);
    ...
    private long doShift(LongChunk<OrderedKeyIndices> preShiftIndices, LongChunk<OrderedKeyIndices> postShiftIndices, int startPosition, int runLength, long destination) {
    ...
    public void modifyIndices(BucketedContext context, LongChunk<? extends KeyIndices> inputIndices, IntChunk<KeyIndices> destinations, IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            stateModified.set(ii, hasRedirection(inputIndices, redirectedRow, startPosition, startPosition + runLength));
        }
    }
    ...
    public boolean shiftChunk(SingletonContext singletonContext, Chunk<? extends Values> previousValues, Chunk<? extends Values> newValues, LongChunk<? extends KeyIndices> preInputIndices, LongChunk<? extends KeyIndices> postInputIndices, long destination) {
        //noinspection unchecked
        final long newValue = doShift((LongChunk<OrderedKeyIndices>) preInputIndices, (LongChunk<OrderedKeyIndices>) postInputIndices, 0, preInputIndices.size(), destination);
        if (exposeRedirections) {
            final long oldValue = redirections.getAndSetUnsafe(destination, newValue);
    ...
    private boolean hasRedirection(LongChunk<? extends KeyIndices> indices, long redirectedRow, int startInclusive, int endExclusive) {
        if (isFirst) {
    ...
    IterativeChunkedAggregationOperator makeSecondaryOperator(boolean isFirst, MatchPair[] comboMatchPairs, Table table, String exposeRedirectionAs) {
    ...
    private class DuplicateOperator implements IterativeChunkedAggregationOperator {
        ...
        private DuplicateOperator(MatchPair[] resultPairs, Table table, String exposeRedirectionAs) {
            for (final MatchPair mp : resultPairs) {
                //noinspection unchecked
                resultColumns.put(mp.left(), new ReadOnlyRedirectedColumnSource(redirectionIndex, table.getColumnSource(mp.right())));
            }
            if (exposeRedirectionAs != null) {
                resultColumns.put(exposeRedirectionAs, redirections);
            }
        ...
        private void checkForChangedRedirections(IntChunk<ChunkPositions> startPositions, IntChunk<KeyIndices> destinations, WritableBooleanChunk<Values> stateModified) {
            for (int ii = 0; ii < startPositions.size(); ++ii) {
                final int startPosition = startPositions.get(ii);
                final long destination = destinations.get(startPosition);
        ...
        private void checkForMatchingRedirections(IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> lengths, LongChunk<? extends KeyIndices> postKeyIndices, IntChunk<KeyIndices> destinations, WritableBooleanChunk<Values> stateModified) {
            for (int ii = 0; ii < startPositions.size(); ++ii) {
                final int startPosition = startPositions.get(ii);
                final long destination = destinations.get(startPosition);
                final long redirection = redirections.getUnsafe(destination);
                final long chunkKey = isFirst ? postKeyIndices.get(startPosition) : postKeyIndices.get(startPosition + lengths.get(ii) - 1);
        ...
        @Override
        public void shiftChunk(BucketedContext context, Chunk<? extends Values> previousValues, Chunk<? extends Values> newValues, LongChunk<? extends KeyIndices> preShiftIndices, LongChunk<? extends KeyIndices> postShiftIndices, IntChunk<KeyIndices> destinations, IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
            checkForChangedRedirections(startPositions, destinations, stateModified);
        }
        ...
        @Override
        public boolean shiftChunk(SingletonContext context, Chunk<? extends Values> previousValues, Chunk<? extends Values> newValues, LongChunk<? extends KeyIndices> preShiftIndices, LongChunk<? extends KeyIndices> postShiftIndices, long destination) {
            if (exposeRedirections) {
                return checkSingletonModification(postShiftIndices, destination);
            } else {
                return false;
        ...
        private boolean checkSingletonModification(LongChunk<? extends KeyIndices> postShiftIndices, long destination) {
            return redirections.getUnsafe(destination) == (isFirst ? postShiftIndices.get(0) : postShiftIndices.get(postShiftIndices.size() - 1));
        }
    ...
    private class ComplementaryOperator implements IterativeChunkedAggregationOperator {
        ...
        private ComplementaryOperator(boolean isFirst, MatchPair[] resultPairs, Table table, String exposeRedirectionAs) {
            this.isFirst = isFirst;
        ...
        private void updateBucketed(IntChunk<ChunkPositions> startPositions, IntChunk<KeyIndices> destinations, WritableBooleanChunk<Values> stateModified) {
            for (int ii = 0; ii < startPositions.size(); ++ii) {
                final int startPosition = startPositions.get(ii);
        ...
        private void checkForModifications(LongChunk<? extends KeyIndices> inputIndices, IntChunk<KeyIndices> destinations, IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
            for (int ii = 0; ii < startPositions.size(); ++ii) {
                final int startPosition = startPositions.get(ii);
