...
public class FirstOrLastChunkedOperator implements IterativeChunkedAggregationOperator {
    ...
    public void shiftChunk(BucketedContext bucketedContext, Chunk<? extends Values> previousValues,
        ...
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            final long newValue = doShift(preShiftIndicesAsOrdered, postShiftIndicesAsOrdered,
    ...
    private long doShift(LongChunk<OrderedKeyIndices> preShiftIndices,
        LongChunk<OrderedKeyIndices> postShiftIndices, int startPosition, int runLength,
        long destination) {
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize,
        ...
        long destination) {
        // noinspection unchecked
        return addChunk((LongChunk<OrderedKeyIndices>) inputIndices, 0, inputIndices.size(),
            destination);
    }
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize,
        ...
        long destination) {
        // noinspection unchecked
        return removeChunk((LongChunk<OrderedKeyIndices>) inputIndices, 0, inputIndices.size(),
            destination);
    }
    ...
    private boolean hasRedirection(LongChunk<? extends KeyIndices> indices, long redirectedRow,
    ...
    private boolean addChunk(LongChunk<OrderedKeyIndices> indices, int start, int length,
        long destination) {
        final Index index = indexForSlot(destination);
    ...
    private boolean removeChunk(LongChunk<OrderedKeyIndices> indices, int start, int length,
        long destination) {
        final Index index = indexForSlot(destination);
    ...
    IterativeChunkedAggregationOperator makeSecondaryOperator(boolean isFirst,
        MatchPair[] comboMatchPairs, Table table, String exposeRedirectionAs) {
        if (this.isFirst == isFirst) {
            // we only need more output columns, the redirectionIndex and redirections column are
            // totally fine
    ...
    private class DuplicateOperator implements IterativeChunkedAggregationOperator {
        ...
        private DuplicateOperator(MatchPair[] resultPairs, Table table,
            String exposeRedirectionAs) {
            for (final MatchPair mp : resultPairs) {
                // noinspection unchecked
                resultColumns.put(mp.left(), new ReadOnlyRedirectedColumnSource(redirectionIndex,
        ...
        public void modifyChunk(BucketedContext context, Chunk<? extends Values> previousValues,
            ...
            IntChunk<KeyIndices> destinations, IntChunk<ChunkPositions> startPositions,
            IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
            checkForMatchingRedirections(startPositions, length, postShiftIndices, destinations,
                stateModified);
        }
        ...
        @Override
        public void modifyIndices(BucketedContext context,
            LongChunk<? extends KeyIndices> inputIndices, IntChunk<KeyIndices> destinations,
            IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length,
            WritableBooleanChunk<Values> stateModified) {
            checkForMatchingRedirections(startPositions, length, inputIndices, destinations,
                stateModified);
        }
        ...
        private boolean checkSingletonModification(LongChunk<? extends KeyIndices> postShiftIndices,
            long destination) {
        ...
        @Override
        public boolean modifyIndices(SingletonContext context,
            LongChunk<? extends KeyIndices> indices, long destination) {
            return redirections.getUnsafe(
                destination) == (isFirst ? indices.get(0) : indices.get(indices.size() - 1));
        }

        @Override
        public boolean addIndex(SingletonContext context, Index index, long destination) {
            return redirections
                .getUnsafe(destination) == (isFirst ? index.firstKey() : index.lastKey());
        }
    ...
    private class ComplementaryOperator implements IterativeChunkedAggregationOperator {
        ...
        private void updateBucketed(IntChunk<ChunkPositions> startPositions,
            IntChunk<KeyIndices> destinations, WritableBooleanChunk<Values> stateModified) {
            for (int ii = 0; ii < startPositions.size(); ++ii) {
                final int startPosition = startPositions.get(ii);
                final int destination = destinations.get(startPosition);
                final Index trackingIndex = indices.getUnsafe(destination);
                final long trackingKey =
                    isFirst ? trackingIndex.firstKey() : trackingIndex.lastKey();
        ...
        public void modifyChunk(BucketedContext context, Chunk<? extends Values> previousValues,
            ...
            IntChunk<KeyIndices> destinations, IntChunk<ChunkPositions> startPositions,
            IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
            checkForModifications(postShiftIndices, destinations, startPositions, length,
                stateModified);
        }
        ...
        public void modifyIndices(BucketedContext context,
            ...
            IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length,
            WritableBooleanChunk<Values> stateModified) {
            checkForModifications(inputIndices, destinations, startPositions, length,
                stateModified);
        }
        ...
        private void checkForModifications(LongChunk<? extends KeyIndices> inputIndices,
            ...
            for (int ii = 0; ii < startPositions.size(); ++ii) {
                ...
                final int modifiedChunkPosition =
                    startPosition + (isFirst ? 0 : (length.get(ii) - 1));
        ...
        private boolean updateSingleton(long destination) {
            final Index trackedIndex =
                Require.neqNull(indices.getUnsafe(destination), "indices.get(destination)");
        ...
        public boolean modifyChunk(SingletonContext context, int chunkSize,
            Chunk<? extends Values> previousValues, Chunk<? extends Values> newValues,
            LongChunk<? extends KeyIndices> postShiftIndices, long destination) {
            return checkSingletonModification(postShiftIndices,
                redirections.getUnsafe(destination));
        }
