...
public abstract class HashMapBase implements TNullableLongLongMap {
    ...
    // There are three "special keys" removed from the range of valid keys that are used to
    // represent various slot
    // states:
    // 1. SPECIAL_KEY_FOR_EMPTY_SLOT is used to represent a slot that has never been used.
    // 2. SPECIAL_KEY_FOR_DELETED_SLOT is used to represent a slot that was once in use, but the key
    // that was formerly
    ...
    //
    // For the sake of efficiency, we define SPECIAL_KEY_FOR_EMPTY_SLOT to be 0. This means that our
    // arrays are ready to
    // go after being allocated, and we don't need to fill them with a special value. However, we
    // are aware that our
    // callers may wish to use 0 as a key. To support this, we remap key=0 on get, put, remove, and
    ...
    /**
     * This is the load factor we use as the hashtable nears its maximum size, in order to try to
     * keep functioning (albeit with reduced performance) rather than failing.
     */
    private static final float NEARLY_FULL_LOAD_FACTOR = 0.9f;

    /**
     * This is the fraction of the maximum possible size at which we just give up and throw an
     * exception. It is kept slightly smaller than the NEARLY_FULL_LOAD_FACTOR (otherwise, we might
     * end up in a situation where every put caused a rehash). Additionally, for some reason K2V2 is
     * much less tolerant of getting full than the other two (it gets very slow as it approaches the
     * max). For this reason, until we figure it out, we maintain individual size factors for each
     * KnVn.
     */
    ...
    /**
     * This is the size at which we just give up and throw an exception rather than do a new put. It
     * is number of entries (aka number of longs / 2) * SIZE_LIMIT_FACTORn.
     */
    ...
    // There are three kinds of slots: empty, holding a value, and deleted (formerly holding a
    // value).
    // 'size' is the number of slots holding a value.
    int size;
    // 'nonEmptySlots' is the number of slots either holding a value or deleted. It is an invariant
    // that
    // nonEmptySlots >= size.
    int nonEmptySlots;
    // The threshold (generally loadFactor * capacity) at which a rehash is triggered. This happens
    // when nonEmptySlots
    // meets or exceeds rehashThreshold. There is a decision to make about whether to rehash at the
    // same capacity or a
    // larger capacity. The heuristic we use is that if size >= (2/3) * nonEmptySlots we rehash to a
    // larger capacity.
    int rehashThreshold;
    ...
    long[] allocateKeysAndValuesArray(int entriesPerBucket) {
        ...
        final int desiredNumBuckets =
            (desiredInitialCapacity + entriesPerBucket - 1) / entriesPerBucket;
    ...
    void rehash(long[] oldKeysAndValues, boolean wantResize, int entriesPerBucket) {
        ...
        if (wantResize) {
            ...
            // If we reach the max bucket capacity, then force the rehash threshold to a high number
            // like 90%.
            final float loadFactorToUse =
                newBucketCapacity < maxBucketCapacity ? loadFactor : NEARLY_FULL_LOAD_FACTOR;
    ...
    protected abstract long putImplNoTranslate(long[] kvs, long key, long value,
        boolean insertOnly);
    ...
    final void clearImpl(long[] keysAndValues) {
        size = 0;
        nonEmptySlots = 0;
        // We leave rehashThreshold alone because the array size (and therefore the hashtable
    ...
    public final long getNoEntryKey() {
        // It doesn't make sense to call this method because the caller can't observe our
        // "noEntryKey" anyway.
        throw new UnsupportedOperationException();
    }
     ...
     *        {@link HashMapBase#size()}, otherwise an array of length {@link HashMapBase#size()}
     *        will be allocated.
    ...
    final long[] keysOrValuesImpl(final long[] kv, final long[] space, final boolean wantValues) {
        ...
        int nextIndex = 0;
        // In a single-threaded case, we would not need the 'nextIndex < sz' part of the
        // conjunction. But in the
        // unsynchronized concurrent case, we might encounter more keys than would fit in the array.
        // To avoid an index
        ...
        for (int ii = 0; ii < kv.length && nextIndex < sz; ii += 2) {
            ...
            if (wantValues) {
                ...
                resultEntry =
                    key == REDIRECTED_KEY_FOR_EMPTY_SLOT ? SPECIAL_KEY_FOR_EMPTY_SLOT : key;
            }
    ...
    /*
     * The strategy used in this class is to keep track of: - The current position (which could be
     * any valid position as well as one before the start) - The next position (which could be any
     * valid position as well as one after the end). Java iterator semantics makes this annoying.
     * Because it's Java!™ We also have to make sure we un-redirect the
     * REDIRECTED_KEY_FOR_EMPTY_SLOT back to 0.
     */
    private static class Iterator implements TLongLongIterator {
        // We keep a local reference to this array so we can avoid crashing if there's an
        // unprotected concurrent write
        // (e.g. a rehash that reallocates the owning array).
        private final long[] keysAndValues;
        private long currentKey;
        private long currentValue;
        /**
         * nextIndex points to the next occupied slot (or the first occupied slot if we have just
         * been constructed), or keysAndValues.length if there is no next occupied slot.
         */
        ...
        @Override
        public void advance() {
            // nextIndex points to some valid key and value (it cannot point past the end of the
            // array, because you're
            // not supposed to call advance() if hasNext() is false). So set current{Key,Value} from
            // the array at
    ...
    // Run this at class load time to confirm that the values returned by getMaxBucketCapacity
    // aren't too large.
    ...
    static {
        ...
        for (int entriesPerBucket : new int[] {1, 2, 4}) {
            ...
            // Assert.isPrime(mbc);
            Assert.leq(mbc * entriesPerBucket * longsPerEntry,
                "mbc * entriesPerBucket * longsPerEntry",
                Integer.MAX_VALUE, "Integer.MAX_VALUE");
        }
    }
    ...
    private static int getMaxBucketCapacity(int entriesPerBucket) {
        switch (entriesPerBucket) {
            ...
            default:
                throw new UnsupportedOperationException(
                    "Unexpected entriesPerBucket " + entriesPerBucket);
        }
    }

    // We don't currently call any of these methods, so I'm not going to bother implementing them.
    // This is not a value
