...
class BigDecimalChunkedAvgOperator implements IterativeChunkedAggregationOperator {
    ...
    private final ObjectArraySource<BigDecimal> runningSum =
        new ObjectArraySource<>(BigDecimal.class);
    ...
    public boolean addChunk(ObjectChunk<BigDecimal, ? extends Values> values, long destination,
        ...
        final long newCount =
            nonNullCount.addNonNullUnsafe(destination, chunkNonNullCount.intValue());
        ...
        resultColumn.set(destination,
            newSum.divide(BigDecimal.valueOf(newCount), BigDecimal.ROUND_HALF_UP));

        return true;
    }

    public boolean removeChunk(ObjectChunk<BigDecimal, ? extends Values> values, long destination,
        int chunkStart, int chunkSize) {
        final MutableInt chunkNonNullCount = new MutableInt();
        final BigDecimal chunkSum =
            SumBigDecimalChunk.sumBigDecimalChunk(values, chunkStart, chunkSize, chunkNonNullCount);
