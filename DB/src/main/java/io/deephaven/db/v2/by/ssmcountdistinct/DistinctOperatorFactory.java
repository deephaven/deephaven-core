...
/**
 * A factory interface to create {@link IterativeChunkedAggregationOperator operators} for the {@link io.deephaven.db.v2.by.AggType#Distinct}
 * {@link io.deephaven.db.v2.by.AggType#CountDistinct}, {@link io.deephaven.db.v2.by.AggType#Unique} and their rollup counterparts
 */
public interface DistinctOperatorFactory {
    ...
    /**
     * Create an {@link IterativeChunkedAggregationOperator operator} for the {@link io.deephaven.db.v2.by.AggType#CountDistinct}
     * aggregation.
     *
     ...
     * @param isRollup true if the returned operator should be suitable for second or higher levels of rollup aggregation
     *
    ...
    /**
     * Create an {@link IterativeChunkedAggregationOperator operator} for the {@link io.deephaven.db.v2.by.AggType#Distinct}
     * aggregation.
     *
     * @param type the type of the column
     * @param resultName the name of the result column
    ...
    /**
     * Create an {@link IterativeChunkedAggregationOperator operator} for the {@link io.deephaven.db.v2.by.AggType#Unique}
     * aggregation.
     *
     * @param type the type of the column
     * @param resultName the name of the result column
    ...
    static IterativeChunkedAggregationOperator createUnique(Class<?> type, String resultName, boolean countNulls, boolean exposeInternal, Object noKeyValue, Object nonUniqueValue, boolean isRollup) {
        ...
        if (type == Byte.class || type == byte.class) {
            ...
            return isRollup ? new CharRollupUniqueOperator(resultName, countNulls, io.deephaven.util.type.TypeUtils.unbox((Character)noKeyValue), io.deephaven.util.type.TypeUtils.unbox((Character)nonUniqueValue))
                            : new CharChunkedUniqueOperator(resultName, countNulls, exposeInternal, io.deephaven.util.type.TypeUtils.unbox((Character)noKeyValue), io.deephaven.util.type.TypeUtils.unbox((Character)nonUniqueValue));
        } else if (type == Double.class || type == double.class) {
    ...
    static void checkType(String resultColName, String valueIntent, Class<?> expected, Object value) {
        ...
        if(value != null && !expected.isAssignableFrom(value.getClass())) {
            if(io.deephaven.util.type.TypeUtils.isNumeric(expected) && TypeUtils.isNumeric(value.getClass())) {
                ...
                throw new IllegalArgumentException("For result column `" + resultColName + "' the " + valueIntent + " '" + value + "' is larger than can be represented with a " + expected.getName());
            }

            throw new IllegalArgumentException("For result column `" + resultColName + "' the " + valueIntent + " must be of type " + expected.getName() + " but is " + value.getClass().getName());
        }
    }
