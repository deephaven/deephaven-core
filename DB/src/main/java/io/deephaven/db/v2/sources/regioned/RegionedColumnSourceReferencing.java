...
/**
 * Base class for a column of {@code DATA_TYPE} which is a wrapped {@code NATIVE_DATA_TYPE}. The
 * column owns the underlying native column and its resources.
 */
abstract class RegionedColumnSourceReferencing<DATA_TYPE, ATTR extends Attributes.Values, NATIVE_DATA_TYPE, NATIVE_REGION_TYPE extends ColumnRegion<ATTR>>
    extends
    RegionedColumnSourceArray<DATA_TYPE, ATTR, ColumnRegionReferencing<ATTR, NATIVE_REGION_TYPE>>
    ...
    RegionedColumnSourceReferencing(@NotNull final NATIVE_REGION_TYPE nullRegion,
        ...
        super(new ColumnRegionReferencing.Null<>(nullRegion), type,
            DeferredColumnRegionReferencing::new);
    ...
    public <ALTERNATE_DATA_TYPE> boolean allowsReinterpret(
        ...
        return nativeSource.getType() == alternateDataType
            || nativeSource.allowsReinterpret(alternateDataType);
    }
    ...
    protected <ALTERNATE_DATA_TYPE> ColumnSource<ALTERNATE_DATA_TYPE> doReinterpret(
        ...
        return nativeSource.getType() == alternateDataType
            ? (ColumnSource<ALTERNATE_DATA_TYPE>) nativeSource
            : nativeSource.reinterpret(alternateDataType);
    }

    @Override
    @Nullable
    public ColumnRegionReferencing<ATTR, NATIVE_REGION_TYPE> makeRegion(
        @NotNull ColumnDefinition<?> columnDefinition, @NotNull ColumnLocation columnLocation,
        int regionIndex) {
        NATIVE_REGION_TYPE nativeRegionType =
            nativeSource.makeRegion(columnDefinition, columnLocation, regionIndex);
        return nativeRegionType == null ? null
            : new ColumnRegionReferencingImpl<>(nativeRegionType);
    }

    final ChunkSource.FillContext makeFillContext(ColumnRegionReferencing.Converter<ATTR> converter,
        int chunkCapacity, SharedContext sharedContext) {
        return new ColumnRegionReferencingImpl.FillContext<>(nativeSource, converter, chunkCapacity,
            sharedContext);
    }
