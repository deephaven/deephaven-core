...
public class ChunkedOperatorAggregationHelper {
        ...
        if (withView.isRefreshing()) {
            ...
            final ShiftAwareListener listener = new BaseTable.ShiftAwareListenerImpl("by(" + aggregationContextFactory + ")", withView, result) {
                ...
                public void onUpdate(@NotNull final Update upstream) {
                    ...
                    try (final KeyedUpdateContext kuc = new KeyedUpdateContext(ac, incrementalStateManager, reinterpretedKeySources, permuteKernels,
                            keysUpstreamModifiedColumnSet, operatorInputModifiedColumnSets, upstream, outputPosition)) {
    ...
    private static class KeyedUpdateContext implements SafeCloseable {
        ...
        private final ShiftAwareListener.Update upstream; // Not to be mutated
        ...
        private KeyedUpdateContext(@NotNull final AggregationContext ac,
                                   ...
                                   @NotNull final ShiftAwareListener.Update upstream,
        ...
        private void doRemoves(@NotNull final OrderedKeys keyIndicesToRemove) {
            ...
            try (final OrderedKeys.Iterator keyIndicesToRemoveIterator = keyIndicesToRemove.getOrderedKeysIterator()) {
                while (keyIndicesToRemoveIterator.hasMore()) {
                    doRemovesForChunk(keyIndicesToRemoveIterator.getNextOrderedKeysWithLength(CHUNK_SIZE));
                }
            }
        }
        ...
        private void propagateRemovesToOperators(@NotNull final OrderedKeys keyIndicesToRemoveChunk, @NotNull final WritableIntChunk<KeyIndices> slotsToRemoveFrom) {
            ...
            if (ac.requiresIndices()) {
                final LongChunk<OrderedKeyIndices> keyIndices = keyIndicesToRemoveChunk.asKeyIndicesChunk();
        ...
        private void doInserts(@NotNull final OrderedKeys keyIndicesToInsert, final boolean addToStateManager) {
        ...
        private void propagateInsertsToOperators(@NotNull final OrderedKeys keyIndicesToInsertChunk, @NotNull final WritableIntChunk<KeyIndices> slotsToAddTo) {
            ...
            if (ac.requiresIndices()) {
                final LongChunk<OrderedKeyIndices> keyIndices = keyIndicesToInsertChunk.asKeyIndicesChunk();
    ...
    private static void extractDownstreamModifiedColumnSet(@NotNull final ShiftAwareListener.Update downstream,
                                                           @NotNull final ModifiedColumnSet resultModifiedColumnSet,
    ...
    private static QueryTable staticGroupedAggregation(QueryTable withView, String keyName, ColumnSource<?> keySource, AggregationContext ac) {
        ...
        final QueryTable result = new QueryTable(Index.FACTORY.getFlatIndex(responsiveGroups), resultColumnSourceMap);
        ac.propagateInitialStateToOperators(result);
    ...
    private static QueryTable noKeyAggregation(ShiftAwareSwapListener swapListener, AggregationContextFactory aggregationContextFactory, QueryTable table, boolean usePrev) {
        ...
        final QueryTable result = new QueryTable(Index.FACTORY.getFlatIndex(initialResultSize), resultColumnSourceMap);
        ac.propagateInitialStateToOperators(result);
    ...
    private static void doNoKeyShifts(QueryTable source, ShiftAwareListener.Update upstream, AggregationContext ac,
