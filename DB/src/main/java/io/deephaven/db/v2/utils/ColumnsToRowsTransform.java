 ...
 *
 * <p>There are times when you have a wide table, that is better displayed to the user as a narrow table with additional
 * rows.  For example, you might have a table with columns for "Bid", "Ask" and "Last", which you may prefer to have
 * three rows, one each for Bid, Ask, and Last with a label for each symbol.</p>
 *
 * <p>The same can be accomplished by calling
 * <code>.update("Label=new String[]{`Bid`, `Ask`, `Last`}", "Value=new double[]{Bid, Ask, Last}").ungroup()</code>,
 * but the creation of arrays in the update statement introduces additional overhead and garbage creation to the query
 * execution.</p>
 *
 * <p>You may have only a single label column, but you may define multiple output value columns, all of which must have
 * the same number of source columns.</p>
 *
 * <p>For each output value column, all of the constituent input columns columns must have the same type.  If the types
 * are different, then an IllegalArgumentException is thrown.</p>
 *
 * <p>
 ...
 *     <pre>       Sym|      Val1|                  D1|                  D2|      Val2|      Val3|                  D3
 * ----------+----------+--------------------+--------------------+----------+----------+--------------------
...
public class ColumnsToRowsTransform {
     ...
     */
    public static Table columnsToRows(final Table source, final String labelColumn, final String valueColumn, final String ... transposeColumns) {
     ...
     * @return the transformed table
     */
    public static Table columnsToRows(final Table source, final String labelColumn, final String valueColumn, final String [] labels, final String [] transposeColumns) {
        return columnsToRows(source, labelColumn, new String[]{valueColumn}, labels, new String[][]{transposeColumns});
    }
     ...
     *                         that are constituents for the output column.  The input columns within each element must be
     *                         the same type, and the cardinality much match labels.
     * @return the transformed table
     */
    public static Table columnsToRows(final Table source, final String labelColumn, final String [] valueColumns, final String [] labels, final String [][] transposeColumns) {
        if (valueColumns.length == 0) {
            throw new IllegalArgumentException("No columns to transpose defined!");
        }
        if (valueColumns.length != transposeColumns.length) {
            throw new IllegalArgumentException("Inconsistent transpose column definition, " + valueColumns.length + " names defined, " + transposeColumns.length + " columns defined.");
        }
        for (int cc = 0; cc < transposeColumns.length; ++cc) {
            if (labels.length != transposeColumns[cc].length) {
                throw new IllegalArgumentException(labels.length + " labels defined, but " + transposeColumns[cc].length + " transpose columns defined for " + valueColumns[cc] + ".");
            }
        }
                    ...
                    if (transposeSet.get(cc).contains(name)) {
                        if (valueTypes[cc] == null) {
                            ...
                            if (valueTypes[cc] != cs.getType()) {
                                throw new IllegalArgumentException("Incompatible transpose types " + typeSourceName[cc] + " is " + valueTypes[cc] + ", " + name + " is " + cs.getType());
                            }
                        }
                        return;
                    }
        ...
        if (source.isLive()) {
            ...
            final ModifiedColumnSet modifyAll = ((DynamicTable) source).newModifiedColumnSet(expandSet.toArray(CollectionUtil.ZERO_LENGTH_STRING_ARRAY));
            ...
            for (int cc = 0; cc < labels.length; ++cc) {
                modifyOneRow[cc] =  ((DynamicTable) source).newModifiedColumnSet(sourcesForRow[cc].toArray(CollectionUtil.ZERO_LENGTH_STRING_ARRAY));
            }

            final ModifiedColumnSet.Transformer transformer = dynamicSource.newModifiedColumnSetTransformer(sourceColumns, resultColumnSets);
            dynamicSource.listenForUpdates(new BaseTable.ShiftAwareListenerImpl("columnsToRows(" + labelColumn + ", " + Arrays.toString(valueColumns) + ", " + Arrays.deepToString(transposeColumns) + ")", dynamicSource, result) {
                @Override
                    ...
                    if (upstream.modified.nonempty()) {
                        ...
                        if (expandModified) {
                            ...
                            if (allTrue) {
                                ...
                                downstream.modified = transformIndex(upstream.modified, fanoutPow2, rowModified, maxModified);
                            }
                        }
                    } else {
    ...
    private static Index transformIndex(final Index index, final int fanoutPow2, final boolean [] rowModified, final int maxModified) {
        final Index.SequentialBuilder sequentialBuilder = Index.FACTORY.getSequentialBuilder();
    ...
    private static class TransposedColumnSource<T> extends AbstractColumnSource<T> {
        ...
        private class TransposeFillContext implements FillContext {
            ...
            private TransposeFillContext(final int chunkCapacity) {
                ...
                innerContexts = Arrays.stream(transposeColumns).map(tc -> tc.makeFillContext(chunkCapacity)).toArray(FillContext[]::new);
                //noinspection unchecked
        ...
        private void updateContext(@NotNull final TransposeFillContext context, @NotNull final OrderedKeys orderedKeys) {
            for (int ii = 0; ii < transposeColumns.length; ++ii) {
        ...
        private void doFillAndPermute(@NotNull final WritableChunk<? super Attributes.Values> destination, final TransposeFillContext transposeFillContext, final boolean usePrev, final long originalSize) {
            for (int ii = 0; ii < transposeColumns.length; ++ii) {
