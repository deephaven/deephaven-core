...
public class HashTableColumnSource<DATA_TYPE> extends AbstractColumnSource<DATA_TYPE> implements ColumnSource<DATA_TYPE> {
    ...
    public final void fillChunk(@NotNull final FillContext context, @NotNull final WritableChunk<? super Values> destination, @NotNull final OrderedKeys orderedKeys) {
        ...
        if (isOverflowLocation(orderedKeys.firstKey())) {
            ...
            overflowSource.fillChunk(typedContext.overflowFillContext, destination, typedContext.overflowShiftedOrderedKeys);
    ...
    private void mergedFillChunk(@NotNull final HashTableFillContext typedContext, @NotNull final WritableChunk<? super Values> destination, @NotNull final OrderedKeys orderedKeys) {
        final int totalSize = orderedKeys.intSize();
        final int firstOverflowChunkPosition;
        try (final OrderedKeys mainOrderedKeysSlice = orderedKeys.getOrderedKeysByKeyRange(0, MINIMUM_OVERFLOW_HASH_SLOT - 1)) {
        ...
        // Set destination size ahead of time, so that resetting our overflow destination slice doesn't run into bounds issues.
        destination.setSize(totalSize);

        try (final OrderedKeys overflowOrderedKeysSlice = orderedKeys.getOrderedKeysByPosition(firstOverflowChunkPosition, sizeFromOverflow)) {
            ...
            overflowSource.fillChunk(typedContext.overflowFillContext,
                    typedContext.overflowDestinationSlice.resetFromChunk(destination, firstOverflowChunkPosition, sizeFromOverflow),
    ...
    public final void fillPrevChunk(@NotNull final FillContext context, @NotNull final WritableChunk<? super Values> destination, @NotNull final OrderedKeys orderedKeys) {
        ...
        if (isOverflowLocation(orderedKeys.firstKey())) {
            // Main locations are always before overflow locations, so there are no responsive main locations
            typedContext.overflowShiftedOrderedKeys.reset(orderedKeys, -MINIMUM_OVERFLOW_HASH_SLOT);
            overflowSource.fillPrevChunk(typedContext.overflowFillContext, destination, typedContext.overflowShiftedOrderedKeys);
            typedContext.overflowShiftedOrderedKeys.clear();
            return;
        }
        // We're going to have to mix main and overflow locations in a single destination chunk, so delegate to fill
    ...
    private void mergedFillPrevChunk(@NotNull final HashTableFillContext typedContext, @NotNull final WritableChunk<? super Values> destination, @NotNull final OrderedKeys orderedKeys) {
        final int totalSize = orderedKeys.intSize();
        final int firstOverflowChunkPosition;
        try (final OrderedKeys mainOrderedKeysSlice = orderedKeys.getOrderedKeysByKeyRange(0, MINIMUM_OVERFLOW_HASH_SLOT - 1)) {
            firstOverflowChunkPosition = mainOrderedKeysSlice.intSize();
    ...
    public final Chunk<? extends Values> getPrevChunk(@NotNull final GetContext context, @NotNull final OrderedKeys orderedKeys) {
        ...
        if (isOverflowLocation(orderedKeys.firstKey())) {
            // Main locations are always before overflow locations, so there are no responsive main locations
            typedContext.overflowShiftedOrderedKeys.reset(orderedKeys, -MINIMUM_OVERFLOW_HASH_SLOT);
            return overflowSource.getPrevChunk(typedContext.overflowGetContext, typedContext.overflowShiftedOrderedKeys);
        }
        // We're going to have to mix main and overflow locations in a single destination chunk, so delegate to fill
    ...
    public final DATA_TYPE get(final long index) {
        return isOverflowLocation(index) ? overflowSource.get(hashLocationToOverflowLocation(index)) : mainSource.get(index);
    }

    @Override
    public final Boolean getBoolean(final long index) {
        return isOverflowLocation(index) ? overflowSource.getBoolean(hashLocationToOverflowLocation(index)) : mainSource.getBoolean(index);
    }

    @Override
    public final byte getByte(final long index) {
        return isOverflowLocation(index) ? overflowSource.getByte(hashLocationToOverflowLocation(index)) : mainSource.getByte(index);
    }

    @Override
    public final char getChar(final long index) {
        return isOverflowLocation(index) ? overflowSource.getChar(hashLocationToOverflowLocation(index)) : mainSource.getChar(index);
    }

    @Override
    public final double getDouble(final long index) {
        return isOverflowLocation(index) ? overflowSource.getDouble(hashLocationToOverflowLocation(index)) : mainSource.getDouble(index);
    }

    @Override
    public final float getFloat(final long index) {
        return isOverflowLocation(index) ? overflowSource.getFloat(hashLocationToOverflowLocation(index)) : mainSource.getFloat(index);
    }

    @Override
    public final int getInt(final long index) {
        return isOverflowLocation(index) ? overflowSource.getInt(hashLocationToOverflowLocation(index)) : mainSource.getInt(index);
    }

    @Override
    public final long getLong(final long index) {
        return isOverflowLocation(index) ? overflowSource.getLong(hashLocationToOverflowLocation(index)) : mainSource.getLong(index);
    }

    @Override
    public final short getShort(final long index) {
        return isOverflowLocation(index) ? overflowSource.getShort(hashLocationToOverflowLocation(index)) : mainSource.getShort(index);
    }

    @Override
    public final DATA_TYPE getPrev(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrev(hashLocationToOverflowLocation(index)) : mainSource.getPrev(index);
    }

    @Override
    public final Boolean getPrevBoolean(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevBoolean(hashLocationToOverflowLocation(index)) : mainSource.getPrevBoolean(index);
    }

    @Override
    public final byte getPrevByte(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevByte(hashLocationToOverflowLocation(index)) : mainSource.getPrevByte(index);
    }

    @Override
    public final char getPrevChar(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevChar(hashLocationToOverflowLocation(index)) : mainSource.getPrevChar(index);
    }

    @Override
    public final double getPrevDouble(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevDouble(hashLocationToOverflowLocation(index)) : mainSource.getPrevDouble(index);
    }

    @Override
    public final float getPrevFloat(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevFloat(hashLocationToOverflowLocation(index)) : mainSource.getPrevFloat(index);
    }

    @Override
    public final int getPrevInt(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevInt(hashLocationToOverflowLocation(index)) : mainSource.getPrevInt(index);
    }

    @Override
    public final long getPrevLong(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevLong(hashLocationToOverflowLocation(index)) : mainSource.getPrevLong(index);
    }

    @Override
    public final short getPrevShort(final long index) {
        return isOverflowLocation(index) ? overflowSource.getPrevShort(hashLocationToOverflowLocation(index)) : mainSource.getPrevShort(index);
    }
