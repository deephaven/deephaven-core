...
/**
 * {@link ColumnSource} implementation that delegates to the main and overflow sources for a hash
 * table column.
 */
public class HashTableColumnSource<DATA_TYPE> extends AbstractColumnSource<DATA_TYPE>
    ...
    private static class HashTableFillContext implements FillContext {
        ...
        private HashTableFillContext(@NotNull final ColumnSource<?> mainSource,
            ...
            // TODO: Implement a proper shareable context to use when combining fills from the main
            // and overflow
            // sources. Current usage is "safe" because sources are only exposed through this
            // wrapper, and all
    ...
    private static final class HashTableGetContext extends HashTableFillContext
    ...
    public final void fillChunk(@NotNull final FillContext context,
        ...
        if (!isOverflowLocation(orderedKeys.lastKey())) {
            // Overflow locations are always after main locations, so there are no responsive
            // overflow locations
            mainSource.fillChunk(typedContext.mainFillContext, destination, orderedKeys);
            return;
        }
        if (isOverflowLocation(orderedKeys.firstKey())) {
            // Main locations are always before overflow locations, so there are no responsive main
            // locations
        ...
        // We're going to have to mix main and overflow locations in a single destination chunk, so
        // delegate to fill
    ...
    private void mergedFillChunk(@NotNull final HashTableFillContext typedContext,
        ...
        // Set destination size ahead of time, so that resetting our overflow destination slice
        // doesn't run into bounds issues.
        ...
        try (final OrderedKeys overflowOrderedKeysSlice =
            ...
            typedContext.overflowShiftedOrderedKeys.reset(overflowOrderedKeysSlice,
                -MINIMUM_OVERFLOW_HASH_SLOT);
            overflowSource.fillChunk(typedContext.overflowFillContext,
                typedContext.overflowDestinationSlice.resetFromChunk(destination,
                    firstOverflowChunkPosition, sizeFromOverflow),
                typedContext.overflowShiftedOrderedKeys);
        }
    ...
    private void mergedFillPrevChunk(@NotNull final HashTableFillContext typedContext,
        ...
        try (final OrderedKeys mainOrderedKeysSlice =
            orderedKeys.getOrderedKeysByKeyRange(0, MINIMUM_OVERFLOW_HASH_SLOT - 1)) {
            firstOverflowChunkPosition = mainOrderedKeysSlice.intSize();
            mainSource.fillPrevChunk(typedContext.mainFillContext, destination,
                mainOrderedKeysSlice);
        }
        final int sizeFromOverflow = totalSize - firstOverflowChunkPosition;
    ...
    public final Chunk<? extends Values> getChunk(@NotNull final GetContext context,
        ...
        if (isOverflowLocation(orderedKeys.firstKey())) {
            ...
            // locations
            typedContext.overflowShiftedOrderedKeys.reset(orderedKeys, -MINIMUM_OVERFLOW_HASH_SLOT);
            return overflowSource.getChunk(typedContext.overflowGetContext,
                typedContext.overflowShiftedOrderedKeys);
        }
        // We're going to have to mix main and overflow locations in a single destination chunk, so
        // delegate to fill
    ...
    public final Boolean getBoolean(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getBoolean(hashLocationToOverflowLocation(index))
    ...
    public final byte getByte(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getByte(hashLocationToOverflowLocation(index))
    ...
    public final char getChar(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getChar(hashLocationToOverflowLocation(index))
    ...
    public final double getDouble(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getDouble(hashLocationToOverflowLocation(index))
    ...
    public final float getFloat(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getFloat(hashLocationToOverflowLocation(index))
    ...
    public final int getInt(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getInt(hashLocationToOverflowLocation(index))
    ...
    public final long getLong(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getLong(hashLocationToOverflowLocation(index))
    ...
    public final short getShort(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getShort(hashLocationToOverflowLocation(index))
    ...
    public final DATA_TYPE getPrev(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrev(hashLocationToOverflowLocation(index))
    ...
    public final Boolean getPrevBoolean(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevBoolean(hashLocationToOverflowLocation(index))
    ...
    public final byte getPrevByte(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevByte(hashLocationToOverflowLocation(index))
    ...
    public final char getPrevChar(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevChar(hashLocationToOverflowLocation(index))
    ...
    public final double getPrevDouble(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevDouble(hashLocationToOverflowLocation(index))
    ...
    public final float getPrevFloat(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevFloat(hashLocationToOverflowLocation(index))
    ...
    public final int getPrevInt(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevInt(hashLocationToOverflowLocation(index))
    ...
    public final long getPrevLong(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevLong(hashLocationToOverflowLocation(index))
    ...
    public final short getPrevShort(final long index) {
        return isOverflowLocation(index)
            ? overflowSource.getPrevShort(hashLocationToOverflowLocation(index))
    ...
    public <ALTERNATE_DATA_TYPE> boolean allowsReinterpret(
        ...
        return mainSource.allowsReinterpret(alternateDataType)
            && overflowSource.allowsReinterpret(alternateDataType);
    }
    ...
    private static final class Reinterpreted<DATA_TYPE> extends HashTableColumnSource<DATA_TYPE> {
        ...
        private Reinterpreted(@NotNull final Class<DATA_TYPE> dataType,
            ...
            super(dataType, original.mainSource.reinterpret(dataType),
                original.overflowSource.reinterpret(dataType));
