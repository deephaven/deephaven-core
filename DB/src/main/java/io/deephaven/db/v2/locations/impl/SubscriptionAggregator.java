...
public abstract class SubscriptionAggregator<LISTENER_TYPE extends BasicTableDataListener> {
    ...
    /**
     * Check if this subscription aggregator still has any valid subscribers - useful if there may have been
    ...
    /**
     * <p>Refresh and activate update pushing from the implementing class.
     * <p>If the implementation will deliver notifications in a different thread than the one that calls this method, then
     * this method must be asynchronous - that is, it must not block pending delivery of results.
     * <em>This requirement holds even if that other thread has nothing to do with the initial activation request!</em>
     *
     * <p>Listeners should guard against duplicate notifications, especially if the implementation delivers synchronous
     * notifications.
     * <p>The implementation should call activationSuccessful() when done activating and delivering initial refresh
     * results, unless activationFailed() was called instead.
     * <p>Must be called under the subscription lock.
     */
    ...
    public final <T> void activationSuccessful(@Nullable final T token) {
        if (!supportsSubscriptions()) {
            throw new IllegalStateException(this + ": completed activations are unexpected when subscriptions aren't supported");
        }
        synchronized (subscriptions) {
    ...
    public final <T> void activationFailed(@Nullable final T token, @NotNull final TableDataException exception) {
        if (!supportsSubscriptions()) {
            throw new IllegalStateException(this + ": asynchronous exceptions are unexpected when subscriptions aren't supported", exception);
        }
        synchronized (subscriptions) {
            if (!matchSubscriptionToken(token)) {
                return;
            ...
            if (activationState == ActivationState.PENDING) {
                onActivationDone(ActivationState.FAILED); // NB: This can be done before or after the notification delivery, since we're holding the lock.
            }
    ...
    /**
     * Deactivate pushed updates from the implementing class.
    ...
    /**
     * Verify that a notification pertains to a currently-active subscription.
     * Must be called under the subscription lock.
     *
     * @param token A subscription-related object that the subclass can use to match a notification
