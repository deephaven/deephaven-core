...
public class ComparableRangeFilter extends AbstractRangeFilter {
    ...
    ComparableRangeFilter(String columnName, Comparable val1, Comparable val2, boolean lowerInclusive, boolean upperInclusive) {
    ...
    @TestUseOnly
    public static ComparableRangeFilter makeForTest(String columnName, Comparable lower, Comparable upper, boolean lowerInclusive, boolean upperInclusive) {
    ...
    public void init(TableDefinition tableDefinition) {
        ...
        Assert.assertion(Comparable.class.isAssignableFrom(def.getDataType()), "Comparable.class.isAssignableFrom(def.getDataType())", def.getDataType(), "def.getDataType()");
    ...
    Index binarySearch(Index selection, ColumnSource columnSource, boolean usePrev, boolean reverse) {
        ...
        long lowerBoundMin = bound(selection, usePrev, comparableColumnSource, 0, selection.size(), startValue, startInclusive, compareSign, false);
        long upperBoundMin = bound(selection, usePrev, comparableColumnSource, lowerBoundMin, selection.size(), endValue, endInclusive, compareSign, true);

        return selection.subindexByPos(lowerBoundMin, upperBoundMin);
    }


    static long bound(Index selection, boolean usePrev, ColumnSource<Comparable> comparableColumnSource, long minPosition, long maxPosition, Comparable targetValue, boolean inclusive, int compareSign, boolean end) {
        while (minPosition < maxPosition) {
            final long midPos = (minPosition + maxPosition) / 2;
            final long midIdx = selection.get(midPos);

            final Comparable<?> compareValue = usePrev ? comparableColumnSource.getPrev(midIdx) : comparableColumnSource.get(midIdx);
