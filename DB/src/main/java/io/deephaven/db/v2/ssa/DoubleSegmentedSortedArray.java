 ...
 * index key.
 */
public final class DoubleSegmentedSortedArray implements SegmentedSortedArray {
     ...
     */
    public DoubleSegmentedSortedArray(int leafSize) {
            ...
            try (final ResettableDoubleChunk<Any> leafValuesInsertChunk = ResettableDoubleChunk.makeResettableChunk();
                ...
                while (firstValuesPosition < insertSize) {
                    ...
                    if (firstLeaf == leafCount - 1) {
                        ...
                        final double leafMaxValue = leafValues[firstLeaf + 1][0];
                    ...
                    if (sizeForThisLeaf <= leafSize) {
                        ...
                        if (isLastLeaf && isAfterLeaf(leafSizes[firstLeaf], leafValues[firstLeaf], leafValuesInsertChunk, leafIndices[firstLeaf], leafKeysInsertChunk)) {
                            ...
                            for (int leaf = firstLeaf; leaf < firstLeaf + newLeafCount; ++leaf) {
                                if (leaf > firstLeaf) {
                                    leafValues[leaf] = new double[leafSize];
    ...
    private void mergeLeaves(int leaf, TIntList leavesToRemove) {
        ...
        final double [] tmpValues = leafValues[leaf + 1];
    ...
    private void moveLeafValues(double[] leafValues, long[] leafIndices, int srcPos, int destPos, int length) {
        System.arraycopy(leafValues, srcPos, leafValues, destPos, length);
        System.arraycopy(leafIndices, srcPos, leafIndices, destPos, length);
    }
    ...
    private void promoteDirectory(int newLeafCount) {
        ...
        leafValues = new double[newLeafCount][];
        ...
        directoryValues = new double[newLeafCount];
    ...
    private void makeLeafHole(int holePosition, int holeSize) {
        ...
        // region fillValue
        Arrays.fill(directoryValues, holePosition, holePosition + holeSize, Double.MIN_VALUE);
        // endregion fillValue
    ...
    private void makeLeavesInitial(DoubleChunk<? extends Any> values, LongChunk<? extends KeyIndices> indices) {
        ...
        leafValues = new double[leafCount][];
        leafIndices = new long[leafCount][];
        directoryValues = new double[leafCount];
        ...
        for (int ii = 0; ii < leafCount; ++ii) {
            ...
            leafValues[ii] = new double[leafSize];
    ...
    private void insertIntoLeaf(int leafSize, double [] leafValues, DoubleChunk<? extends Any> insertValues, long [] leafIndices, LongChunk<? extends KeyIndices> insertIndices) {
        ...
        while (wpos >= 0) {
            ...
            final double vall = leafValues[rposl];
            final double vali = insertValues.get(rposi);
            ...
            if (iwins > minGallop && rposl >= 0) {
                ...
                final double firstInsert = insertValues.get(0);
                ...
                final double firstLeaf = leafValues[0];
                final int gallopLength;
    ...
    private void removeFromLeaf(int leafSize, double [] leafValues, DoubleChunk<? extends Any> removeValues, long [] leafIndices, LongChunk<? extends KeyIndices> removeIndices, @Nullable WritableLongChunk<? extends KeyIndices> priorRedirections, long firstPriorRedirection) {
        ...
        for (int rposl = 0; rposl < leafSize; ) {
            ...
            if (++lwin >= minGallop) {
                ...
                final double searchValue = removeValues.get(rposi);
    ...
    private static int lowerBound(double [] valuesToSearch, long [] indicesToSearch, int lo, int hi, double searchValue, long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
    ...
    private void shiftLeaf(int leafSize, double [] leafValues, DoubleChunk<? extends Any> shiftValues, long [] leafIndices, LongChunk<? extends KeyIndices> shiftIndices, long shiftDelta) {
        ...
        for (int leafPos = 0; leafPos < leafSize; ) {
            ...
            if (++lwin >= minGallop) {
                ...
                final double searchValue = shiftValues.get(shiftPos);
            ...
            try (final ResettableDoubleChunk<Any> leafValuesChunk = ResettableDoubleChunk.makeResettableChunk();
                ...
                while (lastValuesPosition >= 0) {
                    ...
                    if (firstLeaf == 0) {
                        ...
                        final double leafMinValue = leafValues[firstLeaf][0];
                        ...
                        double foundValue = stampChunk.get(firstValueForLeaf);
    ...
    private static void validateLeaf(double[] values, long[] indices, int size) {
        if (!SEGMENTED_SORTED_ARRAY_VALIDATION) {
            return;
        }
    ...
    final class Iterator {
        ...
        private void advanceToInternal(double value, boolean advanceToLast) {
            if (leafCount == 0) {
                return;
            }
         ...
         */
        private void findInLeaf(double value) {
            if (disallowExactMatch) {
                if (leafCount == 1) {
        ...
        private void findLastInLeaf(double value) {
            if (leafCount == 1) {
         ...
         * We know that if the value is to be found, it is to be found in this leaf, advance indexWithinLeaf until we find it.
         */
        private void findFirstInLeaf(double value) {
