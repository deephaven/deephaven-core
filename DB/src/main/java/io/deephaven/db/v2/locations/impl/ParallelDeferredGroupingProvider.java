...
/**
 * Grouping provider that loads column location metadata and assembles grouping indexes lazily on
 * demand.
 */
public class ParallelDeferredGroupingProvider<DATA_TYPE>
    implements KeyRangeGroupingProvider<DATA_TYPE> {

    private static final boolean SORT_RANGES = false;

    private final ColumnDefinition<DATA_TYPE> columnDefinition;

    public ParallelDeferredGroupingProvider(
    ...
    private static class Source<DATA_TYPE, RANGE_TYPE> {
        ...
        /**
         * We a reference to the resultant grouping information. This allows us to avoid re-reading
        ...
        private Source(@NotNull final ColumnLocation columnLocation, final long firstKey,
            final long lastKey) {
         ...
         * <p>
         * If the grouping information was already read, get it from the cached results map. Any
         * grouping information read by this method will be stored in the cached results map.
         *
         * @param columnDefinition The definition of this column
         * @return Grouping metadata as a map from value to position range within this source, or
         *         null if the grouping information was not present
         */
        private Map<DATA_TYPE, RANGE_TYPE> loadMetadata(
            @NotNull final ColumnDefinition<DATA_TYPE> columnDefinition) {
            ...
            if (valuesToLocationIndexRange == null) {
                // noinspection unchecked
                if ((valuesToLocationIndexRange = (Map<DATA_TYPE, RANGE_TYPE>) columnLocation
                    .getMetadata(columnDefinition)) != null) {
         ...
         *
         * @param valuesToLocationIndexRange The metadata as a map from value to position range
         *        within this source
        ...
        private List<GroupingItem<DATA_TYPE>> validateAndTransformMetadata(
            ...
            if (valuesToLocationIndexRange.isEmpty()) {
                // NB: It's impossible for the location to be legitimately empty, since the
                // constructor validates that firstKey <= lastKey.
                throw new IllegalStateException("Invalid empty grouping for " + columnLocation
                    + ": expected " + locationSize + " rows");
            }

            final Object indicativeValue = valuesToLocationIndexRange.values().stream().findAny()
            ...
            if (indicativeValue.getClass() == int[].class) {
                ...
                throw new UnsupportedOperationException("Unexpected range type "
                    + indicativeValue.getClass() + " in grouping metadata for " + columnLocation);
            }

            final List<GroupingItem<DATA_TYPE>> result =
                new ArrayList<>(valuesToLocationIndexRange.size());
            long totalRangeSize = 0;
            if (SORT_RANGES) {
                final Map<RANGE_TYPE, DATA_TYPE> reversedMap = new TreeMap<>(rangeAccessor);
                for (final Map.Entry<DATA_TYPE, RANGE_TYPE> entry : valuesToLocationIndexRange
                    .entrySet()) {
                    reversedMap.put(entry.getValue(), entry.getKey());
                }
                for (final Map.Entry<RANGE_TYPE, DATA_TYPE> entry : reversedMap.entrySet()) {
                    final long firstPositionInclusive =
                        rangeAccessor.getRangeStartInclusive(entry.getKey());
                    final long lastPositionInclusive =
                        rangeAccessor.getRangeEndInclusive(entry.getKey());
                    result.add(new GroupingItem<>(entry.getValue(),
                        firstPositionInclusive + firstKey, lastPositionInclusive + firstKey));
                ...
                for (final Map.Entry<DATA_TYPE, RANGE_TYPE> entry : valuesToLocationIndexRange
                    .entrySet()) {
                    final long firstPositionInclusive =
                        rangeAccessor.getRangeStartInclusive(entry.getValue());
                    final long lastPositionInclusive =
                        rangeAccessor.getRangeEndInclusive(entry.getValue());
            ...
            if (locationSize != totalRangeSize) {
                throw new IllegalStateException("Invalid grouping for " + columnLocation
                    + ": found " + totalRangeSize + " rows, but expected " + locationSize);
            }
            return result;
        }

        /**
         * Get a list of grouping items that represent the grouping information from this source, or
         * null if grouping information was not present.
         * <p>
         * If the grouping information was already read, get it from the cached results map. Any
         * grouping information read by this method will be stored in the cached results map.
         *
    ...
    private static class GroupingItem<DATA_TYPE> {
        ...
        private void updateBuilder(
            @NotNull final Map<DATA_TYPE, Index.SequentialBuilder> valueToBuilder) {
            valueToBuilder.computeIfAbsent(value, v -> Index.FACTORY.getSequentialBuilder())
    ...
    public void addSource(@NotNull final ColumnLocation columnLocation,
        ...
        if (lastKey - firstKey + 1 != locationIndexInTable.size()) {
            /*
             * TODO (https://github.com/deephaven/deephaven-core/issues/816): This constraint is
             * valid for all existing formats that support grouping. Address when we integrate
             * grouping/index tables.
             */
            throw new IllegalArgumentException(ParallelDeferredGroupingProvider.class
                + " only supports a single range per location");
        }
        sources.add(new Source<>(columnLocation, firstKey, lastKey));
    }

    private Map<DATA_TYPE, Index> buildGrouping(
        @NotNull final List<Source<DATA_TYPE, ?>> includedSources) {
    ...
    public Pair<Map<DATA_TYPE, Index>, Boolean> getGroupToRange(@NotNull final Index hint) {
        final List<Source<DATA_TYPE, ?>> includedSources = sources.stream()
            .filter(
                source -> CurrentOnlyIndex.FACTORY.getIndexByRange(source.firstKey, source.lastKey)
                    .overlaps(hint))
