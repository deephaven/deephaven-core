...
public abstract class CopyingPermutedStreamFirstOrLastChunkedOperator extends BaseStreamFirstOrLastChunkedOperator {
    ...
    protected void copyStreamToResult(@NotNull final OrderedKeys destinations) {
        try (final SafeCloseableList toClose = new SafeCloseableList()) {
            ...
            final WritableLongChunk<Attributes.KeyIndices> sourceIndices = toClose.add(WritableLongChunk.makeWritableChunk(COPY_CHUNK_SIZE));
            final WritableIntChunk<Attributes.ChunkPositions> sourceIndicesOrder = toClose.add(WritableIntChunk.makeWritableChunk(COPY_CHUNK_SIZE));
            final LongIntTimsortKernel.LongIntSortKernelContext<Attributes.KeyIndices, Attributes.ChunkPositions> sortKernelContext = toClose.add(LongIntTimsortKernel.createContext(COPY_CHUNK_SIZE));
            ...
            final WritableChunk<Attributes.Values>[] outputChunks = toClose.addArray(new WritableChunk[numResultColumns]);
            ...
            while (destinationsIterator.hasMore()) {
                ...
                try (final OrderedKeys sliceSources = OrderedKeys.wrapKeyIndicesChunkAsOrderedKeys(WritableLongChunk.downcast(sourceIndices))) {
                    for (int ci = 0; ci < numResultColumns; ++ci) {
                        final Chunk<? extends Attributes.Values> inputChunk = inputColumns[ci].getChunk(inputContexts[ci], sliceSources);
