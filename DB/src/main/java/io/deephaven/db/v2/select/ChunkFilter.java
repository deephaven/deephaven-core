...
public interface ChunkFilter {
     ...
     */
    void filter(Chunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);

    interface CharChunkFilter extends ChunkFilter {
        void filter(CharChunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);

        default void filter(Chunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results) {
            filter(values.asCharChunk(), keys, results);
        }
    }

    interface ByteChunkFilter extends ChunkFilter {
        void filter(ByteChunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);
    ...
    interface ShortChunkFilter extends ChunkFilter {
        void filter(ShortChunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);
    ...
    interface IntChunkFilter extends ChunkFilter {
        void filter(IntChunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);
    ...
    interface LongChunkFilter extends ChunkFilter {
        void filter(LongChunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);
    ...
    interface FloatChunkFilter extends ChunkFilter {
        void filter(FloatChunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);
    ...
    interface DoubleChunkFilter extends ChunkFilter {
        void filter(DoubleChunk<? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);
    ...
    interface ObjectChunkFilter<T> extends ChunkFilter {
        void filter(ObjectChunk<T, ? extends Values> values, LongChunk<OrderedKeyIndices> keys, WritableLongChunk<OrderedKeyIndices> results);
     ...
     */
    long INITIAL_INTERRUPTION_SIZE = Configuration.getInstance().getLongWithDefault("ChunkFilter.initialInterruptionSize", 1<<20);
    /**
     * How long we would like to take, in milliseconds between interruption checks
     */
    long INTERRUPTION_GOAL_MILLIS = Configuration.getInstance().getLongWithDefault("ChunkFilter.interruptionGoalMillis", 100);
     ...
     */
    static Index applyChunkFilter(Index selection, ColumnSource<?> columnSource, boolean usePrev, ChunkFilter chunkFilter) {
        ...
        try (final ColumnSource.GetContext getContext = columnSource.makeGetContext(contextSize);
            ...
            while (okIt.hasMore()) {
                if (filteredChunks++ == chunksBetweenChecks) {
                    ...
                    chunksBetweenChecks = Math.max(1, Math.min(1, checkDuration <= 0 ? chunksBetweenChecks * 2 : chunksBetweenChecks * INTERRUPTION_GOAL_MILLIS / checkDuration));
