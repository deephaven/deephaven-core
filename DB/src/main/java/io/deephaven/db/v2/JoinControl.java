...
public class JoinControl {
    ...
    boolean useGrouping(Table leftTable, ColumnSource<?>[] leftSources) {
        return !leftTable.isLive() && leftSources.length == 1
            && leftTable.getIndex().hasGrouping(leftSources[0]);
    }
    ...
    boolean considerSymbolTables(QueryTable leftTable,
        @SuppressWarnings("unused") QueryTable rightTable, boolean useLeftGrouping,
        boolean useRightGrouping, ColumnSource<?> leftSource, ColumnSource<?> rightSource) {
        return !leftTable.isRefreshing() && !useLeftGrouping && leftSource.getType() == String.class
            && !rightTable.isRefreshing() && !useRightGrouping
            && rightSource.getType() == String.class
    ...
    boolean useSymbolTables(long leftSize, long leftSymbolSize, long rightSize,
        long rightSymbolSize) {
    ...
    RedirectionType getRedirectionType(Table leftTable) {
        ...
        } else if (leftTable.getIndex().getAverageRunLengthEstimate() >= Math
            .min(SparseConstants.BLOCK_SIZE / 4, leftTable.getIndex().size() / 2)) {
            // If we are going to use at least a quarter of a sparse array block, then it is a
            // better answer than a
            // hash table for redirection; because the hash table must store both the key and value,
            // and then has a
            // load factor of ~50%. Additionally, the sparse array source will have much faster sets
            // and lookups so is
