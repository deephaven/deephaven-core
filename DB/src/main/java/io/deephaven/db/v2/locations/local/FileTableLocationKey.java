 ...
 * partitions and described by a {@link File}. Sub-classes should override
 * {@link #compareTo(TableLocationKey)} and {@link #equals(Object)} only if they need to prevent
 * equality with other {@link FileTableLocationKey} implementations.
 */
public class FileTableLocationKey extends PartitionedTableLocationKey {
     ...
     *
     * @param file The file (or directory) that backs the keyed location. Will be adjusted to an
     *        absolute path.
     * @param order Explicit ordering value for this location key.
     *        {@link Comparable#compareTo(Object)} will sort FileTableLocationKeys with a lower
     *        {@code order} before other keys. Comparing this ordering value takes precedence over
     *        other fields.
    ...
    /**
     * Precedence-wise this implementation compares {@code order}, then applies a
     * {@link PartitionsComparator} to {@code partitions}, then compares {@code file}.
     * 
     * @inheritDoc
     */
    @Override
    ...
    public int hashCode() {
        if (cachedHashCode == 0) {
            ...
            // Don't use 0; that's used by StandaloneTableLocationKey, and also our sentinel for the
            // need to compute
