...
public abstract class RedefinableTable extends UncoalescedTable {
    ...
    public Table view(SelectColumn... columns) {
        ...
        for(SelectColumn selectColumn : columns) {
            ...
            resultColumnsInternal.addAll(usedColumnNames.stream().filter(usedColumnName -> !resultColumnsExternal.containsKey(usedColumnName)).map(definition::getColumn).collect(Collectors.toList()));
        ...
        TableDefinition newDefExternal = new TableDefinition(resultColumnsExternal.values().toArray(new ColumnDefinition[resultColumnsExternal.size()]));
        if(simpleRetain) {
            //NB: We use the *external* TableDefinition because it's ordered appropriately.
            return redefine(newDefExternal);
        }
        TableDefinition newDefInternal = new TableDefinition(resultColumnsInternal.toArray(new ColumnDefinition[resultColumnsInternal.size()]));
     ...
     *
     * @param newDefinitionExternal A TableDefinition that represents the results of redefine(newDefinitionInternal).view(viewColumns).
     * @param newDefinitionInternal A TableDefinition with a subset of this RedefinableTable's ColumnDefinitions.
     * @param viewColumns A set of SelectColumns to apply in order to transform a table with newDefinitionInternal to a table with newDefinitionExternal.
