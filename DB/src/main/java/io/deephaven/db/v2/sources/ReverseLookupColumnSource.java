 ...
 * Note that int keys are used because this is intended for column sources with a small, contiguous
 * key range starting from 0 and well shorter than Integer.MAX_VALUE.
 */
public interface ReverseLookupColumnSource<DATA_TYPE, EXTRA_VALUE_TYPE>
     ...
     * @return A reverse-lookup function that has all values defined for keys in [0,
     *         highestKeyNeeded]
     */
     ...
     * @return The key, between 0 and highestIndex, for the value. A value outside this range if the
     *         value has no mapping in the range.
     */
