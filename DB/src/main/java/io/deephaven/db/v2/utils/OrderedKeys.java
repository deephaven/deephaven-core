...
public interface OrderedKeys extends SafeCloseable, LongSizedDataStructure {
    ...
    /**
     * <p>Get an ordered subset of the keys in this {@code OrderedKeys} for a position range.
     * The result will contain the set of keys in {@code this} that lie at positions in the half-open range
     * [{@code startPositionInclusive}, {@code startPositionInclusive + length}).
     *
    ...
    /**
     * <p>Get an ordered subset of the keys in this {@code OrderedKeys} for a key range. The returned set will be the
     * intersection of the keys in {@code this}  with the keys in the closed interval
     ...
     * @apiNote This {@code OrderedKeys} owns the result, which is valid only as long as this {@code OrderedKeys} remains valid.
     * @apiNote You must not mutate the result.
     */
    ...
    /**
     * <p>Fill the supplied {@link WritableLongChunk} with individual keys from this {@code OrderedKeys}.
     * <p>The chunk's capacity is assumed to be big enough.
     *
    ...
    /**
     * <p>Fill the supplied {@link WritableLongChunk} with key ranges from this {@code OrderedKeys}.
     * <p>The chunk's capacity is assumed to be big enough.
     *
    ...
    /**
     * <p>Get an estimate of the average (mean) length of runs of adjacent keys in this {@code OrderedKeys}.
     * <p>Implementations should strive to keep this method efficient (<i>O(1)</i> preferred) at the expense of
     * accuracy.
     * <p>Empty {@code OrderedKeys} should return an arbitrary valid value, usually 1.
     *
    ...
    /**
     * For as long as the consumer wants more keys, call accept on the consumer with the individual key instances
    ...
    /**
     * For as long as the consumer wants more ranges, call accept on the consumer with the individual key ranges
     * in this OrderedKeys, in increasing order.
     *
    ...
    /**
     * <p>Free any resources associated with this object.
     * <p>Using any {@code OrderedKeys} methods after {@code close()} is an error and may produce exceptions or
     * undefined results.
     */
    default void close() {}
    ...
    interface Iterator extends SafeCloseable {
         ...
         * If the maximum key provided is smaller than the next key (as would be returned by {@link #peekNextKey()}),
         * the empty OrderedKeys is returned.
         *
         * The returned OrderedKeys object is only borrowed by the caller from the {@link Iterator}, who owns it.
         * It is guaranteed to be valid and not change only until a later call to another {@code getNext*} method.
         * As the returned reference is owned by the {@link Iterator}, the caller <i>should not</i> call {@code close()}
         * on it.
         *
         ...
         *
         * The returned OrderedKeys object is only borrowed by the caller from the {@link Iterator}, who owns it.
         * It is guaranteed to be valid and not change only until the next call to another {@code getNext*} method.
         * As the returned reference is owned by the {@link Iterator}, the caller <i>should not</i> call {@code close()}
         * on it.
         *
        ...
        /**
         * <p>Advance this iterator's position to {@code nextKey}, or to the first present key greater than
         * {@code nextKey} if {@code nextKey} is not found. If {@code nextKey} is less than or equal to the key at this
         * iterator's current position, this method is a no-op.
         * <p>Subsequent calls to {@link #peekNextKey()}, {@link #getNextOrderedKeysThrough(long)}, or
        ...
        /**
         * <p>Free any resources associated with this iterator.
         * <p>Callers of {@link OrderedKeys#getOrderedKeysIterator()} are responsible for ensuring that {@code close()}
         * is called when they are done with resulting {@link Iterator}.
         * <p>Using any {@link Iterator} methods after {@code close()} is an error and may produce exceptions or
         * undefined results.
         */
        default void close() {}

        /**
         * Taking the difference between values returned by this method at different positions in the iterator
    ...
    OrderedKeys EMPTY = new OrderedKeys() {
        ...
        @Override
        public boolean forEachLongRange(final LongRangeAbortableConsumer larc) { return true; }

        @Override
        public String toString() { return "OrderedKeys.EMPTY"; }
    };
}
