...
public class RegionedColumnSourceManager implements ColumnSourceManager {
    ...
    /**
     * State for table locations that have been added, but have never been found to exist with
     * non-zero size.
     */
     ...
     */
    private final List<IncludedTableLocationEntry> orderedIncludedTableLocations =
        new ArrayList<>();
    ...
    public synchronized void addLocation(@NotNull final TableLocation tableLocation) {
        final IncludedTableLocationEntry includedLocation =
            includedTableLocations.get(tableLocation.getKey());
        final EmptyTableLocationEntry emptyLocation =
            emptyTableLocations.get(tableLocation.getKey());
        ...
        if (includedLocation == null && emptyLocation == null) {
            ...
            if (tableLocation != duplicateLocation) {
                // If it ever transpires that we need to compare the locations and not just detect a
                // second add, then
                ...
                throw new TableDataException(
                    "Data Routing Configuration error: TableDataService elements overlap at locations "
                        +
                        tableLocation.toStringDetailed() + " and "
                        + duplicateLocation.toStringDetailed());
            } else {
                // This is unexpected - we got the identical table location object twice
                // If we ever get this, some thought needs to go into why.
                throw new TableDataException(
                    "Unexpected: TableDataService returned the same location twice: " +
    ...
    public synchronized Index refresh() {
        ...
        for (final IncludedTableLocationEntry entry : orderedIncludedTableLocations) { // Ordering
                                                                                       // matters,
                                                                                       // since
                                                                                       // we're
                                                                                       // using a
        ...
        for (final Iterator<EmptyTableLocationEntry> iterator =
            emptyTableLocations.iterator(); iterator.hasNext();) {
            ...
            if (locationIndex != null) {
                if (locationIndex.empty()) {
                    ...
                    (entriesToInclude == null ? entriesToInclude = new TreeSet<>()
                        : entriesToInclude).add(nonexistentEntry);
                    iterator.remove();
                }
            }
        }
        if (entriesToInclude != null) {
            for (final EmptyTableLocationEntry entryToInclude : entriesToInclude) {
                final IncludedTableLocationEntry entry =
                    new IncludedTableLocationEntry(entryToInclude);
    ...
    public final synchronized void disableGrouping() {
        ...
        for (ColumnDefinition<?> columnDefinition : columnDefinitions) {
            if (columnDefinition.isGrouping()) {
                DeferredGroupingColumnSource<?> columnSource =
                    getColumnSources().get(columnDefinition.getName());
    ...
    /**
     * State keeper for a table location and its subscription buffer if it hasn't been found to have
     * a non-null, non-zero size yet.
     */
    private class EmptyTableLocationEntry implements Comparable<EmptyTableLocationEntry> {
        ...
        private void refresh() {
            if (subscriptionBuffer != null) {
                ...
                // NB: This should be hit only once per entry - subscription buffers handle all
                // "isRefreshing"
                // (i.e. "live") tables, regardless of whether the underlying locations support
                // subscriptions.
                location.refresh();
            }
        }
    ...
    /**
     * State-keeper for a table location and its column locations, once it's been found to have a
     * positive size.
     */
    private class IncludedTableLocationEntry implements Comparable<IncludedTableLocationEntry> {
        ...
        private void processInitial(final Index.SequentialBuilder addedIndexBuilder,
            final ReadOnlyIndex initialIndex) {
            ...
            if (initialIndex
                .lastKey() > RegionedColumnSource.ELEMENT_INDEX_TO_SUB_REGION_ELEMENT_INDEX_MASK) {
            ...
            initialIndex.forAllLongRanges(
                (subRegionFirstKey, subRegionLastKey) -> addedIndexBuilder.appendRange(
                    regionFirstKey + subRegionFirstKey, regionFirstKey + subRegionLastKey));
            ...
            try {
                for (final ColumnDefinition columnDefinition : columnDefinitions) {
                    ...
                    if (state.needToUpdateGrouping()) {
                        state.updateGrouping(addIndexInTable == null
                            ? addIndexInTable = initialIndex.shift(regionFirstKey)
                            : addIndexInTable);
                    }
                }
            } finally {
        ...
        private void pollUpdates(final Index.SequentialBuilder addedIndexBuilder) {
            Assert.neqNull(subscriptionBuffer, "subscriptionBuffer"); // Effectively, this is
                                                                      // asserting "isRefreshing".
            ...
            try {
                if (updateIndex == null) {
                    // This should be impossible - the subscription buffer transforms a transition
                    // to null into a pending exception
                    throw new TableDataException(
                        "Location " + location + " is no longer available, data has been removed");
                }
                if (!indexAtLastUpdate.subsetOf(updateIndex)) { // Bad change
                    // noinspection ThrowableNotThrown
                    Assert.statementNeverExecuted("Index keys removed at location " + location
                ...
                try (final ReadOnlyIndex addedIndex = updateIndex.minus(indexAtLastUpdate)) {
                    ...
                    try {
                        for (final ColumnLocationState state : columnLocationStates) {
                            if (state.needToUpdateGrouping()) {
                                state.updateGrouping(addIndexInTable == null
                                    ? addIndexInTable = updateIndex.shift(regionFirstKey)
                                    : addIndexInTable);
                            }
                        }
                    } finally {
    ...
    /**
     * Batches up a definition, source, and location for ease of use. Implements grouping
     * maintenance.
     */
        ...
        private void updateGrouping(@NotNull final ReadOnlyIndex locationAddedIndexInTable) {
            if (definition.isGrouping()) {
                ...
                if (groupingProvider instanceof KeyRangeGroupingProvider) {
                    ((KeyRangeGroupingProvider) groupingProvider).addSource(location,
                        locationAddedIndexInTable);
                }
                ...
                if (current == null) {
                    columnPartitionToIndex.put(columnPartitionValue,
                        locationAddedIndexInTable.clone());
                } else {
