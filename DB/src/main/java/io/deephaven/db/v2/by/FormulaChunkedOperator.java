...
class FormulaChunkedOperator implements IterativeChunkedAggregationOperator {
     ...
     * @param delegateToBy Whether this operator is responsible for passing methods through to
     *        {@code by}. Should be false if {@code by} is updated by the helper (and {@code by}
     *        must come before this operator if so), or if this is not the first operator sharing
     *        {@code by}.
    ...
    FormulaChunkedOperator(@NotNull final ByChunkedOperator by,
        ...
        formulaDataSources = new ChunkSource[resultColumnPairs.length]; // Not populated until
                                                                        // propagateInitialState
        resultColumns = new WritableSource[resultColumnPairs.length];
        resultColumnModifiedColumnSets = new ModifiedColumnSet[resultColumnPairs.length]; // Not
                                                                                          // populated
                                                                                          // until
        ...
        for (int ci = 0; ci < resultColumnPairs.length; ++ci) {
            ...
            final FormulaColumn formulaColumn =
                formulaColumns[ci] = FormulaColumn.createFormulaColumn(outputColumnName,
            ...
            // noinspection unchecked
            resultColumns[ci] =
                ArrayBackedColumnSource.getMemoryColumnSource(0, formulaColumn.getReturnedType());
        }
    }
    ...
    public void addChunk(final BucketedContext bucketedContext,
        ...
        if (delegateToBy) {
            by.addChunk(bucketedContext, values, inputIndices, destinations, startPositions, length,
                stateModified);
        }
    }
    ...
    public void removeChunk(final BucketedContext bucketedContext,
        ...
        @NotNull final WritableBooleanChunk<Values> stateModified) {
        if (delegateToBy) {
            by.removeChunk(bucketedContext, values, inputIndices, destinations, startPositions,
    ...
    public void modifyChunk(final BucketedContext bucketedContext,
        ...
        @NotNull final WritableBooleanChunk<Values> stateModified) {
        if (delegateToBy) {
            by.modifyChunk(bucketedContext, previousValues, newValues, postShiftIndices,
                destinations, startPositions, length, stateModified);
        }
    }
    ...
    public void shiftChunk(final BucketedContext bucketedContext,
        ...
        @NotNull final WritableBooleanChunk<Values> stateModified) {
        if (delegateToBy) {
            by.shiftChunk(bucketedContext, previousValues, newValues, preShiftIndices,
                postShiftIndices, destinations, startPositions, length, stateModified);
        }
    }
    ...
    public void modifyIndices(final BucketedContext context,
        ...
        @NotNull final WritableBooleanChunk<Values> stateModified) {
        if (delegateToBy) {
            by.modifyIndices(context, inputIndices, destinations, startPositions, length,
                stateModified);
        }
    }
    ...
    public boolean modifyChunk(final SingletonContext singletonContext, final int chunkSize,
        ...
        if (delegateToBy) {
            return by.modifyChunk(singletonContext, chunkSize, previousValues, newValues,
                postShiftIndices, destination);
        } else {
            return false;
        }
    }
    ...
    public boolean shiftChunk(final SingletonContext singletonContext,
        ...
        final long destination) {
        if (delegateToBy) {
            return by.shiftChunk(singletonContext, previousValues, newValues, preInputIndices,
                postInputIndices, destination);
        } else {
            return false;
        }
    }
    ...
    public void propagateInitialState(@NotNull final QueryTable resultTable) {
        ...
        try (final DataCopyContext dataCopyContext =
            new DataCopyContext(allColumnsMask, allColumnsMask)) {
    ...
    public UnaryOperator<ModifiedColumnSet> initializeRefreshing(
        ...
        for (int ci = 0; ci < resultColumnNames.length; ++ci) {
            resultColumnModifiedColumnSets[ci] =
                resultTable.newModifiedColumnSet(resultColumnNames[ci]);
        }
        if (delegateToBy) {
            // We cannot use the by's result MCS factory, because the result column names are not
            // guaranteed to be the same.
            by.initializeRefreshing(resultTable, aggregationUpdateListener);
        }
        // Note that we also use the factory in propagateUpdates to identify the set of modified
        // columns to handle.
    ...
    public void propagateUpdates(@NotNull final ShiftAwareListener.Update downstream,
        ...
        final boolean modifiesToProcess =
            downstream.modified.nonempty() && resultModifiedColumnSet.nonempty();
        ...
        if (!addsToProcess && !modifiesToProcess) {
            try (final DataFillerContext dataFillerContext =
                new DataFillerContext(makeObjectColumnsMask())) {
        ...
        final boolean[] columnsToFillMask = addsToProcess ? makeAllColumnsMask()
            : removesToProcess ? makeObjectOrModifiedColumnsMask(resultModifiedColumnSet)
                : modifiedColumnsMask;
        final boolean[] columnsToGetMask = addsToProcess ? columnsToFillMask
            /* This is the result of makeAllColumnsMask() on the line above */ : modifiedColumnsMask;

        try (final DataCopyContext dataCopyContext =
            new DataCopyContext(columnsToFillMask, columnsToGetMask)) {
            if (removesToProcess) {
    ...
    private class DataFillerContext implements SafeCloseable {
        ...
        void clearObjectColumnData(@NotNull final OrderedKeys orderedKeys) {
            try (
                final OrderedKeys.Iterator orderedKeysIterator =
                ...
                while (orderedKeysIterator.hasMore()) {
                    final OrderedKeys orderedKeysSlice =
                        orderedKeysIterator.getNextOrderedKeysThrough(
                    ...
                    for (int ci = 0; ci < columnsToFillMask.length; ++ci) {
                        ...
                        if (columnsToFillMask[ci] && !resultColumn.getType().isPrimitive()) {
                            resultColumn.fillFromChunk(fillFromContexts[ci], nullValueChunk,
                                orderedKeysSlice);
                        }
                    }
                }
    ...
    private class DataCopyContext extends DataFillerContext {
        ...
        private DataCopyContext(@NotNull final boolean[] columnsToFillMask,
            @NotNull final boolean[] columnsToGetMask) {
            ...
            for (int ci = 0; ci < resultColumnNames.length; ++ci) {
                if (columnsToGetMask[ci]) {
                    getContexts[ci] =
                        formulaDataSources[ci].makeGetContext(BLOCK_SIZE, sharedContext);
                }
            }
        }
        ...
        private void copyData(@NotNull final OrderedKeys orderedKeys,
            @NotNull final boolean[] columnsMask) {
            try (final OrderedKeys.Iterator orderedKeysIterator =
                orderedKeys.getOrderedKeysIterator()) {
                while (orderedKeysIterator.hasMore()) {
                    final OrderedKeys orderedKeysSlice =
                        orderedKeysIterator.getNextOrderedKeysThrough(
    ...
    private boolean[] makeModifiedColumnsMask(
    ...
    private boolean[] makeObjectOrModifiedColumnsMask(
        @NotNull final ModifiedColumnSet resultModifiedColumnSet) {
        final boolean[] columnsMask = new boolean[resultColumns.length];
        for (int ci = 0; ci < resultColumns.length; ++ci) {
            columnsMask[ci] = !resultColumns[ci].getType().isPrimitive()
