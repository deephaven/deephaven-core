...
/**
 * An {@link IterativeChunkedAggregationOperator} used in the implementation of {@link io.deephaven.db.tables.Table#applyToAllBy}.
 */
class FormulaChunkedOperator implements IterativeChunkedAggregationOperator {
     ...
     * @param delegateToBy      Whether this operator is responsible for passing methods through to  {@code by}.
     *                          Should be false if {@code by} is updated by the helper (and {@code by} must come before
     *                          this operator if so), or if this is not the first operator sharing {@code by}.
    ...
    FormulaChunkedOperator(@NotNull final ByChunkedOperator by,
        ...
        resultColumnModifiedColumnSets = new ModifiedColumnSet[resultColumnPairs.length]; // Not populated until initializeRefreshing
        ...
        for (int ci = 0; ci < resultColumnPairs.length; ++ci) {
            ...
            final FormulaColumn formulaColumn = formulaColumns[ci] = FormulaColumn.createFormulaColumn(outputColumnName, Utils.replaceFormulaTokens(formula, columnParamName, inputColumnName));
            ...
            final ColumnDefinition<?> inputColumnDefinition = ColumnDefinition
                .fromGenericType(inputColumnName, inputColumnSource.getType(), inputColumnSource.getComponentType());
    ...
    public void modifyChunk(final BucketedContext bucketedContext, final Chunk<? extends Values> previousValues, final Chunk<? extends Values> newValues,
                            ...
                            @NotNull final IntChunk<ChunkLengths> length, @NotNull final WritableBooleanChunk<Values> stateModified) {
        if (delegateToBy) {
            by.modifyChunk(bucketedContext, previousValues, newValues, postShiftIndices, destinations, startPositions, length, stateModified);
        }
    }
    ...
    public void shiftChunk(final BucketedContext bucketedContext, final Chunk<? extends Values> previousValues, final Chunk<? extends Values> newValues,
                           ...
                           @NotNull final IntChunk<ChunkLengths> length, @NotNull final WritableBooleanChunk<Values> stateModified) {
        if (delegateToBy) {
            by.shiftChunk(bucketedContext, previousValues, newValues, preShiftIndices, postShiftIndices, destinations, startPositions, length, stateModified);
        }
    }
    ...
    public boolean modifyChunk(final SingletonContext singletonContext, final int chunkSize,
        ...
        if (delegateToBy) {
            return by.modifyChunk(singletonContext, chunkSize, previousValues, newValues, postShiftIndices, destination);
        } else {
            return false;
        }
    }
    ...
    public boolean shiftChunk(final SingletonContext singletonContext, final Chunk<? extends Values> previousValues, final Chunk<? extends Values> newValues,
                              ...
                              final long destination) {
        if (delegateToBy) {
            return by.shiftChunk(singletonContext, previousValues, newValues, preInputIndices, postInputIndices, destination);
        } else {
            return false;
        }
    }
    ...
    public void ensureCapacity(final long tableSize) {
        ...
        for (@NotNull final WritableSource<?> resultColumn : resultColumns) {
    ...
    public void propagateInitialState(@NotNull final QueryTable resultTable) {
        ...
        for (int ci = 0; ci < inputColumnNames.length; ++ci) {
            ...
            final ColumnSource<?> inputColumnSource = byResultColumns.get(inputColumnName);
            formulaColumn.initInputs(resultTable.getIndex(), Collections.singletonMap(inputColumnName, inputColumnSource));
            //noinspection unchecked
    ...
    public UnaryOperator<ModifiedColumnSet> initializeRefreshing(@NotNull final QueryTable resultTable, @NotNull final LivenessReferent aggregationUpdateListener) {
        ...
        if (delegateToBy) {
            // We cannot use the by's result MCS factory, because the result column names are not guaranteed to be the same.
        ...
        return inputToResultModifiedColumnSetFactory = by.makeInputToResultModifiedColumnSetFactory(resultTable, resultColumnNames);
    }
    ...
    public void resetForStep(@NotNull final ShiftAwareListener.Update upstream) {
        ...
        updateUpstreamModifiedColumnSet = upstream.modified.empty() ? ModifiedColumnSet.EMPTY : upstream.modifiedColumnSet;
    }
    ...
    public void propagateUpdates(@NotNull final ShiftAwareListener.Update downstream, @NotNull final ReadableIndex newDestinations) {
        ...
        final ModifiedColumnSet resultModifiedColumnSet = inputToResultModifiedColumnSetFactory.apply(updateUpstreamModifiedColumnSet);
        ...
        final boolean[] columnsToFillMask = addsToProcess ? makeAllColumnsMask() : removesToProcess ? makeObjectOrModifiedColumnsMask(resultModifiedColumnSet) : modifiedColumnsMask;
        final boolean[] columnsToGetMask = addsToProcess ? columnsToFillMask /* This is the result of makeAllColumnsMask() on the line above */ : modifiedColumnsMask;
    ...
    @Override
    public void propagateFailure(@NotNull final Throwable originalException, @NotNull final UpdatePerformanceTracker.Entry sourceEntry) {
        if (delegateToBy) {
    ...
    private class DataFillerContext implements SafeCloseable {
        ...
        void clearObjectColumnData(@NotNull final OrderedKeys orderedKeys) {
            try (final OrderedKeys.Iterator orderedKeysIterator = orderedKeys.getOrderedKeysIterator();
                 final WritableObjectChunk<?, Values> nullValueChunk = WritableObjectChunk.makeWritableChunk(BLOCK_SIZE)) {
                nullValueChunk.fillWithNullValue(0, BLOCK_SIZE);
                while (orderedKeysIterator.hasMore()) {
                    final OrderedKeys orderedKeysSlice = orderedKeysIterator.getNextOrderedKeysThrough(calculateContainingBlockLastKey(orderedKeysIterator.peekNextKey()));
            ...
            try (final OrderedKeys.Iterator orderedKeysIterator = orderedKeys.getOrderedKeysIterator()) {
                while (orderedKeysIterator.hasMore()) {
                    ...
                    for (int ci = 0; ci < columnsToGetMask.length; ++ci) {
                        if (columnsMask[ci]) {
                            resultColumns[ci].fillFromChunk(fillFromContexts[ci], formulaDataSources[ci].getChunk(getContexts[ci], orderedKeysSlice), orderedKeysSlice);
                        }
                    }
                }
            }
    ...
    private boolean[] makeObjectOrModifiedColumnsMask(@NotNull final ModifiedColumnSet resultModifiedColumnSet) {
        final boolean[] columnsMask = new boolean[resultColumns.length];
        for (int ci = 0; ci < resultColumns.length; ++ci) {
            columnsMask[ci] = !resultColumns[ci].getType().isPrimitive() || resultModifiedColumnSet.containsAny(resultColumnModifiedColumnSets[ci]);
        }
        return columnsMask;
    }
}
