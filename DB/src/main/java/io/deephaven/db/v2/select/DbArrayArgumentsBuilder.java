...
public class DbArrayArgumentsBuilder implements ArgumentsBuilder<DbArrayArgumentsBuilder.Context> {
    ...
    public DbArrayArgumentsBuilder(String npTypeName) {
    ...
    public Object[] packArguments(Chunk incomingData, Context context, int size) {
        ...
        int totalSize = 0;
        ObjectChunk<DbArrayBase, Attributes.Any> typedChunk = incomingData.asObjectChunk();
        for (int i = 0; i < size; i++) {
            totalSize += typedChunk.get(i).size();
        ...
        int offset = 0;
        for (int i = 0; i < size; i++) {
            DbArrayBase srcArray = typedChunk.get(i);
            int len = srcArray.intSize();
            srcArray.fillChunk(chunkDest.slice(offset,len));
            offset += len;
            offsetDest[i] = offset;
        }
