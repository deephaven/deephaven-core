...
public class ReplicateHashing {
    public static void main(String[] args) throws IOException {
        ...
        final String objectHasher = ReplicatePrimitiveCode.charToObject(CharChunkHasher.class, ReplicatePrimitiveCode.MAIN_SRC);
        ...
        final List<String> paths = ReplicatePrimitiveCode.charToAll(CharChunkEquals.class, ReplicatePrimitiveCode.MAIN_SRC);
        final String floatPath = paths.stream().filter(p -> p.contains("Float")).findFirst().orElseThrow(FileNotFoundException::new);
        final String doublePath = paths.stream().filter(p -> p.contains("Double")).findFirst().orElseThrow(FileNotFoundException::new);

        fixupFloatChunkEquals(floatPath);
        fixupDoubleChunkEquals(doublePath);

        final String objectIdentityEquals = ReplicatePrimitiveCode.charToObject(CharChunkEquals.class, ReplicatePrimitiveCode.MAIN_SRC);
        fixupObjectChunkIdentityEquals(objectIdentityEquals);

        final String objectEquals = ReplicatePrimitiveCode.charToObject(CharChunkEquals.class, ReplicatePrimitiveCode.MAIN_SRC);
        fixupObjectChunkEquals(objectEquals);

        final List<String> compactKernels = ReplicatePrimitiveCode.charToAll(CharCompactKernel.class, ReplicatePrimitiveCode.MAIN_SRC);
        final String objectCompact = ReplicatePrimitiveCode.charToObject(CharCompactKernel.class, ReplicatePrimitiveCode.MAIN_SRC);
    ...
    private static void fixupObjectChunkHasher(String objectPath) throws IOException {
        ...
        FileUtils.writeLines(objectFile, globalReplacements(fixupChunkAttributes(lines), "Object.hashCode", "Objects.hashCode", "TypeUtils.unbox\\(\\(Object\\)value\\)", "value"));
    }
    ...
    private static void fixupFloatCompact(String doublePath, String typeOfFloat) throws IOException {
        ...
        List<String> lines = FileUtils.readLines(objectFile, Charset.defaultCharset());
        lines = replaceRegion(lines, "shouldIgnore", Collections.singletonList("        return value == NULL_" + typeOfFloat.toUpperCase() + " || " + typeOfFloat + ".isNaN(value);"));
        FileUtils.writeLines(objectFile, lines);
    }
    ...
    private static void fixupObjectChunkIdentityEquals(String objectPath) throws IOException {
        ...
        final File objectChunkIdentifyEqualsFileName = new File(objectChunkEqualsFileName.getParent(), "ObjectChunkIdentityEquals.java");
        Assert.eqTrue(objectChunkEqualsFileName.renameTo(objectChunkIdentifyEqualsFileName), "objectChunkEqualsFileName.renameTo(objectChunkIdentifyEqualsFileName)");
    ...
    private static void fixupDoubleChunkEquals(String doublePath) throws IOException {
        ...
        FileUtils.writeLines(objectFile, simpleFixup(lines,"eq", "lhs == rhs", "((Double.isNaN(lhs) && Double.isNaN(rhs)) || lhs == rhs)"));
    }

    private static void fixupFloatChunkEquals(String floatPath) throws IOException {
        final File objectFile = new File(floatPath);
        final List<String> lines = FileUtils.readLines(objectFile, Charset.defaultCharset());
        FileUtils.writeLines(objectFile, simpleFixup(lines,"eq", "lhs == rhs", "((Float.isNaN(lhs) && Float.isNaN(rhs)) || lhs == rhs)"));
    }
