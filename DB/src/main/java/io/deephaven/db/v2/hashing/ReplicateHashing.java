...
public class ReplicateHashing {
    public static void main(String[] args) throws IOException {
        ...
        final String objectHasher = ReplicatePrimitiveCode.charToObject(CharChunkHasher.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        fixupObjectChunkHasher(objectHasher);

        ReplicatePrimitiveCode.charToIntegers(CharToIntegerCast.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        ReplicatePrimitiveCode.charToIntegers(CharToLongCast.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        ReplicatePrimitiveCode.charToIntegers(CharToIntegerCastWithOffset.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        ReplicatePrimitiveCode.charToIntegers(CharToLongCastWithOffset.class,
            ReplicatePrimitiveCode.MAIN_SRC);

        final List<String> paths = ReplicatePrimitiveCode.charToAll(CharChunkEquals.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        final String floatPath = paths.stream().filter(p -> p.contains("Float")).findFirst()
            .orElseThrow(FileNotFoundException::new);
        final String doublePath = paths.stream().filter(p -> p.contains("Double")).findFirst()
            .orElseThrow(FileNotFoundException::new);

        fixupFloatChunkEquals(floatPath);
        fixupDoubleChunkEquals(doublePath);

        final String objectIdentityEquals = ReplicatePrimitiveCode
            .charToObject(CharChunkEquals.class, ReplicatePrimitiveCode.MAIN_SRC);
        fixupObjectChunkIdentityEquals(objectIdentityEquals);

        final String objectEquals = ReplicatePrimitiveCode.charToObject(CharChunkEquals.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        fixupObjectChunkEquals(objectEquals);

        final List<String> compactKernels = ReplicatePrimitiveCode
            .charToAll(CharCompactKernel.class, ReplicatePrimitiveCode.MAIN_SRC);
        final String objectCompact = ReplicatePrimitiveCode.charToObject(CharCompactKernel.class,
            ReplicatePrimitiveCode.MAIN_SRC);
        fixupObjectCompact(objectCompact);
        // noinspection OptionalGetWithoutIsPresent
        fixupBooleanCompact(
            compactKernels.stream().filter(x -> x.contains("Boolean")).findFirst().get());
        fixupFloatCompact(
            compactKernels.stream().filter(x -> x.contains("Double")).findFirst().get(), "Double");
        fixupFloatCompact(
            compactKernels.stream().filter(x -> x.contains("Float")).findFirst().get(), "Float");
    }
    ...
    private static void fixupObjectChunkHasher(String objectPath) throws IOException {
        ...
        FileUtils.writeLines(objectFile,
            globalReplacements(fixupChunkAttributes(lines), "Object.hashCode", "Objects.hashCode",
                "TypeUtils.unbox\\(\\(Object\\)value\\)", "value"));
    }
    ...
    private static void fixupBooleanCompact(String booleanPath) throws IOException {
        ...
        lines = replaceRegion(lines, "shouldIgnore",
            Collections.singletonList("        return false;"));
    ...
    private static void fixupFloatCompact(String doublePath, String typeOfFloat)
        throws IOException {
        final File objectFile = new File(doublePath);
        List<String> lines = FileUtils.readLines(objectFile, Charset.defaultCharset());
        lines = replaceRegion(lines, "shouldIgnore",
            Collections.singletonList("        return value == NULL_" + typeOfFloat.toUpperCase()
                + " || " + typeOfFloat + ".isNaN(value);"));
        FileUtils.writeLines(objectFile, lines);
    }
    ...
    private static void fixupObjectChunkIdentityEquals(String objectPath) throws IOException {
        ...
        final List<String> lines =
            FileUtils.readLines(objectChunkIdentifyEqualsFileName, Charset.defaultCharset());
        FileUtils.writeLines(objectChunkIdentifyEqualsFileName,
            simpleFixup(fixupChunkAttributes(lines),
    ...
    private static void fixupDoubleChunkEquals(String doublePath) throws IOException {
        ...
        FileUtils.writeLines(objectFile, simpleFixup(lines, "eq", "lhs == rhs",
            "((Double.isNaN(lhs) && Double.isNaN(rhs)) || lhs == rhs)"));
    }
    ...
    private static void fixupFloatChunkEquals(String floatPath) throws IOException {
        ...
        final List<String> lines = FileUtils.readLines(objectFile, Charset.defaultCharset());
        FileUtils.writeLines(objectFile, simpleFixup(lines, "eq", "lhs == rhs",
            "((Float.isNaN(lhs) && Float.isNaN(rhs)) || lhs == rhs)"));
    }
