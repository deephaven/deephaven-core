...
public class C14nUtil {
     ...
     */
    public static final OpenAddressedCanonicalizationCache CACHE = ENABLED ? new OpenAddressedCanonicalizationCache(10000) : null;

    /**
     * A whitelist of classes that we'll canonicalize in the maybeCanonicalize* methods.  Mutable classes or classes
     * with hashCode() and equals(...) definitions that violate the usual contracts are dangerous and unsupported.
     */
    ...
    /**
     * Canonicalizes an array of objects in-place using the default CanonicalizationCache, if they're instances of
     * known appropriate classes.  May canonicalize some items without canonicalizing all.
     * @param items
    ...
    public static SmartKey makeSmartKey(final Object... values) {
        return maybeCanonicalizeAll(values) ? /*canonicalize(*/new CanonicalizedSmartKey(values)/*)*/ : new SmartKey(values);
    }
