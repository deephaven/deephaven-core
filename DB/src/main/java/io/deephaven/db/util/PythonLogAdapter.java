...
class PythonLogAdapter {
     ...
     * newline; we need to remember that we added one, so that we can suppress the next empty newline.  If there was
     * a newline, we shouldn't suppress it (e.g., when printing just a blank line to the output we need to preserve it).
     *
    ...
    /**
     * If we just allow python to print to it's regular STDOUT and STDERR, then it bypasses the
     * Java System.out/err.
     *
