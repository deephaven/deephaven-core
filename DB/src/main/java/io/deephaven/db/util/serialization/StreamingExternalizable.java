...
public interface StreamingExternalizable {
    ...
    /**
     * <p>
     * Alternative to {@link java.io.Externalizable#writeExternal(ObjectOutput)} for use when a
     * series of tuples of the same type with the same element types are being written in iterative
     * fashion.
     * <p>
     * Primitive elements should be written with their primitive write methods (e.g.
     * {@link ObjectOutput#writeInt(int)}).
     * <p>
     * Object elements are preceded by a boolean, true if null, false otherwise. The first non-null
     * value for a given Object element is then preceded by the name of the class. All non-null
     * values are then written with a writer method from
     * {@link SerializationUtils#getWriter(Class, ObjectOutput)}, cached in cachedWriters.
     *
    ...
    /**
     * <p>
     * Alternative to {@link java.io.Externalizable#readExternal(ObjectInput)} for use when a series
     * of tuples of the same type with the same element types are being read in iterative fashion.
     * <p>
     * Primitive elements should be read with their primitive read methods (e.g.
     * {@link ObjectInput#readInt()}).
     * <p>
     * Object elements are preceded by a boolean, true if null, false otherwise. The first non-null
     * value for a given Object element is then preceded by the name of the class. All non-null
     * values are then read with a reader method from
     * {@link SerializationUtils#getReader(Class, ObjectInput)}, cached in cachedReaders.
     *
     * @param in The input
     * @param cachedReaders The cached readers
     */
    void readExternalStreaming(@NotNull ObjectInput in,
        @NotNull TIntObjectMap<SerializationUtils.Reader> cachedReaders) throws Exception;
     ...
     * @return this
     */
    default <TYPE extends StreamingExternalizable> TYPE initializeExternalStreaming(
