package io.deephaven.db.util.serialization;
...
public interface StreamingExternalizable {
    ...
    /**
     * <p>Alternative to {@link java.io.Externalizable#writeExternal(ObjectOutput)} for use when a series of tuples of
     * the same type with the same element types are being written in iterative fashion.
     * <p>Primitive elements should be written with their primitive write methods
     * (e.g. {@link ObjectOutput#writeInt(int)}).
     * <p>Object elements are preceded by a boolean, true if null, false otherwise. The first non-null value for a given
     ...
     */
    void writeExternalStreaming(@NotNull ObjectOutput out, @NotNull TIntObjectMap<SerializationUtils.Writer> cachedWriters) throws IOException;

    /**
     * Implement the Object element writing protocol described in {@link #writeExternalStreaming(ObjectOutput, TIntObjectMap)}.
     *
     * @param out           The output
     * @param cachedWriters The cached writers
    ...
    /**
     * <p>Alternative to {@link java.io.Externalizable#readExternal(ObjectInput)} for use when a series of tuples of
     * the same type with the same element types are being read in iterative fashion.
     * <p>Primitive elements should be read with their primitive read methods (e.g. {@link ObjectInput#readInt()}).
     * <p>Object elements are preceded by a boolean, true if null, false otherwise. The first non-null value for a given
     * Object element is then preceded by the name of the class. All non-null values are then read with a reader
     * method from {@link SerializationUtils#getReader(Class, ObjectInput)}, cached in cachedReaders.
     *
     * @param in            The input
     * @param cachedReaders The cached readers
     */
    void readExternalStreaming(@NotNull ObjectInput in, @NotNull TIntObjectMap<SerializationUtils.Reader> cachedReaders) throws Exception;

    /**
     * Convenience method to allow chaining of construction and calls to {@link #readExternalStreaming(ObjectInput, TIntObjectMap)}.
     *
     * @param in            The input
     * @param cachedReaders The cached readers
     * @return this
    ...
    /**
     * Implement the Object element reading protocol described in {@link #readExternalStreaming(ObjectInput, TIntObjectMap)}.
     *
     * @param in            The input
     * @param cachedReaders The cached readers
