...
abstract class ReferenceCountedLivenessNode extends ReferenceCounted implements LivenessNode {
    ...
    public final boolean tryManage(@NotNull final LivenessReferent referent) {
        ...
        if (Liveness.DEBUG_MODE_ENABLED) {
            Liveness.log.info().append("LivenessDebug: ").append(getReferentDescription()).append(" managing ").append(referent.getReferentDescription()).endl();
        }
    ...
    /**
     * <p>Attempt to release (destructively when necessary) resources held by this object.  This may render the object
     * unusable for subsequent operations.  Implementations should be sure to call super.destroy().
     * <p>This is intended to only ever be used as a side effect of decreasing the reference count to 0.
     */
