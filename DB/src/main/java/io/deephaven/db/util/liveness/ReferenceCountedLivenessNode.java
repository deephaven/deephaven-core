...
abstract class ReferenceCountedLivenessNode extends ReferenceCounted implements LivenessNode {
    ...
    /**
     * @param enforceStrongReachability Whether this {@link LivenessManager} should maintain strong
     *        references to its referents
     */
    @SuppressWarnings("WeakerAccess") // Needed in order to deserialize Serializable subclass
                                      // instances
    ...
    /**
     * Package-private for {@link java.io.Serializable} sub-classes to use in
     * <code>readObject</code> <em>only</em>. Public to allow unit tests in another package to work
     * around mock issues where the constructor is never invoked.
     */
    @VisibleForTesting
    public final void initializeTransientFieldsForLiveness() {
        ...
        if (Liveness.DEBUG_MODE_ENABLED) {
            Liveness.log.info().append("LivenessDebug: Created tracker ")
                .append(Utils.REFERENT_FORMATTER, tracker).append(" for ")
    ...
    public final void dropReference() {
        ...
        if (Liveness.DEBUG_MODE_ENABLED) {
            Liveness.log.info().append("LivenessDebug: Releasing ")
                .append(Utils.REFERENT_FORMATTER, this).endl();
        }
    ...
    public final boolean tryManage(@NotNull final LivenessReferent referent) {
        ...
        if (Liveness.DEBUG_MODE_ENABLED) {
            Liveness.log.info().append("LivenessDebug: ").append(getReferentDescription())
                .append(" managing ").append(referent.getReferentDescription()).endl();
        }
    ...
    /**
     * <p>
     * Attempt to release (destructively when necessary) resources held by this object. This may
     * render the object unusable for subsequent operations. Implementations should be sure to call
     * super.destroy().
     * <p>
     * This is intended to only ever be used as a side effect of decreasing the reference count to
     * 0.
     */
    ...
    protected final void onReferenceCountAtZero() {
        if (Liveness.REFERENCE_TRACKING_DISABLED) {
            throw new IllegalStateException("Reference count on " + this
                + " reached zero while liveness reference tracking is disabled");
        }
        try {
            destroy();
        } catch (Exception e) {
            Liveness.log.warn().append("Exception while destroying ")
