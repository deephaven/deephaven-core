...
/**
 * <p>N-Tuple key class backed by an array of elements.
 */
public class ArrayTuple implements Comparable<ArrayTuple>, Externalizable, StreamingExternalizable, CanonicalizableTuple<ArrayTuple> {
    ...
    public final int compareTo(@NotNull final ArrayTuple other) {
        ...
        for (int ei = 0; ei < thisLength; ++ei) {
            final int comparison = DBLanguageFunctionUtil.compareTo((Comparable)elements[ei], (Comparable)other.elements[ei]);
            if (comparison != 0) {
                return comparison;
            }
        }
