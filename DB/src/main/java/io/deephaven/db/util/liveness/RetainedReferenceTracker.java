 ...
 * {@link LivenessReferent}s, in order to guarantee that they will each have their references
 * dropped exactly once via an idempotent cleanup process.
 * <p>
 * This cleanup process is initiated one of two ways:
 * <ol>
 * <li>The manager invokes it directly via {@link #ensureReferencesDropped()} because it is
 * releasing all of its retained references.</li>
 * <li>A {@link io.deephaven.util.reference.CleanupReferenceProcessor} or similar code invokes
 * {@link #cleanup()} after the manager is garbage-collected.</li>
 * </ol>
 */
final class RetainedReferenceTracker<TYPE extends LivenessManager>
    ...
    @SuppressWarnings("FieldMayBeFinal") // We are using an AtomicIntegerFieldUpdater (via
                                         // reflection) to change this
     ...
     * @param enforceStrongReachability Whether this tracker should maintain strong references to
     *        the added referents
     */
    RetainedReferenceTracker(@NotNull final TYPE manager, final boolean enforceStrongReachability) {
        ...
        if (Liveness.DEBUG_MODE_ENABLED) {
            ProcessEnvironment.getDefaultLog().info().append("Creating ")
                .append(Utils.REFERENT_FORMATTER, this).append(" at ")
     ...
     * @throws LivenessStateException If {@link #cleanup()} or {@link #ensureReferencesDropped()}
     *         has already been invoked
     */
    synchronized final void addReference(@NotNull final LivenessReferent referent)
    ...
    /**
     * <p>
     * Remove at most one existing reference to referent from this tracker, so that it will no
     * longer be dropped on {@link #cleanup()} or {@link #ensureReferencesDropped()}, and drop it
     * immediately.
     * <p>
     * This is not permitted if {@link #cleanup()} or {@link #ensureReferencesDropped()} has already
     * been invoked.
     *
     ...
     *         has already been invoked
     */
    synchronized final void dropReference(@NotNull final LivenessReferent referent)
        throws LivenessStateException {
        checkOutstanding();
        impl.drop(referent);
    }
    ...
    /**
     * <p>
     * Remove at most one existing reference to each input referent from this tracker, so that it
     * will no longer be dropped on {@link #cleanup()} or {@link #ensureReferencesDropped()}, and
     * drop it immediately.
     * <p>
     * This is not permitted if {@link #cleanup()} or {@link #ensureReferencesDropped()} has already
     * been invoked.
     ...
     *         has already been invoked
     */
    synchronized final void dropReferences(
    ...
    /**
     * <p>
     * Move all {@link LivenessReferent}s previously added to this tracker to other, which becomes
    ...
    /**
     * <p>
     * Remove all {@link LivenessReferent}s previously added to this tracker, unless they have been
     * transferred, without dropping them. Uses to make references "permanent".
     * <p>
     * This is not permitted if {@link #cleanup()} or {@link #ensureReferencesDropped()} has already
     * been invoked.
     *
    ...
    private void checkOutstanding() {
        if (outstandingState == NOT_OUTSTANDING) {
            throw new LivenessStateException("RetainedReferenceTracker " + this
                + " has already performed cleanup for manager " + get());
        }
    }
    ...
    /**
     * <p>
     * Initiate the idempotent cleanup process. This will drop all retained references if their
    ...
    private void ensureReferencesDroppedInternal(final boolean onCleanup) {
        ...
        if (Liveness.DEBUG_MODE_ENABLED || (onCleanup && Liveness.CLEANUP_LOG_ENABLED)) {
            Liveness.log.info().append("LivenessDebug: Ensuring references dropped ")
                .append(onCleanup ? "(on cleanup) " : "").append("for ")
                .append(Utils.REFERENT_FORMATTER, this).endl();
        }
        outstandingCount.decrementAndGet();

        Queue<WeakReference<? extends LivenessReferent>> pendingDropReferences =
            tlPendingDropReferences.get();
        ...
        if (processDrops) {
            ...
            if (savedQueueReference == null
                || (pendingDropReferences = savedQueueReference.get()) == null) {
                tlSavedQueueReference
                    .set(new SoftReference<>(pendingDropReferences = new ArrayDeque<>()));
            }
    ...
    /**
     * <p>
     * Get the number of outstanding trackers (instances of RetainedReferenceTracker that have not
     * had their {@link #cleanup()} or {@link #ensureReferencesDropped()} method called).
     * <p>
     * Note that this number represents the liveness system's current knowledge of the number of
     * live references in the system.
     *
    ...
    private static final class WeakImpl implements Impl {
        ...
        private final List<WeakReference<? extends LivenessReferent>> retainedReferences =
            new ArrayList<>();
        ...
        public void drop(@NotNull final LivenessReferent referent) {
            for (int rrLast = retainedReferences.size() - 1, rri = 0; rri <= rrLast;) {
                final WeakReference<? extends LivenessReferent> retainedReference =
                    retainedReferences.get(rri);
        ...
        public void drop(@NotNull final Collection<? extends LivenessReferent> referents) {
            ...
            for (int rrLast = retainedReferences.size() - 1, rri = 0; rri <= rrLast;) {
                ...
                if (found) {
                    ...
                    if (referent != null) { // Probably unnecessary, unless the referents collection
                                            // is engaged in some reference trickery internally, but
    ...
    private static final class StrongImpl implements Impl {
        ...
        private static final RetentionCache<LivenessReferent> permanentReferences =
            new RetentionCache<>();
        ...
        @Override
        public void makePermanent() {
            // See LivenessScope.transferTo: This is currently unreachable code, but implemented for
            // completeness
