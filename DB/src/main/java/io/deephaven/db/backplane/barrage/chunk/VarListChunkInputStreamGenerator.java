...
import io.deephaven.db.backplane.barrage.chunk.array.ArrayExpansionKernel;
...
public class VarListChunkInputStreamGenerator<T> extends BaseChunkInputStreamGenerator<WritableObjectChunk<T, Attributes.Values>> {
    ...
    public Object asNativeArray(final @Nullable Index subset) {
        ...
        if (subset == null || subset.size() == chunk.size()) {
            ...
            result = (T[]) Array.newInstance(type, chunk.size());
            chunk.copyToTypedArray(0, result, 0, chunk.size());
        } else {
            //noinspection unchecked
            result = (T[])Array.newInstance(type, subset.intSize());
            final MutableInt offset = new MutableInt();
            subset.forAllLongs(i -> {
                result[offset.intValue()] = chunk.get((int)i);
                offset.setValue(offset.intValue() + 1);
            });
        }
        return result;
    }
