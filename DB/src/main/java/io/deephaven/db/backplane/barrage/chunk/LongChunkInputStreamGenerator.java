...
public class LongChunkInputStreamGenerator extends BaseChunkInputStreamGenerator<WritableLongChunk<Attributes.Values>> {
    ...
    private class LongChunkInputStream extends BaseChunkInputStream {
        private LongChunkInputStream(final Options options, final Index subset) {
    ...
    static Chunk<Attributes.Values> extractChunkFromInputStream(
        ...
        final WritableLongChunk<Attributes.Values> chunk = WritableLongChunk.makeWritableChunk(nodeInfo.numElements);
    ...
    public Object asNativeArray(final @Nullable Index subset) {
        ...
        if (subset == null || subset.size() == chunk.size()) {
            result = new long[chunk.size()];
            chunk.copyToTypedArray(0, result, 0, chunk.size());
        } else {
            result = new long[subset.intSize()];
            final WritableLongChunk<Attributes.Values> out = WritableLongChunk.writableChunkWrap(result);
            out.setSize(0);
