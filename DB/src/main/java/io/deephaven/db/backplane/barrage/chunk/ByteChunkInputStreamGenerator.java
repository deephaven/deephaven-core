...
public class ByteChunkInputStreamGenerator extends BaseChunkInputStreamGenerator<WritableByteChunk<Attributes.Values>> {
    private static final String DEBUG_NAME = "ByteChunkInputStreamGenerator";

    ByteChunkInputStreamGenerator(final WritableByteChunk<Attributes.Values> chunk, final int elementSize) {
    ...
    public Object asNativeArray(final @Nullable Index subset) {
        ...
        if (subset == null || subset.size() == chunk.size()) {
            result = new byte[chunk.size()];
            chunk.copyToTypedArray(0, result, 0, chunk.size());
        } else {
            result = new byte[subset.intSize()];
            final WritableByteChunk<Attributes.Values> out = WritableByteChunk.writableChunkWrap(result);
            out.setSize(0);
