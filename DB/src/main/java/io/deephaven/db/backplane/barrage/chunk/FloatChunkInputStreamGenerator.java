...
public class FloatChunkInputStreamGenerator extends BaseChunkInputStreamGenerator<WritableFloatChunk<Attributes.Values>> {
    private static final String DEBUG_NAME = "FloatChunkInputStreamGenerator";

    FloatChunkInputStreamGenerator(final WritableFloatChunk<Attributes.Values> chunk, final int elementSize) {
    ...
    private class FloatChunkInputStream extends BaseChunkInputStream {
        private FloatChunkInputStream(final Options options, final Index subset) {
    ...
    public Object asNativeArray(final @Nullable Index subset) {
        ...
        if (subset == null || subset.size() == chunk.size()) {
            result = new float[chunk.size()];
            chunk.copyToTypedArray(0, result, 0, chunk.size());
        } else {
            result = new float[subset.intSize()];
            final WritableFloatChunk<Attributes.Values> out = WritableFloatChunk.writableChunkWrap(result);
            out.setSize(0);
