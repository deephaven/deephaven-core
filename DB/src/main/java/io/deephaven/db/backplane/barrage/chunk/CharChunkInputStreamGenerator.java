...
public class CharChunkInputStreamGenerator extends BaseChunkInputStreamGenerator<WritableCharChunk<Attributes.Values>> {
    ...
    private class CharChunkInputStream extends BaseChunkInputStream {
        private CharChunkInputStream(final Options options, final Index subset) {
    ...
    public Object asNativeArray(final @Nullable Index subset) {
        ...
        if (subset == null || subset.size() == chunk.size()) {
            result = new char[chunk.size()];
            chunk.copyToTypedArray(0, result, 0, chunk.size());
        } else {
            result = new char[subset.intSize()];
            final WritableCharChunk<Attributes.Values> out = WritableCharChunk.writableChunkWrap(result);
            out.setSize(0);
