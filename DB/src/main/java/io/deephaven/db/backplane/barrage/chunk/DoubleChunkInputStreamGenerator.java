...
public class DoubleChunkInputStreamGenerator extends BaseChunkInputStreamGenerator<WritableDoubleChunk<Attributes.Values>> {
    private static final String DEBUG_NAME = "DoubleChunkInputStreamGenerator";

    DoubleChunkInputStreamGenerator(final WritableDoubleChunk<Attributes.Values> chunk, final int elementSize) {
    ...
    private class DoubleChunkInputStream extends BaseChunkInputStream {
        private DoubleChunkInputStream(final Options options, final Index subset) {
    ...
    static Chunk<Attributes.Values> extractChunkFromInputStream(
        ...
        final WritableDoubleChunk<Attributes.Values> chunk = WritableDoubleChunk.makeWritableChunk(nodeInfo.numElements);
    ...
    public Object asNativeArray(final @Nullable Index subset) {
        ...
        if (subset == null || subset.size() == chunk.size()) {
            result = new double[chunk.size()];
            chunk.copyToTypedArray(0, result, 0, chunk.size());
        } else {
            result = new double[subset.intSize()];
            final WritableDoubleChunk<Attributes.Values> out = WritableDoubleChunk.writableChunkWrap(result);
            out.setSize(0);
