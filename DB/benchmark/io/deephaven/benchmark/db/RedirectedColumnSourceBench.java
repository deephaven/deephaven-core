...
public class RedirectedColumnSourceBench extends RedirectionBenchBase {
    ...
    protected QueryData getQuery() {
        ...
        switch (fillColsType) {
            case "D":
                builderAddColumn = (final String col) ->
                        builder.addColumn(BenchmarkTools.numberCol(col, double.class, -10e6, 10e6));
                chunk = WritableDoubleChunk.makeWritableChunk(chunkCapacity);
                nSelectCols = nFillCols + 1;
                break;
            case "F":
                builderAddColumn = (final String col) ->
                        builder.addColumn(BenchmarkTools.numberCol(col, float.class, -10e6f, 10e6f));
                chunk = WritableFloatChunk.makeWritableChunk(chunkCapacity);
                nSelectCols = nFillCols + 1;
                break;
            case "L":
                builderAddColumn = (final String col) ->
                        builder.addColumn(BenchmarkTools.numberCol(col, long.class, -10_000_000, 10_000_000));
            ...
            case "S":
                builderAddColumn = (final String col) ->
                        builder.addColumn(BenchmarkTools.stringCol(col, 4096,4, 8, 0xDEADBEEF));
        ...
        final Table live;
        if (doSelect) {
            live = LiveTableMonitor.DEFAULT.exclusiveLock().computeLocked(() -> t.where(incrementalReleaseFilter).select(selectCols).sort(sortCol));
        } else {
