 ...
 *
 * <p>Unfortunately, the JVM doesn't seem to natively provide access to how much data has been
 * allocated, so we are using the GC before and after values as a proxy for that.
 * (The JVM *does* provide it on a per-thread basis though, {@link com.sun.management.ThreadMXBean#getThreadAllocatedBytes(long)}.)
 *
 * <p>This implementation relies on {@link GarbageCollectionNotificationInfo#GARBAGE_COLLECTION_NOTIFICATION}
 * to get stats around the memory usage before and after GCs. This means that these statistics are
 * somewhat less useful when GC events are sparse.
 */
class GcEventStats implements NotificationListener {
  ...
  private synchronized void handleGCInfo(GarbageCollectionNotificationInfo info) {
    final long beforeGc = info.getGcInfo().getMemoryUsageBeforeGc().values().stream().mapToLong(MemoryUsage::getUsed).sum();
    final long afterGc = info.getGcInfo().getMemoryUsageAfterGc().values().stream().mapToLong(MemoryUsage::getUsed).sum();
