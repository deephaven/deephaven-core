...
public class BufferUtil {
    ...
    /**
     * <p>
     * Allocate a new ByteBuffer larger than the supplied buffer. This is generally used when dynamically resizing an
     * output buffer.
     * <p>
    ...
    public static ByteBuffer reallocateBuffer(@NotNull final ByteBuffer buffer, final int minimumSize) {
        ...
        final ByteBuffer newBuffer =
                buffer.isDirect() ? ByteBuffer.allocateDirect(newCapacity) : ByteBuffer.allocate(newCapacity);
     ...
     * buffer, and might be the buffer returned. The new buffer's limit will be unchanged if the buffer is not
     * reallocated, and equal to the new capacity if it is reallocated. The new buffer will be allocated with
     * allocateDirect() if the original buffer is direct, else with allocate().
     *
    ...
    public static ByteBuffer ensureSpace(ByteBuffer dataBuffer, int requiredSize) {
        ...
        if (remainingCapacity < requiredSize) {
            final int newCapacity = Math.max((dataBuffer.capacity() > 1024 * 1024) ? dataBuffer.capacity() + 1024 * 1024
                    : dataBuffer.capacity() * 2, dataBuffer.capacity() + (requiredSize - remainingCapacity));
            final ByteBuffer newBuffer =
                    dataBuffer.isDirect() ? ByteBuffer.allocateDirect(newCapacity) : ByteBuffer.allocate(newCapacity);
            dataBuffer.flip();
            newBuffer.put(dataBuffer);
            dataBuffer = newBuffer;
        }
        Require.geq(dataBuffer.capacity() - dataBuffer.position(), "dataBuffer.capacity() - dataBuffer.position()",
                requiredSize, "requiredSize");
        return dataBuffer;
    }

    /**
     * Encode the given string as UTF_8 and write the size and data to the given buffer. The buffer might be reallocated
     * using {@link BufferUtil#ensureSpace} if necessary.
     *
