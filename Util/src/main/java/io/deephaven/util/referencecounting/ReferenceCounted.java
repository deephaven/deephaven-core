...
public abstract class ReferenceCounted implements LogOutputAppendable, Serializable {
    ...
    /**
     * Field updater for referenceCount, so we can avoid creating an {@link java.util.concurrent.atomic.AtomicInteger} for each
     * instance.
     */
    ...
    public final boolean tryIncrementReferenceCount() {
        ...
        while ((currentReferenceCount = getCurrentReferenceCount()) != TERMINAL_ZERO_VALUE) {
            ...
            if (tryUpdateReferenceCount(currentReferenceCount, currentReferenceCount == INITIAL_ZERO_VALUE ? ONE_VALUE : currentReferenceCount + 1)) {
                return true;
            }
        }
        return false;
    ...
    public final boolean tryDecrementReferenceCount() {
        int currentReferenceCount;
        while ((currentReferenceCount = getCurrentReferenceCount()) != TERMINAL_ZERO_VALUE && currentReferenceCount != INITIAL_ZERO_VALUE) {
