 ...
 * <li>If an attempted execution exceeds the maximum number of allowed executor threads, a new Thread will be created dynamically instead of generating an exception</li>
 * </ul>
...
public class ExpandingThreadPoolExecutorFactory {
    ...
    /**
     * Class to handle rejection events from a ThreadPoolExecutor by creating a new Thread to run the task,
    ...
    private static class RejectedExecutionPolicy implements RejectedExecutionHandler, LogOutputAppendable {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor e) {
            if (!e.isShutdown()) {
                ...
                log.warn().append("Executor has run out of threads for ").append(this).append(", creating new thread ").append(newThreadName).endl();
    ...
    /**
     * Create a {@link ThreadPoolExecutor} with the characteristics defined in {@link ExpandingThreadPoolExecutorFactory}.
     *
     * @param log                     a Logger to log messages
     * @param corePoolSize            the core pool size (the executor will use this value for the initial core and maximum pool sizes)
     ...
     * @param dynamicThreadNamePrefix the prefix for dynamic (overflow) threads created when the maximum number of pool threads is exceeded
     */
