...
public class FileHelper {
    ...
    /**
     * Augment {@link FileUtils#deleteRecursivelyOnNFS(File)} with an exclude pattern. Files matching the pattern will
    ...
    public static boolean deleteRecursivelyOnNFS(File file, Pattern excludePattern) {
        return deleteRecursivelyOnNFS(new File(file.getParentFile(), '.' + file.getName() + ".trash"), file,
                excludePattern);
    }

    /**
     * Augment {@link FileUtils#deleteRecursivelyOnNFS(File, File)} with an exclude pattern. Files matching the pattern
    ...
    public static boolean deleteRecursivelyOnNFS(File trashFile, File fileToBeDeleted,
        ...
        if (fileToBeDeleted.isDirectory()) {
            ...
            if (!fileToBeDeleted.renameTo(trashFile)) {
                throw new FileDeletionException("Failed to move file " + fileToBeDeleted.getAbsolutePath()
                        + " to temporary location " + trashFile.getAbsolutePath());
            }
            if (!trashFile.delete()) {
                throw new FileDeletionException("Failed to delete temporary location " + trashFile.getAbsolutePath()
