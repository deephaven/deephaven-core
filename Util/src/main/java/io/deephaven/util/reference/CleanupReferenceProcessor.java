...
public class CleanupReferenceProcessor {
    ...
    /**
     * <p>Get the reference queue for this cleaner.
     * <p>On the first call after construction or {@link #resetForUnitTests()}, this method initializes the instance as
     * a side effect. Initialization entails:
     * <ol>
     ...
     * </ol>
     *
     * @return The {@link ReferenceQueue} constructed in the most recent initialization of this {@link CleanupReferenceProcessor}
     * instance
     */
        ...
        if ((localQueue = referenceQueue) == null) {
            synchronized (this) {
                if ((localQueue = referenceQueue) == null) {
                    ...
                    final Thread cleanerThread = new Thread(new DrainQueue(localQueue), "CleanupReferenceProcessor-" + name + "-drainingThread");
     ...
     * queue.
     * Results in eventual termination of the daemon thread that may have been draining the existing queue.
     */
    @TestUseOnly
                ...
                if (reference instanceof CleanupReference) {
                    try {
                        if (LOG_CLEANED_REFERENCES) {
                            log.info().append("CleanupReferenceProcessor-").append(name).append(", cleaning ").append(Utils.REFERENT_FORMATTER, reference).endl();
                        }
