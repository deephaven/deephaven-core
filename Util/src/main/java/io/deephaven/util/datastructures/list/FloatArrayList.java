...
public class FloatArrayList {
    private float[] arr = ArrayTypeUtils.EMPTY_FLOAT_ARRAY;
    private int size;
     ...
     */
    public void removeFloat(int position) {
    ...
    public boolean equals(Object other) {
        ...
        final FloatArrayList that = (FloatArrayList) other;
        if (size != that.size) {
            return false;
        }
    ...
    public int hashCode() {
        int result = 1;
        for (int i = 0; i < size; ++i) {
            result = 31 * result + FloatComparisons.hashCode(arr[i]);
        }
        return result;
    }
}
