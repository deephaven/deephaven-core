...
public class TestSegmentedSoftPool {
    ...
    public void testWithoutFactory() {
        ...
        IntStream.range(0, 100).boxed().sorted(Comparator.reverseOrder()).forEach(II -> TestCase.assertEquals(II, pool.take()));
        IntStream.range(100, 400).boxed().forEach(pool::give);
        IntStream.range(100, 400).boxed().sorted(Comparator.reverseOrder()).forEach(II -> TestCase.assertEquals(II, pool.take()));
    }
    ...
    public void testForGC() {
        ...
        Assume.assumeTrue("Skipping testForGC, as it is very long under most conditions, requires specific JVM parameters, and isn't really a unit test", false);

        // With the following settings, cleanup should begin in 30 seconds and converge on 72 items remaining: -Xmx4g -XX:SoftRefLRUPolicyMSPerMB=10
        ...
        while (OUTSTANDING_INSTANCES.cardinality() != 72 || System.nanoTime() - nanosLastCleanup < 10_000_000_000L) {
            // Try to simulate some usage of the pool - we want soft refs that have been followed more recently than others.
            int lengthTaken = 0;
            ...
            if (lengthTaken != inUse.length) {
                throw new IllegalStateException("Pool has been cleaned up more than expected, took " + lengthTaken + " out of " + inUse.length);
            }
            ...
            if (dequeuedAny) {
                ...
                synchronized (OUTSTANDING_INSTANCES) {
                    ...
                    System.err.println("elapsedSeconds=" + elapsedSeconds + ", firstRemaining=" + OUTSTANDING_INSTANCES.nextSetBit(0) + ", remaining=" + OUTSTANDING_INSTANCES.cardinality() + ", allocated=" + OUTSTANDING_INSTANCES.length());
                }
            }
        }
    }
