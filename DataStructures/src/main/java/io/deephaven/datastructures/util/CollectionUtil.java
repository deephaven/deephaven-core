...
public class CollectionUtil {
    ...
    //----------------------------------------------------------------
    @SuppressWarnings({"unchecked"})
    public static <K, V> Map<K, V> mapFromArray(Class<K> typeK, Class<V> typeV, final boolean allowDuplicateKeys, Object... data) {
        ...
        for (int nIndex=0; nIndex<data.length; nIndex+=2) {
            ...
            if(!allowDuplicateKeys) {
                Require.requirement(false == map.containsKey(key), "false==map.containsKey(data[nIndex])", key, "data[nIndex]");
            }
    ...
    //----------------------------------------------------------------
    /** Returns an empty {@link HashMap} with a big enough capacity
    ...
    //----------------------------------------------------------------
    /** Returns an empty {@link LinkedHashMap} with a big enough
    ...
    //----------------------------------------------------------------
    /** Returns an empty {@link HashSet} with a big enough capacity
     * such that the given number of entries can be added without
     * resizing. */
    ...
    //----------------------------------------------------------------
    /** Returns an empty {@link LinkedHashSet} with a big enough capacity
     * such that the given number of entries can be added without
     * resizing. */
    ...
    private static class UniversalSet<T> implements Set<T>, Serializable {
        ...
        private Object readResolve() { return UNIVERSAL_SET; }
