...
def learn(table=None, model_func=None, inputs=[], outputs=[], batch_size = None):
    ...
    jpy.get_type("io.deephaven.engine.tables.select.QueryScope").addParam("__computer", _Computer_(table, model_func, [input.input for input in inputs], batch_size))
    ...
    if not outputs == None:
        ...
        jpy.get_type("io.deephaven.engine.tables.select.QueryScope").addParam("__scatterer", __scatterer)
