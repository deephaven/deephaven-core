 ...
 *
 * @param <K> Key type (either some object or a {@code List<?>}). If {@code K} is a {@code List<?>}, then
 *            a {@link #tupleSource} appropriate for the {@link #keyColumns}' types will be used to generate
 *            keys for the inner {@link #toMapListener}. If {@code K} is <b>any type except</b> a {@code List<?>},
 *            then {@code K} will be used directly as the {@code toMapListener}'s key type (and must match the
 *            data type in the {@code keyColumn}).
 * @param <T> Record type
 */
public class KeyedRecordAdapter<K, T> {
     ...
     * retrieved through the ToMapListener.
     */
    @NotNull
    private final ToMapListener<Object, Void> toMapListener;

    /**
     * A source of tuples to use for composite map keys. Will be {@code null} if
     * there is only one {@link #keyColumns key column}.
     */
    @Nullable
    private final TupleSource<?> tupleSource;

    /**
     * Function used when converting user-friendly keys to the ToMapListener's keys. The keys must be
     * either an {@code Object} (if there is one {@link #keyColumns key column}) or a {@code List<?>} (if there
    ...
    /**
     * Function convert a list of user-friendly keys to a list of the ToMapListener's keys. The user-friendly
     * keys must be either a {@code List<K>} (if there is one {@link #keyColumns key column}) or a
     * {@code List<List<?>>} (if there are multiple key columns).
     * <p>
     * This function will call {@link #dataKeyToMapKey}; if there are multiple key columns, the keys must
     ...
     * {@link #keyColumns}). This is separate from singleRowRecordAdapter/multiRowRecordAdapter because
     * the keys do not need to be retrieved from the table (since any key used to retrieve data is already
     * in memory).
     */
    @Nullable
     ...
     * @param rowRecordAdapterDescriptor The descriptor used to build the record adapter to read data from {@code sourceTable} into records
     * @param keyColumns                 The key columns to use when retrieving data
     */
    protected KeyedRecordAdapter(@NotNull Table sourceTable, @NotNull RecordAdapterDescriptor<T> rowRecordAdapterDescriptor, @NotNull String... keyColumns) {
        ...
        // Create a record adapter that excludes the key columns, if they are present, (since all available keys will already be in memory)
        final RecordAdapterDescriptorBuilder<T> recordAdapterDescriptorBuilderNoKeys = RecordAdapterDescriptorBuilder.create(rowRecordAdapterDescriptor);
        ...
        for (String keyColName : keyColumns) {
            final RecordUpdater<T, ?> keyColRecordUpdater = recordAdapterDescriptorBuilderNoKeys.removeColumn(keyColName);
        ...
        if (isSingleKeyCol) {
            /*
             the contents of the dataKeys list are the map keys themselves.
             dataKeys is the list of keys that we want to look up in the ToMapListener.
            */
            ...
            //noinspection unchecked
            final RecordUpdater<T, K> keyColRecordUpdater = (RecordUpdater<T, K>) keyColRecordUpdaters.get(keyColumns[0]);
            if (keyColRecordUpdater != null) {
                ...
                if (!expectedType.isAssignableFrom(keyColumnType)) {
                    throw new IllegalArgumentException("Key column 0: Expected type " + expectedType + ", instead found type " +
            ...
            for (int i = 0; i < keyColumns.length; i++) {
                ...
                if (keyColRecordUpdater != null) {
                    ...
                    if (!expectedType.isAssignableFrom(keyColumnTypes[i])) {
                        throw new IllegalArgumentException("Key column " + i + ": Expected type " + expectedType + ", instead found type " +
            ...
            if (keyDataUpdaters.isEmpty()) {
                ...
                recordKeyDataUpdater = (r, k) -> keyDataUpdaters.forEach(recordKeyUpdater -> recordKeyUpdater.accept(r, (List<?>) k));
            }
        }
    }
     ...
     */
    private static <T> BiConsumer<T, List<?>> getCompositeKeyRecordUpdater(int keyIndex, RecordUpdater<T, ?> keyColRecordUpdater) {
        final Class<?> type = keyColRecordUpdater.getSourceType();
        if (byte.class.equals(type)) {
            return (T record, List<?> v) -> keyColRecordUpdater.updateRecordWithByte(record, TypeUtils.unbox((Byte) v.get(keyIndex)));
        } else if (char.class.equals(type)) {
            return (T record, List<?> v) -> keyColRecordUpdater.updateRecordWithChar(record, TypeUtils.unbox((Character) v.get(keyIndex)));
        } else if (short.class.equals(type)) {
            return (T record, List<?> v) -> keyColRecordUpdater.updateRecordWithShort(record, TypeUtils.unbox((Short) v.get(keyIndex)));
        } else if (int.class.equals(type)) {
            return (T record, List<?> v) -> keyColRecordUpdater.updateRecordWithInt(record, TypeUtils.unbox((Integer) v.get(keyIndex)));
        } else if (float.class.equals(type)) {
            return (T record, List<?> v) -> keyColRecordUpdater.updateRecordWithFloat(record, TypeUtils.unbox((Float) v.get(keyIndex)));
        } else if (long.class.equals(type)) {
            return (T record, List<?> v) -> keyColRecordUpdater.updateRecordWithLong(record, TypeUtils.unbox((Long) v.get(keyIndex)));
        } else if (double.class.equals(type)) {
            return (T record, List<?> v) -> keyColRecordUpdater.updateRecordWithDouble(record, TypeUtils.unbox((Double) v.get(keyIndex)));
        } else {
            //noinspection unchecked
            return (T record, List<?> v) -> ((RecordUpdater<T, Object>) keyColRecordUpdater).updateRecord(record, v.get(keyIndex));
        }
    }

    public static KeyedRecordAdapter<List<?>, Map<String, Object>> makeRecordAdapterCompositeKey(Table sourceTable, List<String> valueColumns, String... keyColumns) {
        ...
        final RecordAdapterDescriptor<Map<String, Object>> genericRecordAdapterDescriptor = RecordAdapterDescriptor.createGenericRecordAdapterDescriptor(sourceTable, allCols);
        return new KeyedRecordAdapter<>(sourceTable, genericRecordAdapterDescriptor, keyColumns);
    }

    public static <T> KeyedRecordAdapter<List<?>, T> makeRecordAdapterCompositeKey(Table sourceTable, RecordAdapterDescriptor<T> recordAdapterDescriptor, String... keyColumns) {
        return new KeyedRecordAdapter<>(sourceTable, recordAdapterDescriptor, keyColumns);
    }

    public static <K, T> KeyedRecordAdapter<K, T> makeKeyedRecordAdapterSimpleKey(Table sourceTable, RecordAdapterDescriptor<T> recordAdapterDescriptor, String keyColumn, Class<K> keyColType) {
    ...
    /**
     * Retrieve records of type {@code T} corresponding to the data in the table for the given
     * {@code dataKeys}.
     */
     ...
     * @param usePrevOrig                  Whether to look at prev values when retrieving data. This is from ConstructSnapshot.
    ...
    private boolean retrieveDataMultipleKeys(
        ...
        // TODO: probably we should just return false here, since it means LTM has ticked and snapshot will not be consistent?
