...
- **`withSerial`**: Rows _within one_ expression are processed sequentially (row 0, then row 1, etc.). Other expressions can still run at the same time.
...
When shared state is involved, you often need both: `withSerial` to protect row-level access to the shared state, and a barrier to ensure one expression is completely done before the other starts.
