...
A table generated by the `tableGenerator` function. The result ticks when the call supplies a refresh trigger: one or more `sourceTables`, a positive `refreshIntervalMs`, or a `spec` carrying dependencies or a refresh interval. Without a trigger, the `tableGenerator` runs once and the result is static.
