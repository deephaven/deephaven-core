...
| `Map`                                 | `java.util.Map`                                                                 | —                                                                              | Map types require that the [Arrow schema to be specified](#set-the-flight-schema). Cells are accumulated into `LinkedHashMap`s; it's not possible to pick another collector at this time.                                           |
| `List` / `ListView` / `FixedSizeList` | `T[]` - a native Java array of the inner field                                  | Supports conversion to Deephaven internal `Vector` types such as `LongVector`. | The component type defaults to the inner field's type; set the `deephaven:componentType` metadata key only to override it. Fixed-size list wire formats will be truncated and/or null-padded to match the data as best as possible. |
...
Filters, sorts, `reverse`, `flatten`, `firstBy`, and `lastBy` keep the schema attribute on the result, but other operations, such as `update`, `view`, joins, and most aggregations, drop it, even for column types that are completely preserved. Be sure to attach the modified schema if you want to preserve the original wire types.
...
Normally, you would need to set the `Table.BARRAGE_SCHEMA_ATTRIBUTE` table attribute to configure the custom schema for the download. However, Deephaven associates the uploaded schema with a table and uses it for the download as long as we're downloading that table, or a result of an operation that keeps the attribute. Being able to round-trip from a client without setting the custom export schema is a quality-of-life feature of Deephaven.
