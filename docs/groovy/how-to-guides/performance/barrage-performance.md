...
Sender statistics are recorded by the server that publishes the table. Receiver statistics are recorded by the subscribing process, in its own metrics tables, and only when that process runs the Deephaven engine — for example, a Deephaven server that subscribes through a [URI](../use-uris.md), or the Java client. Clients such as `pydeephaven` and the JavaScript client record no receiver statistics.
...
tableId = Integer.toHexString(System.identityHashCode(t))
