...
- **The receiver** records the time it takes to deserialize and apply updates, but only when the receiver runs the Deephaven engine: the Java client, or a Deephaven server that subscribes to a remote table through a [URI](../how-to-guides/use-uris.md). Receiver statistics appear in the receiving process's own metrics tables, not the publisher's. Clients that do not run the engine, such as `pydeephaven` and the JavaScript client, record no receiver statistics.
