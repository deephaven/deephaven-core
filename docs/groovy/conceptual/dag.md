...
t3 = null  // Remove reference to the join result

// Now the DAG only has two tables: t1 -> t2
// The naturalJoin operation is removed because nothing references it
```

When `t3` is set to `null`, there are no references to the result of the [`naturalJoin`](../reference/table-operations/join/natural-join.md), so that table and its associated computations are removed from the DAG. However, `t1` and `t2` remain because they still have active references.
...
```groovy order=t3 test-set=dag-example
...
handle_alert = new InstrumentedTableUpdateListenerAdapter("DiskAlertListener", critical_alerts, false) {
    ...
    void onUpdate(TableUpdate upstream) {
        ...
        if (addedRowSet.isNonempty()) {
            println "ALERT: Disk usage exceeded 90% on ${addedRowSet.size()} server(s)"
            // In production: sendEmail() or postToSlack()
        }
    }
}
```
