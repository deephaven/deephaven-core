...
Deephaven's [`perfmon`](/core/pydoc/code/deephaven.perfmon.html?module-deephaven.perfmon) package contains various methods to create and open performance tables in the Deephaven IDE.

```python order=qpl,upl,qopl,pml,pil,ss,ssl,qptt,qoptt
import deephaven.perfmon as pm

qpl = pm.query_performance_log()
upl = pm.update_performance_log()
upal = pm.update_performance_ancestors_log()
...
<!--TODO: add query_update_performance_map when the [bug fix PR](https://deephaven.atlassian.net/browse/DH-19474) goes in -->
...
qup = pm.query_update_performance(n)
```
...
The Query Operation Performance table contains data on how long each individual operation of a query ([`where`](../../reference/table-operations/filter/where.md), [`update`](../../reference/table-operations/select/update.md), [`natural_join`](../../reference/table-operations/join/natural-join.md), etc., as well as internal functions) takes to execute, and the change in resource consumption while each was executing. The `query_operation_performance` method requires an [evaluation number](#evaluation-numbers) to identify the query you want to analyze.
...
The Server State table contains basic memory, update graph processor, and GC stats for the current engine process, sampled on a periodic basis.

| Column Name             | Data Type | Description                                             |
| ----------------------- | --------- | ------------------------------------------------------- |
| `IntervalStart`         | `Instant` | The beginning of the interval this row represents.      |
| `IntervalSecs`          | `double`  | The length of the interval being reported (in seconds). |
| `IntervalEnd`           | `Instant` | The end of the interval this row represents.            |
| `UsedMemMiB`            | `double`  | The amount of used memory in MiB.                       |
| `AvailMemMiB`           | `double`  | The amount of available memory in MiB.                  |
| `MaxMemMiB`             | `double`  | The maximum amount of memory in MiB.                    |
| `AvailMemRatio`         | `double`  | The ratio of available memory to maximum memory.        |
| `UsedMemRatio`          | `double`  | The ratio of used memory to maximum memory.             |
| `GcTimeRatio`           | `float`   | The ratio of GC time to total time.                     |
| `UGPCycles`             | `long`    | The number of cycles in the update graph processor.     |
| `UGPOnBudgetRatio`      | `float`   | The ratio of on-budget cycles to total cycles.          |
| `UGPCycleMaxSecs`       | `double`  | The maximum cycle time in seconds.                      |
| `UGPCycleMedianSecs`    | `double`  | The median cycle time in seconds.                       |
| `UGPCycleMeanSecs`      | `double`  | The mean cycle time in seconds.                         |
| `UGPCycleP90Secs`       | `double`  | The 90th percentile cycle time in seconds.              |
| `UGPTimeRatio`          | `float`   | The ratio of UGP time to total time.                    |
| `UGPSafePointTimeRatio` | `float`   | The ratio of UGP safe point time to total time.         |
...
An entry from the Ancestors log can be visualized with the [`ancestor_image`](https://docs.deephaven.io/core/pydoc/code/deephaven.perfmon.html#deephaven.perfmon.ancestor_image) method:

```python skip-test
ancestor_viz = pm.ancestor_image([entry_id], upl, upal)
```

> [!NOTE]
> You must have [Graphviz](https://graphviz.org/) installed to produce an ancestors image.
...
![The above `upl_update` table](../../assets/how-to/perf-ratio.png)
...
- [`deephaven.perfmon`](/core/pydoc/code/deephaven.perfmon.html?module-deephaven.perfmon)
