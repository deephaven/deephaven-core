...
When more than one right table row has the same exact match keys and the same closest value in the last match column, `raj` uses the first of those rows in the right table.
