...
`agg.avg`returns an aggregator that computes the average (mean) of values, within an aggregation group, for each input column.
...
In this example, `agg.avg`returns the average value of `Number` as grouped by `X`.
...
In this example, `agg.avg`returns the average value of `Number` (renamed to `Avg`), as grouped by `X`.
...
In this example, `agg.avg`returns the average value of `Number`, as grouped by `X` and `Y`.
...
In this example, `agg.avg`returns the average value of `Number`, and `agg.std` returns the sample standard deviation of `Number`, as grouped by `X`.
...
- [Pydoc](/core/pydoc/code/deephaven.agg.html#deephaven.agg.avg)
