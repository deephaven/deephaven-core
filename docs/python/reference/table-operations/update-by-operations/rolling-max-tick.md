---
title: rolling_max_tick
---

`rolling_max_tick` creates a rolling maximum in an [`update_by`](./updateBy.md) table operation using table ticks as the windowing unit. Ticks are row counts. The rolling maximum can be calculated using forward and/or backward windows.
...
The column(s) to be operated on. These can include expressions to rename the output (e.g., `NewCol = Col`). When left empty, the rolling maximum is calculated for all applicable columns.
...
The following example performs an [`update_by`](./updateBy.md) on the `source` table using three `rolling_max_tick` operations. Each argument uses a different `rev_ticks` and `fwd_ticks` value to show how they affect the output.
...
- [Pydoc](/core/pydoc/code/deephaven.updateby.html#deephaven.updateby.rolling_max_tick)
