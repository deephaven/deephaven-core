...
| `QueryTable.dictionaryForWhereThreshold`                 | 0.25 (double) | The maximum dictionary size, as a fraction of the rows remaining to be filtered, for the dictionary to be used during push-down filtering. |

The two `*ForWhereThreshold` properties are cost heuristics rather than on/off switches. Scanning a data index table or a dictionary costs time proportional to its size, so the engine only does so when that structure is small relative to the rows it can eliminate. With the default `0.25`, a data index or dictionary is used only when it is at most 25% of the rows remaining to be filtered. Rows skipped by this check are not eliminated — they are filtered directly by a later stage. Raise the value toward `1.0` to apply these optimizations to columns with more distinct values, or set it to `0` to effectively disable the technique.
