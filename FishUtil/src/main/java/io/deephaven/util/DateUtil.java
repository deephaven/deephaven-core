...
//--------------------------------------------------------------------
/**
 * Useful methods for working with dates. Not for use in the
 * critical path.
 */
public class DateUtil {
    ...
    /** An easy way to get the OS-specific directory name component
     * separator. */
    private static final String DIR_SEP=File.separator;

    /** The "local" time zone. We make it explicit for testing
     * purposes. */
    ...
    //----------------------------------------------------------------
    /** Converts the given date (local timezone) to a string in
     * MMDD format. */
    ...
    //----------------------------------------------------------------
    /** Converts the given integer in YYYYMMDD format to a string in
     * YYYY/YYYYMM/YYYYMMDD format. */
    ...
    //----------------------------------------------------------------
    /** Gets the download path, in
     * [DownloadBaseDir]/sDataSubdir/YYYY/YYYYMM/YYYYMMDD
     * format given a date (local timezone). */
    ...
    /** Gets the download path, in
     * [DownloadBaseDir]/sDataSubdir/YYYY/YYYYMM/YYYYMMDD
     * format given an integer in YYYYMMDD format. */
    public static String getYyyymmddIntDownloadPath(PropertyFile configuration, String sDataSubdir, int nDateYyyymmdd) {
        Require.nonempty(sDataSubdir, "sDataSubdir");
        return configuration.getProperty("DownloadBaseDir")+DIR_SEP+sDataSubdir+DIR_SEP+getYyyymmddIntAsPath(nDateYyyymmdd);
    }
    ...
    //----------------------------------------------------------------
    /** Gets a date object representing the next day at the same hour
     * (which may not be exactly 24 hours in the future). */
    ...
    //----------------------------------------------------------------
    /** Subtracts zero or more 24hr periods from the given date until
     * the day of week for the resulting date (local timezone) is a
     * valid day according to the mask. If the strict flag is true,
     * optional days are not considered valid.
     * <P> See {@link #validateDayOfWeekMask}. */
    ...
    //----------------------------------------------------------------
    /** Adds one or more 24hr periods from the given date until
     * the day of week for the resulting date (local timezone) is a
     * valid day according to the mask. If the strict flag is true,
     * optional days are not considered valid.
     * <P> See {@link #validateDayOfWeekMask}. */
    ...
    //----------------------------------------------------------------
    /** Returns the validity flag from the mask for the given date
     * (local timezone).
     * <P> See {@link #validateDayOfWeekMask}. */
    ...
    //----------------------------------------------------------------
    /** Throws a requirement exception if the given day of week mask
     * is not valid. There must be at least one valid day in the mask.
     * If the strict flag is set, optional days are not considered
     * valid.
     * <P> See {@link #DAY_VALID}, {@link #DAY_INVALID},
     * {@link #DAY_OPTIONAL}, {@link #DAYMASK_STRICT},
    ...
    //----------------------------------------------------------------
    /** Gets the day of the week (Su == 0) for the given date (local
     * timezone). */
    ...
    //----------------------------------------------------------------
    /** Gets the current date (local timezone) as an integer, in
     * YYYYMMDD format. */
    ...
    //----------------------------------------------------------------
    /** Gets the given date (local timezone) as an integer, in
     * YYYYMMDD format. */
    ...
    public static String formatYyyymmddStringAsIso(String sDateYyyymmdd) {
        ...
        return sDateYyyymmdd.substring(0, 4)+"-"+sDateYyyymmdd.substring(4, 6)+"-"+sDateYyyymmdd.substring(6, 8);
    }
    ...
    public static String formatYyyymmddStringAsUs(String sDateYyyymmdd) {
        Require.neqNull(sDateYyyymmdd, "sDateYyyymmdd");
        Require.eq(sDateYyyymmdd.length(), "sDateYyyymmdd.length()", 8);
        return sDateYyyymmdd.substring(4, 6)+"/"+sDateYyyymmdd.substring(6, 8)+"/"+sDateYyyymmdd.substring(0, 4);
    }
     ...
     * @throws ParseException
     */
    public static String formatDateFromFormatToFormat(String date, DateFormat sourceDateFormat, DateFormat resultDateFormat ) {
        try {
    ...
    //----------------------------------------------------------------
    /** Returns the absolute timestamp of the most recent occurrence
     * (before or exactly on the <code>referenceTimestamp</code>)
     * of a daily event. The time of day is taken from
     * <code><I>sPropertyNameRoot</I>.time</code> in "h:mm a" format.
     * The time zone for calculations is taken from
     * <code><I>sPropertyNameRoot</I>.timeZone</code>. */
    public static Date getTimestampOfMostRecentDailyEvent(PropertyFile configuration, String sPropertyNameRoot, Date referenceTimestamp) {
    ...
    //----------------------------------------------------------------
    /** Returns the absolute timestamp of the occurrence of a daily
     * event that happens in the same "day" as right now.
     * The time of day of the event is taken from
     * <code><I>sPropertyNameRoot</I>.time</code> in "h:mm a" format.
     * The time zone for calculations (and for determining the
     * boundaries of "today") is taken from
     * <code><I>sPropertyNameRoot</I>.timeZone</code>. */
    ...
    //----------------------------------------------------------------
    private static Calendar buildEventTimestampCalendar(TimeZone timeZone, String sPropertyNameRoot, PropertyFile configuration) {
        ...
        try {
            ...
            try {
                ...
                throw Require.exceptionNeverCaught("Value of property "+sTimeProperty+" (\""+sTime+"\") not in proper format (\""+timeFormat.toPattern()+"\").", e2);
            }
        }
    ...
    //----------------------------------------------------------------
    /** Gets the timestamp of an event based upon a daily event and a
     * date (retrieved from properties) */
    public static Date getTimestampOfEvent(PropertyFile configuration, String sEventPropertyRoot, String sDateProperty) {
        ...
        try {
            ...
            throw Require.exceptionNeverCaught(sDateProperty+" (\""+sDate+"\") not in \""+dateFormat.toPattern()+"\" format.", e);
        }
        Calendar dateCalendar=dateFormat.getCalendar();

        // set the year, month, and day
        eventTimestampCalendar.set(dateCalendar.get(Calendar.YEAR), dateCalendar.get(Calendar.MONTH), dateCalendar.get(Calendar.DAY_OF_MONTH));
    ...
    //----------------------------------------------------------------
    /** Gets the timestamp of an event based upon a daily event and a
     * date specified by year, month (jan=1), day */
    public static Date getTimestampOfEvent(PropertyFile configuration, String sEventPropertyRoot, int nYear, int nMonth, int nDay) {
        Require.nonempty(sEventPropertyRoot, "sEventPropertyRoot");
    ...
    public static Date getTimestampOfEvent(PropertyFile configuration, String sEventPropertyRoot, int nYYYYMMDD) {
        Require.nonempty(sEventPropertyRoot, "sEventPropertyRoot");
        return getTimestampOfEvent(configuration, sEventPropertyRoot, nYYYYMMDD/10000, (nYYYYMMDD/100)%100, nYYYYMMDD%100);
    }
    ...
    //----------------------------------------------------------------
    /** Returns a date (noon in the local time zone) which is the date
     * of the most recent occurrence (before or exactly on the
     * <code>referenceTimestamp</code>) of the specified event, in
     * the event's timezone. */
    public static Date getDateOfMostRecentDailyEvent(PropertyFile configuration, String sPropertyNameRoot, Date referenceTimestamp) {
        Require.nonempty(sPropertyNameRoot, "sPropertyNameRoot");
        Require.neqNull(referenceTimestamp, "referenceTimestamp");
        ...
        targetCalendar.set(sourceCalendar.get(Calendar.YEAR), sourceCalendar.get(Calendar.MONTH), sourceCalendar.get(Calendar.DAY_OF_MONTH), 12, 0, 0);
     ...
     * of the most recent occurrence (before or exactly on the
     * <code>referenceTimestamp</code>) of the specified event, in
     * the event's timezone. If the (strict) valid days mask
     * indicates that the date is not valid, days will be subtracted
     * until the date is valid.
     * <P> See {@link #validateDayOfWeekMask}. */
    public static Date getDateOfMostRecentDailyEvent(PropertyFile configuration, String sPropertyNameRoot, Date referenceTimestamp, String sValidDaysMask) {
        Require.nonempty(sPropertyNameRoot, "sPropertyNameRoot");
        Require.neqNull(referenceTimestamp, "referenceTimestamp");
    ...
    //----------------------------------------------------------------
    /** Wraps a "daily event" as an object.
    * The time of day of the event is taken from
    * <code><I>sPropertyNameRoot</I>.time</code> in "h:mm a" format.
    * The time zone for calculations (and for determining the
    * boundaries of "today") is taken from
    * <code><I>sPropertyNameRoot</I>.timeZone</code>. */
    public static class DailyEvent {
        ...
        public DailyEvent(PropertyFile configuration, String sPropertyNameRoot) {
            ...
            Require.nonempty(sPropertyNameRoot, "sPropertyNameRoot");
            try {
                buildEventTimestampCalendar(getTimeZoneOfEvent(configuration, sPropertyNameRoot), sPropertyNameRoot, configuration);
        ...
        @Override public String toString() {
            return m_configuration.getProperty(m_sPropertyNameRoot+".time")+", "+m_configuration.getProperty(m_sPropertyNameRoot+".timeZone");
        }
    }
     ...
     * @return true if the dates are on the same calendar day, and
     * false otherwise. */
    ...
    //----------------------------------------------------------------
    /** Returns a string in "0d 0h 0m 0.000'000'000s" format from a
    ...
    //----------------------------------------------------------------
    /** Returns a string in "0d 0h 0m 0.000'000s" format from a time
    ...
    //----------------------------------------------------------------
    /** Returns a string in "0d 0h 0m 0.000s" format from a time
    ...
    //----------------------------------------------------------------
    /** Formats the given microsecond timestamp with the given date
     * formatter and then appends the last three microsend digits. */
    public static String formatWithTrailingMicros(DateFormat dateFormat, long nTimestampMicros) {
        return dateFormat.format(nTimestampMicros/DateUtil.MICROS_PER_MILLI)+DateUtil.formatTrailingMicros(nTimestampMicros);
    }

    //----------------------------------------------------------------
    /** Returns the last three digits of the given microsecond
     * timestamp as a string, suitable for appending to a timestamp
     * formatted to millisecond precision. */
