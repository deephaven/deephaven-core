 ...
 * Processes that use instances of this interface are responsible for determining when to call the various methods.
 *
...
public interface ProcessEnvironment {
    ...
    /**
     * Hook for shutting down an installation-specific environment for a given process.  This is intended for
     * controlled invocation, rather than as part of a shutdown hook - {@code onStartup()} should setup any mandatory
     * shutdown hooks, and code in said shutdown hooks should be idempotent w.r.t. onShutdown().
     */
    void onShutdown();
    ...
    interface Factory {
         ...
         */
        ProcessEnvironment make(@NotNull Configuration configuration, @NotNull String mainClassName, @NotNull Logger log);
    }
    ...
    class GlobalHelper {
        ...
        private static void reportInstantiationError(@NotNull final String message, @NotNull final Exception exception, final int exitStatus) {
     ...
     * Fails if the instance is null, in order to make programming errors clear.
     *
    ...
    /**
     * Get the global log if a global process environment has been installed, or else a {@link LoggerFactory#getLogger(Class)} for the given class.
     * @return A logger that can safely be used by code that doesn't otherwise have access to one
     */
     ...
     * It is an error to invoke this without allowReplace if the global instance may already have been set.
     *
     ...
     * class name properties, in descending order of precedence:
     *   {@code mainClassName}.processEnvironmentFactory
    ...
    static @NotNull ProcessEnvironment instantiateFromConfiguration(@NotNull final Configuration configuration,
        ...
        try {
            factoryClassName = configuration.getStringWithDefault(mainClassName + factoryPropertyNameSuffix, configuration.getProperty("default" + factoryPropertyNameSuffix));
        } catch (PropertyException e) {
            GlobalHelper.reportInstantiationError("Failed to get process environment factory name for " + mainClassName, e, -11);
            return null;
        }
    ...
    /**
     * Do the basic installation most interactive (usually client) processes need:
     * (1) instantiateFromConfiguration
     * (2) set
     * (3) Invoke onStartup()
     *
     * @param configuration The configuration
    ...
    /**
     * Do the basic installation most server processes need:
     * (1) basicInteractiveProcessInitialization (as a subset of what servers need)
     * (2) Set current thread name as {@code mainClassName}.main
     * (3) Install the fatal error reporter as default uncaught exception handler
     *
     * @param configuration The configuration
     * @param mainClassName The main class name
     * @param log The log
    ...
    static @NotNull ProcessEnvironment basicServerInitialization(@NotNull final Configuration configuration,
                                                                 @NotNull final String mainClassName,
                                                                 @NotNull final Logger log) {
        final ProcessEnvironment processEnvironment = basicInteractiveProcessInitialization(configuration, mainClassName, log);
