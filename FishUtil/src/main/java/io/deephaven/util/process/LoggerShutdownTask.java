...
// I think I can replace all of the following (adapted from ServiceFactorImpl.finishLogging() and related methods) with
...
//                ((WriterAppender)log4jAppender).append(new LoggingEvent("LoggerShutdownTask", <some logger>, Level.INFO, null, null));
//            }
//        }
//    }
//
...
//        for (final Enumeration<org.apache.log4j.Logger> currentLoggers = LogManager.getCurrentLoggers(); currentLoggers.hasMoreElements();) {
//            for (final Enumeration<Appender> currentAppenders = currentLoggers.nextElement().getAllAppenders(); currentAppenders.hasMoreElements();) {
//                addLog4jAppenders(appenders, currentAppenders.nextElement());
//            }
//        }
//        for (final Enumeration<Appender> rootLoggerAppenders = LogManager.getRootLogger().getAllAppenders(); rootLoggerAppenders.hasMoreElements();) {
...
//            for (final Enumeration<Appender> attachedAppenders = ((AppenderAttachable)appender).getAllAppenders(); attachedAppenders.hasMoreElements();) {
