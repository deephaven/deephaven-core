...
public final class LoggerUtil {
    ...
    /**
     * Formats an arbitrary object, using a fast method if we can recognize the type, or toString otherwise. Usually you
     * should make the object LogOutputAppendable or create an ObjFormatter, but if the object is truly of unknown type,
     * this is better than calling toString() directly. Outputs "null" if the given object is null.
     */
    ...
    /**
     * Attempt to log a line of items at level to log. Fails silently if any Throwable is thrown, including Throwable's
     * one might ordinarily prefer not to catch (e.g. InterruptedException, subclasses of Error, etc). This is intended
     * for use in processes that are shutting down.
     *
     * @param log
     * @param level
     * @param items
