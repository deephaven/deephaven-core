...
public final class WaitNotification extends AbstractNotification {
    ...
    /**
     * Wait for all the specified dependencies to be satisfied on the specified step.
     *
    ...
    public static boolean waitForSatisfaction(final long step,
        ...
        if (NotificationQueue.Dependency.getUpdateGraph(null, dependencies).maybeAddNotification(waitNotification,
                step)) {
            try {
                waitNotification.await();
            } catch (InterruptedException e) {
                throw new UncheckedDeephavenException("Interrupted while awaiting dependency satisfaction for "
