...
public abstract class RspArray<T extends RspArray> extends RefCountedCow<T> {
    ...
    public long rangesCountUpperBound(final int startIdx, final int endIdx) {
        ...
        for (int idx = startIdx; idx <= endIdx; ++idx) {
            ...
            if (isSingletonSpan(s)) {
                ++nRanges;
            } else if (s instanceof io.deephaven.engine.rowset.impl.rsp.container.RunContainer) {
                nRanges += ((io.deephaven.engine.rowset.impl.rsp.container.RunContainer) s).numberOfRanges();
            } else if (s instanceof io.deephaven.engine.rowset.impl.rsp.container.SingleRangeContainer) {
                nRanges += 1;
            } else if (s instanceof io.deephaven.engine.rowset.impl.rsp.container.TwoValuesContainer) {
                nRanges += 2;
