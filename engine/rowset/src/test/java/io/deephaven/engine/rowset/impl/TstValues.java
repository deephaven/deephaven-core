...
public class TstValues {
    ...
    public static void setup(final Builder b, final int sz, final TstValues.Config c) {
        final int halfClusterWidth = c.clusterWidth / 2;
        final TstValues.Context ctx = new TstValues.Context(c.min + halfClusterWidth, b);
    ...
    public static void setup3(final Builder b1, final Builder b2, final Builder b3, final int sz,
            final TstValues.Config cf) {
        final int halfClusterWidth = cf.clusterWidth / 2;
        final TstValues.Context cx1 = new TstValues.Context(cf.min + halfClusterWidth, b1);
        final TstValues.Context cx2 = new TstValues.Context(cf.max + halfClusterWidth, b2);
        final TstValues.Context cx3 = new TstValues.Context(cf.max + halfClusterWidth, b3);
        final Random r = new Random(0);
        final int d = cf.max - cf.min + 1 - cf.clusterWidth;
        for (int i = 0; i < sz; ++i) {
            final long k = cx1.populateFirstArgStep(cf.jumpPropOneIn, d, halfClusterWidth, r);
            final TstValues.Context cx = (r.nextBoolean()) ? cx2 : cx3;
