...
/**
 * {@link RowSet#invert} requires every key asked for to be present, so a missing key is an argument error and is
 * rejected with an {@link IllegalArgumentException}, whichever backing holds the rowset and whichever container holds
 * the block. The same call must not report a different exception depending only on how a block happens to be stored.
 */
