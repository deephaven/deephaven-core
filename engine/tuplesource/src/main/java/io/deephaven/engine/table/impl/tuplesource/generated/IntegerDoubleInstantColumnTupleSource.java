...
/**
 * <p>{@link TupleSource} that produces key column values from {@link ColumnSource} types Integer, Double, and Instant.
 */
@SuppressWarnings({"unused", "WeakerAccess"})
public class IntegerDoubleInstantColumnTupleSource extends AbstractTupleSource<IntDoubleLongTuple> {

    /** {@link ThreeColumnTupleSourceFactory} instance to create instances of {@link IntegerDoubleInstantColumnTupleSource}. **/
    public static final ThreeColumnTupleSourceFactory<IntDoubleLongTuple, Integer, Double, Instant> FACTORY = new Factory();

    private final ColumnSource<Integer> columnSource1;
    private final ColumnSource<Double> columnSource2;
    private final ColumnSource<Instant> columnSource3;

    public IntegerDoubleInstantColumnTupleSource(
    ...
    protected void convertChunks(@NotNull WritableChunk<? super Values> destination, int chunkSize, Chunk<? extends Values> [] chunks) {
        ...
        for (int ii = 0; ii < chunkSize; ++ii) {
            destinationObjectChunk.set(ii, new IntDoubleLongTuple(chunk1.get(ii), chunk2.get(ii), DateTimeUtils.epochNanos(chunk3.get(ii))));
        }
        destinationObjectChunk.setSize(chunkSize);
    }

    /** {@link ThreeColumnTupleSourceFactory} for instances of {@link IntegerDoubleInstantColumnTupleSource}. **/
    private static final class Factory implements ThreeColumnTupleSourceFactory<IntDoubleLongTuple, Integer, Double, Instant> {
