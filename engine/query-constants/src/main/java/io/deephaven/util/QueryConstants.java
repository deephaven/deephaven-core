...
public class QueryConstants {
     ...
     */
    /*
     * Implemented as a constant so that it can be translated correctly to JS without needing a proper
     * {@code Math.nextUp()} or {@code Math.nextAfter()} implementation that works in JavaScript. Value is generated by
     * running {@code Float.toHexString(Math.nextUp(-Float.MAX_VALUE))} in jshell, and tests validate that this matches
     ...
     * Implemented as a constant so that it can be translated correctly to JS without needing a proper
     * {@code Math.nextUp()} or {@code Math.nextAfter()} implementation that works in JavaScript. Value is generated by
     * running {@code Double.toHexString(Math.nextUp(-Double.MAX_VALUE))} in jshell, and tests validate that this
     * matches the expected expression.
     */
