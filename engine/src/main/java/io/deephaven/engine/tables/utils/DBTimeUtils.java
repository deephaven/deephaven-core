...
public class DBTimeUtils {
     ...
     * DBTimeUtils.dateStyle system property.
     */
    ...
    private static final DateStyle DATE_STYLE = DateStyle
            .valueOf(Configuration.getInstance().getStringWithDefault("DBTimeUtils.dateStyle", DateStyle.MDY.name()));
     ...
     * @return a null {@link DBDateTime} if either input is null; the starting {@link DBDateTime} plus the specified
     *         number of nanoseconds, if the result is not too large for a {@link DBDateTime}; or throws a
     *         {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the resultant value is more than max
     ...
     * @return a null {@link DBDateTime} if either input is null; the starting {@link DBDateTime} minus the specified
     *         number of nanoseconds, if the result is not too negative for a {@link DBDateTime}; or throws a
     *         {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the resultant value is more than min
     *         long nanoseconds from Epoch.
     */
     ...
     * @return a null {@link DBDateTime} if either input is null; the starting {@link DBDateTime} plus the specified
     *         period, if the result is not too large for a DBDateTime; or throws a {@link DBDateTimeOverflowException
     *         DBDateTimeOverflowException} if the resultant value is more than max long nanoseconds from Epoch.
     */
    public static DBDateTime plus(DBDateTime dateTime, DBPeriod period) {
        ...
        if (period.isPositive()) {
            return new DBDateTime(millisToNanos(dateTime.getJodaDateTime().plus(period.getJodaPeriod()).getMillis())
                    + dateTime.getNanosPartial());
        } else {
            return new DBDateTime(millisToNanos(dateTime.getJodaDateTime().minus(period.getJodaPeriod()).getMillis())
                    + dateTime.getNanosPartial());
        }
    }
     ...
     * @return a null {@link DBDateTime} if either input is null; the starting {@link DBDateTime} minus the specified
     *         period, if the result is not too negative for a {@link DBDateTime}; or throws a
     *         {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the resultant value is more than min
     *         long nanoseconds from Epoch.
     */
     ...
     *         first {@link DBDateTime} minus d2, if the result is not out of range for a long value; or throws a
     *         {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the resultant value would be more than
     ...
     *         obtained from the first {@link DBDateTime} value minus d2, if the intermediate value of nanoseconds
     *         difference between the two dates is not out of range for a long value; or throws a
     *         {@link DBDateTimeOverflowException} if the intermediate value would be more than min long or max long
     ...
     *         from the first {@link DBDateTime} value minus d2, if the intermediate value of nanoseconds difference
     *         between the two dates is not out of range for a long value; or throws a
     *         {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the intermediate value would be more
    ...
    public static DBDateTime dateAtMidnight(DBDateTime dateTime, DBTimeZone timeZone) {
        ...
        return new DBDateTime(millisToNanos(new DateMidnight(dateTime.getMillis(), timeZone.getTimeZone()).getMillis())
                + dateTime.getNanosPartial());
    }
     ...
     *        calculated.
     * @param timeZone {@link DBTimeZone} for which the new value at midnight should be calculated.
     * @return A {@link DBDateTime} rounded down to midnight in the selected time zone for the specified number of
     *         milliseconds from Epoch.
     */
    @SuppressWarnings("WeakerAccess")
    public static DBDateTime millisToDateAtMidnight(final long millis, final DBTimeZone timeZone) {
        ...
        return new DBDateTime(millisToNanos(new DateMidnight(millis, timeZone.getTimeZone()).getMillis()));
    }
    ...
    /**
     * Returns a String date/time representation of a {@link DBDateTime} interpreted for the New York time zone.
     *
     * @param dateTime The {@link DBDateTime} to format as a String.
    ...
    /**
     * Returns a String date representation of a {@link DBDateTime} interpreted for a specified time zone.
     *
     * @param dateTime The {@link DBDateTime} to format as a String.
     * @param timeZone The {@link DBTimeZone} to use when formatting the String.
    ...
    /**
     * Returns a String date representation of a {@link DBDateTime} interpreted for the New York time zone.
     *
     * @param dateTime The {@link DBDateTime} to format as a String.
     ...
     *         month represented by the {@link DBDateTime} when interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *         represented by the {@link DBDateTime} when interpreted in the specified time zone.
     */
     ...
     *         midnight for the date/time represented by the {@link DBDateTime} when interpreted in the specified time
     *         zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value of milliseconds since
     *         the top of the second for the date/time represented by the {@link DBDateTime} when interpreted in the
     *         specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *         the top of the millisecond for the date/time represented by the {@link DBDateTime} when interpreted in
     ...
     *         top of the hour for the date/time represented by the {@link DBDateTime} when interpreted in the specified
     *         time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *         date/time represented by the {@link DBDateTime} when interpreted in the specified time zone. January is
     *         1, February is 2, etc.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *         top of the minute for the date/time represented by the {@link DBDateTime} when interpreted in the
     *         specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *         date/time represented by the {@link DBDateTime} when interpreted in the specified time zone.
     */
     ...
     *         for the date/time represented by the {@link DBDateTime} when interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *         of a {@link DBDateTime} in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *         a {@link DBDateTime} in the New York time zone.
     */
    @SuppressWarnings("WeakerAccess")
    ...
    public static long microsToNanos(long micros) {
        ...
        if (Math.abs(micros) > MAX_CONVERTIBLE_MICROS) {
            throw new DBDateTimeOverflowException("Converting " + micros + " micros to nanos would overflow");
        }
        return micros * 1000;
    }
    ...
    public static long millisToNanos(long millis) {
        ...
        if (ENABLE_MICROTIME_HACK) {
            ...
            throw new DBDateTimeOverflowException("Converting " + millis + " millis to nanos would overflow");
        }
    ...
    public static long secondsToNanos(long seconds) {
        ...
        if (Math.abs(seconds) > MAX_CONVERTIBLE_SECONDS) {
            throw new DBDateTimeOverflowException("Converting " + seconds + " seconds to nanos would overflow");
        }
    ...
    /**
     * Converts a long offset from Epoch value to a {@link DBDateTime}. This method uses expected date ranges to infer
     ...
     * @return a null {@link DBDateTime} if either original or period are null; the starting {@link DBDateTime} plus the
     *         specified period, if the result is not too large for a DBDateTime and does not exceed the cap value; the
     *         cap value if this is less than offset plus period. Throws a {@link DBDateTimeOverflowException
     *         DBDateTimeOverflowException} if the resultant value is more than max long nanoseconds from Epoch.
     */
    ...
    /**
     * Returns a {@link DBDateTime} value, which is at the starting (lower) end of a time range defined by the interval
     ...
     *
     * @param dateTime The {@link DBDateTime} for which to evaluate the start of the containing window.
     * @param intervalNanos The size of the window in nanoseconds.
     * @return Null if either input is null, otherwise a {@link DBDateTime} representing the start of the window.
     */
    ...
    /**
     * Returns a {@link DBDateTime} value, which is at the ending (upper) end of a time range defined by the interval
     ...
     *
     * @param dateTime The {@link DBDateTime} for which to evaluate the end of the containing window.
     * @param intervalNanos The size of the window in nanoseconds.
     * @return Null if either input is null, otherwise a {@link DBDateTime} representing the end of the window.
     */
    ...
    public static Result convertExpression(String formula) throws Exception { // TODO: Why throw Exception?
        ...
        while (matcher.find()) {
            ...
            if (convertDateTimeQuiet(s) != null) {
                ...
                instanceVariablesString.append("        private DBDateTime _date").append(dateTimeIndex)
                        .append("=DBTimeUtils.convertDateTime(\"")
                ...
                instanceVariablesString.append("        private java.time.LocalDate _localDate").append(localDateIndex)
                        .append("=DBTimeUtils.convertDate(\"").append(formula, matcher.start() + 1, matcher.end() - 1)
                        .append("\");\n");
                ...
                instanceVariablesString.append("        private long _time").append(timeIndex)
                        .append("=DBTimeUtils.convertTime(\"").append(formula, matcher.start() + 1, matcher.end() - 1)
                        .append("\");\n");
                ...
                instanceVariablesString.append("        private DBPeriod _period").append(periodIndex)
                        .append("=DBTimeUtils.convertPeriod(\"").append(formula, matcher.start() + 1, matcher.end() - 1)
                        .append("\");\n");
                ...
                instanceVariablesString.append("        private java.time.LocalTime _localTime").append(timeIndex)
                        .append("=DBTimeUtils.convertLocalTime(\"")
                        .append(formula, matcher.start() + 1, matcher.end() - 1).append("\");\n");
     ...
     *         nanoseconds for a period. Throws {@link DBDateTimeOverflowException} if the resultant value would be
    ...
    public static long expressionToNanos(String formula) {
        ...
        if (period != null) {
            try {
                ...
                throw new DBDateTimeOverflowException("Period length in nanoseconds exceeds Long.MAX_VALUE : " + s, ex);
            }
        }
     ...
     * @throws RuntimeException if the String cannot be converted, otherwise a {@link DBDateTime} from the parsed
     *         String.
     */
     ...
     * @param s String to be converted, usually in the form yyyy-MM-ddThh:mm:ss and with optional sub-seconds after an
     *        optional decimal point, followed by a mandatory time zone character code
     * @return A DBDateTime from the parsed String, or null if the format is not recognized or an exception occurs
     */
    public static DBDateTime convertDateTimeQuiet(final String s) {
        try {
            ...
            if (DATETIME_PATTERN.matcher(s).matches()) {
                ...
                timeZone = DBTimeZone.valueOf("TZ_" + s.substring(spaceIndex + 1).trim().toUpperCase());
            ...
            if (decimalIndex == -1) {
                return new DBDateTime(millisToNanos(new DateTime(dateTimeString, timeZone.getTimeZone()).getMillis()));
            } else {
                final long subsecondNanos = parseNanos(dateTimeString.substring(decimalIndex + 1));

                return new DBDateTime(millisToNanos(new DateTime(dateTimeString.substring(0, decimalIndex),
    ...
    public static DBDateTime toDateTime(ZonedDateTime zonedDateTime) {
        ...
        if (seconds >= limit) {
            throw new DBDateTimeOverflowException("Overflow: cannot convert " + zonedDateTime + " to new DBDateTime");
        }
    ...
    /**
     * A type of RuntimeException thrown when operations resulting in {@link DBDateTime} values would exceed the range
