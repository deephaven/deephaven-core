...
import io.deephaven.engine.v2.sort.timsort.ByteLongTimsortKernel;
...
public class BytePartitionKernel {
    public static class PartitionKernelContext {
        ...
        private final WritableByteChunk<Any> pivotValues;
        ...
        private PartitionKernelContext(int chunkSize, int numPartitions, boolean preserveEquality) {
            ...
            pivotValues = WritableByteChunk.makeWritableChunk(numPartitions - 1);
...
//            System.out.println("[" + IntStream.range(0, pivotValues.size()).mapToObj(pivotValues::get).map(BytePartitionKernel::format).collect(Collectors.joining(",")) + "]");
...
//    }
    ...
    public static PartitionKernelContext createContext(Index index, ColumnSource<Byte> columnSource, int chunkSize, int nPartitions, boolean preserveEquality) {
        ...
        final WritableByteChunk<Any> tempPivotValues = WritableByteChunk.makeWritableChunk(nPartitions * 3);
    ...
    private static void samplePivots(Index index, int nPartitions, WritableLongChunk<KeyIndices> pivotKeys, WritableByteChunk<Any> pivotValues, ColumnSource<Byte> columnSource) {
        pivotKeys.setSize(0);
        ...
        try (final LongSortKernel sortContext = ByteLongTimsortKernel.createContext(samplesRequired)) {
     ...
     */
    public static void partition(PartitionKernelContext context, LongChunk<KeyIndices> indexKeys, ByteChunk values) {
        ...
        for (int ii = 0; ii < values.size(); ii += accumulatedChunkSize) {
            ...
            for (int jj = ii; jj < last; jj++) {
                ...
                final byte searchValue = values.get(jj);
    ...
    private static int binarySearchPreserve(ByteChunk pivotValues, int lo, int hi, byte searchValue) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final byte compareValue = pivotValues.get(mid);
    ...
    private static int binarySearchTieIndex(ByteChunk pivotValues, LongChunk<KeyIndices> pivotKeys, int lo, int hi, byte searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final byte compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
