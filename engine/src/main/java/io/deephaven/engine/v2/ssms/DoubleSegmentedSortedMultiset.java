...
public final class DoubleSegmentedSortedMultiset implements SegmentedSortedMultiSet<Double>, DbDoubleArray {
    ...
    public boolean insert(WritableChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        ...
        insert(valuesToInsert.asWritableDoubleChunk(), counts);
        return beforeSize != size();
    }

    private int insertExistingIntoLeaf(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, double [] leafValues, long [] leafCounts, double maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        double nextValue;
        while (rlpos < leafSize && ripos < valuesToInsert.size() && (leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
            if (gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    ...
                    valuesToInsert.copyFromTypedChunk((WritableDoubleChunk)valuesToInsert, ripos, wipos.intValue(), lastInsert - ripos);
    ...
    private void distributeNewIntoLeaves(DoubleChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, final int insertStart, final int insertCount, int firstLeaf, int requiredLeaves, int newLeafSize) {
    ...
    private void insertNewIntoLeaf(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, double [] leafValues, long [] leafCounts) {
    ...
    private void copyRemainingValuesToLeaf(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int insertStart, double[] leafValues, long[] leafCounts, int ripos) {
    ...
    private void maybeCompact(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, int wipos) {
        if (wipos == ripos) {
            return;
        }
    ...
    private void insertExisting(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        if (leafCount == 0) {
            return;
        }
        if (leafCount == 1) {
            final MutableInt wipos = new MutableInt(0);
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, 0, wipos, size, directoryValues, directoryCount, NULL_DOUBLE, true);
    ...
    private void insert(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        validate();
    ...
    private void makeLeavesInitial(DoubleChunk<? extends Values> values, IntChunk<ChunkLengths> counts) {
    ...
    private void packValuesIntoLeaves(DoubleChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int rpos, int startLeaf, int valuesPerLeaf) {
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(DoubleChunk<? extends Attributes.Any> valuesToSearch, int lo, int hi, double searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
    ...
    public boolean remove(RemoveContext removeContext, WritableChunk<? extends Values> valuesToRemove, WritableIntChunk<ChunkLengths> counts) {
        final long beforeSize = size();
        remove(removeContext, valuesToRemove.asDoubleChunk(), counts);
        return beforeSize != size();
    }

    private void remove(RemoveContext removeContext, DoubleChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts) {
        validate();
    ...
    private int removeFromLeaf(RemoveContext removeContext, DoubleChunk<? extends Values> valuesToRemove, IntChunk<ChunkLengths> counts, int ripos, int end, double[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
    ...
    private void validateInputs(DoubleChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        if (!SEGMENTED_SORTED_MULTISET_VALIDATION) {
            return;
        }
        ...
        for (int ii = 1; ii < valuesToInsert.size(); ++ii) {
            ...
            Assert.assertion(DhDoubleComparisons.lt(prevValue, curValue), "DhDoubleComparisons.lt(prevValue, curValue)", prevValue, "prevValue", curValue, "curValue");
        }
    }
...
//                }
    ...
    private static void validateLeaf(double[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])", (Double)thisValue, "values[ii]", (Double)nextValue, "values[ii + 1]", ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(geq(getMinDouble(), destination.getMaxDouble()), "geq(getMinDouble(), destination.getMaxDouble())");
            }
        }
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(leq(getMaxDouble(), destination.getMinDouble()), "leq(getMaxDouble(), destination.getMinDouble())");
            }
        }
        ...
        if (completeLeavesToMove > 0) {
            if (leafCount == 1) {
                ...
                if (directoryMoves > 0) {
                    Arrays.fill(directoryValues, firstLeafTozero, firstLeafTozero + directoryMoves - (completeLeavesToMove - numberOfLeavesToRemove), NULL_DOUBLE);
                }
                maybePromoteLastLeaf();
            }
            destination.maybePromoteLastLeaf();
    ...
    private void maybeAccumulateAdditions(WritableDoubleChunk<? extends Values> valuesToInsert) {
