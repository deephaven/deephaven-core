...
public final class CharChunkPool implements ChunkPool {
    ...
    private final WritableCharChunk<Any> EMPTY = WritableCharChunk.writableChunkWrap(ArrayUtils.EMPTY_CHAR_ARRAY);
    ...
    CharChunkPool() {
        ...
        for (int pcci = 0; pcci < NUM_POOLED_CHUNK_CAPACITIES; ++pcci) {
            ...
            writableCharChunks[pcci] = new SegmentedSoftPool<>(
                    ...
                    () -> QueryPerformanceRecorder.recordPoolAllocation(() -> WritableCharChunk.makeWritableChunkForPool(chunkCapacity)),
                    (final WritableCharChunk chunk) -> chunk.setSize(chunkCapacity)
            );
        }
        resettableCharChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableCharChunk::makeResettableChunkForPool),
                ResettableCharChunk::clear
        );
        resettableWritableCharChunks = new SegmentedSoftPool<>(
                SUB_POOL_SEGMENT_CAPACITY,
                () -> QueryPerformanceRecorder.recordPoolAllocation(ResettableWritableCharChunk::makeResettableChunkForPool),
