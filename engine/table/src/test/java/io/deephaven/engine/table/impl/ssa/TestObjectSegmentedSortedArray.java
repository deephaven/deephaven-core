...
public class TestObjectSegmentedSortedArray extends RefreshingTableTestCase {
    ...
    private void testShifts(@NotNull final SsaTestHelpers.TestDescriptor desc) {
        ...
        try (final SafeCloseable ignored = LivenessScopeStack.open(new LivenessScope(true), true)) {
            final TableUpdateListener asObjectListener =
                    new InstrumentedTableUpdateListenerAdapter(asObject, false) {
                        @Override
                        public void onUpdate(TableUpdate upstream) {
                            try (final ColumnSource.GetContext checkContext =
                                    valueSource.makeGetContext(asObject.getRowSet().intSizePrev());
                                    final RowSet relevantIndices = asObject.getRowSet().copyPrev()) {
                                checkSsa(ssa, valueSource.getPrevChunk(checkContext, relevantIndices).asObjectChunk(),
                            ...
                            try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(size);
                                ...
                                if (takeout.isNonempty()) {
                                    final ObjectChunk<Object, ? extends Values> valuesToRemove =
                                            valueSource.getPrevChunk(getContext, takeout).asObjectChunk();
                                ...
                                if (upstream.shifted().nonempty()) {
                                    ...
                                    while (sit.hasNext()) {
                                        ...
                                        try (final RowSet prevRowSet = table.getRowSet().copyPrev();
                                            ...
                                            final ObjectChunk<Object, ? extends Values> shiftValues =
                                                    valueSource.getPrevChunk(getContext, rowSetToShift).asObjectChunk();
                                ...
                                try (final RowSet putin = upstream.added().union(upstream.modified())) {
                                    ...
                                    try (final ColumnSource.GetContext checkContext =
                                            valueSource.makeGetContext(asObject.intSize());
                                            final RowSet relevantIndices = asObject.getRowSet().minus(putin)) {
                                        checkSsa(ssa, valueSource.getChunk(checkContext, relevantIndices).asObjectChunk(),
                                                relevantIndices.asRowKeyChunk(), desc);
                                    }

                                    if (putin.isNonempty()) {
                                        final ObjectChunk<Object, ? extends Values> valuesToInsert =
                                                valueSource.getChunk(getContext, putin).asObjectChunk();
            ...
            while (desc.advance(50)) {
                ...
                try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(asObject.intSize());
                    ...
                    checkSsa(ssa, valueSource.getChunk(getContext, asObjectRowSetCopy).asObjectChunk(),
                            asObjectRowSetCopy.asRowKeyChunk(), desc);
                }
            }
        }
    }
    ...
    private void testUpdates(@NotNull final SsaTestHelpers.TestDescriptor desc, boolean allowAddition,
        ...
        try (final SafeCloseable ignored = LivenessScopeStack.open(new LivenessScope(true), true)) {
            final ShiftObliviousListener asObjectListener =
                    new ShiftObliviousInstrumentedListenerAdapter(asObject, false) {
                        @Override
                        public void onUpdate(RowSet added, RowSet removed, RowSet modified) {
                            try (final ColumnSource.GetContext getContext =
                                ...
                                if (removed.isNonempty()) {
                                    final ObjectChunk<Object, ? extends Values> valuesToRemove =
                                            valueSource.getPrevChunk(getContext, removed).asObjectChunk();
                                    ssa.remove(valuesToRemove, removed.asRowKeyChunk());
                                }
                                if (added.isNonempty()) {
                                    ssa.insert(valueSource.getChunk(getContext, added).asObjectChunk(),
                                            added.asRowKeyChunk());
                                }
                            }
                        }
    ...
    private void checkSsaInitial(Table asObject, ObjectSegmentedSortedArray ssa, ColumnSource<?> valueSource,
        ...
        try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(asObject.intSize());
            ...
            final ObjectChunk<Object, ? extends Values> valueChunk =
                    valueSource.getChunk(getContext, asObjectRowSetCopy).asObjectChunk();
    ...
    private void checkSsa(ObjectSegmentedSortedArray ssa, ObjectChunk<Object, ? extends Values> valueChunk,
