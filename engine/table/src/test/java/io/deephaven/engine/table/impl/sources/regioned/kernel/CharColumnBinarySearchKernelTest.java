...
public class CharColumnBinarySearchKernelTest {
    ...
    public void testBoundsAscending() {
        ...
        final List<Character> data = List.of(
                (char) 1, (char) 2, (char) 3, (char) 3, (char) 3,
        ...
        try {
            ...
            assertEquals(-6, CharColumnBinarySearchKernel.lowerBoundAscending(
                    source, selection, 0, lastPos, (char) 4, false, false));
            assertEquals(-6, CharColumnBinarySearchKernel.lowerBoundAscending(
                    source, selection, 0, lastPos, (char) 4, true, false));
            // lowerBound: 5 is present but exclusive (value > 5) finds nothing; past-end insertion point 8 -> -(8+1)=-9
            assertEquals(-9, CharColumnBinarySearchKernel.lowerBoundAscending(
                    source, selection, 0, lastPos, (char) 5, false, false));
            // lowerBound: 5 is present; inclusive search returns first occurrence at position 5
            assertEquals(5, CharColumnBinarySearchKernel.lowerBoundAscending(
                    source, selection, 0, lastPos, (char) 5, true, false));
            ...
            assertEquals(-6, CharColumnBinarySearchKernel.upperBoundAscending(
                    source, selection, 0, lastPos, (char) 4, false, false));
            // inserting 4 at pos 5 preserves sort order
            assertEquals(-6, CharColumnBinarySearchKernel.upperBoundAscending(
                    source, selection, 0, lastPos, (char) 4, true, false));
            ...
            assertEquals(-1, CharColumnBinarySearchKernel.upperBoundAscending(
                    source, selection, 0, lastPos, (char) 1, false, false));
            // upperBound: 5 is present; inclusive search returns last occurrence at position 7
            // inserting 5 at pos 7 preserves sort order
            assertEquals(7, CharColumnBinarySearchKernel.upperBoundAscending(
                    source, selection, 0, lastPos, (char) 5, true, false));
        } finally {
            selection.close();
            source.clear(true);
    ...
    public void testBoundsDescending() {
        ...
        final List<Character> data = List.of(
                (char) 5, (char) 5, (char) 5, (char) 3, (char) 3, (char) 3,
                (char) 2, (char) 1);
        final CharChunkColumnSource source = makeChunkColumnSource(data);
        final RowSet selection = RowSetFactory.fromRange(0, data.size() - 1);
        final long lastPos = data.size() - 1;
    ...
    public void testMatchMultipleValuesIncludingNull() {
        final List<Character> ascending =
                List.of(NULL_CHAR, (char) 1, (char) 2, (char) 5, (char) 5, (char) 9);

        // The null sentinel alongside an ordinary value, listed both ways round.
        assertMatchPositions(ascending, new Character[] {(char) 5, NULL_CHAR}, List.of(0L, 3L, 4L));
        assertMatchPositions(ascending, new Character[] {NULL_CHAR, (char) 5}, List.of(0L, 3L, 4L));

        // Several ordinary values plus the sentinel, listed out of order.
        assertMatchPositions(ascending, new Character[] {(char) 9, (char) 1, NULL_CHAR}, List.of(0L, 1L, 5L));

        // The sentinel on its own, and ordinary values without it.
        assertMatchPositions(ascending, new Character[] {NULL_CHAR}, List.of(0L));
        assertMatchPositions(ascending, new Character[] {(char) 1, (char) 9}, List.of(1L, 5L));

        // A value absent from the data, between two that are present.
        assertMatchPositions(ascending, new Character[] {(char) 1, (char) 3, (char) 9}, List.of(1L, 5L));

        // Nothing present at all.
        assertMatchPositions(ascending, new Character[] {(char) 3, (char) 7}, List.of());
    }
    ...
    private static void assertMatchPositions(
            final List<Character> ascendingData,
            final Character[] searchValues,
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            try (final RowSet selection = RowSetFactory.fromRange(0, data.size() - 1);
                    final RowSet matched = CharColumnBinarySearchKernel.binarySearchMatch(
