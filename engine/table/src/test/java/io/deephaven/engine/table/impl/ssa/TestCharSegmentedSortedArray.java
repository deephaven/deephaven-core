...
public class TestCharSegmentedSortedArray extends RefreshingTableTestCase {
    ...
    private void testShifts(@NotNull final SsaTestHelpers.TestDescriptor desc) {
        ...
        try (final SafeCloseable ignored = LivenessScopeStack.open(new LivenessScope(true), true)) {
            final TableUpdateListener asCharacterListener =
                    new InstrumentedTableUpdateListenerAdapter(asCharacter, false) {
                        @Override
                        public void onUpdate(TableUpdate upstream) {
                            try (final ColumnSource.GetContext checkContext =
                                    valueSource.makeGetContext(asCharacter.getRowSet().intSizePrev());
                                    final RowSet relevantIndices = asCharacter.getRowSet().copyPrev()) {
                                checkSsa(ssa, valueSource.getPrevChunk(checkContext, relevantIndices).asCharChunk(),
                            ...
                            try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(size);
                                ...
                                if (takeout.isNonempty()) {
                                    final CharChunk<? extends Values> valuesToRemove =
                                            valueSource.getPrevChunk(getContext, takeout).asCharChunk();
                                ...
                                if (upstream.shifted().nonempty()) {
                                    ...
                                    while (sit.hasNext()) {
                                        ...
                                        try (final RowSet prevRowSet = table.getRowSet().copyPrev();
                                            ...
                                            final CharChunk<? extends Values> shiftValues =
                                                    valueSource.getPrevChunk(getContext, rowSetToShift).asCharChunk();
                                ...
                                try (final RowSet putin = upstream.added().union(upstream.modified())) {
                                    ...
                                    try (final ColumnSource.GetContext checkContext =
                                            valueSource.makeGetContext(asCharacter.intSize());
                                            final RowSet relevantIndices = asCharacter.getRowSet().minus(putin)) {
                                        checkSsa(ssa, valueSource.getChunk(checkContext, relevantIndices).asCharChunk(),
                                                relevantIndices.asRowKeyChunk(), desc);
                                    }

                                    if (putin.isNonempty()) {
                                        final CharChunk<? extends Values> valuesToInsert =
                                                valueSource.getChunk(getContext, putin).asCharChunk();
            ...
            while (desc.advance(50)) {
                ...
                try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(asCharacter.intSize());
                    ...
                    checkSsa(ssa, valueSource.getChunk(getContext, asCharRowSetCopy).asCharChunk(),
                            asCharRowSetCopy.asRowKeyChunk(), desc);
                }
            }
        }
    }
    ...
    private void testUpdates(@NotNull final SsaTestHelpers.TestDescriptor desc, boolean allowAddition,
        ...
        try (final SafeCloseable ignored = LivenessScopeStack.open(new LivenessScope(true), true)) {
            final ShiftObliviousListener asCharacterListener =
                    new ShiftObliviousInstrumentedListenerAdapter(asCharacter, false) {
                        @Override
                        public void onUpdate(RowSet added, RowSet removed, RowSet modified) {
                            try (final ColumnSource.GetContext getContext =
                                ...
                                if (removed.isNonempty()) {
                                    final CharChunk<? extends Values> valuesToRemove =
                                            valueSource.getPrevChunk(getContext, removed).asCharChunk();
                                    ssa.remove(valuesToRemove, removed.asRowKeyChunk());
                                }
                                if (added.isNonempty()) {
                                    ssa.insert(valueSource.getChunk(getContext, added).asCharChunk(),
                                            added.asRowKeyChunk());
                                }
                            }
                        }
    ...
    private void checkSsaInitial(Table asCharacter, CharSegmentedSortedArray ssa, ColumnSource<?> valueSource,
        ...
        try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(asCharacter.intSize());
                final RowSet asCharRowSetCopy = asCharacter.getRowSet().copy()) {
            final CharChunk<? extends Values> valueChunk =
                    valueSource.getChunk(getContext, asCharRowSetCopy).asCharChunk();
    ...
    private void checkSsa(CharSegmentedSortedArray ssa, CharChunk<? extends Values> valueChunk,
