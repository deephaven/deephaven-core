...
public class TestRollingWAvg extends BaseUpdateByTest {
    ...
    private void doTestAppendOnly(boolean bucketed, int prevTicks, int fwdTicks) {
        ...
        for (int ii = 0; ii < DYNAMIC_UPDATE_STEPS; ii++) {
            ...
            updateGraph.<ControlledUpdateGraph>cast().runWithinUnitTestCycle(() -> generateAppends(DYNAMIC_UPDATE_SIZE, billy, t, result.infos));
        ...
        for (int ii = 0; ii < DYNAMIC_UPDATE_STEPS; ii++) {
            UpdateGraph updateGraph = ExecutionContext.getContext().getUpdateGraph();
            updateGraph.<ControlledUpdateGraph>cast().runWithinUnitTestCycle(() -> generateAppends(DYNAMIC_UPDATE_SIZE, billy, t2, result2.infos));
            TstUtils.validate("Table", nuggets);
        }
    }
    ...
    private void doTestTicking(final boolean bucketed, final long prevTicks, final long fwdTicks) {
        ...
        for (int ii = 0; ii < DYNAMIC_UPDATE_STEPS; ii++) {
            UpdateGraph updateGraph = ExecutionContext.getContext().getUpdateGraph();
            updateGraph.<ControlledUpdateGraph>cast().runWithinUnitTestCycle(() -> GenerateTableUpdates.generateTableUpdates(DYNAMIC_UPDATE_SIZE, billy, t, result.infos));
        ...
        for (int ii = 0; ii < DYNAMIC_UPDATE_STEPS; ii++) {
            UpdateGraph updateGraph = ExecutionContext.getContext().getUpdateGraph();
            updateGraph.<ControlledUpdateGraph>cast().runWithinUnitTestCycle(() -> GenerateTableUpdates.generateTableUpdates(DYNAMIC_UPDATE_SIZE, billy, t2, result2.infos));
            TstUtils.validate("Table - step " + ii, nuggets);
        }
    }
