...
public class TestLongSegmentedSortedMultiset extends RefreshingTableTestCase {
    ...
    private void checkPartialCopy(final int nodeSize, final int valueCount) {
        ...
        assertEquals(prefix, NULL_LONG, ssm.get(-1)/*EXTRA*/);
        assertEquals(prefix, NULL_LONG, ssm.get(valueCount)/*EXTRA*/);
        assertEquals(prefix, NULL_LONG, ssm.get(valueCount + 1)/*EXTRA*/);
        assertEquals(prefix, NULL_LONG, ssm.get(Long.MAX_VALUE)/*EXTRA*/);
        ...
        try {
            ...
            try (final WritableLongChunk<?> keys = ssm.keyChunk();
                ...
                checkMap.forEach((key, count) -> {
                    assertEquals((long) key, keys.get(offset.get())/*EXTRA*/);
    ...
    private void checkIterator(final int nodeSize, final int valueCount) {
        ...
        try (final ValueIteratorOfLong it = ssm.iterator()) {
            ...
            for (int ii = 0; ii < valueCount; ++ii) {
                ...
                assertEquals(prefix, values[ii], it.nextLong()/*EXTRA*/);
            }
            ...
            for (int to = from; to <= valueCount + 3; ++to) {
                ...
                try (final ValueIteratorOfLong it = ssm.iterator(from, to)) {
                    ...
                    for (int ii = from; ii < to; ++ii) {
                        ...
                        assertEquals(message, reference.get(ii), it.nextLong()/*EXTRA*/);
                ...
                try (final ValueIteratorOfLong it = ssm.iterator(from, to);
                    ...
                    while (sliceIt.hasNext()) {
                        assertTrue(message, it.hasNext());
                        assertEquals(message, sliceIt.nextLong(), it.nextLong()/*EXTRA*/);
                    }
                    assertFalse(message, it.hasNext());
                }
            }
