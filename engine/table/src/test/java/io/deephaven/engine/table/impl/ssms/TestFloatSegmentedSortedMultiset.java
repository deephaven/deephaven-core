...
public class TestFloatSegmentedSortedMultiset extends RefreshingTableTestCase {
    ...
    public void testInsertIntoMiddleLeafSplit() {
        ...
        final float[] initial = new float[12];
        for (int ii = 0; ii < 12; ++ii) {
            initial[ii] = (float) ('a' + 2 * ii);
        }
        final FloatSegmentedSortedMultiset ssm = makeSsm(nodeSize, initial);

        try (final WritableFloatChunk<Values> values = WritableFloatChunk.makeWritableChunk(3);
                final WritableIntChunk<ChunkLengths> counts = WritableIntChunk.makeWritableChunk(3)) {
            values.set(0, (float) ('a' + 1));
            values.set(1, (float) ('a' + 3));
            values.set(2, (float) ('a' + 5));
            counts.set(0, 1);
            counts.set(1, 1);
            counts.set(2, 1);
        ...
        final float[] expected = new float[] {
                ...
                (float) ('a' + 5), (float) ('a' + 6), (float) ('a' + 8), (float) ('a' + 10), (float) ('a' + 12),
                (float) ('a' + 14), (float) ('a' + 16), (float) ('a' + 18), (float) ('a' + 20), (float) ('a' + 22)};
    ...
    public void testRemoveMaxMultiLeaf() {
        ...
        final FloatSegmentedSortedMultiset source = makeSsm(nodeSize, new float[] {
                (float) ('a' + 0), (float) ('a' + 1), (float) ('a' + 2), (float) ('a' + 3), (float) ('a' + 4)});
        final FloatSegmentedSortedMultiset dest =
                makeSsm(nodeSize, new float[] {(float) ('a' + 4), (float) ('a' + 5)});
        source.moveBackToFront(dest, 1);
        verifySsm(source, new float[] {(float) ('a' + 0), (float) ('a' + 1), (float) ('a' + 2), (float) ('a' + 3)}, desc);
        verifySsm(dest, new float[] {(float) ('a' + 4), (float) ('a' + 4), (float) ('a' + 5)}, desc);
    }
    ...
    public void testRemoveMaxSizeOne() {
        ...
        final FloatSegmentedSortedMultiset source =
                makeSsm(nodeSize, new float[] {(float) ('a' + 3), (float) ('a' + 4)});
        final FloatSegmentedSortedMultiset dest1 =
                makeSsm(nodeSize, new float[] {(float) ('a' + 4), (float) ('a' + 5)});
        source.moveBackToFront(dest1, 1);
        verifySsm(source, new float[] {(float) ('a' + 3)}, desc);
        verifySsm(dest1, new float[] {(float) ('a' + 4), (float) ('a' + 4), (float) ('a' + 5)}, desc);

        // removing the maximum of that size-1 directory clears the set (removeMax's size == 1 branch)
        final FloatSegmentedSortedMultiset dest2 =
                makeSsm(nodeSize, new float[] {(float) ('a' + 3), (float) ('a' + 6)});
        source.moveBackToFront(dest2, 1);
        verifySsm(source, new float[0], desc);
        verifySsm(dest2, new float[] {(float) ('a' + 3), (float) ('a' + 3), (float) ('a' + 6)}, desc);
    }
    ...
    public void testMoveFrontToBackPartialAppend() {
        ...
        {
            final FloatSegmentedSortedMultiset source =
                    makeSsm(nodeSize, new float[] {(float) ('a' + 2), (float) ('a' + 3)}, new int[] {2, 2});
            final FloatSegmentedSortedMultiset dest =
                    makeSsm(nodeSize, new float[] {(float) ('a' + 0), (float) ('a' + 1)});
            source.moveFrontToBack(dest, 1);
            verifySsm(source, new float[] {(float) ('a' + 2), (float) ('a' + 3), (float) ('a' + 3)}, desc);
            verifySsm(dest, new float[] {(float) ('a' + 0), (float) ('a' + 1), (float) ('a' + 2)}, desc);
        }
        ...
        {
            final FloatSegmentedSortedMultiset source =
                    makeSsm(nodeSize, new float[] {(float) ('a' + 5), (float) ('a' + 6)}, new int[] {2, 2});
            final FloatSegmentedSortedMultiset dest = makeSsm(nodeSize, new float[] {
                    (float) ('a' + 0), (float) ('a' + 1), (float) ('a' + 2), (float) ('a' + 3), (float) ('a' + 4)});
            source.moveFrontToBack(dest, 1);
            verifySsm(source, new float[] {(float) ('a' + 5), (float) ('a' + 6), (float) ('a' + 6)}, desc);
            verifySsm(dest, new float[] {(float) ('a' + 0), (float) ('a' + 1), (float) ('a' + 2), (float) ('a' + 3),
                    (float) ('a' + 4), (float) ('a' + 5)}, desc);
        }
    }
    ...
    public void testMoveBackToFrontCompleteLeaves() {
        ...
        final FloatSegmentedSortedMultiset dest =
                makeSsm(nodeSize, new float[] {(float) ('a' + 9), (float) ('a' + 10)});
        source.moveBackToFront(dest, 6);
        verifySsm(source,
                new float[] {(float) ('a' + 0), (float) ('a' + 1), (float) ('a' + 2), (float) ('a' + 3)}, desc);
        verifySsm(dest, new float[] {(float) ('a' + 3), (float) ('a' + 4), (float) ('a' + 5), (float) ('a' + 6),
    ...
    /**
     * {@link FloatSegmentedSortedMultiset#subVector} is exclusive at its end, and -- like every Vector -- must accept
     * offsets outside {@code [0, size())}, which read as the null value rather than throwing. Pin every representation
     * against {@link FloatVectorDirect}, the reference implementation of that contract.
     */
    @Test
    ...
    private void checkPartialCopy(final int nodeSize, final int valueCount) {
        ...
        final FloatVector reference = new FloatVectorDirect(values);
        ...
        assertEquals(prefix, NULL_FLOAT, ssm.get(-1), .000001f);
        assertEquals(prefix, NULL_FLOAT, ssm.get(valueCount), .000001f);
        assertEquals(prefix, NULL_FLOAT, ssm.get(valueCount + 1), .000001f);
        assertEquals(prefix, NULL_FLOAT, ssm.get(Long.MAX_VALUE), .000001f);
        ...
        for (int from = -3; from <= valueCount + 3; ++from) {
            for (int to = from; to <= valueCount + 3; ++to) {
                ...
                final FloatVector expected = reference.subVector(from, to);
                final FloatVector actual = ssm.subVector(from, to);
        ...
        try {
            ...
            try (final WritableFloatChunk<?> keys = ssm.keyChunk();
                ...
                checkMap.forEach((key, count) -> {
                    assertEquals((float) key, keys.get(offset.get()), .000001f);
    ...
    private void checkIterator(final int nodeSize, final int valueCount) {
        ...
        try (final ValueIteratorOfFloat it = ssm.iterator()) {
            ...
            for (int ii = 0; ii < valueCount; ++ii) {
                ...
                assertEquals(prefix, values[ii], it.nextFloat(), .000001f);
            }
            ...
            for (int to = from; to <= valueCount + 3; ++to) {
                ...
                try (final ValueIteratorOfFloat it = ssm.iterator(from, to)) {
                    ...
                    for (int ii = from; ii < to; ++ii) {
                        ...
                        assertEquals(message, reference.get(ii), it.nextFloat(), .000001f);
                ...
                try (final ValueIteratorOfFloat it = ssm.iterator(from, to);
                     final ValueIteratorOfFloat sliceIt = ssm.subVector(from, to).iterator()) {
                    while (sliceIt.hasNext()) {
                        assertTrue(message, it.hasNext());
                        assertEquals(message, sliceIt.nextFloat(), it.nextFloat(), .000001f);
                    }
                    assertFalse(message, it.hasNext());
                }
            }
