...
public abstract class TstRegionedColumnSourcePrimitive<DATA_TYPE, ATTR extends Values, REGION_TYPE extends ColumnRegion<ATTR>, CS_REGION_TYPE extends ColumnRegion<ATTR>> {
    ...
    public void setUp() throws Exception {
        ...
        manager = jmock.mock(RegionedColumnSourceManager.class, "manager");

        // noinspection unchecked
        cr = (REGION_TYPE[]) Array.newInstance(regionTypeClass, 10);
        for (int cri = 0; cri < cr.length; ++cri) {
            // noinspection unchecked
            cr[cri] = (REGION_TYPE) jmock.mock(regionTypeClass, "CR_" + cri);
        }
    ...
    public void testAddRegions() {
        ...
        assertEquals(cr[0], doLookupRegion(getFirstRowKey(0)));
        assertEquals(cr[0], doLookupRegion(getLastRowKey(0)));

        // Add the 1st region.
        SUT.addRegionForUnitTests(cr[1]);
        assertEquals(cr[1], doLookupRegion(getFirstRowKey(1)));
        assertEquals(cr[1], doLookupRegion(getLastRowKey(1)));

        // Prove that the 2nd region is missing.
        try {
            assertNull(doLookupRegion(getFirstRowKey(2)));
        } catch (ArrayIndexOutOfBoundsException expected) {
        }
        try {
            assertNull(doLookupRegion(getLastRowKey(2)));
        } catch (ArrayIndexOutOfBoundsException expected) {
        }

        // Prove that 9th region is missing.
        try {
            assertNull(doLookupRegion(getFirstRowKey(9)));
        } catch (ArrayIndexOutOfBoundsException expected) {
        }
        try {
            assertNull(doLookupRegion(getLastRowKey(9)));
        } catch (ArrayIndexOutOfBoundsException expected) {
        }
    }
