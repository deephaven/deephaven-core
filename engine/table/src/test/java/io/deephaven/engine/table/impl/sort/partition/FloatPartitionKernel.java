...
public class FloatPartitionKernel {
    public static class PartitionKernelContext implements SafeCloseable {
        ...
        // pivotValues.size()).mapToObj(pivotValues::get).map(FloatPartitionKernel::format).collect(Collectors.joining(","))
        // + "]");
        // System.out.println("[" + IntStream.range(0,
        ...
        public FloatLongTuple[] getPivots() {
            return IntStream.range(0, pivotValues.size())
                    .mapToObj(ii -> new FloatLongTuple(pivotValues.get(ii), pivotKeys.get(ii)))
                    .toArray(FloatLongTuple[]::new);
        }
    ...
    public static PartitionKernelContext createContext(RowSet rowSet, ColumnSource<Float> columnSource,
    ...
    private static void samplePivots(RowSet rowSet, int nPartitions, WritableLongChunk<RowKeys> pivotKeys,
            WritableFloatChunk<Any> pivotValues, ColumnSource<Float> columnSource) {
        pivotKeys.setSize(0);
     ...
     */
    public static void partition(PartitionKernelContext context, LongChunk<RowKeys> indexKeys, FloatChunk values) {
    ...
    private static int binarySearchTieIndex(FloatChunk pivotValues, LongChunk<RowKeys> pivotKeys, int lo, int hi,
            float searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final float compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
