...
public class IntColumnBinarySearchKernelTest {
    ...
    public void testMatchMultipleValuesIncludingNull() {
        ...
        assertMatchPositions(ascending, new Integer[] {(int) 5, NULL_INT}, List.of(0L, 3L, 4L));
        assertMatchPositions(ascending, new Integer[] {NULL_INT, (int) 5}, List.of(0L, 3L, 4L));

        // Several ordinary values plus the sentinel, listed out of order.
        assertMatchPositions(ascending, new Integer[] {(int) 9, (int) 1, NULL_INT}, List.of(0L, 1L, 5L));

        // The sentinel on its own, and ordinary values without it.
        assertMatchPositions(ascending, new Integer[] {NULL_INT}, List.of(0L));
        assertMatchPositions(ascending, new Integer[] {(int) 1, (int) 9}, List.of(1L, 5L));

        // A value absent from the data, between two that are present.
        assertMatchPositions(ascending, new Integer[] {(int) 1, (int) 3, (int) 9}, List.of(1L, 5L));

        // Nothing present at all.
        assertMatchPositions(ascending, new Integer[] {(int) 3, (int) 7}, List.of());
    }
    ...
    private static void assertMatchPositions(
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            try (final RowSet selection = RowSetFactory.fromRange(0, data.size() - 1);
                    final RowSet matched = IntColumnBinarySearchKernel.binarySearchMatch(
