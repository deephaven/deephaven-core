...
public abstract class AbstractDoubleColumnSourceTest {
    ...
    private void testGet(Random random, int chunkSize) {
        ...
        final DoubleChunk<? extends Values> result =
                source.getChunk(getContext, RowSetFactory.fromRange(0, 1023)).asDoubleChunk();
        for (int ii = 0; ii < 1024; ++ii) {
    ...
    private void checkRandomFill(int chunkSize, WritableColumnSource<Double> source,
    ...
    private void checkRangeFill(int chunkSize, WritableColumnSource<Double> source,
            ColumnSource.FillContext fillContext,
    ...
    private void checkRangeGet(int chunkSize, ColumnSource<Double> source, ColumnSource.GetContext getContext,
            double[] expectations, int firstKey, int lastKey, boolean usePrev) {
        int offset;
        final RowSet rowSet = RowSetFactory.fromRange(firstKey, lastKey);
        offset = firstKey;
        for (final RowSequence.Iterator it = rowSet.getRowSequenceIterator(); it.hasMore();) {
    ...
    private void checkRangeResults(double[] expectations, int offset, RowSequence nextOk,
            DoubleChunk<? extends Values> result) {
