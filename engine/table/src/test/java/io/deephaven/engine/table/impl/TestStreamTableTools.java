...
public class TestStreamTableTools {
    ...
    public void testStreamToAppendOnlyTable() {
        ...
        UpdateGraphProcessor.DEFAULT.runWithinUnitTestCycle(() -> {
            ...
            TstUtils.addToTable(streamTable, i(7), intCol("I", 1), doubleCol("D", Math.PI), instantCol("DT", dt2),
                    col("B", true));
        ...
        UpdateGraphProcessor.DEFAULT.runWithinUnitTestCycle(() -> {
            RowSet removed = streamTable.getRowSet().copyPrev();
            ((WritableRowSet) streamTable.getRowSet()).clear();
            TstUtils.addToTable(streamTable, i(7), intCol("I", 2), doubleCol("D", Math.E), instantCol("DT", dt3),
                    col("B", false));
            streamTable.notifyListeners(i(7), removed, i());
        });
        assertTableEquals(
