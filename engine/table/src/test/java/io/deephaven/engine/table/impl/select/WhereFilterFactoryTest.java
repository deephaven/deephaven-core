...
public class SelectFilterFactoryTest extends RefreshingTableTestCase {
    ...
    public void testInComplex() {
        ...
        assertEquals(MatchFilter.class, SelectFilterFactory.getExpression("Opra in opra1, opra2, opra3").getClass());
        QueryScope.addParam("pmExpiry", "World");
        assertEquals(MatchFilter.class, SelectFilterFactory.getExpression("amExpiry = pmExpiry").getClass());

        QueryScope.addParam("amExpiry", "Hello");
        QueryScope.addParam("pmExpiry", "World");

        WhereFilter filter = SelectFilterFactory.getExpression("Maturity in amExpiry,pmExpiry , \"AGAIN\"  ");
        ...
        filter = SelectFilterFactory.getExpression("Maturity in amExpiry,1 , \"AGAIN\"  ");
        assertEquals(MatchFilter.class, filter.getClass());
    ...
    public void testIcase() {
        ...
        WhereFilter f = SelectFilterFactory.getExpression("Opra in `opra1`, `opra2`, `opra3`,`opra4`");
    ...
    public void testBigIn() {
        final WhereFilter f = SelectFilterFactory.getExpression(
    ...
    public void testInDateTimes() {
        ...
        // match one item
        WhereFilter f = SelectFilterFactory.getExpression("Timestamp in '" + mon + "'");
        f.init(t.getDefinition());
        assertEquals(MatchFilter.class, f.getClass());
        RowSet idx = f.filter(t.getRowSet().copy(), t.getRowSet(), t, false);
        assertEquals(1, idx.size());
    ...
    public void testIncludesMatcher() {
        ...
        WhereFilter f = SelectFilterFactory.getExpression("Phrase icase includes any `T1`, `T2`, `T3`");
