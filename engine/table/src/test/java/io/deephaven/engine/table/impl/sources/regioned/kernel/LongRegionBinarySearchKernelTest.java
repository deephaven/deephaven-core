...
public class LongRegionBinarySearchKernelTest {
    ...
    private void randomizedTestRunner(
        ...
        for (int ii = 0; ii < size; ++ii) {
            ...
            try (final RowSet matchRs = LongRegionBinarySearchKernel.binarySearchMatch(
                    region, startRow, endRow, sortColumn, new Long[] {value});
                    final RowSet minMaxRs = LongRegionBinarySearchKernel.binarySearchMinMax(
        ...
        for (Long missingValue : missingValues) {
            ...
            try (final RowSet valuesFound = LongRegionBinarySearchKernel.binarySearchMatch(
                    region, startRow, endRow, sortColumn, new Long[] {missingValue})) {
                assertTrue(valuesFound.isEmpty());
            }
            try (final RowSet valuesFound = LongRegionBinarySearchKernel.binarySearchMinMax(
    ...
    private static final List<Long> DISPATCH_DATA =
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            for (final boolean lowerInc : new boolean[] {false, true}) {
                for (final boolean upperInc : new boolean[] {false, true}) {
                    try (final RowSet viaFilter = LongRegionBinarySearchKernel.binsearchRangeFilter(region, 0, lastKey,
                            sortColumn, new LongRangeFilter("test", (long) 2, (long) 5, lowerInc, upperInc));
                            final RowSet viaSearch = LongRegionBinarySearchKernel.binarySearchMinMax(region, 0, lastKey,
                                    sortColumn, (long) 2, (long) 5, lowerInc, upperInc)) {
            ...
            try (final RowSet viaFilter = LongRegionBinarySearchKernel.binsearchRangeFilter(region, 0, lastKey,
                    sortColumn, new LongRangeFilter("test", NULL_LONG, (long) 5, true, true));
                    final RowSet viaSearch = LongRegionBinarySearchKernel.binarySearchMax(region, 0, lastKey,
                            sortColumn, (long) 5, true)) {
            ...
            try (final RowSet viaFilter = LongRegionBinarySearchKernel.binsearchRangeFilter(region, 0, lastKey,
                    sortColumn, new LongRangeFilter("test", (long) 2, MAX_LONG, true, true));
                    final RowSet viaSearch = LongRegionBinarySearchKernel.binarySearchMin(region, 0, lastKey,
                            sortColumn, (long) 2, true)) {
                assertEquals("descending=" + descending, viaSearch, viaFilter);
            }
        }
    }
    ...
    public void testMatchFilterEntryPointDispatch() {
        final ColumnRegionLong<Values> region = makeColumnRegionLong(DISPATCH_DATA);
        final long lastKey = DISPATCH_DATA.size() - 1;
        final SortColumn sortColumn = SortColumn.asc(ColumnName.of("test"));
        final TableDefinition tableDefinition = TableDefinition.of(ColumnDefinition.ofLong("test"));

        final MatchFilter matchFilter = new MatchFilter(MatchOptions.REGULAR, "test", (long) 5, NULL_LONG);
        matchFilter.init(tableDefinition);
        try (final RowSet viaFilter = LongRegionBinarySearchKernel.binsearchMatchFilter(region, 0, lastKey, sortColumn,
                matchFilter);
                final RowSet viaSearch = LongRegionBinarySearchKernel.binarySearchMatch(region, 0, lastKey, sortColumn,
                        new Object[] {(long) 5, NULL_LONG})) {
    ...
    public void testMatchMultipleValuesIncludingNull() {
        ...
        assertMatchKeys(ascending, new Long[] {(long) 5, NULL_LONG}, List.of(0L, 3L, 4L));
        assertMatchKeys(ascending, new Long[] {NULL_LONG, (long) 5}, List.of(0L, 3L, 4L));
        assertMatchKeys(ascending, new Long[] {(long) 9, (long) 1, NULL_LONG}, List.of(0L, 1L, 5L));
        assertMatchKeys(ascending, new Long[] {NULL_LONG}, List.of(0L));
        assertMatchKeys(ascending, new Long[] {(long) 1, (long) 9}, List.of(1L, 5L));
        assertMatchKeys(ascending, new Long[] {(long) 1, (long) 3, (long) 9}, List.of(1L, 5L));
        assertMatchKeys(ascending, new Long[] {(long) 3, (long) 7}, List.of());
    }
    ...
    private static void assertMatchKeys(
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            try (final RowSet matched = LongRegionBinarySearchKernel.binarySearchMatch(
