        ...
        try (final WritableShortChunk<Values> allValues = WritableShortChunk.makeWritableChunk(totalSize);
            ...
            for (int chunk = 0; chunk < chunkCount; ++chunk) {
                ...
                try (final WritableShortChunk<Values> valuesChunk = WritableShortChunk.makeWritableChunk(chunkSize);
                    ...
                    if (ascending) {
                        ShortLongMegaMergeKernel.merge(keySource, valuesSource, 0, sizeAfterAddition - chunkSize,
                                keysChunk, valuesChunk);
                    } else {
                        ShortLongMegaMergeDescendingKernel.merge(keySource, valuesSource, 0,
                ...
                try (final ChunkSource.GetContext valueContext = valuesSource.makeGetContext(sizeAfterAddition);
                    ...
                    final ShortChunk<Values> checkValues =
                            valuesSource.getChunk(valueContext, rowSequence).asShortChunk();
