...
public class TestByteSegmentedSortedArray extends RefreshingTableTestCase {
    ...
    private void testShifts(@NotNull final SsaTestHelpers.TestDescriptor desc) {
        ...
        try (final SafeCloseable ignored = LivenessScopeStack.open(new LivenessScope(true), true)) {
            final TableUpdateListener asByteListener =
                    new InstrumentedTableUpdateListenerAdapter(asByte, false) {
                        @Override
                        public void onUpdate(TableUpdate upstream) {
                            try (final ColumnSource.GetContext checkContext =
                                    valueSource.makeGetContext(asByte.getRowSet().intSizePrev());
                                    final RowSet relevantIndices = asByte.getRowSet().copyPrev()) {
                                checkSsa(ssa, valueSource.getPrevChunk(checkContext, relevantIndices).asByteChunk(),
                            ...
                            try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(size);
                                ...
                                if (takeout.isNonempty()) {
                                    final ByteChunk<? extends Values> valuesToRemove =
                                            valueSource.getPrevChunk(getContext, takeout).asByteChunk();
                                ...
                                if (upstream.shifted().nonempty()) {
                                    ...
                                    while (sit.hasNext()) {
                                        ...
                                        try (final RowSet prevRowSet = table.getRowSet().copyPrev();
                                            ...
                                            final ByteChunk<? extends Values> shiftValues =
                                                    valueSource.getPrevChunk(getContext, rowSetToShift).asByteChunk();
                                ...
                                try (final RowSet putin = upstream.added().union(upstream.modified())) {
                                    ...
                                    try (final ColumnSource.GetContext checkContext =
                                            valueSource.makeGetContext(asByte.intSize());
                                            final RowSet relevantIndices = asByte.getRowSet().minus(putin)) {
                                        checkSsa(ssa, valueSource.getChunk(checkContext, relevantIndices).asByteChunk(),
                                                relevantIndices.asRowKeyChunk(), desc);
                                    }

                                    if (putin.isNonempty()) {
                                        final ByteChunk<? extends Values> valuesToInsert =
                                                valueSource.getChunk(getContext, putin).asByteChunk();
            ...
            while (desc.advance(50)) {
                ...
                try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(asByte.intSize());
                    ...
                    checkSsa(ssa, valueSource.getChunk(getContext, asByteRowSetCopy).asByteChunk(),
                            asByteRowSetCopy.asRowKeyChunk(), desc);
                }
            }
        }
    }
    ...
    private void testUpdates(@NotNull final SsaTestHelpers.TestDescriptor desc, boolean allowAddition,
        ...
        try (final SafeCloseable ignored = LivenessScopeStack.open(new LivenessScope(true), true)) {
            final ShiftObliviousListener asByteListener =
                    new ShiftObliviousInstrumentedListenerAdapter(asByte, false) {
                        @Override
                        public void onUpdate(RowSet added, RowSet removed, RowSet modified) {
                            try (final ColumnSource.GetContext getContext =
                                ...
                                if (removed.isNonempty()) {
                                    final ByteChunk<? extends Values> valuesToRemove =
                                            valueSource.getPrevChunk(getContext, removed).asByteChunk();
                                    ssa.remove(valuesToRemove, removed.asRowKeyChunk());
                                }
                                if (added.isNonempty()) {
                                    ssa.insert(valueSource.getChunk(getContext, added).asByteChunk(),
                                            added.asRowKeyChunk());
                                }
                            }
                        }
    ...
    private void checkSsaInitial(Table asByte, ByteSegmentedSortedArray ssa, ColumnSource<?> valueSource,
        ...
        try (final ColumnSource.GetContext getContext = valueSource.makeGetContext(asByte.intSize());
                final RowSet asByteRowSetCopy = asByte.getRowSet().copy()) {
            final ByteChunk<? extends Values> valueChunk =
                    valueSource.getChunk(getContext, asByteRowSetCopy).asByteChunk();
    ...
    private void checkSsa(ByteSegmentedSortedArray ssa, ByteChunk<? extends Values> valueChunk,
