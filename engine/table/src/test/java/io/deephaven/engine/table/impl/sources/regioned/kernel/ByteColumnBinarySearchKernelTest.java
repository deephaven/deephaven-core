...
public class ByteColumnBinarySearchKernelTest {
    ...
    public void testBoundsAscending() {
        ...
        final List<Byte> data = List.of(
                (byte) 1, (byte) 2, (byte) 3, (byte) 3, (byte) 3,
        ...
        try {
            ...
            assertEquals(5, ByteColumnBinarySearchKernel.lowerBoundAscending(
    ...
    public void testBoundsDescending() {
        ...
        final List<Byte> data = List.of(
                (byte) 5, (byte) 5, (byte) 5, (byte) 3, (byte) 3, (byte) 3,
                (byte) 2, (byte) 1);
        final ByteChunkColumnSource source = makeChunkColumnSource(data);
        final RowSet selection = RowSetFactory.fromRange(0, data.size() - 1);
        final long lastPos = data.size() - 1;
        ...
        try {
            ...
            assertEquals(7, ByteColumnBinarySearchKernel.lowerBoundDescending(
                    source, selection, 0, lastPos, (byte) 1, true, false));
            ...
            assertEquals(2, ByteColumnBinarySearchKernel.upperBoundDescending(
                    source, selection, 0, lastPos, (byte) 5, true, false));
        } finally {
            selection.close();
            source.clear(true);
    ...
    public void testMatchMultipleValuesIncludingNull() {
        ...
        assertMatchPositions(ascending, new Byte[] {(byte) 5, NULL_BYTE}, List.of(0L, 3L, 4L));
        assertMatchPositions(ascending, new Byte[] {NULL_BYTE, (byte) 5}, List.of(0L, 3L, 4L));

        // Several ordinary values plus the sentinel, listed out of order.
        assertMatchPositions(ascending, new Byte[] {(byte) 9, (byte) 1, NULL_BYTE}, List.of(0L, 1L, 5L));

        // The sentinel on its own, and ordinary values without it.
        assertMatchPositions(ascending, new Byte[] {NULL_BYTE}, List.of(0L));
        assertMatchPositions(ascending, new Byte[] {(byte) 1, (byte) 9}, List.of(1L, 5L));

        // A value absent from the data, between two that are present.
        assertMatchPositions(ascending, new Byte[] {(byte) 1, (byte) 3, (byte) 9}, List.of(1L, 5L));

        // Nothing present at all.
        assertMatchPositions(ascending, new Byte[] {(byte) 3, (byte) 7}, List.of());
    }
    ...
    private static void assertMatchPositions(
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            try (final RowSet selection = RowSetFactory.fromRange(0, data.size() - 1);
                    final RowSet matched = ByteColumnBinarySearchKernel.binarySearchMatch(
