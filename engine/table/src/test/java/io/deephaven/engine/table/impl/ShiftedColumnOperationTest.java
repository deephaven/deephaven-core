...
public class ShiftedColumnOperationTest {
    ...
    private void testSimpleMatchPair(final int shiftConst) {
        ...
        assertTableEquals(UpdateContext.sharedLock().computeLocked(() -> table.update(minusConstCols)),
                shiftMinusConst);

        String[] plusConstCols =
                new String[] {"CV2=Value_[i+" + shiftConst + "]", "CV22=Value2_[i+" + shiftConst + "]"};
        assertTableEquals(UpdateContext.sharedLock().computeLocked(() -> table.update(plusConstCols)),
                shiftPlusConst);
    ...
    private void testSimpleFillChunkSingleColumnChanges(final int shiftConst) {
        ...
        assertTableEquals(UpdateContext.sharedLock()
    ...
    public void testAddRemove() {
        ...
        assertTableEquals(
                UpdateContext.sharedLock().computeLocked(() -> table.update("V3 = Value_[i - 1]")),
                shiftMinusOne);
        assertTableEquals(
                UpdateContext.sharedLock().computeLocked(() -> table.update("V2 = Value_[i + 1]")),
                shiftPlusOne);
        assertTableEquals(UpdateContext.sharedLock()
                .computeLocked(() -> table.update("CV3 = Value_[i - " + shiftConst + "]")), shiftMinusConst);
        assertTableEquals(UpdateContext.sharedLock()
    ...
    public void testSimpleAddMultipleColumnChanges() {
        ...
        assertTableEquals(
                UpdateContext.sharedLock().computeLocked(
    ...
    public void testBigShift() {
        ...
        final Table shifted = UpdateContext.sharedLock()
                .computeLocked(() -> ShiftedColumnOperation.addShiftedColumns(table, -1, "VS=Value"));
        TableTools.showWithRowSet(shifted);

        assertTableEquals(UpdateContext.sharedLock().computeLocked(() -> table.update("VS=Value_[i-1]")),
                shifted);
        ...
        assertTableEquals(UpdateContext.sharedLock().computeLocked(() -> table.update("VS=Value_[i-1]")), shifted);
        pl.stop();
    }
