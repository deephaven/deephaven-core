...
public class CharRegionBinarySearchKernelTest {
    ...
    private static final List<Character> DISPATCH_DATA =
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            for (final boolean lowerInc : new boolean[] {false, true}) {
                for (final boolean upperInc : new boolean[] {false, true}) {
                    try (final RowSet viaFilter = CharRegionBinarySearchKernel.binsearchRangeFilter(region, 0, lastKey,
                            sortColumn, new CharRangeFilter("test", (char) 2, (char) 5, lowerInc, upperInc));
                            final RowSet viaSearch = CharRegionBinarySearchKernel.binarySearchMinMax(region, 0, lastKey,
                                    sortColumn, (char) 2, (char) 5, lowerInc, upperInc)) {
            ...
            try (final RowSet viaFilter = CharRegionBinarySearchKernel.binsearchRangeFilter(region, 0, lastKey,
                    sortColumn, new CharRangeFilter("test", NULL_CHAR, (char) 5, true, true));
                    final RowSet viaSearch = CharRegionBinarySearchKernel.binarySearchMax(region, 0, lastKey,
                            sortColumn, (char) 5, true)) {
            ...
            try (final RowSet viaFilter = CharRegionBinarySearchKernel.binsearchRangeFilter(region, 0, lastKey,
                    sortColumn, new CharRangeFilter("test", (char) 2, MAX_CHAR, true, true));
                    final RowSet viaSearch = CharRegionBinarySearchKernel.binarySearchMin(region, 0, lastKey,
                            sortColumn, (char) 2, true)) {
                assertEquals("descending=" + descending, viaSearch, viaFilter);
            }
        }
    }
    ...
    public void testMatchFilterEntryPointDispatch() {
        final ColumnRegionChar<Values> region = makeColumnRegionChar(DISPATCH_DATA);
        final long lastKey = DISPATCH_DATA.size() - 1;
        final SortColumn sortColumn = SortColumn.asc(ColumnName.of("test"));
        final TableDefinition tableDefinition = TableDefinition.of(ColumnDefinition.ofChar("test"));

        final MatchFilter matchFilter = new MatchFilter(MatchOptions.REGULAR, "test", (char) 5, NULL_CHAR);
        matchFilter.init(tableDefinition);
        try (final RowSet viaFilter = CharRegionBinarySearchKernel.binsearchMatchFilter(region, 0, lastKey, sortColumn,
                matchFilter);
                final RowSet viaSearch = CharRegionBinarySearchKernel.binarySearchMatch(region, 0, lastKey, sortColumn,
                        new Object[] {(char) 5, NULL_CHAR})) {
    ...
    public void testMatchMultipleValuesIncludingNull() {
        ...
        assertMatchKeys(ascending, new Character[] {(char) 5, NULL_CHAR}, List.of(0L, 3L, 4L));
        assertMatchKeys(ascending, new Character[] {NULL_CHAR, (char) 5}, List.of(0L, 3L, 4L));
        assertMatchKeys(ascending, new Character[] {(char) 9, (char) 1, NULL_CHAR}, List.of(0L, 1L, 5L));
        assertMatchKeys(ascending, new Character[] {NULL_CHAR}, List.of(0L));
        assertMatchKeys(ascending, new Character[] {(char) 1, (char) 9}, List.of(1L, 5L));
        assertMatchKeys(ascending, new Character[] {(char) 1, (char) 3, (char) 9}, List.of(1L, 5L));
        assertMatchKeys(ascending, new Character[] {(char) 3, (char) 7}, List.of());
    }
    ...
    private static void assertMatchKeys(
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            try (final RowSet matched = CharRegionBinarySearchKernel.binarySearchMatch(
