...
public class ObjectColumnBinarySearchKernelTest {
    ...
    /**
     * The search navigates by {@code ObjectComparisons.compare} but a match is decided by
     * {@code ObjectComparisons.eq}, and for a type whose natural ordering is inconsistent with equals those disagree:
     * {@code new BigDecimal("1.0").compareTo(new BigDecimal("1.00")) == 0} while the two are not equal. Ordering-equal
     * values therefore share a contiguous run, and only some members of that run may match. Exactly the equal ones
     * must be returned -- the relation the chunk filter this feeds uses -- whichever member the search happens to land
     * on.
     *
     * <p>
        ...
        for (final boolean descending : new boolean[] {false, true}) {
            ...
            try (final RowSet selection = RowSetFactory.fromRange(0, data.size() - 1);
                    final RowSet matched = ObjectColumnBinarySearchKernel.binarySearchMatch(
                            source, selection, sortColumn, new Object[] {toFind}, false)) {
