...
public abstract class AbstractIntegerColumnSourceTest {
    ...
    private void testGet(Random random, int chunkSize) {
        ...
        final IntChunk<? extends Values> result =
                source.getChunk(getContext, RowSetFactory.fromRange(0, 1023)).asIntChunk();
        for (int ii = 0; ii < 1024; ++ii) {
    ...
    private void checkRandomFill(int chunkSize, WritableColumnSource<Integer> source,
    ...
    private void checkRangeFill(int chunkSize, WritableColumnSource<Integer> source,
            ColumnSource.FillContext fillContext,
    ...
    private void checkRangeGet(int chunkSize, ColumnSource<Integer> source, ColumnSource.GetContext getContext,
            int[] expectations, int firstKey, int lastKey, boolean usePrev) {
        int offset;
        final RowSet rowSet = RowSetFactory.fromRange(firstKey, lastKey);
        offset = firstKey;
        for (final RowSequence.Iterator it = rowSet.getRowSequenceIterator(); it.hasMore();) {
    ...
    private void checkRangeResults(int[] expectations, int offset, RowSequence nextOk,
            IntChunk<? extends Values> result) {
