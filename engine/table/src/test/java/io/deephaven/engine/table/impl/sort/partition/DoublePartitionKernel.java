...
public class DoublePartitionKernel {
    public static class PartitionKernelContext implements SafeCloseable {
        ...
        // pivotValues.size()).mapToObj(pivotValues::get).map(DoublePartitionKernel::format).collect(Collectors.joining(","))
        // + "]");
        // System.out.println("[" + IntStream.range(0,
        ...
        public DoubleLongTuple[] getPivots() {
            return IntStream.range(0, pivotValues.size())
                    .mapToObj(ii -> new DoubleLongTuple(pivotValues.get(ii), pivotKeys.get(ii)))
                    .toArray(DoubleLongTuple[]::new);
        }
    ...
    public static PartitionKernelContext createContext(RowSet rowSet, ColumnSource<Double> columnSource,
    ...
    private static void samplePivots(RowSet rowSet, int nPartitions, WritableLongChunk<RowKeys> pivotKeys,
            WritableDoubleChunk<Any> pivotValues, ColumnSource<Double> columnSource) {
        pivotKeys.setSize(0);
    ...
    private static int binarySearchTieIndex(DoubleChunk pivotValues, LongChunk<RowKeys> pivotKeys, int lo, int hi,
            double searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final double compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
