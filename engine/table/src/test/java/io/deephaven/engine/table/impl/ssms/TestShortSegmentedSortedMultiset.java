...
public class TestShortSegmentedSortedMultiset extends RefreshingTableTestCase {
    ...
    private void checkPartialCopy(final int nodeSize, final int valueCount) {
        ...
        assertEquals(prefix, NULL_SHORT, ssm.get(-1)/*EXTRA*/);
        assertEquals(prefix, NULL_SHORT, ssm.get(valueCount)/*EXTRA*/);
        assertEquals(prefix, NULL_SHORT, ssm.get(valueCount + 1)/*EXTRA*/);
        assertEquals(prefix, NULL_SHORT, ssm.get(Long.MAX_VALUE)/*EXTRA*/);
        ...
        try {
            ...
            try (final WritableShortChunk<?> keys = ssm.keyChunk();
                ...
                checkMap.forEach((key, count) -> {
                    assertEquals((short) key, keys.get(offset.get())/*EXTRA*/);
    ...
    private void checkIterator(final int nodeSize, final int valueCount) {
        ...
        try (final ValueIteratorOfShort it = ssm.iterator()) {
            ...
            for (int ii = 0; ii < valueCount; ++ii) {
                ...
                assertEquals(prefix, values[ii], it.nextShort()/*EXTRA*/);
            }
            ...
            for (int to = from; to <= valueCount + 3; ++to) {
                ...
                try (final ValueIteratorOfShort it = ssm.iterator(from, to)) {
                    ...
                    for (int ii = from; ii < to; ++ii) {
                        ...
                        assertEquals(message, reference.get(ii), it.nextShort()/*EXTRA*/);
                ...
                try (final ValueIteratorOfShort it = ssm.iterator(from, to);
                    ...
                    while (sliceIt.hasNext()) {
                        assertTrue(message, it.hasNext());
                        assertEquals(message, sliceIt.nextShort(), it.nextShort()/*EXTRA*/);
                    }
                    assertFalse(message, it.hasNext());
                }
            }
