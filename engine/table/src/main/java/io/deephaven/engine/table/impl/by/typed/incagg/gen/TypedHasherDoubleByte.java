...
public final class TypedHasherDoubleByte extends IncrementalChunkedOperatorAggregationStateManagerTypedBase {
  private final DoubleArraySource keySource0;

  private final DoubleArraySource overflowKeySource0;

  private final ByteArraySource keySource1;

  private final ByteArraySource overflowKeySource1;

  public TypedHasherDoubleByte(ColumnSource[] tableKeySources, int tableSize,
      double maximumLoadFactor, double targetLoadFactor) {
    super(tableKeySources, tableSize, maximumLoadFactor, targetLoadFactor);
    this.keySource0 = (DoubleArraySource) super.keySources[0];
    this.overflowKeySource0 = (DoubleArraySource) super.overflowKeySources[0];
    this.keySource1 = (ByteArraySource) super.keySources[1];
  ...
  private int hash(double v0, byte v1) {
  ...
  private int maybeMoveMainBucket(HashHandler handler, int bucket, int destBucket,
    ...
    final byte v1 = keySource1.getUnsafe(bucket);
    final int hash = hash(v0, v1);
    ...
    if (location == bucket) {
      ...
      keySource1.set(bucket, QueryConstants.NULL_BYTE);
      handler.moveMain(bucket, destBucket);
    }
    return mainInsertLocation;
  }

  private boolean findOverflow(HashHandler handler, double v0, byte v1, int chunkPosition,
