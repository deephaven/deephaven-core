...
final class IncrementalAggOpenHasherWithTombstoneIntObject extends IncrementalChunkedOperatorAggregationStateManagerOpenAddressedBaseWithTombstones {
    ...
    public IncrementalAggOpenHasherWithTombstoneIntObject(ColumnSource[] tableKeySources,
    ...
    protected void maybeNullMain(RowSet rows) {
        this.mainKeySource1.setNull(rows);
    }

    @Override
    protected void maybeNullAlternate(final RowSet rows) {
        if (rows.isNonempty()) {
            this.alternateKeySource1.setNull(rows);
        }
    }
}
