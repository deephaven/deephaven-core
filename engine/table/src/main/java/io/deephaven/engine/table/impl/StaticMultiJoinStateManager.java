    ...
    // endregion extensions
{
     ...
     *
     * <p>When hashing a value, if HashCodes % 2^L < tableHashPivot; then the destination location is HashCodes % 2^L.
     * If HashCodes % 2^L >= tableHashPivot, then the destination location is HashCodes % 2^(L-1).  Once the pivot reaches
    ...
    StaticMultiJoinStateManager(ColumnSource<?>[] tableKeySources
                                         , int tableSize
    ...
    public void build(final Table table, ColumnSource<?>[] sources, int tableNumber) {
        ...
        try (final BuildContext bc = makeBuildContext(sources, table.size())) {
            buildTable(bc, table.getRowSet(), sources, tableNumber, sourceForTable);
        }
    }
            ...
            while (okIt.hasMore()) {
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        throw new IllegalStateException("Duplicate key found for " + ChunkUtils.extractKeyStringFromChunks(keyChunkTypes, sourceKeyChunks, chunkPosition) + " in table " + tableNumber + ".");
                ...
                for (int ii = 0; ii <= maxAddedPosition; ++ii) {
                    ...
                    if (longSlot != NULL_ROW_KEY) {
                        ...
                        addIndex(intSlot, tableNumber, resultSource, nextOutputPosition, keyToAdd);
        ...
        while (rehashRequired()) {
            ...
            for (int ii = 0; ii < bc.shouldMoveBucket.size(); ++ii) {
                if (bc.shouldMoveBucket.get(ii)) {
                    ...
                    resultSlot.set(oldHashLocation, EMPTY_RIGHT_VALUE);
    ...
    private void addIndex(long tableLocation, int tableNumber, LongArraySource resultSource, long outputPosition, long indexKey) {
        final long oldValue = resultSource.getAndSetUnsafe(outputPosition, indexKey);
