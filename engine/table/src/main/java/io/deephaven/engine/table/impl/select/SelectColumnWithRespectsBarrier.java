...
public class SelectColumnWithRespectsBarrier extends WrappedSelectColumn {
    ...
    public SelectColumnWithRespectsBarrier(SelectColumn wrapped, Object ... respectBarriers) {
        ...
        if (wrapped.respectedBarriers() != null && wrapped.respectedBarriers().length > 0) {
            this.respectedBarriers = Arrays.copyOf(wrapped.respectedBarriers(), wrapped.respectedBarriers().length + respectBarriers.length);
            System.arraycopy(respectBarriers, 0, this.respectedBarriers, wrapped.respectedBarriers().length, respectBarriers.length);
        } else {
