 ...
 */
public class Flat2DDoubleArraySource extends AbstractColumnSource<Double> implements ImmutableColumnSourceGetDefaults.ForDouble, WritableColumnSource<Double>, FillUnordered, InMemoryColumnSource, ChunkedBackingStoreExposedWritableSource {
    ...
    private static double [][] allocateArray(long size) {
        ...
        while (size > SEGMENT_SIZE) {
            data[segment++] = new double[SEGMENT_SIZE];
            size -= SEGMENT_SIZE;
        }
    ...
    private void fillChunkByRanges(WritableChunk<? super Values> destination, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                asDoubleChunk.copyFromTypedArray(data[segment], Math.toIntExact(start), srcPos.getAndAdd(rangeLength), rangeLength);
                start += rangeLength;
            }
        });
    }
    ...
    private void fillFromChunkByRanges(Chunk<? extends Values> src, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                final long realEnd = Math.min(segmentEnd, end);
                final int rangeLength = Math.toIntExact(realEnd - start + 1);
                asDoubleChunk.copyToTypedArray(srcPos.getAndAdd(rangeLength), data[segment], Math.toIntExact(start), rangeLength);
                start += rangeLength;
            }
        });
    }
