...
/**
 * This implements a timsort kernel for a sort key (NullAwareChar descending) that never moves the column values:
 ...
 */
public final class NullAwareCharDescIndirectTimsortKernel {
    private NullAwareCharDescIndirectTimsortKernel() {
        throw new UnsupportedOperationException();
    }

    public static <PERMUTE_VALUES_ATTR extends Any> NullAwareCharDescIndirectSortKernelContext<PERMUTE_VALUES_ATTR> createContext(
            final int size) {
        return new NullAwareCharDescIndirectSortKernelContext<>(size);
    }
     ...
     */
    public static void sort(NullAwareCharDescIndirectSortKernelContext<?> context,
    ...
    private static void timSort(NullAwareCharDescIndirectSortKernelContext<?> context,
    ...
    private static boolean gt(CharChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) > 0;
    }

    private static boolean lt(CharChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) < 0;
    }

    private static boolean geq(CharChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) >= 0;
    }

    private static boolean leq(CharChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) <= 0;
    }

    private static void ensureMergeInvariants(NullAwareCharDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, CharChunk<?> valuesToSort0) {
        while (context.runCount > 1) {
    ...
    private static void merge(NullAwareCharDescIndirectSortKernelContext<?> context,
     ...
     */
    private static void frontMerge(NullAwareCharDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, CharChunk<?> valuesToSort0,
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static void backMerge(NullAwareCharDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, CharChunk<?> valuesToSort0,
    ...
    private static void copyToTemporary(NullAwareCharDescIndirectSortKernelContext<?> context,
    ...
    public static class NullAwareCharDescIndirectSortKernelContext<PERMUTE_VALUES_ATTR extends Any> implements MultiColumnSortKernel<PERMUTE_VALUES_ATTR> {
        int minGallop;
        ...
        private NullAwareCharDescIndirectSortKernelContext(int size) {
            this.size = size;
        ...
        public void sort(WritableLongChunk<PERMUTE_VALUES_ATTR> valuesToPermute,
            ...
            NullAwareCharDescIndirectTimsortKernel.sort(this, positions, valuesToSort[0].asCharChunk());
        ...
        public void sortPositions(WritableIntChunk<ChunkPositions> positions,
            ...
            NullAwareCharDescIndirectTimsortKernel.timSort(this, positions, valuesToSort[0].asCharChunk(), offset, length);
        }
        ...
        public void mergePositions(WritableIntChunk<ChunkPositions> positions,
            ...
            NullAwareCharDescIndirectTimsortKernel.merge(this, positions, valuesToSort[0].asCharChunk(), start1, length1, length2);
        }
