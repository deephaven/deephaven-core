...
public class AggregationSpecAdapter implements Key.Visitor {
    ...
    public static AggregationSpec of(Key key, BaseTable parent) {
        return key.walk(new AggregationSpecAdapter(parent)).out();
    }

    private BaseTable parent;
    private AggregationSpec out;

    public AggregationSpecAdapter(BaseTable parent) {
    ...
    public void visit(KeyMax max) {
        out = new MinMaxBySpecImpl(false, parent.isAddOnly());
    }
