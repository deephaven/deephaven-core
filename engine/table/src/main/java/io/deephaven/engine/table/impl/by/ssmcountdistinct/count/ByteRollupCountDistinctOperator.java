...
public class ByteRollupCountDistinctOperator implements IterativeChunkedAggregationOperator {
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final WritableByteChunk<? extends Values> valueCopy =
                context.valueCopy.get() == null ? null : context.valueCopy.get().asWritableByteChunk();
    ...
    private void flattenDeltas(SizedChunk<Values> targetValues, SizedIntChunk<ChunkLengths> targetCounts,
        ...
        for (int ii = 0; ii < starts.size(); ii++) {
            ...
            for (int kk = startPos; kk < startPos + curLength; kk++) {
                ...
                if (removed) {
                    ssm.fillRemovedChunk(targetValues.get().asWritableByteChunk(), currentPos + newLength);
                } else {
                    ssm.fillAddedChunk(targetValues.get().asWritableByteChunk(), currentPos + newLength);
                }
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
        ...
        final SegmentedSortedMultiSet.RemoveContext removeContext = removeContextFactory.get();
        final WritableByteChunk<? extends Values> preValueCopy = context.valueCopy.get().asWritableByteChunk();
        final WritableIntChunk<ChunkLengths> removedCounts = context.counts.get();
        final WritableByteChunk<? extends Values> postValueCopy = context.postValues.get().asWritableByteChunk();
     ...
     * {@link ByteCompactModifications#compactAndCountModifications} sorts, counts, and diffs the run in place.
     */
    private int flattenDeltas(SizedChunk<Values> targetValues, SizedIntChunk<ChunkLengths> targetCounts,
        ...
        for (int ii = 0; ii < values.size(); ii++) {
            ...
            if (removed) {
                ssm.fillRemovedChunk(targetValues.get().asWritableByteChunk(), currentPos);
            } else {
                ssm.fillAddedChunk(targetValues.get().asWritableByteChunk(), currentPos);
            }
            currentPos += size;
            // we have to do this every time otherwise ensureCapacityPreserve will not do anything.
