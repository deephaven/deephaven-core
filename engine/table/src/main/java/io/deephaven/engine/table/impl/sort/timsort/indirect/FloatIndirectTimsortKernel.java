...
/**
 * This implements a timsort kernel for a sort key (Float) that never moves the column values:
 ...
 */
public final class FloatIndirectTimsortKernel {
    private FloatIndirectTimsortKernel() {
        throw new UnsupportedOperationException();
    }

    public static <PERMUTE_VALUES_ATTR extends Any> FloatIndirectSortKernelContext<PERMUTE_VALUES_ATTR> createContext(
            final int size) {
        return new FloatIndirectSortKernelContext<>(size);
    }
     ...
     */
    public static void sort(FloatIndirectSortKernelContext<?> context,
    ...
    private static void timSort(FloatIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, FloatChunk<?> valuesToSort0, int offset,
            int length) {
        if (length <= 1) {
            return;
        }
     ...
     */
    private static int compareColumns(FloatChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        final int cmp0 = doComparison0(valuesToSort0.get(lhsPos), valuesToSort0.get(rhsPos));
        return cmp0;
    }

    private static boolean gt(FloatChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) > 0;
    }

    private static boolean lt(FloatChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) < 0;
    }

    private static boolean geq(FloatChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) >= 0;
    }

    private static boolean leq(FloatChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) <= 0;
    }

    private static void ensureMergeInvariants(FloatIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, FloatChunk<?> valuesToSort0) {
        while (context.runCount > 1) {
    ...
    private static void merge(FloatIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, FloatChunk<?> valuesToSort0, int start1,
     ...
     */
    private static void frontMerge(FloatIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, FloatChunk<?> valuesToSort0,
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static void backMerge(FloatIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, FloatChunk<?> valuesToSort0,
    ...
    private static void copyToTemporary(FloatIndirectSortKernelContext<?> context,
    ...
    private static void insertionSort(WritableIntChunk<ChunkPositions> positions,
            FloatChunk<?> valuesToSort0, int offset, int length) {
    ...
    public static class FloatIndirectSortKernelContext<PERMUTE_VALUES_ATTR extends Any> implements MultiColumnSortKernel<PERMUTE_VALUES_ATTR> {
        int minGallop;
        ...
        private FloatIndirectSortKernelContext(int size) {
            this.size = size;
        ...
        public void sort(WritableLongChunk<PERMUTE_VALUES_ATTR> valuesToPermute,
            ...
            FloatIndirectTimsortKernel.sort(this, positions, valuesToSort[0].asFloatChunk());
        ...
        public void sortPositions(WritableIntChunk<ChunkPositions> positions,
            ...
            FloatIndirectTimsortKernel.timSort(this, positions, valuesToSort[0].asFloatChunk(), offset, length);
        }
        ...
        public void mergePositions(WritableIntChunk<ChunkPositions> positions,
            ...
            FloatIndirectTimsortKernel.merge(this, positions, valuesToSort[0].asFloatChunk(), start1, length1, length2);
        }
