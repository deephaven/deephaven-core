 ...
 *
 * <p>
 * A state holding zero values or one distinct value is stored without a {@link LongSegmentedSortedMultiset SSM}: the
...
public class LongChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return addToState(destination, (WritableLongChunk<? extends Values>) context.valueCopy, context.counts, 0,
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return removeFromState(destination, context.removeContext,
                (WritableLongChunk<? extends Values>) context.valueCopy, context.counts, 0, context.valueCopy.size());
    }
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        return modifyState(destination, context.removeContext, (WritableLongChunk<? extends Values>) context.valueCopy,
                context.counts, 0, context.removedSize.get(), (WritableLongChunk<? extends Values>) context.postValues,
     ...
     */
    private boolean addToState(long destination, WritableLongChunk<? extends Values> values,
        ...
        if (isUnique(priorState)) {
            ...
            return exposeInternal || !LongComparisons.eq(held, nonUniqueSentinel);
        }
    ...
    private boolean removeFromState(long destination, SegmentedSortedMultiSet.RemoveContext removeContext,
            WritableLongChunk<? extends Values> values, WritableIntChunk<ChunkLengths> counts, int start, int len) {
        final long priorState = singletonCount.getUnsafe(destination);
        if (isUnique(priorState)) {
    ...
    private boolean modifyState(long destination, SegmentedSortedMultiSet.RemoveContext removeContext,
            WritableLongChunk<? extends Values> removedValues, WritableIntChunk<ChunkLengths> removedCounts,
            int removedStart, int removed,
            WritableLongChunk<? extends Values> addedValues, WritableIntChunk<ChunkLengths> addedCounts,
     ...
     * changed.
     */
    private boolean finishFromSsm(long destination, LongSegmentedSortedMultiset ssm) {
        if (ssm.isEmpty()) {
        ...
        if (ssm.size() == 1) {
            ...
            return exposeInternal || !LongComparisons.eq(nonUniqueSentinel, value);
        }
    ...
    private void setUnique(long destination, long value, long count) {
