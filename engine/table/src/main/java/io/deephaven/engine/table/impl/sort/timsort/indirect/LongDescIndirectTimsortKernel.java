...
/**
 * This implements a timsort kernel for a sort key (Long descending) that never moves the column values:
 ...
 */
public final class LongDescIndirectTimsortKernel {
    private LongDescIndirectTimsortKernel() {
        throw new UnsupportedOperationException();
    }

    public static <PERMUTE_VALUES_ATTR extends Any> LongDescIndirectSortKernelContext<PERMUTE_VALUES_ATTR> createContext(
            final int size) {
        return new LongDescIndirectSortKernelContext<>(size);
    }
     ...
     */
    public static void sort(LongDescIndirectSortKernelContext<?> context,
    ...
    private static void timSort(LongDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, LongChunk<?> valuesToSort0, int offset,
            int length) {
        if (length <= 1) {
            return;
        }
     ...
     */
    private static int compareColumns(LongChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        final int cmp0 = doComparison0(valuesToSort0.get(lhsPos), valuesToSort0.get(rhsPos));
        return cmp0;
    }

    private static boolean gt(LongChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) > 0;
    }

    private static boolean lt(LongChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) < 0;
    }

    private static boolean geq(LongChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) >= 0;
    }

    private static boolean leq(LongChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) <= 0;
    }

    private static void ensureMergeInvariants(LongDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, LongChunk<?> valuesToSort0) {
        while (context.runCount > 1) {
    ...
    private static void merge(LongDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, LongChunk<?> valuesToSort0, int start1,
     ...
     */
    private static void frontMerge(LongDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, LongChunk<?> valuesToSort0,
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static void backMerge(LongDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, LongChunk<?> valuesToSort0,
    ...
    private static void copyToTemporary(LongDescIndirectSortKernelContext<?> context,
    ...
    private static int bound(IntChunk<ChunkPositions> positions, LongChunk<?> valuesToSort0, int lo,
    ...
    private static void insertionSort(WritableIntChunk<ChunkPositions> positions,
            LongChunk<?> valuesToSort0, int offset, int length) {
    ...
    public static class LongDescIndirectSortKernelContext<PERMUTE_VALUES_ATTR extends Any> implements MultiColumnSortKernel<PERMUTE_VALUES_ATTR> {
        int minGallop;
        ...
        private LongDescIndirectSortKernelContext(int size) {
            this.size = size;
        ...
        public void sort(WritableLongChunk<PERMUTE_VALUES_ATTR> valuesToPermute,
            ...
            LongDescIndirectTimsortKernel.sort(this, positions, valuesToSort[0].asLongChunk());
        ...
        public void sortPositions(WritableIntChunk<ChunkPositions> positions,
            ...
            LongDescIndirectTimsortKernel.timSort(this, positions, valuesToSort[0].asLongChunk(), offset, length);
        }
        ...
        public void mergePositions(WritableIntChunk<ChunkPositions> positions,
            ...
            LongDescIndirectTimsortKernel.merge(this, positions, valuesToSort[0].asLongChunk(), start1, length1, length2);
        }
