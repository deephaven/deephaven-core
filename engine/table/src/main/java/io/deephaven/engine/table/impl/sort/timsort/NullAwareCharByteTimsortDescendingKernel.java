 ...
 */
/* ---------------------------------------------------------------------------------------------------------------------
 * AUTO-GENERATED CLASS - DO NOT EDIT MANUALLY - for any changes edit CharByteTimsortDescendingKernel and regenerate
 * ------------------------------------------------------------------------------------------------------------------ */
/*
 * ---------------------------------------------------------------------------------------------------------------------
 ...
 * the algorithm.
 */
public class NullAwareCharByteTimsortDescendingKernel {
    private NullAwareCharByteTimsortDescendingKernel() {
        throw new UnsupportedOperationException();
    }

    // region Context
    public static class CharByteSortKernelContext<ATTR extends Any, KEY_INDICES extends Indices> implements ByteSortKernel<ATTR, KEY_INDICES> {
        int minGallop;
        int runCount = 0;
        private final int [] runStarts;
        private final int [] runLengths;
        private final WritableByteChunk<KEY_INDICES> temporaryKeys;
        ...
        public void sort(WritableByteChunk<KEY_INDICES> indexKeys, WritableChunk<ATTR> valuesToSort) {
            NullAwareCharByteTimsortDescendingKernel.sort(this, indexKeys, valuesToSort.asWritableCharChunk());
        }
        ...
        public void sort(WritableByteChunk<KEY_INDICES> indexKeys, WritableChunk<ATTR> valuesToSort, IntChunk<? extends ChunkPositions> offsetsIn, IntChunk<? extends ChunkLengths> lengthsIn) {
            NullAwareCharByteTimsortDescendingKernel.sort(this, indexKeys, valuesToSort.asWritableCharChunk(), offsetsIn, lengthsIn);
        }
    ...
    public static <ATTR extends Any, KEY_INDICES extends Indices> CharByteSortKernelContext<ATTR, KEY_INDICES> createContext(int size) {
     ...
     */
    static <ATTR extends Any, KEY_INDICES extends Indices> void sort(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort, IntChunk<? extends ChunkPositions> offsetsIn, IntChunk<? extends ChunkLengths> lengthsIn) {
     ...
     * runs sorted on each pass.
     */
    public static <ATTR extends Any, KEY_INDICES extends Indices> void sort(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort) {
        timSort(context, indexKeys, valuesToSort, 0, indexKeys.size());
    }

    static private <ATTR extends Any, KEY_INDICES extends Indices> void timSort(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort, int offset, int length) {
        if (length <= 1) {
            return;
        }
     ...
     */
    private static <ATTR extends Any, KEY_INDICES extends Indices> void ensureMergeInvariants(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort) {
        while (context.runCount > 1) {
    ...
    private static <ATTR extends Any, KEY_INDICES extends Indices> void merge(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort, int start1, int length1, int length2) {
     ...
     */
    private static <ATTR extends Any, KEY_INDICES extends Indices> void frontMerge(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort, final int mergeStartPosition, final int start2, final int length2) {
        int tempCursor = 0;
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static <ATTR extends Any, KEY_INDICES extends Indices> void backMerge(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort, final int mergeStartPosition, final int length1) {
    ...
    private static <ATTR extends Any, KEY_INDICES extends Indices> void copyToTemporary(CharByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableCharChunk<ATTR> valuesToSort, int mergeStartPosition, int remaining1) {
    ...
    private static <ATTR extends Any, KEY_INDICES extends Indices> void copyToChunk(ByteChunk<KEY_INDICES> rowSetSource, CharChunk<ATTR> valuesSource, WritableByteChunk<KEY_INDICES> indexDest, WritableCharChunk<ATTR> valuesDest, int sourceStart, int destStart, int length) {
    ...
    private static int upperBound(CharChunk<?> valuesToSort, int lo, int hi, char searchValue) {
    ...
    private static int lowerBound(CharChunk<?> valuesToSort, int lo, int hi, char searchValue) {
        return bound(valuesToSort, lo, hi, searchValue, true);
    }

    private static int bound(CharChunk<?> valuesToSort, int lo, int hi, char searchValue, final boolean lower) {
    ...
    private static void insertionSort(WritableByteChunk<? extends Indices> indexKeys, WritableCharChunk<?> valuesToSort, int offset, int length) {
    ...
    static private void swap(WritableByteChunk<? extends Indices> indexKeys, WritableCharChunk<?> valuesToSort, int a, int b) {
        final byte tempIndexKey = indexKeys.get(a);
...
//    private static void doCheck(Chunk.ByteChunk indexKeys, Chunk.CharChunk valuesToSort, int startCheck, int mergeEnd) {
//        char lastCheck;
//        lastCheck = valuesToSort.get(startCheck);
//        for (int jj = startCheck + 1; jj < mergeEnd; ++jj) {
//            final char newCheck = valuesToSort.get(jj);
...
//    private static void dumpValues(Chunk.CharChunk valuesToSort, int start1, int length1, int start2, int length2, String msg) {
//        System.out.println(msg + " merge (" + start1 + ", " + length1 + ") -> (" + start2 + ", " + length2 + ")");
//        char last = valuesToSort.get(start1);
//        System.out.print("[" + format(last));
//        for (int ii = start1 + 1; ii < start2 + length2; ++ii) {
//            final char current = valuesToSort.get(ii);
//            if (current < last) {
//                System.out.println("****");
//            }
//            last = current;
...
//    private static void dumpValues(Chunk.CharChunk valuesToSort, int start1, int length1, String msg) {
//        dumpValues(valuesToSort, start1, length1, msg, -1);
//    }
//
//    private static void dumpValues(Chunk.CharChunk valuesToSort, int start1, int length1, String msg, int highlight) {
...
//    }
//
//    private static String format(char last, boolean highlight) {
