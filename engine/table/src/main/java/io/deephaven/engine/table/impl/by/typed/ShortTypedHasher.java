...
public class ShortTypedHasher extends StaticChunkedOperatorAggregationStateManagerTypedBase {
    private final ShortArraySource keySource;
    private final ShortArraySource overflowKeySource;

    ShortTypedHasher(ColumnSource<?>[] tableKeySources, int tableSize, double maximumLoadFactor, double targetLoadFactor) {
        super(tableKeySources, tableSize, maximumLoadFactor, targetLoadFactor);
        this.keySource = (ShortArraySource) super.keySources[0];
        this.overflowKeySource = (ShortArraySource) super.overflowKeySources[0];
    }

    @Override
    protected void build(StaticAggHashHandler handler, RowSequence rowSequence, Chunk<Values>[] sourceKeyChunks) {
        final ShortChunk<? extends Values> keyChunk = sourceKeyChunks[0].asShortChunk();

        for (int chunkPosition = 0; chunkPosition < keyChunk.size(); ++chunkPosition) {
            final short value = keyChunk.get(chunkPosition);
    ...
    private int hash(short value) {
        return ShortChunkHasher.hashInitialSingle(value);
    }
    ...
    protected void rehashBucket(StaticAggHashHandler handler, int bucket, int destBucket, int bucketsToAdd) {
        ...
        while (overflowLocation != QueryConstants.NULL_INT) {
            ...
            final short overflowKey = overflowKeySource.getUnsafe(overflowLocation);
            ...
            if (overflowTableLocation == mainInsertLocation) {
                ...
                overflowKeySource.set(overflowLocation, QueryConstants.NULL_SHORT);
    ...
    private int maybeMoveMainBucket(StaticAggHashHandler handler, int bucket, int destBucket, int bucketsToAdd) {
        final short key = keySource.getUnsafe(bucket);
    ...
    private boolean findOverflow(StaticAggHashHandler handler, short value, int chunkPosition, int overflowLocation) {
        while (overflowLocation != QueryConstants.NULL_INT) {
    ...
    public int findPositionForKey(Object value) {
        final short key = TypeUtils.unbox((Short)value);
        int hash = hash(key);
