 ...
 *
 * We must be totally ordered, which is accomplished by sorting on the float values, and then on the corresponding row
 * key.
 */
public final class FloatSegmentedSortedArray implements SegmentedSortedArray {
     ...
     */
    public FloatSegmentedSortedArray(int leafSize) {
    ...
    void insert(FloatChunk<? extends Any> valuesToInsert, LongChunk<? extends RowKeys> rowKeysToInsert) {
        ...
        if (leafCount == 0) {
            ...
            try (final ResettableFloatChunk<Any> leafValuesInsertChunk = ResettableFloatChunk.makeResettableChunk();
    ...
    private void copyToLeaf(int leafOffset, float[] leafValues, FloatChunk<? extends Any> insertValues,
     ...
     */
    private static void clearValues(float[] values, int from, int to) {
        // region clearValues
    ...
    private void removeFromLeaf(int leafSize, float[] leafValues, FloatChunk<? extends Any> removeValues,
    ...
    private static int lowerBound(FloatChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, float searchValue, long searchKey) {
        while (lo < hi) {
    ...
    private static int upperBound(FloatChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, float searchValue, long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final float testValue = valuesToSearch.get(mid);
    ...
    private static int lowerBound(float[] valuesToSearch, long[] rowKeysToSearch, int lo, int hi, float searchValue,
            long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
     ...
     */
    private static int bound(float[] valuesToSearch, long[] rowKeysToSearch, final float searchValue, long searchRowKey,
            int lo, int hi) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final float testValue = valuesToSearch[mid];
    ...
    private void remove(FloatChunk<? extends Any> valuesToRemove, LongChunk<? extends RowKeys> rowKeysToRemove) {
     ...
     */
    private void removeAndGetNextInternal(FloatChunk<? extends Any> valuesToRemove,
    ...
    public void applyShift(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk, long shiftDelta) {
        applyShift(stampChunk.asFloatChunk(), keyChunk, shiftDelta);
    }

    @Override
    public void applyShiftReverse(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        applyShiftReverse(stampChunk.asFloatChunk(), keyChunk, shiftDelta);
    }

    private void applyShift(FloatChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
    ...
    private void shiftLeaf(int leafSize, float[] leafValues, FloatChunk<? extends Any> shiftValues, long[] leafRowKeys,
    ...
    private void applyShiftReverse(FloatChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
        final int shiftSize = stampChunk.size();
        if (shiftSize == 0) {
    ...
    private void shiftLeafReverse(int leafSize, float[] leafValues, FloatChunk<? extends Any> shiftValues,
    ...
    public final class Iterator {
         ...
         */
        public void advanceToLast(float value) {
        ...
        private void advanceToInternal(float value, boolean advanceToLast) {
            if (leafCount == 0) {
                return;
            }
         ...
         * find it.
         */
        private void findInLeaf(float value) {
            if (disallowExactMatch) {
                if (leafCount == 1) {
         ...
         * find it.
         */
        private void findFirstInLeaf(float value) {
