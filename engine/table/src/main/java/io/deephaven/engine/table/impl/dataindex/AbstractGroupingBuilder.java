...
/**
 * The base implementation for {@link GroupingBuilder} provides fields and methods that are common to all implementations.
 */
public abstract class AbstractGroupingBuilder implements GroupingBuilder {
    ...
    private static class MemoKey implements GroupingMemoKey {
        ...
        @Override
        public int hashCode() {
            int result = Objects.hash(indexOfInterest, strictIntersect, sortByFirstKey, inverter, regionKeys, matchCase, invert);
    ...
    protected <DATA_TYPE> Map<DATA_TYPE, RowSet> convertToMap(@Nullable final Table groupingTable) {
        ...
        try (final ChunkSource.GetContext keyGetContext = keySource.makeGetContext(chunkSize);
             ...
             final ChunkBoxer.BoxerKernel boxer = ChunkBoxer.getBoxer(ChunkType.fromElementType(keySource.getType()), chunkSize);
            ...
            while (okIt.hasMore()) {
                ...
                //noinspection unchecked
                final ObjectChunk<DATA_TYPE, ? extends Values> keyChunk = (ObjectChunk<DATA_TYPE, ? extends Values>) boxer.box(keySource.getChunk(keyGetContext, nextKeys));
    ...
    protected Table applyIntersectAndInvert(@NotNull final Table groupingTable) {
        ...
        final ObjectArraySource<RowSet> resultIndexSource = (ObjectArraySource<RowSet>) ArrayBackedColumnSource.getMemoryColumnSource(RowSet.class, null);
        ...
        try(final RowSequence.Iterator okIt = groupingTable.getRowSet().getRowSequenceIterator();
            ...
            while(okIt.hasMore()) {
                ...
                final ObjectChunk<RowSet, ? extends Values> indexChunk = indexSource.getChunk(indexCtx, ok).asObjectChunk();
            ...
            final WrappedRowSetRowRedirection redirection = new WrappedRowSetRowRedirection(redirectionBuilder.build().toTracking());
    ...
    protected Table maybeApplyMatch(final @NotNull Table baseTable) {
        ...
        return baseTable.where(new MatchFilter(matchCase ? MatchFilter.CaseSensitivity.MatchCase : MatchFilter.CaseSensitivity.IgnoreCase,
    ...
    protected GroupingMemoKey makeMemoKey() {
        ...
        return new MemoKey(rowSetOfInterest,
                ...
                sortByFirstKey,
                inverter,
                regionMutators.isEmpty() ? null : regionMutators.stream().map(m -> (GroupingMemoKey)m).collect(Collectors.toList()),
                matchCase,
                invert,
                groupKeys);
    }
