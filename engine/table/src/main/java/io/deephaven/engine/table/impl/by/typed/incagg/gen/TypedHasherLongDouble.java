...
final class TypedHasherLongDouble extends IncrementalChunkedOperatorAggregationStateManagerTypedBase {
  ...
  public TypedHasherLongDouble(ColumnSource[] tableKeySources, int tableSize,
  ...
  private int hash(long v0, double v1) {
  ...
  private int maybeMoveMainBucket(HashHandler handler, int bucket, int destBucket,
    ...
    final double v1 = mainKeySource1.getUnsafe(bucket);
    final int hash = hash(v0, v1);
    ...
    if (location == bucket) {
      ...
      mainKeySource1.set(bucket, QueryConstants.NULL_DOUBLE);
      handler.doMoveMain(bucket, destBucket);
    }
    return mainInsertLocation;
  }

  private boolean findOverflow(HashHandler handler, long v0, double v1, int chunkPosition,
