...
public class ByteRollupUniqueOperator implements IterativeChunkedAggregationOperator {
     ...
     */
    private void removeValueFromMultiset(long destination, byte value) {
        final long mc = internalSingletonCount.getUnsafe(destination);
        if (mc > 0) {
            if (mc == 1) {
     ...
     */
    private boolean exposedChanged(long destination, long priorState, byte priorValue) {
