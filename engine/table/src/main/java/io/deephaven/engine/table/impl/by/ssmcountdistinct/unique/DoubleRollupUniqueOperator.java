...
public class DoubleRollupUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    private BucketSsmDistinctRollupContext updateAddValues(BucketSsmDistinctRollupContext bucketedContext,
        ...
        for (int ii = 0; ii < starts.size(); ii++) {
            ...
            if (newLength > 0) {
                ...
                newLength = DoubleCompactKernel.compactAndCount(bucketedContext.valueCopy.get().asWritableDoubleChunk(),
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final WritableDoubleChunk<? extends Values> valueCopy =
                context.valueCopy.get() == null ? null : context.valueCopy.get().asWritableDoubleChunk();
    ...
    private BucketSsmDistinctRollupContext updateRemoveValues(BucketSsmDistinctRollupContext context,
        ...
        for (int ii = 0; ii < starts.size(); ii++) {
            ...
            if (newLength > 0) {
                ...
                newLength = DoubleCompactKernel.compactAndCount(context.valueCopy.get().asWritableDoubleChunk(),
     ...
     * {@link DoubleCompactModifications#compactAndCountModifications} sorts, counts, and diffs each run in place. The
    ...
    private void flattenDeltas(SizedChunk<Values> targetValues, SizedIntChunk<ChunkLengths> targetCounts,
        ...
        for (int ii = 0; ii < starts.size(); ii++) {
            ...
            for (int kk = startPos; kk < startPos + curLength; kk++) {
                ...
                if (removed) {
                    ssm.fillRemovedChunk(targetValues.get().asWritableDoubleChunk(), currentPos + newLength);
                } else {
                    ssm.fillAddedChunk(targetValues.get().asWritableDoubleChunk(), currentPos + newLength);
                }
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
        ...
        final SegmentedSortedMultiSet.RemoveContext removeContext = removeContextFactory.get();
        final WritableDoubleChunk<? extends Values> preValueCopy = context.valueCopy.get().asWritableDoubleChunk();
        final WritableIntChunk<ChunkLengths> removedCounts = context.counts.get();
        final WritableDoubleChunk<? extends Values> postValueCopy = context.postValues.get().asWritableDoubleChunk();
        ...
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            if (removedRunLength != 0 || addedRunLength != 0) {
                DoubleCompactModifications.compactAndCountModifications(preValueCopy, removedCounts, postValueCopy,
    ...
    private SsmDistinctRollupContext updateAddValues(SsmDistinctRollupContext context,
        ...
        if (currentPos > 0) {
            ...
            DoubleCompactKernel.compactAndCount(context.valueCopy.get().asWritableDoubleChunk(), context.counts.get(),
     ...
     * {@link DoubleCompactModifications#compactAndCountModifications} sorts, counts, and diffs the run in place.
     */
    private int flattenDeltas(SizedChunk<Values> targetValues, SizedIntChunk<ChunkLengths> targetCounts,
        ...
        for (int ii = 0; ii < values.size(); ii++) {
            ...
            if (removed) {
                ssm.fillRemovedChunk(targetValues.get().asWritableDoubleChunk(), currentPos);
            } else {
                ssm.fillAddedChunk(targetValues.get().asWritableDoubleChunk(), currentPos);
            }
            currentPos += size;
            // we have to do this every time otherwise ensureCapacityPreserve will not do anything.
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        DoubleCompactModifications.compactAndCountModifications(context.valueCopy.get().asWritableDoubleChunk(),
                context.counts.get(), context.postValues.get().asWritableDoubleChunk(), context.postCounts.get(),
    ...
    public void startTrackingPrevValues() {
        ...
        prevFlusher = new UpdateCommitter<>(this, ExecutionContext.getContext().getUpdateGraph(),
                DoubleRollupUniqueOperator::flushPrevious);
