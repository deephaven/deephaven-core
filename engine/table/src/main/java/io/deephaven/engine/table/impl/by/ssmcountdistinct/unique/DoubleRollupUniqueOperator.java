...
public class DoubleRollupUniqueOperator implements IterativeChunkedAggregationOperator {
     ...
     */
    private void addValueToMultiset(long destination, double value) {
     ...
     */
    private void removeValueFromMultiset(long destination, double value) {
        final DoubleSegmentedSortedMultiset ssm = ssms.getCurrentSsm(destination);
        if (ssm != null) {
            ssm.remove(value, 1);
            if (ssm.isEmpty()) {
     ...
     */
    private boolean exposedChanged(long destination, long priorState, double priorValue) {
