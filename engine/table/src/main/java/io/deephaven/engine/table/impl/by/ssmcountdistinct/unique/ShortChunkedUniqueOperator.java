 ...
 *
 * <p>
 * A state holding exactly one distinct value is stored without a {@link ShortSegmentedSortedMultiset SSM}: the value
...
public class ShortChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public ShortChunkedUniqueOperator(
            ...
            String name, boolean countNullNaN, boolean exposeInternal, short onlyNullsSentinel, short nonUniqueSentinel) {
        this.name = name;
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final WritableShortChunk<? extends Values> valueCopy = context.valueCopy.asWritableShortChunk();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            final short priorValue = internalResult.getUnsafe(destination);
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
        ...
        final SegmentedSortedMultiSet.RemoveContext removeContext = removeContextFactory.get();
        final WritableShortChunk<? extends Values> preValueCopy =
                (WritableShortChunk<? extends Values>) context.valueCopy;
        final WritableShortChunk<? extends Values> postValueCopy =
                (WritableShortChunk<? extends Values>) context.postValues;
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
            final long destination = destinations.get(startPosition);
            ...
            ShortCompactModifications.compactAndCountModifications(preValueCopy, context.counts,
    ...
    private SsmDistinctContext getAndUpdateContext(Chunk<? extends Values> preValues,
        ...
        ShortCompactModifications.compactAndCountModifications(
                (WritableShortChunk<? extends Values>) context.valueCopy, context.counts,
                (WritableShortChunk<? extends Values>) context.postValues, context.postCounts,
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final short priorValue = internalResult.getUnsafe(destination);
        addToState(destination, (WritableShortChunk<? extends Values>) context.valueCopy, context.counts, 0,
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final short priorValue = internalResult.getUnsafe(destination);
        removeFromState(destination, context.removeContext, (WritableShortChunk<? extends Values>) context.valueCopy,
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final short priorValue = internalResult.getUnsafe(destination);
        modifyState(destination, context.removeContext, (WritableShortChunk<? extends Values>) context.valueCopy,
                context.counts, 0, context.removedSize.get(), (WritableShortChunk<? extends Values>) context.postValues,
     ...
     */
    private void addToState(long destination, WritableShortChunk<? extends Values> values,
        ...
        if (isUnique(sc)) {
            final short held = internalResult.getUnsafe(destination);
            if (len == 1 && ShortComparisons.eq(values.get(start), held)) {
    ...
    private void removeFromState(long destination, SegmentedSortedMultiSet.RemoveContext removeContext,
            WritableShortChunk<? extends Values> values, WritableIntChunk<ChunkLengths> counts, int start, int len) {
        if (len == 0) {
            return;
        }
        final long sc = singletonCount.getUnsafe(destination);
        if (isUnique(sc)) {
            ...
            Assert.eq(len, "len", 1);
            Assert.assertion(ShortComparisons.eq(values.get(start), internalResult.getUnsafe(destination)),
    ...
    private void modifyState(long destination, SegmentedSortedMultiSet.RemoveContext removeContext,
            WritableShortChunk<? extends Values> removedValues, WritableIntChunk<ChunkLengths> removedCounts,
            int removedStart, int removed,
            WritableShortChunk<? extends Values> addedValues, WritableIntChunk<ChunkLengths> addedCounts,
     ...
     */
    private void finishFromSsm(long destination, ShortSegmentedSortedMultiset ssm) {
        if (ssm.isEmpty()) {
            clearSsm(destination);
            setEmpty(destination);
        } else if (ssm.size() == 1) {
            final short value = ssm.get(0);
    ...
    private void setUnique(long destination, short value, long count) {
     ...
     * value does not.
     */
    private boolean stateChanged(long destination, long priorState, short priorValue) {
        if (!ShortComparisons.eq(internalResult.getUnsafe(destination), priorValue)) {
            return true;
        }
