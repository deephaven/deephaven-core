 ...
 *
 * <p>
 * A state holding zero values or one distinct value is stored without a {@link ShortSegmentedSortedMultiset SSM}: the
...
public class ShortChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return addToState(destination, (WritableShortChunk<? extends Values>) context.valueCopy, context.counts, 0,
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return removeFromState(destination, context.removeContext,
                (WritableShortChunk<? extends Values>) context.valueCopy, context.counts, 0, context.valueCopy.size());
    }
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        return modifyState(destination, context.removeContext, (WritableShortChunk<? extends Values>) context.valueCopy,
     ...
     */
    private boolean addToState(long destination, WritableShortChunk<? extends Values> values,
        ...
        if (isUnique(priorState)) {
            ...
            return exposeInternal || !ShortComparisons.eq(held, nonUniqueSentinel);
        }
     ...
     * changed.
     */
    private boolean finishFromSsm(long destination, ShortSegmentedSortedMultiset ssm) {
        if (ssm.isEmpty()) {
        ...
        if (ssm.size() == 1) {
            ...
            return exposeInternal || !ShortComparisons.eq(nonUniqueSentinel, value);
        }
