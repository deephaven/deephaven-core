    ...
    // endregion extensions
{
    ...
    IncrementalChunkedNaturalJoinStateManager(ColumnSource<?>[] tableKeySources
        ...
        // region constructor
        rightRowSetStorage = new ObjectArraySource<>(WritableRowSet.class);
    ...
    private void ensureOverflowCapacity(WritableIntChunk<ChunkPositions> chunkPositionsToInsertInOverflow) {
        ...
        overflowLeftRowSetource.ensureCapacity(newCapacity);
    ...
    private long allocateRightIndexSlot() {
        if (freeRightRowSetLocations.size() > 0) {
            final long allocatedKey = freeRightRowSetLocations.lastRowKey();
            freeRightRowSetLocations.remove(allocatedKey);
            return allocatedKey;
        }
    ...
    private void ensureRightIndexCapacity() {
        final long freeSlots = freeRightRowSetLocations.size();
        if (freeSlots > CHUNK_SIZE) {
            return;
        }
        rightRowSetStorage.ensureCapacity(nextRightIndexLocation + CHUNK_SIZE - freeSlots);
    }
    ...
    private void addRightIndex(long tableLocation, long keyToAdd) {
        ...
        if (existingRightIndex == NO_RIGHT_ENTRY_VALUE) {
            ...
            final WritableRowSet rowSetToUpdate = rightRowSetStorage.getUnsafe(rightIndexSlot);
    ...
    private void shiftLeftIndex(long tableLocation, long shiftedKey, long shiftDelta) {
        final WritableRowSet existingLeftRowSet = leftRowSetSource.get(tableLocation);
    ...
    private void shiftDuplicateRightIndex(long shiftedKey, long shiftDelta, long existingRightIndex) {
        if (!isDuplicateRightIndex(existingRightIndex)) {
            throw Assert.statementNeverExecuted("Existing Right TrackingWritableRowSet: " + existingRightIndex + ", shiftedKey" + shiftedKey + ", shiftDelta=" + shiftDelta);
        }

        final long rightIndexSlot = getRightIndexSlot(existingRightIndex);
        final WritableRowSet rowSetToUpdate = rightRowSetStorage.get(rightIndexSlot);
        rowSetToUpdate.remove(shiftedKey - shiftDelta);
        rowSetToUpdate.insert(shiftedKey);
    }

    private void removeFromDuplicateIndex(LongArraySource rowSetSource, long location, long keyToRemove, long existingRightIndex) {
        if (!isDuplicateRightIndex(existingRightIndex)) {
            throw Assert.statementNeverExecuted("Existing Right TrackingWritableRowSet: " + existingRightIndex + ", keyToRemove=" + keyToRemove);
        }
        ...
        if (rowSetToUpdate.isEmpty()) {
            ...
            rowSetSource.set(location, rowSetToUpdate.firstRowKey());
            rightRowSetStorage.set(rightIndexSlot, null);
            freeRightRowSetLocations.insert(rightIndexSlot);
        }
    }

    private void addLeftIndexOverflow(long overflowLocation, long keyToAdd) {
        final WritableRowSet rowSet = overflowLeftRowSetource.get(overflowLocation);
        if (rowSet == null) {
            overflowLeftRowSetource.set(overflowLocation, RowSetFactory.fromKeys(keyToAdd));
        } else {
            rowSet.insert(keyToAdd);
        }
    }
    ...
    void compactAll() {
        ...
        for (int ii = 0; ii < nextOverflowLocation; ++ii) {
            final WritableRowSet rowSet = overflowLeftRowSetource.get(ii);
            if (rowSet != null) {
                rowSet.compact();
            }
        }
    ...
    private void buildTable(final BuildContext bc,
        ...
        try (final RowSequence.Iterator rsIt = buildIndex.getRowSequenceIterator();
             ...
             final WritableLongChunk<RowKeys> workingLeftRedirections = isLeftSide ? WritableLongChunk.makeWritableChunk(bc.chunkSize) : null
            ...
            for (int loop = 0; loop < 2; loop++) {
                ...
                for (int ii = 0; ii < bc.overflowLocationForPromotionLoop.size(); ++ii) {
                    ...
                    if ((firstLoop && tableLocation < tableHashPivot) || (!firstLoop && tableLocation >= tableHashPivot)) {
                        ...
                        leftRowSetSource.set(tableLocation, overflowLeftRowSetource.get(overflowLocation));
                        overflowLeftRowSetource.set(overflowLocation, null);
    ...
    private void nullOverflowObjectSources(IntChunk<Values> locationsToNull) {
        ...
        for (int ii = 0; ii < locationsToNull.size(); ++ii) {
            overflowLeftRowSetource.set(locationsToNull.get(ii), null);
        }
            ...
            while (rsIt.hasMore()) {
                ...
                for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                    if (pc.equalValues.get(ii)) {
                        ...
                        if (removeLeft) {
                            leftRowSetSource.get(tableLocation).remove(indexKey);
                        } else if (removeRight) {
                ...
                while (pc.overflowLocationsToFetch.size() > 0) {
                    ...
                    for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                        ...
                        if (pc.equalValues.get(ii)) {
                            ...
                            if (removeLeft) {
                                overflowLeftRowSetource.get(overflowLocation).remove(indexKey);
                            } else if (removeRight) {
    ...
    public RowSet getLeftIndex(long slot) {
        if (isOverflowLocation(slot)) {
            return overflowLeftRowSetource.get(hashLocationToOverflowLocation(slot));
        } else {
