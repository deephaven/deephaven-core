...
public class CharColumnBinarySearchKernel {
    ...
    /**
     * Performs a binary search on a sorted {@link ElementSource} using bounds from a {@link CharRangeFilter}, returning
    ...
    public static RowSet binsearchRangeFilter(
            ...
            @NotNull final CharRangeFilter filter,
            final boolean usePrev) {
        if (filter.getLower() == NULL_CHAR && filter.isLowerInclusive()) {
            return binarySearchMax(source, selection, sortColumn, filter.getUpper(), filter.isUpperInclusive(),
                    usePrev);
        } else if (filter.getUpper() == MAX_CHAR && filter.isUpperInclusive()) {
