...
public abstract class BaseObjectUpdateByOperator<T> implements UpdateByOperator {
    ...
    protected ObjectArraySource<T> bucketLastVal;

    /** These are only used in grouped operations */
    protected T singletonVal;
    ...
    public void initializeForUpdate(@NotNull final UpdateContext context,
        ...
        if(!initialized) {
            ...
            if(usingBuckets) {
                ...
                this.bucketLastVal = new ObjectArraySource(colType);
    ...
    public void resetForReprocess(@NotNull final UpdateContext context,
        ...
        final Context ctx = (Context) context;
        if(!ctx.canProcessDirectly) {
            singletonVal = firstUnmodifiedKey == NULL_ROW_KEY ? null : outputSource.get(firstUnmodifiedKey);
        }
    }
