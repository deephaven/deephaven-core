...
public final class DoubleSegmentedSortedMultiset implements SegmentedSortedMultiSet<Double>, DoubleVector {
     ...
     */
    private double[] directoryValues;
    private long[] directoryCount;

    private int[] leafSizes;
    private double[][] leafValues;
    ...
    // region Constructor
    /**
     * Create a DoubleSegmentedSortedArray with the given leafSize.
     *
     * @param leafSize the maximumSize for any leaf
     */
    public DoubleSegmentedSortedMultiset(int leafSize) {
    ...
    public boolean insert(WritableChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
        ...
        return insert(valuesToInsert.asWritableDoubleChunk(), counts, offset, length);
    }
     ...
     */
    public boolean insert(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int offset, int length) {
     ...
     * {@link #insert(WritableDoubleChunk, WritableIntChunk, int, int)} for one value without requiring the caller to wrap
     ...
     */
    public boolean insert(double value, long count) {
        ...
        if (isSingleton()) {
            if (DoubleComparisons.eq(value, singletonValue)) {
                singletonCount += count;
                totalSize += count;
                validate();
                return false;
        ...
        if (DoubleComparisons.gt(value, max)) {
            ...
            added = true;
        } else if (DoubleComparisons.eq(value, max)) {
            addMaxCount(count);
            added = false;
        } else {
            ...
            if (DoubleComparisons.lt(value, min)) {
                ...
                added = true;
            } else if (DoubleComparisons.eq(value, min)) {
                addMinCount(count);
                added = false;
            } else {
     ...
     */
    private boolean insertInterior(double value, long count) {
        if (leafCount == 1) {
            final int ip = upperBound(directoryValues, 0, size, value);
            if (DoubleComparisons.eq(directoryValues[ip], value)) {
     ...
     */
    private void splitLeafForInsert(int leaf, int ip, double value, long count) {
        makeLeafHole(leaf + 1, 1);
        leafCount++;
        final double[] lowerValues = leafValues[leaf];
        final long[] lowerCounts = leafCounts[leaf];
        final double[] upperValues = leafValues[leaf + 1] = new double[leafSize];
    ...
    private int insertExistingIntoLeaf(WritableDoubleChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, double[] leafValues,
            long[] leafCounts, double maxInsert, boolean lastLeaf, int end) {
        int rlpos = 0;
        double nextValue;
        while (rlpos < leafSize && ripos < end
            ...
            if (DoubleComparisons.gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    ...
                    valuesToInsert.copyFromTypedChunk((WritableDoubleChunk) valuesToInsert, ripos, wipos.get(),
                            lastInsert - ripos);
    ...
    private void distributeNewIntoLeaves(DoubleChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts,
        ...
        for (int li = firstLeaf + 1; li < firstLeaf + requiredLeaves; ++li) {
            leafValues[li] = new double[leafSize];
    ...
    private void insertNewIntoLeaf(WritableDoubleChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, double[] leafValues,
            long[] leafCounts) {
    ...
    private void copyRemainingValuesToLeaf(WritableDoubleChunk<? extends Values> valuesToInsert,
     ...
     */
    private int maybeCompact(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
        ...
        if (wipos != ripos && toCopy > 0) {
            ...
            valuesToInsert.copyFromTypedChunk((DoubleChunk) valuesToInsert, ripos, wipos, toCopy);
     ...
     */
    private int insertExisting(WritableDoubleChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount == 1) {
            ...
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, offset, wipos, size, directoryValues,
                    directoryCount, NULL_DOUBLE, true, end);
        ...
        int nextLeaf = 0;
        while (ripos < end) {
            final double startValue = valuesToInsert.get(ripos);
            ...
            final double maxValue = lastLeaf ? NULL_DOUBLE : directoryValues[nextLeaf];
    ...
    private void insertInternal(WritableDoubleChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount > 1 && DoubleComparisons.gt(valuesToInsert.get(offset), getMaxDouble())) {
        ...
        do {
            final double insertValue = valuesToInsert.get(rpos);
            ...
            if (nextLeaf == leafCount - 1) {
                ...
                final double lastLeafValue = directoryValues[nextLeaf];
    ...
    private void moveDirectoryToLeaf(int desiredLeafCount, int directoryLocation) {
        leafValues = new double[desiredLeafCount][];
        ...
        directoryCount = null;
        directoryValues = new double[desiredLeafCount - 1];
    }

    private void doAppend(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int offset, int length) {
    ...
    private void makeLeafHole(int holePosition, int holeSize) {
        ...
        if (holePosition + holeSize < leafValues.length) {
            Arrays.fill(directoryValues, holePosition, holePosition + holeSize, NULL_DOUBLE);
        } else {
            Arrays.fill(directoryValues, holePosition, holePosition + holeSize - 1, NULL_DOUBLE);
        }
        // endregion fillValue
    }
    ...
    private void allocateLeafArrays(int newSize) {
        leafValues = new double[newSize][];
        leafCounts = new long[newSize][];
        leafSizes = new int[newSize];
        directoryValues = new double[newSize - 1];
    }
    ...
    private void makeLeavesInitial(DoubleChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        ...
        if (leafCount == 1) {
            directoryValues = new double[length];
    ...
    private void packValuesIntoLeaves(DoubleChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int rpos,
        ...
        while (rpos < end) {
            ...
            leafValues[startLeaf] = new double[leafSize];
    ...
    private void materializeSingleton(int incomingValueCount) {
        ...
        directoryValues = new double[capacity];
     ...
     */
    private static int lowerBound(double[] valuesToSearch, int lo, int hi, double searchValue) {
        while (lo < hi) {
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(DoubleChunk<? extends Any> valuesToSearch, int lo, int hi, double searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(double[] valuesToSearch, int lo, int hi, double searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final double testValue = valuesToSearch[mid];
            final boolean moveLo = DoubleComparisons.leq(testValue, searchValue);
     ...
     */
    private static int upperBound(double[] valuesToSearch, int lo, int hi, double searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final double testValue = valuesToSearch[mid];
     ...
     */
    private static int lowerBoundExclusive(double[] valuesToSearch, int lo, int hi, double searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final double testValue = valuesToSearch[mid];
    ...
    public boolean remove(RemoveContext removeContext, WritableChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        return remove(removeContext, valuesToRemove.asWritableDoubleChunk(), counts, offset, length);
    }
     ...
     */
    public boolean remove(RemoveContext removeContext, WritableDoubleChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        final long beforeSize = size();
     ...
     * {@link #remove(RemoveContext, WritableDoubleChunk, WritableIntChunk, int, int)} for one value without requiring the
     ...
     */
    public boolean remove(double value, long count) {
        Assert.gtZero(count, "count");
        validate();
        if (isSingleton()) {
            Assert.assertion(DoubleComparisons.eq(value, singletonValue),
                    "DoubleComparisons.eq(value, singletonValue)");
        ...
        if (leafCount == 1) {
            ...
            Assert.assertion(pos < size && DoubleComparisons.eq(directoryValues[pos], value),
                    "pos < size && DoubleComparisons.eq(directoryValues[pos], value)");
        ...
        Assert.assertion(pos < leafSz && DoubleComparisons.eq(leafValue[pos], value),
                "pos < leafSz && DoubleComparisons.eq(leafValue[pos], value)");
    ...
    private void removeInternal(RemoveContext removeContext, DoubleChunk<? extends Values> valuesToRemove,
        ...
        if (leafCount == 1) {
            ...
            do {
                ...
                final double firstValueToRemove = valuesToRemove.get(rpos);
    ...
    private int removeFromLeaf(RemoveContext removeContext, DoubleChunk<? extends Values> valuesToRemove,
            IntChunk<ChunkLengths> counts, int ripos, int end, double[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
            final double removeValue = valuesToRemove.get(ripos);
    ...
    private int compactValues(RemoveContext removeContext, double[] leafValues, long[] leafCounts, int sz, int cl) {
        int removed = 0;
        for (int cli = 0; cli <= cl; cli++) {
    ...
    private void validateInputs(DoubleChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        if (!SEGMENTED_SORTED_MULTISET_VALIDATION) {
            return;
        }
        ...
        for (int ii = offset + 1; ii < end; ++ii) {
            ...
            final double prevValue = valuesToInsert.get(ii - 1);
            final double curValue = valuesToInsert.get(ii);
            Assert.assertion(DoubleComparisons.lt(prevValue, curValue), "DoubleComparisons.lt(prevValue, curValue)",
    ...
    private static void validateLeaf(double[] values, long[] counts, int size) {
     ...
     */
    private void appendMaximum(double value, long count) {
        totalSize += count;
        if (leafCount == 0) {
            singletonValue = value;
            singletonCount = count;
        ...
        if (leafCount == 1) {
            ...
            leafValues[1] = new double[leafSize];
        ...
        if (lastLeafSize < leafSize) {
            ...
            leafValues[leafCount] = new double[leafSize];
     ...
     */
    private void prependMinimum(double value, long count) {
        totalSize += count;
        if (leafCount == 0) {
            singletonValue = value;
            singletonCount = count;
        ...
        if (leafCount == 1) {
            ...
            if (newSize <= leafSize) {
                if (directoryValues.length < newSize) {
                    ...
                    final double[] grownValues = new double[newSize];
            ...
            leafValues[0] = new double[leafSize];
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        final DoubleSegmentedSortedMultiset destination = (DoubleSegmentedSortedMultiset) untypedDestination;
        validate();
        ...
        if (isSingleton()) {
            ...
            if (destination.size > 0 && DoubleComparisons.eq(singletonValue, destination.getMaxDouble())) {
            ...
            if (leafCount == 1) {
                ...
                for (int rli = 0; rli < rleaf; ++rli) {
                    ...
                    if (rli == rleaf - 1 && leftOver > 0 && partialUnique == 0) {
                        ...
                        final double[] tmpValues = new double[leafSize];
        ...
        if (partialUnique > 0) {
            ...
            final double[] destinationValues;
            ...
            if (appendToExtra) {
                ...
                destinationValues = destination.leafValues[wleaf] = new double[leafSize];
    ...
    private boolean prepareAppend(int finalSlots, int completeLeaves) {
        ...
        if (completeLeaves == 0) {
            ...
            if (leafCount == 0) {
                directoryValues = new double[finalSlots];
            ...
            if (extraLeaf) {
                leafValues[leafCount + completeLeaves] = new double[leafSize];
    ...
    private boolean preparePrepend(int initialSlots, int completeLeaves) {
        ...
        if (completeLeaves > 0) {
            ...
            if (leafCount == 0) {
                ...
                extraLeafCount = 1;
                directoryValues = new double[initialSlots];
        ...
        if (extraLeafCount == 0 && initialSlots > 0) {
            ...
            if (directoryCount != null) {
                final double[] tmpValues = new double[initialSlots + size];
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (isSingleton()) {
            ...
            if (destination.size > 0 && DoubleComparisons.eq(singletonValue, destination.getMinDouble())) {
        ...
        if (slotsInPartialLeaf > 0) {
            ...
            final double[] destValues;
            final long[] destCounts;

            final double[] srcValues;
            ...
            if (destination.directoryCount != null) {
                ...
                if (extraLeaf) {
                    destination.leafValues[0] = new double[leafSize];
        ...
        if (completeLeavesToMove > 0) {
            if (leafCount == 1) {
                ...
                if (hasLeftOverSlot) {
                    ...
                    leafValues[rleaf + 1] = new double[leafSize];
    ...
    public void fillKeyChunk(WritableChunk<?> keyChunk, int offset) {
        fillKeyChunk(keyChunk.asWritableDoubleChunk(), offset);
    }

    private void fillKeyChunk(WritableDoubleChunk<?> keyChunk, int offset) {
    ...
    private void maybeAccumulateAdditions(WritableDoubleChunk<? extends Values> valuesToInsert, int offset, int length) {
        ...
        if (added == null) {
            added = new DoubleCompareOpenHashSet(length);
        }
    ...
    private void maybeAccumulateAddition(double valueAdded) {
        if (!accumulateDeltas) {
            return;
        }
        ...
        if (added == null) {
            added = new DoubleCompareOpenHashSet(1);
        }
    ...
    private void maybeAccumulateRemoval(double valueRemoved) {
        if (!accumulateDeltas) {
            return;
        }
