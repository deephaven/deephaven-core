...
public final class DoubleSegmentedSortedMultiset implements SegmentedSortedMultiSet<Double>, DoubleVector {
    ...
    private int insertExistingIntoLeaf(WritableDoubleChunk<? extends Values> valuesToInsert,
            ...
            long[] leafCounts, double maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        double nextValue;
    ...
    private void maybeCompact(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int ripos, int wipos) {
        if (wipos == ripos) {
            return;
        }
    ...
    private void insertExisting(WritableDoubleChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount == 1) {
            ...
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, 0, wipos, size, directoryValues,
                    directoryCount, NULL_DOUBLE, true);
    ...
    private void insert(WritableDoubleChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        ...
        if (isSingleton()) {
            if (valuesToInsert.size() == 1 && DoubleComparisons.eq(valuesToInsert.get(0), singletonValue)) {
    ...
    private void remove(RemoveContext removeContext, DoubleChunk<? extends Values> valuesToRemove,
        ...
        if (isSingleton()) {
            ...
            Assert.assertion(DoubleComparisons.eq(valuesToRemove.get(0), singletonValue),
                    "DoubleComparisons.eq(valuesToRemove.get(0), singletonValue)");
            singletonCount -= counts.get(0);
    ...
    private boolean equalsArray(ObjectVector<?> o) {
        ...
        try (final CloseableIterator<?> oit = o.iterator()) {
            if (size == 1) {
                ...
                // region VectorEquals
                if (get(0) == NULL_DOUBLE && val != null && val != NULL_DOUBLE) {
                    return false;
                }
                // endregion VectorEquals
