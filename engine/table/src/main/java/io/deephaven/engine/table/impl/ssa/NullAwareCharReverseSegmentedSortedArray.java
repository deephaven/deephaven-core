...
// ****** Edit NullAwareCharSegmentedSortedArray and run "./gradlew replicateSegmentedSortedArray" to regenerate
//
// @formatter:off
 ...
 *
 * We must be totally ordered, which is accomplished by sorting on the char values, and then on the corresponding row
 * key.
 */
public final class NullAwareCharReverseSegmentedSortedArray implements SegmentedSortedArray {
    ...
    /**
     * Create a NullAwareCharReverseSegmentedSortedArray with the given leafSize.
     *
     * @param leafSize the maximumSize for any leaf
     */
    public NullAwareCharReverseSegmentedSortedArray(int leafSize) {
    ...
    void insert(CharChunk<? extends Any> valuesToInsert, LongChunk<? extends RowKeys> rowKeysToInsert) {
        ...
        if (leafCount == 0) {
            ...
            try (final ResettableCharChunk<Any> leafValuesInsertChunk = ResettableCharChunk.makeResettableChunk();
                ...
                while (firstValuesPosition < insertSize) {
                    ...
                    if (firstLeaf == leafCount - 1) {
                        ...
                        final char leafMaxValue = leafValues[firstLeaf + 1][0];
                    ...
                    if (sizeForThisLeaf <= leafSize) {
                        ...
                        if (isLastLeaf && isAfterLeaf(leafSizes[firstLeaf], leafValues[firstLeaf],
                            ...
                            for (int leaf = firstLeaf; leaf < firstLeaf + newLeafCount; ++leaf) {
                                if (leaf > firstLeaf) {
                                    leafValues[leaf] = new char[leafSize];
    ...
    private void mergeLeaves(int leaf, TIntList leavesToRemove) {
        ...
        final char[] tmpValues = leafValues[leaf + 1];
    ...
    private void copyToLeaf(int leafOffset, char[] leafValues, CharChunk<? extends Any> insertValues,
    ...
    private void promoteDirectory(int newLeafCount) {
        ...
        leafValues = new char[newLeafCount][];
        ...
        directoryValues = new char[newLeafCount];
    ...
    private void makeLeafHole(int holePosition, int holeSize) {
        ...
        // region fillValue
        Arrays.fill(directoryValues, holePosition, holePosition + holeSize, Character.MIN_VALUE);
        // endregion fillValue
    ...
    private void makeSingletonLeaf(CharChunk<? extends Any> values, LongChunk<? extends RowKeys> rowKeys) {
    ...
    private void makeLeavesInitial(CharChunk<? extends Any> values, LongChunk<? extends RowKeys> rowKeys) {
        ...
        leafValues = new char[leafCount][];
        leafRowKeys = new long[leafCount][];
        directoryValues = new char[leafCount];
        ...
        for (int ii = 0; ii < leafCount; ++ii) {
            ...
            leafValues[ii] = new char[leafSize];
    ...
    private void insertIntoLeaf(int leafSize, char[] leafValues, CharChunk<? extends Any> insertValues,
        ...
        while (wpos >= 0) {
            ...
            final char vall = leafValues[rposl];
    ...
    private boolean isAfterLeaf(int leafSize, char[] leafValues, CharChunk<? extends Any> insertValues,
        ...
        final char lastLeafValue = leafValues[leafSize - 1];
    ...
    private void removeFromLeaf(int leafSize, char[] leafValues, CharChunk<? extends Any> removeValues,
        ...
        for (int rposl = 0; rposl < leafSize;) {
            ...
            if (++lwin >= minGallop) {
                ...
                final char searchValue = removeValues.get(rposi);
    ...
    private static int lowerBound(CharChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, char searchValue, long searchKey) {
        while (lo < hi) {
    ...
    private static int upperBound(CharChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, char searchValue, long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch.get(mid);
    ...
    private static int lowerBound(char[] valuesToSearch, long[] rowKeysToSearch, int lo, int hi, char searchValue,
            long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch[mid];
            final int comparison = doComparison(testValue, searchValue);
    ...
    private static int upperBound(char[] valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch[mid];
            final int comparison = doComparison(testValue, searchValue);
     ...
     */
    private static int bound(char[] valuesToSearch, long[] rowKeysToSearch, final char searchValue, long searchRowKey,
            int lo, int hi) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch[mid];
    ...
    public void remove(Chunk<? extends Any> valuesToRemove, LongChunk<? extends RowKeys> rowKeysToRemove) {
        remove(valuesToRemove.asCharChunk(), rowKeysToRemove);
    }
    ...
    public void removeAndGetPrior(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> rowKeysToRemove,
        ...
        removeAndGetNextInternal(stampChunk.asCharChunk(), rowKeysToRemove, priorRedirections);
    }

    private void remove(CharChunk<? extends Any> valuesToRemove, LongChunk<? extends RowKeys> rowKeysToRemove) {
     ...
     */
    private void removeAndGetNextInternal(CharChunk<? extends Any> valuesToRemove,
        ...
        if (leafCount == 1) {
            ...
            try (final ResettableCharChunk<Any> leafValuesRemoveChunk = ResettableCharChunk.makeResettableChunk();
    ...
    public void applyShift(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk, long shiftDelta) {
        applyShift(stampChunk.asCharChunk(), keyChunk, shiftDelta);
    }

    @Override
    public void applyShiftReverse(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        applyShiftReverse(stampChunk.asCharChunk(), keyChunk, shiftDelta);
    }

    private void applyShift(CharChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
        ...
        if (leafCount == 1) {
            ...
            try (final ResettableCharChunk<Any> leafValuesChunk = ResettableCharChunk.makeResettableChunk();
    ...
    private void shiftLeaf(int leafSize, char[] leafValues, CharChunk<? extends Any> shiftValues, long[] leafRowKeys,
        ...
        for (int leafPos = 0; leafPos < leafSize;) {
            ...
            if (++lwin >= minGallop) {
                ...
                final char searchValue = shiftValues.get(shiftPos);
    ...
    private void applyShiftReverse(CharChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
        final int shiftSize = stampChunk.size();
        if (shiftSize == 0) {
            ...
            try (final ResettableCharChunk<Any> leafValuesChunk = ResettableCharChunk.makeResettableChunk();
                ...
                while (lastValuesPosition >= 0) {
                    ...
                    if (firstLeaf == 0) {
                        ...
                        final char leafMinValue = leafValues[firstLeaf][0];
    ...
    private void shiftLeafReverse(int leafSize, char[] leafValues, CharChunk<? extends Any> shiftValues,
    ...
    void validateInternal() {
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                validateLeaf(ii);
                final char lastValue = leafValues[ii][leafSizes[ii] - 1];
                final char directoryValue = directoryValues[ii];
                ...
                if (ii < leafCount - 1) {
                    final char nextFirstValue = leafValues[ii + 1][0];
    ...
    private void validateLeafOrdering(int firstLeaf, int lastLeaf) {
        ...
        for (int leaf = firstLeaf; leaf < lastLeaf; ++leaf) {
            final char lastValue = leafValues[leaf][leafSizes[leaf] - 1];
    ...
    private static void validateLeaf(char[] values, long[] rowKeys, int size) {
        if (!SEGMENTED_SORTED_ARRAY_VALIDATION) {
            return;
        }
    ...
    public final class Iterator {
        ...
        private void advanceToInternal(char value, boolean advanceToLast) {
            if (leafCount == 0) {
                return;
            }
         ...
         * find it.
         */
        private void findInLeaf(char value) {
            if (disallowExactMatch) {
                if (leafCount == 1) {
        ...
        private void findLastInLeaf(char value) {
            if (leafCount == 1) {
                indexWithinLeaf =
         ...
         * find it.
         */
        private void findFirstInLeaf(char value) {
