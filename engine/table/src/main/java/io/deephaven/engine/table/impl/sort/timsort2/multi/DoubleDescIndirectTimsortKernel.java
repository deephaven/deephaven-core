...
/**
 * This implements a timsort kernel for a sort key (Double descending) that never moves the column values:
 ...
 */
public final class DoubleDescIndirectTimsortKernel {
    private DoubleDescIndirectTimsortKernel() {
        throw new UnsupportedOperationException();
    }

    public static <PERMUTE_VALUES_ATTR extends Any> DoubleDescIndirectSortKernelContext<PERMUTE_VALUES_ATTR> createContext(
            final int size) {
        return new DoubleDescIndirectSortKernelContext<>(size);
    }
     ...
     */
    public static void sort(DoubleDescIndirectSortKernelContext<?> context,
    ...
    private static void timSort(DoubleDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, DoubleChunk<?> valuesToSort0, int offset,
            int length) {
        if (length <= 1) {
            return;
        }
     ...
     */
    private static int compareColumns(DoubleChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        final int cmp0 = doComparison0(valuesToSort0.get(lhsPos), valuesToSort0.get(rhsPos));
        return cmp0;
    }

    private static boolean gt(DoubleChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) > 0;
    }

    private static boolean lt(DoubleChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) < 0;
    }

    private static boolean geq(DoubleChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) >= 0;
    }

    private static boolean leq(DoubleChunk<?> valuesToSort0, int lhsPos, int rhsPos) {
        return compareColumns(valuesToSort0, lhsPos, rhsPos) <= 0;
    }

    private static void ensureMergeInvariants(DoubleDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, DoubleChunk<?> valuesToSort0) {
        while (context.runCount > 1) {
    ...
    private static void merge(DoubleDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, DoubleChunk<?> valuesToSort0, int start1,
     ...
     */
    private static void frontMerge(DoubleDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, DoubleChunk<?> valuesToSort0,
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static void backMerge(DoubleDescIndirectSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, DoubleChunk<?> valuesToSort0,
    ...
    private static void copyToTemporary(DoubleDescIndirectSortKernelContext<?> context,
    ...
    private static void insertionSort(WritableIntChunk<ChunkPositions> positions,
            DoubleChunk<?> valuesToSort0, int offset, int length) {
    ...
    public static class DoubleDescIndirectSortKernelContext<PERMUTE_VALUES_ATTR extends Any> implements MultiColumnSortKernel<PERMUTE_VALUES_ATTR> {
        int minGallop;
        ...
        private DoubleDescIndirectSortKernelContext(int size) {
        ...
        public void sort(WritableLongChunk<PERMUTE_VALUES_ATTR> valuesToPermute,
            ...
            DoubleDescIndirectTimsortKernel.sort(this, positions, valuesToSort[0].asDoubleChunk());
