 ...
 */
public final class FloatTimsortDescendingKernel {
    private FloatTimsortDescendingKernel() {
        throw new UnsupportedOperationException();
    }

    public static <ATTR extends Any> FloatSortKernelContext<ATTR> createContext(final int size) {
     ...
     */
    public static <ATTR extends Any> void sort(FloatSortKernelContext<ATTR> context,
            final WritableFloatChunk<ATTR> valuesToSort) {
        timSort(context, valuesToSort, 0, valuesToSort.size());
    }

    private static <ATTR extends Any> void timSort(FloatSortKernelContext<ATTR> context,
            WritableFloatChunk<ATTR> valuesToSort, int offset, int length) {
        if (length <= 1) {
            return;
        }
    ...
    private static <ATTR extends Any> void ensureMergeInvariants(
            FloatSortKernelContext<ATTR> context, WritableFloatChunk<ATTR> valuesToSort) {
        while (context.runCount > 1) {
    ...
    private static <ATTR extends Any> void merge(FloatSortKernelContext<ATTR> context,
            WritableFloatChunk<ATTR> valuesToSort, int start1, int length1, int length2) {
     ...
     */
    private static <ATTR extends Any> void frontMerge(FloatSortKernelContext<ATTR> context,
            WritableFloatChunk<ATTR> valuesToSort, final int mergeStartPosition, final int start2,
            final int length2) {
        int tempCursor = 0;
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static <ATTR extends Any> void backMerge(FloatSortKernelContext<ATTR> context,
            WritableFloatChunk<ATTR> valuesToSort, final int mergeStartPosition,
            final int length1) {
    ...
    private static <ATTR extends Any> void copyToTemporary(FloatSortKernelContext<ATTR> context,
            WritableFloatChunk<ATTR> valuesToSort, int mergeStartPosition, int remaining1) {
    ...
    private static <ATTR extends Any> void copyToChunk(FloatChunk<ATTR> valuesSource,
            WritableFloatChunk<ATTR> valuesDest, int sourceStart, int destStart, int length) {
    ...
    private static void insertionSort(WritableFloatChunk<?> valuesToSort, int offset, int length) {
    ...
    private static void swap(WritableFloatChunk<?> valuesToSort, int a, int b) {
