...
public abstract class FunctionGeneratedTableSpec {
     ...
     * table's own {@link io.deephaven.engine.rowset.RowSet} shape. In either case, each refresh that produces a table
     * is a full replacement (all previous rows removed, all new rows added). Because the generated column sources are
     * retained across cycles when not copying, a refreshing generated table must expose immutable column sources.
     *
