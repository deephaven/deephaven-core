...
public class IntPartitionKernel {
    public static class PartitionKernelContext implements SafeCloseable {
        ...
        // pivotValues.size()).mapToObj(pivotValues::get).map(IntPartitionKernel::format).collect(Collectors.joining(","))
        // + "]");
        // System.out.println("[" + IntStream.range(0,
        ...
        public IntLongTuple[] getPivots() {
            return IntStream.range(0, pivotValues.size())
                    .mapToObj(ii -> new IntLongTuple(pivotValues.get(ii), pivotKeys.get(ii)))
                    .toArray(IntLongTuple[]::new);
        }
    ...
    public static PartitionKernelContext createContext(RowSet rowSet, ColumnSource<Integer> columnSource,
    ...
    private static void samplePivots(RowSet rowSet, int nPartitions, WritableLongChunk<RowKeys> pivotKeys,
            WritableIntChunk<Any> pivotValues, ColumnSource<Integer> columnSource) {
        pivotKeys.setSize(0);
    ...
    private static int binarySearchTieIndex(IntChunk pivotValues, LongChunk<RowKeys> pivotKeys, int lo, int hi,
            int searchValue, long searchKey) {
        while (lo != hi) {
            final int mid = (lo + hi) / 2;
            final int compareValue = pivotValues.get(mid);
            if (eq(searchValue, compareValue)) {
