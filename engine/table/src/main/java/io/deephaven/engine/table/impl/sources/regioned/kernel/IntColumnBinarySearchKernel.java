...
public class IntColumnBinarySearchKernel {
    // region binsearchRangeFilter
    /**
     * Performs a binary search on a sorted {@link ElementSource} using bounds from a {@link IntRangeFilter}, returning
    ...
    public static RowSet binsearchRangeFilter(
            ...
            @NotNull final IntRangeFilter filter,
            final boolean usePrev) {
        if (filter.getLower() == NULL_INT && filter.isLowerInclusive()) {
            return binarySearchMax(source, selection, sortColumn, filter.getUpper(), filter.isUpperInclusive(),
                    usePrev);
        } else if (filter.getUpper() == MAX_INT && filter.isUpperInclusive()) {
     ...
     * @param usePrev If true, the search will use the previous values (getPrevInt) instead of current values
     *        (getInt).
     *
     ...
     * @param usePrev If true, uses getPrevInt instead of getInt.
    ...
    static long lowerBoundAscending(
        ...
        while (low <= high) {
            ...
            final int midValue = usePrev ? source.getPrevInt(selection.get(mid)) : source.getInt(selection.get(mid));
