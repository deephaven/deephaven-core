 ...
 *
 * <p>
 * A state holding zero values or one distinct value is stored without a {@link DoubleSegmentedSortedMultiset SSM}: the
...
public class DoubleChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return addToState(destination, (WritableDoubleChunk<? extends Values>) context.valueCopy, context.counts, 0,
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return removeFromState(destination, context.removeContext,
                (WritableDoubleChunk<? extends Values>) context.valueCopy, context.counts, 0, context.valueCopy.size());
    }
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        return modifyState(destination, context.removeContext, (WritableDoubleChunk<? extends Values>) context.valueCopy,
     ...
     */
    private boolean addToState(long destination, WritableDoubleChunk<? extends Values> values,
        ...
        if (isUnique(priorState)) {
            ...
            return exposeInternal || !DoubleComparisons.eq(held, nonUniqueSentinel);
        }
     ...
     * changed.
     */
    private boolean finishFromSsm(long destination, DoubleSegmentedSortedMultiset ssm) {
        if (ssm.isEmpty()) {
        ...
        if (ssm.size() == 1) {
            ...
            return exposeInternal || !DoubleComparisons.eq(nonUniqueSentinel, value);
        }
