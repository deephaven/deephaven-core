...
public final class ShortSegmentedSortedMultiset implements SegmentedSortedMultiSet<Short>, ShortVector {
    ...
    public boolean insert(WritableChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
        ...
        return insert(valuesToInsert.asWritableShortChunk(), counts, offset, length);
    }
     ...
     */
    public boolean insert(WritableShortChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int offset, int length) {
     ...
     * {@link #insert(WritableShortChunk, WritableIntChunk, int, int)} for one value without requiring the caller to wrap
    ...
    public boolean insert(short value, long count) {
        ...
        if (isSingleton()) {
            if (ShortComparisons.eq(value, singletonValue)) {
                singletonCount += count;
                totalSize += count;
                validate();
                return false;
        ...
        final boolean added;
        final short max = getMaxShort();
        if (ShortComparisons.gt(value, max)) {
            maybeAccumulateAddition(value);
            appendMaximum(value, count);
            added = true;
        } else if (ShortComparisons.eq(value, max)) {
            addMaxCount(count);
            added = false;
        } else {
            final short min = getMinShort();
            if (ShortComparisons.lt(value, min)) {
                maybeAccumulateAddition(value);
                prependMinimum(value, count);
                added = true;
            } else if (ShortComparisons.eq(value, min)) {
                addMinCount(count);
                added = false;
            } else {
     ...
     */
    private boolean insertInterior(short value, long count) {
        if (leafCount == 1) {
            final int ip = upperBound(directoryValues, 0, size, value);
            if (ShortComparisons.eq(directoryValues[ip], value)) {
     ...
     */
    private void splitLeafForInsert(int leaf, int ip, short value, long count) {
        makeLeafHole(leaf + 1, 1);
        leafCount++;
        final short[] lowerValues = leafValues[leaf];
        final long[] lowerCounts = leafCounts[leaf];
        final short[] upperValues = leafValues[leaf + 1] = new short[leafSize];
    ...
    private int insertExistingIntoLeaf(WritableShortChunk<? extends Values> valuesToInsert,
            ...
            long[] leafCounts, short maxInsert, boolean lastLeaf, int end) {
        int rlpos = 0;
        short nextValue;
        while (rlpos < leafSize && ripos < end
                && (ShortComparisons.leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
            if (ShortComparisons.gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    if (ShortComparisons.eq(leafValues[rlpos], nextValue)) {
    ...
    private void distributeNewIntoLeaves(ShortChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts,
        ...
        while (remaining-- > 0) {
            ...
            final boolean useInsertValue = ShortComparisons.gt(insertValue, leafValue);
    ...
    private void insertNewIntoLeaf(WritableShortChunk<? extends Values> valuesToInsert,
        ...
        while (wpos >= 0) {
            ...
            if (ShortComparisons.gt(insertValue, leafValue)) {
     ...
     */
    private int maybeCompact(WritableShortChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
     ...
     */
    private int insertExisting(WritableShortChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount == 1) {
            ...
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, offset, wipos, size, directoryValues,
                    directoryCount, NULL_SHORT, true, end);
    ...
    private void insertInternal(WritableShortChunk<? extends Values> valuesToInsert,
        ...
        if (isSingleton()) {
            if (length == 1 && ShortComparisons.eq(valuesToInsert.get(offset), singletonValue)) {
        ...
        if (leafCount > 1 && ShortComparisons.gt(valuesToInsert.get(offset), getMaxShort())) {
    ...
    private void makeLeavesInitial(ShortChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        ...
        if (leafCount == 1) {
            directoryValues = new short[length];
    ...
    private void materializeSingleton(int incomingValueCount) {
        ...
        directoryValues = new short[capacity];
    ...
    private static int lowerBound(short[] valuesToSearch, int lo, int hi, short searchValue) {
        while (lo < hi) {
            ...
            final boolean moveLo = ShortComparisons.leq(testValue, searchValue);
            if (moveLo) {
                lo = mid;
                if (lo == hi - 1) {
                    break;
    ...
    private static int upperBound(short[] valuesToSearch, int lo, int hi, short searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final short testValue = valuesToSearch[mid];
            final boolean moveHi = ShortComparisons.geq(testValue, searchValue);
            if (moveHi) {
                hi = mid;
            } else {
                lo = mid + 1;
    ...
    private static int lowerBoundExclusive(short[] valuesToSearch, int lo, int hi, short searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final short testValue = valuesToSearch[mid];
            final boolean moveLo = ShortComparisons.lt(testValue, searchValue);
            if (moveLo) {
                lo = mid + 1;
                if (lo == hi) {
                    break;
    ...
    public boolean remove(RemoveContext removeContext, WritableChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        return remove(removeContext, valuesToRemove.asWritableShortChunk(), counts, offset, length);
    }
     ...
     */
    public boolean remove(RemoveContext removeContext, WritableShortChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        final long beforeSize = size();
     ...
     * {@link #remove(RemoveContext, WritableShortChunk, WritableIntChunk, int, int)} for one value without requiring the
    ...
    public boolean remove(short value, long count) {
        ...
        if (isSingleton()) {
            Assert.assertion(ShortComparisons.eq(value, singletonValue),
                    "ShortComparisons.eq(value, singletonValue)");
        ...
        if (leafCount == 1) {
            ...
            Assert.assertion(pos < size && ShortComparisons.eq(directoryValues[pos], value),
                    "pos < size && ShortComparisons.eq(directoryValues[pos], value)");
        ...
        Assert.assertion(pos < leafSz && ShortComparisons.eq(leafValue[pos], value),
                "pos < leafSz && ShortComparisons.eq(leafValue[pos], value)");
    ...
    private void removeInternal(RemoveContext removeContext, ShortChunk<? extends Values> valuesToRemove,
        ...
        if (isSingleton()) {
            ...
            Assert.assertion(ShortComparisons.eq(valuesToRemove.get(offset), singletonValue),
                    "ShortComparisons.eq(valuesToRemove.get(offset), singletonValue)");
            singletonCount -= counts.get(offset);
            totalSize -= counts.get(offset);
    ...
    private void validateInputs(ShortChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        if (!SEGMENTED_SORTED_MULTISET_VALIDATION) {
            return;
        }
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    Assert.assertion(ShortComparisons.leq(lastValue, directoryValue), "lt(lastValue, directoryValue)",
                            lastValue,
                            "leafValues[ii][leafSizes[ii] - 1]", directoryValue, "directoryValue");

                    if (ii < leafCount - 2) {
                        final short nextDirectoryValue = directoryValues[ii + 1];
                        Assert.assertion(ShortComparisons.lt(directoryValue, nextDirectoryValue),
                    ...
                    Assert.assertion(ShortComparisons.lt(directoryValue, nextFirstValue),
    ...
    private void validateLeafOrdering() {
        for (int leaf = 0; leaf < leafCount - 1; ++leaf) {
            ...
            Assert.assertion(ShortComparisons.lt(lastValue, nextValue), lastValue + " < " + nextValue);
        }
    }
    ...
    private static void validateLeaf(short[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(ShortComparisons.lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])",
                    (Short) thisValue,
     ...
     */
    private void appendMaximum(short value, long count) {
        totalSize += count;
        if (leafCount == 0) {
            singletonValue = value;
            singletonCount = count;
        ...
        if (leafCount == 1) {
            ...
            leafValues[1] = new short[leafSize];
        ...
        if (lastLeafSize < leafSize) {
            ...
            leafValues[leafCount] = new short[leafSize];
     ...
     */
    private void prependMinimum(short value, long count) {
        totalSize += count;
        if (leafCount == 0) {
            singletonValue = value;
            singletonCount = count;
        ...
        if (leafCount == 1) {
            ...
            if (newSize <= leafSize) {
                if (directoryValues.length < newSize) {
                    ...
                    final short[] grownValues = new short[newSize];
            ...
            leafValues[0] = new short[leafSize];
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(ShortComparisons.geq(getMinShort(), destination.getMaxShort()),
        ...
        if (isSingleton()) {
            ...
            if (destination.size > 0 && ShortComparisons.eq(singletonValue, destination.getMaxShort())) {
        ...
        if (destination.size > 0 && ShortComparisons.eq(getMinShort(), destination.getMaxShort())) {
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(ShortComparisons.leq(getMaxShort(), destination.getMinShort()),
        ...
        if (isSingleton()) {
            ...
            if (destination.size > 0 && ShortComparisons.eq(singletonValue, destination.getMinShort())) {
        ...
        if (destination.size > 0 && ShortComparisons.eq(getMaxShort(), destination.getMinShort())) {
    ...
    private void maybeAccumulateAdditions(WritableShortChunk<? extends Values> valuesToInsert, int offset, int length) {
        ...
        if (added == null) {
            added = new ShortOpenHashSet(length);
        }
    ...
    private void maybeAccumulateAddition(short valueAdded) {
        if (!accumulateDeltas) {
            return;
        }
    ...
    public ValueIteratorOfShort iterator(final long fromIndexInclusive, final long toIndexExclusive) {
        ...
        return new ValueIteratorOfShort() {
            ...
            private short[] values = startLeaf < leafCount ? leafValues[startLeaf] : null;
