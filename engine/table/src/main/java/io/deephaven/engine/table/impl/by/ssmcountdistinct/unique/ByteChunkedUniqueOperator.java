...
public class ByteChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return addToState(destination, (WritableByteChunk<? extends Values>) context.valueCopy, context.counts, 0,
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        return removeFromState(destination, context.removeContext,
                (WritableByteChunk<? extends Values>) context.valueCopy, context.counts, 0, context.valueCopy.size());
    }
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        return modifyState(destination, context.removeContext, (WritableByteChunk<? extends Values>) context.valueCopy,
     ...
     */
    private boolean addToState(long destination, WritableByteChunk<? extends Values> values,
        ...
        if (isUnique(priorState)) {
            ...
            return exposeInternal || !ByteComparisons.eq(held, nonUniqueSentinel);
        }
     ...
     * changed.
     */
    private boolean finishFromSsm(long destination, ByteSegmentedSortedMultiset ssm) {
        if (ssm.isEmpty()) {
        ...
        if (ssm.size() == 1) {
            ...
            return exposeInternal || !ByteComparisons.eq(nonUniqueSentinel, value);
        }
