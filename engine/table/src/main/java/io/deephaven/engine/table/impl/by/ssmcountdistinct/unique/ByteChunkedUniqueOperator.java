...
public class ByteChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final WritableByteChunk<? extends Values> valueCopy = context.valueCopy.asWritableByteChunk();
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final byte priorValue = internalResult.getUnsafe(destination);
        addToState(destination, (WritableByteChunk<? extends Values>) context.valueCopy, context.counts, 0,
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final byte priorValue = internalResult.getUnsafe(destination);
        removeFromState(destination, context.removeContext, (WritableByteChunk<? extends Values>) context.valueCopy,
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final byte priorValue = internalResult.getUnsafe(destination);
        modifyState(destination, context.removeContext, (WritableByteChunk<? extends Values>) context.valueCopy,
                context.counts, 0, context.removedSize.get(), (WritableByteChunk<? extends Values>) context.postValues,
     ...
     */
    private void addToState(long destination, WritableByteChunk<? extends Values> values,
        ...
        if (isUnique(sc)) {
            final byte held = internalResult.getUnsafe(destination);
            if (len == 1 && ByteComparisons.eq(values.get(start), held)) {
    ...
    private void removeFromState(long destination, SegmentedSortedMultiSet.RemoveContext removeContext,
            WritableByteChunk<? extends Values> values, WritableIntChunk<ChunkLengths> counts, int start, int len) {
        if (len == 0) {
            return;
        }
        final long sc = singletonCount.getUnsafe(destination);
        if (isUnique(sc)) {
            ...
            Assert.eq(len, "len", 1);
            Assert.assertion(ByteComparisons.eq(values.get(start), internalResult.getUnsafe(destination)),
     ...
     */
    private void finishFromSsm(long destination, ByteSegmentedSortedMultiset ssm) {
        if (ssm.isEmpty()) {
            clearSsm(destination);
            setEmpty(destination);
        } else if (ssm.size() == 1) {
            final byte value = ssm.get(0);
    ...
    private void setUnique(long destination, byte value, long count) {
     ...
     * value does not.
     */
    private boolean stateChanged(long destination, long priorState, byte priorValue) {
        if (!ByteComparisons.eq(internalResult.getUnsafe(destination), priorValue)) {
            return true;
        }
