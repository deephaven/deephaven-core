...
public class FloatRegionBinarySearchKernel {
    ...
    /**
     * Performs a binary search on a sorted column region using bounds from a {@link FloatRangeFilter}, returning the row
    ...
    public static RowSet binsearchRangeFilter(
            ...
            @NotNull final FloatRangeFilter filter) {
        if (filter.getLower() == NULL_FLOAT && filter.isLowerInclusive()) {
            return binarySearchMax(region, firstKey, lastKey, sortColumn, filter.getUpper(),
                    filter.isUpperInclusive());
        } else if (Float.isNaN(filter.getUpper()) && filter.isUpperInclusive()) {
    ...
    static long lowerBoundAscending(
        ...
        if (minInc && low <= lastKey) {
            if (FloatComparisons.compare(region.getFloat(low), min) == 0) {
                return low;
            }
        }
    ...
    static long upperBoundAscending(
        ...
        if (maxInc && high >= firstKey) {
            if (FloatComparisons.compare(region.getFloat(high), max) == 0) {
                return high;
            }
        }
        return insertionPoint(low);
    ...
    static long lowerBoundDescending(
        ...
        if (maxInc && low <= lastKey) {
            if (FloatComparisons.compare(region.getFloat(low), max) == 0) {
                return low;
            }
        }
        return insertionPoint(low);
    ...
    static long upperBoundDescending(
        ...
        if (minInc && high >= firstKey) {
            if (FloatComparisons.compare(region.getFloat(high), min) == 0) {
                return high;
            }
        }
        return insertionPoint(low);
