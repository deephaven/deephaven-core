...
public class DoubleChunkSsaStamp implements ChunkSsaStamp {
    ...
    public void processEntry(Chunk<Values> leftStampValues, Chunk<RowKeys> leftStampKeys, SegmentedSortedArray ssa,
        ...
        processEntry(leftStampValues.asDoubleChunk(), leftStampKeys, (DoubleSegmentedSortedArray) ssa, rightKeysForLeft,
                disallowExactMatch);
    }
    ...
    public int findModified(int first, Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        return findModified(first, leftStampValues.asDoubleChunk(), leftStampKeys, rowRedirection,
                rightStampChunk.asDoubleChunk(), rightStampIndices, modifiedBuilder, disallowExactMatch);
    }

    private static int findModified(int leftLowIdx, DoubleChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            RowRedirection rowRedirection, DoubleChunk<? extends Values> rightStampChunk,
            LongChunk<RowKeys> rightStampIndices, RowSetBuilderRandom modifiedBuilder, boolean disallowExactMatch) {
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final double rightStampValue = rightStampChunk.get(ii);
    ...
    public void applyShift(Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        applyShift(leftStampValues.asDoubleChunk(), leftStampKeys, rightStampChunk.asDoubleChunk(), rightStampKeys,
                shiftDelta, rowRedirection, disallowExactMatch);
    }

    private void applyShift(DoubleChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            DoubleChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightStampKeys, long shiftDelta,
            WritableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final double rightStampValue = rightStampChunk.get(ii);
    ...
    private static int findFirstResponsiveLeft(int leftLowIdx, DoubleChunk<Values> leftStampValues,
            boolean disallowExactMatch, double rightStampValue) {
