...
public class FloatTypedHasher extends StaticChunkedOperatorAggregationStateManagerTypedBase {
    private final FloatArraySource keySource;
    private final FloatArraySource overflowKeySource;

    FloatTypedHasher(ColumnSource<?>[] tableKeySources, int tableSize, double maximumLoadFactor, double targetLoadFactor) {
        super(tableKeySources, tableSize, maximumLoadFactor, targetLoadFactor);
        this.keySource = (FloatArraySource) super.keySources[0];
        this.overflowKeySource = (FloatArraySource) super.overflowKeySources[0];
    }

    @Override
    protected void build(StaticAggHashHandler handler, RowSequence rowSequence, Chunk<Values>[] sourceKeyChunks) {
        final FloatChunk<? extends Values> keyChunk = sourceKeyChunks[0].asFloatChunk();

        for (int chunkPosition = 0; chunkPosition < keyChunk.size(); ++chunkPosition) {
            final float value = keyChunk.get(chunkPosition);
    ...
    private int hash(float value) {
        return FloatChunkHasher.hashInitialSingle(value);
    }
    ...
    protected void rehashBucket(StaticAggHashHandler handler, int bucket, int destBucket, int bucketsToAdd) {
        ...
        while (overflowLocation != QueryConstants.NULL_INT) {
            ...
            final float overflowKey = overflowKeySource.getUnsafe(overflowLocation);
            ...
            if (overflowTableLocation == mainInsertLocation) {
                ...
                overflowKeySource.set(overflowLocation, QueryConstants.NULL_FLOAT);
    ...
    private int maybeMoveMainBucket(StaticAggHashHandler handler, int bucket, int destBucket, int bucketsToAdd) {
        final float key = keySource.getUnsafe(bucket);
    ...
    private boolean findOverflow(StaticAggHashHandler handler, float value, int chunkPosition, int overflowLocation) {
        while (overflowLocation != QueryConstants.NULL_INT) {
    ...
    public int findPositionForKey(Object value) {
        final float key = TypeUtils.unbox((Float)value);
        int hash = hash(key);
