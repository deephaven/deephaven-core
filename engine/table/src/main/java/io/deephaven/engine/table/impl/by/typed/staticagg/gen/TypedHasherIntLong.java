...
final class TypedHasherIntLong extends StaticChunkedOperatorAggregationStateManagerTypedBase {
  ...
  public TypedHasherIntLong(ColumnSource[] tableKeySources, int tableSize, double maximumLoadFactor,
  ...
  private int hash(int v0, long v1) {
  ...
  private int maybeMoveMainBucket(HashHandler handler, int bucket, int destBucket,
      int bucketsToAdd) {
    final int v0 = mainKeySource0.getUnsafe(bucket);
    final long v1 = mainKeySource1.getUnsafe(bucket);
    final int hash = hash(v0, v1);
    ...
    if (location == bucket) {
      ...
      mainKeySource0.set(bucket, QueryConstants.NULL_INT);
      mainKeySource1.set(destBucket, v1);
      mainKeySource1.set(bucket, QueryConstants.NULL_LONG);
      handler.doMoveMain(bucket, destBucket);
    }
    return mainInsertLocation;
  }

  private boolean findOverflow(HashHandler handler, int v0, long v1, int chunkPosition,
