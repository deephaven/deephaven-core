 ...
 *
 * <p>
 * A state holding exactly one distinct value is stored without a {@link CharSegmentedSortedMultiset SSM}: the value
...
public class CharChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public CharChunkedUniqueOperator(
            ...
            String name, boolean countNullNaN, boolean exposeInternal, char onlyNullsSentinel, char nonUniqueSentinel) {
        this.name = name;
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final WritableCharChunk<? extends Values> valueCopy = context.valueCopy.asWritableCharChunk();
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
        ...
        final WritableCharChunk<? extends Values> preValueCopy =
                (WritableCharChunk<? extends Values>) context.valueCopy;
        final WritableCharChunk<? extends Values> postValueCopy =
                (WritableCharChunk<? extends Values>) context.postValues;
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
            final long destination = destinations.get(startPosition);
    ...
    public boolean addChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final char priorValue = internalResult.getUnsafe(destination);
        addToState(destination, (WritableCharChunk<? extends Values>) context.valueCopy, context.counts, 0,
    ...
    public boolean removeChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> values,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final char priorValue = internalResult.getUnsafe(destination);
        removeFromState(destination, context.removeContext, (WritableCharChunk<? extends Values>) context.valueCopy,
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        final long priorState = singletonCount.getUnsafe(destination);
        final char priorValue = internalResult.getUnsafe(destination);
        modifyState(destination, context.removeContext, (WritableCharChunk<? extends Values>) context.valueCopy,
     ...
     */
    private void addToState(long destination, WritableCharChunk<? extends Values> values,
     ...
     */
    private void finishFromSsm(long destination, CharSegmentedSortedMultiset ssm) {
        if (ssm.isEmpty()) {
     ...
     * value does not.
     */
    private boolean stateChanged(long destination, long priorState, char priorValue) {
        if (!CharComparisons.eq(internalResult.getUnsafe(destination), priorValue)) {
            return true;
        }
