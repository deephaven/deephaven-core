    ...
    // endregion extensions
{
    ...
    private void buildTable(final BuildContext bc,
        ...
        try (final RowSequence.Iterator rsIt = buildIndex.getRowSequenceIterator();
            ...
            while (rsIt.hasMore()) {
                ...
                addedSlots.ensureCapacity(slotCount.intValue() + bc.chunkSize);
                overflowBuildCookieSource.ensureCapacity(nextOverflowLocation + bc.chunkSize);
                ...
                for (int ii = 0; ii < bc.insertPositionsInSourceChunk.size(); ) {
                    ...
                    // region main insert
                    addedSlots.set(slotCount.intValue(), currentHashLocation);
                    buildCookieSource.set(currentHashLocation, slotCount.intValue());
                    slotCount.increment();
                    final long keyToAdd = bc.sourceIndexKeys.get(firstChunkPositionForHashLocation);
                    stateSource.set(currentHashLocation, ENTRY_EXISTS);
                    if (isLeftSide) {
                        addLeftIndex(currentHashLocation, keyToAdd);
                    } else {
                        addRightIndex(currentHashLocation, keyToAdd);
                    }
                ...
                if (bc.overflowPositionInSourceChunk.size() > 0) {
                    ...
                    while (bc.chunkPositionsToInsertInOverflow.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.chunkPositionsToInsertInOverflow.size(); ) {
                            ...
                            final long slot = overflowLocationToHashLocation(allocatedOverflowLocation);
                            addedSlots.set(slotCount.intValue(), slot);
                            overflowBuildCookieSource.set(allocatedOverflowLocation, slotCount.intValue());
                            slotCount.increment();
                            final long keyToAdd = bc.sourceIndexKeys.get(chunkPosition);
                            if (isLeftSide) {
                                addLeftIndexOverflow(allocatedOverflowLocation, keyToAdd);
                            } else {
                                addRightIndexOverflow(allocatedOverflowLocation, keyToAdd);
                            }
                            overflowStateSource.set(allocatedOverflowLocation, ENTRY_EXISTS);
                    ...
                    doRehash(bc, buildCookieSource, overflowBuildCookieSource, addedSlots);
    ...
    public void doRehash(BuildContext bc
                          ...
                          , IntegerArraySource buildCookieSource
                          , IntegerArraySource overflowBuildCookieSource
            ...
            for (int ii = 0; ii < bc.shouldMoveBucket.size(); ++ii) {
                if (bc.shouldMoveBucket.get(ii)) {
                    ...
                    stateSource.set(oldHashLocation, EMPTY_VALUE);
                                ...
                                buildCookieSource.set(newHashLocation, cookie);
                                buildCookieSource.set(oldHashLocation, QueryConstants.NULL_INT);
                ...
                for (int ii = 0; ii < bc.overflowLocationForPromotionLoop.size(); ++ii) {
                    ...
                    if ((firstLoop && tableLocation < tableHashPivot) || (!firstLoop && tableLocation >= tableHashPivot)) {
                        ...
                        bc.destinationLocationPositionInWriteThrough.add((int)(tableLocation - firstBackingChunkLocation));
                                    ...
                                    buildCookieSource.set(tableLocation, cookie);
                                    overflowBuildCookieSource.set(overflowLocation, QueryConstants.NULL_INT);
    ...
    private void decorationProbe(ProbeContext pc
        ...
        if (slots != null) {
            ...
            Assert.neqNull(foundBuilder, "foundBuilder");
        } else {
            ...
            while (rsIt.hasMore()) {
                ...
                for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                    if (pc.equalValues.get(ii)) {
                        ...
                        if (isLeftSide) {
                            if (addLeftIndex(tableLocation, indexKey) && slots != null) {
                                slots.set(slotCount.intValue(), tableLocation);
                                slotCount.increment();
                ...
                while (pc.overflowLocationsToFetch.size() > 0) {
                    ...
                    for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                        ...
                        if (pc.equalValues.get(ii)) {
                            ...
                            if (isLeftSide) {
                                if (addLeftIndexOverflow(overflowLocation, indexKey) && slots != null) {
                                    slots.set(slotCount.intValue(), overflowLocationToHashLocation(overflowLocation));
                                    slotCount.increment();
                                    foundBuilder.addKey(indexKey);
                                }
                            } else {
    ...
    String keyString(long slot) {
        ...
        final WritableLongChunk<RowKeys> slotChunk = WritableLongChunk.makeWritableChunk(1);
        if (isOverflowLocation(slot)) {
