...
public class ObjectTypedHasher extends StaticChunkedOperatorAggregationStateManagerTypedBase {
    private final ObjectArraySource keySource;
    private final ObjectArraySource overflowKeySource;

    ObjectTypedHasher(ColumnSource<?>[] tableKeySources, int tableSize, double maximumLoadFactor, double targetLoadFactor) {
        super(tableKeySources, tableSize, maximumLoadFactor, targetLoadFactor);
        this.keySource = (ObjectArraySource) super.keySources[0];
        this.overflowKeySource = (ObjectArraySource) super.overflowKeySources[0];
    }

    @Override
    protected void build(StaticAggHashHandler handler, RowSequence rowSequence, Chunk<Values>[] sourceKeyChunks) {
        final ObjectChunk<Object, ? extends Values> keyChunk = sourceKeyChunks[0].asObjectChunk();

        for (int chunkPosition = 0; chunkPosition < keyChunk.size(); ++chunkPosition) {
            final Object value = keyChunk.get(chunkPosition);
    ...
    protected void rehashBucket(StaticAggHashHandler handler, int bucket, int destBucket, int bucketsToAdd) {
        ...
        while (overflowLocation != QueryConstants.NULL_INT) {
            ...
            final Object overflowKey = overflowKeySource.getUnsafe(overflowLocation);
            ...
            if (overflowTableLocation == mainInsertLocation) {
                ...
                overflowKeySource.set(overflowLocation, null);
    ...
    private int maybeMoveMainBucket(StaticAggHashHandler handler, int bucket, int destBucket, int bucketsToAdd) {
        final Object key = keySource.getUnsafe(bucket);
    ...
    private boolean findOverflow(StaticAggHashHandler handler, Object value, int chunkPosition, int overflowLocation) {
        while (overflowLocation != QueryConstants.NULL_INT) {
