...
/*
 * ---------------------------------------------------------------------------------------------------------------------
 * AUTO-GENERATED CLASS - DO NOT EDIT MANUALLY - for any changes edit ByteLongTimsortDescendingKernel and regenerate
 * ---------------------------------------------------------------------------------------------------------------------
 */
 ...
 * the algorithm.
 */
public class ByteByteTimsortDescendingKernel {
    private ByteByteTimsortDescendingKernel() {
        throw new UnsupportedOperationException();
    }

    // region Context
    public static class ByteByteSortKernelContext<ATTR extends Any, KEY_INDICES extends Indices> implements ByteSortKernel<ATTR, KEY_INDICES> {
        int minGallop;
        int runCount = 0;
        ...
        private ByteByteSortKernelContext(int size) {
        ...
        public void sort(WritableByteChunk<KEY_INDICES> indexKeys, WritableChunk<ATTR> valuesToSort) {
            ByteByteTimsortDescendingKernel.sort(this, indexKeys, valuesToSort.asWritableByteChunk());
        }
        ...
        public void sort(WritableByteChunk<KEY_INDICES> indexKeys, WritableChunk<ATTR> valuesToSort, IntChunk<? extends ChunkPositions> offsetsIn, IntChunk<? extends ChunkLengths> lengthsIn) {
            ByteByteTimsortDescendingKernel.sort(this, indexKeys, valuesToSort.asWritableByteChunk(), offsetsIn, lengthsIn);
        }
    ...
    public static <ATTR extends Any, KEY_INDICES extends Indices> ByteByteSortKernelContext<ATTR, KEY_INDICES> createContext(int size) {
        return new ByteByteSortKernelContext<>(size);
    }
     ...
     */
    static <ATTR extends Any, KEY_INDICES extends Indices> void sort(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort, IntChunk<? extends ChunkPositions> offsetsIn, IntChunk<? extends ChunkLengths> lengthsIn) {
     ...
     * runs sorted on each pass.
     */
    public static <ATTR extends Any, KEY_INDICES extends Indices> void sort(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort) {
        timSort(context, indexKeys, valuesToSort, 0, indexKeys.size());
    }

    static private <ATTR extends Any, KEY_INDICES extends Indices> void timSort(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort, int offset, int length) {
        if (length <= 1) {
            return;
        }
     ...
     */
    private static <ATTR extends Any, KEY_INDICES extends Indices> void ensureMergeInvariants(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort) {
        while (context.runCount > 1) {
    ...
    private static <ATTR extends Any, KEY_INDICES extends Indices> void merge(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort, int start1, int length1, int length2) {
     ...
     */
    private static <ATTR extends Any, KEY_INDICES extends Indices> void frontMerge(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort, final int mergeStartPosition, final int start2, final int length2) {
        int tempCursor = 0;
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static <ATTR extends Any, KEY_INDICES extends Indices> void backMerge(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort, final int mergeStartPosition, final int length1) {
    ...
    private static <ATTR extends Any, KEY_INDICES extends Indices> void copyToTemporary(ByteByteSortKernelContext<ATTR, KEY_INDICES> context, WritableByteChunk<KEY_INDICES> indexKeys, WritableByteChunk<ATTR> valuesToSort, int mergeStartPosition, int remaining1) {
    ...
    private static <ATTR extends Any, KEY_INDICES extends Indices> void copyToChunk(ByteChunk<KEY_INDICES> rowSetSource, ByteChunk<ATTR> valuesSource, WritableByteChunk<KEY_INDICES> indexDest, WritableByteChunk<ATTR> valuesDest, int sourceStart, int destStart, int length) {
    ...
    private static void insertionSort(WritableByteChunk<? extends Indices> indexKeys, WritableByteChunk<?> valuesToSort, int offset, int length) {
    ...
    static private void swap(WritableByteChunk<? extends Indices> indexKeys, WritableByteChunk<?> valuesToSort, int a, int b) {
...
//    private static void doCheck(Chunk.ByteChunk indexKeys, Chunk.ByteChunk valuesToSort, int startCheck, int mergeEnd) {
//        byte lastCheck;
//        lastCheck = valuesToSort.get(startCheck);
//        for (int jj = startCheck + 1; jj < mergeEnd; ++jj) {
//            final byte newCheck = valuesToSort.get(jj);
...
//    private static void dumpValues(Chunk.ByteChunk valuesToSort, int start1, int length1, int start2, int length2, String msg) {
//        System.out.println(msg + " merge (" + start1 + ", " + length1 + ") -> (" + start2 + ", " + length2 + ")");
//        byte last = valuesToSort.get(start1);
//        System.out.print("[" + format(last));
//        for (int ii = start1 + 1; ii < start2 + length2; ++ii) {
//            final byte current = valuesToSort.get(ii);
//            if (current < last) {
//                System.out.println("****");
//            }
//            last = current;
...
//    private static void dumpValues(Chunk.ByteChunk valuesToSort, int start1, int length1, String msg) {
//        dumpValues(valuesToSort, start1, length1, msg, -1);
//    }
//
//    private static void dumpValues(Chunk.ByteChunk valuesToSort, int start1, int length1, String msg, int highlight) {
...
//        if (last >= 'A' && last <= 'Z') {
//            return Byte.toString(last);
//        }
//        return String.format("0x%04x", (int) last);
//    }
//
//    private static String format(byte last, boolean highlight) {
