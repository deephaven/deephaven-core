...
public class FloatCompactKernel implements CompactKernel {
    ...
    public void compactAndCount(
            ...
            boolean countNull,
            boolean countNaN) {
        compactAndCount(valueChunk.asWritableFloatChunk(), counts, countNull, countNaN);
    }
    ...
    public void compactAndCount(
            ...
            boolean countNull,
            boolean countNaN) {
        compactAndCount(valueChunk.asWritableFloatChunk(), counts, startPositions, lengths, countNull, countNaN);
    }

    public static void compactAndCount(
            WritableFloatChunk<? extends Values> valueChunk,
            WritableIntChunk<ChunkLengths> counts) {
    ...
    public static int compactAndCount(
        ...
        for (int rpos = start; rpos < end; ++rpos) {
            ...
            if (!countNull && nextValue == NULL_FLOAT) {
                continue;
            }
            // region maybeCountNaN
            if (!countNaN && Float.isNaN(nextValue)) {
                continue;
            }
