 ...
 */
public class FloatRollupCountDistinctOperator implements IterativeChunkedAggregationOperator {
    ...
    private UpdateCommitter<FloatRollupCountDistinctOperator> prevFlusher = null;
    private WritableRowSet touchedStates;

    public FloatRollupCountDistinctOperator(
                                           // region Constructor
    ...
    private BucketSsmDistinctRollupContext updateAddValues(BucketSsmDistinctRollupContext bucketedContext,
        ...
        final ObjectChunk<FloatSegmentedSortedMultiset, ? extends Values> inputValues = inputs.asObjectChunk();
        ...
        for(int ii = 0; ii< starts.size(); ii++) {
            ...
            for(int kk = startPos; kk < startPos + curLength; kk++) {
                final FloatSegmentedSortedMultiset ssm = inputValues.get(kk);
                final int size;
            ...
            if(newLength > 0) {
                ...
                newLength = FloatCompactKernel.compactAndCount(bucketedContext.valueCopy.get().asWritableFloatChunk(), bucketedContext.counts.get(), currentPos, newLength, countNull);
            }
    ...
    private BucketSsmDistinctRollupContext updateRemoveValues(BucketSsmDistinctRollupContext context,
        ...
        for(int ii = 0; ii< starts.size(); ii++) {
            ...
            for(int kk = startPos; kk < startPos + curLength; kk++) {
                ...
                ssm.fillRemovedChunk(context.valueCopy.get().asWritableFloatChunk(), currentPos + newLength);
            ...
            if(newLength > 0) {
                ...
                newLength = FloatCompactKernel.compactAndCount(context.valueCopy.get().asWritableFloatChunk(), context.counts.get(), currentPos, newLength, countNull);
            }
    ...
    @Override
    public void removeChunk(BucketedContext bucketedContext, Chunk<? extends Values> values, LongChunk<? extends Attributes.RowKeys> inputRowKeys, IntChunk<RowKeys> destinations, IntChunk<ChunkPositions> startPositions, IntChunk<ChunkLengths> length, WritableBooleanChunk<Values> stateModified) {
    ...
    private void updateModifyAddValues(BucketSsmDistinctRollupContext context,
        ...
        for(int ii = 0; ii< starts.size(); ii++) {
            ...
            for(int kk = startPos; kk < startPos + curLength; kk++) {
                ...
                context.valueCopy.ensureCapacityPreserve(currentPos + newLength + size);
                ssm.fillAddedChunk(context.valueCopy.get().asWritableFloatChunk(), currentPos + newLength);
    ...
    private SsmDistinctRollupContext updateAddValues(SsmDistinctRollupContext context,
                                                     Chunk<? extends Values> inputs) {
        final ObjectChunk<FloatSegmentedSortedMultiset, ? extends Values> values = inputs.asObjectChunk();
        ...
        for(int ii = 0; ii < values.size(); ii++) {
            final FloatSegmentedSortedMultiset ssm = values.get(ii);
            final int size;
            if(ssm == null || (size = ssm.intSize()) == 0) {
                continue;
            }
        ...
        if(currentPos > 0) {
            ...
            FloatCompactKernel.compactAndCount(context.valueCopy.get().asWritableFloatChunk(), context.counts.get(), countNull);
        }
        return context;
    }
    ...
    private SsmDistinctRollupContext updateRemoveValues(SsmDistinctRollupContext context,
        ...
        for(int ii = 0; ii < values.size(); ii++) {
            ...
            context.valueCopy.ensureCapacityPreserve(currentPos + size);
            ssm.fillRemovedChunk(context.valueCopy.get().asWritableFloatChunk(), currentPos);
            currentPos += size;
            //we have to do this every time otherwise ensureCapacityPreserve will not do anything.
            context.valueCopy.get().setSize(currentPos);
        }
    ...
    private void updateModifyAddValues(SsmDistinctRollupContext context,
        ...
        for(int ii = 0; ii < values.size(); ii++) {
            ...
            context.valueCopy.ensureCapacityPreserve(currentPos + size);
            ssm.fillAddedChunk(context.valueCopy.get().asWritableFloatChunk(), currentPos);
            currentPos += size;
            //we have to do this every time otherwise ensureCapacityPreserve will not do anything.
            context.valueCopy.get().setSize(currentPos);
        }
    ...
    @Override
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues, Chunk<? extends Values> postValues, LongChunk<? extends Attributes.RowKeys> postShiftRowKeys, long destination) {
        final SsmDistinctRollupContext context = updateRemoveValues((SsmDistinctRollupContext)singletonContext, preValues);
        FloatSegmentedSortedMultiset ssm = null;
    ...
    private static void flushPrevious(FloatRollupCountDistinctOperator op) {
    ...
    public BucketedContext makeBucketedContext(int size) {
        return new BucketSsmDistinctRollupContext(ChunkType.Float, size);
    }

    @Override
    public SingletonContext makeSingletonContext(int size) {
        return new SsmDistinctRollupContext(ChunkType.Float);
    }
