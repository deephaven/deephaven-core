 ...
 */
public final class NullAwareCharTimsortKernel {
    private NullAwareCharTimsortKernel() {
        throw new UnsupportedOperationException();
    }

    public static <ATTR extends Any> NullAwareCharSortKernelContext<ATTR> createContext(
            final int size) {
        return new NullAwareCharSortKernelContext<>(size);
    }

    /**
     * Sort the values in valuesToSort.
     */
    public static <ATTR extends Any> void sort(NullAwareCharSortKernelContext<ATTR> context,
            final WritableCharChunk<ATTR> valuesToSort) {
        timSort(context, valuesToSort, 0, valuesToSort.size());
    }

    private static <ATTR extends Any> void timSort(NullAwareCharSortKernelContext<ATTR> context,
            WritableCharChunk<ATTR> valuesToSort, int offset, int length) {
        if (length <= 1) {
            return;
        }
    ...
    private static <ATTR extends Any> void ensureMergeInvariants(
            NullAwareCharSortKernelContext<ATTR> context, WritableCharChunk<ATTR> valuesToSort) {
        while (context.runCount > 1) {
    ...
    private static <ATTR extends Any> void merge(NullAwareCharSortKernelContext<ATTR> context,
            WritableCharChunk<ATTR> valuesToSort, int start1, int length1, int length2) {
     ...
     */
    private static <ATTR extends Any> void frontMerge(NullAwareCharSortKernelContext<ATTR> context,
            WritableCharChunk<ATTR> valuesToSort, final int mergeStartPosition, final int start2,
            final int length2) {
        int tempCursor = 0;
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static <ATTR extends Any> void backMerge(NullAwareCharSortKernelContext<ATTR> context,
            WritableCharChunk<ATTR> valuesToSort, final int mergeStartPosition, final int length1) {
    ...
    private static <ATTR extends Any> void copyToTemporary(
            NullAwareCharSortKernelContext<ATTR> context, WritableCharChunk<ATTR> valuesToSort,
            int mergeStartPosition, int remaining1) {
    ...
    private static <ATTR extends Any> void copyToChunk(CharChunk<ATTR> valuesSource,
            WritableCharChunk<ATTR> valuesDest, int sourceStart, int destStart, int length) {
    ...
    private static void insertionSort(WritableCharChunk<?> valuesToSort, int offset, int length) {
    ...
    private static void swap(WritableCharChunk<?> valuesToSort, int a, int b) {
        final char tempChar = valuesToSort.get(a);
        valuesToSort.set(a, valuesToSort.get(b));
        valuesToSort.set(b, tempChar);
    }

    public static class NullAwareCharSortKernelContext<ATTR extends Any> implements Context {
        int minGallop;
        ...
        private NullAwareCharSortKernelContext(int size) {
