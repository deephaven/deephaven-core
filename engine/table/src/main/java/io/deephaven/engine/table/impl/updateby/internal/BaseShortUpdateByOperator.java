...
public abstract class BaseShortUpdateByOperator extends UpdateByCumulativeOperator {
    ...
    public void initializeForUpdate(@NotNull final UpdateContext context,
        ...
        ctx.curVal = lastPrevKey == NULL_ROW_KEY ? QueryConstants.NULL_SHORT : outputSource.getPrevShort(lastPrevKey);
    }
    ...
    public void resetForReprocess(@NotNull final UpdateContext context,
        ...
        final Context ctx = (Context) context;
        if(!ctx.canProcessDirectly) {
            ctx.curVal = firstUnmodifiedKey == NULL_ROW_KEY ? QueryConstants.NULL_SHORT : outputSource.getShort(firstUnmodifiedKey);
        }
    }
