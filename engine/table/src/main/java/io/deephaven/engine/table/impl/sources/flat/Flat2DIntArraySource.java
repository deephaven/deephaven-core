...
public class Flat2DIntArraySource extends AbstractColumnSource<Integer> implements ImmutableColumnSourceGetDefaults.ForInt, WritableColumnSource<Integer>, FillUnordered, InMemoryColumnSource, ChunkedBackingStoreExposedWritableSource {
    ...
    private static int [][] allocateArray(long size) {
        ...
        while (size > SEGMENT_SIZE) {
            data[segment++] = new int[SEGMENT_SIZE];
            size -= SEGMENT_SIZE;
        }
    ...
    private void fillChunkByRanges(WritableChunk<? super Values> destination, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                asIntChunk.copyFromTypedArray(data[segment], Math.toIntExact(start), srcPos.getAndAdd(rangeLength), rangeLength);
                start += rangeLength;
            }
        });
    }
    ...
    private void fillFromChunkByRanges(Chunk<? extends Values> src, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                final long realEnd = Math.min(segmentEnd, end);
                final int rangeLength = Math.toIntExact(realEnd - start + 1);
                asIntChunk.copyToTypedArray(srcPos.getAndAdd(rangeLength), data[segment], Math.toIntExact(start), rangeLength);
                start += rangeLength;
            }
        });
    }
