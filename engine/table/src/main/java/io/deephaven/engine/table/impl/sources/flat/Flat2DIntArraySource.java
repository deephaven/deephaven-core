 ...
 * copy.
 *
 * If your size is smaller than Integer.MAX_VALUE, prefer {@link FlatIntArraySource}.
 */
public class Flat2DIntArraySource extends AbstractDeferredGroupingColumnSource<Integer> implements ImmutableColumnSourceGetDefaults.ForInt, WritableColumnSource<Integer>, FillUnordered, InMemoryColumnSource, ChunkedBackingStoreExposedWritableSource {
    ...
    // region constructor
    public Flat2DIntArraySource(long size) {
        this(size, DEFAULT_SEGMENT_SHIFT);
    }

    public Flat2DIntArraySource(long size, int segmentShift) {
        super(int.class);
    ...
    private static int [][] allocateArray(long size, int segmentSize, boolean nullFilled) {
        final int segments = Math.toIntExact((size + segmentSize - 1) / segmentSize);
        final int [][] data = new int[segments][];
        int segment = 0;
        while (size > segmentSize) {
            data[segment] = new int[segmentSize];
            if (nullFilled) {
                Arrays.fill(data[segment], 0, segmentSize, NULL_INT);
            }
            segment++;
            size -= segmentSize;
        }
        final int remainingSize = Math.toIntExact(size);
        data[segment] = new int[remainingSize];
        if (nullFilled) {
            Arrays.fill(data[segment], 0, remainingSize, NULL_INT);
        }
        return data;
    }
    ...
    public long resetWritableChunkToBackingStore(@NotNull ResettableWritableChunk<?> chunk, long position) {
        ...
        chunk.asResettableWritableIntChunk().resetFromTypedArray((int[])data[segment], 0, data[segment].length);
    ...
    public long resetWritableChunkToBackingStoreSlice(@NotNull ResettableWritableChunk<?> chunk, long position) {
        ...
        chunk.asResettableWritableIntChunk().resetFromTypedArray((int[])data[segment], offset, capacity);
        return capacity;
    }
    ...
    private void fillChunkByRanges(WritableChunk<? super Values> destination, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                asIntChunk.copyFromTypedArray(data[segment], offset, destPos.getAndAdd(rangeLength), rangeLength);
                start += rangeLength;
            }
        });
    }
    ...
    public Chunk<? extends Values> getChunk(@NotNull GetContext context, long firstKey, long lastKey) {
        ...
        return contextWithResettable.resettableIntChunk.resetFromTypedArray(data[segment], Math.toIntExact(firstKey), len);
    }
    ...
    private void fillFromChunkByRanges(Chunk<? extends Values> src, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                final long realEnd = Math.min(segmentEnd, end);
                final int rangeLength = Math.toIntExact(realEnd - start + 1);
                asIntChunk.copyToTypedArray(srcPos.getAndAdd(rangeLength), data[segment], destOffset, rangeLength);
                start += rangeLength;
            }
        });
    }
