...
public class Flat2DIntArraySource extends AbstractDeferredGroupingColumnSource<Integer> implements ImmutableColumnSourceGetDefaults.ForInt, WritableColumnSource<Integer>, FillUnordered, InMemoryColumnSource, ChunkedBackingStoreExposedWritableSource {
    ...
    private static int [][] allocateArray(long size, int segmentSize, boolean nullFilled) {
        ...
        while (size > 0) {
            ...
            data[segment] = new int[thisSegmentSize];
            if (nullFilled) {
                Arrays.fill(data[segment], 0, thisSegmentSize, NULL_INT);
            }
            segment++;
    ...
    public Chunk<? extends Values> getChunk(@NotNull GetContext context, long firstKey, long lastKey) {
        ...
        return contextWithResettable.resettableIntChunk.resetFromTypedArray(data[segment], (int)firstKey, len);
    }
    ...
    public void fillChunkUnordered(@NotNull FillContext context, @NotNull WritableChunk<? super Values> dest, @NotNull LongChunk<? extends RowKeys> keys) {
        ...
        for (int ii = 0; ii < keys.size(); ++ii) {
            ...
            if (rowKey == RowSequence.NULL_ROW_KEY) {
                ...
                intDest.set(ii, getUnsafe((int)(rowKey)));
            }
        }
    }
