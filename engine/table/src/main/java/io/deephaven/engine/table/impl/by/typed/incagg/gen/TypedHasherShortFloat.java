...
final class TypedHasherShortFloat extends IncrementalChunkedOperatorAggregationStateManagerTypedBase {
  ...
  public TypedHasherShortFloat(ColumnSource[] tableKeySources, int tableSize,
  ...
  private int hash(short v0, float v1) {
  ...
  private int maybeMoveMainBucket(HashHandler handler, int bucket, int destBucket,
    ...
    final float v1 = mainKeySource1.getUnsafe(bucket);
    final int hash = hash(v0, v1);
    ...
    if (location == bucket) {
      ...
      mainKeySource1.set(bucket, QueryConstants.NULL_FLOAT);
      handler.doMoveMain(bucket, destBucket);
    }
    return mainInsertLocation;
  }

  private boolean findOverflow(HashHandler handler, short v0, float v1, int chunkPosition,
