...
/**
 * A {@link ColumnSource} for {@link LocalTime}s that is backed by an inner {@link ColumnSource} of
 ...
 */
public class LocalTimeWrapperSource extends AbstractColumnSource<LocalTime>
        implements MutableColumnSourceGetDefaults.ForObject<LocalTime> {
    ...
    public LocalTimeWrapperSource(ColumnSource<ZonedDateTime> inner, ZoneId zone) {
    ...
    private void convertInnerChunk(@NotNull WritableChunk<? super Values> dest, ConvertingFillContext fillContext) {
        final WritableObjectChunk<LocalTime, ? super Values> typedDest = dest.asWritableObjectChunk();
