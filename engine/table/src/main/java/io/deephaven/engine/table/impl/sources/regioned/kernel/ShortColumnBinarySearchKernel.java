...
public class ShortColumnBinarySearchKernel {
    /**
     ...
     * @param usePrev If true, the search will use the previous values (getPrevShort) instead of current values
     *        (getShort).
     *
    ...
    public static RowSet binarySearchMatch(
        ...
        if (order.isAscending()) {
            ...
            for (int idx = 0; idx < unboxed.length && firstPos <= lastPos; ++idx) {
                ...
                final short startValue = usePrev
                        ? source.getPrevShort(selection.get(startResult))
                        : source.getShort(selection.get(startResult));
     ...
     * @param usePrev If true, uses getPrevShort instead of getShort.
    ...
    private static long findStartPosAscending(
        ...
        while (low <= high) {
            ...
            final short midValue = usePrev ? source.getPrevShort(selection.get(mid)) : source.getShort(selection.get(mid));
