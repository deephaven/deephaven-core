...
public class ByteColumnBinarySearchKernel {
     ...
     * @param usePrev If true, the search will use the previous values (getPrevByte) instead of current values
     *        (getByte).
     *
    ...
    public static RowSet binarySearchMatch(
        ...
        if (order.isAscending()) {
            ...
            for (int idx = 0; idx < unboxed.length && firstPos <= lastPos; ++idx) {
                ...
                final byte startValue = usePrev
                        ? source.getPrevByte(selection.get(startResult))
                        : source.getByte(selection.get(startResult));
     ...
     * @param usePrev If true, uses getPrevByte instead of getByte.
    ...
    private static long findStartPosAscending(
        ...
        while (low <= high) {
            ...
            final byte midValue = usePrev ? source.getPrevByte(selection.get(mid)) : source.getByte(selection.get(mid));
