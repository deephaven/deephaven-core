...
public class ByteFindRunsKernel {
    ...
    static void findRuns(
            @NotNull final ByteChunk<? extends Any> sortedValues,
    ...
    public static int compactRuns(
            @NotNull final WritableByteChunk<? extends Any> sortedValues,
            @NotNull final IntChunk<ChunkPositions> offsetsIn) {
        final int numRuns = offsetsIn.size();
        if (numRuns == 0) {
            return -1;
        }
        byte last = sortedValues.get(offsetsIn.get(0));
        sortedValues.set(0, last);
        for (int ri = 1; ri < numRuns; ++ri) {
            final int nextOffset = offsetsIn.get(ri);
            final byte next = sortedValues.get(nextOffset);
            if (leq(next, last)) {
                return ri;
            }
    ...
    private static class ByteFindRunsKernelContext implements FindRunsKernel {
        ...
        public int compactRuns(
                ...
                @NotNull final IntChunk<ChunkPositions> offsetsIn) {
            return ByteFindRunsKernel.compactRuns(sortedValues.asWritableByteChunk(), offsetsIn);
        }
    }
