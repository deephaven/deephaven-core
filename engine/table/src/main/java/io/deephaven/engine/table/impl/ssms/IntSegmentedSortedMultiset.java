...
public final class IntSegmentedSortedMultiset implements SegmentedSortedMultiSet<Integer>, IntVector {
    ...
    private boolean equalsArray(IntVector o) {
        ...
        try (final CloseablePrimitiveIteratorOfInt oit = o.iterator()) {
            if (size == 1) {
                return get(0) == oit.nextInt();
            }

            if (leafCount == 1) {
                for (int ii = 0; ii < size; ii++) {
                    // region DirObjectEquals
                    if (directoryValues[ii] != oit.nextInt()) {
                        return false;
                    }
            ...
            for (int li = 0; li < leafCount; ++li) {
                for (int ai = 0; ai < leafSizes[li]; ai++) {
                    if (leafValues[li][ai] != oit.nextInt()) {
                        return false;
                    }
                }
            }
    ...
    private boolean equalsArray(ObjectVector<?> o) {
        ...
        try (final CloseableIterator<?> oit = o.iterator()) {
            if (size == 1) {
                final Integer val = (Integer) oit.next();
                // region VectorEquals
                if (val == null) {
            ...
            if (leafCount == 1) {
                for (int ii = 0; ii < size; ii++) {
                    ...
                    if (val == null) {
                        ...
                        if (directoryValues[ii] != NULL_INT) {
                            return false;
                        }
                        continue;
                    }
