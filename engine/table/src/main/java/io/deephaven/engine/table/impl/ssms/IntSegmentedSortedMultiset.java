...
public final class IntSegmentedSortedMultiset implements SegmentedSortedMultiSet<Integer>, IntVector {
    ...
    public boolean insert(WritableChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
        ...
        return insert(valuesToInsert.asWritableIntChunk(), counts, offset, length);
    }
     ...
     */
    public boolean insert(WritableIntChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int offset, int length) {
    ...
    private int insertExistingIntoLeaf(WritableIntChunk<? extends Values> valuesToInsert,
        ...
        while (rlpos < leafSize && ripos < end
                && (IntComparisons.leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
            if (IntComparisons.gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    if (IntComparisons.eq(leafValues[rlpos], nextValue)) {
                    ...
                    valuesToInsert.copyFromTypedChunk((WritableIntChunk) valuesToInsert, ripos, wipos.get(),
                            lastInsert - ripos);
    ...
    private void distributeNewIntoLeaves(IntChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts,
        ...
        while (remaining-- > 0) {
            ...
            final boolean useInsertValue = IntComparisons.gt(insertValue, leafValue);
    ...
    private void insertNewIntoLeaf(WritableIntChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, int[] leafValues,
            long[] leafCounts) {
        ...
        while (wpos >= 0) {
            ...
            if (IntComparisons.gt(insertValue, leafValue)) {
    ...
    private void copyRemainingValuesToLeaf(WritableIntChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int insertStart, int[] leafValues, long[] leafCounts, int ripos) {
     ...
     */
    private int maybeCompact(WritableIntChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
     ...
     */
    private int insertExisting(WritableIntChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount == 1) {
            ...
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, offset, wipos, size, directoryValues,
                    directoryCount, NULL_INT, true, end);
    ...
    private void insertInternal(WritableIntChunk<? extends Values> valuesToInsert,
        ...
        if (isSingleton()) {
            if (length == 1 && IntComparisons.eq(valuesToInsert.get(offset), singletonValue)) {
        ...
        if (leafCount > 1 && IntComparisons.gt(valuesToInsert.get(offset), getMaxInt())) {
    ...
    private void makeLeavesInitial(IntChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        ...
        if (leafCount == 1) {
            directoryValues = new int[length];
    ...
    private void packValuesIntoLeaves(IntChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int rpos,
    ...
    private void materializeSingleton(int incomingValueCount) {
        ...
        directoryValues = new int[capacity];
    ...
    private static int lowerBound(int[] valuesToSearch, int lo, int hi, int searchValue) {
        while (lo < hi) {
            ...
            final boolean moveLo = IntComparisons.leq(testValue, searchValue);
            if (moveLo) {
                lo = mid;
                if (lo == hi - 1) {
                    break;
    ...
    private static int upperBound(int[] valuesToSearch, int lo, int hi, int searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final int testValue = valuesToSearch[mid];
            final boolean moveHi = IntComparisons.geq(testValue, searchValue);
            if (moveHi) {
                hi = mid;
            } else {
                lo = mid + 1;
    ...
    private static int lowerBoundExclusive(int[] valuesToSearch, int lo, int hi, int searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final int testValue = valuesToSearch[mid];
            final boolean moveLo = IntComparisons.lt(testValue, searchValue);
            if (moveLo) {
                lo = mid + 1;
                if (lo == hi) {
                    break;
    ...
    public boolean remove(RemoveContext removeContext, WritableChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        return remove(removeContext, valuesToRemove.asWritableIntChunk(), counts, offset, length);
    }
     ...
     */
    public boolean remove(RemoveContext removeContext, WritableIntChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        final long beforeSize = size();
    ...
    private void removeInternal(RemoveContext removeContext, IntChunk<? extends Values> valuesToRemove,
        ...
        if (isSingleton()) {
            ...
            Assert.assertion(IntComparisons.eq(valuesToRemove.get(offset), singletonValue),
                    "IntComparisons.eq(valuesToRemove.get(offset), singletonValue)");
            singletonCount -= counts.get(offset);
            totalSize -= counts.get(offset);
    ...
    private int removeFromLeaf(RemoveContext removeContext, IntChunk<? extends Values> valuesToRemove,
            IntChunk<ChunkLengths> counts, int ripos, int end, int[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
    ...
    private void validateInputs(IntChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        if (!SEGMENTED_SORTED_MULTISET_VALIDATION) {
            return;
        }
        ...
        for (int ii = offset + 1; ii < end; ++ii) {
            ...
            Assert.assertion(IntComparisons.lt(prevValue, curValue), "IntComparisons.lt(prevValue, curValue)",
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    Assert.assertion(IntComparisons.leq(lastValue, directoryValue), "lt(lastValue, directoryValue)",
                            lastValue,
                            "leafValues[ii][leafSizes[ii] - 1]", directoryValue, "directoryValue");

                    if (ii < leafCount - 2) {
                        final int nextDirectoryValue = directoryValues[ii + 1];
                        Assert.assertion(IntComparisons.lt(directoryValue, nextDirectoryValue),
                    ...
                    Assert.assertion(IntComparisons.lt(directoryValue, nextFirstValue),
    ...
    private void validateLeafOrdering() {
        for (int leaf = 0; leaf < leafCount - 1; ++leaf) {
            ...
            Assert.assertion(IntComparisons.lt(lastValue, nextValue), lastValue + " < " + nextValue);
        }
    }
    ...
    private static void validateLeaf(int[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(IntComparisons.lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])",
                    (Integer) thisValue,
                    "values[ii]", (Integer) nextValue, "values[ii + 1]", ii, "ii");
        }
        if (size > 0) {
     ...
     */
    private void appendMaximum(int value, long count) {
        totalSize += count;
        if (leafCount == 0) {
            singletonValue = value;
            singletonCount = count;
        ...
        if (lastLeafSize < leafSize) {
            ...
            leafValues[leafCount] = new int[leafSize];
     ...
     */
    private void prependMinimum(int value, long count) {
        totalSize += count;
        if (leafCount == 0) {
            singletonValue = value;
            singletonCount = count;
        ...
        if (leafCount == 1) {
            ...
            if (newSize <= leafSize) {
                if (directoryValues.length < newSize) {
                    ...
                    final int[] grownValues = new int[newSize];
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(IntComparisons.geq(getMinInt(), destination.getMaxInt()),
                        "geq(getMinInt(), destination.getMaxInt())");
            }
        }
        ...
        if (isSingleton()) {
            ...
            if (destination.size > 0 && IntComparisons.eq(singletonValue, destination.getMaxInt())) {
        ...
        if (destination.size > 0 && IntComparisons.eq(getMinInt(), destination.getMaxInt())) {
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(IntComparisons.leq(getMaxInt(), destination.getMinInt()),
                        "leq(getMaxInt(), destination.getMinInt())");
            }
        }
        ...
        if (isSingleton()) {
            ...
            if (destination.size > 0 && IntComparisons.eq(singletonValue, destination.getMinInt())) {
        ...
        if (destination.size > 0 && IntComparisons.eq(getMaxInt(), destination.getMinInt())) {
     ...
     * (exclusive). Following the {@link IntVector} contract, offsets outside {@code [0, size())} are legal and
     ...
     */
    private int[] keyArray(final long fromIndexInclusive, final long toIndexExclusive) {
        ...
        if (destOffset > 0) {
            Arrays.fill(keyArray, 0, destOffset, NULL_INT);
        }
        if (destOffset + remaining < totalSize) {
            Arrays.fill(keyArray, destOffset + remaining, totalSize, NULL_INT);
        }
    ...
    private void maybeAccumulateAdditions(WritableIntChunk<? extends Values> valuesToInsert, int offset, int length) {
        ...
        if (added == null) {
            added = new IntOpenHashSet(length);
        }
    ...
    public int get(long index) {
        // offsets outside [0, size()) are legal and read as null, per the IntVector contract
    ...
    public ValueIteratorOfInt iterator(final long fromIndexInclusive, final long toIndexExclusive) {
        ...
        return ValueIteratorOfInt.wrapWithNulls(
     ...
     */
    private ValueIteratorOfInt inRangeIterator(final long fromIndexInclusive, final long toIndexExclusive) {
        if (leafCount <= 1) {
        ...
        return new ValueIteratorOfInt() {
            ...
            private int[] values = startLeaf < leafCount ? leafValues[startLeaf] : null;
    ...
    public IntVector subVector(long fromIndexInclusive, long toIndexExclusive) {
        // materialized rather than a IntVectorSlice view: an SSM is live and mutable, and a slice would capture our
        // size at construction and then read stale bounds
        return new IntVectorDirect(keyArray(fromIndexInclusive, toIndexExclusive));
    }
    ...
    private boolean equalsArray(IntVector o) {
        ...
        try (final CloseablePrimitiveIteratorOfInt oit = o.iterator()) {
            if (size == 1) {
                return IntComparisons.eq(get(0), oit.nextInt());
            }

            if (leafCount == 1) {
                for (int ii = 0; ii < size; ii++) {
                    if (!IntComparisons.eq(directoryValues[ii], oit.nextInt())) {
                        return false;
                    }
                }
            ...
            for (int li = 0; li < leafCount; ++li) {
                for (int ai = 0; ai < leafSizes[li]; ai++) {
                    if (!IntComparisons.eq(leafValues[li][ai], oit.nextInt())) {
                        return false;
                    }
                }
            }
     ...
     * {@link IntVector#hashCode(IntVector)} would: the same seed, the same multiplier, and the same per-element
     * {@link IntComparisons#hashCode(int)}. That per-element hash is also why {@link #equals(Object)} must compare
     * elements with {@link IntComparisons#eq(int, int)} rather than {@code ==}.
     *
     * <p>
     ...
     * {@code TestIntSegmentedSortedMultiset#testHashCodeMatchesVectorHelper} pins the two against each other across
    ...
    public int hashCode() {
        ...
        if (leafCount == 1) {
            if (directoryValues == null) {
                return 31 * result + IntComparisons.hashCode(singletonValue);
            }

            for (int ii = 0; ii < size; ++ii) {
                result = 31 * result + IntComparisons.hashCode(directoryValues[ii]);
            }
        ...
        for (int li = 0; li < leafCount; ++li) {
            final int[] values = leafValues[li];
            final int leafSz = leafSizes[li];
            for (int ai = 0; ai < leafSz; ++ai) {
                result = 31 * result + IntComparisons.hashCode(values[ai]);
            }
        }
