...
public class IntRollupUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final WritableIntChunk<? extends Values> addValues = context.values.asWritableIntChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<IntSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
    ...
    public void removeChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final SsmUniqueRollupContext context = (SsmUniqueRollupContext) bucketedContext;
        final IntChunk<? extends Values> valueChunk = values.asIntChunk();
        final WritableIntChunk<? extends Values> removeValues = context.values.asWritableIntChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<IntSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
        ...
        final WritableIntChunk<? extends Values> removeValues = context.values.asWritableIntChunk();
        final WritableIntChunk<? extends Values> addValues = context.postValues.asWritableIntChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<IntSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
    ...
    private void loadState(long destination, MutableLong count,
            MutableObject<IntSegmentedSortedMultiset> ssmHolder) {
     ...
     * {@link IntCompactModifications} instead and calls {@link #applyAdds} directly.
     */
    private void compactAndApplyAdds(long destination, WritableIntChunk<? extends Values> values, int addCount,
     ...
     */
    private void applyAdds(long destination, WritableIntChunk<? extends Values> values, int distinctCount,
            WritableIntChunk<ChunkLengths> counts, MutableLong count,
            MutableObject<IntSegmentedSortedMultiset> ssmHolder) {
        if (distinctCount == 0) {
            return;
        }
        final IntSegmentedSortedMultiset ssm = ssmHolder.getValue();
        if (ssm != null) {
     ...
     * with {@link IntCompactModifications} instead and calls {@link #applyRemoves} directly.
     */
    private void compactAndApplyRemoves(long destination, WritableIntChunk<? extends Values> values, int removeCount,
            WritableIntChunk<ChunkLengths> counts, SegmentedSortedMultiSet.RemoveContext removeContext,
            MutableLong count, MutableObject<IntSegmentedSortedMultiset> ssmHolder) {
        if (removeCount == 0) {
            return;
        }
     ...
     */
    private void applyRemoves(long destination, WritableIntChunk<? extends Values> values, int distinctCount,
            WritableIntChunk<ChunkLengths> counts, SegmentedSortedMultiSet.RemoveContext removeContext,
            MutableLong count, MutableObject<IntSegmentedSortedMultiset> ssmHolder) {
        if (distinctCount == 0) {
            return;
     ...
     */
    private boolean finalizeState(long destination, long priorState, int priorValue, long count,
            IntSegmentedSortedMultiset ssm) {
        final int newValue;
        final long newCount;
        if (ssm != null) {
        ...
        if (!IntComparisons.eq(newValue, priorValue)) {
    ...
    public BucketedContext makeBucketedContext(int size) {
        return new SsmUniqueRollupContext(ChunkType.Int, size);
    }
