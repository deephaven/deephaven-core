...
// ****** Edit CharChunkSsaStamp and run "./gradlew replicateReverseSegmentedSortedArray" to regenerate
//
// @formatter:off
...
public class CharReverseChunkSsaStamp implements ChunkSsaStamp {
    ...
    public void processEntry(Chunk<Values> leftStampValues, Chunk<RowKeys> leftStampKeys, SegmentedSortedArray ssa,
        ...
        processEntry(leftStampValues.asCharChunk(), leftStampKeys, (CharReverseSegmentedSortedArray) ssa, rightKeysForLeft,
                disallowExactMatch);
    }

    private static void processEntry(CharChunk<Values> leftStampValues, Chunk<RowKeys> leftStampKeys,
            CharReverseSegmentedSortedArray ssa, WritableLongChunk<RowKeys> rightKeysForLeft, boolean disallowExactMatch) {
    ...
    public int findModified(int first, Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        return findModified(first, leftStampValues.asCharChunk(), leftStampKeys, rowRedirection,
                rightStampChunk.asCharChunk(), rightStampIndices, modifiedBuilder, disallowExactMatch);
    }

    private static int findModified(int leftLowIdx, CharChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            RowRedirection rowRedirection, CharChunk<? extends Values> rightStampChunk,
            LongChunk<RowKeys> rightStampIndices, RowSetBuilderRandom modifiedBuilder, boolean disallowExactMatch) {
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final char rightStampValue = rightStampChunk.get(ii);
    ...
    public void applyShift(Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        applyShift(leftStampValues.asCharChunk(), leftStampKeys, rightStampChunk.asCharChunk(), rightStampKeys,
                shiftDelta, rowRedirection, disallowExactMatch);
    }

    private void applyShift(CharChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            CharChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightStampKeys, long shiftDelta,
            WritableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final char rightStampValue = rightStampChunk.get(ii);
    ...
    private static int findFirstResponsiveLeft(int leftLowIdx, CharChunk<Values> leftStampValues,
            boolean disallowExactMatch, char rightStampValue) {
