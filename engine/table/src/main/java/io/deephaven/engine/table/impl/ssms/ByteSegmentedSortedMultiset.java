...
public final class ByteSegmentedSortedMultiset implements SegmentedSortedMultiSet<Byte>, ByteVector {
    ...
    public boolean insert(WritableChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
        ...
        return insert(valuesToInsert.asWritableByteChunk(), counts, offset, length);
    }
     ...
     */
    public boolean insert(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int offset, int length) {
     ...
     * {@link #insert(WritableByteChunk, WritableIntChunk, int, int)} for one value without requiring the caller to wrap
    ...
    private boolean insertInterior(byte value, long count) {
        ...
        final byte[] leafValue = leafValues[leaf];
     ...
     */
    private void splitLeafForInsert(int leaf, int ip, byte value, long count) {
        makeLeafHole(leaf + 1, 1);
        leafCount++;
        final byte[] lowerValues = leafValues[leaf];
        final long[] lowerCounts = leafCounts[leaf];
        final byte[] upperValues = leafValues[leaf + 1] = new byte[leafSize];
    ...
    private int insertExistingIntoLeaf(WritableByteChunk<? extends Values> valuesToInsert,
            ...
            long[] leafCounts, byte maxInsert, boolean lastLeaf, int end) {
        int rlpos = 0;
        byte nextValue;
        while (rlpos < leafSize && ripos < end
                && (ByteComparisons.leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
     ...
     */
    private int maybeCompact(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
     ...
     */
    private int insertExisting(WritableByteChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount == 1) {
            ...
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, offset, wipos, size, directoryValues,
                    directoryCount, NULL_BYTE, true, end);
    ...
    private void insertInternal(WritableByteChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount > 1 && ByteComparisons.gt(valuesToInsert.get(offset), getMaxByte())) {
    ...
    private void makeLeavesInitial(ByteChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        ...
        if (leafCount == 1) {
            directoryValues = new byte[length];
    ...
    private void materializeSingleton(int incomingValueCount) {
        ...
        directoryValues = new byte[capacity];
    ...
    public boolean remove(RemoveContext removeContext, WritableChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        return remove(removeContext, valuesToRemove.asWritableByteChunk(), counts, offset, length);
    }
     ...
     */
    public boolean remove(RemoveContext removeContext, WritableByteChunk<? extends Values> valuesToRemove,
            WritableIntChunk<ChunkLengths> counts, int offset, int length) {
        final long beforeSize = size();
     ...
     * {@link #remove(RemoveContext, WritableByteChunk, WritableIntChunk, int, int)} for one value without requiring the
    ...
    private void removeInternal(RemoveContext removeContext, ByteChunk<? extends Values> valuesToRemove,
    ...
    private void validateInputs(ByteChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts, int offset,
            int length) {
        if (!SEGMENTED_SORTED_MULTISET_VALIDATION) {
            return;
        }
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    Assert.assertion(ByteComparisons.leq(lastValue, directoryValue), "lt(lastValue, directoryValue)",
                            lastValue,
                            "leafValues[ii][leafSizes[ii] - 1]", directoryValue, "directoryValue");

                    if (ii < leafCount - 2) {
                        final byte nextDirectoryValue = directoryValues[ii + 1];
                        Assert.assertion(ByteComparisons.lt(directoryValue, nextDirectoryValue),
                    ...
                    Assert.assertion(ByteComparisons.lt(directoryValue, nextFirstValue),
    ...
    private static void validateLeaf(byte[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(ByteComparisons.lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])",
                    (Byte) thisValue,
    ...
    private void appendMaximum(byte value, long count) {
        ...
        if (leafCount == 1) {
            ...
            leafValues[1] = new byte[leafSize];
        ...
        if (lastLeafSize < leafSize) {
            ...
            leafValues[leafCount] = new byte[leafSize];
        ...
        if (leafCount == 1) {
            ...
            if (newSize <= leafSize) {
                if (directoryValues.length < newSize) {
                    ...
                    final byte[] grownValues = new byte[newSize];
            ...
            leafValues[0] = new byte[leafSize];
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(ByteComparisons.geq(getMinByte(), destination.getMaxByte()),
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(ByteComparisons.leq(getMaxByte(), destination.getMinByte()),
     ...
     * (exclusive). Following the {@link ByteVector} contract, offsets outside {@code [0, size())} are legal and
     ...
     */
    private byte[] keyArray(final long fromIndexInclusive, final long toIndexExclusive) {
        ...
        if (remaining == 0) {
            ...
            Arrays.fill(keyArray, NULL_BYTE);
            return keyArray;
        }
        ...
        if (destOffset > 0) {
            Arrays.fill(keyArray, 0, destOffset, NULL_BYTE);
        }
        if (destOffset + remaining < totalSize) {
            Arrays.fill(keyArray, destOffset + remaining, totalSize, NULL_BYTE);
        }
    ...
    private void maybeAccumulateAdditions(WritableByteChunk<? extends Values> valuesToInsert, int offset, int length) {
    ...
    private void maybeAccumulateAddition(byte valueAdded) {
        if (!accumulateDeltas) {
            return;
        }
    ...
    public byte get(long index) {
        // offsets outside [0, size()) are legal and read as null, per the ByteVector contract
    ...
    public ValueIteratorOfByte iterator(final long fromIndexInclusive, final long toIndexExclusive) {
        ...
        return ValueIteratorOfByte.wrapWithNulls(
     ...
     */
    private ValueIteratorOfByte inRangeIterator(final long fromIndexInclusive, final long toIndexExclusive) {
        if (leafCount <= 1) {
        ...
        return new ValueIteratorOfByte() {
            ...
            private byte[] values = startLeaf < leafCount ? leafValues[startLeaf] : null;
    ...
    public ByteVector subVector(long fromIndexInclusive, long toIndexExclusive) {
        // materialized rather than a ByteVectorSlice view: an SSM is live and mutable, and a slice would capture our
    ...
    private boolean equalsArray(ByteVector o) {
        ...
        try (final CloseablePrimitiveIteratorOfByte oit = o.iterator()) {
            if (size == 1) {
                return ByteComparisons.eq(get(0), oit.nextByte());
            }

            if (leafCount == 1) {
                for (int ii = 0; ii < size; ii++) {
                    if (!ByteComparisons.eq(directoryValues[ii], oit.nextByte())) {
                        return false;
                    }
                }
            ...
            for (int li = 0; li < leafCount; ++li) {
                for (int ai = 0; ai < leafSizes[li]; ai++) {
                    if (!ByteComparisons.eq(leafValues[li][ai], oit.nextByte())) {
                        return false;
                    }
                }
            }
     ...
     * {@link ByteVector#hashCode(ByteVector)} would: the same seed, the same multiplier, and the same per-element
     * {@link ByteComparisons#hashCode(byte)}. That per-element hash is also why {@link #equals(Object)} must compare
     * elements with {@link ByteComparisons#eq(byte, byte)} rather than {@code ==}.
     *
     * <p>
     ...
     * {@code TestByteSegmentedSortedMultiset#testHashCodeMatchesVectorHelper} pins the two against each other across
    ...
    public int hashCode() {
        ...
        if (leafCount == 1) {
            if (directoryValues == null) {
                return 31 * result + ByteComparisons.hashCode(singletonValue);
            }

            for (int ii = 0; ii < size; ++ii) {
                result = 31 * result + ByteComparisons.hashCode(directoryValues[ii]);
            }
        ...
        for (int li = 0; li < leafCount; ++li) {
            final byte[] values = leafValues[li];
            final int leafSz = leafSizes[li];
            for (int ai = 0; ai < leafSz; ++ai) {
                result = 31 * result + ByteComparisons.hashCode(values[ai]);
            }
        }
