...
public class LongReverseChunkSsaStamp implements ChunkSsaStamp {
    ...
    private static void processEntry(LongChunk<Values> leftStampValues, Chunk<RowKeys> leftStampKeys, LongReverseSegmentedSortedArray ssa, WritableLongChunk<RowKeys> rightKeysForLeft, boolean disallowExactMatch) {
    ...
    private static int findModified(int leftLowIdx, LongChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys, RowRedirection rowRedirection, LongChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightStampIndices, RowSetBuilderRandom modifiedBuilder, boolean disallowExactMatch) {
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final long rightStampValue = rightStampChunk.get(ii);
    ...
    private void applyShift(LongChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys, LongChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightStampKeys, long shiftDelta, WritableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final long rightStampValue = rightStampChunk.get(ii);
