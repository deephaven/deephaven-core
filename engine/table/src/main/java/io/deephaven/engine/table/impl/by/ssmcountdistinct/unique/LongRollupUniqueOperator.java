 ...
 */
public class LongRollupUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    private final LongArraySource singletonValue;
    ...
    public LongRollupUniqueOperator(
            // region Constructor
            Class<?> type,
        ...
        this.singletonValue = new LongArraySource();
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final LongChunk<? extends Values> valueChunk = values.asLongChunk();
        final WritableLongChunk<? extends Values> addValues = context.values.asWritableLongChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<LongSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            final long priorValue = internalResult.getUnsafe(destination);
    ...
    public void removeChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final SsmUniqueRollupContext context = (SsmUniqueRollupContext) bucketedContext;
        final LongChunk<? extends Values> valueChunk = values.asLongChunk();
        final WritableLongChunk<? extends Values> removeValues = context.values.asWritableLongChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<LongSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
            ...
            WritableBooleanChunk<Values> stateModified) {
        final SsmUniqueRollupContext context = (SsmUniqueRollupContext) bucketedContext;
        final LongChunk<? extends Values> prevValueChunk = preValues.asLongChunk();
        final LongChunk<? extends Values> postValueChunk = postValues.asLongChunk();
        final WritableLongChunk<? extends Values> removeValues = context.values.asWritableLongChunk();
        final WritableLongChunk<? extends Values> addValues = context.postValues.asWritableLongChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<LongSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
    ...
    private void loadState(long sc, long destination, MutableLong count,
            MutableObject<LongSegmentedSortedMultiset> ssmHolder) {
        if (isNonUniqueState(sc)) {
            // only a non-unique encoding can be backed by an SSM holding two or more distinct values
            final LongSegmentedSortedMultiset ssm = ssms.getCurrentSsm(destination);
     ...
     * {@link LongCompactModifications} instead and calls {@link #applyAdds} directly.
     */
    private void compactAndApplyAdds(long destination, WritableLongChunk<? extends Values> values, int addCount,
     ...
     */
    private void applyAdds(long destination, WritableLongChunk<? extends Values> values, int distinctCount,
            WritableIntChunk<ChunkLengths> counts, MutableLong count,
            MutableObject<LongSegmentedSortedMultiset> ssmHolder) {
        if (distinctCount == 0) {
            return;
        }
        final LongSegmentedSortedMultiset ssm = ssmHolder.getValue();
        if (ssm != null) {
        ...
        if (count.get() == 0) {
            ...
            final LongSegmentedSortedMultiset newSsm = ssmForSlot(destination);
     ...
     * with {@link LongCompactModifications} instead and calls {@link #applyRemoves} directly.
     */
    private void compactAndApplyRemoves(long destination, WritableLongChunk<? extends Values> values, int removeCount,
            WritableIntChunk<ChunkLengths> counts, SegmentedSortedMultiSet.RemoveContext removeContext,
            MutableLong count, MutableObject<LongSegmentedSortedMultiset> ssmHolder) {
        if (removeCount == 0) {
            return;
        }
     ...
     */
    private void applyRemoves(long destination, WritableLongChunk<? extends Values> values, int distinctCount,
            WritableIntChunk<ChunkLengths> counts, SegmentedSortedMultiSet.RemoveContext removeContext,
            MutableLong count, MutableObject<LongSegmentedSortedMultiset> ssmHolder) {
        if (distinctCount == 0) {
            return;
     ...
     */
    private boolean finalizeState(long destination, long priorState, long priorValue, long count,
            LongSegmentedSortedMultiset ssm) {
        final long newValue;
        final long newCount;
        if (ssm != null) {
        ...
        if (!LongComparisons.eq(newValue, priorValue)) {
    ...
    public BucketedContext makeBucketedContext(int size) {
        return new SsmUniqueRollupContext(ChunkType.Long, size);
    }
