...
public class LongRollupUniqueOperator implements IterativeChunkedAggregationOperator {
     ...
     * {@link LongCompactModifications#compactAndCountModifications} sorts, counts, and diffs each run in place. The
    ...
    private void flattenDeltas(SizedChunk<Values> targetValues, SizedIntChunk<ChunkLengths> targetCounts,
        ...
        for (int ii = 0; ii < starts.size(); ii++) {
            ...
            for (int kk = startPos; kk < startPos + curLength; kk++) {
                ...
                if (removed) {
                    ssm.fillRemovedChunk(targetValues.get().asWritableLongChunk(), currentPos + newLength);
                } else {
                    ssm.fillAddedChunk(targetValues.get().asWritableLongChunk(), currentPos + newLength);
                }
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
        ...
        final SegmentedSortedMultiSet.RemoveContext removeContext = removeContextFactory.get();
        final WritableLongChunk<? extends Values> preValueCopy = context.valueCopy.get().asWritableLongChunk();
        final WritableIntChunk<ChunkLengths> removedCounts = context.counts.get();
        final WritableLongChunk<? extends Values> postValueCopy = context.postValues.get().asWritableLongChunk();
        ...
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            ...
            if (removedRunLength != 0 || addedRunLength != 0) {
                LongCompactModifications.compactAndCountModifications(preValueCopy, removedCounts, postValueCopy,
     ...
     * {@link LongCompactModifications#compactAndCountModifications} sorts, counts, and diffs the run in place.
     */
    private int flattenDeltas(SizedChunk<Values> targetValues, SizedIntChunk<ChunkLengths> targetCounts,
        ...
        for (int ii = 0; ii < values.size(); ii++) {
            ...
            if (removed) {
                ssm.fillRemovedChunk(targetValues.get().asWritableLongChunk(), currentPos);
            } else {
                ssm.fillAddedChunk(targetValues.get().asWritableLongChunk(), currentPos);
            }
            currentPos += size;
            // we have to do this every time otherwise ensureCapacityPreserve will not do anything.
    ...
    public boolean modifyChunk(SingletonContext singletonContext, int chunkSize, Chunk<? extends Values> preValues,
        ...
        LongCompactModifications.compactAndCountModifications(context.valueCopy.get().asWritableLongChunk(),
                context.counts.get(), context.postValues.get().asWritableLongChunk(), context.postCounts.get(),
    ...
    public void startTrackingPrevValues() {
        ...
        prevFlusher = new UpdateCommitter<>(this, ExecutionContext.getContext().getUpdateGraph(),
                LongRollupUniqueOperator::flushPrevious);
        touchedStates = RowSetFactory.empty();
        ssms.startTrackingPrevValues();
        internalResult.startTrackingPrevValues();
    }

    private static void flushPrevious(LongRollupUniqueOperator op) {
