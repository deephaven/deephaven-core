...
public class LongRollupUniqueOperator implements IterativeChunkedAggregationOperator {
     ...
     */
    private void addValueToMultiset(long destination, long value, MutableLong count,
            MutableObject<LongSegmentedSortedMultiset> ssmHolder) {
        ...
        if (LongComparisons.eq(value, held)) {
            count.increment();
            return;
        }
     ...
     */
    private void removeValueFromMultiset(long destination, long value, MutableLong count,
            MutableObject<LongSegmentedSortedMultiset> ssmHolder) {
        final LongSegmentedSortedMultiset ssm = ssmHolder.getValue();
        if (ssm != null) {
            ssm.remove(value, 1);
