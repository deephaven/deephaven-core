...
public class FloatChunkedUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
        ...
        final WritableFloatChunk<? extends Values> preValueCopy = context.valueCopy.asWritableFloatChunk();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int runLength = context.lengthCopy.get(ii);
            if (runLength == 0) {
                continue;
        ...
        final WritableFloatChunk<? extends Values> postValueCopy = context.valueCopy.asWritableFloatChunk();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int runLength = context.lengthCopy.get(ii);
            final int startPosition = startPositions.get(ii);
            final long destination = destinations.get(startPosition);
