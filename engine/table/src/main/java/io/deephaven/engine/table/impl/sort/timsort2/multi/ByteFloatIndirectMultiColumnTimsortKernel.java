...
/**
 * This implements a timsort kernel for a multi-column key (Byte, Float) that never moves the column values:
 ...
 */
public final class ByteFloatIndirectMultiColumnTimsortKernel {
    private ByteFloatIndirectMultiColumnTimsortKernel() {
        throw new UnsupportedOperationException();
    }

    public static <PERMUTE_VALUES_ATTR extends Any> ByteFloatIndirectMultiColumnSortKernelContext<PERMUTE_VALUES_ATTR> createContext(
            final int size) {
        return new ByteFloatIndirectMultiColumnSortKernelContext<>(size);
    }
     ...
     */
    public static void sort(ByteFloatIndirectMultiColumnSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1) {
        timSort(context, positions, valuesToSort0, valuesToSort1, 0, positions.size());
    }

    private static void timSort(ByteFloatIndirectMultiColumnSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1, int offset, int length) {
        if (length <= 1) {
            return;
        }
     ...
     */
    private static int compareColumns(ByteChunk<?> valuesToSort0, FloatChunk<?> valuesToSort1,
    ...
    private static void ensureMergeInvariants(
            ByteFloatIndirectMultiColumnSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1) {
        while (context.runCount > 1) {
    ...
    private static void merge(ByteFloatIndirectMultiColumnSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1, int start1, int length1, int length2) {
     ...
     */
    private static void frontMerge(ByteFloatIndirectMultiColumnSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1, final int mergeStartPosition, final int start2,
            final int length2) {
        int tempCursor = 0;
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static void backMerge(ByteFloatIndirectMultiColumnSortKernelContext<?> context,
            WritableIntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1, final int mergeStartPosition, final int length1) {
    ...
    private static void copyToTemporary(ByteFloatIndirectMultiColumnSortKernelContext<?> context,
    ...
    private static int upperBound(IntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1, int lo, int hi, int searchPos) {
    ...
    private static int bound(IntChunk<ChunkPositions> positions, ByteChunk<?> valuesToSort0,
            FloatChunk<?> valuesToSort1, int lo, int hi, int searchPos, final boolean lower) {
    ...
    private static void insertionSort(WritableIntChunk<ChunkPositions> positions,
            ByteChunk<?> valuesToSort0, FloatChunk<?> valuesToSort1, int offset, int length) {
    ...
    public static class ByteFloatIndirectMultiColumnSortKernelContext<PERMUTE_VALUES_ATTR extends Any> implements MultiColumnSortKernel<PERMUTE_VALUES_ATTR> {
        int minGallop;
        ...
        private ByteFloatIndirectMultiColumnSortKernelContext(int size) {
        ...
        public void sort(WritableLongChunk<PERMUTE_VALUES_ATTR> valuesToPermute,
            ...
            ByteFloatIndirectMultiColumnTimsortKernel.sort(this, positions, valuesToSort[0].asByteChunk(), valuesToSort[1].asFloatChunk());
