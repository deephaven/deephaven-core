 ...
 * key.
 */
public final class ByteSegmentedSortedArray implements SegmentedSortedArray {
     ...
     */
    private byte[] directoryValues;
    private long[] directoryRowKeys;

    private int[] leafSizes;
    private byte[][] leafValues;
     ...
     */
    public ByteSegmentedSortedArray(int leafSize) {
    ...
    public <T extends Any> int insertAndGetNextValue(Chunk<T> valuesToInsert,
        ...
        return findNext(valuesToInsert.asByteChunk(), rowKeysToInsert, nextValue.asWritableByteChunk());
    }
     ...
     */
    private <T extends Any> int findNext(ByteChunk<T> stampValues, LongChunk<? extends RowKeys> stampRowKeys,
            WritableByteChunk<T> nextValues) {
        ...
        while (stampsFound < stampValues.size()) {
            ...
            final byte searchValue = stampValues.get(stampsFound);
    ...
    private static <T extends Any> int findNextOneLeaf(int offset, ByteChunk<T> stampValues,
            LongChunk<? extends RowKeys> stampRowKeys, WritableByteChunk<T> nextValues, int leafSize, byte[] leafValues,
            long[] leafKeys) {
        int lo = 0;

        for (int ii = offset; ii < stampValues.size(); ++ii) {
            final byte searchValue = stampValues.get(ii);
     ...
     */
    void insert(ByteChunk<? extends Any> valuesToInsert, LongChunk<? extends RowKeys> rowKeysToInsert) {
        ...
        if (leafCount == 0) {
            ...
            try (final ResettableByteChunk<Any> leafValuesInsertChunk = ResettableByteChunk.makeResettableChunk();
                ...
                while (firstValuesPosition < insertSize) {
                    ...
                    if (firstLeaf == leafCount - 1) {
                        ...
                        final byte leafMaxValue = leafValues[firstLeaf + 1][0];
                    ...
                    if (sizeForThisLeaf <= leafSize) {
                        ...
                        if (isLastLeaf && isAfterLeaf(leafSizes[firstLeaf], leafValues[firstLeaf],
                            ...
                            for (int leaf = firstLeaf; leaf < firstLeaf + newLeafCount; ++leaf) {
                                if (leaf > firstLeaf) {
                                    leafValues[leaf] = new byte[leafSize];
    ...
    private void mergeLeaves(int leaf, IntList leavesToRemove) {
        ...
        final byte[] tmpValues = leafValues[leaf + 1];
    ...
    private void copyToLeaf(int leafOffset, byte[] leafValues, ByteChunk<? extends Any> insertValues,
    ...
    private void moveLeafValues(byte[] leafValues, long[] leafRowKeys, int srcPos, int destPos, int length) {
        System.arraycopy(leafValues, srcPos, leafValues, destPos, length);
        System.arraycopy(leafRowKeys, srcPos, leafRowKeys, destPos, length);
    }
     ...
     */
    private static void clearValues(byte[] values, int from, int to) {
        // region clearValues
    ...
    private void promoteDirectory(int newLeafCount) {
        ...
        leafValues = new byte[newLeafCount][];
        ...
        directoryValues = new byte[newLeafCount];
    ...
    private void makeLeafHole(int holePosition, int holeSize) {
        ...
        // region fillValue
        Arrays.fill(directoryValues, holePosition, holePosition + holeSize, Byte.MIN_VALUE);
        // endregion fillValue
    ...
    private void distributeValues(int targetSize, int startingLeaf, int distributionSlots,
        ...
        for (int workingSlot = startingLeaf + distributionSlots - 1; workingSlot >= startingLeaf; workingSlot--) {
            if (workingSlot > startingLeaf) {
                leafValues[workingSlot] = new byte[leafSize];
            ...
            for (int wpos = leafSize - 1; wpos >= 0; wpos--) {
                ...
                final byte vall = leafValues[startingLeaf][rposl];
    ...
    private void makeSingletonLeaf(ByteChunk<? extends Any> values, LongChunk<? extends RowKeys> rowKeys) {
        directoryValues = new byte[values.size()];
        directoryRowKeys = new long[rowKeys.size()];
        copyToLeaf(0, directoryValues, values, directoryRowKeys, rowKeys);
        leafCount = 1;
    }

    private void makeLeavesInitial(ByteChunk<? extends Any> values, LongChunk<? extends RowKeys> rowKeys) {
        ...
        leafValues = new byte[leafCount][];
        leafRowKeys = new long[leafCount][];
        directoryValues = new byte[leafCount];
        ...
        for (int ii = 0; ii < leafCount; ++ii) {
            ...
            leafValues[ii] = new byte[leafSize];
    ...
    private void insertIntoLeaf(int leafSize, byte[] leafValues, ByteChunk<? extends Any> insertValues,
        ...
        while (wpos >= 0) {
            ...
            final byte vall = leafValues[rposl];
            ...
            if (iwins > minGallop) {
                ...
                final byte searchValue = leafValues[rposl];
                ...
                final byte searchValue = insertValues.get(rposi);
    ...
    private boolean isAfterLeaf(int leafSize, byte[] leafValues, ByteChunk<? extends Any> insertValues,
            long[] leafRowKeys, LongChunk<? extends RowKeys> insertRowKeys) {
        final byte firstInsertValue = insertValues.get(0);
        final byte lastLeafValue = leafValues[leafSize - 1];
    ...
    private void removeFromLeaf(int leafSize, byte[] leafValues, ByteChunk<? extends Any> removeValues,
        ...
        for (int rposl = 0; rposl < leafSize;) {
            ...
            if (++lwin >= minGallop) {
                ...
                final byte searchValue = removeValues.get(rposi);
    ...
    private static int lowerBound(ByteChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, byte searchValue, long searchKey) {
        while (lo < hi) {
    ...
    private static int upperBound(ByteChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, byte searchValue, long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final byte testValue = valuesToSearch.get(mid);
    ...
    private static int lowerBound(byte[] valuesToSearch, long[] rowKeysToSearch, int lo, int hi, byte searchValue,
            long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
    ...
    private static int upperBound(byte[] valuesToSearch, int lo, int hi, byte searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final byte testValue = valuesToSearch[mid];
            final int comparison = doComparison(testValue, searchValue);
     ...
     */
    private static int bound(byte[] valuesToSearch, long[] rowKeysToSearch, final byte searchValue, long searchRowKey,
            int lo, int hi) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final byte testValue = valuesToSearch[mid];
    ...
    public void remove(Chunk<? extends Any> valuesToRemove, LongChunk<? extends RowKeys> rowKeysToRemove) {
        remove(valuesToRemove.asByteChunk(), rowKeysToRemove);
    }
    ...
    public void removeAndGetPrior(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> rowKeysToRemove,
        ...
        removeAndGetNextInternal(stampChunk.asByteChunk(), rowKeysToRemove, priorRedirections);
    }

    private void remove(ByteChunk<? extends Any> valuesToRemove, LongChunk<? extends RowKeys> rowKeysToRemove) {
     ...
     */
    private void removeAndGetNextInternal(ByteChunk<? extends Any> valuesToRemove,
        ...
        if (leafCount == 1) {
            ...
            try (final ResettableByteChunk<Any> leafValuesRemoveChunk = ResettableByteChunk.makeResettableChunk();
                ...
                while (firstValuesPosition < removeSize) {
                    ...
                    if (firstLeaf == leafCount - 1) {
                        ...
                        final byte leafMaxValue = directoryValues[firstLeaf];
    ...
    public void applyShift(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk, long shiftDelta) {
        applyShift(stampChunk.asByteChunk(), keyChunk, shiftDelta);
    }

    @Override
    public void applyShiftReverse(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        applyShiftReverse(stampChunk.asByteChunk(), keyChunk, shiftDelta);
    }

    private void applyShift(ByteChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
        ...
        if (leafCount == 1) {
            ...
            try (final ResettableByteChunk<Any> leafValuesChunk = ResettableByteChunk.makeResettableChunk();
    ...
    private void shiftLeaf(int leafSize, byte[] leafValues, ByteChunk<? extends Any> shiftValues, long[] leafRowKeys,
        ...
        for (int leafPos = 0; leafPos < leafSize;) {
            ...
            if (++lwin >= minGallop) {
                ...
                final byte searchValue = shiftValues.get(shiftPos);
    ...
    private void applyShiftReverse(ByteChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
        final int shiftSize = stampChunk.size();
        if (shiftSize == 0) {
            ...
            try (final ResettableByteChunk<Any> leafValuesChunk = ResettableByteChunk.makeResettableChunk();
                ...
                while (lastValuesPosition >= 0) {
                    ...
                    if (firstLeaf == 0) {
                        ...
                        final byte leafMinValue = leafValues[firstLeaf][0];
    ...
    private void shiftLeafReverse(int leafSize, byte[] leafValues, ByteChunk<? extends Any> shiftValues,
    ...
    private static void validateLeaf(byte[] values, long[] rowKeys, int size) {
        if (!SEGMENTED_SORTED_ARRAY_VALIDATION) {
            return;
        }
    ...
    public final class Iterator {
        ...
        private void advanceToInternal(byte value, boolean advanceToLast) {
            if (leafCount == 0) {
                return;
            }
         ...
         * find it.
         */
        private void findInLeaf(byte value) {
            if (disallowExactMatch) {
                if (leafCount == 1) {
        ...
        private void findLastInLeaf(byte value) {
            if (leafCount == 1) {
                indexWithinLeaf =
         ...
         * find it.
         */
        private void findFirstInLeaf(byte value) {
