    ...
    // endregion extensions
{
    ...
    private static final long EMPTY_STATE_VALUE = QueryConstants.NULL_LONG;
    ...
    private final LongArraySource resultSlot
    ...
    // @StateColumnSourceType@ from \QLongArraySource\E
    private final LongArraySource overflowResultSlot
            // @StateColumnSourceConstructor@ from \QLongArraySource()\E
            = new LongArraySource();
    ...
    private final RowRedirection resultIndexToHashSlot = new IntColumnSourceWritableRowRedirection(outputPositionToHashSlot);
    private long nextOutputPosition = 0;
    // endregion extra variables

    // region constructor visibility
    // endregion constructor visibility
    IncrementalMultiJoinStateManager(ColumnSource<?>[] tableKeySources
                                         , int tableSize
        ...
        for (int ii = 0; ii < keyColumnCount; ++ii) {
            ...
            keySources[ii] = (ArrayBackedColumnSource<?>)ArrayBackedColumnSource.getMemoryColumnSource(tableSize, tableKeySources[ii].getType(), tableKeySources[ii].getComponentType());
            keyChunkTypes[ii] = tableKeySources[ii].getChunkType();
            overflowKeySources[ii] = (ArrayBackedColumnSource<?>)ArrayBackedColumnSource.getMemoryColumnSource(CHUNK_SIZE, tableKeySources[ii].getType(), tableKeySources[ii].getComponentType());

            chunkHashers[ii] = ChunkHasher.makeHasher(keyChunkTypes[ii]); //, tableKeySources[ii].getType(), tableKeySources[ii].getComponentType());
            chunkEquals[ii] = ChunkEquals.makeEqual(keyChunkTypes[ii]); //, tableKeySources[ii].getType(), tableKeySources[ii].getComponentType());
            chunkCopiers[ii] = PermuteKernel.makePermuteKernel(keyChunkTypes[ii]); //, tableKeySources[ii].getType(), tableKeySources[ii].getComponentType());
        }
    ...
    private void ensureCapacity(int tableSize) {
        resultSlot.ensureCapacity(tableSize);
    ...
    private void ensureOverflowCapacity(WritableIntChunk<ChunkPositions> chunkPositionsToInsertInOverflow) {
        ...
        overflowResultSlot.ensureCapacity(newCapacity);
    ...
    @Override
    public void build(final Table table, ColumnSource<?>[] sources, int tableNumber) {
        processAdded(table.getRowSet(), sources, tableNumber, null, false);
    }

    public void processAdded(final RowSet rowSet, ColumnSource<?>[] sources, int tableNumber, @Nullable MultiJoinModifiedSlotTracker slotTracker, boolean addedForKeyModified) {
        if (rowSet.isEmpty()) {
            return;
        }
        ...
        final LongArraySource sourceForTable = indexSources.get(tableNumber);
        sourceForTable.ensureCapacity(nextOutputPosition);

        try (final BuildContext bc = makeBuildContext(sources, rowSet.size())) {
            buildTable(bc, rowSet, sources, tableNumber, sourceForTable, slotTracker, addedForKeyModified);
        }
    }
    ...
    class BuildContext implements Context {
        ...
        final WritableLongChunk<OrderedRowKeys> tableLocationsChunk;
        ...
        final WritableLongChunk<OrderedRowKeys> overflowLocationsToFetch;
        ...
        final WritableLongChunk<OrderedRowKeys> insertTableLocations;
        ...
        final WritableLongChunk<OrderedRowKeys> overflowLocationForEqualityCheck;
        ...
        // mixin rehash
        final WritableLongChunk<OrderedRowKeys> rehashLocations;
        final WritableIntChunk<Values> overflowLocationsToMigrate;
        final WritableLongChunk<OrderedRowKeys> overflowLocationsAsKeyIndices;
        final WritableBooleanChunk<Any> shouldMoveBucket;

        final ResettableWritableLongChunk<OrderedRowKeys> overflowLocationForPromotionLoop = ResettableWritableLongChunk.makeResettableChunk();
        ...
        private BuildContext(ColumnSource<?>[] buildSources,
            ...
            stateSourceFillContext = resultSlot.makeFillContext(chunkSize);
            ...
            overflowStateSourceFillContext = overflowResultSlot.makeFillContext(chunkSize);
        ...
        public void close() {
            ...
            SafeCloseableArray.close(workingFillContexts);
            SafeCloseableArray.close(overflowContexts);
            SafeCloseableArray.close(buildContexts);

            hashChunk.close();
            tableLocationsChunk.close();
            SafeCloseableArray.close(writeThroughChunks);
            ...
            SafeCloseableArray.close(workingKeyChunks);
            SafeCloseableArray.close(overflowKeyChunks);
    ...
    private void buildTable(final BuildContext bc,
            ...
            // region extra build arguments
                            ...
                            , final MultiJoinModifiedSlotTracker slotTracker
                            , final boolean forKeyModify
                            // endregion extra build arguments
    ) {
        long hashSlotOffset = 0;
        // region build start
        int maxAddedPosition = -1;
        bc.addedSlotsByPosition.setSize(bc.chunkSize);
        bc.addedSlotsByPosition.fillWithValue(0, bc.addedSlotsByPosition.size(), NULL_ROW_KEY);
        final byte flag = forKeyModify ? MultiJoinModifiedSlotTracker.FLAG_MODIFY : MultiJoinModifiedSlotTracker.FLAG_ADD;
        ...
        try (final RowSequence.Iterator okIt = buildIndex.getRowSequenceIterator();
            ...
            resultSource.ensureCapacity(nextOutputPosition);
            ...
            while (okIt.hasMore()) {
                ...
                outputPositionToHashSlot.ensureCapacity(nextOutputPosition + chunkOk.intSize());
                modifiedTrackerCookieSource.ensureCapacity(nextOutputPosition + chunkOk.intSize());
                bc.addedSlotsByPosition.setSize(chunkOk.intSize());
                bc.addedSlotsByPosition.fillWithValue(0, bc.addedSlotsByPosition.size(), NULL_ROW_KEY);
                maxAddedPosition = -1;
                ...
                resultSlot.fillChunkUnordered(bc.stateSourceFillContext, bc.workingStateEntries, bc.tableLocationsChunk);

                // find things that exist
                // @StateChunkIdentityName@ from \QLongChunk\E
                LongChunkEquals.notEqual(bc.workingStateEntries, EMPTY_STATE_VALUE, bc.filledValues);
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ++ii) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        long outputPosition = bc.workingStateEntries.get(ii);
                        addIndex(tableLocation, tableNumber, resultSource, outputPosition, sourceIndexKeys.get(ii));
                        if (slotTracker != null) {
                            final long cookie = modifiedTrackerCookieSource.getUnsafe(outputPosition);
                            modifiedTrackerCookieSource.set(outputPosition, slotTracker.addSlot(cookie, outputPosition, tableNumber, NULL_ROW_KEY, flag));
                        }
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        throw new IllegalStateException("Duplicate key found for " + keyString(tableLocation) + " in table " + tableNumber + ".");
                    ...
                    while (bc.overflowPositionInSourceChunk.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.equalValues.size(); ++ii) {
                            ...
                            if (bc.equalValues.get(ii)) {
                                ...
                                final long outputPosition = overflowResultSlot.getUnsafe(overflowLocation);
                                final long keyToAdd = sourceIndexKeys.get(chunkPosition);
                                addIndex(overflowLocationToHashLocation((int)overflowLocation), tableNumber, resultSource, outputPosition, keyToAdd);

                                if (slotTracker != null) {
                                    final long cookie = modifiedTrackerCookieSource.getUnsafe(outputPosition);
                                    modifiedTrackerCookieSource.set(outputPosition, slotTracker.addSlot(cookie, outputPosition, tableNumber, NULL_ROW_KEY, MultiJoinModifiedSlotTracker.FLAG_ADD));
                                }
                    ...
                    while (bc.chunkPositionsToInsertInOverflow.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                            ...
                            if (bc.equalValues.get(ii)) {
                                ...
                                throw new IllegalStateException("Duplicate key found for " + keyString(insertedOverflowLocation) + " in table " + tableNumber + ".");
                ...
                outputPositionToHashSlot.ensureCapacity(nextOutputPosition + maxAddedPosition + 1);
                resultSource.ensureCapacity(nextOutputPosition + maxAddedPosition + 1);
                for (int ii = 0; ii <= maxAddedPosition; ++ii) {
                    final long longSlot = bc.addedSlotsByPosition.get(ii);
                    if (longSlot != NULL_ROW_KEY) {
                        final int intSlot = Math.toIntExact(longSlot);
                        if (isOverflowLocation(longSlot)) {
                            overflowResultSlot.set(hashLocationToOverflowLocation(intSlot), nextOutputPosition);
                        } else {
                            resultSlot.set(longSlot, nextOutputPosition);
                        }

                        final long keyToAdd = sourceIndexKeys.get(ii);
                        addIndex((int)longSlot, tableNumber, resultSource, nextOutputPosition, keyToAdd);

                        if (slotTracker != null) {
                            final long cookie = modifiedTrackerCookieSource.getUnsafe(nextOutputPosition);
                            modifiedTrackerCookieSource.set(nextOutputPosition, slotTracker.addSlot(cookie, nextOutputPosition, tableNumber, NULL_ROW_KEY, MultiJoinModifiedSlotTracker.FLAG_ADD));
                        }

                        outputPositionToHashSlot.set(nextOutputPosition, intSlot);
                        nextOutputPosition++;
                    }
                }
    ...
    public void doRehash(BuildContext bc
        ...
        while (rehashRequired()) {
            ...
            resultSlot.fillChunkUnordered(bc.stateSourceFillContext, bc.workingStateEntries, bc.rehashLocations);
            // @StateChunkIdentityName@ from \QLongChunk\E
            LongChunkEquals.notEqual(bc.workingStateEntries, EMPTY_STATE_VALUE, bc.shouldMoveBucket);
            ...
            for (int ii = 0; ii < bc.shouldMoveBucket.size(); ++ii) {
                if (bc.shouldMoveBucket.get(ii)) {
                    ...
                    final long stateValueToMove = resultSlot.getUnsafe(oldHashLocation);
                    resultSlot.set(newHashLocation, stateValueToMove);
                    resultSlot.set(oldHashLocation, EMPTY_STATE_VALUE);
                    // region rehash move values
                    outputPositionToHashSlot.set(stateValueToMove, Math.toIntExact(newHashLocation));
            ...
            for (int loop = 0; loop < 2; loop++) {
                ...
                overflowResultSlot.fillChunkUnordered(bc.overflowStateSourceFillContext, bc.workingStateEntries, bc.overflowLocationsAsKeyIndices);
                ...
                for (int ii = 0; ii < bc.overflowLocationForPromotionLoop.size(); ++ii) {
                    ...
                    if ((firstLoop && tableLocation < tableHashPivot) || (!firstLoop && tableLocation >= tableHashPivot)) {
                        ...
                        final long outputPosition = overflowResultSlot.getUnsafe(overflowLocation);
                        resultSlot.set(tableLocation, outputPosition);
                        outputPositionToHashSlot.set(outputPosition, Math.toIntExact(tableLocation));
    ...
    private void verifyKeyHashes() {
        ...
        try (final WritableLongChunk<OrderedRowKeys> positions = WritableLongChunk.makeWritableChunk(maxSize);
             ...
             final WritableLongChunk<OrderedRowKeys> tableLocationsChunk = WritableLongChunk.makeWritableChunk(maxSize);
             ...
             final ChunkSource.FillContext fillContext = resultSlot.makeFillContext(maxSize)) {

            resultSlot.fillChunk(fillContext, stateChunk, RowSetFactory.flat(tableHashPivot));

            ChunkUtils.fillInOrder(positions);

            // @StateChunkIdentityName@ from \QLongChunk\E
            LongChunkEquals.notEqual(stateChunk, EMPTY_STATE_VALUE, exists);
    ...
    private void createOverflowPartitions(WritableLongChunk<OrderedRowKeys> overflowLocationsToFetch, WritableLongChunk<OrderedRowKeys> rehashLocations, WritableBooleanChunk<Any> shouldMoveBucket, int moves) {
    ...
    private void initializeRehashLocations(WritableLongChunk<OrderedRowKeys> rehashLocations, int bucketsToAdd) {
    ...
    private void compactOverflowLocations(IntChunk<Values> overflowLocations, WritableLongChunk<OrderedRowKeys> overflowLocationsToFetch) {
    ...
    private void swapOverflowPointers(LongChunk<OrderedRowKeys> tableLocationsChunk, LongChunk<OrderedRowKeys> overflowLocationsToFetch) {
    ...
    private void updateWriteThroughState(ResettableWritableLongChunk<Values> writeThroughState, long firstPosition, long expectedLastPosition) {
        final long firstBackingChunkPosition = resultSlot.resetWritableChunkToBackingStore(writeThroughState, firstPosition);
    ...
    private void fillKeys(ColumnSource.FillContext[] fillContexts, WritableChunk<Values>[] keyChunks, WritableLongChunk<OrderedRowKeys> tableLocationsChunk) {
        fillKeys(keySources, fillContexts, keyChunks, tableLocationsChunk);
    }

    private void fillOverflowKeys(ColumnSource.FillContext[] fillContexts, WritableChunk<Values>[] keyChunks, WritableLongChunk<OrderedRowKeys> overflowLocationsChunk) {
        fillKeys(overflowKeySources, fillContexts, keyChunks, overflowLocationsChunk);
    }

    private static void fillKeys(ArrayBackedColumnSource<?>[] keySources, ColumnSource.FillContext[] fillContexts, WritableChunk<Values>[] keyChunks, WritableLongChunk<OrderedRowKeys> RowKeys) {
        for (int ii = 0; ii < keySources.length; ++ii) {
            keySources[ii].fillChunkUnordered(fillContexts[ii], keyChunks[ii], RowKeys);
        }
    }
    ...
    public void processRemoved(final RowSet rowSet, ColumnSource<?>[] sources, int tableNumber, @NotNull MultiJoinModifiedSlotTracker slotTracker, boolean removedForKeyModify) {
        if (rowSet.isEmpty()) {
            return;
        }
        ...
        try (final ProbeContext pc = makeProbeContext(sources, rowSet.size())) {
            decorationProbe(pc, rowSet, sources, true, true, false, removedForKeyModify, 0, tableNumber, sourceForTable, slotTracker);
        }
    }

    public void processModified(final RowSet rowSet, ColumnSource<?>[] sources, int tableNumber, @NotNull MultiJoinModifiedSlotTracker slotTracker) {
        if (rowSet.isEmpty()) {
            return;
        }
        ...
        try (final ProbeContext pc = makeProbeContext(sources, rowSet.size())) {
            decorationProbe(pc, rowSet, sources, false, false, true, false, 0, tableNumber, sourceForTable, slotTracker);
        }
    }

    public void applyShift(final ProbeContext pc, final RowSet rowSet, ColumnSource<?>[] sources, int tableNumber, long shiftDelta, @NotNull MultiJoinModifiedSlotTracker slotTracker) {
        if (rowSet.isEmpty()) {
            return;
        }
        ...
        decorationProbe(pc, rowSet, sources, true, false, false, false, shiftDelta, tableNumber, sourceForTable, slotTracker);
    }
    ...
    class ProbeContext implements Context {
        ...
        private ProbeContext(ColumnSource<?>[] probeSources,
            ...
            RowKeys = WritableLongChunk.makeWritableChunk(chunkSize);
        ...
        public void close() {
            ...
            SafeCloseableArray.close(workingFillContexts);
            SafeCloseableArray.close(overflowContexts);
            SafeCloseableArray.close(probeContexts);
            hashChunk.close();
            tableLocationsChunk.close();
            workingStateEntries.close();
            overflowLocationsToFetch.close();
    ...
    private void decorationProbe(ProbeContext pc
                                ...
                                , boolean usePrev
                                 ...
                                 , final boolean removeForKeyModify
                                 , final long shiftDelta
                                 , final int tableNumber
                                 , final LongArraySource resultSource
                                 , @NotNull final MultiJoinModifiedSlotTracker slotTracker
                                 // endregion additional probe arguments
    )  {
        // region probe start
        final boolean doShift = shiftDelta != 0;
        Assert.assertion(remove ^ modify ^ doShift, "remove ^ modify ^ doShift");
        final byte removeFlag = removeForKeyModify ? MultiJoinModifiedSlotTracker.FLAG_MODIFY : MultiJoinModifiedSlotTracker.FLAG_REMOVE;
        ...
        try (final RowSequence.Iterator okIt = probeIndex.getRowSequenceIterator();
            ...
            while (okIt.hasMore()) {
                ...
                chunkOk.fillRowKeyChunk(pc.RowKeys);
                ...
                resultSlot.fillChunkUnordered(pc.stateSourceFillContext, pc.workingStateEntries, pc.tableLocationsChunk);

                // @StateChunkIdentityName@ from \QLongChunk\E
                LongChunkEquals.notEqual(pc.workingStateEntries, EMPTY_STATE_VALUE, pc.equalValues);
                ...
                for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                    if (pc.equalValues.get(ii)) {
                        ...
                        final long outputPosition = pc.workingStateEntries.get(ii);
                        final long cookie = modifiedTrackerCookieSource.getUnsafe(outputPosition);

                        if (remove) {
                            // we know what hash slot this maps to
                            final long removedKey = removeIndex(resultSource, outputPosition);
                            final long keyToRemove = pc.RowKeys.get(ii);
                            Assert.eq(removedKey, "removedKey", keyToRemove, "keyToRemove");
                            modifiedTrackerCookieSource.set(outputPosition, slotTracker.addSlot(cookie, outputPosition, tableNumber, removedKey, removeFlag));
                        } else if (modify) {
                            // we could fetch our index and check it, but it is not actually necessary for proper function; as we have a single right index and we must match
                            modifiedTrackerCookieSource.set(outputPosition, slotTracker.modifySlot(cookie, outputPosition, tableNumber, MultiJoinModifiedSlotTracker.FLAG_MODIFY));
                        } else if (doShift) {
                            final long shiftedKey = shiftIndex(resultSource, outputPosition, pc.RowKeys.get(ii), shiftDelta);
                            modifiedTrackerCookieSource.set(outputPosition, slotTracker.addSlot(cookie, outputPosition, tableNumber, shiftedKey, MultiJoinModifiedSlotTracker.FLAG_SHIFT));
                        } else {
                            throw new IllegalStateException("BUG: invalid probe type.");
                        }

                        // endregion probe main found
                    } else if (pc.workingStateEntries.get(ii) != EMPTY_STATE_VALUE) {
                        // we must handle this as part of the overflow bucket
                        ...
                        throw new IllegalStateException("BUG: tried to remove something that didn't exist in multi-join!");
                ...
                while (pc.overflowLocationsToFetch.size() > 0) {
                    ...
                    for (int ii = 0; ii < pc.equalValues.size(); ++ii) {
                        ...
                        if (pc.equalValues.get(ii)) {
                            ...
                            if (remove) {
                                final long removedKey = removeIndex(resultSource, outputPosition);
                                final long keyToRemove = pc.RowKeys.get(chunkPosition);
                                Assert.eq(removedKey, "removedKey", keyToRemove, "keyToRemove");
                                modifiedTrackerCookieSource.set(outputPosition, slotTracker.addSlot(cookie, outputPosition, tableNumber, removedKey, MultiJoinModifiedSlotTracker.FLAG_REMOVE));
                            } else if (modify) {
                                modifiedTrackerCookieSource.set(outputPosition, slotTracker.modifySlot(cookie, outputPosition, tableNumber, MultiJoinModifiedSlotTracker.FLAG_REMOVE));
                            } else if (doShift) {
                                final long shiftedKey = shiftIndex(resultSource, outputPosition, pc.RowKeys.get(chunkPosition), shiftDelta);
                                modifiedTrackerCookieSource.set(outputPosition, slotTracker.addSlot(cookie, outputPosition, tableNumber, shiftedKey, MultiJoinModifiedSlotTracker.FLAG_SHIFT));
                            } else {
                                throw new IllegalStateException("BUG: invalid probe type.");
                            }
    ...
    private void convertHashToTableLocations(WritableIntChunk<HashCodes> hashChunk, WritableLongChunk<OrderedRowKeys> tablePositionsChunk) {
        // mixin rehash
    ...
    private void convertHashToTableLocations(WritableIntChunk<HashCodes> hashChunk, WritableLongChunk<OrderedRowKeys> tablePositionsChunk, int pivotPoint) {
        // endmixin rehash
    ...
    public void ensureTableCapacity(int tables) {
        while (indexSources.size() < tables) {
            ...
            newRedirection.ensureCapacity(nextOutputPosition);
    ...
    public ColumnSource[] getKeyHashTableSources() {
        ...
        for (int kci = 0; kci < keyColumnCount; ++kci) {
            // noinspection unchecked
            keyHashTableSources[kci] = RedirectedColumnSource.alwaysRedirect(resultIndexToHashSlot, new HashTableColumnSource(keySources[kci], overflowKeySources[kci]));
        }
        return keyHashTableSources;
    }

    private void addIndex(long slot, long tableNumber, LongArraySource resultSource, long outputPosition, long indexKey) {
        final long oldValue = resultSource.getUnsafe(outputPosition);
        if (oldValue != QueryConstants.NULL_LONG && oldValue != NULL_ROW_KEY) {
            throw new IllegalStateException("Duplicate key found for " + keyString(slot) + " in table " + tableNumber + ".");
        }
        resultSource.set(outputPosition, indexKey);
    }

    private long removeIndex(LongArraySource resultSource, long outputPosition) {
        final long oldValue = resultSource.getUnsafe(outputPosition);
        resultSource.set(outputPosition, NULL_ROW_KEY);
        return oldValue;
    }

    private long shiftIndex(LongArraySource resultSource, long outputPosition, long expected, long shiftDelta) {
        final long oldValue = resultSource.getUnsafe(outputPosition);
        Assert.eq(oldValue, "oldValue", expected, "expected");
        resultSource.set(outputPosition, expected + shiftDelta);
        return oldValue;
    }
    ...
    public String keyString(long slot) {
        ...
        try (final SafeCloseable ignored = new SafeCloseableArray<>(keyChunk);
             final WritableLongChunk<OrderedRowKeys> slotChunk = WritableLongChunk.makeWritableChunk(1)) {
            if (isOverflowLocation(slot)) {
                slotChunk.set(0, hashLocationToOverflowLocation(Math.toIntExact(slot)));
