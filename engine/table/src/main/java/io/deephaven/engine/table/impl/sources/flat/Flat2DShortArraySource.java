 ...
 */
public class Flat2DShortArraySource extends AbstractColumnSource<Short> implements ImmutableColumnSourceGetDefaults.ForShort, WritableColumnSource<Short>, FillUnordered, InMemoryColumnSource, ChunkedBackingStoreExposedWritableSource {
    ...
    // region constructor
    public Flat2DShortArraySource(long size) {
        super(short.class);
        this.size = size;
    ...
    private static short [][] allocateArray(long size) {
        ...
        while (size > SEGMENT_SIZE) {
            data[segment++] = new short[SEGMENT_SIZE];
            size -= SEGMENT_SIZE;
        }
        data[segment] = new short[Math.toIntExact(size)];
        return data;
    }
    ...
    private void fillChunkByRanges(WritableChunk<? super Values> destination, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                asShortChunk.copyFromTypedArray(data[segment], Math.toIntExact(start), srcPos.getAndAdd(rangeLength), rangeLength);
                start += rangeLength;
            }
        });
    }
    ...
    public Chunk<? extends Values> getChunk(@NotNull GetContext context, long firstKey, long lastKey) {
        ...
        return contextWithResettable.resettableShortChunk.resetFromTypedArray(data[segment], Math.toIntExact(firstKey), len);
    }
    ...
    private void fillFromChunkByRanges(Chunk<? extends Values> src, RowSequence rowSequence) {
        ...
        rowSequence.forAllRowKeyRanges((long start, long end) -> {
            while (start < end) {
                ...
                final long realEnd = Math.min(segmentEnd, end);
                final int rangeLength = Math.toIntExact(realEnd - start + 1);
                asShortChunk.copyToTypedArray(srcPos.getAndAdd(rangeLength), data[segment], Math.toIntExact(start), rangeLength);
                start += rangeLength;
            }
        });
    }
