...
class IncrementalUpdateByStateManager
        // region extensions
        implements ChunkedUpdateByStateManager
    // endregion extensions
{
    ...
    IncrementalUpdateByStateManager(ColumnSource<?>[] tableKeySources
                                         , int tableSize
    ...
    public void add(final SafeCloseable bc,
        ...
        buildTable((BuildContext)bc, orderedKeys, sources, nextOutputPosition, outputPositions);
    }

    @Override
    public SafeCloseable makeUpdateByBuildContext(ColumnSource<?>[] buildSources, long maxSize) {
    ...
    public SafeCloseable makeUpdateByProbeContext(ColumnSource<?>[] buildSources, long maxSize) {
        return makeProbeContext(buildSources, maxSize);
    }
        ...
        try (final RowSequence.Iterator rsIt = buildIndex.getRowSequenceIterator();
            ...
            while (rsIt.hasMore()) {
                ...
                for (int ii = 0; ii < bc.insertPositionsInSourceChunk.size(); ) {
                    ...
                    // region main insert
                    final int outputPosForLocation = outputPosition.getAndIncrement();
                    stateSource.set(currentHashLocation, outputPosForLocation);
                    outputPositions.set(firstChunkPositionForHashLocation, outputPosForLocation);
                ...
                for (int ii = 0; ii < bc.equalValues.size(); ii++) {
                    ...
                    if (bc.equalValues.get(ii)) {
                        ...
                        final int outputPositionToCopy = outputPositions.get(bc.chunkPositionsToCheckForEquality.get(ii * 2));
                        outputPositions.set(chunkPosition, outputPositionToCopy);
                ...
                if (bc.overflowPositionInSourceChunk.size() > 0) {
                    ...
                    while (bc.chunkPositionsToInsertInOverflow.size() > 0) {
                        ...
                        for (int ii = 0; ii < bc.chunkPositionsToInsertInOverflow.size(); ) {
                            ...
                            final int outputPosForLocation = outputPosition.getAndIncrement();
                            overflowStateSource.set(allocatedOverflowLocation, outputPosForLocation);
    ...
    public void remove(@NotNull final SafeCloseable pc,
        ...
        decorationProbe((ProbeContext) pc, indexToRemove, sources, true, outputPositions);
    }
    ...
    public void findModifications(@NotNull final SafeCloseable pc,
        ...
        decorationProbe((ProbeContext) pc, modifiedIndex, leftSources, false, outputPositions);
    }
