...
public final class CharSegmentedSortedMultiset implements SegmentedSortedMultiSet<Character>, CharVector {
     ...
     * {@link #insert(WritableCharChunk, WritableIntChunk, int, int)} for one value without requiring the caller to wrap
    ...
    private boolean insertInterior(char value, long count) {
        ...
        final char[] leafValue = leafValues[leaf];
     ...
     */
    private void splitLeafForInsert(int leaf, int ip, char value, long count) {
        makeLeafHole(leaf + 1, 1);
        leafCount++;
        final char[] lowerValues = leafValues[leaf];
        final long[] lowerCounts = leafCounts[leaf];
        final char[] upperValues = leafValues[leaf + 1] = new char[leafSize];
    ...
    private void distributeNewIntoLeaves(CharChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts,
        ...
        while (remaining-- > 0) {
            ...
            final boolean useInsertValue = CharComparisons.gt(insertValue, leafValue);
    ...
    private void insertInternal(WritableCharChunk<? extends Values> valuesToInsert,
    ...
    private static int lowerBound(char[] valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            ...
            final boolean moveLo = CharComparisons.leq(testValue, searchValue);
            if (moveLo) {
                lo = mid;
                if (lo == hi - 1) {
                    break;
    ...
    private static int upperBound(char[] valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch[mid];
            final boolean moveHi = CharComparisons.geq(testValue, searchValue);
            if (moveHi) {
                hi = mid;
            } else {
                lo = mid + 1;
    ...
    private static int lowerBoundExclusive(char[] valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final char testValue = valuesToSearch[mid];
            final boolean moveLo = CharComparisons.lt(testValue, searchValue);
            if (moveLo) {
                lo = mid + 1;
                if (lo == hi) {
                    break;
     ...
     * {@link #remove(RemoveContext, WritableCharChunk, WritableIntChunk, int, int)} for one value without requiring the
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    Assert.assertion(CharComparisons.leq(lastValue, directoryValue), "lt(lastValue, directoryValue)",
                            lastValue,
                            "leafValues[ii][leafSizes[ii] - 1]", directoryValue, "directoryValue");

                    if (ii < leafCount - 2) {
                        final char nextDirectoryValue = directoryValues[ii + 1];
                        Assert.assertion(CharComparisons.lt(directoryValue, nextDirectoryValue),
                    ...
                    Assert.assertion(CharComparisons.lt(directoryValue, nextFirstValue),
    ...
    private void validateLeafOrdering() {
        for (int leaf = 0; leaf < leafCount - 1; ++leaf) {
            ...
            Assert.assertion(CharComparisons.lt(lastValue, nextValue), lastValue + " < " + nextValue);
        }
    }
    ...
    private static void validateLeaf(char[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(CharComparisons.lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])",
                    (Character) thisValue,
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(CharComparisons.geq(getMinChar(), destination.getMaxChar()),
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(CharComparisons.leq(getMaxChar(), destination.getMinChar()),
     ...
     * (exclusive). Following the {@link CharVector} contract, offsets outside {@code [0, size())} are legal and
     ...
     */
    private char[] keyArray(final long fromIndexInclusive, final long toIndexExclusive) {
        ...
        if (remaining == 0) {
            ...
            Arrays.fill(keyArray, NULL_CHAR);
            return keyArray;
        }
        ...
        if (destOffset > 0) {
            Arrays.fill(keyArray, 0, destOffset, NULL_CHAR);
        }
        if (destOffset + remaining < totalSize) {
            Arrays.fill(keyArray, destOffset + remaining, totalSize, NULL_CHAR);
        }
    ...
    private void maybeAccumulateAddition(char valueAdded) {
        if (!accumulateDeltas) {
            return;
        }
    ...
    public char get(long index) {
        // offsets outside [0, size()) are legal and read as null, per the CharVector contract
    ...
    public ValueIteratorOfChar iterator(final long fromIndexInclusive, final long toIndexExclusive) {
        ...
        return ValueIteratorOfChar.wrapWithNulls(
     ...
     */
    private ValueIteratorOfChar inRangeIterator(final long fromIndexInclusive, final long toIndexExclusive) {
        if (leafCount <= 1) {
    ...
    public CharVector subVector(long fromIndexInclusive, long toIndexExclusive) {
        // materialized rather than a CharVectorSlice view: an SSM is live and mutable, and a slice would capture our
    ...
    private boolean equalsArray(CharVector o) {
        ...
        try (final CloseablePrimitiveIteratorOfChar oit = o.iterator()) {
            if (size == 1) {
                return CharComparisons.eq(get(0), oit.nextChar());
            }

            if (leafCount == 1) {
                for (int ii = 0; ii < size; ii++) {
                    if (!CharComparisons.eq(directoryValues[ii], oit.nextChar())) {
                        return false;
                    }
                }
            ...
            for (int li = 0; li < leafCount; ++li) {
                for (int ai = 0; ai < leafSizes[li]; ai++) {
                    if (!CharComparisons.eq(leafValues[li][ai], oit.nextChar())) {
                        return false;
                    }
                }
            }
     ...
     * {@link CharVector#hashCode(CharVector)} would: the same seed, the same multiplier, and the same per-element
     * {@link CharComparisons#hashCode(char)}. That per-element hash is also why {@link #equals(Object)} must compare
     * elements with {@link CharComparisons#eq(char, char)} rather than {@code ==}.
     *
     * <p>
     ...
     * {@code TestCharSegmentedSortedMultiset#testHashCodeMatchesVectorHelper} pins the two against each other across
    ...
    public int hashCode() {
        ...
        if (leafCount == 1) {
            if (directoryValues == null) {
                return 31 * result + CharComparisons.hashCode(singletonValue);
            }

            for (int ii = 0; ii < size; ++ii) {
                result = 31 * result + CharComparisons.hashCode(directoryValues[ii]);
            }
        ...
        for (int li = 0; li < leafCount; ++li) {
            final char[] values = leafValues[li];
            final int leafSz = leafSizes[li];
            for (int ai = 0; ai < leafSz; ++ai) {
                result = 31 * result + CharComparisons.hashCode(values[ai]);
            }
        }
