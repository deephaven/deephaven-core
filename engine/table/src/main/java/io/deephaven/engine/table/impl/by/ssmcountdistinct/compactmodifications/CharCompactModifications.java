 ...
 */
public class CharCompactModifications {
    /**
    ...
    public static void compactAndCountModifications(
        ...
        while (rRead < removedEnd && aRead < addedEnd) {
            ...
            if (CharComparisons.eq(removedValue, addedValue)) {
    ...
    private static int countRun(WritableCharChunk<? extends Values> values, int pos, int end) {
        final char value = values.get(pos);
        int run = 1;
        while (pos + run < end && CharComparisons.eq(values.get(pos + run), value)) {
            run++;
        }
        return run;
    }

    private static boolean ignore(char value, boolean countNull, boolean countNaN) {
        if (!countNull && value == NULL_CHAR) {
            return true;
        }
