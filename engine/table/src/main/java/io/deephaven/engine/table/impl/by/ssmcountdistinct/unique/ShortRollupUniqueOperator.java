...
public class ShortRollupUniqueOperator implements IterativeChunkedAggregationOperator {
     ...
     */
    private void addValueToMultiset(long destination, short value, MutableLong count,
            MutableObject<ShortSegmentedSortedMultiset> ssmHolder) {
        ...
        if (ShortComparisons.eq(value, held)) {
            count.increment();
            return;
        }
     ...
     */
    private void removeValueFromMultiset(long destination, short value, MutableLong count,
            MutableObject<ShortSegmentedSortedMultiset> ssmHolder) {
        final ShortSegmentedSortedMultiset ssm = ssmHolder.getValue();
        if (ssm != null) {
            ssm.remove(value, 1);
