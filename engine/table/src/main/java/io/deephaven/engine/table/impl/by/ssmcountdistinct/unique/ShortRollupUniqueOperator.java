...
public class ShortRollupUniqueOperator implements IterativeChunkedAggregationOperator {
    ...
    private final ShortArraySource singletonValue;
    ...
    public ShortRollupUniqueOperator(
        ...
        this.singletonValue = new ShortArraySource();
    ...
    public void addChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final ShortChunk<? extends Values> valueChunk = values.asShortChunk();
        final WritableShortChunk<? extends Values> addValues = context.values.asWritableShortChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<ShortSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
    ...
    public void removeChunk(BucketedContext bucketedContext, Chunk<? extends Values> values,
        ...
        final SsmUniqueRollupContext context = (SsmUniqueRollupContext) bucketedContext;
        final ShortChunk<? extends Values> valueChunk = values.asShortChunk();
        final WritableShortChunk<? extends Values> removeValues = context.values.asWritableShortChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<ShortSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
    ...
    public void modifyChunk(BucketedContext bucketedContext, Chunk<? extends Values> preValues,
            ...
            WritableBooleanChunk<Values> stateModified) {
        final SsmUniqueRollupContext context = (SsmUniqueRollupContext) bucketedContext;
        final ShortChunk<? extends Values> prevValueChunk = preValues.asShortChunk();
        final ShortChunk<? extends Values> postValueChunk = postValues.asShortChunk();
        final WritableShortChunk<? extends Values> removeValues = context.values.asWritableShortChunk();
        final WritableShortChunk<? extends Values> addValues = context.postValues.asWritableShortChunk();
        final MutableLong count = new MutableLong();
        final MutableObject<ShortSegmentedSortedMultiset> ssmHolder = new MutableObject<>();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
            ...
            ShortCompactModifications.compactAndCountModifications(removeValues, context.counts, addValues,
    ...
    private void loadState(long destination, MutableLong count,
            MutableObject<ShortSegmentedSortedMultiset> ssmHolder) {
        ...
        if (isNonUniqueState(sc)) {
            ...
            final ShortSegmentedSortedMultiset ssm = ssms.getCurrentSsm(destination);
     ...
     * {@link ShortCompactModifications} instead and calls {@link #applyAdds} directly.
     */
    private void compactAndApplyAdds(long destination, WritableShortChunk<? extends Values> values, int addCount,
        ...
        ShortCompactKernel.compactAndCount(values, counts, true, true);
     ...
     * afterward.
     */
    private void applyAdds(long destination, WritableShortChunk<? extends Values> values, int distinctCount,
            WritableIntChunk<ChunkLengths> counts, MutableLong count,
            MutableObject<ShortSegmentedSortedMultiset> ssmHolder) {
        if (distinctCount == 0) {
            return;
        }
        final ShortSegmentedSortedMultiset ssm = ssmHolder.getValue();
        if (ssm != null) {
        ...
        if (count.get() == 0) {
            ...
            final ShortSegmentedSortedMultiset newSsm = ssmForSlot(destination);
     ...
     * with {@link ShortCompactModifications} instead and calls {@link #applyRemoves} directly.
     */
    private void compactAndApplyRemoves(long destination, WritableShortChunk<? extends Values> values, int removeCount,
            WritableIntChunk<ChunkLengths> counts, SegmentedSortedMultiSet.RemoveContext removeContext,
            MutableLong count, MutableObject<ShortSegmentedSortedMultiset> ssmHolder) {
        if (removeCount == 0) {
            return;
        }
     ...
     */
    private void applyRemoves(long destination, WritableShortChunk<? extends Values> values, int distinctCount,
            WritableIntChunk<ChunkLengths> counts, SegmentedSortedMultiSet.RemoveContext removeContext,
            MutableLong count, MutableObject<ShortSegmentedSortedMultiset> ssmHolder) {
        if (distinctCount == 0) {
            return;
     ...
     */
    private boolean finalizeState(long destination, long priorState, short priorValue, long count,
            ShortSegmentedSortedMultiset ssm) {
        ...
        if (!ShortComparisons.eq(newValue, priorValue)) {
    ...
    public BucketedContext makeBucketedContext(int size) {
        return new SsmUniqueRollupContext(ChunkType.Short, size);
    }
