...
public class ShortRollupUniqueOperator implements IterativeChunkedAggregationOperator {
     ...
     */
    private void addValueToMultiset(long destination, short value) {
     ...
     */
    private void removeValueFromMultiset(long destination, short value) {
        final ShortSegmentedSortedMultiset ssm = ssms.getCurrentSsm(destination);
        if (ssm != null) {
            ssm.remove(value, 1);
            if (ssm.isEmpty()) {
     ...
     */
    private boolean exposedChanged(long destination, long priorState, short priorValue) {
