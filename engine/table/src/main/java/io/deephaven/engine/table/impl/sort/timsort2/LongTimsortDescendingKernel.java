 ...
 */
public final class LongTimsortDescendingKernel {
    private LongTimsortDescendingKernel() {
        throw new UnsupportedOperationException();
    }

    public static <ATTR extends Any> LongSortKernelContext<ATTR> createContext(final int size) {
     ...
     */
    public static <ATTR extends Any> void sort(LongSortKernelContext<ATTR> context,
            final WritableLongChunk<ATTR> valuesToSort) {
        timSort(context, valuesToSort, 0, valuesToSort.size());
    }

    private static <ATTR extends Any> void timSort(LongSortKernelContext<ATTR> context,
            WritableLongChunk<ATTR> valuesToSort, int offset, int length) {
        if (length <= 1) {
            return;
        }
    ...
    private static <ATTR extends Any> void ensureMergeInvariants(
            LongSortKernelContext<ATTR> context, WritableLongChunk<ATTR> valuesToSort) {
        while (context.runCount > 1) {
    ...
    private static <ATTR extends Any> void merge(LongSortKernelContext<ATTR> context,
            WritableLongChunk<ATTR> valuesToSort, int start1, int length1, int length2) {
     ...
     */
    private static <ATTR extends Any> void frontMerge(LongSortKernelContext<ATTR> context,
            WritableLongChunk<ATTR> valuesToSort, final int mergeStartPosition, final int start2,
            final int length2) {
        int tempCursor = 0;
     ...
     * We eventually need to do galloping here, but are skipping that for now
     */
    private static <ATTR extends Any> void backMerge(LongSortKernelContext<ATTR> context,
            WritableLongChunk<ATTR> valuesToSort, final int mergeStartPosition, final int length1) {
    ...
    private static <ATTR extends Any> void copyToTemporary(LongSortKernelContext<ATTR> context,
            WritableLongChunk<ATTR> valuesToSort, int mergeStartPosition, int remaining1) {
    ...
    private static <ATTR extends Any> void copyToChunk(LongChunk<ATTR> valuesSource,
            WritableLongChunk<ATTR> valuesDest, int sourceStart, int destStart, int length) {
    ...
    private static void insertionSort(WritableLongChunk<?> valuesToSort, int offset, int length) {
    ...
    private static void swap(WritableLongChunk<?> valuesToSort, int a, int b) {
