...
public final class FloatSegmentedSortedMultiset implements SegmentedSortedMultiSet<Float>, FloatVector {
    ...
    private void insert(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts) {
        ...
        if (isSingleton()) {
            if (valuesToInsert.size() == 1 && FloatComparisons.eq(valuesToInsert.get(0), singletonValue)) {
        ...
        if (leafCount > 1 && FloatComparisons.gt(valuesToInsert.get(0), getMaxFloat())) {
    ...
    private void remove(RemoveContext removeContext, FloatChunk<? extends Values> valuesToRemove,
        ...
        if (isSingleton()) {
            ...
            Assert.assertion(FloatComparisons.eq(valuesToRemove.get(0), singletonValue),
                    "FloatComparisons.eq(valuesToRemove.get(0), singletonValue)");
            singletonCount -= counts.get(0);
    ...
    private boolean equalsArray(ObjectVector<?> o) {
        ...
        if (size == 1) {
            final Float val = (Float) o.get(0);
            // region VectorEquals
            if (get(0) == NULL_FLOAT && val != null && val != NULL_FLOAT) {
                return false;
            }
            // endregion VectorEquals
