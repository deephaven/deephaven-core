 ...
 */
public class ByteToIntegerCastNullAware<T extends Any> implements ToIntFunctor<T> {
    private final WritableIntChunk<T> result;

    ByteToIntegerCastNullAware(int size) {
    ...
    public static <T2 extends Any> void castInto(ByteChunk<? extends T2> input, WritableIntChunk<T2> result) {
        for (int ii = 0; ii < input.size(); ++ii) {
            final byte c = input.get(ii);
            result.set(ii, c == QueryConstants.NULL_BYTE ? QueryConstants.NULL_INT : (int) c);
        }
