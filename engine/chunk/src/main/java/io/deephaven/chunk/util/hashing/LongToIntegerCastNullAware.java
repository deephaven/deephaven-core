 ...
 */
public class LongToIntegerCastNullAware<T extends Any> implements ToIntFunctor<T> {
    private final WritableIntChunk<T> result;

    LongToIntegerCastNullAware(int size) {
    ...
    public static <T2 extends Any> void castInto(LongChunk<? extends T2> input, WritableIntChunk<T2> result) {
        for (int ii = 0; ii < input.size(); ++ii) {
            final long c = input.get(ii);
            result.set(ii, c == QueryConstants.NULL_LONG ? QueryConstants.NULL_INT : (int) c);
        }
