...
public class TestByteChunk {
    ...
    public void chunkWrapBounds() {
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableByteChunk.writableChunkWrap(data0, 0, data0.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableByteChunk.writableChunkWrap(data0, data0.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableByteChunk.writableChunkWrap(data0, data0.length, 1));
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableByteChunkChunk.writableChunkWrap(wccData, 0, ccData.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableByteChunkChunk.writableChunkWrap(wccData, ccData.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableByteChunkChunk.writableChunkWrap(wccData, ccData.length, 1));
    ...
    public void copyFromChunk() {
        ...
        final WritableByteChunk<Values> destChunk =
                WritableByteChunk.writableChunkWrap(destData, destChunkOffset, destChunkCapacity);
    ...
    public void testWritableChunkChunk() {
        ...
        WritableByteChunk<Values> replacementChunk =
                WritableByteChunk.writableChunkWrap(replacementData, MIDCHOFF, MIDCHOFF);
    ...
    public void testArray() {
        final byte[] underlyingData = new byte[100];
        for (int ii = 0; ii < underlyingData.length; ++ii) {
            underlyingData[ii] = (byte) ii;
        }
        final byte[] array;
        final int offset;
        // noinspection rawtypes
        try (WritableByteChunk chunk = WritableByteChunk.writableChunkWrap(underlyingData, 10, 20)) {
            array = chunk.array();
            ...
            for (int ii = offset; ii < lastOffset; ++ii) {
                TestCase.assertEquals((byte) ii, array[ii]);
            }

            array[offset] = 42;
            TestCase.assertEquals((byte) 42, chunk.get(0));
            chunk.set(1, (byte) 97);
            TestCase.assertEquals((byte) 97, array[offset + 1]);
        }
    }
    // endregion testArray

    private static <ATTR extends Values> void verifyChunkEqualsArray(ByteChunk<ATTR> chunk, byte[] data, int offset,
            int size) {
        for (int ii = 0; ii < size; ++ii) {
