...
public class TestDoubleChunk {
    ...
    public void chunkWrapBounds() {
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableDoubleChunk.writableChunkWrap(data0, 0, data0.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableDoubleChunk.writableChunkWrap(data0, data0.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableDoubleChunk.writableChunkWrap(data0, data0.length, 1));
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableDoubleChunkChunk.writableChunkWrap(wccData, 0, ccData.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableDoubleChunkChunk.writableChunkWrap(wccData, ccData.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableDoubleChunkChunk.writableChunkWrap(wccData, ccData.length, 1));
    ...
    public void copyFromChunk() {
        ...
        final WritableDoubleChunk<Values> destChunk =
                WritableDoubleChunk.writableChunkWrap(destData, destChunkOffset, destChunkCapacity);
    ...
    public void testWritableChunkChunk() {
        ...
        WritableDoubleChunk<Values> replacementChunk =
                WritableDoubleChunk.writableChunkWrap(replacementData, MIDCHOFF, MIDCHOFF);
    ...
    public void testArray() {
        final double[] underlyingData = new double[100];
        for (int ii = 0; ii < underlyingData.length; ++ii) {
            underlyingData[ii] = (double) ii;
        }
        final double[] array;
        final int offset;
        // noinspection rawtypes
        try (WritableDoubleChunk chunk = WritableDoubleChunk.writableChunkWrap(underlyingData, 10, 20)) {
            array = chunk.array();
            ...
            for (int ii = offset; ii < lastOffset; ++ii) {
                TestCase.assertEquals((double) ii, array[ii]);
            }

            array[offset] = 42;
            TestCase.assertEquals((double) 42, chunk.get(0));
            chunk.set(1, (double) 97);
            TestCase.assertEquals((double) 97, array[offset + 1]);
        }
    }
    // endregion testArray

    private static <ATTR extends Values> void verifyChunkEqualsArray(DoubleChunk<ATTR> chunk, double[] data, int offset,
            int size) {
        for (int ii = 0; ii < size; ++ii) {
