...
public class TestIntChunk {
    ...
    public void chunkWrapBounds() {
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableIntChunk.writableChunkWrap(data0, 0, data0.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableIntChunk.writableChunkWrap(data0, data0.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableIntChunk.writableChunkWrap(data0, data0.length, 1));
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableIntChunkChunk.writableChunkWrap(wccData, 0, ccData.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableIntChunkChunk.writableChunkWrap(wccData, ccData.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableIntChunkChunk.writableChunkWrap(wccData, ccData.length, 1));
    ...
    public void copyFromChunk() {
        ...
        final WritableIntChunk<Values> destChunk =
                WritableIntChunk.writableChunkWrap(destData, destChunkOffset, destChunkCapacity);
    ...
    public void testWritableChunkChunk() {
        ...
        WritableIntChunk<Values> replacementChunk =
                WritableIntChunk.writableChunkWrap(replacementData, MIDCHOFF, MIDCHOFF);
    ...
    public void testArray() {
        ...
        try (WritableIntChunk chunk = WritableIntChunk.writableChunkWrap(underlyingData, 10, 20)) {
            ...
            for (int ii = offset; ii < lastOffset; ++ii) {
                assertEquals((int) ii, array[ii]);
            }

            array[offset] = 42;
            assertEquals((int) 42, chunk.get(0));
            chunk.set(1, (int) 97);
            assertEquals((int) 97, array[offset + 1]);
        }
    }
    // endregion testArray

    private static <ATTR extends Values> void verifyChunkEqualsArray(IntChunk<ATTR> chunk, int[] data, int offset,
            int size) {
        for (int ii = 0; ii < size; ++ii) {
