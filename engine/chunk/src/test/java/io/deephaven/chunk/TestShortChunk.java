...
public class TestShortChunk {
    ...
    public void chunkWrapBounds() {
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableShortChunk.writableChunkWrap(data0, 0, data0.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableShortChunk.writableChunkWrap(data0, data0.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableShortChunk.writableChunkWrap(data0, data0.length, 1));
        ...
        expectException(IllegalArgumentException.class,
                () -> WritableShortChunkChunk.writableChunkWrap(wccData, 0, ccData.length + 1));
        expectException(IllegalArgumentException.class,
                () -> WritableShortChunkChunk.writableChunkWrap(wccData, ccData.length - 1, 2));
        expectException(IllegalArgumentException.class,
                () -> WritableShortChunkChunk.writableChunkWrap(wccData, ccData.length, 1));
    ...
    public void copyFromChunk() {
        ...
        final WritableShortChunk<Values> destChunk =
                WritableShortChunk.writableChunkWrap(destData, destChunkOffset, destChunkCapacity);
    ...
    public void testWritableChunkChunk() {
        ...
        WritableShortChunk<Values> replacementChunk =
                WritableShortChunk.writableChunkWrap(replacementData, MIDCHOFF, MIDCHOFF);
    ...
    public void testArray() {
        ...
        try (WritableShortChunk chunk = WritableShortChunk.writableChunkWrap(underlyingData, 10, 20)) {
            ...
            for (int ii = offset; ii < lastOffset; ++ii) {
                assertEquals((short) ii, array[ii]);
            }

            array[offset] = 42;
            assertEquals((short) 42, chunk.get(0));
            chunk.set(1, (short) 97);
            assertEquals((short) 97, array[offset + 1]);
        }
    }
    // endregion testArray

    private static <ATTR extends Values> void verifyChunkEqualsArray(ShortChunk<ATTR> chunk, short[] data, int offset,
            int size) {
        for (int ii = 0; ii < size; ++ii) {
