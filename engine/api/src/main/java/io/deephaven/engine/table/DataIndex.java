...
public interface DataIndex {
    ...
    /**
     * Build a lookup function of the index values based on the current configuration of this builder.
     *
    ...
    /**
     * Build a lookup function of positions on the current configuration of this builder.
     *
     * @return a function that provides map-like lookup of positional from a given index key.
     */
    @NotNull
     ...
     *
     * @param intersectRowSet the {@link RowSet} to {@link RowSet#intersect(RowSet) intersect} with each output row set. Provide null to skip intersection.
     * @param invertRowSet convert the output rows to positions in the provided {@link RowSet}. Provide null to skip inversion.
     * @param sortByFirstRowKey sort by the first key within each row set. This can be useful to maintain visitation order.
    ...
    default DataIndex applyIntersect(@NotNull final RowSet intersectRowSet) {
        return apply(intersectRowSet, null, false, null);
    }
    ...
    default DataIndex applyInvert(@NotNull final RowSet invertRowSet) {
        return apply(null, invertRowSet, false, null);
    }
    ...
    default DataIndex applySortByFirstRowKey() {
        return apply(null, null, true, null);
    }
    ...
    default DataIndex applyColumnRemap(final Map<ColumnSource<?>, ColumnSource<?>> keyColumnRemap) {
        return apply(null, null, false, keyColumnRemap);
    }
