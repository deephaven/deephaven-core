...
public interface GroupingBuilder {
    /**
     * Add a method that will process tables per region. If the mutator returns a null or empty table, it will not be
     * passed to the following mutators.
     *
    ...
    /**
     * Clamp the returned table of indices to the specified range. In strict mode, guarantees that the indices returned
     * for each group include ony indices in the specified index.
     *
    ...
    /**
     * Indicate that only groups which intersect the specified index are returned in the final table. Note that this may
     * include indices in specified groups that are outside the index, but allows the implementation to optimize out
     * entire regions which may not be contained in the specified index.
     *
     * <p>
     * Users may use {@link #clampToIndex(RowSet, boolean)} for more control over this behavior.
     *
    ...
    /**
     * Sort the final grouping table by the first key within each group. This can be useful to maintain visitation
     * order.
     * 
     * @return this {@link GroupingBuilder}
     */
    ...
    /**
     * Estimate the total size of the resultant grouping table based on the current builder parameters. This method is
     * guaranteed to return a worse-case size, that is, the final grouping may be smaller than reported, but will never
     * be larger.
     *
