...
public class MTTestSupport {
    ...
    public static <T> void assertBecomesEquals(int timeout, T expectedVal, Function.Nullary<T> f) {
        ...
        for (;;) {
            ...
            if (now < deadline) {
                if (now - start >= 1000) {
                    start = now;
                    System.out.printf("assertBecomesEquals(%d millis, %s expected, %s actual) still waiting after %d millis%n",
        ...
        Assert.fail("value did not become equal to "+expectedVal+" within "+timeout+" millis, most recent value was "+testVal);
    }
    ...
    public static <T> void assertRemainsEquals(int timeout, T val, Function.Nullary<T> f) {
        ...
        do {
            ...
            if ( !val.equals(sample) ) {
                Assert.fail("value did not remain equal to "+val+" for "+timeout+" millis, most recent value was "+sample);
            }
            Thread.yield();
    ...
    public static void assertThreadTerminates(int timeout, Thread t) {
        assertThreadTerminates(timeout, t, new Procedure.Nullary() { public void call() { } });
    }
