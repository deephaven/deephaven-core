...
//--------------------------------------------------------------------
/**
 * Temporarily intercepts logging for a given {@link Logger} and
 * saves all logged
 * messages to memory so that a unit test can verify the correct
 * messages were logged.
 * <P> Usage:
 * <LI> To start capturing (in {@link TestCase#setUp}):
 * <Pre>m_loggingRecorder=new LoggingRecorder(Logger.getLogger(Target.class));</Pre>
 * <LI> To stop capturing (in {@link TestCase#tearDown}):
 * <Pre>m_loggingRecorder.detach();<BR>m_loggingRecorder=null;</Pre>
 * <LI> To check results:
 * <Pre>m_loggingRecorder.assertNoMessagesLogged();</Pre>
 * &nbsp; &nbsp; &nbsp; or
 * <Pre>m_loggingRecorder.assertMessagesLogged(<BR>    new CheckedMessage("Hello, World!", Level.INFO)<BR>);</Pre>
 */
public class LoggingRecorder {
    ...
    public void assertMessagesLogged(CheckedMessage... checkedMessages) {
        ...
        for (int nIndex=0; nIndex<checkedMessages.length; nIndex++) {
            ...
            if (null!=checkedMessage.getDetailFragment()) {
                SimpleTestSupport.assertStringContains(message.getThrowableInformation().getThrowable().toString(), checkedMessage.getDetailFragment());
            }
    ...
    private static void dumpLogMessages(List<LoggingEvent> messages) {
        for (LoggingEvent message : messages) {
            System.err.println("Possibly unexpected log message: ["+message.getLevel()+"] "+message.getRenderedMessage()
                    +(null!=message.getThrowableInformation()?" ("+message.getThrowableInformation().getThrowable().toString()+")":""));
        }
    }
}
