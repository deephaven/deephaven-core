...
public class TestConcurrentUnboundedStringCache extends TestCase {
    ...
    public void testStringPopulation() {
        final StringCache<String> debugCache = new ConcurrentUnboundedStringCache<>(StringCacheTypeAdapterStringImpl.INSTANCE, 10, true);
        final StringCache<String> regularCache = new ConcurrentUnboundedStringCache<>(StringCacheTypeAdapterStringImpl.INSTANCE, 10, false);
    ...
    public void testStringPopulationCompressed() {
        for (StringCacheTypeAdapter<? extends CompressedString> typeAdapter : COMPRESSED_TYPE_ADAPTERS) {
            final StringCache<? extends CompressedString> debugCache = new ConcurrentUnboundedStringCache<>(typeAdapter, 10, true);
            final StringCache<? extends CompressedString> regularCache = new ConcurrentUnboundedStringCache<>(typeAdapter, 10, false);
            //noinspection unchecked
