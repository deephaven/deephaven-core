...
//--------------------------------------------------------------------
/**
 * A very simple {@link Set} for small sets (uses linear time
 * algorithms) that creates no garbage (except when expanding).
 * This set only has one {@link Iterator}, which is reused. This set
 * is not thread safe.
 * <P> Note: This class extends {@link HashSet} rather than
 * {@link Set} (or {@link AbstractSet}) only because one of the fields
 * where we want to use it ({@link sun.nio.ch.SelectorImpl#keys}) is
...
public class LowGarbageArraySet<T> extends HashSet<T> {
    ...
    //----------------------------------------------------------------
    @Override public LowGarbageArraySet<T> clone() {
