...
public class RequirementFailure extends RuntimeException {
    ...
    /** The number of stack frames that should be removed from the
     * stack to find the method whose requirements did not hold. */
    ...
    //----------------------------------------------------------------
    /** Gets the number of stack frames that should be removed from
     * the stack to find the caller which failed to meet
     * requirements. */
    ...
    //----------------------------------------------------------------
    /** Gets a stack trace with a line added identifying the
    ...
    //----------------------------------------------------------------
    /** Gets the unmodified stack trace, instead of the one with the
     * culprit identified. */
    ...
    //----------------------------------------------------------------
    /** If this stack frame caused the exception, adjust the culprit
     * to be the caller. Used when a delegating method can't verify
     * all requirements itself but shouldn't receive the blame. */
    ...
    //----------------------------------------------------------------
    /** Returns true if this stack frame is blamed for causing the
     * exception.*/
