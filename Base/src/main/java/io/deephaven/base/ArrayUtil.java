...
public class ArrayUtil {
    ...
    public static <T, A> T[] addUnless(T[] a, Class<T> c, Predicate.Binary<T, A> pred, Function.Unary<T, A> factory, A arg) {
        if ( a != null ) {
            for ( int i = 0; i < a.length; ++i ) {
    ...
    public static <T, A> T[] replaceOrAdd(T[] a, Class<T> c, Predicate.Binary<T, A> pred, Function.Unary<T, A> factory, A arg) {
        if ( a != null ) {
            for ( int i = 0; i < a.length; ++i ) {
                if ( pred.call(a[i], arg) ) {
    ...
    public static String toString(double[] doubles, int nOffset, int nLength) {
        ...
        for (int nIndex=nOffset; ; nIndex++) {
            ...
            if (nIndex==nLastIndex) { return stringBuilder.append(']').toString(); }
            stringBuilder.append(", ");
        }
    }
    ...
    public static <T extends Comparable<? super T>> boolean isSorted(T[] objects) {
        if (objects != null && objects.length >= 2) {
            for(int i = 1; i < objects.length; i++) {
                ...
                // if the later element is null (previous one cannot be) then its out of order, otherwise compare values (o1 < o2 == negative integer)
