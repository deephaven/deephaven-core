...
public class FileUtils {
    ...
    private static void __findAllSubDirectories(ArrayList<File> results, File dir) {
        for (File f : dir.listFiles()) {
            if (f.isDirectory()) __findAllSubDirectories(results, f);
            else results.add(f);
        }
    }
    ...
    /**
     * I have no idea what to call this class.  It accepts all directories, and normal files accepted by its
     * delegate filter.
     */
    private static class RecursingNormalFileFilter implements FileFilter {
        ...
        public boolean accept(File pathname) {
            return pathname.isDirectory() || ( pathname.isFile() && ( normalFileFilter == null || normalFileFilter.accept(pathname) ) );
        }
    }
}
