...
public class AggregatingCharRingBuffer extends CharRingBuffer {
    ...
    /**
     * Create a ring buffer for char values that will perform pairwise aggregation of the internal data values using an
    ...
    private void evaluateRangeFast(int start, int end, char[] src, int dstOffset) {
    ...
    private char evaluateTree(int startA, int endA) {
        while (endA > 1) {
            // compute this level
    ...
    private char evaluateTree(int startA, int endA, int startB, int endB) {
        while (endB > 1) {
    ...
    private char evaluateTree(int startA, int endA, int startB, int endB, int startC, int endC) {
        while (endC > 1) {
            if (rangesCollapse(startA, endA, startB, endB)) {
                // A and B overlap
