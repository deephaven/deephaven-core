...
public class AggregatingByteRingBuffer {
    ...
    private final ByteFunction aggInitialFunction;
    private final ByteFunction aggTreeFunction;
    ...
    /**
     * Creates a ring buffer for byte values which aggregates its contents according to a user-defined aggregation
     ...
     */
    public AggregatingByteRingBuffer(final int capacity, final byte identityVal,
            @NotNull final ByteFunction aggFunction) {
     ...
     */
    public AggregatingByteRingBuffer(final int capacity,
            final byte identityVal,
            @NotNull final ByteFunction aggTreeFunction,
            @NotNull final ByteFunction aggInitialFunction) {
    ...
    public void addUnsafe(byte e) {
        ...
        if (internalBuffer.tail >= ByteRingBuffer.FIXUP_THRESHOLD) {
    ...
    public byte removeUnsafe() {
        ...
        final byte val = internalBuffer.removeUnsafe();
     ...
     * {@link #evaluateTwoRanges(int, int, int, int, ByteFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateThreeRanges(int r1h, int r1t, int r2h, int r2t, int r3h, int r3t, ByteFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     * {@link #evaluateRange(int, int, ByteFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateTwoRanges(int r1h, int r1t, int r2h, int r2t, ByteFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     */
    private void evaluateRange(int r1h, int r1t, ByteFunction evalFunction) {
        while (r1t > 1) {
     ...
     */
    private void evaluateAndStoreResults(int start, int end, byte[] src, int dstOffset, ByteFunction evalFunction) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final byte computeVal = evalFunction.apply(leftVal, rightVal);
