...
public class AggregatingObjectRingBuffer<T> extends ObjectRingBuffer<T> {
    ...
    /**
     * Create a ring buffer for Object values that will perform pairwise aggregation of the internal data values using
    ...
    private void evaluateRangeFast(int start, int end, Object[] src, int dstOffset) {
    ...
    private Object evaluateTree(int startA, int endA) {
        while (endA > 1) {
            // compute this level
    ...
    private Object evaluateTree(int startA, int endA, int startB, int endB) {
        while (endB > 1) {
    ...
    private Object evaluateTree(int startA, int endA, int startB, int endB, int startC, int endC) {
        while (endC > 1) {
            if (rangesCollapse(startA, endA, startB, endB)) {
                // A and B overlap
