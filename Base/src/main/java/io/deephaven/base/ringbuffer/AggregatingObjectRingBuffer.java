...
public class AggregatingObjectRingBuffer<T> {
    ...
    /**
     * Create a ring buffer for T values which aggregates its contents according to a user-defined aggregation function.
     * This aggregation calculation is performed lazily, when the user calls evaluate(). Internally the class manages a
     * tree of intermediate aggregation values. This allows the class to efficiently update the final aggregated value
     * when entries enter and leave the buffer, without necessarily running the calculation over the whole buffer.
     *
    ...
    public T removeUnsafe() {
        ...
        T val = internalBuffer.removeUnsafe();
     ...
     */
    private void computeAndStoreResults(int start, int end, T[] src, int dstOffset) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final T computeVal = aggFunction.apply(leftVal, rightVal);
