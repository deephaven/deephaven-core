...
public class AggregatingIntRingBuffer {
    ...
    private final IntFunction aggFunction;
     ...
     */
    public AggregatingIntRingBuffer(int capacity, int identityVal, IntFunction aggFunction) {
     ...
     */
    public AggregatingIntRingBuffer(int capacity, int identityVal, IntFunction aggFunction, boolean growable) {
     ...
     */
    private void evaluateAndStoreResults(int start, int end, int[] src, int dstOffset) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final int computeVal = aggFunction.apply(leftVal, rightVal);
