...
public class AggregatingIntRingBuffer extends IntRingBuffer {
    private final IntFunction pairwiseFunction;
    ...
    /**
     * Create a ring buffer for int values that will perform pairwise aggregation of the internal data values using an
    ...
    public AggregatingIntRingBuffer(int capacity, int identityVal, IntFunction aggFunction, boolean growable) {
        ...
        treeStorage = new int[storage.length];
    ...
    private void evaluateRangeFast(int start, int end, int[] src, int dstOffset) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final int computeVal = pairwiseFunction.apply(leftVal, rightVal);
            treeStorage[parent + dstOffset] = computeVal;
        }
    }

    private int evaluateTree(int startA, int endA) {
        while (endA > 1) {
            // compute this level
    ...
    private int evaluateTree(int startA, int endA, int startB, int endB) {
        while (endB > 1) {
    ...
    private int evaluateTree(int startA, int endA, int startB, int endB, int startC, int endC) {
        while (endC > 1) {
            if (rangesCollapse(startA, endA, startB, endB)) {
                // A and B overlap
