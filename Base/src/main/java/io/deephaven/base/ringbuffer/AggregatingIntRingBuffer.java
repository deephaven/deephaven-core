 ...
 */
public class AggregatingIntRingBuffer {
    private final IntRingBuffer internalBuffer;
    private final IntFunction aggInitialFunction;
    private final IntFunction aggTreeFunction;
    private final int identityVal;
    private static int defaultValueForThisType;
    private int[] treeStorage;
    ...
    /**
     * Creates a ring buffer for int values which aggregates its contents according to a user-defined aggregation
     ...
     */
    public AggregatingIntRingBuffer(final int capacity, final int identityVal,
            @NotNull final IntFunction aggFunction) {
     ...
     */
    public AggregatingIntRingBuffer(final int capacity,
            final int identityVal,
            @NotNull final IntFunction aggTreeFunction,
            @NotNull final IntFunction aggInitialFunction) {
    ...
    public void addUnsafe(int e) {
        ...
        if (internalBuffer.tail >= IntRingBuffer.FIXUP_THRESHOLD) {
    ...
    public int removeUnsafe() {
        ...
        final int val = internalBuffer.removeUnsafe();
    ...
    public int[] remove(int count) {
        ...
        final int[] result = internalBuffer.remove(count);
     ...
     * {@link #evaluateTwoRanges(int, int, int, int, IntFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateThreeRanges(int r1h, int r1t, int r2h, int r2t, int r3h, int r3t, IntFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     * {@link #evaluateRange(int, int, IntFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateTwoRanges(int r1h, int r1t, int r2h, int r2t, IntFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     */
    private void evaluateRange(int r1h, int r1t, IntFunction evalFunction) {
        while (r1t > 1) {
     ...
     */
    private void evaluateAndStoreResults(int start, int end, int[] src, int dstOffset, IntFunction evalFunction) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final int leftVal = src[left];
            final int rightVal = src[right];
            ...
            final int computeVal = evalFunction.apply(leftVal, rightVal);
