...
public class ShortRingBuffer implements Serializable {
    ...
    private void grow(int increase) {
        ...
        Assert.leq(newSize, "ShortRingBuffer capacity", RING_BUFFER_MAX_CAPACITY);

        final short[] newStorage = new short[Integer.highestOneBit((int) newSize - 1) << 1];
    ...
    private void copyRingBufferToArray(short[] dest) {
        final int size = size();
