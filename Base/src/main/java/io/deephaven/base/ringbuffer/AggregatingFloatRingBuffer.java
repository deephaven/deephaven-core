...
public class AggregatingFloatRingBuffer extends FloatRingBuffer {
    ...
    /**
     * Create a ring buffer for float values that will perform pairwise aggregation of the internal data values using an
    ...
    public AggregatingFloatRingBuffer(int capacity, float identityVal, FloatFunction aggFunction, boolean growable) {
        ...
        treeStorage = new float[storage.length];
    ...
    private float evaluateTree(int startA, int endA) {
        while (endA > 1) {
            // compute this level
    ...
    private float evaluateTree(int startA, int endA, int startB, int endB) {
        while (endB > 1) {
    ...
    private float evaluateTree(int startA, int endA, int startB, int endB, int startC, int endC) {
        while (endC > 1) {
            if (rangesCollapse(startA, endA, startB, endB)) {
                // A and B overlap
