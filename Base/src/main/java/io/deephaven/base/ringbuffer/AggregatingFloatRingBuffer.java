 ...
 */
public class AggregatingFloatRingBuffer {
    private final FloatRingBuffer internalBuffer;
    private final FloatFunction aggInitialFunction;
    private final FloatFunction aggTreeFunction;
    private final float identityVal;
    private static float defaultValueForThisType;
    private float[] treeStorage;
    ...
    /**
     * Creates a ring buffer for float values which aggregates its contents according to a user-defined aggregation
     ...
     */
    public AggregatingFloatRingBuffer(final int capacity, final float identityVal,
            @NotNull final FloatFunction aggFunction) {
     ...
     */
    public AggregatingFloatRingBuffer(final int capacity,
            final float identityVal,
            @NotNull final FloatFunction aggTreeFunction,
            @NotNull final FloatFunction aggInitialFunction) {
    ...
    public void addUnsafe(float e) {
        ...
        if (internalBuffer.tail >= FloatRingBuffer.FIXUP_THRESHOLD) {
    ...
    public float removeUnsafe() {
        ...
        final float val = internalBuffer.removeUnsafe();
    ...
    public float[] remove(int count) {
        ...
        final float[] result = internalBuffer.remove(count);
     ...
     * {@link #evaluateTwoRanges(int, int, int, int, FloatFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateThreeRanges(int r1h, int r1t, int r2h, int r2t, int r3h, int r3t, FloatFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     * {@link #evaluateRange(int, int, FloatFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateTwoRanges(int r1h, int r1t, int r2h, int r2t, FloatFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     */
    private void evaluateRange(int r1h, int r1t, FloatFunction evalFunction) {
        while (r1t > 1) {
     ...
     */
    private void evaluateAndStoreResults(int start, int end, float[] src, int dstOffset, FloatFunction evalFunction) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final float leftVal = src[left];
            final float rightVal = src[right];
            ...
            final float computeVal = evalFunction.apply(leftVal, rightVal);
