...
public class AggregatingDoubleRingBuffer extends DoubleRingBuffer {
    ...
    /**
     * Create a ring buffer for double values that will perform pairwise aggregation of the internal data values using
    ...
    /**
     * Create a ring buffer for double values that will perform pairwise aggregation of the internal data values using
     * an efficient binary-tree implementation to compute only changed values. The buffer can grow exponentially as
     * items are pushed into it but will not shrink as values are removed
     *
     * @param capacity the minimum size for the structure to hold
     * @param identityVal an innocuous value that will not affect the user-provided function results. for example, 0.0f
    ...
    private double evaluateTree(int startA, int endA) {
        while (endA > 1) {
            // compute this level
    ...
    private double evaluateTree(int startA, int endA, int startB, int endB) {
        while (endB > 1) {
    ...
    private double evaluateTree(int startA, int endA, int startB, int endB, int startC, int endC) {
        while (endC > 1) {
            if (rangesCollapse(startA, endA, startB, endB)) {
                // A and B overlap
