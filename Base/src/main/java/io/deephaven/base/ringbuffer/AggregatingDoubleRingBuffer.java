 ...
 */
public class AggregatingDoubleRingBuffer {
    private final DoubleRingBuffer internalBuffer;
    private final DoubleFunction aggInitialFunction;
    private final DoubleFunction aggTreeFunction;
    private final double identityVal;
    private static double defaultValueForThisType;
    ...
    /**
     * Creates a ring buffer for double values which aggregates its contents according to a user-defined aggregation
     ...
     */
    public AggregatingDoubleRingBuffer(final int capacity, final double identityVal,
            @NotNull final DoubleFunction aggFunction) {
     ...
     */
    public AggregatingDoubleRingBuffer(final int capacity,
            final double identityVal,
            @NotNull final DoubleFunction aggTreeFunction,
            @NotNull final DoubleFunction aggInitialFunction) {
    ...
    public void addUnsafe(double e) {
        ...
        if (internalBuffer.tail >= DoubleRingBuffer.FIXUP_THRESHOLD) {
    ...
    public double removeUnsafe() {
        ...
        final double val = internalBuffer.removeUnsafe();
    ...
    public double[] remove(int count) {
        ...
        final double[] result = internalBuffer.remove(count);
     ...
     * {@link #evaluateTwoRanges(int, int, int, int, DoubleFunction)} for better performance.
     * <p>
     ...
     * {@link #evaluateRange(int, int, DoubleFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateTwoRanges(int r1h, int r1t, int r2h, int r2t, DoubleFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     */
    private void evaluateRange(int r1h, int r1t, DoubleFunction evalFunction) {
        while (r1t > 1) {
     ...
     */
    private void evaluateAndStoreResults(int start, int end, double[] src, int dstOffset, DoubleFunction evalFunction) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final double computeVal = evalFunction.apply(leftVal, rightVal);
