...
// ****** Edit AggregatingCharRingBuffer and run "./gradlew replicateRingBuffers" to regenerate
//
// @formatter:off
 ...
 */
public class AggregatingShortRingBuffer {
    private final ShortRingBuffer internalBuffer;
    private final ShortFunction aggInitialFunction;
    private final ShortFunction aggTreeFunction;
    private final short identityVal;
    private static short defaultValueForThisType;
    private short[] treeStorage;
    ...
    /**
     * Creates a ring buffer for short values which aggregates its contents according to a user-defined aggregation
     ...
     */
    public AggregatingShortRingBuffer(final int capacity, final short identityVal,
            @NotNull final ShortFunction aggFunction) {
     ...
     */
    public AggregatingShortRingBuffer(final int capacity,
            final short identityVal,
            @NotNull final ShortFunction aggTreeFunction,
            @NotNull final ShortFunction aggInitialFunction) {
    ...
    public void addUnsafe(short e) {
        ...
        if (internalBuffer.tail >= ShortRingBuffer.FIXUP_THRESHOLD) {
    ...
    public short removeUnsafe() {
        ...
        final short val = internalBuffer.removeUnsafe();
    ...
    public short[] remove(int count) {
        ...
        final short[] result = internalBuffer.remove(count);
     ...
     * {@link #evaluateTwoRanges(int, int, int, int, ShortFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateThreeRanges(int r1h, int r1t, int r2h, int r2t, int r3h, int r3t, ShortFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     * {@link #evaluateRange(int, int, ShortFunction)} for better performance.
     * <p>
     * The provided ranges ore closed-interval. The head and tail are both included in the range.
     */
    private void evaluateTwoRanges(int r1h, int r1t, int r2h, int r2t, ShortFunction evalFunction) {
        while (true) {
            if (r1t >= r2h) {
     ...
     */
    private void evaluateRange(int r1h, int r1t, ShortFunction evalFunction) {
        while (r1t > 1) {
     ...
     */
    private void evaluateAndStoreResults(int start, int end, short[] src, int dstOffset, ShortFunction evalFunction) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final short leftVal = src[left];
            final short rightVal = src[right];
            ...
            final short computeVal = evalFunction.apply(leftVal, rightVal);
