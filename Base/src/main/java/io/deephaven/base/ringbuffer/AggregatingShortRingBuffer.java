...
public class AggregatingShortRingBuffer extends ShortRingBuffer {
    private final ShortFunction pairwiseFunction;
    ...
    /**
     * Create a ring buffer for short values that will perform pairwise aggregation of the internal data values using an
    ...
    public AggregatingShortRingBuffer(int capacity, short identityVal, ShortFunction aggFunction, boolean growable) {
        ...
        treeStorage = new short[storage.length];
    ...
    private void evaluateRangeFast(int start, int end, short[] src, int dstOffset) {
        ...
        for (int left = start & 0xFFFFFFFE; left <= end; left += 2) {
            ...
            final short computeVal = pairwiseFunction.apply(leftVal, rightVal);
            treeStorage[parent + dstOffset] = computeVal;
        }
    }

    private short evaluateTree(int startA, int endA) {
        while (endA > 1) {
            // compute this level
    ...
    private short evaluateTree(int startA, int endA, int startB, int endB) {
        while (endB > 1) {
    ...
    private short evaluateTree(int startA, int endA, int startB, int endB, int startC, int endC) {
        while (endC > 1) {
            if (rangesCollapse(startA, endA, startB, endB)) {
                // A and B overlap
