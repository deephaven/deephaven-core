...
public class IntRingBuffer implements Serializable {
    ...
    private void grow(int increase) {
        ...
        Assert.leq(newSize, "IntRingBuffer capacity", RING_BUFFER_MAX_CAPACITY);

        final int[] newStorage = new int[Integer.highestOneBit((int) newSize - 1) << 1];
    ...
    private void copyRingBufferToArray(int[] dest) {
        final int size = size();
