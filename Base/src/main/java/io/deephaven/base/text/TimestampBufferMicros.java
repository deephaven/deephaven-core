...
public class TimestampBufferMicros {
    ...
    private class ThreadLocalState {
        private long currentTimeMicros = Long.MIN_VALUE; // Ensure we enter the calculation logic
        ...
        public void update(long nowMicros) {
            // See if the change is more than the seconds/millis/micros component - if so then we
            // should check for DST transition and
            ...
            if (nowMicros / 60_000_000 != currentTimeMicros / 60_000_000) {
                ...
                buffer.put(Convert.ISO8601_SECOND_OFFSET + 1,
                    (byte) ('0' + (v % 10_000_000) / 1_000_000));
                buffer.put(Convert.ISO8601_MILLIS_OFFSET, (byte) ('0' + (v % 1_000_000) / 100_000));
                buffer.put(Convert.ISO8601_MILLIS_OFFSET + 1,
                    (byte) ('0' + (v % 100_000) / 10_000));
        ...
        private void calculateDSTTransitions(final long nowMicros) {
            ...
            previousDSTTransitionMicros = previousTransitionOffset != null
                ? previousTransitionOffset.toEpochSecond() * 1000L * 1000L
                : Long.MIN_VALUE;
            ...
            if (gmtOffsetMicros == 0) {
                ...
                int minutes =
                    (int) ((Math.abs(gmtOffsetMicros / 1000) - hours * 3_600_000) / 60_000);
     ...
     * 
     * @throws java.nio.BufferOverflowException if there is not enough room in the destination
     *         buffer.
     */
