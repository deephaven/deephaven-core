...
public class TimestampBufferMicros {
    ...
    private class ThreadLocalState {
        ...
        public void update(long nowMicros) {
            // See if the change is more than the seconds/millis/micros component - if so then we should check for DST transition and
            ...
            if ( nowMicros / 60_000_000 != currentTimeMicros / 60_000_000 ) {
                ...
                if ( v < 0 ) { v += 60_000_000; } // for dates before the epoch
        ...
        private void calculateDSTTransitions(final long nowMicros) {
            ...
            previousDSTTransitionMicros = previousTransitionOffset != null ? previousTransitionOffset.toEpochSecond() * 1000L * 1000L : Long.MIN_VALUE;
            nextDSTTransitionMicros = nextTransitionOffset != null ? nextTransitionOffset.toEpochSecond() * 1000L * 1000L : Long.MAX_VALUE;
