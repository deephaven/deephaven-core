...
public class HistogramState extends State {
    ...
    private int getBucket(long sample) {
        if (sample < rangeMin) return 0;
        if (sample >= rangeMax) return buckets.length-1;
    ...
    public HistogramState(long now, Spec spec) {
        ...
        this.buckets[0] = Stats.makeItem(spec.groupName, spec.itemName+"[0]", State.FACTORY, "Values of "+spec.itemName+" less than "+rangeMin, now).getValue();
        for (int i = 1; i <= spec.numBuckets; ++i) {
            this.buckets[i] = Stats.makeItem(spec.groupName, spec.itemName+'['+i+']', State.FACTORY, "Values of "+spec.itemName+" between "+(long)(rangeMin+(i-1)*rangeBucket)+" (incl.) and "+(long)(rangeMin+i*rangeBucket)+" (excl.)", now).getValue();
        }
        this.buckets[spec.numBuckets+1] = Stats.makeItem(spec.groupName, spec.itemName+'['+(spec.numBuckets+1)+']', State.FACTORY, "Values of "+spec.itemName+" at least "+rangeMax, now).getValue();
    }
    ...
    public static final Function.Binary<HistogramState, Long, Spec> FACTORY = new Function.Binary<HistogramState, Long, Spec>() {
