...
public class ThreadSafeFixedSizePool<T> implements Pool<T> {
    ...
    public static Factory FACTORY=new Factory() {
        @Override public <T> Pool<T> create(int nSize, Function.Nullary<T> itemFactory, Procedure.Unary<T> clearingProcedure) {
            return new ThreadSafeFixedSizePool<T>(Require.geq(nSize, "nSize", MIN_SIZE, "MIN_SIZE"), Require.neqNull(itemFactory, "itemFactory"), clearingProcedure);
        }
    };
    ...
    public ThreadSafeFixedSizePool(int size, Function.Nullary<T> factory, Procedure.Unary<T> clearingProcedure, @Nullable Logger log, @Nullable String logPfx) {
    ...
    private ThreadSafeFixedSizePool(int size, @Nullable Function.Nullary<T> factory, Procedure.Unary<T> clearingProcedure, Logger log, String logPfx, boolean dummy) {
        Require.geq(size, "size", MIN_SIZE, "MIN_SIZE");
        Require.requirement((log == null) == (logPfx == null), "log and logPfx must either both be null, or both non-null");
        ...
        try {
            ...
            if ( log != null ) {
                ...
                if ( now > nextGiveLog ) {
                    nextGiveLog = (now + 100000) - (now % 100000);
                    long dt = (now - t0);
                    log.warn(logPfx+": give() took "+dt+" micros, with "+yields+" yields and "+spins+" additional spins");
                }
            }
        }
    }
        ...
        try {
                ...
                if ( ++spins > SPIN_COUNT ) {
                    ...
                    if ( log != null ) {
                        ...
                        if ( now > nextTakeLog ) {
                            ...
                            long dt = (now - t0);
                            log.warn(logPfx+": take() can't dequeue from pool, waiting for "+dt+" micros, yield count = "+yields);
                        }
                    }
                    Thread.yield();
                    spins = 0;
            ...
            if ( log != null ) {
                ...
                if ( now > nextTakeLog ) {
                    nextTakeLog = (now + 100000) - (now % 100000);
                    long dt = (now - t0);
                    log.warn(logPfx+": take() took "+dt+" micros, with "+yields+" yields and "+spins+" additional spins");
                }
            }
        }
    }
