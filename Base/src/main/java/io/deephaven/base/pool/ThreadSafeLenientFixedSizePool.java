...
public class ThreadSafeLenientFixedSizePool<T> implements Pool.MultiPool<T> {
    ...
    public static Factory FACTORY=new Factory() {
        @Override public <T> Pool<T> create(int nSize, Function.Nullary<T> itemFactory, Procedure.Unary<T> clearingProcedure) {
            return new ThreadSafeLenientFixedSizePool<T>(Require.geq(nSize, "nSize", MIN_SIZE, "MIN_SIZE"), Require.neqNull(itemFactory, "itemFactory"), clearingProcedure);
        }
    };

    private static <T> Function.Unary<T, ThreadSafeLenientFixedSizePool<T>> makeNullaryFactoryAdapter(final Function.Nullary<T> factory) {
        return new Function.Unary<T, ThreadSafeLenientFixedSizePool<T>>() {
            @Override public T call(ThreadSafeLenientFixedSizePool<T> arg) {
                return factory.call();
            }
        };
    }
    ...
    public ThreadSafeLenientFixedSizePool(int size, Function.Nullary<T> factory, Procedure.Unary<? super T> clearingProcedure) {
        this(
    ...
    public ThreadSafeLenientFixedSizePool(String name, int size, Function.Nullary<T> factory, Procedure.Unary<? super T> clearingProcedure) {
        this(
            name,
            Require.geq(size, "size", MIN_SIZE, "MIN_SIZE"),
            makeNullaryFactoryAdapter(Require.neqNull(factory, "factory")),
    ...
    public ThreadSafeLenientFixedSizePool(int size, Function.Unary<T, ThreadSafeLenientFixedSizePool<T>> factory, Procedure.Unary<? super T> clearingProcedure) {
        this(null, size, factory, clearingProcedure);
    }

    public ThreadSafeLenientFixedSizePool(String name, int size, Function.Unary<T, ThreadSafeLenientFixedSizePool<T>> factory, Procedure.Unary<? super T> clearingProcedure) {
        Require.geq(size, "size", MIN_SIZE, "MIN_SIZE");
    ...
    protected boolean giveInternal(T item) {
        ...
        if (null!=clearingProcedure) { clearingProcedure.call(item); }
