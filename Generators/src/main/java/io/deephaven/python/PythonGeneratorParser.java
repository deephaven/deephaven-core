...
class PythonGeneratorParser{
     ...
     */
    static GroovyStaticImportGenerator.JavaFunction[] getValidMethods(String javaClass, Predicate<Method> filter) throws ClassNotFoundException{
     ...
     * @throws ClassNotFoundException
     */
    static GroovyStaticImportGenerator.JavaFunction[] getPublicStaticMethods(final String javaClass, final List<String> skipGeneration) throws ClassNotFoundException{
        return getValidMethods(javaClass, (m) -> PythonGeneratorParser.isValidPublicStatic(m) && (skipGeneration==null || !skipGeneration.contains(javaClass + "," + m.getName())));
    }
     ...
     * @throws ClassNotFoundException
     */
    static MethodContainer getPublicStaticMethodsDetails(final String javaClass, final List<String> skipGeneration) throws ClassNotFoundException{
    ...
    /**
     * Get the List of DocStringContainers corresponding to the java class specified by path and the documentation
     * roots specified in docRoot.
     *
     * Note that it is currently expected that many classes have no docs populated, so nulls (corresponding to
     ...
     */
    static List<DocstringContainer> getDocstringContainer(final String path, final List<String> docRoot, final Logger log){
    ...
    /**
     * Format the input string as a (multiline) Python docstring. It is assumed that the beginning/trailing
     * quotation marks are not present, and there is no indentation formatting.
     ...
     * The priority of the DocstringContainers is implicit in their order in the List, so the first one
     * (in List order) which returns a non-null docstring will be used.
    ...
    /**
     * Fetch the doc string for a given method from a List of DocstringContainers and format appropriately.
     * The priority of the DocstringContainers is implicit in their order in the List, so the first one
     * (in List order) which returns a non-null docstring will be used.
     * @param docList List of the DocstringContainer object for the class in question
    ...
    public static class MethodSignatureCollection{
        ...
        public List<MethodSignature> getSignatures(){ return signatures; }
    ...
    public static class MethodContainer{
        ...
        public Map<String, MethodSignatureCollection> getMethodSignatures(){ return methodSignatures; }
    }
