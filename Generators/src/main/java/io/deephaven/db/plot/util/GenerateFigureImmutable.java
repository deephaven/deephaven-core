...
public class GenerateFigureImmutable {
    ...
    private GenerateFigureImmutable(final boolean isInterface, final String[] imports,
        final String[] interfaces, final String[] seriesInterfaces,
    ...
    private String generateCode() {
        ...
        code += "public" + (isInterface ? " interface " : " class ") + outputClassNameShort
            + generateImplements() + " {\n";
    ...
    private String createConstructor() {
        ...
        return "    private static final long serialVersionUID = -4519904656095275663L;\n" +
            ...
            "\n" +
            "    private " + outputClassNameShort
            + " make(final AxesImpl axes, final AxisImpl axis){\n" +
    ...
    private String createFigureFuncs() {
        return "    /**\n" +
            ...
            "     */\n" +
            "    " + (isInterface ? "" : "@Override public ") + "Figure"
            + (isInterface ? "" : "Impl") + " show()" +
            (isInterface ? ";\n"
                : " {\n"
    ...
    private static String createInstanceGetter(final JavaFunction f) {
        switch (f.getClassName()) {
            ...
            default:
                System.out.println(
                    "Don't know how to create instance getter: className=" + f.getClassName());
                return "WTF(fc)";
        }
    }
    ...
    private String createFunction(final JavaFunction f) {
        ...
        if (returnClass != null && BaseFigure.class.isAssignableFrom(returnClass)) {
            s += indent(2) + createInstanceGetter(f) + "." + signature.getMethodName() + "("
                + callArgs + ");\n" +
            ...
            && f.getClassName().equals("io.deephaven.db.plot.Axes")) {
            s += indent(2) + "final AxesImpl axes = " + createInstanceGetter(f) + ";\n";
            s += indent(2) + "final AxisImpl axis = (AxisImpl) axes." + signature.getMethodName()
                + "(" + callArgs + ");\n" +
            ...
            s += indent(2) + "final DataSeriesInternal series = (DataSeriesInternal) "
                + createInstanceGetter(f) + "." + signature.getMethodName() + "(" + callArgs
                + ");\n" +
                indent(2) + "return make(series);\n";
        } else if (returnClass != null && Series.class.isAssignableFrom(returnClass)) {
            s += indent(2) + "final SeriesInternal series = (SeriesInternal) "
                + createInstanceGetter(f) + "." + signature.getMethodName() + "(" + callArgs
                + ");\n" +
                indent(2) + "return make(series);\n";
        } else if (returnClass != null && MultiSeries.class.isAssignableFrom(returnClass)) {
            s += indent(2) + "final " + returnClass.getSimpleName() + " mseries = "
                + createInstanceGetter(f) + "." + signature.getMethodName() + "(" + callArgs
                + ");\n" +
    ...
    private String createSignatureGroupFunction(final TreeSet<JavaFunction> fs) {
        ...
        for (final JavaFunction f : fs) {
            ...
            s += indent(3) + returnClass.getSimpleName() + " result = ((" + f.getClassNameShort()
                + ") series)." + f.getMethodName() + "(" + callArgs + ");\n";
    ...
    private Map<String, TreeSet<GroovyStaticImportGenerator.JavaFunction>> commonSignatureGroups(
        ...
        final Map<String, TreeSet<GroovyStaticImportGenerator.JavaFunction>> methods =
            new TreeMap<>();
        ...
        for (String iface : interfaces) {
            ...
            for (final java.lang.reflect.Method m : c.getMethods()) {
                ...
                if (!isStatic && isPublic && !isObject) {
                    final GroovyStaticImportGenerator.JavaFunction f =
                        new GroovyStaticImportGenerator.JavaFunction(m);
                    if (functionSet.add(f)) { // avoids repeating methods that have the same
                                              // parameter types but different parameter names
    ...
    private static String makeMultiSeriesGetter(final JavaFunction f) {
        ...
        return "else if(series instanceof MultiSeries) {\n" +
            "                final MultiSeries result = ((MultiSeries) series)." + f.getMethodName()
            + "(" + args + ");\n" +
    ...
    private static void generateFile(final String devroot, final boolean assertNoChange,
        final boolean isInterface) throws ClassNotFoundException, IOException {
        ...
        final List<Predicate<JavaFunction>> skips = Arrays.asList(x -> {
            try {
                return x
                    .equals(new JavaFunction(PlotExceptionCause.class.getMethod("getPlotInfo")));
            ...
            try {
                return x
                    .equals(new JavaFunction(BaseFigureImpl.class.getMethod("save", String.class)))
                    ||
                    x.equals(new JavaFunction(
                        BaseFigureImpl.class.getMethod("save", String.class, int.class, int.class)))
                    ||
                    x.equals(new JavaFunction(BaseFigureImpl.class.getMethod("save", String.class,
                        boolean.class, long.class)))
                    ||
                    x.equals(new JavaFunction(BaseFigureImpl.class.getMethod("save", String.class,
                        int.class, int.class, boolean.class, long.class)));
            } catch (NoSuchMethodException e) {
                throw new RuntimeException(e);
            }
        });
        ...
        String file =
            devroot + "/Plot/src/main/java/" + gen.outputClass.replace(".", "/") + ".java";
