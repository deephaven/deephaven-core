...
public class GenerateMultiSeries {
    ...
    public static void main(String[] args) throws ClassNotFoundException, IOException, NoSuchMethodException {
        ...
        new Generator(
                ...
                false,
                false,
                "io.deephaven.db.plot.datasets.category.CategoryDataSeries").generateCode(devroot, assertNoChange, skip);
    ...
    static class Generator {
        ...
        Generator(final String outputClass, final String parameterizedType, final boolean isGeneric, final boolean isTransform, final boolean isSwappable, final String... interfaces) throws ClassNotFoundException {
        ...
        private void generateCode(final String devroot, final boolean assertNoChange, final Set<Method> skip) throws ClassNotFoundException, IOException {
            ...
            final String header2 = headerSpace + headerComment + " TO REGENERATE RUN GenerateMultiSeries " + headerComment;
            final String header3 = headerSpace + headerComment + " AND THEN RUN GenerateFigureImmutable " + headerComment;
            ...
            if (cutPoint != axes.indexOf(header)) {
                throw new IllegalArgumentException("Input source code contains two autogenerated sections! file=" + outputFile);
            }
            ...
            if (assertNoChange) {
                ...
                if (!newcode.equals(oldCode)) {
                    throw new RuntimeException("Change in generated code for " + outputFile + ".  Run GenerateMultiSeries or \"./gradlew :Generators:generateMultiSeries\" to regenerate\n");
                }
            } else {
        ...
        private String createInitializeFunction(final Map<String, GroovyStaticImportGenerator.JavaFunction> mapToFunction) {
            final StringBuilder code = new StringBuilder();
            final Map<GroovyStaticImportGenerator.JavaFunction, Function<String, String>> functionToGenerics = createFunctionToGenerics(mapToFunction.values());
            ...
            for (final Map.Entry<String, GroovyStaticImportGenerator.JavaFunction> entry : mapToFunction.entrySet()) {
                ...
                if (oneArg) {
                    ...
                    try {
                        ...
                        typeName = typeName.contains(Closure.class.getCanonicalName()) ? ClosureFunction.class.getCanonicalName() : typeName;
                        code.append(indent(2)).append("java.util.function.Consumer<")
                                .append(toGenerics == null ? TypeUtils.getBoxedType(ClassUtil.lookupClass(typeName)).getCanonicalName() : toGenerics.apply(typeName))
                ...
                if (!oneArg) {
                    ...
                    for (int i = 0; i < types.length; i++) {
                        ...
                        if (typeName.contains("java.util.Map") || typeName.contains("groovy.lang.Closure") || typeName.contains("java.util.function.Function")) {
                            typeName = typeName.replace(Closure.class.getCanonicalName(), ClosureFunction.class.getCanonicalName());
        ...
        private Map<GroovyStaticImportGenerator.JavaFunction, Function<String, String>> createFunctionToGenerics(Collection<GroovyStaticImportGenerator.JavaFunction> functionSet) {
            ...
            for (final GroovyStaticImportGenerator.JavaFunction function : functionSet) {
                for (TypeVariable<Method> typeVariable : function.getTypeParameters()) {
                    ...
                    if (bounds.length > 1) { //probably don't need more than one extend at this point, this is just much easier right now todo?
        ...
        private String createGenericInitializeSeries(final Map<String, GroovyStaticImportGenerator.JavaFunction> mapToFunction, final Map<GroovyStaticImportGenerator.JavaFunction, Function<String, String>> functionToGenerics) {
            final List<String> args = new ArrayList<>();
        ...
        private String createMapName(final GroovyStaticImportGenerator.JavaFunction function) {
            return "%METHOD%SeriesNameTo%TYPES%Map".replaceAll("%TYPES%", createArgsString(function)).replaceAll("%METHOD%", function.getMethodName());
        }
        ...
        private String createMethod(final String returnClass, final GroovyStaticImportGenerator.JavaFunction function, final String mapName) {
            final StringBuilder code = new StringBuilder();
        ...
        private String createMethodWithFunctionParameter(String returnClass, GroovyStaticImportGenerator.JavaFunction function) throws ClassNotFoundException {
            final StringBuilder code = new StringBuilder();
            code.append(createMethodHeader(returnClass, function));
            if (isTransform) {
                code.append(createTransformBody(function));
        ...
        private String createFunctionalBody(final String returnClass, GroovyStaticImportGenerator.JavaFunction function) throws ClassNotFoundException {
            if(function.getParameterTypes()[0].getTypeName().startsWith("groovy.lang.Closure")) {
                return indent(2) + "return " + function.getMethodName() + "(new io.deephaven.db.plot.util.functions.ClosureFunction<>(" + function.getParameterNames()[0] + "), keys);\n" + indent(1) + "}\n\n";
            }
            ...
            return indent(2) + "final String newColumn = " + getColumnNameConstant(function.getMethodName()) + " + this.hashCode();\n" +
                   ...
                   indent(2) + "chart().figure().registerFigureFunction(new io.deephaven.db.plot.util.functions.FigureImplFunction(f -> f." +
                    tableMethodName +
        ...
        private String getFigureFunctionInput(final String returnClass, final GroovyStaticImportGenerator.JavaFunction function, final String tableMethodName) throws ClassNotFoundException {
            final StringBuilder code = new StringBuilder();
            code.append(isSwappable ? "new SelectableDataSetSwappableTable(getSwappableTable()), " : "getTableMapHandle().getTable(), ");
            ...
            if(function.getMethodName().equals("pointColorByY")) {
                ...
                switch(tableMethod.getParameterCount()) {
                    ...
                    default:
                        ...
                        throw new IllegalStateException("Can not calculate function input for function " + tableMethodName + " in class " + function.getClassNameShort());
                }
            ...
            if (methods.length != 1) {
                log.warning("More than 1 possible table method for function input for function " + tableMethodName + " in class " + function.getClassNameShort());
        ...
        private static GroovyStaticImportGenerator.JavaFunction[] filterBadMethods(final GroovyStaticImportGenerator.JavaFunction[] functions) {
            ...
            for (final GroovyStaticImportGenerator.JavaFunction function : functions) {
                if(function.getParameterTypes()[0].getTypeName().contains("Function")) {
                    final Pair<String, Integer> methodPair = new Pair<>(function.getMethodName(), function.getParameterNames().length);
        ...
        private String getFunctionInput(GroovyStaticImportGenerator.JavaFunction function) {
            ...
            throw new IllegalStateException("Don't know what to make function input for method " + function.getMethodName() + " param class " + function.getParameterTypes()[0].getClass());
        }
        ...
        private String getTableMethodName(String methodName) {
            methodName = methodName.endsWith("ByX") || methodName.endsWith("ByY") ? methodName.substring(0, methodName.length() - 3) : methodName;
        ...
        private String createTransformBody(final GroovyStaticImportGenerator.JavaFunction function) {
            ...
            args.add("keys");
            return indent(2) + "//noinspection unchecked\n" + indent(2) + "return (AbstractMultiSeries<SERIES>) series." + function.getMethodName() + "(" + String.join(", ", args) + ");\n" + indent(1) + "}\n\n";
        }

        private String createExceptionMethodBody(final GroovyStaticImportGenerator.JavaFunction function) {
            return indent(2) + "throw new PlotUnsupportedOperationException(\"DataSeries \" + this.getClass() + \" does not support method " + function.getMethodName() + " for arguments " + Arrays.toString(function.getParameterTypes()) + ". If you think this method should work, try placing your keys into an Object array\", this);\n" + indent(1) + "}\n\n";
        }

        private String createMethodHeader(final String returnClass, final GroovyStaticImportGenerator.JavaFunction function) {
            String methodHeader = indent(1) + (!isInterface ? "@Override public " : "") + getGenericTypes(function) + returnClass + (isGeneric ? "<SERIES>" : "") + " " + function.getMethodName() + "(";
            ...
            return methodHeader + String.join(", ", arguments) + ", final Object... keys)" + (!isInterface ? " {\n" : ";\n");
        }
        ...
        private String createMapCode(final GroovyStaticImportGenerator.JavaFunction function, final String mapName) {
            ...
            for(int i = 0; i < vars.length; i++) {
                ...
                if(var.getTypeName().equals("io.deephaven.db.tables.Table")) {
                    ...
                    code.append(indent(1)).append("final io.deephaven.db.plot.util.tables.TableHandle ").append(handleName).append(" = new io.deephaven.db.plot.util.tables.TableHandle(").append(names[i]);
        ...
        private String createSmartKeyArgs(final GroovyStaticImportGenerator.JavaFunction function, final Map<String, String> tableToTableHandleVarMap) {
            List<String> args = new ArrayList<>();
            final Type[] vars = function.getParameterTypes();
            final String[] names = Arrays.stream(function.getParameterNames()).map(s -> tableToTableHandleVarMap.getOrDefault(s, s)).toArray(String[]::new);
