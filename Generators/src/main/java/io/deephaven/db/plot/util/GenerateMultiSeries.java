...
public class GenerateMultiSeries {
    ...
    public static void main(String[] args)
        ...
        skip.add(Class.forName("io.deephaven.db.plot.datasets.DataSeries").getMethod("pointSize",
            int.class));
        skip.add(Class.forName("io.deephaven.db.plot.datasets.DataSeries").getMethod("pointSize",
            double.class));
        skip.add(Class.forName("io.deephaven.db.plot.datasets.DataSeries").getMethod("pointSize",
        ...
        new Generator("io.deephaven.db.plot.datasets.multiseries.MultiSeries",
            ...
            "io.deephaven.db.plot.datasets.ohlc.OHLCDataSeries").generateCode(devroot,
                assertNoChange, skip);
        ...
        new Generator(
            ...
            false,
            false,
            "io.deephaven.db.plot.datasets.category.CategoryDataSeries").generateCode(devroot,
                assertNoChange, skip);
        ...
        new Generator(
            ...
            false,
            false,
            "io.deephaven.db.plot.datasets.xy.XYDataSeries").generateCode(devroot, assertNoChange,
                skip);
        new Generator(
    ...
    static class Generator {
        ...
        private void generateCode(final String devroot, final boolean assertNoChange,
            final Set<Method> skip) throws ClassNotFoundException, IOException {
            final StringBuilder code = new StringBuilder();
            final String outputFile =
                devroot + "/Plot/src/main/java/" + outputClass.replace(".", "/") + ".java";
            ...
            final String header2 = headerSpace + headerComment
                + " TO REGENERATE RUN GenerateMultiSeries " + headerComment;
            final String header3 = headerSpace + headerComment
                + " AND THEN RUN GenerateFigureImmutable " + headerComment;
            ...
            final String axes =
                Files.lines(Paths.get(outputFile)).reduce("\n", (a, b) -> a + "\n" + b);
        ...
        private String generateClasses(final Set<Method> skip) throws ClassNotFoundException {
            ...
            if (!isInterface && !isAbstract) {
                code.append(indent(1)).append("@Override public void initializeSeries(")
                    ...
                    .append(" series) {\n").append(indent(2))
                    .append("$$initializeSeries$$(series);\n").append(indent(1)).append("}\n\n");
            }
            final List<GroovyStaticImportGenerator.JavaFunction> sortedMethods = new ArrayList<>();
            final List<GroovyStaticImportGenerator.JavaFunction> methodsWithFunctionParameter =
                new ArrayList<>();
            ...
            for (final GroovyStaticImportGenerator.JavaFunction function : methodsWithFunctionParameter) {
                ...
                final String outputSimple =
                    isTransform ? "AbstractMultiSeries<SERIES>" : output.getSimpleName();
        ...
        private String createCopyConstructor(final Set<String> strings) {
            ...
            for (final String var : strings) {
                copiedVars
                    .add("        %NEWSERIES%.%VAR% = %VAR%.copy();".replaceAll("%VAR%", var));
            }
            ...
            return ret.replaceAll("%CLASSNAME%", output.getSimpleName()).replaceAll("%NEWSERIES%",
                "__s__");
        }
        ...
        private String createInitializeFunction(
            ...
            int numConsumers = 0;
            for (final Map.Entry<String, GroovyStaticImportGenerator.JavaFunction> entry : mapToFunction
                .entrySet()) {
                ...
                if (oneArg) {
                    ...
                    try {
                        ...
                        code.append(indent(2)).append("java.util.function.Consumer<")
                            .append(
                                toGenerics == null
                                    ? TypeUtils.getBoxedType(ClassUtil.lookupClass(typeName))
                                        .getCanonicalName()
                ...
                if (!oneArg) {
                    ...
                    for (int i = 0; i < types.length; i++) {
                        ...
                        if (typeName.contains("java.util.Map")
                            || typeName.contains("groovy.lang.Closure")
                        ...
                        if (typeName.equals("io.deephaven.db.tables.Table")) {
                            arg = "((io.deephaven.db.plot.util.tables.TableHandle) objectArray[" + i
                                + "]).getTable()";
        ...
        private Map<GroovyStaticImportGenerator.JavaFunction, Function<String, String>> createFunctionToGenerics(
            ...
            final Map<GroovyStaticImportGenerator.JavaFunction, Function<String, String>> map =
                new HashMap<>();
            ...
            for (final GroovyStaticImportGenerator.JavaFunction function : functionSet) {
                for (TypeVariable<Method> typeVariable : function.getTypeParameters()) {
                    ...
                    if (bounds.length > 1) { // probably don't need more than one extend at this
                                             // point, this is just much easier right now todo?
                        throw new UnsupportedOperationException(
                            "Doesn't support more than one type right now.");
                    }
                    ...
                    if (!generics.containsKey(typeName)) {
                        ...
                        if (oldFunction == null) {
                            ...
                            newFunction =
                                s -> oldFunction.apply(s).replaceAll(typeName, newTypeName);
                        }
        ...
        private String createMapName(final GroovyStaticImportGenerator.JavaFunction function) {
            return "%METHOD%SeriesNameTo%TYPES%Map"
                .replaceAll("%TYPES%", createArgsString(function))
                .replaceAll("%METHOD%", function.getMethodName());
        }

        private String createMap(String mapType, final String mapName) {
            return indent(1) + "private " + mapType + " " + mapName
                + " = new io.deephaven.db.plot.util.PlotUtils" +
                ".HashMapWithDefault<>();\n";
        }

        private String createMethod(final String returnClass,
            final GroovyStaticImportGenerator.JavaFunction function, final String mapName) {
            final StringBuilder code = new StringBuilder();
        ...
        private String createFunctionalBody(final String returnClass,
            GroovyStaticImportGenerator.JavaFunction function) throws ClassNotFoundException {
            ...
            return indent(2) + "final String newColumn = "
                + getColumnNameConstant(function.getMethodName()) + " + this.hashCode();\n" +
        ...
        private String getFigureFunctionInput(final String returnClass,
            ...
            if (function.getMethodName().equals("pointColorByY")) {
                final Class c =
                    Class.forName("io.deephaven.db.plot.datasets.multiseries." + returnClass);
                final Method[] methods = Arrays.stream(c.getDeclaredMethods())
                    .filter(m -> m.getName().equals(tableMethodName))
                    .filter(m -> m.getParameterTypes().length > 0
                        && m.getParameterTypes()[0].equals(Table.class))
                    .toArray(Method[]::new);
                ...
                switch (tableMethod.getParameterCount()) {
                    ...
                    default:
                        ...
                        throw new IllegalStateException(
                            "Can not calculate function input for function " + tableMethodName
            ...
            if (methods.length < 1) {
                throw new IllegalStateException("No table methods for method " + tableMethodName
                    + " in class " + function.getClassNameShort());
            }

            if (methods.length != 1) {
                log.warning("More than 1 possible table method for function input for function "
        ...
        private static GroovyStaticImportGenerator.JavaFunction[] filterBadMethods(
            ...
            final Map<Pair<String, Integer>, GroovyStaticImportGenerator.JavaFunction> functionMap =
                new HashMap<>();
            ...
            for (final GroovyStaticImportGenerator.JavaFunction function : functions) {
                if (function.getParameterTypes()[0].getTypeName().contains("Function")) {
                    ...
                    final GroovyStaticImportGenerator.JavaFunction oldFunction =
                        functionMap.get(methodPair);

                    if (oldFunction != null) {
                        if (oldFunction.getTypeParameters().length < 1
                            && function.getTypeParameters().length > 0) {
        ...
        private String getFunctionInput(GroovyStaticImportGenerator.JavaFunction function) {
            if (function.getMethodName().endsWith("ByX")) {
                ...
                return "getY()";
            } else if (function.getParameterTypes()[0].getTypeName()
                .startsWith("java.util.function.Function")) {
                return "getX()";
            }

            throw new IllegalStateException(
                "Don't know what to make function input for method " + function.getMethodName()
                    + " param class " + function.getParameterTypes()[0].getClass());
        }
        ...
        private String getReturnTypeName(final GroovyStaticImportGenerator.JavaFunction function) {
            ...
            return function.getTypeParameters()[function.getTypeParameters().length - 1]
                .getBounds()[0].getTypeName();
        }
        ...
        private String getColumnNameConstant(String methodName) {
            ...
            throw new IllegalStateException(
                "No column name constant corresponds to method name " + methodName);
        }

        private String createTransformBody(
            ...
            return indent(2) + "//noinspection unchecked\n" + indent(2)
                + "return (AbstractMultiSeries<SERIES>) series." + function.getMethodName() + "("
                + String.join(", ", args) + ");\n" + indent(1) + "}\n\n";
        }

        private String createExceptionMethodBody(
            final GroovyStaticImportGenerator.JavaFunction function) {
            return indent(2)
                + "throw new PlotUnsupportedOperationException(\"DataSeries \" + this.getClass() + \" does not support method "
                + function.getMethodName() + " for arguments "
                + Arrays.toString(function.getParameterTypes())
        ...
        private String createMethodHeader(final String returnClass,
            final GroovyStaticImportGenerator.JavaFunction function) {
            String methodHeader = indent(1) + (!isInterface ? "@Override public " : "")
                + getGenericTypes(function) + returnClass + (isGeneric ? "<SERIES>" : "") + " "
            ...
            if (args.length != types.length) {
                throw new IllegalStateException(
                    "Number of parameter names and number of parameter types not equal!");
            }
        ...
        private String createMapCode(final GroovyStaticImportGenerator.JavaFunction function,
            ...
            for (int i = 0; i < vars.length; i++) {
                ...
                if (var.getTypeName().equals("io.deephaven.db.tables.Table")) {
                    ...
                    code.append(indent(1))
                        .append("final io.deephaven.db.plot.util.tables.TableHandle ")
                        .append(handleName)
                        .append(" = new io.deephaven.db.plot.util.tables.TableHandle(")
                        .append(names[i]);
                    ...
                    code.append(");\n");
                    code.append(indent(1)).append("addTableHandle(").append(handleName)
                        .append(");\n");
                }
            }
            ...
            code.append("\n").append(indent(3))
                .append(mapName)
                .append(
                    ".put(namingFunction.apply(keys.length == 1 ? keys[0] : new io.deephaven.datastructures.util.SmartKey(keys)), ");
            if (oneArgument) {
                code.append("\n").append(indent(4)).append(args).append(");\n").append(indent(2))
                    .append("}\n");
            } else {
                code.append("\n").append(indent(4)).append("new Object[]{ ").append(args)
        ...
        private String createArgsString(final GroovyStaticImportGenerator.JavaFunction function) {
            ...
            for (final Type type : function.getParameterTypes()) {
                ...
                args += typeName.substring(indDot + 1, typeName.length()).replaceAll("\\[", "Array")
                    .replaceAll("\\]", "");
            }
            return args;
        }
    ...
    private static String getMapType(GroovyStaticImportGenerator.JavaFunction function) {
        ...
        if (types.length == 1) {
            ...
            try {
                possiblyBoxed =
                    TypeUtils.getBoxedType(ClassUtil.lookupClass(types[0].getTypeName()));
            } catch (ClassNotFoundException e) {
