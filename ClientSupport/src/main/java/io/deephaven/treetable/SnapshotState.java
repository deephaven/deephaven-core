...
class SnapshotState {
    ...
    /**
     * A List of column-name, data array mappings. The arrays in this mapping are references to
     * items in dataMatrix below
     */
    ...
    /**
     * The number of total rows that have been skipped by the snapshot algorithm before data is
     * collected
     */
    int skippedRows = 0;
    ...
    /**
     * Copiers that copy data from the original table to the intermediate columns. These are for
     * rollups only
     */
    ...
    interface Copier {
        void copy(boolean usePrev, ColumnSource columnSource, Index.Iterator it, Object target,
            int offset, Table table, TableMap tableMap, BitSet childPresenceColumn);
    }

    SnapshotState(HierarchicalTable baseTable, String hierarchicalColumnName) {
        this.baseTable = baseTable;
        this.info = baseTable.getInfo();
        includeConstituents = (info instanceof RollupInfo
            && ((RollupInfo) info).getLeafType() == RollupInfo.LeafType.Constituent);
        ...
        if (includeConstituents) {
            ...
            aggToSourceMap = ((RollupInfo) info).getMatchPairs()
                .stream()
                .filter(p -> source.hasColumns(p.rightColumn)) // Filter out any columns that don't
                                                               // exist in the parent
                                                               // this is a concern for the Count
                                                               // aggregation.
    ...
    void beginSnapshot(Map<Object, TableDetails> tablesByKey, BitSet requestedColumns,
        long firstViewportRow, long lastViewportRow) {
        ...
        final int newViewportSize = (int) (Math.min(lastViewportRow + 1, totalRowCount)
            - Math.min(firstViewportRow, totalRowCount));
        ...
        if (ensureSpace(newViewportSize, tablesByKey.get(TreeTableConstants.ROOT_TABLE_KEY))
            ...
            for (int ii = 0; ii < data.size(); ii++) {
                if (requestedColumns.get(ii)) {
                    ...
                    // If including constituents, populate the set of original columns that we
                    // actually need, to save us work
                    ...
                    if (includeConstituents) { // Do we include constituents?
                        ...
                        if (!io.deephaven.db.util.string.StringUtils.isNullOrEmpty(sourceName) &&
                            !((RollupInfo) info).getSelectColumnNames().contains(dataPair.first) && // Is
                                                                                                    // it
                                                                                                    // one
                                                                                                    // of
                                                                                                    // the
                                                                                                    // grouping
                                                                                                    // columns?
                            !RollupInfo.ROLLUP_COLUMN.equals(dataPair.first) && // Is it the magic
                                                                                // rollup column?
                            !ColumnFormattingValues.isFormattingColumn(dataPair.first)) { // Is it a
                                                                                          // formatting
                                                                                          // column?
    ...
    /**
     * Copy data from the specified table into the resultant data matrix at the offsets specified in
     * the viewport state.
     *
     * @implNote This method populates an additional data column that maps each row back to the
     *           table they came from by it's table key, so that clients can map rows back into the
     *           tree structure.
     *
     ...
     */
    void addToSnapshot(boolean usePrev, Table table, Object tableKey, TableMap tableMap,
        Index snapshotIndex) {
        Assert.leq(copied + snapshotIndex.size(), "dataOffset + snapshotIndex.size()",
            actualViewportSize, "viewport size");
     ...
     */
    private void addToSnapshotNormal(boolean usePrev, Table table, TableMap tableMap,
        Index snapshotIndex) {
        for (int ii = 0; ii < data.size(); ii++) {
            if (!columns.get(ii)) {
                continue;
            ...
            columnCopiers[ii].copy(usePrev, cs, snapshotIndex.iterator(), data.get(ii).second,
                copied, table, tableMap, childPresenceColumn);
        }
    }

    /**
     * Copy the data from the table in question, assuming that the table is a constituent table.
     * This means that we need to copy the data to the alternate data set because column types may
     * be reused or changed.
     */
    private void addToSnapshotConstituent(boolean usePrev, Table table, TableMap tableMap,
        Index snapshotIndex) {
        includesLeafData = true;
        includedConstituentColumns.forEach(cn -> {
            final ColumnSource<?> cs = table.getColumnSource(cn);
            constituentCopiers.get(cn).copy(usePrev, cs, snapshotIndex.iterator(),
                constituentData.get(cn).getSecond(), copied, table, tableMap, childPresenceColumn);
        });
        ...
        for (int ii = 0; ii < data.size(); ii++) {
            ...
            if (columns.get(ii) &&
                ...
                // In the case of constituent rows, because column types can change, we may have
                // omitted formatting columns
                // so we'll allow those to not exist and be null-filled
                final ColumnSource<?> cs = columnSourceMap.get(columnName);
                if (cs != null) {
                    columnCopiers[ii].copy(usePrev, cs, snapshotIndex.iterator(),
                        data.get(ii).second, copied, table, tableMap, childPresenceColumn);
    ...
    /**
     * Allocate a type-correct matrix to store the resultant snapshot into. It will create Columns+1
     * columns to allow for the extra table-mapping column.
     *
    ...
    private boolean ensureSpace(int requestedViewportSize, TableDetails rootData) {
        if (requestedViewportSize == actualViewportSize) {
            // Since viewports will always be exactly sized, we don't need to iterate the arrays and
            // null values out. Copiers will
            // always overwrite.
            return false;
        }
        ...
        if (includeConstituents) {
            // TODO: With a little bit of creativity we can re-use the existing column array if the
            // original and rolldup
            // TODO: column types are the same. I will do this in a second pass, after this one
            // works.
            final Table originalTable = baseTable.getSourceTable();
            constituentData.clear();
            originalTable.getDefinition().getColumnList()
                .forEach(col -> constituentData.put(col.getName(),
                    makeData(col, requestedViewportSize)));
        }
    ...
    /**
     * Create a Pair containing the name and appropriately typed and sized array store for a
     * particular column.
     *
    ...
    private Copier makeChildCopier(Class type) {
        if (type == DBDateTime.class) {
            return (usePrev, columnSource, it, target, offset, table, tableMap,
                childPresenceColumn) -> {
                Assert.neqNull(tableMap, "Child table map");
                while (it.hasNext()) {
                    final long next = it.nextLong();
                    final DBDateTime keyVal = (DBDateTime) (usePrev ? columnSource.getPrev(next)
                        : columnSource.get(next));
            ...
            return (usePrev, columnSource, it, target, offset, table, tableMap,
                ...
                while (it.hasNext()) {
                    final long next = it.nextLong();
                    final long keyVal =
                        usePrev ? columnSource.getPrevLong(next) : columnSource.getLong(next);
                    final Table child = tableMap.get(keyVal);
            ...
            return (usePrev, columnSource, it, target, offset, table, tableMap,
                ...
                while (it.hasNext()) {
                    final long next = it.nextLong();
                    final int keyVal =
                        usePrev ? columnSource.getPrevInt(next) : columnSource.getInt(next);
                    final Table child = tableMap.get(keyVal);
            ...
            return (usePrev, columnSource, it, target, offset, table, tableMap,
                ...
                while (it.hasNext()) {
                    final long next = it.nextLong();
                    final byte keyVal =
                        usePrev ? columnSource.getPrevByte(next) : columnSource.getByte(next);
                    final Table child = tableMap.get(keyVal);
            ...
            return (usePrev, columnSource, it, target, offset, table, tableMap,
                ...
                while (it.hasNext()) {
                    final long next = it.nextLong();
                    final char keyVal =
                        usePrev ? columnSource.getPrevChar(next) : columnSource.getChar(next);
                    final Table child = tableMap.get(keyVal);
    ...
    private Copier makeCopier(Class type) {
        if (type == DBDateTime.class) {
            return (usePrev, columnSource, it, target, offset, table, tableMap,
                ...
                while (it.hasNext()) {
                    final long next = it.nextLong();
                    final DBDateTime dbDateTime = (DBDateTime) (usePrev ? columnSource.getPrev(next)
                        : columnSource.get(next));
                    ((long[]) target)[offset++] =
            ...
            return (usePrev, columnSource, it, target, offset, table, tableMap,
                ...
                while (it.hasNext()) {
                    final long next = it.nextLong();
                    ((byte[]) target)[offset++] = BooleanUtils.booleanAsByte(
                        (Boolean) (usePrev ? columnSource.getPrev(next) : columnSource.get(next)));
                }
            };
        } else if (type == long.class || type == Long.class) {
            return (usePrev, columnSource, it, target, offset, table, tableMap,
        ...
        return (usePrev, columnSource, it, target, offset, table, tableMap,
            ...
            while (it.hasNext()) {
                final long next = it.nextLong();
                ((Object[]) target)[offset++] =
                    usePrev ? columnSource.getPrev(next) : columnSource.get(next);
            }
        };
    }
    ...
    /**
     * Get any required alternate data columns. This is happens when a rollup is showing constituent
     * rows and those rows are part of the viewport.
     *
     */
