...
class RollupSnapshotImpl<CLIENT_TYPE extends TreeTableClientTableManager.Client<CLIENT_TYPE>> extends AbstractTreeSnapshotImpl<RollupInfo, CLIENT_TYPE> {
    ...
    private HierarchicalTable applyFilters(@NotNull HierarchicalTable table) {
        ...
        return (HierarchicalTable)source.where(filters).rollup(info.factory, info.getLeafType() == RollupInfo.LeafType.Constituent, info.getSelectColumns());
    }
    ...
    Table applySorts(@NotNull Table table) {
        ...
        } else return super.applySorts(table);
    }
    ...
    TableMap getTableMap(Table t) {
        return Require.neqNull((TableMap)t.getAttribute(Table.HIERARCHICAL_CHILDREN_TABLE_MAP_ATTRIBUTE), "Child Table Map");
    }
