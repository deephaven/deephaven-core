...
/**
 * Downsamples a table assuming its contents will be rendered in a run chart, with the each
 * subsequent row holding a later X value (i.e. is sorted on that column). Multiple Y columns can be
 * specified, as can a range of values for the X value (providing a "zoom" feature, with high
 * resolution data in that range, and low resolution data outside of it).
 */
/*
 * TODO Remaining work to make this class more efficient. This work can be done incrementally as we
 * find we need specific cases to be faster, but at the time of writing, this is "fast enough" for
 * updating, appending tables with 10m+ rows in them to look good in the web UI: o switching
 * downsample<->passthrough is very untested, likely buggy (PRESENTLY DISABLED) o support automatic
 * re-ranging, due to too many items being added/removed o read MCS on updates to decide whether or
 * not to even check for changes o handle non-QueryTable instances o make shifting more efficient o
...
public class RunChartDownsample implements Function.Unary<Table, Table> {
    ...
    public static final int CHUNK_SIZE =
        Configuration.getInstance().getIntegerWithDefault("chunkSize", 1 << 14);

    /** Enable this to add additional checks at runtime. */
    private static final boolean VALIDATE = Configuration.getInstance()
        .getBooleanForClassWithDefault(RunChartDownsample.class, "validate", false);
    private static final String BUCKET_SIZES_KEY =
        RunChartDownsample.class.getSimpleName() + ".bucketsizes";
    /**
     * Specifies the bucket sizes to round up to when a client specifies some number of pixels. If
     * empty, each user will get exactly the size output table that they asked for, but this likely
     * will not be memoized
     */
    private static final int[] BUCKET_SIZES =
        Configuration.getInstance().hasProperty(BUCKET_SIZES_KEY)
    ...
    public RunChartDownsample(final int pxCount, @Nullable final long[] zoomRange,
        final String xColumnName, final String[] yColumnNames) {
    ...
    public Table call(final Table wholeTable) {
        ...
        if (wholeTable instanceof QueryTable) {
            ...
            return QueryPerformanceRecorder
                .withNugget(
                    "downsample(" + minBins + ", " + xColumnName + " {"
                        + Arrays.toString(yColumnNames) + "})",
                    wholeQueryTable.sizeForInstrumentation(), () -> {
                        final DownsampleKey memoKey =
                            new DownsampleKey(minBins, xColumnName, yColumnNames, zoomRange);
        ...
        // final Index indexToUse = usePrev ? baseTable.getIndex().getPrevIndex() :
        // baseTable.getIndex();
        //
        // // process existing rows
        // handleAdded(indexToUse, columnSourceToBin, getNanosPerPx(minBins, usePrev, indexToUse,
        // columnSourceToBin), valueColumnSource, states, usePrev);
        //
        // // construct the initial index, table
        // //TODO copy def, columns that we actually need here
        // QueryTable resultTable = new QueryTable(buildIndexFromGroups(states),
        // baseTable.getColumnSourceMap());
        ...
        throw new IllegalArgumentException(
            "Can't downsample table of type " + wholeTable.getClass());
    }

    private Table makeDownsampledQueryTable(final QueryTable wholeQueryTable,
        final DownsampleKey memoKey) {
        final ShiftAwareSwapListener swapListener =
        ...
        wholeQueryTable.initializeWithSnapshot("downsample", swapListener,
            (prevRequested, beforeClock) -> {
                final boolean usePrev = prevRequested && wholeQueryTable.isRefreshing();

                final DownsamplerListener downsampleListener =
                    DownsamplerListener.of(wholeQueryTable, memoKey);
                downsampleListener.init(usePrev);
                result.setValue(downsampleListener.resultTable);

                if (swapListener != null) {
                    swapListener.setListenerAndResult(downsampleListener,
                        downsampleListener.resultTable);
    ...
    private final static class DownsampleKey extends MemoizedOperationKey {
        ...
        public boolean equals(final Object o) {
            ...
            if (!Arrays.equals(yColumnNames, that.yColumnNames))
                return false;
            return zoomRange != null ? Arrays.equals(zoomRange, that.zoomRange)
                : that.zoomRange == null;
        }
    ...
    private static class DownsamplerListener extends BaseTable.ShiftAwareListenerImpl {
        ...
        public static DownsamplerListener of(final QueryTable sourceTable,
        ...
        private DownsamplerListener(final QueryTable sourceTable, final QueryTable resultTable,
            ...
            if (xSource.getType() == DBDateTime.class) {
                ...
                throw new IllegalArgumentException("Cannot use non-DBDateTime, non-long x column "
                    + key.xColumnName + " in downsample");
            }
            ...
            // pre-size the array sources, indicate that these indexes are available as bucketstates
            // are created
        ...
        public void onUpdate(final Update upstream) {
            try (final DownsampleChunkContext context =
                ...
                // TODO Complete this so we can switch modes. In the meantime, this is wrapped in
                // if(false)
                // so that if other changes happen in the APIs this uses, local code won't break.
                // When
                ...
                if (VALIDATE) {
                    ...
                    states.values()
                        .forEach(state -> state.validate(false, context, allYColumnIndexes));
                    if (!index.subsetOf(sourceTable.getIndex())) {
                        throw new IllegalStateException(
                            "index.subsetOf(sourceTable.getIndex()) is false, extra items= "
        ...
        protected void maybeSwitchModes(Update upstream, DownsampleChunkContext context) {
            ...
            if (indexMode == IndexMode.PASSTHROUGH
                ...
                log.info().append("Switching from PASSTHROUGH to DOWNSAMPLE ")
                    .append(sourceTable.size()).append(key.toString()).endl();
                // If there are more than 4x items in the source table as there are PX to draw,
                // convert to downsampled
                ...
                log.info().append("Switching from DOWNSAMPLE to PASSTHROUGH ")
                    .append(sourceTable.size()).append(key.toString()).endl();
                // if the table has shrunk until there are less items in the table than there are Px
                // to draw, just show the items
                ...
                // notify downstream tables that the index changed, add all missing rows from the
                // source table, since we're un-downsampling
                // TODO
                final Update switchToPassThrough = new Update(addToResultTable, removed, modified,
                ...
                if (rangeMode == RangeMode.ZOOM) {
                    ...
                    // // Decide if it is time to re-bucket. As above, we want this to infrequently
                    // done, but we also don't
                    // // want to bump into the next bucket size up or down. Right now, the bucket
                    // count should start at
                    // // 110% of the requested PX size, and can shrink to 100%, or grow to up to
                    // 150%.
                    // // We rebucket _before_ getting any work done, since otherwise we would
                    // bucket twice in this one pass.
                    // // This does seem a bit silly, but we're assuming that if the last change
                    // pushed us over the edge so
                    // // that we need to rebucket, it still makes sense to do so. With that said,
                    // we don't use the last
        ...
        public void init(final boolean usePrev) {
            ...
            // log.info().append("initial downsample
            // index.size()=").append(initialIndex.size()).append(",
        ...
        private void rerange(final boolean usePrev) {
            // read the first and last value in the source table, and work out our new nanosPerPx
            // value
            final long first;
            final long last;
            if (rangeMode == RangeMode.ZOOM) {
        ...
        private void handleAdded(final DownsampleChunkContext context, final boolean usePrev,
            ...
            while (it.hasMore()) {
                ...
                final Chunk<? extends Attributes.Values>[] valueChunks =
                    context.getYValues(all, next, usePrev);
        ...
        private void handleModified(final DownsampleChunkContext context, final Index modified,
            ...
            while (it.hasMore()) {
                ...
                final LongChunk<Attributes.OrderedKeyIndices> keyChunk = next.asKeyIndicesChunk();
                final Chunk<? extends Attributes.Values>[] valueChunks =
                    context.getYValues(yColIndexes, next, false);

                final long lastBin = 0;
                BucketState bucket = null;
                for (int indexInChunk = 0; indexInChunk < oldDateChunk.size(); indexInChunk++) {
                    final long bin =
                        LongNumericPrimitives.lowerBin(oldDateChunk.get(indexInChunk), nanosPerPx);
                    final long newBin =
                        LongNumericPrimitives.lowerBin(newDateChunk.get(indexInChunk), nanosPerPx);
        ...
        private void performRescans(final DownsampleChunkContext context) {
            ...
            for (final Iterator<BucketState> iterator = states.values().iterator(); iterator
                .hasNext();) {
        ...
        /**
         * Indicates that a change has probably happened and we should notify the result table. The
         * contents of the change will be our state map (i.e. there is
         * 
         * @param upstream the change that happened upstream
         * @param lastIndex the base index to use when considering what items to tell the result
         *        table changed. if this.index, then update it normally, otherwise this.index must
         *        be empty and this.index should be populated.
         */
        private void notifyResultTable(final Update upstream, final Index lastIndex) {
            ...
            // log.info().append("After downsample update,
            // index.size=").append(index.size()).append(", index=").endl();//.append(index).endl();

            final Update update =
                new Update(added, removed, modified, upstream.shifted, upstream.modifiedColumnSet);
        ...
        private Index indexFromStates() {
            ...
            if (rangeMode == RangeMode.ZOOM) {
                return Stream.concat(
                    Stream.of(head, tail).filter(s -> !s.getIndex().empty()), // note: we only
                                                                              // filter these two,
                                                                              // since states
                                                                              // shouldn't contain
                                                                              // empty indexes
