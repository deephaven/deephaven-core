...
public class DownsampleChunkContext implements AutoCloseable {
     ...
     * @param yColumnSources any Y value column source which may be used. Indexes into this list are used when specifying
     *                       columns which are used later
     * @param chunkSize the size of chunks to specify when actually creating any GetContext
     */
    DownsampleChunkContext(final ColumnSource<Long> xColumnSource, final List<ColumnSource<?>> yColumnSources, final int chunkSize) {
    ...
    public LongChunk<Attributes.Values> getXValues(final OrderedKeys keys, final boolean usePrev) {
        //noinspection unchecked
        return (LongChunk<Attributes.Values>) (usePrev ? xColumnSource.getPrevChunk(xContext, keys) : xColumnSource.getChunk(xContext, keys));
    }
     ...
     */
    public Chunk<? extends Attributes.Values>[] getYValues(final int[] yCols, final OrderedKeys keys, final boolean usePrev) {
     ...
     */
    public Chunk<? extends Attributes.Values> getYValues(final int yColIndex, final OrderedKeys keys, final boolean usePrev) {
        final ColumnSource<?> columnSource = yColumnSources.get(yColIndex);
        final ChunkSource.GetContext getContext = yContexts[yColIndex];
