...
/**
 * Provides chunks for given sources so that downsampling can walk several columns at once, allowing
 * GetContext instances to be lazily created as needed, and all tracked together so they can all be
 * closed with a single call.
 */
public class DownsampleChunkContext implements AutoCloseable {
    ...
    /**
     * Creates an object to track the contexts to be used to read data from an upstream table for a
     * given operation
     * 
     * @param xColumnSource the X column source, always a long column source, currently
     *        reinterpreted from DBDateTime
     * @param yColumnSources any Y value column source which may be used. Indexes into this list are
     *        used when specifying columns which are used later
     * @param chunkSize the size of chunks to specify when actually creating any GetContext
     */
    DownsampleChunkContext(final ColumnSource<Long> xColumnSource,
        final List<ColumnSource<?>> yColumnSources, final int chunkSize) {
    ...
    /**
     * Indicates that any of these Y columns will actually be used, and should be pre-populated if
     * not yet present
     * 
    ...
    /**
     * Requests an array of chunks from the given Y column sources, using the internally tracked
     * GetContexts.
     *
     * This assumes that addYColumnsOfInterest has been called on at least the columns indicated in
     * yCols.
     *
     ...
     * @param usePrev whether or not previous values should be fetched
     * @return an array containing the data in the specified rows. The array will be the same size
     *         as the original yColumnSources, with only the indexes in yCols populated.
     */
    public Chunk<? extends Attributes.Values>[] getYValues(final int[] yCols,
        final OrderedKeys keys, final boolean usePrev) {
    ...
    /**
     * Requests a chunk of data from the specified Y column source, using the internally tracked
     * GetContexts.
     * 
     ...
     */
    public Chunk<? extends Attributes.Values> getYValues(final int yColIndex,
        final OrderedKeys keys, final boolean usePrev) {
        final ColumnSource<?> columnSource = yColumnSources.get(yColIndex);
        final ChunkSource.GetContext getContext = yContexts[yColIndex];
        Assert.neqNull(getContext, "yContexts.get(yColIndex)");

        return usePrev ? columnSource.getPrevChunk(getContext, keys)
            : columnSource.getChunk(getContext, keys);
    }
