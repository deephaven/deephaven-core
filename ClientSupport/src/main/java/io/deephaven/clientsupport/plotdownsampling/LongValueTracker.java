...
public final class LongValueTracker extends ValueTracker {
    ...
    private long minValue(int offset) {
        return source.getUnsafe(minValuePosition(offset));
    }
    private long maxValue(int offset) {
        return source.getUnsafe(maxValuePosition(offset));
    }

    private void setMinValue(int offset, long value) {
        source.set(minValuePosition(offset), value);
    }
    private void setMaxValue(int offset, long value) {
    ...
    public void append(int offset, long rowKey, Chunk<? extends Values> valuesChunk, int indexInChunk, @Nullable WritableRowSet nulls) {
        final long val = valuesChunk.asLongChunk().get(indexInChunk);
        if (val == NULL_LONG) {
            if (nulls != null) {
                nulls.insert(rowKey);
            }
    ...
    public void update(int offset, long rowKey, Chunk<? extends Values> valuesChunk, int indexInChunk, @Nullable WritableRowSet nulls) {
        long val = valuesChunk.asLongChunk().get(indexInChunk);
        if (val == NULL_LONG) {
            if (nulls != null) {
                nulls.insert(rowKey);
            }
    ...
    public String toString(int offset) {
        return "LongValueTracker("+offset+") { max=" + maxValue(offset) + ", min=" + minValue(offset) + " }";
    }
}
