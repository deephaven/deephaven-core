...
public class TreeSnapshotQueryTest extends QueryTableTestBase {
    ...
    /** Since we're not using a remote client we need to provide TSQ with a way to map Table Ids to tables
     * so we'll just assign them and pass this thing in as the ExportedObjectClient
     */
    ...
    private class TTState {
        ...
        TTState(Table theTree) {
            ...
            final HierarchicalTableInfo info = (HierarchicalTableInfo) theTree.getAttribute(Table.HIERARCHICAL_SOURCE_INFO_ATTRIBUTE);
        ...
        void applyTsq(BitSet columns, long start, long end, SelectFilter[] filters, List<SortDirective> sorts) {
            ...
            final TreeSnapshotQuery<?> tsq = new TreeSnapshotQuery(getIdForTable(theTree), makeDetailsMap(expansionMap.values()), start, end, columns, filters, sorts, mockClient, ops);
            ...
            if(result.getConstituentData() != null && result.getConstituentData().length > 0) {
                Arrays.stream(result.getConstituentData())
                        .forEach(p -> constituentSources.put(p.getFirst(), makeConstituentColumnSource(p.getFirst(), p.getSecond())));
            }

            expansionMap = result.getTableData().stream().collect(Collectors.toMap(TableDetails::getKey, Function.identity()));
            ops.clear();
        }
        ...
        void updateTableIds() {
            expansionMap = expansionMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, td -> new TableDetails(td.getValue().getKey(), td.getValue().getChildren())));
        }

        TableMap getTableMap(Table table) {
            return (TableMap) (rollup ? table : forComparisons != null ? forComparisons : theTree).getAttribute(Table.HIERARCHICAL_CHILDREN_TABLE_MAP_ATTRIBUTE);
        }
    }
    ...
    public void testSortandFilter() throws IOException {
        ...
        testViewportAgainst(filterThenTree, state, 0, 30, allColumns, Collections.emptyList(), SelectFilterFactory.getExpressions("!isNull(Website) && Website.contains(`ny`)"), true);
        ...
        testViewportAgainst(filterAndSortThenTree, state, 37, 63, allColumns, directives, SelectFilterFactory.getExpressions("!isNull(Website) && Website.contains(`ny`)"), true);
    ...
    public void testRollupTsq() {
        ...
        testViewportAgainst(filtered, state, 0, 7, allColumns, Collections.emptyList(), SelectFilterFactory.getExpressions("Bagel"), true);
        ...
        testViewportAgainst(rollup, state, 0, 14, allColumns, directives, SelectFilter.ZERO_LENGTH_SELECT_FILTER_ARRAY,
                ct -> {
                    final Table sortTarget = ct instanceof HierarchicalTable ? ((HierarchicalTable)ct).getRawRootTable() : ct;
    ...
    public void testGetPrev() throws Exception {
        ...
        LiveTableMonitor.DEFAULT.runWithinUnitTestCycle(() -> {
            ...
            final Runnable awaitFlushJob = LiveTableMonitor.DEFAULT.flushAllNormalNotificationsForUnitTests(done::get, 60_000L);
    ...
    private static void testViewport(TTState state, long start, long end, BitSet columns, boolean showAfter) {
        testViewportAgainst(state.theTree, state, start, end, columns, Collections.emptyList(), SelectFilter.ZERO_LENGTH_SELECT_FILTER_ARRAY, showAfter);
    }

    private static void testViewportAgainst(Table against, TTState state, long start, long end, BitSet columns, List<SortDirective> sorts, SelectFilter[] filters, boolean showAfter) {
        testViewportAgainst(against, state, start, end, columns, sorts, filters, Function.identity(), showAfter);
    }

    private static void testViewportAgainst(Table against, TTState state, long start, long end, BitSet columns, List<SortDirective> sorts, SelectFilter[] filters, Function<Table, Table> childMutator, boolean showAfter) {
    ...
    private static void checkSnapshotAgainst(TTState state, Table against, long start, long end, Function<Table, Table> childMutator) {
    ...
    private static void checkSnapshotAgainst(TTState state, Table currentTable, Object tableKey, long start, long end, TraversalState ts, Function<Table, Table> childMutator) {
        final boolean usePrev = LogicalClock.DEFAULT.currentState() == LogicalClock.State.Updating
                && ((NotificationStepSource) state.theTree.getSourceTable()).getLastNotificationStep() != LogicalClock.DEFAULT.currentStep();
        ...
        for(int rowNo = 0; rowNo < currentTable.size(); rowNo++) {
            ...
            if(ts.traversed >= start && ts.traversed <= end) {
                final Object[] record = state.snapshot.getRecord(ts.consumed, currentTable.getDefinition().getColumnNamesArray());
                ...
                if(constituentData != null && currentTable.hasAttribute(Table.ROLLUP_LEAF_ATTRIBUTE)) {
                    for(int i = 0; i < constituentData.length; i++) {
                        final int colIndex = currentTable.getDefinition().getColumnNames().indexOf(constituentData[i].first);
                ...
                assertEquals(childKey != null && childMap.get(childKey) != null && !childMap.get(childKey).isEmpty(), state.snapshot.getColumnSource(CHILD_PRESENCE_COLUMN).get(ts.consumed));
            ...
            if(childKey != null) {
                ...
                if(childDetails != null) {
                    checkSnapshotAgainst(state, childMutator.apply(childMap.get(childKey)), childKey, start, end, ts, childMutator);
                }
            }
        }
    }
