...
ps_lines = subprocess.run(['ps', '-ahxww', '-o', 'pid,command' ], stdout=subprocess.PIPE).stdout.decode('utf-8').splitlines()
...
for ps_line in ps_lines:
    ...
    for name, regex in proc_specs.items():
        if re.search(regex, cmd) is not None:
            ...
            if prev is not None:
                print(sys.argv[0] + f": found more than one match for '{name}': {prev}, {pid}, aborting", file=sys.stderr)
                sys.exit(1)
            matches[name] = pid

for name in proc_specs.keys():
    if matches.get(name, None) is None:
        print(sys.argv[0] + f": couldn't find a match for {name}, aborting", file=sys.stderr)
        sys.exit(1)
