...
public class DhDemoServer implements QuarkusApplication {
    ...
    public static void main(String ... args) throws IOException, InterruptedException, ApiException, KubectlException {
    ...
    public int run(final String... args) throws Exception {
        ...
        router.get("/health").handler(rc -> {
            System.out.println("Health check! " + rc.request().uri() + " : " +
                    rc.request().headers().get("User-Agent") + " ( " + rc.request().headers().get("host") + " ) "
            );
        ...
        router.get().handler(req -> {
            ...
            vertx.setTimer(1, t->{
                ...
                long deadline = System.currentTimeMillis() + 180_000; // three minutes... wayyy too long, really.
                while (!worker.checkReady(state.getHttpClient())) {
                    // TODO: instead of this, send a response so client can give feeback while this happens
                    System.out.println("Waiting for " + worker.getUserName() + " (" + worker.getPodName() + ") to be ready");
                    try {
                        Thread.sleep(1000);
                    ...
                    if (System.currentTimeMillis() > deadline) {
                        vertx.runOnContext(()->{
                            req.response()
                                    .setStatusCode(503)
                                    .end("Waited more than three minutes for " + worker.getUserName() + " to be ready");
                        });
                        return;
                    }
                }
                ...
                vertx.runOnContext(()->{
                    req.response()
                            .putHeader("Set-Cookie", "dh-user=" + worker.getUserName() + "; Secure; HttpOnly; Domain=" + DOMAIN)
                            .putHeader("Location", uri)
                            .setStatusCode(302)
                            .end();
                });
    ...
    public static String createIsolatedSession() throws ApiException, KubectlException {
        ...
        try {
            ...
            throw new Error("You likely have an expired auth session from kubeconfig;\n" +
                    ...
                    "`kubectl get pods` will suffice. The java KubeTools bail trying to call refresh() on stale auth tokens", e);
        }
        ...
        // ok, now that we have a labeled pod, wait until our services is ready so we can add a route!
        while (serviceReady[0] == null) {
            synchronized (serviceReady) {
                ...
                if (failure[0] != null) {
                    throw new RuntimeException("Failed to prepare service for " + userName, failure[0]);
                }
            }
        }
        ...
        // maybe send back myPod.metadata.name as well? ...guess that _shouldn't_ be useful to calling code
        return userName;
//
...
//        final V1ServiceList result = api.listNamespacedService("default", "true", false, conti, fieldSel, labelSel, 100, null, null, 10, false);
...
//        final V1PodList pods = api.listNamespacedPod(ns, pretty, false, conti, fieldSel, labelSel, 100, null, null, 10, false);
//        if (pods.getItems().isEmpty()) {
//            // this will never happen in the running cluster,
//            // as this controller will be finding it's own pod.
//            throw new IllegalStateException("No pods found with dh.purpose=controller; make sure the system is healthy!");
//        }
//
...
//                final KubectlGet<V1Pod>.KubectlGetSingle podGetSingle = podGet.namespace(ns).name(userName);
//                try {
//
//                    final V1Pod foundPod = podGetSingle.execute();
//                    System.out.println("The pod: " + (foundPod == null ? "is null" : "has status: " + foundPod.getStatus()));
//                    //System.out.println(foundPod.stream().map(new Function<V1Pod, String>(){ public String apply(V1Pod p)  { return p.getMetadata().getNamespace() + " : " + p.getMetadata().getName();}}).collect(Collectors.joining("\n")));
...
//                // (though, we should probably tail the controller health check logs instead, they are boring atm)
...
//        }
    ...
    private static V1Service prepareService(final CoreV1Api api, final String userName) throws ApiException {
    ...
    private static V1Pod claimPod(final CoreV1Api api, final String podName, final int tries) throws ApiException, KubectlException {
        V1PodList freePods = findFreePods(api);
        for (V1Pod pod :
            freePods.getItems().stream().sorted((a, b)-> {
                ...
                return a.getMetadata().getCreationTimestamp().compareTo(b.getMetadata().getCreationTimestamp());
            ...
            if (user == null) {
                ...
                System.out.println("Someone else (" + user + ") won " + pod.getMetadata().getName() +" over us (" +
                        podName + ")");
            }
        }
        if (tries > 5) {
    ...
    private static boolean tryClaim(final ApiClient apiClient, final V1Pod pod, final String userName) throws KubectlException {
        ...
        if (userName.equals(realLabel)) {
            System.out.println(NameGen.getMyName() + " won pod " + realName +" for user " + userName);
            return true;
        }
    ...
    private static void scaleUp(final ApiClient apiClient) {
        final KubectlGet<V1Deployment>.KubectlGetSingle deployment = Kubectl.get(V1Deployment.class).name(NAME_DEPLOYMENT);
        ...
        try {
            ...
            final List<V1Pod> podList = Kubectl.get(V1Pod.class)
                    ...
                    .options(countOpts)
                    .execute();
            System.out.println("There are " + podList.size() + " pods for " + replicas + " replicas");
            ...
            if (shouldSkipScaleup(apiClient, podList)) {
                System.out.println("Too many scale ups in too short of a period; skipping scale up request");
                return;
            }
...
//        }
    ...
    private static V1Pod findChosenOne(final CoreV1Api api, final HttpServerRequest request) throws ApiException {
        ...
        V1PodList list = api.listNamespacedPod(DH_NAMESPACE, null, null, null, "metadata.name=dh-local", "run=dh-local", null, null, null, 10, false);
        findTheOne:
        for (V1Pod item : list.getItems()) {
            ...
            for (Map.Entry<String, String> label : labels.entrySet()) {
                patchLabel.append(indent).append(label.getKey()).append(": \"").append(label.getValue()).append("\"\n");
            }
    ...
    private static int countMachines(final CoreV1Api api) throws ApiException {
        final V1PodList results = api.listNamespacedPod(DH_NAMESPACE, null, null, null, "metadata.name=dh-local", null, 10_000, null, null,2, null);
...
//        final V1Deployment result = patchDeploy.execute();
    ...
    private static V1NamespaceList getNs(final CoreV1Api api) throws ApiException {
        return api.listNamespace(null, null, null, "metadata.name=" + DH_NAMESPACE, null, null, null, null,2, null);
    }
