...
import io.deephaven.demo.control.ClusterController;
...
public class ImageDeployer {
    ...
    private void deploySingleWorker(final String workerName) throws IOException, InterruptedException {
        ...
        boolean rebuildWorker = "true".equals(System.getProperty("rebuildWorker"));
        boolean needScriptRun = false;
        if (rebuildWorker) {
            manager.deleteMachine(workerName);
        } else {
            // if we're not rebuilding, and the target machine already exists,
            // we'll need to know whether we should explicitly run our start script.
            needScriptRun = manager.hasMachine(workerName);
            if (needScriptRun) {
