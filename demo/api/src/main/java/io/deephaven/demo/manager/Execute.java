...
public class Execute {
    ...
    public static ThreadLocal<Boolean> quietMode = new ThreadLocal<>();
    ...
    public static ExecutionResult executeNoFail(String cmd, Map<String, String> env, File workDir, File saveTo, Consumer<CharSequence> logOut, Consumer<CharSequence> logErr) throws IOException, InterruptedException {
        if (env == null) {
            env = new HashMap<>();
        }
    ...
    public static ExecutionResult bashQuiet(String name, String code) throws IOException, InterruptedException {
        return bash(name, code, new HashMap<>(),null, null, DO_NOTHING, DO_NOTHING);
    }

    public static ExecutionResult bash(String name, String code, Map<String, String> env, File workDir, File saveTo, Consumer<CharSequence> logOut,Consumer<CharSequence>logErr) throws IOException, InterruptedException {
        return execute(Arrays.asList("bash", "-c", code.startsWith("function ") ? code : "function " + name + "() {\n"+
code +
"\n} ; " + name),env, workDir, saveTo, logOut, logErr);
    }

    public static ExecutionResult executeNoFail(String ... cmd) throws IOException, InterruptedException {
        return executeNoFail(Arrays.asList(cmd));
    }
    public static ExecutionResult executeNoFail(List<String> cmd) throws IOException, InterruptedException {
        return executeNoFail(cmd, new HashMap<>());
    }
    public static ExecutionResult executeNoFail(List<String> cmd, Map<String, String> env) throws IOException, InterruptedException {
        return executeNoFail(cmd, env, null, null, null, null);
    }
    public static ExecutionResult executeNoFail(List<String> cmd, Map<String, String> env, File workDir, File saveTo, Consumer<CharSequence> logOut,Consumer<CharSequence>logErr) throws IOException, InterruptedException {
        ExecutionResult result = execute(cmd, env, workDir, saveTo, logOut, logErr);
        if (result.code != 0) {
...
//                "-o", "LogLevel=DEBUG2",
    ...
    public static ExecutionResult sshQuiet(String machine, boolean allowFail, String... command) throws IOException, InterruptedException {
        if (!machine.contains("@")) {
