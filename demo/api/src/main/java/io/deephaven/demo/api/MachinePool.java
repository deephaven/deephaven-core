...
import static io.deephaven.demo.manager.NameConstants.DOMAIN;
...
public class MachinePool {
    ...
    public Machine createMachine(final IClusterController ctrl, final String name, final boolean multiDomain) {
        DeploymentManager manager = ctrl.getDeploymentManager();
        final IpPool ips = manager.getIpPool();
        String realIp = null;
        IpMapping oldIp = null;
        if (name != null) {
            // try to figure out the real IP
            realIp = manager.guessRealIp(name);
            LOG.infof("Found realIp %s for machine %s", realIp, name);
            if (realIp != null) {
                oldIp = manager.findIp(realIp);
                LOG.infof("Found ip mapping %s for machine %s", oldIp, name);
            }
        }
        final String newName = name == null || name.isEmpty() ? NameGen.newName() : name;
        final Machine machine = getOrCreate(newName, ctrl, oldIp, realIp);
        machine.setOnline(true);
        if (machine.getIp() == null) {
            LOG.infof("No IP known yet for %s, reserving a new one", machine.getHost());
...
//                final Machine candidate = candidates.remove(0);
    ...
    public Machine getOrCreate(final String name, final IClusterController ctrl, final IpMapping ip, final String realIP) {
    ...
    public void expireInTimeString(final Machine machine, final String lease) {
        final long parsedTime = IClusterController.parseTime(lease, machine.getHost());
