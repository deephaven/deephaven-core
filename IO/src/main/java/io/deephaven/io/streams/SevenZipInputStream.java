...
public class SevenZipInputStream extends InputStream {
     ...
     * <LI> A list of folders, each of which consumes one or more of
     * the packed streams (in order) and produces one output unpacked
     * stream.
     * <LI> A list of substream counts and lengths. Each unpacked
     * stream (in order) is split into one or more substreams.
     * <LI> A list of files. Each file (in order) may or may not
     * consume one substream. (Directories and anit-files do not
     * consume a stream.) Files have things like names, timestamps,
     * attributes, etc.
     * </OL> */
    // 7zItem.h
    ...
    private static class ArchiveDatabaseEx extends ArchiveDatabase {
        ...
        public long getFolderStreamPos(int folderIndex, int indexInFolder) {
            return ArchiveInfo.DataStartPosition+PackStreamStartPositions.get(FolderStartPackStreamIndex.get(folderIndex)+indexInFolder);
        }
    ...
    /**
    A Folder is one compressed chunk of data.
    A folder has one codec and the cyphertext is a small number
    of packed streams (usually one). Since the plaintext is one
    stream, a folder has one CRC. The folder's plaintext stream
    will often be the concatenation of a bunch of files, but the
    Folder knows nothing of this.
    <P>
    A <U>codec</U> (my term) is a small graph of coders.
    A <U>coder</U> does a transform from n input streams to m output streams.
    <U>Bind pairs</U> attach the output stream of one coder to the input stream of another coder.
    A codec always has one (unbound) output stream, but can have many (unboud) input streams.
    The most common codec consists of one coder with one input stream,
    one output stream, and no bind pairs.

    <P>Input streams and output streams are numbered in the order the coders are listed.
    PackStreams is used to map from the (implied) list of packed
    (input) streams in the archive to the input streams of the
    coders.
    */
    // 7zItem.h
    private static class Folder {

        /** List of coders. Input and output stream indices (as
         * referenced by the BindPairs) are defined by the order of
         * this list. */
        List<CoderInfo> Coders=new LinkedList<CoderInfo>();

        /** List of BindPairs. Bind pairs attach the output stream of
         * one coder to the input stream of another coder. Stream
         * indices are defined by the coders list. */
        List<BindPair> BindPairs=new LinkedList<BindPair>();

        /** Map [ packed stream index (in this folder)
         *  -&gt; input stream index (in the list of coders) ] */
        List<Integer> PackStreams=new LinkedList<Integer>();

        /** Lengths of the output streams from the coders. Includes
         * bound and unbound output streams. */
    ...
    private static class MethodID {
        ...
        public boolean equals(Object that) {
            ...
            if (IDSize!=methodID.IDSize) { return false; }
    ...
    private static class InArchive {
        ...
        public void readDatabase(InputStream inputStream, ArchiveDatabaseEx database, InputStreamFactory inputStreamFactory, int nBeginStreamPosition) throws IOException {
            ...
            database.ArchiveInfo.StartPositionAfterHeader=database.ArchiveInfo.StartPosition+VERSION_INFO_LENGTH+START_HEADER_CRC_LENGTH+START_HEADER_LENGTH;
            ...
            while (true) {
                ...
                Reference<Long> startPositionAfterHeaderRef=new Reference<Long>(database.ArchiveInfo.StartPositionAfterHeader);
                Reference<Long> dataStartPosition2Ref=new Reference<Long>(database.ArchiveInfo.DataStartPosition2);
                readAndDecodePackedStreams(byteBuffer, startPositionAfterHeaderRef, dataStartPosition2Ref, dataVector, inputStreamFactory);
        ...
        private void readHeader(ByteBuffer byteBuffer, ArchiveDatabaseEx database, InputStreamFactory inputStreamFactory) throws IOException {
            ...
            if (BlockType.MAIN_STREAMS_INFO==nBlockType) {
                ...
                readStreamsInfo(byteBuffer, dataVector, dataStartPositionRef, database.PackSizes, database.PackCRCs, database.Folders, database.NumUnpackStreamsVector, unPackSizes, digests);
        ...
        private void readAndDecodePackedStreams(ByteBuffer byteBuffer, Reference<Long> baseOffset, Reference<Long> dataOffset, List<ByteBuffer> dataVector, InputStreamFactory inputStreamFactory) throws IOException {
            ...
            for (int i=0; i<folders.size(); i++) {
                ...
                decoder.Decode(inputStreamFactory, dataStartPos,
                    packSizes.subList(packIndex, packSizes.size()), folder, new SequentialOutStreamWrapper(outStream), null);
        ...
        // 7zIn.cpp
        private void readStreamsInfo(ByteBuffer byteBuffer, List<ByteBuffer> alternateByteBuffers, Reference<Long> dataOffsetRef,
            List<Long> packedStreamSizes, List<Integer> packedStreamDigests, List<Folder> folders,
            List<Integer> numUnpackStreamsInFolders, List<Long> unpackedStreamSizes, List<Integer> digests) throws ZipException
        {
            while (true) {
                long  nBlockType=readId(byteBuffer);
                if (BlockType.END==nBlockType) {
        ...
        private void skipData(ByteBuffer byteBuffer, long nBytesToSkip) {
            if (nBytesToSkip>Integer.MAX_VALUE) { nBytesToSkip=Integer.MAX_VALUE; }
    ...
    private static class SequentialOutStreamWrapper extends Reference<OutputStream> implements ISequentialOutStream {
        public SequentialOutStreamWrapper(OutputStream value) { super(value); }
    }
    ...
    private static class SequentialInStreamWrapper extends Reference<InputStream> implements ISequentialInStream {
        public SequentialInStreamWrapper(InputStream value) { super(value); }
    }
    ...
    private static class LzmaWrapper implements ICompressCoder, ICompressSetDecoderProperties2 {
        ...
        public void Code(ISequentialInStream inStream, ISequentialOutStream outStream, long inSize, long outSize, ICompressProgressInfo progress) throws IOException {
            if (false==decoder.Code(((Reference<InputStream>)inStream).getValue(), ((Reference<OutputStream>)outStream).getValue(), outSize)) {
    ...
    private interface ICompressCoder {
        void Code(ISequentialInStream inStream, ISequentialOutStream outStream, long inSize, long outSize, ICompressProgressInfo progress) throws IOException;
    }

    // ICoder.h
    private interface ICompressCoder2 extends ICompressCoder {
        void Code(List<ISequentialInStream> inStreams, List<List<Long>> inSizes, int nInStreams, List<ISequentialOutStream> outStreams, List<List<Long>> outSizes, int nOutStreams, ICompressProgressInfo progress) throws IOException;
    }
            ...
            try {
                ...
                for (i=0; i<numCoders; i++) {
                    ...
                    for (j=0; j<numInStreams; j++, packStreamIndex++) {
                        ...
                        if (bindPairIndex>=0) {
                            packSizesPointers.add(folderInfo.UnpackSizes.subList(folderInfo.BindPairs.get(bindPairIndex).OutIndex, folderInfo.UnpackSizes.size()));
                        } else {
                ...
                //_mixerCoderMTSpec.SetProgressCoderIndex(mainCoder); // set which coder in the graph best represents the actual progress of the entire
    ...
    private static class Handler {
        ...
        public void setDatabase(ArchiveDatabaseEx database) { m_database=database; }

        private InputStreamFactory m_inputStreamFactory;
        public void setInputStreamFactory(InputStreamFactory inputStreamFactory) { m_inputStreamFactory=inputStreamFactory; }
    ...
    //----------------------------------------------------------------
    /** Incremental decompressor based on
     * {@link SevenZip.Compression.LZMA.Decoder}. */
    private static class LzmaIncrementalDecoder {
        ...
        public boolean Code() throws IOException {
            ...
            if (m_outSize<0 || m_nowPos64<m_outSize) {
                ...
                if (m_RangeDecoder.DecodeBit(m_IsMatchDecoders, (m_state<<Base.kNumPosStatesBitsMax)+posState)==0) {
                    LzmaIncrementalDecoder.LiteralDecoder.Decoder2 decoder2=m_LiteralDecoder.GetDecoder((int)m_nowPos64, m_prevByte);
                    ...
                    if (m_RangeDecoder.DecodeBit(m_IsRepDecoders, m_state)==1) {
                        ...
                        if (m_RangeDecoder.DecodeBit(m_IsRepG0Decoders, m_state)==0) {
                            if (m_RangeDecoder.DecodeBit(m_IsRep0LongDecoders, (m_state<<Base.kNumPosStatesBitsMax)+posState)==0) {
                        ...
                        if (posSlot>=Base.kStartPosModelIndex) {
                            ...
                            if (posSlot<Base.kEndPosModelIndex) {
                                ...
                                m_rep0+=(m_RangeDecoder.DecodeDirectBits(numDirectBits-Base.kNumAlignBits)<<Base.kNumAlignBits);
    ...
    private static class LimitedInputStream extends InputStream {
        ...
        enum CloseUnderlyingOnClose { YES, NO }
        private InputStream m_inputStream;
    ...
    private static class ArchiveIterator {
        ...
        public ArchiveIterator(ArchiveDatabaseEx archiveDatabaseEx) {
            ...
            m_nOffsetIntoArchive=archiveDatabaseEx.ArchiveInfo.DataStartPosition+archiveDatabaseEx.ArchiveInfo.StartPositionAfterHeader;
        }
    }
    ...
    public enum Behavior { SKIP_WHEN_NO_STREAM, INCLUDE_WHEN_NO_STREAM }
    ...
    public Entry getNextEntry(Behavior behavior) throws IOException {
        ...
        try {
            ...
            if (0==m_archiveIterator.m_nStreamsRemainingInFolder) {
                ...
                Folder folder;
                do {
                    if (false==m_archiveIterator.m_folderItr.hasNext() || false==m_archiveIterator.m_unpackStreamsForFolderItr.hasNext()) {
                        throw new ZipException("Bad header.");
                    }
                    folder=m_archiveIterator.m_folderItr.next();
                    m_archiveIterator.m_nStreamsRemainingInFolder=m_archiveIterator.m_unpackStreamsForFolderItr.next();
                ...
                m_currentFolderStream=new LzmaDecompressingInputStream(m_inputStreamFactory, nOffsetIntoArchive, nPackedSize, nUnpackedSize, properties);
            }
            ...
            m_currentFileStream=new LimitedInputStream(m_currentFolderStream, fileItem.UnPackSize, LimitedInputStream.CloseUnderlyingOnClose.NO);
            return new Entry(fileItem);
