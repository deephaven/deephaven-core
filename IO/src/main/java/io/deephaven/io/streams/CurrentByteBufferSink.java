...
/**
 * A ByteBufferSink that knows and provides a getter for the last buffer it gave out, to avoid unnecessary state
 * in code that uses the buffer.
 */
public interface CurrentByteBufferSink extends ByteBufferSink {

    /**
     * Access the current buffer for this sink.
     * This is either the initial buffer, or the last one provided by
    ...
    /**
     * Return the current buffer, guaranteed to have sufficient space remaining to append the requested number
     * of bytes. The existing current buffer may be accepted (see {@link ByteBufferSink#acceptBuffer(ByteBuffer, int)})
     * as a side effect of this operation.
     ...
     * Effectively the same as invoking {@link ByteBufferSink#close(ByteBuffer)} with the result of
    ...
    class Adapter implements CurrentByteBufferSink {
        ...
        public ByteBuffer acceptBuffer(@NotNull final ByteBuffer buffer, final int need) throws IOException {
            if (buffer != current) {
                throw new UnsupportedOperationException("Expected current buffer " + current + ", instead tried to accept " + buffer);
            }
        ...
        public void close(@NotNull final ByteBuffer buffer) throws IOException {
            if (buffer != current) {
                throw new UnsupportedOperationException("Expected current buffer " + current + ", instead tried to close with " + buffer);
            }
