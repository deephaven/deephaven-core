...
/**
 * This is an InputStream implementation which reads from a java.nio.ByteBuffer.  If
 * a read operation crosses the end of the buffer, the BufferUnderflowException is
 * converted to an EOFException.
 *
 * The stream contains no state other than that in in the buffer itself, so the buffer
 * can be exchanged at will with the setBuffer() method.
 */
public class ByteBufferInputStream extends java.io.InputStream implements DataInput {
    ...
    /**
     * The DataOutput interface always writes bytes in big-endian order, while ByteBuffer
     * allows the order to be big- or little-endian.  Set this flag true to assume that
     * the buffer is bid-endian, or false to check the buffer's order at each write.
     */
