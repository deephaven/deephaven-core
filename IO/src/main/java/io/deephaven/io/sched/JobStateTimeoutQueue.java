...
class JobStateTimeoutQueue implements Cloneable {
    ...
    boolean testInvariantAux(int i, String what) {
        if ( i <= size ) {
            if ( queue[i].tqPos != i ) {
                log.error().append(what).append(": queue[").append(i).append("].tqPos=").append(queue[i].tqPos).append(" != ").append(i).endl();
            }
            ...
            if ( i > 1 ) {
                if ( queue[i].deadline < queue[i/2].deadline ) {
                    log.error().append(what).append(": child[").append(i).append("]=").append(queue[i].deadline).append(" < parent[").append((i/2)).append("]=").append(queue[i/2].deadline).endl();
                    return false;
                }
            }
        }
    ...
    boolean testInvariant(String what) {
        ...
        if ( result ) {
            for ( int i = size+1; i < queue.length; ++i ) {
                if ( queue[i] != null ) {
                    log.error().append(what).append(": size = ").append(size).append(", child[").append(i).append("]=").append(queue[i].deadline).append(" != null").endl();
                    result = false;
                }
            }
        }
