 ...
 *
 * This class provides a singleton wrapper for scheduling invocations of multiple
 * Job instances from a single thread.  Job are scheduled in accordance with an
 * interest set on a java.nio.Channel, deadline based time scheduling, and/or
 * custom criteria defined by the Jobs' implementation of the ready() method.
 *
 * Jobs are instantiated by the application and made known to the scheduler by
 * one of the installJob() methods.  A previously installed job can be removed
 * from the scheduler with the cancelJob() method.  The installJob() and cancelJob()
 * methods are thread-safe.  It is allowed to call installJob() on a job that
 * is already installed, or cancelJob() on a job that is not current in the
 * scheduler.  In the former case, the channel and/or deadline will be updated
 * accordingly; in the latter, the call will be ignored.
 *
 * Once the job is installed, the scheduler promises to call exactly one of
 * its invoke(), timedOut() or cancelled() methods exactly once.  The invoke() method
 * will be called only if the job was (last) installed with a channel and non-zero
 * interest set.  The timedOut() method can be called for any job, since all jobs
 * have an associated deadline (although the timeout value can be set to Integer.MAX_VALUE
 * to make if effectively infinite).  The cancelled() method is called only if
 * the job is removed by a cancelJob() call before either the channe is ready or the
 * deadline expires.
 *
 * After the job is called back, the scheduler forgets about the job completely,
 * unless the application installs it again.  That is, from the scheduler's point
 * of view *all* jobs are one-shots.  This design is based on the observation that
 * it is easier to reschedule jobs on every invocation in the style of a tail-recursive
 * loop, as opposed to maintaining persistent state in the scheduler.
 *
 * The application must drive the scheduler by calling the work() method in a loop.
 * The work() method is *not* thread-safe; the application must either call it from
 * a single thread or synchronize calls accordingly.
 */
...
//        Assert.eqTrue(isInChangedStates(state), "isInChangedStates(state)"); // temporary
    ...
    public void cancelJob(Job job) {
        synchronized ( stateLock ) {
            if ( log.isDebugEnabled() ) {
                log.debug().append(name).append(" explicitly cancelling ").append(job).append(" in YAScheduler.cancelJob").endl();
            }
            ...
            try {
                if ( key != null && key.isValid() && key.attachment() == state ) {
                    ...
                    if ( key.interestOps() != 0 ) {
                        key.interestOps(0);
                        if ( log.isDebugEnabled() ) {
                            log.debug().append(name).append(" setting interest on orphaned key ").append(key.toString()).append(" to 0").endl();
                        }
                        keyUpdates.sample(1);
                    }
                }
                ...
                // ignore it
                if ( log.isDebugEnabled() ) {
                    log.info().append(name).append(" got CancelledKeyException while dropping channel ").append(state.waitChannel.toString()).endl();
                }
            }
    ...
    private boolean grabChannel(JobState state) {
        try {
            ...
            if ( key == null ) {
                ...
                if ( key.interestOps() != state.nextOps ) {
                    if ( log.isDebugEnabled() ) {
                        log.debug().append(name).append(" update ").append(state.job)
                                .append(": setting interest on key ").append(key.toString()).append(" to ").append(state.nextOps)
                                .endl();
                    }
                    ...
                    if ( log.isDebugEnabled() ) {
                        log.debug().append(name).append(" update ").append(state.job)
                                .append(": interest on key ").append(key.toString()).append(" already at ").append(state.nextOps)
                                .endl();
                    }
                }
            }
        ...
        state.waitChannel = null;
        log.error().append(name).append(" tried to register ").append(state.job).append(" on closed channel ").append(state.nextChannel.toString()).endl();
        return false;
    }
        ...
        for ( int i = 0; i < size; i++ ) {
            ...
            if ( state.gathered ) {
                ...
                if ( !grabChannel(state) ) {
                    log.error().append(name).append(" cancelling ").append(state.job).append(" after failed I/O registration").endl();
    ...
    private long computeTimeout(long now, long timeout) {
        if ( !dispatchQueue.isEmpty() ) {
            ...
            if ( log.isDebugEnabled() ) {
                log.debug().append(name).append(" update: next timeout due in ").append(remain).append(" millis: ").append(next.job).endl();
            }
    ...
    private void select(long timeout) {
        try {
            ...
            if ( java.util.regex.Pattern.matches(".*Operation not permitted.*", x.toString()) ) {
                ...
                // thrown.
                log.warn().append(name).append(" Ignoring 'Operation not permitted' exception, see http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6481709").endl();
            }
            else {
                if (!isClosed()) {
                    log.fatal(x).append(name).append(" Unexpected IOException in select(): ").append(x.getMessage()).endl();
                    System.exit(1);
                }
            }
        }
            ...
            if (!isClosed()) {
                log.fatal(x).append(name).append(" ClosedSelectorException in select(): ").append(x.getMessage()).endl();
                System.exit(1);
            }
        }
    }
    ...
    private void spinSelect(long times) {
        try {
            ...
            if ( java.util.regex.Pattern.matches(".*Operation not permitted.*", x.toString()) ) {
                ...
                if (!isClosed()) {
                    log.fatal(x).append(name).append(" Unexpected IOException in spinSelect(): ").append(x.getMessage()).endl();
                    System.exit(1);
                }
            }
        }
            ...
            if (!isClosed()) {
                log.fatal(x).append(name).append(" ClosedSelectorException in spinSelect(): ").append(x.getMessage()).endl();
                System.exit(1);
            }
        }
    }
    ...
    private void gather(long now) {
            ...
            try {
                if ( (state = (JobState) key.attachment()) == null ) {
                    ...
                    if ( key.isValid() && key.interestOps() != 0 ) {
                        if ( log.isDebugEnabled() ) {
                            log.debug().append(name).append(" clearing interest in orphaned key ").append(key.toString()).append(" in YASchedulerImpl.gather").endl();
                        }
                        if ( key.isValid() ) {
                            key.interestOps(0);
                        }
                    ...
                    timeoutQueue.remove(state);
                    if ( log.isDebugEnabled() ) {
                        log.debug().append(name).append(" gather ").append(key.toString()).append(" -> ").append(state.job)
        ...
        if ( log.isDebugEnabled() ) {
            log.debug().append(name).append(" gathered ").append(numInvokes).append(" for I/O and ").append(numTimeouts).append(" timeouts").endl();
        }
    }
    ...
    private boolean dispatch(Procedure.Nullary handoff) {
        ...
        try {
            if ( cancelled ) {
                ...
                if (doTimingStats) gatheredDuration.sample((System.nanoTime() - state.gatheredNanos + 500) / 1000);
                if ( readyOps != 0 ) {
                    if ( log.isDebugEnabled() ) {
            ...
            log.fatal(x).append(": unhandled Throwable in dispatch on job [").append(state.job).append("]: ").append(x.getMessage()).endl();
     ...
     * This method will form the core of the main loop of a scheduler-driven
     * application.  The method first waits until:
     *
     * -- the given timeout expires,
     * -- the earliest job-specific timeout expires, or
     * -- one or more jobs becomes ready
     *
     * Note that this method is not synchronized.  The application must ensure
     * that it is never called concurrently by more than one thread.
     *
    ...
    public void close() {
        ...
        try {
            ...
            log.warn(x).append(name).append(" Scheduler.close: ignoring exception from selector.close(): ").append(x.getMessage()).endl();
        }
    }
    ...
    private void clear() {
        ...
        log.info().append(name).append(" Scheduler.clear: starting with ").append(allJobs.size()).append(" jobs").endl();
        synchronized( stateLock ) {
            update();
        }
