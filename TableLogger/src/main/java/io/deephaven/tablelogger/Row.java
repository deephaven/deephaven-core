...
public interface Row {
    ...
    default <T> RowSetter<T> getSetter(String name, Class<T> tClass) {
        ...
        if (tClass.isAssignableFrom(setter.getType())
                || (unboxedType = TypeUtils.getUnboxedType(tClass)) != null && unboxedType.isAssignableFrom(setter.getType())) {
            //noinspection unchecked
            return (RowSetter<T>)setter;
        }
        throw new ClassCastException(name + " is of type " + setter.getType() + ", not of type " + tClass + (unboxedType == null ? "" : " or " + unboxedType));
    }
     ...
     *
     * @deprecated {@link Row#size()} is somewhat ambiguously specified in the interface and its implementations.
     * Some implementations keep track of all rows written. Others keep track of number of rows buffered.
     * <p>It seems safer to simply not allow the question to be asked.
     *
