...
test_that("agg_min fails nicely when 'cols' is a bad type", {
  expect_error(
    agg_min(5),
    "'cols' must be passed as a string or a vector of strings. Got an object of class numeric instead."
  )
  expect_error(
    agg_min(TRUE),
    "'cols' must be passed as a string or a vector of strings. Got an object of class logical instead."
  )
})
...
test_that("agg_w_avg fails nicely when 'wcol' is a bad type", {
  expect_error(
    agg_w_avg(5, "string"),
    "'wcol' must be passed as a single string. Got an object of class numeric instead."
  )
  expect_error(
    agg_w_avg(TRUE, "string"),
    "'wcol' must be passed as a single string. Got an object of class logical instead."
  )
  expect_error(
    agg_w_avg(c("Multiple", "strings", "bad"), "string"),
    "'wcol' must be passed as a single string. Got a string vector of length 3 instead."
  )
})
...
test_that("agg_count fails nicely when 'col' is a bad type", {
  expect_error(
    agg_count(5),
    "'col' must be passed as a single string. Got an object of class numeric instead."
  )
  expect_error(
    agg_count(TRUE),
    "'col' must be passed as a single string. Got an object of class logical instead."
  )
  expect_error(
    agg_count(c("Many", "strings")),
    "'col' must be passed as a single string. Got a string vector of length 2 instead."
  )
})
