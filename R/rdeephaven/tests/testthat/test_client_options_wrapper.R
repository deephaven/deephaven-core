...
test_that("using tls with bad input type fails nicely", {
    ...
    client_options <- ClientOptions$new()
    expect_error(client_options$use_tls(12345),
        "'root_certs' must be passed as a single string. Got an object of class numeric instead.")
    expect_error(client_options$use_tls(c("double", "string")),
...
test_that("add_int_option with bad types fails nicely", {
    ...
    client_options <- ClientOptions$new()
    expect_error(client_options$add_int_option(12345, 12345),
        "'opt' must be passed as a single string. Got an object of class numeric instead.")
    expect_error(client_options$add_int_option(c("several", "strings"), 12345),
        "'opt' must be passed as a single string. Got a character vector of length 2 instead.")
    expect_error(client_options$add_int_option("option_key", "blah blah"),
        "'val' must be an integer. Got an object of class character instead.")
    expect_error(client_options$add_int_option("option_key", 12345.6789),
        "'val' must be an integer. Got a non-integer numeric type instead.")
    expect_error(client_options$add_int_option("option_key", c(1, 2, 3, 4, 5)),
...
test_that("add_string_option with bad types fails nicely", {
    ...
    client_options <- ClientOptions$new()
    expect_error(client_options$add_string_option(12345, "option_val"),
        "'opt' must be passed as a single string. Got an object of class numeric instead.")
    expect_error(client_options$add_string_option(c("several", "strings"), "option_val"),
        "'opt' must be passed as a single string. Got a character vector of length 2 instead.")
    expect_error(client_options$add_string_option("option_key", 12345),
        "'val' must be passed as a single string. Got an object of class numeric instead.")
    expect_error(client_options$add_string_option("option_key", c("several", "many", "strings")),
...
test_that("add_extra_header with bad types fails nicely", {
    ...
    client_options <- ClientOptions$new()
    expect_error(client_options$add_extra_header(12345, "header_value"),
        "'header_name' must be passed as a single string. Got an object of class numeric instead.")
    expect_error(client_options$add_extra_header(c("several", "strings"), "header_value"),
        "'header_name' must be passed as a single string. Got a character vector of length 2 instead.")
    expect_error(client_options$add_extra_header("header_name", 12345),
        "'header_value' must be passed as a single string. Got an object of class numeric instead.")
    expect_error(client_options$add_extra_header("header_name", c("several", "many", "strings")),
