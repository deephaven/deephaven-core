...
test_that("dh_to_arrow_table doesn't die on invalid arg types", {

    client <- setup_client(client_args=CLIENT_ARGS)

    expect_error(dh_to_arrow_table(NULL))
    expect_error(dh_to_arrow_table(1))
    expect_error(dh_to_arrow_table("hello!"))

    rm(client) # just in case
})


test_that("dh_to_arrow_table correctly pulls data from the server", {
    ...
    at1 <- dh_to_arrow_table(th1)
    at2 <- dh_to_arrow_table(th2)
    at3 <- dh_to_arrow_table(th3)
    at4 <- dh_to_arrow_table(th4)

    expect_equal(at1, arrow_table(dataframes[[1]]))
    expect_equal(at2, arrow_table(dataframes[[2]]))
    expect_equal(at3, arrow_table(dataframes[[3]]))
    expect_equal(at4, arrow_table(dataframes[[4]]))
