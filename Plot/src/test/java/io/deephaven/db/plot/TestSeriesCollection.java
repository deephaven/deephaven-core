...
public class TestSeriesCollection extends BaseArrayTestCase {
    ...
    public void testSeriesCollection(){
        ...
        checking(new Expectations() {{
            atLeast(1).of(s1).name(); will(returnValue("S1"));
            atLeast(1).of(s1).id(); will(returnValue(1));
            atLeast(1).of(s2).name(); will(returnValue("S2"));
            atLeast(1).of(s2).id(); will(returnValue(2));
            atLeast(1).of(s3).name(); will(returnValue("S2"));
            atLeast(0).of(s3).id(); will(returnValue(3));
        }});
    ...
    private void assertSeriesDescription(final SeriesCollection.SeriesDescription sd, final SeriesCollection.SeriesType type, final boolean isMultiSeries, final SeriesInternal series){
        assertNotNull(sd);
    ...
    public void testCopy(){
        ...
        checking(new Expectations() {{
            ...
            atLeast(1).of(s1).copy(axes); will(returnValue(s1copy));
            atLeast(1).of(s2).copy(axes); will(returnValue(s2copy));

            atLeast(1).of(s1copy).name(); will(returnValue("S1C"));
            atLeast(1).of(s1copy).id(); will(returnValue(11));
            atLeast(1).of(s2copy).name(); will(returnValue("S2C"));
            atLeast(1).of(s2copy).id(); will(returnValue(22));
        ...
        assertSeriesDescription(copy.getSeriesDescriptions().get("S1C"),SeriesCollection.SeriesType.CATEGORY, false, s1copy);
