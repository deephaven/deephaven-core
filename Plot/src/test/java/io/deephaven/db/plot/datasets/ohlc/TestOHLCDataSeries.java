...
public class TestOHLCDataSeries extends BaseArrayTestCase {
    ...
    private final OHLCDataSeriesInternal dataSeries = new OHLCDataSeriesArray(new BaseFigureImpl().newChart().newAxes(), 1, "Test", dates, open, high, low, close);
    private final OHLCDataSeriesInternal dataSeries2 = new OHLCDataSeriesArray(new BaseFigureImpl().newChart().newAxes(), 1, "Test2", dates, close, high, low, open);
    ...
    private void checkOHLCDataSeriesArray(OHLCDataSeriesInternal dataSeries, DBDateTime[] time, double[] open, double[] high, double[] low, double[] close) {
    ...
    public void testCopy() {
        final OHLCDataSeriesArray ohlc1 = new OHLCDataSeriesArray(new BaseFigureImpl().newChart().newAxes(), 1, "Test", dates, open, high, low, close);
        testCopy(ohlc1, ohlc1.copy(new BaseFigureImpl().newChart().newAxes()));

        final OHLCDataSeriesArray ohlc2 = new OHLCDataSeriesArray(new BaseFigureImpl().newChart().newAxes(), 1, "Test2", dates, close, high, low, open);
