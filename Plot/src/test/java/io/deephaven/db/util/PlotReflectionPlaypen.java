...
public class PlotReflectionPlaypen {
    public static void main(String [] args) {
        ...
        final List<Method> plotMethods = Arrays.stream(methods).filter(m -> m.getName().equals("plot")).collect(Collectors.toList());
        System.out.println(plotMethods.size());

        final List<Method> fourArg = plotMethods.stream().filter(m -> m.getParameterCount() == 4).collect(Collectors.toList());
        System.out.println(fourArg.size());

//        fourArg.forEach(System.out::println);

        final List<Method> table2nd = fourArg.stream().filter(m -> m.getParameterTypes()[1] == Table.class).collect(Collectors.toList());
