...
public class ScatterPlotMatrix extends FigureImpl implements PlotExceptionCause {
     ...
     * going left to right, top to bottom. E.g. for a 2x2 ScatterPlotMatrix,
     ...
     * For example, in a 2x2 ScatterPlotMatrix the coordinates would be
     * [(0,0), (0,1)]
     * [(1,0), (1,1)]
     *
     ...
     * [2, 3]
     *
     * @throws io.deephaven.base.verify.RequirementFailure {@code factors} must not be null.
     *                              The number of {@code factors} must be equal to the number of plots.
    ...
    // these functions return an object which extends figure so that we can add ScatterPlotMatrix specific modifier methods later.
     ...
     * @return new {@link Figure} containing the scatter plot matrix where variable names are assigned as x1, x2, ... in order.
     */
    ...
    public static <T extends Number> ScatterPlotMatrix scatterPlotMatrix(final T[]... variables) {
        ...
        final String[] variableNames = IntStream.range(0, variables.length).mapToObj(i->"x"+i).toArray(String[]::new);
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T extends Number> ScatterPlotMatrix scatterPlotMatrix(final String[] variableNames, final T[]... variables) {
        final IndexableNumericData[] data = Arrays.stream(variables).map(x -> new IndexableNumericDataArrayNumber(x, null)).toArray(IndexableNumericData[]::new);
    ...
    public static ScatterPlotMatrix scatterPlotMatrix(final String[] variableNames, final int[]... variables) {
        ...
        final IndexableNumericData[] data = Arrays.stream(variables).map(x -> new IndexableNumericDataArrayInt(x, null)).toArray(IndexableNumericData[]::new);
        return scatterPlotMatrix(variableNames, data);
    }
    ...
    public static ScatterPlotMatrix scatterPlotMatrix(final String[] variableNames, final long[]... variables) {
        final IndexableNumericData[] data = Arrays.stream(variables).map(x -> new IndexableNumericDataArrayLong(x, null)).toArray(IndexableNumericData[]::new);
        return scatterPlotMatrix(variableNames, data);
    }
    ...
    public static ScatterPlotMatrix scatterPlotMatrix(final String[] variableNames, final float[]... variables) {
        final IndexableNumericData[] data = Arrays.stream(variables).map(x -> new IndexableNumericDataArrayFloat(x, null)).toArray(IndexableNumericData[]::new);
        return scatterPlotMatrix(variableNames, data);
    }
    ...
    public static ScatterPlotMatrix scatterPlotMatrix(final String[] variableNames, final double[]... variables) {
        final IndexableNumericData[] data = Arrays.stream(variables).map(x -> new IndexableNumericDataArrayDouble(x, null)).toArray(IndexableNumericData[]::new);
        return scatterPlotMatrix(variableNames, data);
    }

    private static ScatterPlotMatrix scatterPlotMatrix(final String[] variableNames, final IndexableNumericData[] columns) {
