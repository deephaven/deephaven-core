...
public abstract class AbstractXYDataSeries extends AbstractDataSeries implements XYDataSeriesInternal {
     ...
     */
    public AbstractXYDataSeries(final AxesImpl axes, final int id, final Comparable name, final AbstractXYDataSeries series) {
    ...
    @Override
    public String getPointLabel(int i) { return shapeLabels.get(i);}
    ...
    public AbstractXYDataSeries pointSize(final Table t, final String columnName) {
        ...
        final ColumnHandlerFactory.ColumnHandler columnHandler = ColumnHandlerFactory.newNumericHandler(tableHandle, columnName, getPlotInfo());
        ...
        if (columnHandler.typeClassification().isNumeric()) {
            ...
            throw new PlotUnsupportedOperationException("Column can not be converted into a size: column=" + columnName, this);
        }
    ...
    public AbstractXYDataSeries pointColor(final Table t, final String columnName) {
        ...
        if (columnHandler.typeClassification() == ColumnHandlerFactory.TypeClassification.INTEGER &&
            ...
            throw new PlotUnsupportedOperationException("Column can not be converted into a color: column=" + columnName + "\ttype=" + columnHandler.type(), this);
        }
    }
    ...
    public AbstractXYDataSeries pointColor(final SelectableDataSet sds, final String columnName) {
        ...
        if (!isInt && !isPaint) {
            throw new PlotUnsupportedOperationException("Column can not be converted into a color: column=" + columnName + "\ttype=" + type, this);
        }
    ...
    public AbstractXYDataSeries pointLabel(@NotNull Table t, @NotNull String columnName) {
        final TableHandle tableHandle = new TableHandle(t, columnName);
        addTableHandle(tableHandle);
        final ColumnHandlerFactory.ColumnHandler columnHandler = ColumnHandlerFactory.newObjectHandler(tableHandle, columnName, getPlotInfo());
    ...
    public XYDataSeries pointShape(final Shape... shapes) {
        ArgumentValidations.assertNotNull(shapes, "shapes", getPlotInfo());
        pointShapes.setSpecific(new IndexableDataPointShapeObject(new IndexableDataArray<>(shapes, getPlotInfo())), true);
        return this;
    }
    ...
    public XYDataSeries pointShape(final Table t, final String columnName) {
        ...
        if (String.class.isAssignableFrom(columnType)) {
            ...
            throw new PlotRuntimeException("column is not a supported type (String or Shape): columnName=" + columnName, this);
        }
    }
    ...
    public XYDataSeries pointShape(final SelectableDataSet sds, final String columnName) {
        ...
        if (String.class.isAssignableFrom(columnType)) {
            ...
            final SwappableTable t = sds.getSwappableTable(name(), chart(), columnName);
            addSwappableTable(t);
            pointShapes.setSpecific(new IndexableDataSwappableTablePointShapeObject(t, columnName, getPlotInfo()), true);
            return this;
        } else {
            throw new PlotRuntimeException("column is not a supported type (String or Shape): columnName=" + columnName, this);
        }
    ...
    private static class IndexableDataSwappableTablePointShapeObject extends IndexableDataSwappableTable<Shape> implements Serializable {
        ...
        IndexableDataSwappableTablePointShapeObject(final SwappableTable swappableTable, final String column, PlotInfo plotInfo) {
    ...
    private static class IndexableDataSwappableTableString<T> extends IndexableDataSwappableTable<String> implements Serializable {
    ...
    private static class IndexableDataSwappableTablePaint extends IndexableDataSwappableTable<Paint> implements Serializable {
