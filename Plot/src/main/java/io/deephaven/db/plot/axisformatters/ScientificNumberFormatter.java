...
public class ScientificNumberFormatter extends NumberFormat {
    ...
    private static final double DEFAULT_LOWER_LIMIT = Configuration.getInstance().getDoubleWithDefault("plot.axis.ticks.lowerlimit", 1e-7);
    private static final double DEFAULT_UPPER_LIMIT = Configuration.getInstance().getDoubleWithDefault("plot.axis.ticks.upperlimit", 1e7);
     ...
     * @param upperLimit gives a range around 0 [-upperLimit, upperLimit] for which each number outside the range
     *                   will be formatted with scientific notation
     */
    @SuppressWarnings("WeakerAccess")
    ...
    private StringBuffer formatInternal(final double abs, final StringBuffer toAppendTo, final FieldPosition pos, final long number1, final double number2, final boolean isLong) {
