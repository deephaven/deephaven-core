...
package io.deephaven.db.plot.colors;

import io.deephaven.base.verify.Require;
import io.deephaven.gui.color.Color;
import io.deephaven.gui.color.Paint;
import io.deephaven.db.plot.util.functions.SerializableClosure;
import io.deephaven.numerics.util.Range;
...
public class ColorMaps implements Serializable {
     ...
     * <p>
     * Values less than or equal to {@code min} return the starting color. Values greater than or
     * equal to {@code max} return the ending color. Values in between this range are a linear
     * combination of the RGB components of these two colors. Higher values return colors that are
     * closer to the ending color, and lower values return colors that are closer to the starting
     * color.
     * <p>
     ...
     * @param min minimum
     * @param max maximum
     * @return function for mapping double values to colors. The starting color is blue (#0000FF)
     *         and the ending color is yellow (#FFFF00).
     */
     ...
     */
    public static Function<Double, Color> heatMap(final double min, final double max,
        final Color startColor, final Color endColor, final Color nullColor) {
    ...
    /**
     * Maps {@link Range}s of values to specific colors. Values inside a given {@link Range} return
     * the corresponding {@link Paint}.
     * <p>
     * Values not in any of the specified {@link Range} return an out of range color. Null inputs
     * return a null color.
     *
     * @param map map of {@link Range}s to {@link Paint}s.
     * @param <COLOR> type of {@link Paint} in the map
     * @return function for mapping double values to colors. Null and out of range values return
     *         null.
     */
    public static <COLOR extends Paint> Function<Double, Paint> rangeMap(
        final Map<Range, COLOR> map) {
     ...
     */
    public static <COLOR extends Paint> Function<Double, Paint> rangeMap(
        final Map<Range, COLOR> map, final Color outOfRangeColor) {
     ...
     */
    public static <COLOR extends Paint> Function<Double, Paint> rangeMap(
        final Map<Range, COLOR> map, final Paint outOfRangeColor, final Paint nullColor) {
    ...
    /**
     * Returns a function which uses predicate functions to determine which colors is returned for
     * an input value. For each input value, a map is iterated through until the predicate function
     * (map key) returns true. When the predicate returns true, the associated color (map value) is
     * returned. If no such predicate is found, an out of range color is returned.
     *
     * @param map map from {@link SerializablePredicate} to color
     * @param <COLOR> type of {@link Paint} in {@code map}
     * @return function which returns the color mapped to the first {@link SerializablePredicate}
     ...
     *
     * @param map map from {@link SerializablePredicate} to color
     * @param outOfRangeColor color returned when the input satisfies no
     *        {@link SerializablePredicate} in the {@code map}
     * @param <COLOR> type of {@link Paint} in {@code map}
     * @return function which returns the color mapped to the first {@link SerializablePredicate}
     ...
     */
    public static <COLOR extends Paint> Function<Double, Paint> predicateMap(
        final Map<SerializablePredicate<Double>, COLOR> map, final Paint outOfRangeColor,
        final Paint nullColor) {
        Require.neqNull(map, "map");
    ...
    /**
     * Returns a function which uses closure functions to determine which colors is returned for an
     * input value. For each input value, a map is iterated through until the closure function (map
     * key) returns true. When the closure returns true, the associated color (map value) is
     * returned. If no such closure is found, an out of range color is returned.
     *
     * @param map map from {@link Closure} to color
     * @param <COLOR> type of {@link Paint} in {@code map}
     * @return function which returns the color mapped to the first {@link Closure} for which the
     *         input is true. Out of range, null, and NaN inputs return null.
     */
    public static <COLOR extends Paint> Function<Double, Paint> closureMap(
        final Map<Closure<Boolean>, COLOR> map) {
     ...
     *
     * @param map map from {@link Closure} to color
     * @param outOfRangeColor color returned when the input satisfies no {@link Closure} in the
     *        {@code map}
     * @param <COLOR> type of {@link Paint} in {@code map}
     * @return function which returns the color mapped to the first {@link Closure} for which the
     *         input is true. Null and NaN inputs return null.
     */
    public static <COLOR extends Paint> Function<Double, Paint> closureMap(
        final Map<Closure<Boolean>, COLOR> map, final Color outOfRangeColor) {
     ...
     */
    public static <COLOR extends Paint> Function<Double, Paint> closureMap(
        final Map<Closure<Boolean>, COLOR> map, final Paint outOfRangeColor,
        final Paint nullColor) {
        ...
        for (final Map.Entry<Closure<Boolean>, COLOR> e : map.entrySet()) {
            ...
            final COLOR color = e.getValue();
            final SerializableClosure<Boolean> serializableClosure =
