...
import io.deephaven.db.plot.datasets.data.AssociativeDataSwappableTable;
...
public abstract class AbstractSwappableTableBasedCategoryDataSeries extends AbstractCategoryDataSeries {
    ...
    public AbstractSwappableTableBasedCategoryDataSeries(final AxesImpl axes, final int id, final Comparable name, final AbstractCategoryDataSeries series) {
    ...
    public CategoryDataSeries pointShape(final Function<Comparable, String> shapes) {
        ...
        chart().figure().registerTableMapFunction(getSwappableTable().getTableMapHandle(), constructTableMapFromFunctionOnCategoryCol(shapes, String.class, colName));
        getSwappableTable().getTableMapHandle().addColumn(colName);
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts().getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .shapesSetSpecific(new AssociativeDataSwappableTable<Comparable, Shape, String>(getSwappableTable(), getCategoryCol(), colName, Comparable.class, String.class, getPlotInfo()) {
                        @Override
    ...
    public <NUMBER extends Number> CategoryDataSeries pointSize(final Function<Comparable, NUMBER> factors) {
        ...
        chart().figure().registerTableMapFunction(getSwappableTable().getTableMapHandle(), constructTableMapFromFunctionOnCategoryCol(factors, Number.class, colName));
        getSwappableTable().getTableMapHandle().addColumn(colName);
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts().getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .sizesSetSpecific(new AssociativeDataSwappableTable<>(getSwappableTable(), getCategoryCol(), colName, Comparable.class, Number.class, getPlotInfo()));
            return figImpl;
        }, this));
        return this;
    }
    ...
    public <COLOR extends Paint> CategoryDataSeries pointColor(final Function<Comparable, COLOR> colors) {
        ...
        chart().figure().registerTableMapFunction(getSwappableTable().getTableMapHandle(), constructTableMapFromFunctionOnCategoryCol(colors, Paint.class, colName));
        getSwappableTable().getTableMapHandle().addColumn(colName);
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts().getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .colorsSetSpecific(new AssociativeDataSwappableTable<>(getSwappableTable(), getCategoryCol(), colName, Comparable.class, Paint.class, getPlotInfo()));
            return figImpl;
        }, this));
        return this;
    }
    ...
    public <COLOR extends Integer> CategoryDataSeries pointColorInteger(final Function<Comparable, COLOR> colors) {
        final String colName = ColumnNameConstants.POINT_COLOR + this.hashCode();
        chart().figure().registerTableMapFunction(getSwappableTable().getTableMapHandle(), constructTableMapFromFunctionOnCategoryCol(colors, Integer.class, colName));
        getSwappableTable().getTableMapHandle().addColumn(colName);
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts().getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .colorsSetSpecific(new AssociativeDataSwappableTable<Comparable, Paint, Integer>(getSwappableTable(), getCategoryCol(), colName, Comparable.class, Integer.class, getPlotInfo()) {
                @Override
    ...
    public <LABEL> CategoryDataSeries pointLabel(final Function<Comparable, LABEL> labels) {
        ...
        chart().figure().registerTableMapFunction(getSwappableTable().getTableMapHandle(), constructTableMapFromFunctionOnCategoryCol(labels, Object.class, colName));
        getSwappableTable().getTableMapHandle().addColumn(colName);
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts().getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .labelsSetSpecific(new AssociativeDataSwappableTable<Comparable, String, Object>(getSwappableTable(), getCategoryCol(), colName, Comparable.class, Object.class, getPlotInfo()) {
                        @Override
    ...
    private <S, T> Function<Table, Table> constructTableMapFromFunctionOnCategoryCol(final Function<S, T> function, final Class resultClass, final String columnName) {
        return constructTableMapFromFunction(function, resultClass, getCategoryCol(), columnName);
    }

    protected <S, T> Function<Table, Table> constructTableMapFromFunction(final Function<S, T> function, final Class resultClass, final String onColumn, final String columnName) {
