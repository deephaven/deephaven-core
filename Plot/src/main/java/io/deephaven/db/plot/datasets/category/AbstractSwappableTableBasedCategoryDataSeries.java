...
public abstract class AbstractSwappableTableBasedCategoryDataSeries
    ...
    public AbstractSwappableTableBasedCategoryDataSeries(final AxesImpl axes, final int id,
        final Comparable name, final AbstractCategoryDataSeries series) {
        super(axes, id, name, series);
    }

    public AbstractSwappableTableBasedCategoryDataSeries(final AbstractCategoryDataSeries series,
        final AxesImpl axes) {
        super(series, axes);
    }
        ...
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts()
                .getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .shapesSetSpecific(new AssociativeDataSwappableTable<Comparable, Shape, String>(
                        getSwappableTable(), getCategoryCol(), colName, Comparable.class,
                        String.class, getPlotInfo()) {
                        @Override
    ...
    public <NUMBER extends Number> CategoryDataSeries pointSize(
        ...
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts()
                .getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .sizesSetSpecific(new AssociativeDataSwappableTable<>(getSwappableTable(),
                        getCategoryCol(), colName, Comparable.class, Number.class, getPlotInfo()));
            return figImpl;
        }, this));
        return this;
    }
    ...
    public <COLOR extends Integer> CategoryDataSeries pointColorInteger(
        ...
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts()
                .getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .colorsSetSpecific(
                        new AssociativeDataSwappableTable<Comparable, Paint, Integer>(
                            getSwappableTable(), getCategoryCol(), colName, Comparable.class,
                            Integer.class, getPlotInfo()) {
                            @Override
    ...
    public <LABEL> CategoryDataSeries pointLabel(final Function<Comparable, LABEL> labels) {
        ...
        chart().figure().registerFigureFunction(new FigureImplFunction(figImpl -> {
            ((AbstractSwappableTableBasedCategoryDataSeries) figImpl.getFigure().getCharts()
                .getChart(chart().row(), chart().column()).axes(axes().id()).series(id()))
                    .labelsSetSpecific(
                        new AssociativeDataSwappableTable<Comparable, String, Object>(
                            getSwappableTable(), getCategoryCol(), colName, Comparable.class,
                            Object.class, getPlotInfo()) {
                            @Override
    ...
    private <S, T> Function<Table, Table> constructTableMapFromFunctionOnCategoryCol(
    ...
    protected <S, T> Function<Table, Table> constructTableMapFromFunction(
