...
public abstract class AbstractTableBasedCategoryDataSeries extends AbstractCategoryDataSeries {
    ...
    public AbstractTableBasedCategoryDataSeries(final AxesImpl axes, final int id, final Comparable name, final AbstractCategoryDataSeries series) {
    ...
    public CategoryDataSeries pointShape(final Function<Comparable, String> shapes) {
        ...
        chart().figure().registerTableFunction(getTable(), t -> constructTableFromFunctionOnCategoryCol(t, shapes, String.class, colName));
        chart().figure().registerFigureFunction(new FigureImplFunction(f -> f.pointShape(getTable(), getCategoryCol(), colName), this));
        return this;
    }
    ...
    public <NUMBER extends Number> CategoryDataSeries pointSize(final Function<Comparable, NUMBER> factors) {
        ...
        chart().figure().registerTableFunction(getTable(), t -> constructTableFromFunctionOnCategoryCol(t, factors, Number.class, colName));
        chart().figure().registerFigureFunction(new FigureImplFunction(f -> f.pointSize(getTable(), getCategoryCol(), colName), this));
        return this;
    }
    ...
    public <COLOR extends Paint> CategoryDataSeries pointColor(final Function<Comparable, COLOR> colors) {
        ...
        chart().figure().registerTableFunction(getTable(), t -> constructTableFromFunctionOnCategoryCol(t, colors, Paint.class, colName));
        chart().figure().registerFigureFunction(new FigureImplFunction(f -> f.pointColor(getTable(), getCategoryCol(), colName), this));
        return this;
    }
    ...
    public <COLOR extends Integer> CategoryDataSeries pointColorInteger(final Function<Comparable, COLOR> colors) {
        final String colName = ColumnNameConstants.POINT_COLOR + this.hashCode();
        chart().figure().registerTableFunction(getTable(), t -> constructTableFromFunctionOnCategoryCol(t, colors, Integer.class, colName));
        chart().figure().registerFigureFunction(new FigureImplFunction(f -> f.pointColor(getTable(), getCategoryCol(), colName), this));
        return this;
    }
    ...
    public <T extends Paint> CategoryDataSeries pointColorByY(Function<Double, T> colors) {
        final String colName = ColumnNameConstants.POINT_COLOR + this.hashCode();
        chart().figure().registerTableFunction(getTable(), t -> constructTableFromFunctionOnNumericalCol(t, colors, Paint.class, colName));
        chart().figure().registerFigureFunction(new FigureImplFunction(f -> f.pointColor(getTable(), getCategoryCol(), colName), this));
        return this;
    }
    ...
    public <LABEL> CategoryDataSeries pointLabel(final Function<Comparable, LABEL> labels) {
        ...
        chart().figure().registerTableFunction(getTable(), t -> constructTableFromFunctionOnCategoryCol(t, labels, Object.class, colName));
        chart().figure().registerFigureFunction(new FigureImplFunction(f -> f.pointLabel(getTable(), getCategoryCol(), colName), this));
        return this;
    }

    private <S, T> Table constructTableFromFunctionOnCategoryCol(final Table t, final Function<S, T> function, final Class resultClass, final String columnName) {
        return constructTableFromFunction(t, function, resultClass, getCategoryCol(), columnName);
    }

    private <S, T> Table constructTableFromFunctionOnNumericalCol(final Table t, final Function<S, T> function, final Class resultClass, final String columnName) {
        return constructTableFromFunction(t, function, resultClass, getValueCol(), columnName);
    }

    protected <S, T> Table constructTableFromFunction(final Table t, final Function<S, T> function, final Class resultClass, final String onColumn, final String columnName) {
