...
public class HistogramCalculator {
    ...
    private static Table range(final Table t, final int nbins) {
        return t.by(AggCombo(AggMin("RangeMin=X"), AggMax("RangeMax=X"), AggCount("NSamples")))
                .update("Range = new io.deephaven.db.plot.datasets.histogram.DiscretizedRangeEqual(RangeMin, RangeMax, "
                        + nbins + ")")
                .view("Range");
    }
    ...
    private static Table range(final double rangeMin, final double rangeMax, final int nbins) {
        return TableTools.emptyTable(1)
                .update("Range = new io.deephaven.db.plot.datasets.histogram.DiscretizedRangeEqual(" + rangeMin + ","
                        + rangeMax + "," + nbins + ")");
    }
