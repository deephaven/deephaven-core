...
public class HistogramCalculator {
    ...
    private static Table clean(final Table t, final String histogramColumn, final PlotInfo plotInfo, final String... byColumns) {
    ...
    private static Table counts(final Table data, final Table range, final String... byColumns) {
        ...
        return data.join(range)
                ...
                .updateView("BinMin = Range.binMin(RangeIndex)", "BinMax = Range.binMax(RangeIndex)", "BinMid=0.5*(BinMin+BinMax)");
    }
    ...
    private static Table range(final Table t, final int nbins) {
        return t.by(AggCombo(AggMin("RangeMin=X"), AggMax("RangeMax=X"), AggCount("NSamples")))
                .update("Range = new io.deephaven.db.plot.datasets.histogram.DiscretizedRangeEqual(RangeMin, RangeMax, " + nbins + ")")
                .view("Range");
    }
    ...
    /**
     * Finds the minimum and maximum of the data in the {@code column}
     * of the {@code table} and splits this range into {@code nbins}
     * equally sized bins.
     * Calculates the number of data values in each bin.
     * <p>
     * Data which is not normal as defined in
     * {@link io.deephaven.libs.primitives.DoubleFpPrimitives#isNormal}
     * is filtered out of the data set.
     *
     * @throws io.deephaven.base.verify.RequirementFailure {@code table} and
     ...
     * @throws RuntimeException                           {@code column} must be numeric
     */
    public static Table calc(final Table table, final String column, final int nbins, final PlotInfo plotInfo, final String... byColumns) {
     ...
     * @throws RuntimeException                           {@code column} must be numeric
     */
    public static Table calc(final Table table, final String column, final int nbins, final PlotInfo plotInfo, final List<String> byColumns) {
        return calc(table, column, nbins, plotInfo, byColumns != null && !byColumns.isEmpty() ? byColumns.toArray(new String[byColumns.size()]) : null);
    }

    /**
     * Splits the specified range into {@code nbins} equally sized bins.
     * Calculates the number of data values in each bin.
     * <p>
     * Data which is not normal as defined in
     * {@link io.deephaven.libs.primitives.DoubleFpPrimitives#isNormal}
     ...
     * @return table holding the calculated bins and their counts
     * @throws io.deephaven.base.verify.RequirementFailure {@code table} and
     *                                                    {@code histogramColumn} must not be null
     * @throws RuntimeException                           {@code histogramColumn} must be numeric
     */
    public static Table calc(final Table table, final String histogramColumn, final double rangeMin, final double rangeMax, final int nbins, final PlotInfo plotInfo, final String... byColumns) {
     ...
     * @throws RuntimeException                           {@code histogramColumn} must be numeric
     */
    public static Table calc(final Table table, final String histogramColumn, final double rangeMin, final double rangeMax, final int nbins, final PlotInfo plotInfo, final List<String> byColumns) {
        return calc(table, histogramColumn, rangeMin, rangeMax, nbins, plotInfo, byColumns != null && !byColumns.isEmpty() ? byColumns.toArray(new String[byColumns.size()]) : null);
    }
