...
/**
 * Utility for calculating histogram plot information: bin locations and data frequencies within
 * these bins.
 */
public class HistogramCalculator {
    ...
    private static Table clean(final Table t, final String histogramColumn, final PlotInfo plotInfo,
        ...
        if (ArgumentValidations.isBoxedNumeric(t, histogramColumn, plotInfo)) {
            view[i++] = "X= " + histogramColumn + "==null ? Double.NaN : " + histogramColumn
                + ".doubleValue()";
        } else if (ArgumentValidations.isPrimitiveNumeric(t, histogramColumn, plotInfo)) {
            view[i++] =
                "X= isNull(" + histogramColumn + ") ? Double.NaN : (double) " + histogramColumn;
        } else {
            throw new PlotUnsupportedOperationException(
                "Unsupported histogram histogramColumn: histogramColumn=" + histogramColumn
                    + " type=" + ArgumentValidations.getColumnType(t, histogramColumn, plotInfo),
                plotInfo);
        }
    ...
    private static Table range(final Table t, final int nbins) {
        return t.by(AggCombo(AggMin("RangeMin=X"), AggMax("RangeMax=X"), AggCount("NSamples")))
            .update(
                "Range = new io.deephaven.db.plot.datasets.histogram.DiscretizedRangeEqual(RangeMin, RangeMax, "
                    + nbins + ")")
            .view("Range");
    }
    ...
    private static Table range(final double rangeMin, final double rangeMax, final int nbins) {
        return TableTools.emptyTable(1)
            .update("Range = new io.deephaven.db.plot.datasets.histogram.DiscretizedRangeEqual("
                + rangeMin + "," + rangeMax + "," + nbins + ")");
    }

    /**
     * Finds the minimum and maximum of the data in the {@code column} of the {@code table} and
     * splits this range into {@code nbins} equally sized bins. Calculates the number of data values
     * in each bin.
     * <p>
     * Data which is not normal as defined in
     * {@link io.deephaven.libs.primitives.DoubleFpPrimitives#isNormal} is filtered out of the data
     * set.
     *
     * @throws io.deephaven.base.verify.RequirementFailure {@code table} and {@code column} must not
     *         be null
     ...
     * @param byColumns other columns needed to calaculate histogram, these columns will be included
     *        while grouping
     ...
     * @throws RuntimeException {@code column} must be numeric
     */
    public static Table calc(final Table table, final String column, final int nbins,
        final PlotInfo plotInfo, final String... byColumns) {
     ...
     */
    public static Table calc(final Table table, final String column, final int nbins,
        final PlotInfo plotInfo, final List<String> byColumns) {
        return calc(table, column, nbins, plotInfo,
            byColumns != null && !byColumns.isEmpty()
                ? byColumns.toArray(new String[byColumns.size()])
                : null);
    }

    /**
     * Splits the specified range into {@code nbins} equally sized bins. Calculates the number of
     * data values in each bin.
     * <p>
     * Data which is not normal as defined in
     * {@link io.deephaven.libs.primitives.DoubleFpPrimitives#isNormal} is filtered out of the data
     * set.
     ...
     *        while grouping
     * @return table holding the calculated bins and their counts
     * @throws io.deephaven.base.verify.RequirementFailure {@code table} and {@code histogramColumn}
     *         must not be null
    ...
    public static Table calc(final Table table, final String histogramColumn, final double rangeMin,
        final double rangeMax, final int nbins, final PlotInfo plotInfo,
        final String... byColumns) {
