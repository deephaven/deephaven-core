...
TypeConverter TypeConverter::CreateNew(const std::vector<T> &values) {
  ...
  for (const auto &value : values) {
    ...
    if (valid) {
      OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(builder.Append(*contained_value)));
    } else {
      OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(builder.AppendNull()));
    }
  }
...
inline TypeConverter TypeConverter::CreateNew(const std::vector<deephaven::dhcore::DateTime> &values) {
  ...
  for (const auto &value : values) {
    ...
    const auto *contained_value = TryGetContainedValue(&value, &valid);
    if (valid) {
      OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(builder.Append(contained_value->Nanos())));
    } else {
      OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(builder.AppendNull()));
    }
  }
