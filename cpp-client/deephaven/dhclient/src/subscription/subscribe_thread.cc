...
std::shared_ptr<SubscriptionHandle> SubscribeState::InvokeHelper() {
  ...
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(client->DoExchange(fco, descriptor, &fsw, &fsr)));
  ...
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(fsw->WriteMetadata(std::move(buffer))));
...
void UpdateProcessor::RunForeverHelper() {
  ...
  while (true) {
    OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(fsr_->Next(&flight_stream_chunk)));
...
ColumnSourceAndSize ArrayToColumnSource(const arrow::Array &array) {
  const auto *list_array = VerboseCast<const arrow::ListArray *>(DEEPHAVEN_LOCATION_WITH_EXPR(&array));
  ...
  const auto *list_scalar = VerboseCast<const arrow::ListScalar *>(
      DEEPHAVEN_LOCATION_WITH_EXPR(listElement.get()));
  const auto &list_scalar_value = list_scalar->value;

  ArrayToColumnSourceVisitor v(list_scalar_value);
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(list_scalar_value->Accept(&v)));
