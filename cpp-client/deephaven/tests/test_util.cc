...
void CompareTableHelper(int depth, const std::shared_ptr<arrow::Table> &table,
  ...
  while (element_index < column->length()) {
    if (l_chunk_num >= column->num_chunks() || r_chunk_num >= chunked_data.num_chunks()) {
      throw std::runtime_error(DEEPHAVEN_LOCATION("Logic error"));
    }
    ...
    const auto l_item = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(l_chunk->GetScalar(l_chunk_index)));
    const auto r_item = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(r_chunk->GetScalar(r_chunk_index)));
  ...
  throw std::runtime_error(DEEPHAVEN_LOCATION("Some other difference"));
}
...
std::shared_ptr<arrow::Table> BasicValidate(const deephaven::client::TableHandle &table, int expected_columns) {
  ...
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(fsr->ReadAll(&arrow_table)));
