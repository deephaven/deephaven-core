...
struct ColumnSourceImpls {
  ...
  static void FillChunk(const RowSequence &rows, Chunk *dest, BooleanChunk *optional_null_flags,
    ...
    auto *typed_dest = VerboseCast<ChunkType *>(DEEPHAVEN_LOCATION_WITH_EXPR(dest));
    TrueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(rows.Size() <= typed_dest->Size()));
  ...
  static void FillChunkUnordered(const UInt64Chunk &row_keys, Chunk *dest,
    ...
    auto *typed_dest = VerboseCast<ChunkType *>(DEEPHAVEN_LOCATION_WITH_EXPR(dest));
    TrueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(row_keys.Size() <= typed_dest->Size()));
  ...
  static void FillFromChunk(const Chunk &src, const BooleanChunk *optional_src_null_flags,
    ...
    const auto *typed_src = VerboseCast<const ChunkType *>(DEEPHAVEN_LOCATION_WITH_EXPR(&src));
    TrueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(rows.Size() <= typed_src->Size()));
  ...
  static void FillFromChunkUnordered(const Chunk &src, const BooleanChunk *optional_src_null_flags,
    ...
    const auto *typed_src = VerboseCast<const ChunkType *>(DEEPHAVEN_LOCATION_WITH_EXPR(&src));
    TrueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(row_keys.Size() <= typed_src->Size()));
    TrueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(optional_src_null_flags == nullptr ||
