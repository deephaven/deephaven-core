...
struct ImmerColumnSourceImpls {
  ...
  static void FillChunk(const immer::flex_vector<T> &src_data,
    ...
    auto *typed_dest = VerboseCast<chunkType_t *>(DEEPHAVEN_LOCATION_WITH_EXPR(dest_data));
    ...
    TrueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(optional_dest_null_flags == nullptr ||
        rows.Size() <= optional_dest_null_flags->Size()));
    if (!kTypeIsNumeric) {
      TrueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(src_null_flags != nullptr));
    } else {
