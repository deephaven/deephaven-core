...
arrow::Status Doit(const TableHandleManager &manager, const std::string &csvfn) {
  auto input_file = ValueOrThrow(
      DEEPHAVEN_LOCATION_WITH_EXPR(arrow::io::ReadableFile::Open(csvfn)));
  auto csv_reader = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(
  ...
  auto arrow_table = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(csv_reader->Read()));
  ...
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(
