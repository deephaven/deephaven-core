...
void dumpSymbolColumn(const TableHandle &tableHandle) {
  ...
  while (true) {
    ...
    if (symbolChunk == nullptr) {
      throw std::runtime_error(DEEPHAVEN_LOCATION("Symbol column not found"));
    }
    auto priceChunk = chunk.data->GetColumnByName("Price");
    if (priceChunk == nullptr) {
      throw std::runtime_error(DEEPHAVEN_LOCATION("Price column not found"));
    }
    ...
    if (symbolAsStringArray == nullptr) {
      throw std::runtime_error(DEEPHAVEN_LOCATION("symbolChunk was not an arrow::StringArray"));
    }
    if (priceAsDoubleArray == nullptr) {
      throw std::runtime_error(DEEPHAVEN_LOCATION("priceChunk was not an arrow::DoubleArray"));
    }
