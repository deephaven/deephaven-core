...
void Doit(const TableHandleManager &manager) {
  ...
  // 2. Add "Symbol" column (type: string) to schema
  {
    ...
    OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(symbolMetadata->Set("deephaven:type", "java.lang.String")));
    auto symbolField = std::make_shared<arrow::Field>("Symbol",
        std::make_shared<arrow::StringType>(), true, std::move(symbolMetadata));
    OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(schemaBuilder.AddField(symbolField)));
  }

  // 3. Add "Price" column (type: double) to schema
  {
    auto priceMetadata = std::make_shared<arrow::KeyValueMetadata>();
    OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(priceMetadata->Set("deephaven:type", "double")));
    auto priceField = std::make_shared<arrow::Field>("Price",
        std::make_shared<arrow::DoubleType>(), true, std::move(priceMetadata));
    OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(schemaBuilder.AddField(priceField)));
  }

  // 4. Add "Volume" column (type: int32) to schema
  {
    auto volumeMetadata = std::make_shared<arrow::KeyValueMetadata>();
    OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(volumeMetadata->Set("deephaven:type", "int")));
    auto volumeField = std::make_shared<arrow::Field>("Volume",
        std::make_shared<arrow::Int32Type>(), true, std::move(volumeMetadata));
    OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(schemaBuilder.AddField(volumeField)));
  }

  // 4. Schema is done
  auto schema = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(schemaBuilder.Finish()));
  ...
  if (numRows != prices.size() || numRows != volumes.size()) {
    throw DEEPHAVEN_LOCATION_WITH_EXPR(std::runtime_error(DEEPHAVEN_LOCATION("sizes don't match")));
  }
  ...
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(symbolBuilder.AppendValues(symbols)));
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(priceBuilder.AppendValues(prices)));
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(volumeBuilder.AppendValues(volumes)));

  auto symbolArray = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(symbolBuilder.Finish()));
  auto priceArray = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(priceBuilder.Finish()));
  auto volumeArray = ValueOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(volumeBuilder.Finish()));
  ...
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(wrapper.FlightClient()->DoPut(options, fd, schema, &fsw, &fmr)));
  ...
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(fsw->WriteRecordBatch(*batch)));
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(fsw->DoneWriting()));

  // 14. Read back a metadata message (ignored), then close the Writer
  std::shared_ptr<arrow::Buffer> buf;
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(fmr->ReadMetadata(&buf)));
  OkOrThrow(DEEPHAVEN_LOCATION_WITH_EXPR(fsw->Close()));
