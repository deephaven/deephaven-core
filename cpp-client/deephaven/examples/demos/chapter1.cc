...
void PrintTable(const TableHandle &table, bool null_aware) {
  ...
  while (true) {
    ...
    CheckNotNull(int64_data.get(), DEEPHAVEN_LOCATION("Int64Value column not found"));
    auto double_data = chunk.data->GetColumnByName("DoubleValue");
    CheckNotNull(double_data.get(), DEEPHAVEN_LOCATION("DoubleValue column not found"));

    auto int64_array = std::dynamic_pointer_cast<arrow::Int64Array>(int64_data);
    CheckNotNull(int64_array.get(), DEEPHAVEN_LOCATION("intData was not an arrow::Int64Array"));
    auto double_array = std::dynamic_pointer_cast<arrow::DoubleArray>(double_data);
    CheckNotNull(double_array.get(), DEEPHAVEN_LOCATION("doubleData was not an arrow::DoubleArray"));

    if (int64_array->length() != double_array->length()) {
      throw std::runtime_error(DEEPHAVEN_LOCATION("Lengths differ"));
    }
